...
public class CombatSeederMessageStyleHelperTests
{
    ...
    {
        string message = CombatSeederMessageStyleHelper.BuildDefenseFailure(
    ...
    {
        string natural = CombatSeederMessageStyleHelper.FormatAttackMessage(
            "@ rake|rakes &0's {0} across $1 with @hand precision",
            SeedCombatMessageStyle.Sentences);
        string renderedNatural = string.Format(natural, "claws").Replace("@hand", "left");
        Assert.AreEqual("@ rake|rakes &0's claws across $1 with left precision.", renderedNatural);

        string ranged = string.Format(
    ...
    public void MergeQuestionAnswersWithRecordedChoice_MissingMessagestyle_AddsCombatSeederAnswer()
    {
        ...
        IReadOnlyDictionary<string, string> effectiveAnswers = CombatSeederMessageStyleHelper.MergeQuestionAnswersWithRecordedChoice(context, answers);
    ...
    {
        using FuturemudDatabaseContext context = BuildContext();
        (string Id, string Question, Func<FuturemudDatabaseContext, IReadOnlyDictionary<string, string>, bool> Filter, Func<string, FuturemudDatabaseContext, (bool Success, string error)> Validator) question = NonHumanSeederQuestions.GetQuestions().First(x => x.Id == "messagestyle");
