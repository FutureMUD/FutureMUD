...
public static class SeederAnswerMemory
{
		...
		IReadOnlyDictionary<string, string> currentAnswers)
	{
		...
		{
			var sharedAnswer = GetLatestSharedAnswer(context, question.SharedAnswerKey);
	...
	public static void PersistAnswers(
		...
		DateTime timestamp)
	{
		var questionLookup = questions.ToDictionary(x => x.Id, StringComparer.OrdinalIgnoreCase);
		var persistedSharedKeys = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
