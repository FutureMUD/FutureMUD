...
It creates reusable butchery outputs, raw carcass cuts, hides, glands, trophies and family-level products while keeping the item catalogue intentionally compact. Soft organic outputs spoil into the generic rotten meat item, while durable materials such as bone, tusk, horn, shell, chitin, feather and scale remain stable crafting products.";
	...
	private static IReadOnlyDictionary<string, GameItemProto> EnsureItems(FuturemudDatabaseContext context)
	{
		...
		var components = context.GameItemComponentProtos
			...
			.ToDictionary(
				x => x.Key,
				x => x.OrderByDescending(component => component.RevisionNumber).ThenBy(component => component.Id).First(),
		...
		var existingItems = context.GameItemProtos
			...
			.ToDictionary(x => x.Key, x => x.OrderBy(item => item.Id).First(), StringComparer.OrdinalIgnoreCase);
		var nextId = context.GameItemProtos.Any() ? context.GameItemProtos.Max(x => x.Id) + 1 : 1;
		var now = DateTime.UtcNow;
		var rottenMeat = EnsureRottenMeat(
			context,
			account,
			components,
			tags,
	...
	private static GameItemProto EnsureRottenMeat(
		...
		Account account,
		IReadOnlyDictionary<string, GameItemComponentProto> components,
		...
		DateTime now)
	{
		var rottenMeat = context.GameItemProtos
			.Include(x => x.GameItemProtosTags)
			.Include(x => x.GameItemProtosGameItemComponentProtos)
			.FirstOrDefault(x => x.ShortDescription == RottenMeatShortDescription);
		if (rottenMeat is null)
		{
			rottenMeat = new GameItemProto
			{
				Id = nextId++,
				Name = "meat",
				Keywords = "rotten meat",
				MaterialId = materials["meat"].Id,
				EditableItem = new EditableItem
				{
					RevisionNumber = 0,
					RevisionStatus = 4,
				...
				ShortDescription = RottenMeatShortDescription,
				FullDescription = "The rotten meat is discoloured, slick with decay, and gives off a pungent foul odour.",
				PermitPlayerSkins = false,
				CostInBaseCurrency = 0,
				IsHiddenFromPlayers = false
			};
			context.GameItemProtos.Add(rottenMeat);
		}

		foreach (var componentName in new[] { "Holdable", "Destroyable_Misc" })
		{
			var component = components[componentName];
			if (rottenMeat.GameItemProtosGameItemComponentProtos.All(x =>
				    x.GameItemComponentProtoId != component.Id))
			{
				rottenMeat.GameItemProtosGameItemComponentProtos.Add(new GameItemProtosGameItemComponentProtos
				{
					GameItemProto = rottenMeat,
					GameItemComponent = component,
					GameItemProtoRevision = rottenMeat.RevisionNumber,
					GameItemComponentRevision = component.RevisionNumber
				});
			}
		}

		if (tags.TryGetValue("Debris", out var debrisTag) &&
		    rottenMeat.GameItemProtosTags.All(x => x.TagId != debrisTag.Id))
		{
			rottenMeat.GameItemProtosTags.Add(new GameItemProtosTags
			{
				GameItemProto = rottenMeat,
				Tag = debrisTag,
				GameItemProtoRevisionNumber = rottenMeat.RevisionNumber
			});
		}
		...
		IReadOnlyDictionary<string, IReadOnlyList<ButcheryProducts>> productsByFamily)
	{
		...
		var trait = ResolveButcheryTrait(context) ??
		            throw new InvalidOperationException("Animal butchery requires a Butchery trait or the simple skill package's Survival trait.");
