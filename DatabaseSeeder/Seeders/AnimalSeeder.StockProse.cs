...
public partial class AnimalSeeder
{
	...
	{
		var primaryAdult = GetPrimaryAdultVariant(template.DescriptionPack);
		var secondaryAdult = GetSecondaryAdultVariant(template.DescriptionPack) ?? primaryAdult;
		var summary = string.IsNullOrWhiteSpace(template.Description)
	...
	{
		if (AnimalEthnicityDescriptions.TryGetValue($"{raceName}:{ethnicityName}", out var description))
		{
			return description;
		}

		if (!RaceTemplates.TryGetValue(raceName, out var template))
		{
			return SeederDescriptionHelpers.JoinParagraphs(
