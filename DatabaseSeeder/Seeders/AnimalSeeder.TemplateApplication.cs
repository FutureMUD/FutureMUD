...
public partial class AnimalSeeder
{
    ...
    private bool TryApplyTemplateRaceAttacks(Race race)
    {
        ...
        AnimalAttackLoadoutTemplate loadout = AttackLoadouts[template.AttackLoadoutKey];
        ...
        {
            foreach (AnimalVenomAttackTemplate venomAttack in loadout.VenomAttacks)
            {
    ...
    private void AddVenomAttackToRace(Race race, AnimalVenomAttackTemplate template)
    {
        ...
        AnimalVenomProfileTemplate venomProfile = VenomProfiles[template.VenomProfileKey];
        Drug drug = new()
        {
        ...
        foreach (AnimalVenomEffectTemplate effect in venomProfile.Effects)
        {
        ...
        BodypartShape attackShape = _context.BodypartShapes.First(x => x.Name == template.AttackShapeName);
        WeaponAttack attack = AddAttack(
   ...
   <MinimumWoundSeverity>{template.MinimumWoundSeverity}</MinimumWoundSeverity>
        ...
        List<BodypartProto> bodyparts = template.TargetBodypartAliases.Any()
        ...
        foreach (BodypartProto? bodypart in bodyparts)
        {
            _context.RacesWeaponAttacks.Add(new RacesWeaponAttacks
            {
                Bodypart = bodypart,
        ...
        FutureProg isBabyFemaleRaceProg)
    {
        ...
        {
            foreach (AnimalDescriptionVariant variant in variants)
            {
