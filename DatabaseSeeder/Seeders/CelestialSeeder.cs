...
What epoch date do you want to use?",
    ...
    {
        return answers.TryGetValue(key, out string? value) && value.EqualToAny("yes", "y");
    }
...
{stockDefaultExplanation}",
    ...
    {
        info = null!;
        if (!answers.TryGetValue(calendarAnswerKey, out string? calendarAnswer) ||
            !TryGetCalendar(context, calendarAnswer, out Calendar? calendar) ||
        ...
        int yearIndex = FindYearIndex(parts);
        if (yearIndex == -1)
        {
            return false;
        }
    ...
    internal static string FormatCalendarDate(Calendar calendar, int day, string monthAlias, string year)
    {
        ...
        List<string> parts = SplitDateParts(calendar.Date);
        List<string> separators = SplitDateSeparators(calendar.Date);
        ...
        int yearIndex = FindYearIndex(parts);
        int dayIndex = FindDayIndex(parts, yearIndex);
        int monthIndex = Enumerable.Range(0, 3).First(x => x != yearIndex && x != dayIndex);
    ...
    {
        Celestial? existing = GetEarthSunCandidates(context).FirstOrDefault();
        if (existing is not null)
        {
            return existing;
        }

        Calendar calendar = GetCalendar(context, answers["suncalendar"]);
        long calendarId = calendar.Id;
        Celestial sun = CreateCelestial("Sun", calendar.FeedClockId,
            BuildSunDefinition(
    ...
    private void EnsureEarthMoonPackage(FuturemudDatabaseContext context, IReadOnlyDictionary<string, string> answers, Celestial? createdEarthSun)
    {
        ...
        if (!TryGetEarthSunForMoonPackage(context, createdEarthSun, out Celestial? sun, out string? error))
        {
            throw new InvalidOperationException(error);
        }

        Calendar calendar = GetCalendar(context, answers["mooncalendar"]);
        long calendarId = calendar.Id;
        Celestial moon = CreateCelestial("PlanetaryMoon", calendar.FeedClockId,
            BuildMoonDefinition(
    ...
    private void EnsureGasGiantPackage(FuturemudDatabaseContext context, IReadOnlyDictionary<string, string> answers)
    {
        ...
        Calendar calendar = GetCalendar(context, answers["gasgiantcalendar"]);
        long calendarId = calendar.Id;
        Celestial sun = CreateCelestial("Sun", calendar.FeedClockId,
            BuildSunDefinition(
    ...
    {
        XElement root = new("SunFromPlanetaryMoon",
            new XElement("Name", name),
            Metadata(packageName, role),
            new XElement("Moon", moonId),
            new XElement("Sun", sunId));
        XElement? illumination = string.IsNullOrWhiteSpace(illuminationXml) ? null : XElement.Parse(illuminationXml);
    ...
    private bool TryGetEarthSunForMoonPackage(FuturemudDatabaseContext context, Celestial? createdEarthSun, out Celestial? sun, out string error)
    {
        ...
        List<Celestial> candidates = GetEarthSunCandidates(context).Take(2).ToList();
    ...
    private bool IsEarthSunCelestial(Celestial celestial)
    {
        ...
        XElement? root = TryGetDefinitionRoot(celestial);
    ...
    private IEnumerable<(Celestial Moon, Celestial Planet, Celestial SunView)> GetLegacyEarthMoonTriples(FuturemudDatabaseContext context)
    {
        ...
        List<Celestial> planets = celestials.Where(x => x.CelestialType == "PlanetFromMoon").ToList();
        List<Celestial> sunViews = celestials.Where(x => x.CelestialType == "SunFromPlanetaryMoon").ToList();
        foreach (Celestial? moon in moons)
        {
            string moonId = moon.Id.ToString(System.Globalization.CultureInfo.InvariantCulture);
            Celestial? planet = planets.FirstOrDefault(x =>
            {
                XElement? root = TryGetDefinitionRoot(x);
            ...
            string? sunId = TryGetDefinitionRoot(planet)?.Element("Sun")?.Value;
            Celestial? sunView = sunViews.FirstOrDefault(x =>
            {
                XElement? root = TryGetDefinitionRoot(x);
    ...
    {
        string? value = GetPathValue(root, path);
        return double.TryParse(value, NumberStyles.Float | NumberStyles.AllowThousands, System.Globalization.CultureInfo.InvariantCulture, out double actual) &&
    ...
    {
        return TryGetCalendar(context, answer, out Calendar? calendar)
            ? calendar!
    ...
    private static bool TryGetCalendar(FuturemudDatabaseContext context, string answer, out Calendar? calendar)
    {
        ...
        calendar = context.Calendars.AsEnumerable().FirstOrDefault(x =>
        {
            ...
            try
            {
                XElement definition = XElement.Parse(x.Definition);
