...
public partial class CultureSeeder
{
	...
	internal static IReadOnlyList<string> ValidateHistoricalLanguageCataloguesForTesting()
	{
		...
		ValidateHistoricalLanguagePack(
			...
			RenaissanceEuropeLanguageCoverageMap,
	...
	internal static IReadOnlyDictionary<string, string[]> ExistingPackLanguageCoverageForTesting =>
		ModernLanguageCoverageMap
			.Concat(AntiquityLanguageCoverageMap)
			.Concat(RenaissanceEuropeLanguageCoverageMap)
			.ToDictionary(x => x.Key, x => x.Value, System.StringComparer.OrdinalIgnoreCase);

	internal static IReadOnlyCollection<string> ExistingPackLanguageNamesForTesting =>
		ModernLanguageCoverage
			.Concat(AntiquityLanguageCoverage)
			.Concat(RenaissanceEuropeLanguageCoverage)
			.Select(x => x.Name)
			.ToArray();
}
