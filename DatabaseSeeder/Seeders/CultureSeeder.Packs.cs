...
public partial class CultureSeeder
{
    ...
    public void SeedCulturePacks(FuturemudDatabaseContext context, IReadOnlyDictionary<string, string> questionAnswers)
    {
        ...
        _intelligenceTrait =
            ...
            ;
        foreach (PopulationBloodModel? value in context.PopulationBloodModels.ToList())
        {
            _bloodModels[value.Name] = value;
        }
 ...
 return 20 + (@boosts * 15)"
        ...
        foreach (RacesAdditionalCharacteristics? definition in _humanRace.RacesAdditionalCharacteristics.Concat(
                     ...
                     Enumerable.Empty<RacesAdditionalCharacteristics>()))
        {
            ...
            foreach (CharacteristicProfile? profile in context.CharacteristicProfiles.Where(x =>
        ...
        switch (NormalizeCulturePackAnswer(questionAnswers["culturepacks"]))
        {
            ...
            case "renaissanceworldexpansion":
                SeedRenaissanceWorld(questionAnswers);
                return;
            case "earthantiquity":
            case "antiquity":
    ...
    public void SeedRenaissanceEurope(IReadOnlyDictionary<string, string> questionAnswers)
    {
        ...
        if (questionAnswers["seedlanguages"].EqualToAny("y", "yes"))
        {
            SeedMedievalEuropeLanguages();
        }

        if (questionAnswers["seedheritage"].EqualToAny("y", "yes"))
        {
            SeedMedievalHeritage();
        }
    }
    ...
    public void SeedRepublicanRome(IReadOnlyDictionary<string, string> questionAnswers)
    {
        if (questionAnswers["seednames"].EqualToAny("y", "yes"))
        {
            ...
            SeedAntiquityRegionalNameCultures();
        }
    ...
    public void SeedMiddleEarth(IReadOnlyDictionary<string, string> questionAnswers)
    {
        if (questionAnswers["seednames"].EqualToAny("y", "yes"))
        {
            SeedMiddleEarthNames();
        }

        if (questionAnswers["seedheritage"].EqualToAny("y", "yes"))
        {
            SeedMiddleEarthHeritage();
        }

        if (questionAnswers["seedlanguages"].EqualToAny("y", "yes"))
        {
            SeedMiddleEarthLanguages();
        }
    }
}
