...
public partial class CultureSeeder
{
	...
	{
		var culture = SeederRepeatabilityHelper.EnsureNamedEntity(
			_context.NameCultures,
			name,
			x => x.Name,
			() =>
			{
				var created = new NameCulture();
	...
	private RandomNameProfile EnsureRandomNameProfile(string name, Gender gender, NameCulture culture)
	{
		...
		foreach (var dice in profile.RandomNameProfilesDiceExpressions.ToList())
		{
			_context.RandomNameProfilesDiceExpressions.Remove(dice);
		}

		foreach (var element in profile.RandomNameProfilesElements.ToList())
		{
	...
	{
		var expression = SeederRepeatabilityHelper.EnsureNamedEntity(
	...
	private TraitDefinition EnsureLanguageTrait(string name, FutureProg? canSelectProg)
	{
		...
		var expression = EnsureTraitExpression($"{name} Skill Cap", capFormula);

		var trait = SeederRepeatabilityHelper.EnsureNamedEntity(
			_context.TraitDefinitions,
			name,
			x => x.Name,
			() =>
			{
				var created = new TraitDefinition();
	...
	{
		var trait = EnsureLanguageTrait(name, canSelectProg);
		var language = SeederRepeatabilityHelper.EnsureNamedEntity(
			_context.Languages,
			name,
			x => x.Name,
			() =>
			{
				var created = new Language();
		...
		params string[] languages)
	{
		...
		var script = SeederRepeatabilityHelper.EnsureNamedEntity(
			_context.Scripts,
			name,
			x => x.Name,
			() =>
			{
				var created = new Script();
		...
		foreach (var existing in script.ScriptsDesignedLanguages.ToList())
		{
	...
	private Ethnicity EnsureEthnicity(
		...
		string description)
	{
		var ethnicity = SeederRepeatabilityHelper.EnsureNamedEntity(
			_context.Ethnicities,
			name,
			x => x.Name,
			() =>
			{
				var created = new Ethnicity();
	...
	{
		foreach (var existing in culture.CulturesNameCultures.ToList())
		{
			_context.CulturesNameCultures.Remove(existing);
		}

		foreach (var (gender, cultureName) in mappings)
		{
