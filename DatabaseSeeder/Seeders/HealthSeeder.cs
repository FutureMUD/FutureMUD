...
Please answer #3primitive#F, #3pre-modern#0, or #3modern#F: ", (context, answers) => true,
		...
		private void AddSurgicalProcedurePhase(MudSharp.Models.SurgicalProcedure procedure, int phaseNumber, double seconds, string? special, string emote, IInventoryPlanTemplate? template = null)
		{
			procedure.SurgicalProcedurePhases.Add(new MudSharp.Models.SurgicalProcedurePhase { 
				...
				InventoryActionPlan = template?.SaveToXml().ToString()
			});
		}
		...
		private void SeedModernSurgery()
		{
			...
			AddSurgicalProcedure("Physical", "physical", "Human Medicine", SurgicalProcedureType.DetailedExamination, 0.0, _knowledges["Diagnostic Medicine"].Id, CheckType.TriageCheck, "examining", "@ begin|begins examining $1=0", @"This procedure is used to perform a basic physical examination of the patient. At conclusion, and if successful, you will receive a report of the target's age, general health attributes, and current wounds.", "");
			AddSurgicalProcedurePhase(_procedures["Physical"], 1, 25, null, "@ begin|begins &0's physical examination of $1 by taking &1's blood pressure, listening to &1's breathing, and checking &1's pulse");
			AddSurgicalProcedurePhase(_procedures["Physical"], 2, 25, null, "@ continue|continue's &0's examination of $1 by testing the flexibility of various joints and limbs, and squeezing and prodding various parts");
			AddSurgicalProcedurePhase(_procedures["Physical"], 3, 25, null, "@ step|steps back from $1, having completed the physical examination");

			AddSurgicalProcedure("Stitch Up", "stitchup", "Human Medicine", SurgicalProcedureType.InvasiveProcedureFinalisation, 0.0, _knowledges["Surgery"].Id, CheckType.TriageCheck, "examining", "@ begin|begins stitching up $1", @"This procedure is a surgical suture and tidy-up procedure. It is used after other procedures that make any kind of substantial incision, and effectively finalises the procedure. Without the successful completion of this procedure, the patient is likely to get rather nasty infections on their internal organs.", "");
		...
		private string ProduceInventoryPlanDefinition((InventoryState State, string Tag, int Quantity)[] actions)
		{
