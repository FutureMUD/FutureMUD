...
Please answer #3yes#F or #3no#F: ", (context, answers) => !HasHumanFoundation(context),
    ...
    public string SeedData(FuturemudDatabaseContext context, IReadOnlyDictionary<string, string> questionAnswers)
    {
        ...
        TraitDefinition healthTrait = _context.TraitDefinitions
            .Where(x => x.Type == 1)
            .AsEnumerable()
        ...
        TraitDefinition secondaryTrait = _context.TraitDefinitions
                                 .Where(x => x.Type == 1)
                                 .AsEnumerable()
                                 .FirstOrDefault(x => x.Name.In("Willpower", "Resilience", "Mind")) ??
                             healthTrait;

        TraitDefinition strengthTrait = _context.TraitDefinitions
            .Where(x => x.Type == 1)
            .AsEnumerable()
        ...
        FutureProg liverFunctionProg = new()
        {
...
return (0.01 * (@ch.Weight / 70000)) * @raceFactor * @genderFactor * @meritFactor"
        ...
        FutureProg bloodVolumeProg = new()
        {
...
return @ch.Weight / 20000"
        ...
        FutureProg maximumAliasesProg = new()
        {
        ...
        FutureProg chargenNeedsProg = new()
        {
...
& he has % ears[$ears ears][1:a single $ears ear][0:no ears at all]." +
        ...
        NameCulture nameCulture = SetupNameCultures();
        ...
        Account account = _context.Accounts.First(x => x.AuthorityGroup.AuthorityLevel == (int)PermissionLevel.Founder);

        EntityDescriptionPattern sdescPattern = _context.EntityDescriptionPatterns
            .Where(x => x.Type == 0 && x.ApplicabilityProg.FunctionName == "IsHumanoidNonFemale")
            .GetRandomElement();
        EntityDescriptionPattern fdescPattern = _context.EntityDescriptionPatterns
        ...
        foreach (RacesAdditionalCharacteristics? definition in race.RacesAdditionalCharacteristics.Concat(race.ParentRace
                     .RacesAdditionalCharacteristics))
        {
            CharacteristicDefinition cdef = definition.CharacteristicDefinition;
            CharacteristicValue value = cdef.CharacteristicValues.GetRandomElement();
            while (value == null)
            {
                if (cdef.Parent == null)
                {
                    throw new ApplicationException("Couldn't find a characteristic value.");
                }
        ...
        foreach (RacesAttributes? attribute in race.RacesAttributes.Concat(race.ParentRace.RacesAttributes))
        {
            body.Traits.Add(new Trait
            { Body = body, TraitDefinition = attribute.Attribute, Value = 25, AdditionalValue = 0 });
        }

        Regex dateRegex = new(@"(?<prelude>\d+/\w+/)(?<year>\d+)", RegexOptions.IgnoreCase);
  ...
  <Effect>
   ...
   <CurrentName>0</CurrentName>
    ...
    {
        bool distinctive = _questionAnswers["distinctive"].EqualToAny("yes", "y");

        #region Corpse Models

        CorpseModel organicHumanCorpse = new()
        {
 ...
 "
        ...
        long nextId = _context.BloodtypeAntigens.Select(x => x.Id).AsEnumerable().DefaultIfEmpty(0).Max() + 1;
        BloodtypeAntigen aAntigen = new()
        {
            Id = nextId++,
            Name = "A Antigen"
        };
        _context.BloodtypeAntigens.Add(aAntigen);
        BloodtypeAntigen bAntigen = new()
        {
            Id = nextId++,
            Name = "B Antigen"
        };
        _context.BloodtypeAntigens.Add(bAntigen);
        BloodtypeAntigen rhAntigen = new()
        {
            Id = nextId++,
            Name = "Rh Antigen"
        };
        ...
        Dictionary<string, Bloodtype> bloodTypes = new(StringComparer.OrdinalIgnoreCase);
        nextId = _context.Bloodtypes.Select(x => x.Id).AsEnumerable().DefaultIfEmpty(0).Max() + 1;
        Bloodtype bloodType = new() { Id = nextId++, Name = "O+" };
        ...
        PopulationBloodModel popModel = new() { Id = nextId++, Name = "Overwhelmingly O" };
        ...
        var (blood, sweat) = SeedBloodAndSweat(_context);
        ...
        bool useNonBinary = _questionAnswers["nonbinary"].ToLowerInvariant().In("yes", "y");
        nextId = _context.Races.Select(x => x.Id).AsEnumerable().DefaultIfEmpty(0).Max() + 1;
        Race humanoidRace = new()
        {
            Id = nextId++,
            Name = "Humanoid",
            Description =
        ...
        foreach (TraitDefinition? attribute in _context.TraitDefinitions.Where(x => x.Type == 1 || x.Type == 3))
        {
    ...
    {
        FutureProg humanProg = _context.FutureProgs.First(x => x.FunctionName == "IsHumanoid");
        FutureProg femaleProg = _context.FutureProgs.First(x => x.FunctionName == "IsHumanoidFemale");
        FutureProg nonFemaleProg = _context.FutureProgs.First(x => x.FunctionName == "IsHumanoidNonFemale");
