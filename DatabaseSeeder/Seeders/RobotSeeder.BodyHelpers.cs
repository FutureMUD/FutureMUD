...
public partial class RobotSeeder
{
        ...
        BodypartProto? countAs = null)
    {
        BodypartProto part = new()
        {
			...
			RelativeHitChance = ResolveRobotRelativeHitChance(type, size, relativeHitChance),
			SeveredThreshold = NormalizeRobotSeverThreshold(alias, severedThreshold, size, type),
			SeverFormula = ResolveRobotSeverFormula(alias, size, type),
			MaxLife = ResolveRobotBodypartLife(alias, type, size, maxLife),
