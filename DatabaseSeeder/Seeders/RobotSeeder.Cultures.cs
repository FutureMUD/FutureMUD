...
public partial class RobotSeeder
{
	...
	{
		var simpleNameCulture = _context.NameCultures.First(x => x.Name == "Simple");
		...
		foreach (var (gender, names) in RobotNameProfiles)
		{
	...
	{
		var profileName = $"Robot {gender.DescribeEnum()}";
		var profile = _context.RandomNameProfiles.FirstOrDefault(x => x.Name == profileName);
		if (profile is null)
		{
