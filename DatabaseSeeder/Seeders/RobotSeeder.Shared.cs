...
public partial class RobotSeeder
{
	...
	private void SeedSharedRobotContent()
	{
		...
		_robotPlatingArmour = EnsureArmourClone("Robot Natural Armour", "Human Natural Armour");
		_robotLightPlatingArmour = EnsureArmourClone("Robot Light Armour", "Non-Human Natural Armour");
		_robotInternalArmour = EnsureArmourClone("Robot Internal Armour", "Human Natural Armour");
		...
		var articulatedMaxHp = EnsureTraitExpression("Robot Max HP Formula", $"140+(con:{_healthTrait.Id}*2)");
		var articulatedMaxStun = EnsureTraitExpression("Robot Max Stun Formula",
			$"200+((con:{_healthTrait.Id}+wil:{_secondaryTrait.Id})*2)");
		var utilityMaxHp = EnsureTraitExpression("Robot Utility Max HP Formula", $"90+(con:{_healthTrait.Id})");
	...
	{
		var existing = _context.BodypartShapes.FirstOrDefault(x => x.Name == name);
		if (existing is not null)
		{
			return existing;
		}

		var shape = new BodypartShape { Name = name };
	...
	{
		var existing = _context.Materials.FirstOrDefault(x => x.Name == name);
		if (existing is not null)
		{
			return existing;
		}
	...
	{
		var existing = _context.Liquids.FirstOrDefault(x => x.Name == name);
		if (existing is not null)
		{
			return existing;
		}
