...
public partial class RobotSeeder : IDatabaseSeeder
{
    ...
    public string SeedData(FuturemudDatabaseContext context, IReadOnlyDictionary<string, string> questionAnswers)
    {
        ...
        Dictionary<string, BodyProto> bodyCatalogue = EnsureBodyCatalogue(summary);
        SeedKnowledges();
        SeedRobotCulture();
    ...
    public ShouldSeedResult ShouldSeedData(FuturemudDatabaseContext context)
    {
        ...
        string[] requiredStrategies = new[] { "Robot Articulated Model", "Robot Utility Construct" };
        string[] requiredKnowledges = RobotKnowledgeTemplates.Select(x => x.Name).ToArray();
        string[] requiredProcedures = new[]
        {
        ...
        HashSet<string> availableBodyNames = context.BodyProtos.Select(x => x.Name).ToHashSet(StringComparer.OrdinalIgnoreCase);
        List<RobotRaceTemplate> installableTemplates = Templates.Values
    ...
    {
        string[] requiredBodies = new[] { "Humanoid", "Toed Quadruped", "Insectoid", "Arachnid" };
        string[] requiredRaces = new[] { "Human", "Humanoid" };
        ...
        string[] requiredProgs = new[] { "AlwaysTrue", "AlwaysFalse" };
        bool hasBodies = requiredBodies.All(body => context.BodyProtos.Any(x => x.Name == body));
        bool hasRaces = requiredRaces.All(race => context.Races.Any(x => x.Name == race));
        bool hasRequiredProfiles = requiredProfiles.All(profile =>
                   context.CharacteristicProfiles.Any(x => x.Name == profile) ||
                   context.CharacteristicDefinitions.Any(x => x.Name == profile));
        bool hasProgs = requiredProgs.All(prog => context.FutureProgs.Any(x => x.FunctionName == prog));
        bool hasTools = RequiredToolTags.All(tag => context.Tags.Any(x => x.Name == tag));
        bool hasAttacks = RequiredPrerequisiteAttackNames.All(attack => context.WeaponAttacks.Any(x => x.Name == attack));
