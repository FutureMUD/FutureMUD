...
public abstract class SkillSeederBase : IDatabaseSeeder
{
		...
		bool branching)
	{
			...
			IEnumerable<(Difficulty Difficulty, int Modifier)> modifiers)
		{
			...
			foreach (var existing in template.CheckTemplateDifficulties.ToList())
			{
				context.CheckTemplateDifficulties.Remove(existing);
			}

			foreach (var (difficulty, modifier) in modifiers)
			{
			...
			string skillGainModel)
	{
		...
		var professional = EnsureDecorator(
			context,
			"Professional Skill",
			"Range",
		...
		var languageDecorator = EnsureDecorator(
			context,
			"Language Skill",
			"Range",
	...
	protected static TraitExpression EnsureTraitExpression(
		...
		string expression)
	{
		var traitExpression = SeederRepeatabilityHelper.EnsureNamedEntity(
	...
	{
		var traitExpression = EnsureTraitExpression(context, expressionName, expression);
		context.SaveChanges();

		var check = context.Checks.FirstOrDefault(x => x.Type == (int)type);
		if (check is null)
		{
	...
	{
		var helpfile = SeederRepeatabilityHelper.EnsureNamedEntity(
			context.Helpfiles,
			name,
			x => x.Name,
			() =>
			{
				var created = new Helpfile();
	...
	private static TraitDecorator EnsureDecorator(
		...
		string contents)
	{
		var decorator = SeederRepeatabilityHelper.EnsureNamedEntity(
			context.TraitDecorators,
			name,
			x => x.Name,
			() =>
			{
				var created = new TraitDecorator();
