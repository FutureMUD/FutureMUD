...
definition);
		...
		void CreateContainer(string name, string description, double weight, SizeCategory maxSize, bool closable, bool transparent, string preposition)
		{
			CreateItemProto(nextId++, now, "Container", name, description, $"<Definition Weight=\"{weight}\" MaxSize=\"{(int)maxSize}\" Preposition=\"{preposition}\" Closable=\"{closable}\" Transparent=\"{transparent}\" />");
		}
