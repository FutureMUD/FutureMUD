...
For receiver-side mechanics, `emitnoise(source, volume, propagation, mode, type, echo)` uses an explicit finite propagation budget with either `topological` or coordinate-aware route costs. Each eligible character reached through the cheapest valid route receives `CharacterNoiseReceived` once with `(listener, origin, source, received volume, proximity, type, initial direction, echo)`; the direction identifies the listener's first step toward the source, including the local longitudinal direction when the listener is inside a RouteCell. Reception means the sound reached the listener's exact spatial location, not that they noticed it. A receiver prog calls `canhearnoise(listener, source, received volume, proximity)` to apply the shared native hearing-profile and listen-check rules. This structured path remains separate from player-facing `AudioOutput` presentation.
