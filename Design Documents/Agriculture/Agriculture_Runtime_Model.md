...
## Environmental resource inputs

Environmental magic profiles can read field presence, crop and woodland presence, crop and woodland health and yield potential, pasture biomass, and field condition. Native scores retain their 0-100 units; each environmental binding supplies its own explicit scale. Field absence is distinct from a present field whose score is zero. These reads do not consume agricultural yields or advance the daily tick.

The environmental coordinator maintains the field-by-cell lookup through world field creation, replacement, deletion and load. Changes to pasture or condition notify it directly. Daily ticks and field operations compare their native environmental values before and after the operation and send one coalesced invalidation when those values change. Crop and woodland yield consumption also invalidates the owning cell. An unchanged value or an unrelated field score does not invalidate native environmental inputs. Field profile default-score edits request bounded source-definition reconciliation; arbitrary policy-prog dependencies still require the explicit dirty helper or periodic reconciliation.

Forage-backed environmental inputs use `ICell.TryPeekForagableYield`, which projects the effective profile without synchronising stored pools, subscribing a yield heartbeat, or enqueueing a save. A newly introduced yield key projects its profile maximum; an existing depleted key retains its stored amount, clamped to the current maximum. A missing configured key is an input error. After approval replaces a profile revision, cell, zone and terrain bindings resolve the current revision; pure previews project its values while leaving the stored pools untouched. Owning forage load/configuration/mutation paths perform the ordinary synchronisation and notify the environmental coordinator when the relevant source changes. Changing a terrain or zone forage default requests bounded source-definition reconciliation without scanning its cells in the setter; assigning the same default again does not notify. Environmental reads do not alter forage recovery rates.

## Native organic accounting and ecological production

An environmental profile may explicitly authorise the current crop/orchard, woodland or pasture native stock for a later Land gathering method. Authorisation remains on the environmental definition; source-unit accounting lives in the field's existing versioned definition XML and is saved with the corresponding integer native stock.

Crop and orchard share one generation because both use the same native crop row. Crop, woodland and pasture each retain a monotonic generation counter, a prepaid decimal source fraction, and their applicable positive-production progress remainders. A legacy live source receives a stable initial generation. Annual harvest/removal/death/replacement clears predecessor credit and progress but retains the counter so a successor cannot reuse its identity. A retained orchard harvest keeps its generation. Leaving and later re-establishing pasture creates a new pasture generation. Field deletion/recreation changes the owner ID; cell scars are unaffected.

Fractional conversion of an integer source conservatively prepays whole native units. Given integer stock `Q`, saved credit `C` and logical debit `d`, the field calculates `max(0, ceiling(d-C))`, requires that many whole units, subtracts them through the same native mutation path ordinary consumers observe, and saves `C + wholeDebit - d`. Ordinary crafts, harvest and grazing see only `Q`; all later callers share `C`.

Each positive crop-health, crop-yield, woodland-health, woodland-yield or pasture-biomass contribution is multiplied once by its current configured ecological factor. Whole points are realized and a separate decimal production remainder is retained. Same-operation losses create headroom for the positive contribution before the final native clamp: a full retained orchard with a +5 work bonus and a -20 harvest cost closes at 85. Crop tick negative nutrient/pollination contributions follow the same final native clamp. Negative stress, grazing, nutrient use, harvest costs, output weights and maturity/overripe calendars remain unchanged. Remainders are discarded when no same-operation contribution can fit at a full limit, on an invalid factor, absent/dead source or lifecycle replacement, and do not accrue during downtime.

Sowing, orchard planting, woodland establishment and pasture establishment apply their matching initial factor to productive values assigned by the operation. A new fallow field records its staged pasture default as pending in the field definition XML. Its first productive pasture entry assesses the staged default plus positive establishment allocation once, including herd-entry and drive transitions. The marker and result save with the native stock; a later re-entry does not refill or reassess retained biomass. Legacy definitions without the marker preserve existing stock as already assessed. Later environmental scar repair changes subsequent factors only; it does not restore current field stock, resurrect a failed crop, or recreate commodities. See [Native Organic Yields and Ecological Penalties](../Magic/Native_Organic_Yields_and_Ecological_Penalties.md) for the exact public owner/service contracts and Task 3B save boundary.

Destructive Land gathering can charge a configured crop or woodland health price after its native source debits. The field checks the living lifecycle and current health, applies at most the quoted integer loss, and clears the vegetation and its prepaid fraction if health reaches zero. The gathering parent saves the field before any body price or personal credit; a failed save restores dirty state for a later ordinary save and quarantines the parent receipt. This health loss is separate from the yield-unit debit and cannot resurrect or transfer an old prepaid fraction. See [Magic Gathering](../Magic/Magic_Gathering.md) for method authoring, receipt stages and player use.
