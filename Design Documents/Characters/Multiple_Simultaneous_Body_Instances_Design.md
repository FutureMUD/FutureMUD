...
| Project labour queue | `ProjectLabourQueues.CharacterId` plus nullable `ClaimingCharacterInstanceId` | Identity-owned schedule, physical instance claim | The scheduler stores start/join entries, completion state, and the physical claimant; only one instance may claim an entry at a time | Existing queues remain identity-wide and migrate as one-shot joins | Removing a claiming instance clears only the claim, preserving elapsed duration and phase goals for a later eligible instance | Any focused body may activate an unclaimed entry; live labour remains instance-owned and a second body cannot double-claim the same queue entry |
