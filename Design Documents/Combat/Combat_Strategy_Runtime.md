...
Weapon attacks, natural attacks, and auxiliary actions with `MaximumTargets` greater than one are wrapped by `MultiTargetCombatMove`. Ordinary melee secondaries must be hostile combatants currently targeting the assailant at melee range; ranged-natural secondaries may ignore melee spacing but must be visible and within the authored range. Each victim receives an independent defense and effect resolution while the attacker pays stamina and recovery once. `PullToMelee` and `PullToMeleeUnarmed` are the intentional spacing exception for colocated hostile targets and are considered by ranged strategies when closing contact. A successful pull hit must also win an opposed `ForcedMovementCheck` versus `OpposeForcedMovementCheck` at the attack's resist difficulty before it establishes melee range.
...
Inside a RouteCell, successful pushback also displaces the target's coordinate away from the attacker by `RouteCellPushbackMetresPerSuccessDegree` (default `1m`) for each success degree, with a minimum of one degree and endpoint clamping. An exact-coordinate overlap resolves toward the farther endpoint, with the positive endpoint winning a midpoint tie. A surface-water vehicle's existing stability result takes precedence: a successful stability check absorbs coordinate displacement, while a fall-overboard result follows the established disembark path. Ordinary-cell pushback remains unchanged.
