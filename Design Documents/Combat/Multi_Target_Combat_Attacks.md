...
Ranged-natural attacks may select visible hostile combatants despite melee spacing. Each additional victim must still be within the attack's authored `RangeInRooms`. `ScreechAttack`, `BreathWeaponAttack`, `ExplosiveNaturalAttack`, and `AquaticVehicleAttack` retain their built-in area/payload resolution and are not wrapped again, preventing multiplied area effects.

Conventional ammunition-based `IRangedWeapon` fire remains outside the `MaximumTargets` field because it does not use `WeaponAttack` definitions. Modern firearm volleys and shotgun loads nevertheless support multiple impacts through the ammunition pipeline: each consumed cartridge creates its authored projectile count, and each projectile independently resolves cover, obstruction, damage, lodging, and scatter. Unlike authored multi-target weapon attacks, collateral scatter is not limited to hostile combatants; allies and bystanders are eligible and legal consequences are checked per collateral impact. See [Modern Firearms, Attachments, and Alternate Fire Modes](./Modern_Firearms_and_Attachments.md).
