...
From Engine 2.0.0, `DEBUG UPDATE` downloads the no-store latest redirect for the host runtime (`/downloads/engine/latest/win-x64`, `/downloads/engine/latest/linux-x64`, or `/downloads/engine/latest/linux-arm64`) and fails closed on unsupported platforms. The command verifies the response SHA-256, rejects archive entries that escape its configured binary directory, and stages the release for the restart script to apply. It uses `AppContext.BaseDirectory`, which remains valid for single-file applications.

The `EngineUpdateBinariesPath` static configuration selects that staging directory. Its default is `Binaries`, resolved relative to `AppContext.BaseDirectory`, which preserves the generated-launcher layout. Set it with `EDITSTATICCONFIG EngineUpdateBinariesPath` to another relative directory or an absolute path only when the restart script has been customised to apply updates from the same location.

The DatabaseSeeder writes a versioned `Start-MUD.bat` or `Start-MUD.sh` in that installation directory. Both launchers apply a staged `Binaries` payload before starting the engine, honour `BOOTING` and `STOP-REBOOTING`, attempt the engine at most 100 times with a five-second delay between unexpected exits, and return a failing exit code for boot failures, missing engines, failed updates, or exhausted retries. The Linux launcher resolves its own directory and is marked executable; the Windows launcher quotes all install paths. Seeder-owned launchers are upgraded when their version marker or the recognised legacy template is present. A script without either marker is treated as customised and preserved; remove or rename it before rerunning the seeder to regenerate it.
