...
TrapEffect is a saving effect on an ICell, IGameItem, or character/NPC anchor. It pins template ID and revision, creator ID, state, charges, a stable instance GUID, and the IDs/roles/recovery settings of matched component items. Exit traps are always owned by the origin cell and persist the exit ID and origin-side cell ID separately from any door or component. Permanent exits continue to match by their stable positive ID. Transient exits additionally implement `ITransientExit` and expose a namespaced stable key: spell-portal instance GUIDs, portal-network/link IDs, vehicle/compartment-link IDs, and docking IDs supply deterministic logical identities. A transient trap persists that key in its effect XML and matches it centrally through `ITrap.MatchesExit`. This makes `north` and other exit keywords first-class bindings, permits distinct traps on several exits in one cell, and prevents traversing the opposite side from firing the wrong trap. Installed components receive a non-saving no-get reservation recreated from the trap on load; removal, safe dismantling, or administrative deletion releases it. A template revision is never retrospectively changed for an installed trap.

Cells are constructed before trap templates, item prototypes, and world item instances because item groups themselves depend on the loaded world. Persisted cell-owned trap effects therefore parse during cell construction but defer dependency-sensitive runtime initialisation. After world items load, a dedicated startup pass performs interrupted-resolution recovery, signal/proximity subscriptions, and component reservations. Vehicles and portal topology can then recreate transient exits. Registration with the same stable key and the same two endpoint cells atomically rebinds an installed trap to the replacement runtime exit ID; a reboot or topology refresh therefore does not consume the trap. A changed endpoint is removal of the old logical exit followed by creation of a new one. Once vehicle and portal topology loading is complete, a final reconciliation binds saved keys or destructively removes unresolved and legacy negative-ID bindings that have no recoverable key. Item- and character-owned traps load after their dependencies and retain the ordinary effect lifecycle. This avoids changing the established global load order or delaying unrelated cell effects.

True transient-exit removal includes spell-portal expiry, portal-network link deactivation or deletion, docking closure, and endpoint-cell changes. It cancels that trap's delayed payload, reset, and spent-cleanup effects; releases reservations; removes the trap; and deletes each distinct surviving installed component instead of returning items to the room. Consequences from payloads that already resolved are not rolled back. This destructive rule is intentional: installing equipment on a vanishing portal accepts the risk that the equipment vanishes with it.
...
Mechanical templates require positive setup, disarm (when disarmable), and recovery times before review. `trap lay`, `trap disarm`, and `trap recover` use cancellable general/movement actions for non-administrators and revalidate the anchor and trap state at completion. With no `using` clauses, `trap lay` first matches usable held or wielded parts, then loose parts at the actor's current layer; explicit clauses constrain that choice. Before the trap is installed, one inventory plan validates and moves the distinct union of matched components and any held item anchor to the current location using normal drop rules, then captures the components' installed spatial positions. This includes persistence, reservation, custody/location, drop feasibility, and the ordinary `CanManipulateItem` access decision for every selected physical item. A held anchor that also satisfies a component role is moved only once. Administrative operations remain immediate. Magical and natural deployment continue to use their spell, AI, or FutureProg action timing rather than the mundane setup timer.
...
`arm item` first requires the ordinary `CanManipulateItem` permission. It recognises only a trap the actor already knows and permits player re-arming only from `Unarmed` or `Disarmed`; resolving, cooling-down, spent, and expired traps cannot bypass their lifecycle through the generic command. An unknown trap is not disclosed and the command continues through the ordinary armable-explosive path. `disarm item` directs players to the safer `trap disarm` workflow. Administrative `trap arm` remains the explicit force-control command.
...
TrapTemplates has a composite Id/RevisionNumber key, an EditableItem foreign key, a name, and XML definition. Deployed traps, knowledge, cooldown, delayed payload, gas cloud, and restraint data are stored in existing effect XML. Transient trap binding keys and spell-portal instance GUIDs extend that existing XML; no relational schema change is required. Migration AddTrapTemplates creates the definition table and updates EF snapshot.
...
| Broad fast test suite | Complete | All 3,557 tests passed after the transient-exit lifecycle follow-up on 15 August 2026: 429 library, 21 expression engine, 687 seeder, 2,271 core, 33 database library, 22 Discord, 43 converter, and 51 web. |
