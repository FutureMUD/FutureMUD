...
| Seeder and stock examples | Seeded FutureProgs and stock craft content | `DatabaseSeeder/Seeders/ItemSeeder/ItemSeeder.Crafting.cs` |
...
| `progcookedfood` | `ProgCookedFoodProduct` | Let a FutureProg select one prepared-food prototype (or a collection for custom workflows), then initialise its ingredient ledger | `prog`, `purify`, `ingredient` | Uses `ProgProduct`'s item/liquid input dictionaries but creates selected prototypes through `CookedFoodProduct`; catalogue selectors return one shuffled group member per craft |
| `liquid` | `LiquidProduct` | Load one or more liquid-container item prototypes and fill each with a concrete liquid | `item`, `skin`, `quantity`, `liquid`, `volume` | Fails validation if the prototype is not a liquid container, if the liquid is missing, or if the configured volume cannot fit |
| `variable` | `SimpleVariableProduct` | Load items with characteristics inherited from `IVariableInput` sources and/or selected exact values | `item`, `skin`, `quantity`, `variable <definition> <input#>`, `variable <definition> value <value>` | One rule per definition; selected values must belong to it and mapped inputs must supply it |
...
`SimpleVariableProduct` retains the existing zero-based `<Variable inputindex="0">definitionId</Variable>` XML and adds optional `<FixedVariable value="valueId">definitionId</FixedVariable>` siblings for selected values. Existing input-only products need no migration. Mixed rules preserve exact skin selection, quantity and ordinary craft-quality calculation for success and failure products. Duplicate definitions, missing definitions/values and incapable inputs invalidate the product; unresolved consumed values fail before any output is created. Invalid configured references remain invalid across save/reload instead of being silently discarded. Item characteristic-channel and palette compatibility remains a content/preflight obligation, not an implicit consequence of a valid value ID.
...
### Seeder content
`DatabaseSeeder/Seeders/ItemSeeder/ItemSeeder.Crafting.cs` seeds:
