...
`ProgCookedFoodProduct` extends that contract for generalized recipes. Its FutureProg receives the same consumed item and liquid dictionaries as `ProgProduct`, returns one or more prepared-food items, and uses each returned prototype as the target for the normal cooked-food ledger transfer. The pre-industrial catalogue uses this product to group rows by scope, kind, family, and source material, reducing craft count without losing a craftable path for any prepared-food row. The selector may return a scalar item for ordinary household work or a collection when one craft deliberately creates several prepared servings; each selected result receives the same ingredient, liquid, and transferable-effect handling.
