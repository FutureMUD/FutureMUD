# Medieval ItemSeeder Reset

The medieval `ItemSeeder.Rework` item and craft implementation has been intentionally reset to launch stubs for a from-scratch rebuild.
...
- Each `ItemSeeder.Rework.Medieval*.cs` category file is currently a no-op launch point only.
...
The retired medieval catalogue and suite documents were removed with the implementation they described. A future medieval rebuild should add new design documents only when the new source of truth exists or when an accepted implementation plan needs a durable specification.
...
- update this reset note or replace it with the new current-state document when the rebuild begins.
