...
- `CoreDataSeeder` now owns the stock terrain catalogue and terrain-tag taxonomy so terrain-aware stock packages can rely on those records without an extra prompt.
...
| --- | ---: | --- | --- | --- | --- | --- | --- | --- | --- |
| `CoreDataSeeder` | 0 | No prerequisite beyond database itself | `MayAlreadyBeInstalled` if any account exists | None | Seeds canonical bootstrap records plus stock terrain foundations and terrain tags | None | `OneShot` / `None` until separate design | High | Keep terrain foundations in core ownership and create a broader canonical-record plan before expanding repeatability claims |
