...
Current users of this shared reserve/ledger path include auction houses, stables, hospitals, hotel rentals, property-owner revenues, clan virtual treasuries, legal-authority fine and bail revenue, economic-zone retained revenue, estate fallback liquidation, active-project payment reserves, and job coffer/audit movements. Arenas keep their existing `VirtualBalance` column but now write virtual-cash ledger rows for arena cash and bank settlement movements.

Active-project payment reserves are funded and withdrawn through physical character cash, but stored as virtual reserve balances while the project is live. Labour payments create persisted project payables that a worker later claims as cash or deposits into an owned bank account. Material contribution payments draw immediately from the same active-project reserve. Paid project work is pay-before-progress: an unfunded paid labour tick is removed before progress is applied, and an unfunded paid material supply is rejected before the item is consumed. Lowering or removing a labour payment rate while workers are active requires an `Accept` confirmation and removes those workers before the new rate applies. Unspent reserve cash on project deletion is returned to the project owner as a claimable project payable rather than disappearing with the active project.
...
Patients use `hospital services`, `hospital service <#|name>`, `hospital request <service> [for <target>] [cash|debt|with <payment item>]`, `hospital cancel [<#>]`, `hospital debt [person]`, and `hospital debt pay <amount> [for <target>] [cash|with <payment item>]`. Hospital waiting rooms participate in the central LOOK addendum block and advertise `HOSPITAL SERVICES` when ambient service-location hints are enabled. Hospital managers and proprietors can also use `hospital operations` to review room, theatre, active procedure, staff action, blocker, and reserved-resource status in wrapped multi-line blocks rather than wide operational tables, plus `hospital cash`, `hospital deposit <amount>`, `hospital withdraw <amount>`, and `hospital ledger [count]` to review, fund, withdraw from, and audit the hospital virtual cash balance; `hospital ledger` also uses bounded multi-line ledger blocks. Withdrawals deliberately draw only from virtual cash, not the linked bank account. Patient service lists show a compact table with simple availability, price or donor-payout semantics, and debt permission; stocked-material, unfunded donor-payout, and no-task-capable-medical-NPC failures appear as currently unavailable before any payment is taken, while managers and administrators retain detailed service availability diagnostics. A service request can target the requester or another visible patient. Conscious third-party patients receive a consent proposal; unconscious or otherwise helpless patients are presumed to consent for emergency treatment. The requester, patient, managers, proprietors, and administrators can cancel an active request; cancellation cancels the linked employment task and aborts any staged hospital surgery effect, but amounts already paid or charged remain owed and no refund is issued for completed or partially completed work. Payment can be immediate cash, a bank-payment item, waived by authorised hospital staff, or medical debt when the service allows debt and the patient's account limit permits the charge. Debt payments can also create prepaid hospital credit for later account-funded services. Managers, proprietors, and administrators can list all owing debt accounts and forgive part or all of an outstanding hospital debt without creating prepaid credit. Stabilisation and full treatment are shown as usage-billed services rather than fixed-price services and charge the patient's hospital debt account from the actual component services performed unless the request is waived. Blood donation is always non-chargeable: its service price is the default flat donor payout, while a matching per-blood-type stock policy replaces that default with its target-aware per-litre price.

Availability checks count both supply-area stock and equipment already staged in operating theatres, so moving a reusable IV drip or another requirement into its treatment room does not make the service appear unstocked. Blood-donation availability also applies the executor's donor-physiology and safe-minimum-blood checks before creating a task, rather than escorting an ineligible donor into theatre and failing there. A hospital accepts only one active request per patient at a time; the existing request must complete or be cancelled before another can be created, preventing concurrent tasks from moving the same patient between reserved theatres.

Hospital requests accept `target <bodypart>` as the bodypart-focused form of procedure parameters. Blood services validate that target when one is supplied; otherwise routine IV access is restricted to peripheral limb bodyparts. Automatic surgical target selection filters out bodyparts forbidden by the procedure and prefers exposed valid sites. Before a procedure whose phase metadata requires an exposed target begins, the medical worker removes only the worn items needed to expose that site. Those patient belongings remain associated with the request: one item is retained directly, multiple items are combined into the standard temporary item bundle, and the retained item or bundle is delivered to the recovery or waiting room when the patient leaves the theatre after completion or abandonment.

A successful request creates a hospital active employment task. Theatre-required services always insert a `HospitalPatientPreparationActionStep`, so a doctor first reserves a treatment theatre and moves the patient and medical worker into it before supply preparation can block clinical progress. If preparatory supply work is still needed, the task then requeues for `HospitalSupplyPreparationActionStep`, allowing an available orderly to collect configured equipment or implicit `ITreatment` stock while the patient is already in the theatre, and requeues back to a medical worker for `HospitalServiceActionStep` execution. Hospital manager goals can also maintain procedure supplies: `hospitalconsumables` purchases consumable deficits for a configured number of active-service procedure repeats, while `hospitaltools` maintains reusable tool counts and treats supply-room stock, theatre-held tools, and tools held or wielded by active medical staff as available. Theatre-staging goals `hospitaltheatreconsumables` and `hospitaltheatretools`, backed by `hospitaltheatre consumables|tools <procedure-count>`, move existing matching supply-room stock into each configured operating theatre for active-service equipment and implicit wound-care treatment supplies; they generate collection and delivery tasks and defer without blocking when the only problem is temporarily missing supply-room stock. Supply workers need `PrepareMedicalSupplies` and `CanPrepareHospitalSupplies`; doctors with medical and supply authority are fallback candidates only when no active, reachable, unassigned non-medical supply worker can take collection work. Actual supply collection and delivery still require supply authority, but a medical worker can acknowledge already prepared or already staged theatre supplies so the task can advance. The supply step collects the configured items from one hospital supply room and delivers them to the reserved treatment theatre through the task-item custody APIs. Command-routed bedside hospital services without explicit equipment insert the same supply-preparation step only when useful `ITreatment` stock exists and the reserved theatre does not already contain the needed staged supplies. In-progress supply custody is hydrated for the current task step as well as completed earlier steps, and carried transport bundles are tracked by their held-or-wielded wrapper while their contents remain visible to supply matching. Carried bundle contents and relevant supplies already staged in the reserved theatre count as held or prepared task supplies, so the worker does not keep returning to the supply room for items already in the bundle or theatre. The supply step can complete from staged theatre stock after a manual bundle empty or previous delivery attempt. Loose stock in a supply room is not treated as available to the medical step until it has been collected or delivered to the treatment location.

The medical step needs `PerformMedicalServices` and `CanPerformMedicalServices`. A service is not advertised as currently available unless at least one active, able, unassigned medical NPC employee has a task-enabled `EmploymentWorkerAI` with the `CanPerformMedicalServices` capability for that hospital. Theatre-preferring services reserve an empty operating theatre; full-treatment and stabilisation services also force theatre use even if an older service row was saved without the theatre preference flag. Empty means no other active hospital request reserves it and no unrelated non-staff occupant is present. The availability check ignores the same request by request id or linked employment task id, and it treats the request patient as related by character identity so the patient cannot block their own procedure. For theatre-required requests that have a patient-preparation step, the assigned medical worker first paths to the patient and that step transfers both patient and worker into the reserved theatre with source-room, destination-room, and direct patient echoes; otherwise the service step performs the same transfer immediately before treatment. Supply and service execution fail the request gracefully if the patient disappears, dies, enters stasis, leaves configured hospital locations, leaves the reserved theatre after preparation, or becomes unseen by the assigned worker while colocated. If a non-theatre service intentionally treats in place, the worker emits an in-room emote before beginning. If no empty theatre exists for a theatre-required service, the task blocks rather than silently falling back to an occupied room.

Failed request notes and employment-step diagnostics remain persisted after a procedure leaves the active queue. `hospital operations` reports only genuinely active task counts and includes the five most recent service failures with their reasons; `hospital failures [count]` provides a bounded history view including request history and task diagnostics. The general employment task list also appends the recorded failure diagnostic for failed tasks instead of displaying only `next step none`.

The action runner uses existing health systems rather than a parallel treatment model. Bedside hospital services start the same player command routes used by live characters (`bind`, `suture`, `cleanwounds`, `tend`, and `relocate`) and then leave the employment task in progress while those commands' normal delayed effects, blockers, item plans, room echoes, and restrictions resolve. Combined stabilisation and full-treatment services advance one command-routed phase at a time, persisting phase state on the employment step; cleaning and tending phases are marked complete after one command effect, so repeatable antiseptic, cleaning, and anti-inflammatory treatments do not restart indefinitely on the same request. If a requested combined service has no valid visible wounds, fractures, or blood-loss need, it fails with that reason rather than completing silently. Completed stabilisation, full-treatment, surgical, and other multi-step hospital procedures emit a procedure-complete echo before any post-treatment routing. Configured surgery or implant services start the selected `ISurgicalProcedure`. When surgery removes target-covering clothing, room placement is idempotent in case removal behavior has already placed the item there, and each item is kept as a distinct stack so its recorded belongings identity remains valid for recovery routing. Surgical services that need an unconscious patient can administer a configured injectable anesthesia drug at a calculated intensity before the procedure starts; if a cannulation procedure is configured, the hospital starts cannulation as a staged prep procedure, then connects a prepared IV liquid container through a drip to the installed cannula, primes a calculated anesthetic volume through the liquid injection path, sets the drip rate, and starts the bag in drip mode. Implant services can create a configured implant prototype, pass it into the install procedure, and then continue through configured implant power and interface procedures before completing the hospital request. Staged surgical requests remain in progress until `SurgicalProcedureEffect` completes or aborts, then the hospital request is completed or failed from the effect callback.

Blood donation removes up to the configured volume from the donor while respecting a safe minimum blood percentage and the supplied IV container's spare capacity, then stores a `BloodLiquidInstance` in that container. Donation availability only requires a drain-capable IV blood container with compatible spare capacity; it does not require a single bag that can hold the full target draw. Blood-service cannulation and decannulation callbacks remain part of the staged IV blood workflow even when the service happens to reference the same procedure as its anesthesia cannulation setting; they cannot continue into the anesthetic-drip setup path. The runner reuses an already safe drip rate when a rate-selector rejects its formatted value, selects only an empty IV bag with useful spare capacity for donation, echoes the IV connection to the theatre, and neutralises, disconnects, and removes a hospital-inserted cannula before a terminal blood-workflow failure can route the patient out of the theatre. When a standalone or combined blood-capable service completes, fails, or is cancelled, disconnected full, used, and otherwise unused blood bags in that theatre are returned to the first configured hospital supply room. The donor is never charged. If a matching blood-stock policy exists, its per-litre price pays only the volume needed up to the target; otherwise the blood-donation service price is paid as a flat successful-donation amount. The request preflights hospital virtual cash or linked-bank funds before drawing blood. Payment is issued after collection succeeds, and a transient payout failure leaves the completed draw in a retryable `payingdonor` phase rather than marking the request complete unpaid. Blood transfusion consumes compatible blood from a prepared, held, or in-room liquid container, rejects incompatible donor blood through `IBloodtype.IsCompatibleWithDonorBlood`, and caps restoration at the recipient's total blood volume.

If a service requires recovery, post-treatment routing prefers the first recovery room for all patients and tries to place helpless patients on a suitable bed-like item; the first waiting room is only a fallback when no recovery room is configured. The patient receives a direct completion-and-routing message, and the origin and destination rooms receive movement echoes. If neither recovery nor waiting room is available, the patient remains in place and the request records an audit note. Terminal hospital execution failures mark the service request failed and fail the active employment task so the assigned worker is eligible for other work; recoverable blockers such as missing prepared supplies remain blocked with the reason visible on the task list and detail views.

Terminal failures and aborted staged procedures also emit the recorded failure reason directly to the patient. If the patient is still in the reserved operating theatre, failed-treatment routing moves the patient and any colocated escorting employee to recovery, or to the waiting-room fallback, with source and destination room echoes instead of leaving the patient in theatre.
...
- `EmploymentWorkerAI` for NPC application, workplace travel, task claiming, action-step execution, payroll claiming, and arrears-driven resignation. `capability manager` maps to `CanManageEmploymentHost`; that capability is implicitly required by every `Manager` opening, including legacy openings whose stored requirement list predates the capability, and only a manager/proprietor contract with that capability performs worker-driven manager-goal evaluation. Ordinary scheduled-rule, payroll, lifecycle, and assignment-audit evaluation remains available to task-enabled workers. Hospital staff use the same AI: `host hospital|clinic|infirmary` narrows job searching, while `capability doctor|medical` maps to `CanPerformMedicalServices` and `capability orderly|nurse|supplies` maps to `CanPrepareHospitalSupplies`.
...
- physical worker task custody is verified through the body held-or-wielded item abstraction rather than hand-slot or inventory internals; host-managed stock and other virtual logistics sources carry an explicit context-managed marker so audits and step hydration do not mistake them for physical custody
...
- the stock seeder now demonstrates this with family-level combination categories such as `Medicine`, `Writing Materials`, `Clothing`, `Intoxicants`, `Household Goods`, `Hospitality`, `Entertainment`, `Personal Services`, `Communications`, `Military Goods`, and `Professional Tools`, using non-equal stock weights over the current `UsefulSeeder` direct child tags while leaving more world-specific baskets such as `Nourishment` for builders to tune themselves; the medicine family weights include herbal remedies, treatment supplies, surgical supplies, and prosthetics/mobility, while Clothing includes work, ceremonial, religious, and maritime categories alongside simple, standard, winter, luxury, and military clothing
...
`EconomicActivityRecords` is an append-only semantic ledger. Each row stores UTC time, the economic zone, current financial period and zone calendar keys, native value, contemporaneous global-base value, and source/destination control buckets resolved at event time. Exchange volume includes sale/service/property consideration; gross movement adds genuine owner-to-owner transfers, sources, and sinks while excluding internal layer movement. Volume reports aggregate matching rows in SQL and return only the small activity/control summary; they do not hydrate the bounded ledger window into the game process. New installations begin with an empty ledger and report their coverage start rather than treating bounded bank or shop histories as complete backfill data.
...
`IEconomyAnalyticsService.GetEmploymentMarket` combines posted, unarchived legacy job listings with unified host openings that accept applications. Full, suspended, filled and closed host postings are excluded. Vacancies are remaining positions, not posting counts; unlimited legacy listings are shown separately rather than assigned an invented capacity. PC-accessible means the posting does not forbid PCs, not that every PC meets its eligibility requirements.

Employment counts deduplicate character IDs. Active and suspended host contracts and unfinished legacy jobs count as employed. Host and role breakdowns describe unified employment only. The free labour pool consists of loaded, living NPCs with an `EmploymentWorkerAI` whose search is enabled and no current employment anywhere, including outside the queried zone. Hosts/listings are scoped by their economic zone, while free workers are scoped by their current cell. Free capability counts overlap: a worker may be both medically capable and manager-capable. They describe configured capabilities, not a guarantee of eligibility, pay acceptance, pathfinding, schedule or immediate task availability.

Volume aggregation additionally sums payments by currency and activity whose destination is a directly PC-controlled Character. Source-side PC involvement, shared organisational control, NPCs, staff and internal movement do not qualify as PC income. Unified payroll uses its existing settlement boundary (cash made claimable or a bank payment); legacy jobs record wage activity when collected, and clan payroll history records a clan payment on `PayCollected`. Accruals and administrative backpay adjustments are excluded. Project pay remains separately identifiable. Original currencies and contemporaneous global values are retained, including when legacy wages are converted during collection.

Each new snapshot records PC-accessible posting count, total NPC employment/free labour, host vacant postings/positions, employed medical workers/managers and free medical/manager capabilities. It also records rolling real-day PC job, project and clan income separately for every convertible currency, including zero values. Count metrics use the existing snapshot entry scalar fields without currency conversion; the required currency foreign key is only a storage anchor. Income metrics retain native and global values. New global trends use global snapshot headers only, avoiding mixed global and zone-specific samples. Existing snapshots have no new metric rows and are not backfilled. Per-host and full role/capability breakdowns remain live reports. No EF schema change is required.
...
## Restaurant and Cafe Service

`IRestaurant` is a specialised `IPermanentShop`, not a parallel payment or tax system. `Restaurant` keeps the normal shop's economic zone, ledger, taxes, tills, virtual cash, bank integration, merchandise, and `EmploymentHostType.Shop` employment host. Its additional durable state is the restaurant configuration, designated physical tables, menu entries, table sessions and participants, orders, order-item provenance, payment/refund records, server cleanup cadence, restaurant service-emote templates, one restaurant-wide outer-takeaway-bag prototype, and physical kitchen storage-role assignments. `FuturemudLoaders.LoadEconomy()` loads that graph with the shop so reboot recovery resumes the same queue rather than inventing a second economy path.

A designated table must be a physical item carrying both `ITable` and `IContainer`, placed in a configured service cell. Restaurant cell roles are `Service`, `Internal`, and `Kitchen`. A diner may move anywhere within the restaurant's configured footprint without being treated as leaving; a kitchen remains part of the shop for staff but is not a customer table-service area. A short persisted abandonment-pending state gives returning participants time to re-enter before a skipping-bill crime is considered. A persisted abandonment marker prevents repeated reports, and each unpaid orderer is considered separately rather than accusing a recipient whose meal was paid for by somebody else. A fully paid session can be closed to new orders while its already-queued service remains deliverable.

Table sessions keep accepted membership for their lifetime. An empty table creates a session with the first character accepted. An occupied table admits a requester automatically only for a current shared party or when an existing, in-scope participant evaluates `participant.IsAlly(requester)` as true. Otherwise the runtime creates transient `Accept` proposals for current accepted participants. Those proposals are deliberately not persisted, expire with the normal acceptance timeout, suppress duplicates, and revalidate table, requester, approver, and location when accepted. Before acceptance a requester cannot order, target a recipient, inspect the bill, pay it, or request a split.

For an NPC chef, a craft-required line is claimed and started through the ordinary `ICraft.BeginCraft` path, then resumed with `ICraft.ResumeCraft` if an interruptible craft pauses. The runtime only accepts the real configured craft output after it appears, so a missing tool, input, failed craft, or wrong output remains an auditable recoverable state rather than becoming simulated food. In a kitchen, workers draw reusable plates, inner containers and bags from their assigned stores when available, and return held craft tools and cleaned serviceware to their corresponding stores. A server retrieves the outermost prepared item using the same native inventory `GET`, `PUT`, and `DROP` pathways used by characters, walks directly from its current location to the designated table, and silently performs the native table placement before emitting the configured service emote. Prepared and cleared serviceware remains resolvable while it is carried by an on-duty restaurant worker, so the next AI tick and reboot recovery do not lose an in-progress handoff. This makes the plate a real visible table item rather than a hidden inventory grant. Empty served plates are checked on the restaurant's configurable, default two-minute cleanup cadence; an NPC server visibly retrieves all eligible empty serviceware, bundles compatible items for the trip, carries the bundle to the kitchen, and unbundles it into servingware storage. If no configured storage can accept an item, it is placed on the kitchen floor rather than retained by the worker. Restaurant-level templates cover chef start, opening, plating, ready, server serving, clearing, and return-to-kitchen moments; invalid persisted text falls back to a neutral visible service emote. Those templates are broadcast locally from the worker's actual cell even for an unpossessed NPC, so diners receive each narrated service step.

Orders progress through durable queued, preparing, ready, served, failed, cancelled, and refunded states. Dine-in orders record an unpaid liability against the orderer; takeaway takes a normal shop payment before entering the queue. Service recognition creates the existing sale and tax ledger entries when goods are delivered, while refunds use an auditable refund transaction. A refund record never silently transfers a bill or makes an already-settled line appear newly unpaid; cancellation and failure clear the customer liability while retaining payment/refund evidence. Chef and server work uses the same queue transitions for PC duties and the optional automated fallback. Real crafting is the default: a chef must provide the configured craft output. Simulated crafting requires both automated service and an explicit simulation configuration option.

Menu fulfilment supports unaltered stock, open-and-bring, craft-and-bring, craft-and-plate, and takeaway packaging. A plate and per-menu inner package are ordinary real container items. The outer bag is configured once for the restaurant, and all compatible ready takeaway roots for a recipient are packed together using the fewest bags that can contain the complete delivery; the runtime creates further bags only when capacity makes them necessary. Every produced root and packaging layer is persisted against the order so failures, cleanup, and recovery have an auditable trail. Queue estimates combine configured menu preparation, the configured craft's real phase duration, handling time, queued work, quantity, and the batching window. Ready lines are held for related table lines where practical and released when every related line is ready or the configured maximum grouping wait expires.

Restaurant employment hosts can use the native `restaurantingredients` manager goal. Its `restaurantstock` condition derives the inputs for every active craft-backed menu item, checks all configured ingredient stores, and creates ordinary purchase-and-delivery work for enough ingredients to make the configured number of meals. The native planner deliberately blocks and reports unsupported craft-input forms rather than guessing an unsafe stock requirement.
