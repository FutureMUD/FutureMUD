...
`IRestaurant` is a specialised `IPermanentShop`, not a parallel payment or tax system. `Restaurant` keeps the normal shop's economic zone, ledger, taxes, tills, virtual cash, bank integration, merchandise, and `EmploymentHostType.Shop` employment host. Its additional durable state is the restaurant configuration, designated physical tables, menu entries, table sessions and participants, orders, order-item provenance, payment/refund records, server cleanup cadence, and restaurant service-emote templates. `FuturemudLoaders.LoadEconomy()` loads that graph with the shop so reboot recovery resumes the same queue rather than inventing a second economy path.

A designated table must be a physical item carrying both `ITable` and `IContainer`, placed in a configured service cell. Restaurant cell roles are `Service`, `Internal`, and `Kitchen`. A diner may move among `Service` and `Internal` cells, such as a bathroom, without being treated as leaving; a kitchen remains part of the shop for staff but is not a customer table-service area. A short persisted abandonment-pending state gives returning participants time to re-enter before a skipping-bill crime is considered. A persisted abandonment marker prevents repeated reports, and each unpaid orderer is considered separately rather than accusing a recipient whose meal was paid for by somebody else.
...
For an NPC chef, a craft-required line is claimed and started through the ordinary `ICraft.BeginCraft` path, then resumed with `ICraft.ResumeCraft` if an interruptible craft pauses. The runtime only accepts the real configured craft output after it appears, so a missing tool, input, failed craft, or wrong output remains an auditable recoverable state rather than becoming simulated food. A server retrieves the outermost prepared item using the same native inventory `GET`, `PUT`, and `DROP` pathways used by characters, walks it to the designated table, and puts a plated dish into that table's container before the named recipient. Prepared and cleared serviceware remains resolvable while it is carried by an on-duty restaurant worker, so the next AI tick and reboot recovery do not lose an in-progress handoff. This makes the plate a real visible table item rather than a hidden inventory grant. Empty served plates are checked on the restaurant's configurable, default two-minute cleanup cadence; an NPC server visibly retrieves one, carries it to a kitchen or stockroom cell, and sets it aside. Restaurant-level templates cover chef start, opening, plating, ready, server serving, clearing, and return-to-kitchen moments; invalid persisted text falls back to a neutral visible service emote. Those templates are broadcast locally from the worker's actual cell even for an unpossessed NPC, so diners receive each narrated service step.
