...
- stock combination-category examples for setting-agnostic family baskets such as `Medicine`, `Writing Materials`, `Clothing`, `Intoxicants`, `Household Goods`, `Hospitality`, `Entertainment`, `Personal Services`, `Communications`, `Military Goods`, and `Professional Tools`, each seeded as a deterministic non-equal roll-up of the current `UsefulSeeder` direct child categories with heavier emphasis on common or standard goods; the medicine basket now includes herbal medicine, treatment supplies, surgical supplies, and prosthetics/mobility alongside the older simple, standard, and high-quality medicine leaves
