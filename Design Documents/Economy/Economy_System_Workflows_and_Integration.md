...
Place real furniture in a service cell and register only items that have both `Table` and `Container` components: `restaurant table add <item>`. Add existing shop merchandise to the menu with `restaurant menu add <merchandise>`. Deactivate a menu entry before using `restaurant menu set <item> mode|prep|craft|plate|package|bag|dinein|takeaway|desc ...`, then reactivate it with `active true`. Activation validates the configured craft output, opening capability, and each required real container. `restaurant set handling <seconds>` and `restaurant set batchwait <seconds>` tune ETA and coordinated service; `restaurant set cleanup <seconds>` controls the server's periodic empty-plate sweep (default 120 seconds). `automation true` plus `simulate true` is the explicit opt-in for non-real crafting fallback.

Assign chefs and servers through ordinary shop employment, or use the restaurant shortcuts `restaurant chef <target>` and `restaurant server <target>`. A clocked-in employee uses `restaurant service list` to find queue ids; a chef uses `restaurant service prepare <order> [with <crafted item>]`; a server uses `restaurant service serve <order>` and `restaurant service clear <table>`. Managers can record `restaurant service cancel <order>` and refund a present orderer with `restaurant service refund <order> <amount> [payment]`; cancelled and failed lines waive customer liability but retain their prepayment/refund audit. NPC worker AI and PC employees act through this same persisted queue: a capable chef walks to the kitchen, starts/resumes the real craft and supplies its actual output, while a capable server uses the same visible `GET`, `PUT`, and `DROP` operations to carry the ready plate to the diner, place it on the table, and later clear it on the configured cadence. Restaurant service templates are emitted locally from the chef or server's actual cell, including for unpossessed NPC staff, so everyone nearby sees the configured narration as well as the native item-operation echoes. A plate being carried remains part of the recoverable service state, so the next AI tick can finish that handoff rather than treating it as missing. If automation is disabled or no appropriate worker exists, queued orders stay visible with their revised ETA rather than being silently completed.
