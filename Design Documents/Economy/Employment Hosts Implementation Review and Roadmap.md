...
As of 2026-06-19, `EmploymentHostOperationsScheduler` centralises scheduled-rule evaluation, manager-goal evaluation, payroll accrual, fixed-term contract lifecycle expiry, and active-task assignment audits for a host. Worker AI host-work evaluation now calls this scheduler rather than directly invoking scheduled-rule and manager-goal services. The scheduler returns countable result objects for tests and future command/daemon callers. As of 2026-07-11, idle hosts exit without constructing an employment task context, worker-triggered maintenance is coalesced at host level so multiple employees do not repeat the same pass, empty assignment boards return before allocating audit state, and the global scheduled-rule pass uses a lazy active-host registry initialised by one batched persisted-key query and maintained on rule creation/cancellation. This avoids both per-host persistence queries and accidental employment-state creation for hosts with no rules. Stale grant/reservation cleanup, retry/escalation policy, cross-process host locking, and idempotent distributed scheduling remain deferred.
