...
Active patrol enforcers immediately hand an arrestable active-enforcement target over to the patrol custody loop when the target becomes helpless. Patrols can begin active enforcement for identity-known wanted characters, or for identity-unknown crimes when the currently visible character is the same physical actor recorded on the crime. The patrol leader must be able to see the character before the warning state is applied, but identity-obscuring clothing or equipment does not by itself block enforcement when the crime still applies to that visible actor. If a patrol member is already dragging the wanted character, the patrol can recover that active target and continue the normal prison path instead of treating the drag as unrelated custody. Kill-on-sight enforcement does not use this custody recovery path.
