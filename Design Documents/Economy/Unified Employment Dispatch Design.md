...
- Additional player/builder command adapters, richer condition expressions (`OR`, grouping, reusable named predicates), real account-transfer mutations, physical employer cash-reserve debiting for wage settlement, autonomous vehicle driving, animal leading/return, deeper craft tool/station reservation semantics, price adjustment, job-opening/rule/task administrative actions, skill/knowledge/tag candidate profiles, and deeper real board command integration are deferred. The completed command-adapter, worker-AI, inventory-movement, payroll-liability, durable step-state, action-catalogue, logistics, broadened host finance, scheduled-rule authoring, finance-foundation, physical-float, and craft-progress slices supply read/access views, authorised host-board posting, safe creation of non-financial openings, manager application decisions, drafted retrieval/delivery/planning/finance/craft active tasks, central host-level scheduled-rule evaluation, real shop purchase/store-account/tax/float/craft-trigger adapters where supported, a first live NPC application/task loop, and recoverable overdue-wage reputation pressure without replacing legacy host-specific employment commands.

### Active milestone

- The active milestone has moved past scheduled-rule v1 hardening into action-family closure. Central host-level scheduled-rule evaluation now runs independently of worker heartbeats, and the finance foundation makes supported purchase, store-account payment, tax payment, bank/virtual-cash, register-float, physical-float, and craft-progress actions executable with durable authorise/reserve checks and employment audit cross-references. Item-selector and commodity-selector purchases now hand the exact resolved stock items to the native shop sale path instead of relying on keyword fallback, preserving normal shop transactions, taxes, echoes, and restock hooks while guaranteeing the selected stock is what the employee buys. The next logical implementation slice is supplier/craft optimisation and remaining native subsystem closure: reserve craft inputs/tools before craft start and decide the native service boundary for account transfers and physical wage-reserve movements. After that, autonomous vehicle driving, animal leading/return, cargo optimisation, price adjustment, richer condition expressions, and job-opening/rule/task administrative actions remain later slices.

### Blockers / decisions needed

- No blocker was found for the durable employment persistence, hotel-root split, command-inspection, initial inventory action-step, task-draft, action-catalogue, scheduled-rule authoring, condition-catalogue, host-finance adapter, physical-float, craft-progress, or `EmploymentWorkerAI` slices.
- Exact item/commodity purchasing now resolves concrete stocked items and invokes the native exact-stock shop sale path. The remaining purchase limitation is commodity granularity: commodities are still purchased as whole matching stocked items rather than split to an exact target weight unless the underlying stocked item quantity/splitting model naturally supports that precision.
