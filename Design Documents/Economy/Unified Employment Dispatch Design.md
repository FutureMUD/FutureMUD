...
- Perfect optimisation of employee routing, vehicle loading, vehicle assignment, delivery batching, or market arbitrage.
...
The task system should be composable. Avoid creating large monolithic task types such as â€œpurchase butter taskâ€ or â€œcarrot delivery taskâ€ that encode both the trigger condition and the full workflow as a special case.
...
The dispatcher blocks at the action-step level and now matches candidates against the next pending step rather than the whole action plan. When a worker completes a step and cannot execute the next one, the task can be released back to `Pending` for another worker if no task items are in that worker's custody. If physical custody exists, the task remains a manager-review problem until a persisted custody handoff model is designed.
...
As of 2026-06-19, manager goals also carry a runtime policy object with per-currency budgets and risk limits. Creation and draft authoring reject plans outside budget/action-step limits, evaluation revalidates policies, and active-task caps throttle work before the idempotency guard. These policy limits are runtime-only for now; persisted goals reload with the default policy until durable policy persistence is designed.

As of 2026-06-19, manager goals have explicit runtime `Satisfied` and `Failed` states. A standing goal becomes `Satisfied` when its trigger conditions are not currently calling for work, but that state is non-terminal and the goal can reactivate on a later evaluation. A goal becomes `Failed` when it has no viable action plan or when one of its spawned active tasks fails, carrying a manager-readable diagnostic forward.
...
Price changes, sale/deal changes, shop-stocktake checks, shop-cash reconciliation, explicit shop stock-transfer finalisation, and job-opening changes are task-executable administrative mutations. `price merch <id|name> <amount>` changes exact shop merchandise base price and records the native shop price-adjustment transaction plus employment audit evidence. `sale create|modify|cancel` manages native shop-owned `ShopDeal` rows for temporary sales and volume deals with normal authority validation, register entries, and optional expiry. `stocktake all|merch <id|name>` calls native shop stocktake routines, recording item count or commodity weight evidence on the task step and employment register without automatically writing off discrepancies. `cashreconcile [note]` snapshots expected, virtual, and same-currency physical shop cash, calls the native shop float check, and records variance evidence without directly moving funds. `stocktransfer to <shop> merch <merchandise> [destination <cell>] [container <selector>]` finalises carried source-shop stock into target permanent-shop merchandise by calling native stock disposal/addition paths and writing paired employment register evidence, without creating sale or profit ledger entries. `price market ...` is deprecated, parser-hidden, and quarantined on persisted load because employment tasks, rules, and goals must not mutate `MarketInfluence` state. `jobopening create|close|modify` creates, closes, or fully modifies employment openings with normal authority validation and register entries.
...
Auction houses expose managers and employees through the same model. The first native-wrapper action slice supports task-authored auction-house-owned lot listing, due-lot settlement, and unclaimed movable lot claiming while keeping bid settlement, commission, payout/refund, results, and unclaimed records in the native auction subsystem. Third-party consignment intake, valuation policy, seller deposits, authority to bind outside sellers, and automatic buyer/seller delivery planning remain future auction-management design work.

### Arenas

Arenas use the model for managers and employees who operate events, payouts, scheduling, maintenance, or contestant handling. The first event-specific task automation slice now supports manager-authored native event creation, phase advancement, and abort handling through `arenaevent` steps while leaving competitor intake, purses/prizes, staffing/security, stable/equipment preparation, medical follow-up, cleanup, recurring event policy, and automatic scheduling as future arena-management work.

### Banks

Banks are both employers and payment infrastructure. Be careful to avoid circular assumptions: employee wage deposits may use banks, but bank employment must not depend on a working employment-seeking flow to initialise. Bank employees may include tellers, managers, and couriers. Account transaction records are the canonical financial evidence for bank account movement, while reserve-balancing employment tasks should update native bank reserves and a host-level virtual-cash counterbalance with correlated bank manager audit and employment register entries. The current bank automation slice now covers reserve audit/deposit/withdraw, account credit/status/close, and branch post/courier evidence through `bankadmin` task actions. Rollover, account opening, physical cash-in-transit custody, teller rosters, per-branch vaults, and branch-local controls remain future bank-service design work.

### Stables

Stables should use common employees/managers for stable hands, animal care, rental/boarding management, and transport-related work. The action-step design should be compatible with stable-managed animals, carts, and consists. The current stable automation slice covers `animal` lodge/return operations plus `stableadmin` care evidence, fee assessment, stay/ticket reconciliation, and account reconciliation against native stable stays/accounts; durable feed inventory, welfare/capacity policy, equipment readiness, and automatic account payment flows remain future stable-service work.
...
Hotel business state includes hotel rooms, rental offerings, active rentals, patron balances, lost property, taxes, manager workflows, host board, bank account, virtual reserve, task board, manager-goal board, and operational register. Hotels may employ clerks, cleaners, maintenance workers, security, managers, and couriers. Potential manager goals and action plans include maintaining room readiness, collecting unpaid rent, posting guest or staff notices, buying cleaning supplies, moving linen or fuel, depositing takings, reconciling patron balances, managing lost property, and paying taxes. The current hotel automation slice covers `hoteladmin` room-operation evidence, native lost-property expiry checks, lost-property audits, and patron-balance audits against native hotel/property rows; durable cleanliness/readiness/maintenance tickets, autonomous check-in/check-out, deposit/furnishing replacement policy, and physical lost-property release/auction flows remain future hotel-service work.
...
- Employment-host purchases, physical floats, completed crafts, and failed-craft salvage now carry the employer's durable item ownership reference rather than the assigned employee's. Host-controlled private-property cells admit every active employee role and timed permit holder; managers and proprietors can grant those permits and assign possessed items to their host.
...
- Added permanent-shop stock movement bridges: authorised managers can create stockroom-to-shopfront restock movement tasks from existing stocked merchandise, and explicit cross-shop stock-transfer tasks from a source permanent-shop stockroom to a target permanent-shop destination. These bridges use real shop stockroom/shopfront/display-container resolution, physical cell/container movement through the employment action context, native stock disposal/addition paths for cross-shop retagging, and paired employment register evidence. They enforce both `AssignTasks` and `ManageDeliveryRoutes` authority and do not use host-board posts as the task carrier.
- Added manager-controlled application decisions: authorised managers can accept pending applications into active contracts using the opening's role, compensation, schedule, duration, payment method, and authority, or reject applications with a reason. These paths update persisted application state, reject closed/full/stale-revision applications, re-evaluate in-memory candidate profiles, enforce active occupied-position capacity, honour administrator authority, and write application plus hire register entries.
...
- Added the first employment payroll-liability and wage-disbursement slices. Periodic paid contracts now accrue durable wage payables with in-game due dates and overdue-day accounting; managers with `ManagePayroll` authority can run payroll evaluation and settle outstanding wage liabilities through `payroll run` and `payroll settle`; cash payables debit backed employer funds into durable claimable payroll escrow and can be claimed by active or former employees through `payroll claim`; employee-bank and specified-bank payables require a destination account in the payable currency and credit it through native bank-account transactions. Settlement writes wage ledger rows and payroll register entries. Hourly rates use explicit schedule windows to calculate guaranteed worked hours within the minimum daily payroll period, with final partial periods still payable when a short engagement ends. Unsettled payables remain after resignation/termination so an employer can settle the matter later and recover payroll reliability even if the employee is no longer actively employed.
...
- `EmploymentWorkerAI` now evaluates host payroll while employed, avoids employers whose unresolved host-wide payroll arrears exceed its configured overdue-day tolerance, and resigns with an `UnpaidWages` termination reason only when its own employee-specific arrears reach that tolerance. The AI setting is `MaximumUnpaidOverdueDays`, configured with the builder `arrears <days>` command and persisted in the AI XML definition.
...
- Worker AI host discovery now treats hotel roots as existing durable hosts only. Passive job-search and task scans do not call `Property.Hotel`, so they cannot lazy-create hotel rows, employment host states, or staff boards for every property; explicit hotel/employment access still performs the compatibility lazy creation path. Hot-path active-host selection uses a single pass over hosts and pending tasks while preserving assigned-work, priority, due-time, creation-time, task-id, and host-name ordering; employment-opening ranking also reuses its path-distance result instead of running reachability and commute pathfinding separately.
...
- Added catalogue-backed manager-goal authoring. `goals types [all|category|type]` documents the initial explicit goal families (`cashfloatlow`, `cashfloathigh`, `taxes`, `accounts`, `craftstock`, `craftmaterials`, `payroll`, `bankbalancelow`, `bankbalancehigh`, `pricemargin`, and `staffing`); managers with delegated `CreateManagerGoals` authority can draft, copy, edit by draft, add goal conditions through the scheduled-condition parser, add rich goal expressions through the scheduled-rule expression parser, add goal action steps through the task-action parser, set priority/cadence/budget/risk/required authority, finalise goals through `IManagerGoalBoard.CreateGoal`, inspect details, cancel goals with `ModifyManagerGoals`, and manually evaluate goals for testing. Existing scheduled-rule condition authority is normalised so manager goals do not require `CreateScheduledRules` merely to use read-only finance conditions, while stock/action authorities still contribute to the final required authority. Payroll, bank-balance, price-margin, and staffing goals can now use executable native action steps.
- Added the employment task action catalogue. `tasks actions [all|category|action]` now exposes grouped action metadata, syntax, catalogue status, required authority, required AI capabilities, payment-authorisation requirements, and financial flags. Existing retrieval/delivery, movement, board-post, command, supplier-selection, purchase, bank deposit/withdrawal, store-account payment, tax payment, shop-float adjustment, physical-float, craft-station, craft-trigger, inspection, price-change, shop stock-transfer, auction lot-listing, auction settlement, auction-claim, arena event-administration, and job-opening administration steps are catalogue-backed; planning/authorisation/report actions (`report`, `authorise`, `reserve`, `release`, `select`, `estimate`, and `route`) are now executable catalogue steps that write operational register entries and durable per-step operational state. The logistics catalogue now includes inventory-plan-backed `load`, `unload`, and `return container` actions plus `vehicle cargo` selection that validates and records accessible cargo spaces without autonomous driving. Supplier-selection steps preview the same native purchase resolver used by real purchases and record cheapest stocked supplier, merchandise, price, and location evidence without spending employer funds. Real shop purchases now call the native shop sale flow using an employer-backed payment method, store-account payments mutate native line-of-credit balances, shop and hotel tax payments use supported native tax paths, supported host bank/virtual-cash movements now use native finance adapters, physical-float steps can issue, return, or settle task-custody cash, craft-station steps validate a craft location/selector, craft-trigger steps start/resume native crafts, adopt newly produced item outputs into task custody, record carried task-custody items as later craft inputs, and fail closed with salvage custody when a native craft component fails, inspection steps record QC notes against task-custody items, price steps mutate exact shop merchandise prices only, sale steps create, modify, or cancel native shop deals without touching market influences, shop stocktake steps call native count/weight stocktake APIs and record discrepancy-review evidence, shop cash-reconciliation steps snapshot native float variance, shop stock-transfer steps call native stock disposal/addition paths and write paired source/target register evidence without financial ledger entries, auction lot-listing steps list carried task-custody items through native auction-house lot records, auction settlement steps complete due lots through native result, commission, payout/refund, and unclaimed-item handling, auction-claim steps move selected unclaimed movable lots into task custody for downstream delivery, arena event-administration steps create, advance, or abort native combat arena events through `ICombatArena` and `IArenaLifecycleService`, bank administration steps cover reserve audit/deposit/withdraw, account credit/status/close, and branch post/courier evidence, stable administration steps record care/stay/account evidence and call native stable fee assessment, hotel administration steps record room/lost-property/patron-balance evidence and call native lost-property expiry checks, deprecated persisted market-price steps quarantine for builder review, job-opening steps create, close, or fully modify `JobOpeningDefinition` state, scheduled-rule lifecycle/evaluation steps pause, resume, cancel, or manually evaluate existing rules through the host task board, active-task administration steps retry, requeue, assign, or cancel existing active tasks through the host task board with custody safeguards, and manager-goal administration steps evaluate, cancel, or reactivate existing manager goals through the goal board with terminal cancellation safeguards. Cross-currency account transfer, parent/consolidated host settlement, autonomous vehicle driving, autonomous animal leading, deeper stable welfare/capacity/feed/account services, deeper hotel room-state/check-in/payment/lost-property release services, load optimisation, persisted station queue/retry scheduling, scheduled-rule create/edit, hard active-task state marking, and manager-goal definition mutation remain deferred until they can call their owning subsystem services with complete resource custody.
...
- Added the first real employment finance foundation. Active tasks now carry runtime `EmploymentTaskProvenance`, explicit creator/authoriser principals, priority, lifecycle timestamps, and an `EmploymentAuthorisationGrant`; `authorise [<amount> for] <description>` contributes authored grant limits at task creation/spawn time and records an audit step, but completing that step no longer acts as managerial approval by itself. `reserve [<amount> for] <description>` creates durable reservation references in step operational state after checking host available funds; and `release [reservation <id>|all]` clears matching task reservations. `bankdeposit <amount>` and `bankwithdraw <amount>` are now executable for supported employment hosts with a linked native bank account: deposits debit only employer virtual cash before calling `IBankAccount.DepositFromTransaction`, and withdrawals call native `IBankAccount.CanWithdraw`/`WithdrawFromTransaction` before crediting employer virtual cash. `transfer <amount> to <bank account id|bankcode:account|alias>` now moves same-currency funds from the linked native bank account to an active target native bank account with paired `WithdrawFromTransfer`/`DepositFromTransfer` records, employment ledger/register evidence, and persisted transaction/reservation references. `settle <amount> to <host type> <id|name>` now moves same-currency backed funds from one supported employment host to another through paired native virtual finance records plus source and target employment evidence. The finance adapter now covers shops, stables, hotels, auction houses, combat arenas, and banks where those hosts expose currency/cash/bank state; unsupported combinations block with diagnostics and do not write partial records. Employment register/ledger rows cross-reference native finance movements rather than becoming the canonical accounting record. Runtime grants now carry amount limits plus derived payment-source and counterparty scopes for financial steps, so account-transfer and host-settlement tasks can fail closed when a grant names a different source or counterparty. Grant persistence, revocation, route-pair scoping, and parent-organisation scoped grants remain deferred; loaded tasks without persisted grants fail closed for financial authority.
- Added scheduled-rule authoring and condition-catalogue command support. Managers with delegated `CreateScheduledRules` authority can create transient scheduled-rule drafts, set idempotency keys and cooldowns, add AND-composed manual/time/stock/account conditions, reuse the existing action-step parser for planned work, and finalise through `IEmploymentTaskBoard.CreateScheduledRule` so condition authority and action-plan authority are rechecked. `tasks conditions [all|category|condition]` exposes condition syntax and authority; `tasks rule show`, `diagnose`, `evaluate [manual <key>]`, `pause`, `resume`, and `cancel` make rules inspectable, test-triggerable, temporarily suppressible, resumable, and removable from the shared employment command and all host aliases. Rule status is persisted in the `EmploymentScheduledTaskRules` table, paused rules do not spawn work, and `tasks rule draft copy <rule> [new name]` provides a safe edit-by-copy workflow that reuses the original rule's conditions, action steps, and cooldown while assigning a fresh idempotency key to the draft. Manual triggers are transient evaluation inputs, stock merchandise conditions read real shop merchandise stock where supported, item-count conditions inspect real room/container contents using the shared item selector grammar, commodity-weight conditions sum material/tag/variable-matching commodity weights using compact descriptors such as `Iron|Nails|grade=refined`, shop-account conditions read line-of-credit balances, shop-float conditions inspect permanent shop till currency piles, weather conditions detect newly-started precipitation or wind-level transitions, and account conditions read supported shop cash/bank/available finance state while leaving financial mutation guarded by task grants and reservation checks.
...
- Added central host operations scheduling. `EmploymentHostOperationsScheduler` is now the host-level entry point for scheduled-rule evaluation, optionally gated manager-goal evaluation, payroll accrual, fixed-term contract lifecycle expiry, and active-task assignment audits, returning countable result summaries for tests and future command/daemon callers. `EmploymentWorkerAI` delegates host-work evaluation to this scheduler instead of directly invoking scheduled-rule and manager-goal boards. `CanManageEmploymentHost` is implicitly required for all `Manager` openings, and worker-driven manager-goal evaluation requires both that capability and an active manager/proprietor contract; scheduled rules and the other maintenance passes retain their ordinary cadence. Host evaluation is now demand-gated: hosts with no live contracts, scheduled rules, evaluable manager goals, or auditable assigned work return before allocating a task context, and worker-triggered evaluations are coalesced once per host/cadence rather than once per employee. The global scheduled-rule pass uses a lazy active-host registry initialised by one batched persisted-key query and maintained when rules are created/cancelled; it therefore avoids both per-host database probes and employment-state creation for hosts with no rules. Scheduler-wide distributed idempotency, stale grant/reservation cleanup, retry/escalation policies, and capability-provider host discovery remain deferred.
...
- Additional player/builder command adapters, the fuller payroll reservation/disbursing/failed state machine, persisted attendance/time-record and explicit earning tables, payment-item and employer-float wage disbursement, autonomous vehicle driving, autonomous animal movement, persisted craft station queue/retry policy, automatic craft production-chain planning, scheduled-rule create/edit, hard active-task state marking, manager-goal definition mutation actions, schedule/personality/local-labour candidate preferences, automatic legacy-staff bootstrapping, and deeper real board command integration are deferred. The completed command-adapter, worker-AI, inventory-movement, payroll-liability, wage-disbursement, scheduled-hourly payroll, employee-specific arrears, runtime application revision guards, fixed-term expiry, durable step-state, action-catalogue, logistics, broadened host finance, scheduled-rule authoring, rich scheduled-rule expressions, finance-foundation, runtime task provenance/grants, priority task assignment, priority-aware active-host selection, central host operations scheduling, same-currency account-transfer, same-currency host-settlement, runtime grant source/counterparty scoping, physical-float, exact-stock and weighted-commodity purchase, hotel normalization, craft-progress, craft-resource-reservation, configurable craft-station capacity, no-custody step-boundary task handoff, basic sequential craft-chain custody, failed-craft salvage custody, task-custody inspection, explicit demand/storage batch-planning, task-scoped vehicle assignment, executable stable-animal operations, executable price-change, executable job-opening, task-executable scheduled-rule lifecycle/evaluation, task-executable active-task retry/requeue/assign/cancel, and task-executable manager-goal evaluate/cancel/reactivate, executable shop supplier-selection, shop stocktake, shop cash-reconciliation, shop stock-transfer, auction lot-listing, auction settlement, auction-claim, arena event-administration, bank reserve/account/branch administration, stable care/fee/account, hotel room/lost-property/patron-balance administration, and skill/knowledge/tag candidate-profile matching with normalised offer ranking slices supply read/access views, authorised host-board posting, safe creation and modification of non-financial openings, manager application decisions, drafted retrieval/delivery/planning/finance/craft/price/opening active tasks, central host-level scheduled-rule evaluation, real shop purchase/store-account/tax/float/craft-trigger/price adapters where supported, a first live NPC application/task loop, recoverable overdue-wage reputation pressure, real candidate-profile matching, and read-only legacy/new divergence reporting without replacing legacy host-specific employment commands.

### Current gap map

The foundation is now testable for live NPC employment, scheduled rules, exact-stock and weighted-commodity purchases, physical movement, host finance, native craft progress, expiring craft resource reservations, configurable station capacity, no-custody step-boundary task handoff, basic sequential craft-chain custody, failed-craft salvage custody, task-custody inspection, manager-goal-driven materials replenishment, runtime manager-goal budget/risk policy, runtime manager-goal satisfaction/failure states, runtime manager-goal rich condition expressions, task-executable scheduled-rule lifecycle/evaluation administration, task-executable active-task retry/requeue/assign/cancel administration, task-executable manager-goal evaluate/cancel/reactivate administration, executable shop supplier-selection evidence, executable shop stocktake count/weight evidence, executable shop cash-reconciliation evidence, executable shop stock-transfer evidence, executable auction-house lot listing/settlement/claim evidence, executable arena event-administration evidence, executable bank reserve/account/branch administration evidence, executable stable care/fee/stay/account administration evidence, executable hotel room/lost-property/patron-balance administration evidence, explicit demand/storage batch planning, task-scoped vehicle assignment, capacity-aware vehicle cargo compartment selection, executable stable-animal operations, persisted ordered route plans, auditable multi-stop route batching, task-local load/unload reservation evidence, auditable trip checks for fuel/feed/maintenance/rest policies, and assignment-audit recovery for unavailable workers, vehicles, animals, stables, and recorded destinations. Remaining gaps are narrower and mostly fall into native subsystem closure rather than the original host-shell architecture.

1. **Account transfer and broader finance targets.** Bank deposit/withdrawal, same-currency linked-bank account transfer, same-currency supported-host settlement, store-account payment, tax payment, register float, physical float, weighted commodity purchase, supported host virtual-cash movement, bank wage settlement, cash wage escrow, and runtime grant amount/source/counterparty scoping are executable. Cross-currency account transfer, parent/consolidated host settlement, the fuller wage state machine, persisted attendance/time-record evidence, durable opening revisions/application snapshots/contract origin columns, explicit per-task/commission earnings, employer reliability scoring, payment-item wage settlement, employer-float wage settlement, richer employee-float recovery policies, durable grant persistence/revocation, and route-pair grant scoping still need native service boundaries.
2. **Craft production-chain recovery and capacity polish.** Craft steps now reserve exact input/tool/item-station resources with expiry and cleanup, enforce configurable station reservation capacity for item-backed stations, start/resume native crafts, adopt outputs, record carried intermediate outputs as later craft inputs, adopt visible failed-craft salvage before failing for manager review, and allow no-custody step-boundary handoff from a non-craft worker to a craft-capable worker. Remaining craft work is persisted station queue/retry policy, persisted multi-assignee/custody handoff, automatic chain planning, quality scoring/triage, automatic material supplier/quantity selection, autonomous demand forecasting, and richer recovery workflows for manager-directed salvage, interrupted crafts, and rework planning.
3. **Autonomous vehicles and animals.** Vehicle assignment now records a task-scoped driver/vehicle reservation, cargo selection records the chosen compartment/projection and capacity-checks carried task items through the cargo projection container, stable-animal operations can record lead plans plus call native ride/lodge/return APIs, `tripcheck` records manager-authored fuel/feed/maintenance/rest policy evidence against optional route stops, and active-task assignment audit revalidates completed worker, vehicle, animal, stable, and destination evidence. Autonomous driving, autonomous animal leading movement, route optimisation, automatic load balancing, direct native refuel/feed/repair/rest execution, cross-task cargo holds, richer cargo-space recovery, and richer stable-account/payment handling remain deferred.
4. **Remaining administrative task mutations.** Price changes, shop-deal administration, supplier-selection previews, shop stocktakes, shop cash reconciliation, shop stock-transfer finalisation, auction lot listing/settlement/claim, arena event creation/phase/abort administration, job-opening administration, existing scheduled-rule pause/resume/cancel/evaluate, active-task retry/requeue/assign/cancel, and manager-goal evaluate/cancel/reactivate are task-executable with native service calls and audit evidence. Scheduled-rule create/edit, hard active-task completed/failed marking, and manager-goal definition mutation remain command/service surfaces rather than task-executable actions.
5. **Remaining host-specific native workflows.** Shop supplier-selection, stocktake, cash reconciliation, stock transfer, auction lot listing/settlement/claim, arena event creation/phase/abort, bank reserve/account/branch administration, stable care/fee/stay/account administration, and hotel room/lost-property/patron-balance administration now have task-executable native wrappers. Remaining host-specific work includes bank rollover, account-opening, teller-roster, per-branch vault, and physical cash-custody design; arena competitor intake, purses/prizes, security/crowd control, stable/equipment preparation, medical and cleanup workflows; durable stable welfare/capacity/feed/payment services; and durable hotel room-state/check-in/payment/lost-property release services.
6. **Manager AI autonomy.** Manager-goal boards and catalogue-backed manager-goal authoring exist, including runtime priority, budget, risk-limit, satisfaction, failure-state, and rich-expression policies, and can create operational work through services. There is not yet a broad autonomous manager AI that routinely interprets goals, chooses between purchase/craft/price/account actions, or tunes rules over time.
7. **Candidate profile depth.** Worker matching covers reservation wage, accepted payment methods, host filters, capabilities, pathability, real skill traits, knowledges, derived candidate tags, case-insensitive profile requirements, normalised compensation ranking, overdue-payroll reputation pressure, commute tie-breaking, and simple rejection memory. Schedule availability, accepted-duty preferences, personality modifiers, fatigue/capacity, expected per-task/commission/mixed earnings, richer employer reputation history, training/progression, and local labour-market dynamics are still future slices.
8. **Legacy bridge and command replacement.** Several host aliases now route employment views and selected staff operations through the shared model, and `EmploymentLegacyBridgeReporter` can report loaded legacy/new-model divergence for migration audits. The PC-facing `IJobListing`/`IActiveJob` system, `job` command, job-finding cells, job coffers, automatic legacy-staff bootstrapping, and remaining legacy host-specific behaviours remain separate by design until a convergence design is approved.

### Active milestone

The recommended active milestone is **manager autonomy and administrative-services closure**:

- extend manager-readable diagnostics for no-viable-plan outcomes beyond the current satisfied/failed/reactivation runtime diagnostics;
...
```text
/goal Continue the Employment Hosts roadmap with Epic 5 manager autonomy and administrative services. Start from Design Documents/Economy/Employment Hosts Implementation Review and Roadmap.md and preserve the Unified Employment Dispatch invariants: task routing is through employment task boards, host staff boards are communication only, legacy IJob remains separate, and all employer-money or employer-stock actions require delegated authority, durable authorisation/reservation, native subsystem records where available, and employment register/ledger evidence.

Recommended slice: continue with remaining recursive administration and host-specific workflow closure. The manager-goal priority, budget, risk-limit, satisfied, failed, rich-expression, scheduled-rule lifecycle/evaluation administration, active-task retry/requeue/assign/cancel, manager-goal evaluate/cancel/reactivate, shop deal, bounded repricing, supplier-selection, shop stocktake, shop cash-reconciliation, shop stock-transfer, auction lot-listing, auction settlement, auction-claim, and arena event-administration, bank administration, stable administration, and hotel administration evidence semantics are implemented; next design scheduled-rule create/edit, hard active-task state marking, manager-goal definition mutation through owning services, or continue the remaining Epic 6 host-specific workflow gaps such as remaining bank rollover/teller/vault/cash-custody design, remaining arena competitor/purse/security/cleanup workflows, durable hotel check-in/payment/room-state workflows, or durable stable welfare/capacity/account workflows. Keep each phase acceptance-tested and update the roadmap after each phase.
