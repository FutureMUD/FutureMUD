...
- Added the employment task action catalogue. `tasks actions [all|category|action]` now exposes grouped action metadata, syntax, catalogue status, required authority, required AI capabilities, payment-authorisation requirements, and financial flags. Existing retrieval/delivery, movement, board-post, command, purchase, bank deposit/withdrawal, store-account payment, and craft-trigger steps are catalogue-backed; planning/authorisation/report actions (`report`, `authorise`, `reserve`, `release`, `select`, `estimate`, and `route`) are now executable catalogue steps that write operational register entries and durable per-step operational state. The logistics catalogue now includes inventory-plan-backed `load`, `unload`, and `return container` actions plus `vehicle cargo` selection that validates and records accessible cargo spaces without autonomous driving. Real purchasing, store-account, crafting, pricing, autonomous vehicle driving, animal leading, load optimisation, and administrative mutations remain deliberately deferred or audit-only until they can call their owning subsystem services.
...
- Additional player/builder command adapters, richer condition expressions (`OR`, grouping, reusable named predicates), real purchasing, real crafting, real store-account/account-transfer/employee-float mutations, physical employer cash-reserve debiting for wage settlement, autonomous vehicle driving, animal leading/return, real craft station/tool use, price adjustment, job-opening/rule/task administrative actions, skill/knowledge/tag candidate profiles, and deeper real board command integration are deferred. The completed command-adapter, worker-AI, inventory-movement, payroll-liability, durable step-state, action-catalogue, logistics, shop bank/virtual-cash finance, and scheduled-rule authoring slices supply read/access views, authorised host-board posting, safe creation of non-financial openings, manager application decisions, drafted retrieval/delivery/planning/finance active tasks, first production scheduled-rule authoring/evaluation with persisted pause/resume and edit-by-copy ergonomics, a first live NPC application/task loop, and recoverable overdue-wage reputation pressure without replacing legacy host-specific employment commands.

### Active milestone

- The active milestone is now scheduled-rule v1 hardening. The next logical implementation slice should add a central host-level scheduled-rule evaluation service so rules can fire without depending on an employed worker's heartbeat. After that, the next action-family slice should either extend finance from the shop bank/virtual-cash proof into real purchase/store-account/account-transfer/employee-float adapters, or move into craft/station execution if production testing is more urgent. In both cases, new actions should only become executable when they call their owning subsystem services, reserve/authorise employer resources durably, write native canonical records first, and leave employment ledger/register rows as audit cross-references. Autonomous vehicle driving, animal leading/return, cargo optimisation, price adjustment, richer condition expressions, and job-opening/rule/task administrative actions remain later slices.
