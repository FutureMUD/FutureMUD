...
- The next active milestone should complete the logistics catalogue family before money or administration: implement real `load`, `unload`, `return`, and `vehicle` actions through existing inventory, container, animal, and transport APIs where safe; keep unsupported host/resource combinations as precise diagnostics rather than false success. After that, promote the financial family in order: durable `authorise`/`reserve`/`release` state is now present, so purchase, bank, store-account, transfer, and float actions can be connected to their native subsystem records without making the employment ledger a parallel finance system. Craft/station execution and administrative price/job-opening/rule/task actions remain later slices.
