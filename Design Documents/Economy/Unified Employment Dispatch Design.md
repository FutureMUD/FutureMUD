...
- Added the employment task action catalogue. `tasks actions [all|category|action]` now exposes grouped action metadata, syntax, catalogue status, required authority, required AI capabilities, payment-authorisation requirements, and financial flags. Existing retrieval/delivery, movement, board-post, command, purchase, bank deposit/withdrawal, store-account payment, tax payment, shop-float adjustment, and craft-trigger steps are catalogue-backed; planning/authorisation/report actions (`report`, `authorise`, `reserve`, `release`, `select`, `estimate`, and `route`) are now executable catalogue steps that write operational register entries and durable per-step operational state. The logistics catalogue now includes inventory-plan-backed `load`, `unload`, and `return container` actions plus `vehicle cargo` selection that validates and records accessible cargo spaces without autonomous driving. Real shop purchases now call the native shop sale flow using an employer-backed payment method, store-account payments mutate native line-of-credit balances, shop tax payments use the native shop tax payment path, and craft-trigger steps start native crafts when the worker can perform them. Physical teller trips, employee payment floats, craft output tracking, craft station/tool reservation, pricing, autonomous vehicle driving, animal leading, load optimisation, and administrative mutations remain deferred until they can call their owning subsystem services with complete resource custody.
- Added the first real employment finance foundation. `authorise [<amount> for] <description>` records durable payment authorisation for later financial steps in the same active task; `reserve [<amount> for] <description>` creates durable reservation references in step operational state after checking host available funds; and `release [reservation <id>|all]` clears matching task reservations. Shop-hosted `bankdeposit <amount>` and `bankwithdraw <amount>` are now executable when a covering authorisation/reservation exists: deposits debit only the shop's employer virtual cash before calling the linked native `IBankAccount.DepositFromTransaction`, and withdrawals call native `IBankAccount.CanWithdraw`/`WithdrawFromTransaction` before crediting employer virtual cash. Employment register/ledger rows cross-reference those native finance movements rather than becoming the canonical accounting record. Unsupported host finance adapters and shops without a linked bank account block with diagnostics and do not write partial records.
...
- Additional player/builder command adapters, richer condition expressions (`OR`, grouping, reusable named predicates), real account-transfer/employee-float mutations, physical teller/bank-trip cash handling, physical employer cash-reserve debiting for wage settlement, autonomous vehicle driving, animal leading/return, real craft station/tool use and output tracking, price adjustment, job-opening/rule/task administrative actions, skill/knowledge/tag candidate profiles, and deeper real board command integration are deferred. The completed command-adapter, worker-AI, inventory-movement, payroll-liability, durable step-state, action-catalogue, logistics, shop bank/virtual-cash finance, scheduled-rule authoring, and finance-foundation slices supply read/access views, authorised host-board posting, safe creation of non-financial openings, manager application decisions, drafted retrieval/delivery/planning/finance active tasks, central host-level scheduled-rule evaluation, real shop purchase/store-account/tax/float/craft-trigger adapters where supported, a first live NPC application/task loop, and recoverable overdue-wage reputation pressure without replacing legacy host-specific employment commands.

### Active milestone

- The active milestone has moved past scheduled-rule v1 hardening into action-family closure. Central host-level scheduled-rule evaluation now runs independently of worker heartbeats, and the finance foundation makes supported shop purchase, store-account payment, tax payment, bank/virtual-cash, and register-float actions executable with durable authorise/reserve checks and employment audit cross-references. The next logical implementation slice is craft/station/output closure: reserve craft inputs, validate tools/stations, track active craft progress and outputs, and make crafted outputs available to later delivery steps. After that, account-transfer/employee-float, physical teller trips, autonomous vehicle driving, animal leading/return, cargo optimisation, price adjustment, richer condition expressions, and job-opening/rule/task administrative actions remain later slices.

### Blockers / decisions needed

- No blocker was found for the durable employment persistence, hotel-root split, command-inspection, initial inventory action-step, task-draft, action-catalogue, scheduled-rule authoring, condition-catalogue, or `EmploymentWorkerAI` slices.
- The remaining narrow decision is whether the next production action family should be real purchase/store-account finance or real craft/station execution.
