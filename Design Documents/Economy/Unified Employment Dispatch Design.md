...
5. **Richer scheduled-rule expressions.** Scheduled rules are AND-composed. OR conditions, nested groups, named reusable predicates, richer market-price triggers, and reusable rule templates remain future work.
6. **Manager AI autonomy.** Manager-goal boards exist and can create operational work through services, but there is not yet a broad autonomous manager AI that routinely interprets goals, chooses between purchase/craft/price/account actions, and tunes rules over time.
7. **Candidate profile depth.** Worker matching covers reservation wage, accepted payment methods, host filters, capabilities, pathability, skills, and simple rejection memory. Deeper knowledge checks, world-specific tags, personality preferences, reliability, employer reputation beyond overdue wage tolerance, and local labour-market dynamics are still future slices.
8. **Legacy command replacement.** Several host aliases now route employment views and selected staff operations through the shared model, but the PC-facing `IJobListing`/`IActiveJob` system, `job` command, job-finding cells, job coffers, and remaining legacy host-specific behaviours remain separate by design.

### Active milestone

The recommended active milestone is **native subsystem closure and scheduled-operations polish**:

- add craft input/tool/station reservation and deeper output custody recovery;
...
Recommended slice: deepen craft/resource custody. Add craft input/tool/station reservation before craft start, persist those reservations in active-task step state, verify reserved resources before each craft advancement, and make craft output recovery visible in tasks show/diagnose after reload or failure.
