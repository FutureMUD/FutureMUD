...
- Added central host operations scheduling. `EmploymentHostOperationsScheduler` is now the host-level entry point for scheduled-rule evaluation, manager-goal evaluation, payroll accrual, fixed-term contract lifecycle expiry, and active-task assignment audits, returning countable result summaries for tests and future command/daemon callers. `EmploymentWorkerAI` delegates host-work evaluation to this scheduler instead of directly invoking scheduled-rule and manager-goal boards. Scheduler-wide locking, distributed idempotency, stale grant/reservation cleanup, retry/escalation policies, and capability-provider host discovery remain deferred.
