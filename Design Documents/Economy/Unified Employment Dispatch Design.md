...
- Additional player/builder command adapters, richer manager-goal/scheduled-rule authoring, real purchasing, real crafting, real store-account/account-transfer/employee-float mutations, physical employer cash-reserve debiting for wage settlement, autonomous vehicle driving, animal leading/return, real craft station/tool use, price adjustment, job-opening/rule/task administrative actions, skill/knowledge/tag candidate profiles, and deeper real board command integration are deferred. The completed command-adapter, worker-AI, inventory-movement, payroll-liability, durable step-state, action-catalogue, logistics, and shop bank/virtual-cash finance slices supply read/access views, authorised host-board posting, safe creation of non-financial openings, manager application decisions, drafted retrieval/delivery/planning/finance active tasks, a first live NPC application/task loop, and recoverable overdue-wage reputation pressure without replacing legacy host-specific employment commands.

### Active milestone

- The next active milestone should either extend finance from the shop bank/virtual-cash proof into real purchase/store-account/account-transfer/employee-float adapters, or move into craft/station execution if production testing is more urgent. In both cases, new actions should only become executable when they call their owning subsystem services, reserve/authorise employer resources durably, write native canonical records first, and leave employment ledger/register rows as audit cross-references. Autonomous vehicle driving, animal leading/return, cargo optimisation, price adjustment, and job-opening/rule/task administrative actions remain later slices.

### Blockers / decisions needed

- No blocker was found for the durable employment persistence, hotel-root split, command-inspection, initial inventory action-step, task-draft, action-catalogue, or `EmploymentWorkerAI` slices.
