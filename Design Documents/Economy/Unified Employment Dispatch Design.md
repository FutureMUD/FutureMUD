...
Last reviewed against the implementation on 2026-06-01.
...
- Additional player/builder command adapters, richer condition expressions (`OR`, grouping, reusable named predicates), real account-transfer mutations, physical employer cash-reserve debiting for wage settlement, autonomous vehicle driving, animal leading/return, deeper craft input/tool/station reservation semantics, price adjustment, job-opening/rule/task administrative actions, skill/knowledge/tag candidate profiles, deeper real board command integration, and hotel-room/rental normalization are deferred. The completed command-adapter, worker-AI, inventory-movement, payroll-liability, durable step-state, action-catalogue, logistics, broadened host finance, scheduled-rule authoring, finance-foundation, physical-float, exact-stock purchase, and craft-progress slices supply read/access views, authorised host-board posting, safe creation of non-financial openings, manager application decisions, drafted retrieval/delivery/planning/finance/craft active tasks, central host-level scheduled-rule evaluation, real shop purchase/store-account/tax/float/craft-trigger adapters where supported, a first live NPC application/task loop, and recoverable overdue-wage reputation pressure without replacing legacy host-specific employment commands.

### Current gap map

The foundation is now testable for live NPC employment, scheduled rules, exact-stock purchases, physical movement, host finance, and native craft progress. Remaining gaps are narrower and mostly fall into native subsystem closure rather than the original host-shell architecture.

1. **Craft custody and reservation depth.** Craft steps can start/resume native crafts and adopt outputs, but they do not yet reserve exact inputs, tools, station capacity, or intermediate products before craft start. A failure partway through a production chain can still require manager diagnosis rather than automatic resource recovery.
2. **Commodity purchase granularity.** Commodity purchase steps now buy exact selected stock items, but they still purchase whole matching stocked items. If a stocked item contains more commodity weight than the task requested, the system does not yet split or measure out only the requested commodity weight unless the underlying item naturally supports that split through the existing shop/item model.
3. **Account transfer and broader finance targets.** Bank deposit/withdrawal, store-account payment, tax payment, register float, physical float, and supported host virtual-cash movement are executable. Cross-account transfers, arbitrary host-to-host settlement, wage-reserve physical-cash backing, and richer employee-float recovery policies still need native service boundaries.
4. **Autonomous vehicles and animals.** Vehicle cargo selection validates and records accessible cargo spaces, but autonomous driving, animal leading, mount return, route optimisation, load balancing, and multi-stop delivery batching remain deferred.
5. **Administrative task mutations.** Price changes, job-opening administration, scheduled-rule administration, active-task administration from inside tasks, and manager-goal mutation are still command/service surfaces rather than task-executable actions. That is deliberate to avoid recursive task mutation until the audit and authority model is tighter.
6. **Richer scheduled-rule expressions.** Scheduled rules are AND-composed. OR conditions, nested groups, named reusable predicates, richer market-price triggers, and reusable rule templates remain future work.
7. **Manager AI autonomy.** Manager-goal boards exist and can create operational work through services, but there is not yet a broad autonomous manager AI that routinely interprets goals, chooses between purchase/craft/price/account actions, and tunes rules over time.
8. **Candidate profile depth.** Worker matching covers reservation wage, accepted payment methods, host filters, capabilities, pathability, skills, and simple rejection memory. Deeper knowledge checks, world-specific tags, personality preferences, reliability, employer reputation beyond overdue wage tolerance, and local labour-market dynamics are still future slices.
9. **Hotel normalization.** Durable hotel root rows exist, but rooms, furnishings, rentals, bans, patron balances, lost property, tax detail, and most hotel cash/bank internals remain in the compatibility XML model until a later hotel-split migration.
10. **Legacy command replacement.** Several host aliases now route employment views and selected staff operations through the shared model, but the PC-facing `IJobListing`/`IActiveJob` system, `job` command, job-finding cells, job coffers, and remaining legacy host-specific behaviours remain separate by design.
11. **Central evaluation host discovery.** Worker AI avoids passive hotel lazy creation by using existing hotel roots only. The central scheduled-rule evaluator should receive the same explicit-host treatment before very large worlds rely on it heavily, so it does not accidentally instantiate durable hotel roots or employment host state while merely scanning all properties.

### Active milestone

The recommended active milestone is **native subsystem closure and scheduled-operations polish**:

- harden central scheduled-rule host discovery so passive scans only inspect existing durable hotel roots and adopted employment hosts;
- add craft input/tool/station reservation and output custody recovery;
- add commodity splitting or explicit "whole stocked item" diagnostics for commodity purchases;
- define native service boundaries for account transfers, wage-reserve cash backing, and richer physical employee floats;
- then move to autonomous vehicle/animal logistics, richer condition expressions, price/admin actions, and manager AI autonomy.

## 27. Suggested next Codex goal

```text
/goal Close the remaining native-subsystem gaps in the Unified Employment Dispatch implementation. Start from Design Documents/Economy/Unified Employment Dispatch Design.md and preserve its invariants: task routing is through employment task boards, host staff boards are communication only, legacy IJob remains separate, and all employer-money or employer-stock actions require delegated authority, durable authorisation/reservation, native subsystem records where available, and employment register/ledger evidence.

Recommended slice: harden scheduled-rule host discovery and craft/resource custody. Ensure central scheduled-rule evaluation only scans adopted employment hosts and existing durable hotel roots without lazy-creating property hotels. Then add craft input/tool/station reservation before craft start, persist those reservations in active-task step state, and make craft output recovery visible in tasks show/diagnose after reload or failure.
