...
- seeded medical knowledges now install stock chargen availability progs so they are offered only for appropriate medical skills rather than every selected skill
- when `ChargenFreeKnowledges` exists, the seeder reconciles a marked stock section that grants installed stock medical knowledges for free whenever a selected skill passes the knowledge's existing acquisition prog; a later `ChargenSeeder` run performs the same repair when Health was installed first
...
- `InstallProstheticProcedure` now gives stock prosthetic items a formal surgery-framework fitting path in addition to the generic attach command, with the stock exposure phase treating visible severed roots as valid fitting access.
