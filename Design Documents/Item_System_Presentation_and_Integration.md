...
- the current shipped built-in applications are `SysMon`, `FileManager`, and `Directory`
...
- `Directory` presentation is terminal-session oriented in a lighter-weight way: it opens with a local host summary and then continues as a foreground `UserInput()`-driven session where `type` commands browse built-in services, mounted storage devices, connected terminals, and local network adapters without implying broader network discovery that the current slice does not yet implement
