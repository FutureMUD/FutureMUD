...
- `SysMon` presentation is explicitly terminal-session oriented: it writes a diagnostics report to the connected terminal session covering host power and storage state, connected terminals and storage devices, network adapters, running processes, and accessible local signal sources and sinks
...
- `Directory` presentation is terminal-session oriented in a lighter-weight way: it opens with a local host summary and then continues as a foreground `UserInput()`-driven session where `type` commands browse built-in services, mounted storage devices, connected terminals, and local network adapters, and now also list reachable telecom-backed hosts with `hosts`, inspect one with `show <host>`, and inspect its advertised remote services with `services <host>`
- `Directory` and `SysMon` should present network discovery in terms of canonical address, stable device id, and access-route summary so players understand whether a host is public, exchange-local, or VPN-scoped without having to infer it from topology alone
...
- future authorised tunnelling, VPN login, or hacking flows should build on that same presentation model by making new route memberships visible when they are gained, rather than replacing the existing host and adapter diagnostics
