...
- the current shipped built-in applications are `SysMon`, `FileManager`, `Directory`, `Mail`, and `FTP`
...
- remote-directory presentation should now show `Mail` and `FTP` when a reachable host has those network services enabled and actually configured, including domain detail for mail and public-file-count detail for FTP rather than only the bare service name; reserved identities like `Boards` and `Messenger` must still stay hidden until they really ship
