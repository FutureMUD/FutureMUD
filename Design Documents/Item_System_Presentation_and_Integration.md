...
- the current shipped built-in applications are `SysMon`, `FileManager`, `Directory`, and `Mail`
...
- remote-directory presentation should now show `Mail` when a reachable host has that network service enabled and at least one hosted domain, including domain-detail text rather than only the bare service name; reserved identities like `Boards` and `Messenger` must still stay hidden until they really ship
