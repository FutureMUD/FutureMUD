...
- powered machine automation modules now also perform a short post-login heartbeat retry if they are switched on but could not yet resolve a usable power source during the first login pass, which covers reboot load-order races for mounted modules on powered hosts
