...
Stage 2 ordinary content lives in embedded, domain-separated TSV files under `DatabaseSeeder/Seeders/IndustrialisedCatalogue`. The current `scripts/sync-industrialised-item-catalogue.ps1` refresh regenerates the draft 24-domain source set, craft/outfit tables and `Industrialised_Item_Catalogue_Audit.tsv`; it is not yet safe for preserving curated clothing text. Its `-Check` mode proves draft-generator parity, not production readiness. Before bulk clothing authoring, convert that path to validate/package authored source without assembling prose or overwriting it. The strict loader requires exact headers, invariant numbers, stable ordering and source-file/line diagnostics. Existing graph preflight and provenance infrastructure must be extended to the accepted clothing contract before its new records are created.

The [Industrialised clothing, footwear and uniforms specification](../Seeding/FutureMUD_Industrialised_Clothing_Footwear_Uniforms_Design_Reference.md) is authoritative for garment-family coverage, complete layered outfits, physical/economic base-versus-skin boundaries, period-sensitive production, finished authored descriptions and seven acceptance gates. Variable colours are the default; conventional religious, ceremonial, mourning and occupational colours are overridable outfit defaults, not base/skin restrictions. Wave 1 approves no fixed-colour exception. Resolve explicit instance choices before entry defaults and ensemble palettes, reject unsupported selections, and preserve coordinated colours on reload. Runtime outfit entries already support skins, load arguments, placement and wear order, and craft products can bind skins; the current Industrialised TSV contracts still need to expose those capabilities. Keep skin quality overrides unset by default so they do not hide earned craft quality. A passed count check or the current Industrial-selectable flag does not close editorial, manufacturing or live outfit acceptance.
