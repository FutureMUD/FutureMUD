...
The Renaissance ItemSeeder selection installs the eleven canonical medieval clothing and footwear prototypes required by its Latin Mass priest and Buddhist formal officiant outfits, even when Medieval is not selected. Both eras call the same stock definitions; selecting both creates them once, without importing the rest of the medieval clothing catalogue into Renaissance-only installs. Stable references and stock ownership reconciliation remain unchanged.
...
The maintained `Item_Component_Types.json` registration snapshot exports this restriction for every canonical component type. Seeder catalogue and outfit preflight should consume that fact when they need to prove that a persisted template can be materialised; do not duplicate a hand-maintained deny-list. A preserved outfit must also resolve current prototype/component/skin revisions and deterministic characteristic arguments before any stock update is applied.
...
Stage 2 ordinary content lives in embedded, domain-separated TSV files under `DatabaseSeeder/Seeders/IndustrialisedCatalogue`. `scripts/sync-industrialised-item-catalogue.ps1` now reads the source TSVs and refreshes only `Industrialised_Item_Catalogue_Audit.tsv`; it never assembles or rewrites prose. Its `-Check` mode proves source/audit parity, not production readiness. The normalized clothing sources and remaining integration obligations are recorded in the [Wave 2 evidence register](../Seeding/Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md). The strict loader requires exact headers, invariant numbers, stable ordering and source-file/line diagnostics. Existing graph preflight and provenance infrastructure must be extended to the accepted clothing contract before its new records are created.

The [Industrialised clothing, footwear and uniforms specification](../Seeding/FutureMUD_Industrialised_Clothing_Footwear_Uniforms_Design_Reference.md) is authoritative for garment-family coverage, complete layered outfits, physical/economic base-versus-skin boundaries, period-sensitive production, finished authored descriptions and seven acceptance gates. Variable colours are the default; conventional religious, ceremonial, mourning and occupational colours are overridable outfit defaults, not base/skin restrictions. Wave 1 approves no fixed-colour exception. Resolve explicit instance choices before entry defaults and ensemble palettes, reject unsupported selections, and preserve coordinated colours on reload. Runtime outfit entries support skins, load arguments, placement and wear order, and craft products can bind skins; Wave 2 connects normalized clothing TSVs to their database-backed resolution, preflight and persistence paths. Keep skin quality overrides unset by default so they do not hide earned craft quality. A passed infrastructure gate or the current Industrial-selectable flag does not close editorial, manufacturing or live outfit acceptance.

Every unskinned base garment is a complete standalone stock and craft product, usable in outfits without a skin. Its default presentation requires finished prose, valid colour configuration and truthful physical/economic properties; it is not a placeholder or a template-only record. Where a proposed plain skin supplies that default, author it as the base presentation and count it once, not as an additional skin. Validate loading, crafting, wearing and persistence with no skin as well as with additional presentations.

Standalone colour preflight validates every possible random value of the actual Variable component profile against the base's authored palette. A source-table default alone does not constrain bare item creation. Restricted material-native palettes require compatible stock profiles. Explicit skin/outfit/craft selections can narrow choices through exact load arguments; any production skin that narrows a palette must prove its ordinary load and later-change behaviour before Gates 3–5 accept that presentation.

Historical source corrections now give 51 reused garments complete authored variable-colour descriptions and Variable components, including three material-specific profiles. Their conventional white, black, red, saffron or maroon selections are historical outfit-entry defaults, not restrictions on standalone stock. The historical exporter copies shared complete descriptions verbatim and preserves an explicitly authored entry choice ahead of a conventional default. CoreDataSeeder now disambiguates 34 secondary legacy Colour lookup names without changing IDs or displayed colours, and anchors stock general/numbered patterns to prevent channel overlap. Real-stock tests resolve complete component profiles for all 364 approved inventory rows, including all 51 corrected sources. This proves stock binding integrity, not approval of every material-native palette, unrestricted skin change or full outfit. Customised ambiguities remain untouched and fail exact preflight; full fresh/populated-world execution is still required.

Historical clothing reuse resolves the approved 113 identities through shared authoritative definitions, preserving original owners and expanding admissions rather than requiring installation of an earlier-era package. Its preflight checks unmanaged/legacy stock signatures and provenance IDs without adopting or mutating records. Existing component links resolve their actual `(Id, RevisionNumber)`; the shared item reconciliation writer retains the attached logical component's revision instead of appending a second revision when newer stock becomes current. Whole-inventory physical/colour binding and full-batch ownership are part of the passed Wave 2 Gate 2 evidence.

Read-only clothing preflight projects the actual component writer: exact untouched managed items may replace obsolete component relationships, while a missing-link repair remains additive. Continuing logical components retain their attached revisions. Unknown or duplicate links omitted by the name-only fingerprint block replacement; customised aggregates are validated as installed and never silently repaired to fit an outfit. Component definitions and live item instances are not deleted.

HumanSeeder supplies 43 added wear configurations and 109 consumed layer variants, including narrow knee-band garters and explicitly non-bulky variants for selected under-/over-robes, lounge jackets and rain shells. Shared baseline components stay unchanged; variants retain source profiles/defaults and all other supported flags. Existing variants and revisions remain untouched on rerun, and fresh/baseline-upgrade parity is tested. All 364 approved bases resolve stock geometry, and all 134 proposed outfits pass actual runtime Direct/Shape footprint and layer checks in four stock-anatomy fixtures, both on designed-body locations and derived locations that nontrivially `CountsAs` them. These tests are not race/gender/age sizing, attachment capacity or finished-outfit materialisation proof. Fractional XML values load invariantly across host cultures. See the [ensemble checkpoint](../Seeding/Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#stock-anatomy-ensemble-layering-and-safe-component-replacement).

The entire admitted normalized skin batch now receives read-only ownership/signature checks before clothing application. The historical skin writer uses that same check for its whole requested batch before changing even the first skin. Duplicate current revisions/names, conflicting provenance, unmanaged drift, non-current-only targets and preserved customised skins bound to the wrong base reject. Exact unmanaged stock is adopted only during application; compatible custom text and renamed managed skins retain their logical IDs, including renamed later revisions. This does not prove arbitrary customised-skin colour semantics. The [skin ownership checkpoint](../Seeding/Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#skin-batch-ownership-before-mutation) records the scope and regressions.

Outfits receive equivalent read-only whole-batch target/signature preflight before clothing application and in the historical outfit writer. Duplicate names or stock markers, incompatible provenance, competing ownership and unmanaged signature drift reject before any earlier template is changed. Managed templates retain their identity after compatible builder renames or marker removal. Live signatures resolve managed skins by logical ID and stable provenance key, so a renamed skin does not falsely make an unchanged outfit customised. Preserved templates also validate their complete current item/component/skin/profile/colour/relation graph from immutable snapshots before mutation. The [outfit ownership checkpoint](../Seeding/Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#outfit-batch-ownership-before-mutation) distinguishes infrastructure proof from later finished-outfit materialisation.

Normalized clothing crafts preflight their whole admitted recipe/access-prog batch before clothing mutation and before the adapter applies its first recipe. Existing unmanaged crafts require complete signatures; managed targets resolve unique current revisions and retain valid customisations or renames. New recipes may plan access progs that do not yet exist. Conflicting names/ownership, duplicate current targets, incompatible callable gate contracts and conflicting prospective gate definitions reject before earlier recipe/prog creation. Shared identical gates remain valid. Preserved crafts additionally validate their complete current phase/input/tool/product graph, runtime-supported XML references, hook signatures and dependency-before-product timing. Application uses tracked traits rather than attaching detached preflight objects during updates. Actual production recipe stock, failure behaviour and tools remain Gate 5 content work.

New physical clothing bases now use complete target/signature ownership preflight, including unique current revisions, unmanaged exact adoption and competing ownership. Their physical preflight checks the retained or projected component graph, not just the proposed source graph. Managed unique-name changes remain customized and retain the original logical item. The complete new-clothing ownership batch is rechecked before its first item write. First-install flushes also reconcile generated outfit/prog/knowledge IDs against their exact created entities, validating the entire pending batch before persisting IDs; an unchanged rerun is no longer required to establish ownership. The [first-install checkpoint](../Seeding/Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#first-install-identities-and-new-base-ownership) supersedes the earlier item-batch/final-ID gaps above, but not actual preserved outfit/recipe validation or relational transaction proof.

The [Wave 1 scope review](../Seeding/Industrialised_Clothing_Wave1_Evidence_and_Coverage.md) links the garment-first inventory, ordered ensemble proposals and source-reuse decisions. Gate 1 passed on 2026-09-03 with the user's approval of current counts and overall scope, incorporating conventional colours as outfit defaults. Infrastructure/dependency Gate 2 passed on 2026-09-04; Gates 3–7 remain open. The planning checker neither installs items nor accepts descriptions, crafts or finished wearability.

The [Industrialised food and drink design reference](../Seeding/FutureMUD_Industrialised_Food_Drink_Design_Reference.md) governs later food catalogue work. Every edible base must remain complete outside logistics-owned packaging; rare skins cannot change ingredients, taste, portion, freshness, quality or mechanics. Exact serving manifests compose separate foods, liquids and eventual vessels without creating a player-facing meal entity. Gate 1 passed on 4 September 2026. Gate 2 now supplies the strict 464-concept/307-adoption source graph, factual prepared-food metadata, health-owned ingestion profiles and persisted liquid freshness; its dependency matrix still awaits editorial acceptance. Sealed-package integration remains only an interface boundary.

Clothing preflight also checks actual component capabilities and revisions, Wearable XML and Direct/Shape wear-profile geometry against a read-only body/shape snapshot. It rejects malformed or unresolved references, empty profiles, impossible mandatory shape counts and garments exceeding the world's layer limit even alone. Ordered ensembles reject definite layer overloads and bulky conflicts before item/provenance mutation; the complete planning set is also exercised through the runtime profile resolver, including nontrivial bodypart equivalence. This closes the Gate 2 dependency boundary. Race/gender/age sizing, exact authored attachment relations and materialised finished outfits remain Gate 5 work; the dormant `IWearableSize` model is not currently consulted by `Body.CanWear`.

The ItemSeeder manifest contract is version 3. Its source fingerprint includes every consumed Industrialised TSV plus the existing food and medical catalogues, and canonical capture uses the neutral technology profile while traversing inactive shared admissions. Outfit fingerprints now include actual item/skin references, entry keys, wear profiles, placement, related entries, colour arguments and order; numeric colour IDs are normalized to stable value identities. Old manifest files require recapture. Untouched legacy default outfit fingerprints can be upgraded, but customized extended fields are preserved. Profile choice changes compatible components, not stable identities; once managed `shared-industrialised` or `industrial` records exist, normal reruns reject a changed normalized profile before stock creation.

Clothing Wave 2 binds exact characteristic/profile/value names to live IDs, validates complete channels and garment-supported wear profiles, and routes normalized skin/outfit rows through the established appliers. Admitted draft records fail preflight. Stock skin quality may remain null. Normalized crafts compile to the existing writer using stable item/skin/craft identities and unchanged authored prose; invalid consumption/production timing is rejected, not patched with generated echoes. The complete approved dependency plan and whole-batch prerequisite/ownership paths passed Gate 2. The production tables remain intentionally unpopulated pending Gate 3 review; nonempty canonical capture and fresh/populated relational runs belong to later gates.

The existing `SimpleVariableProduct` craft type supports mixed inherited and deliberately selected values. Seeder imports retain `variable Colour=$i1` for inheritance and add `fixedvariable Accent=white` for exact selection; the latter selects a crafted instance's characteristic, not a fixed-colour garment or skin. Each definition has exactly one rule. Duplicate or incompatible rules are rejected, and runtime resolution stops before output creation if a consumed input cannot supply its declared value. The normalized adapter requires complete channels and rejects incapable inputs or inherited value sets that exceed the output palette. `UnusedInput` recovery is item-count based, not mass recovery, and the adapter rejects incompatible uses. A future recipe needing narrower inherited input filtering must use an exact selected product or add a separately supported filter before Gate 5 admission.

The developer sync command maintains the ordinary item audit, [clothing source-graph audit](../Seeding/Industrialised_Clothing_Catalogue_Audit.tsv) and [full-inventory dependency plan](../Seeding/Industrialised_Clothing_Dependency_Plan.tsv). The source-graph audit accounts for every row of all 13 clothing tables, including ordered children, exact references, default colours and craft mappings, while preserving complete source text and null skin quality. The dependency audit separately reconciles all 364 approved bases, 697 skin briefs and 134 outfit consumers; it lists candidate material/component/tag names, proposed wear geometry/layers and exact missing stock or capabilities. These are prerequisite decisions, not authored item descriptions or physically validated installations. Tests prove source-byte preservation, stale-audit detection and no partial refresh when graph or scope validation fails. Audit labels explicitly distinguish structural/name validation from database and production acceptance; refresh never authors catalogue prose.
