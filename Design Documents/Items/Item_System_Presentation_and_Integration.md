...
Restaurant delivery presents the outermost real item at the named recipient's designated table: a plated dish remains visibly on its plate in the table container, while takeaway can arrive in a carton and optional bag. The contained meal remains part of ordinary item inspection and weight behaviour. Empty serving containers can be cleared only from a designated table and only after their contents are gone; an employed server visibly gets the item, carries it to the kitchen, and sets it aside on the configured cleanup cadence rather than making it disappear. Chef start/open/plate/ready and server serve/clear/return events each produce an observable emote, with restaurant-level templates allowing the venue's own service voice. Menu and bill presentation uses the shop currency and identifies orderer, recipient, service state, paid amount, and outstanding amount so ordering for another participant does not obscure the actual debtor.
