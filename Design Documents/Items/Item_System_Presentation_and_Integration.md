...
Colour characteristic lookup names and their displayed colour text are distinct. Core stock secondary lookup names may include a qualifier such as `banana yellow (stock colour 284)` to disambiguate retained legacy identities; `ColourCharacteristicValue.GetValue`, basic and fancy descriptions still use the unchanged referenced Colour. Numeric characteristic selections continue to refer to the same IDs. Use quotes for spaces or punctuation in load arguments, for example `colour="banana yellow (stock colour 284)"`; hyphenated single-token values such as `colour=blue-green` also resolve without truncation. The stock general and numbered colour patterns are anchored so `finecolour` cannot be mistaken for `colour`. ItemSeeder preflight resolves exact names to numeric arguments and still rejects real ambiguity; the permissive runtime builder lookup is not a substitute for that validation.
...
Outfit templates expose batch equipment creation through `loadoutfittemplate(number templateId, character target)` and `loadoutfittemplate(text templateName, character target)`, with overloads that add a text outfit-name override. Four-argument overloads add `loadArgs` after `outfitName`; pass blank text for `outfitName` to keep the template's normal name while still applying load arguments. Template-level load arguments are appended after each item's own load arguments so runtime calls can apply common variable values such as colour across the whole outfit. The function returns the normal created `outfit`. Scripts can iterate `outfit.items`; each `outfititem` exposes `item`, and the existing item `proto` dot returns the created item's prototype id. This keeps prototype inspection on the item scripting surface instead of adding a dedicated FutureProg outfit-template variable type.
...
Readable book items expose a small scripting surface for dynamic document generation. `addprintedwriting(item, page, text, language, script, provenance)` creates a new immutable provenance-aware printed writing and attaches it to a book page, with an overload accepting colour and style text. `copywritingto(item, page, writing)` attaches an existing immutable writing reference to a book page without cloning the original row. `setbooktitle(item, title)` changes the live book title. These functions return false instead of mutating internals when the item is not a book, the page is invalid or torn, a language/script/colour/style cannot be resolved, or the page lacks capacity.
...
Seals and measuring tools add a small player-facing command surface. `seal <target> with <stamp> [using <medium>]`, `break seal <target>`, `inspect seal <target>`, and `compare seal <sealed item> with <stamp|sealed item>` operate on `ISealStamp` and `ISealable`; ordinary read, write, draw, retitle, and open interactions break an intact seal before continuing only when the actor can manipulate the item. If the actor can see but cannot manipulate a sealed item, the access command reports the normal manipulation error and leaves the seal intact. `weigh <item> on <instrument>`, `measure <target> with <instrument>`, `calibrate <instrument>`, `calibrate <instrument> wrong <+/-amount|+/-percent>`, and `inspect calibration <instrument>` operate on `IMeasuringInstrument`.
...
- `NetworkAdapter` decorates the full description with its switch state, power state, connected host, attached telecommunications grid, canonical published address, stable device id, access route summary, and whether it is currently network-ready
- `NetworkSwitch` decorates the full description with its switch state, power state, upstream transport source, attached telecommunications grid, and how many downstream ports are currently occupied
- `WirelessModem` decorates the full description with its switch state, power state, connected host, current cellular-coverage source, canonical published address, stable device id, access route summary, and whether it is currently network-ready
...
- `FileSignalGenerator` decorates the full description to show its switch state, power state, designated signal file, and currently emitted numeric value
...
- `programming terminal status` is now part of that player-facing integration surface as well, because it is how players see which powered host they are currently connected to, whether they are editing the host itself or one mounted storage device, and whether the session currently has any temporary tunnel routes active
- `programming apps` and `programming app <name>` are now part of that same integration surface, because built-in host applications are no longer just catalog entries; they are visible and executable on the connected powered host
- `programming network` is now part of that same integration surface for administrators, because shared hosted domains, shared `user@domain` accounts, and exposed VPN gateway networks are configured against the live connected host rather than as a separate builder-only subsystem
- `programming mail` is now part of that same integration surface for administrators, because host mail-service enablement, hosted-domain bindings, and mail-account management are configured against the live connected host rather than as a separate builder-only subsystem
- `programming boards` is now part of that same integration surface for administrators, because board-service enablement and hosted-board exposure are configured against the live connected host rather than as a separate builder-only subsystem
- `programming ftp` is now part of that same integration surface for administrators, because FTP enablement, FTP-account management, and public-file publishing are configured against the live connected host or selected storage owner rather than as a separate builder-only subsystem
- `programming item <item> file ...` is now part of that same integration surface for automation authors, because file-backed signal generators are edited and published directly on the live item rather than through a separate builder-only file tool
...
- the current shipped built-in applications are `SysMon`, `FileManager`, `Directory`, `Mail`, `FTP`, and `Boards`
- `SysMon` presentation is explicitly terminal-session oriented: it writes a diagnostics report to the connected terminal session covering host power and storage state, connected terminals and storage devices, network adapters, hosted VPN networks, active session tunnels, running processes, and accessible local signal sources and sinks
- `FileManager` presentation is likewise terminal-session oriented: it opens with a file-target overview, then continues as a foreground `UserInput()`-driven session where `type` commands list, show, edit, write, append, delete, copy, and retarget files across the host and mounted storage devices
- component-backed file owners such as `FileSignalGenerator` should present through `FileManager` and `FTP` using parent-item / component notation rather than raw ids, so players understand that they are editing a subordinate device file rather than the host root file system
- `type edit <file>` should feel like ordinary in-game document editing rather than a bespoke terminal mini-language: it hands off to the normal multiline editor, preserves the file's current text as recalled content, and uses the editor's existing `@` submit and `*cancel` flows
- `Directory` presentation is terminal-session oriented in a lighter-weight way: it opens with a local host summary and then continues as a foreground `UserInput()`-driven session where `type` commands browse built-in services, mounted storage devices, connected terminals, and local network adapters, list reachable telecom-backed hosts with `hosts`, inspect one with `show <host>`, inspect its advertised remote services with `services <host>`, and manage tunnel state through `routes`, `gateways`, `tunnel connect ...`, and `tunnel disconnect ...`
- `Directory` and `SysMon` should present network discovery in terms of canonical address, stable device id, base access-route summary, and active session tunnels so players understand whether a host is public, exchange-local, or VPN-scoped without having to infer it from topology alone
- that access-route presentation is also the current UX guardrail against clutter: devices on unrelated private exchange subnets or VPNs should simply not appear as reachable remote hosts, rather than appearing as unusable noise in a global directory listing
- authorised tunnelling and VPN login are now part of that same presentation model: when a session gains a temporary tunnel route it should become visible in `Directory`, `SysMon`, and terminal status rather than silently changing discovery behaviour
- future hacking flows should build on that same presentation model by making newly granted or stolen temporary route memberships visible when they are gained, rather than replacing the existing host and adapter diagnostics
- `Mail` presentation is also terminal-session oriented: it opens with login guidance, then continues as a foreground `UserInput()`-driven client where `type` commands authenticate, browse mailbox state, read and delete entries, and compose drafts using the normal multiline editor for the message body
- `Boards` presentation is terminal-session oriented in the same family: it opens with reachable-host guidance, then continues as a foreground `UserInput()`-driven client where `type` commands select a board-service host, authenticate with a shared `user@domain` identity, browse hosted boards, read posts, and compose new posts using the normal multiline editor for the post body
- `FTP` presentation is terminal-session oriented in a more operations-focused way: it opens with reachable-host guidance, then continues as a foreground `UserInput()`-driven client where `type` commands open a remote host, inspect reachable public files anonymously, and optionally log in for full host or mounted-storage file manipulation
- `FileManager` now also has a lighter-weight public-network presentation path for reachable hosts: `type list public <host>`, `type show public <host> <file>`, and `type copy public <host> <file>` should feel like ordinary file browsing rather than like a separate remote-login shell
- remote-directory presentation should now show `Mail`, `FTP`, and `Boards` when a reachable host has those network services enabled and actually configured, including domain detail for mail, public-file-count detail for FTP, and hosted-board detail for Boards rather than only the bare service name; `Messenger` should still stay hidden until it really ships
...
- for file-backed signal generators, `electrical` and `programming` inspection should also present the designated signal file name, current parse status, and whether that file is anonymously published for remote file access
...
## Modern Appliance Presentation and Integration

Refrigeration is intentionally an environmental simulation rather than a visible freshness-number overlay. Players interact with ordinary container doors and power infrastructure; builders see the four authored rates and the morph-sensitivity flag. Always-open display refrigerators retain ordinary open-container presentation. Implant refrigerators follow the implant's powered/function state without exposing a separate switch.
