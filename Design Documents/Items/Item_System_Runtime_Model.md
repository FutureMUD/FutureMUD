...
Room-level liquid uses the same model as bounded per-layer cell surface state. Rain, overflow, blood loss, magic-created liquid, and deliberate pouring add to that state rather than creating saved puddle game items. Legacy puddle items remain loadable for database compatibility; when a layer is resolved with puddles enabled, their mixtures are merged into the cell surface and the legacy items are deleted before the visible-item snapshot is taken. This prevents a migrated room from presenting both a virtual puddle and an older physical puddle for the same accumulated liquid.
