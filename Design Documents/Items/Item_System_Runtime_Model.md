...
Restaurant preparation does not clone a conceptual "dish" into a player inventory. It removes a real stock item or receives a real configured craft output, moves it through the restaurant kitchen, optionally opens it, and then inserts it into real newly-created configured containers. `CraftAndPlate` creates a serving-container parent; takeaway may add an inner container and then an outer bag. The order retains persistent records for product, plate, package, and bag ids, with the outermost layer selected for delivery. A dine-in server gets that outermost item, physically carries it to the registered table, and places it in the table's container instead of silently granting it to the diner. The diner can therefore use ordinary `eat ... from <plate> on <table>` handling, and the later server cleanup retrieves only an empty, persisted serving-container id. This preserves ordinary deep-item traversal and leaves a recoverable audit trail if a container is missing, destroyed, or cannot accept the item.
