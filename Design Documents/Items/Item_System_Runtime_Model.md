...
Deliberate poison coatings are a saved item effect rather than a weapon component. `WeaponPoisonCoating` implements `IWeaponPoisonCoatingEffect` and derives from the same liquid-contamination model used for ordinary wet items, so it contributes liquid description, cleaning, added weight, transfer, and drying behaviour. The dedicated effect is the combat-deliverable marker: normal `LiquidContamination` from spilling, rain, washing, or other incidental liquid exposure remains descriptive/reactive and does not deliver poison through weapon hits.
