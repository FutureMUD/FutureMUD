...
| Builder/runtime token | Class | Subsystem | Where registered or dispatched | Builder-creatable | Purpose |
| --- | --- | --- | --- | --- | --- |
| `astralprojection` | `AstralProjectionSpellEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/AstralProjectionSpellEffect.cs` via `SpellEffectFactory` | Yes | Ensures or reuses a keyed astral form, spawns a temporary player-focusable secondary instance, applies astral planar presence, anchors the primary body, and collapses only the projection when the spell ends |
| `animatecorpse` | `AnimateCorpseSpellEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/DirectPossessionSpellEffects.cs` via `SpellEffectFactory` | Yes | Animates a corpse item's `OriginalBody` as a temporary scripted-AI `AnimatedCorpse` actor with configured AIs, hides and restores the same corpse item, and supports `dispelmagic effect animatecorpse` from either the hidden corpse effect or animated actor proxy |
...
| `createclone` | `CloneSpellEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/CopyCloneSpellEffects.cs` via `SpellEffectFactory` | Yes | Ensures or reuses a keyed tangible clone form, spawns a cell-local physical-clone secondary instance, and cleans up according to configured persistence and clone death policy without final-killing the identity |
| `createcopy` | `CopySpellEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/CopyCloneSpellEffects.cs` via `SpellEffectFactory` | Yes | Ensures or reuses a keyed magical-copy form, spawns a temporary cell-local copy that may be passive or player-focusable, and collapses without creating body remains |
...
| `phantomillusion` | `PhantomIllusionEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/MagicPhase3Effects.cs` via `SpellEffectFactory` | Yes | Adds non-interactive phantom room LOOK text for eligible scoped viewers, with priority, colour, viewer-prog, clan, and illusion-key controls |
...
| `rejuvenateland` | `RejuvenateLandEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/RejuvenateLandEffect.cs` via `SpellEffectFactory` | Yes | One bounded gradual scar-repair treatment per physical cell; direct room/rooms invocation only |
...
| `seizebody` | `SeizeBodySpellEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/DirectPossessionSpellEffects.cs` via `SpellEffectFactory` | Yes | Temporarily moves the caster's controller into a living target body, parks PC victims in a bound spectator context, pauses/restores NPC control, and supports `dispelmagic effect seizebody` |
...
| `subjectivedesc` | `SubjectiveDescriptionEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/MagicPhase3Effects.cs` via `SpellEffectFactory` | Yes | Adds audience-scoped subjective full-description replacement with priority, optional clan/viewer-prog gates, and optional illusion key |
| `subjectivesdesc` | `SubjectiveSDescEffect` | Spell effect | Static `RegisterFactory` in `MudSharpCore/Magic/SpellEffects/MagicPhase3Effects.cs` via `SpellEffectFactory` | Yes | Adds audience-scoped subjective short-description replacement with priority, optional clan/viewer-prog gates, and optional illusion key |
...
| n/a | `IIllusionEffect` | Shared illusion metadata and audience-scope contract for subjective description and phantom room illusions |
| n/a | `IPrioritisedOverrideDescEffect` | Subjective-description priority and illusion-key contract used by body/item description override resolution; keyed dispel matching now uses `IIllusionEffect` so phantom room illusions participate too |
...
| Token or API | Class or interface | Role |
| --- | --- | --- |
| `PossessedCorpse` | `CharacterInstanceKind.PossessedCorpse` | Temporary character-instance kind for animated corpse actors using the corpse's `OriginalBody` and `PlayerRemoteCommandable` control policy |
| `AnimatedCorpse` | `CharacterInstanceKind.AnimatedCorpse` | Temporary character-instance kind for AI-controlled corpse actors using the corpse's `OriginalBody` and `ScriptOnly` control policy |
...
| n/a | `PossessionVictimContext` | Bound spectator `IControllable` context for hostile PC victims while another controller commands their body |
