...
`mindbroadcast` persists its range as `PowerDistance`; loaders also accept the older `Distance` element. Broadcast and directed `mindsay` text is sanitised before it is embedded into the internal emote used for psychic speech delivery. This includes the sender's `mindbroadcast` echo when language handling is enabled and its template substitutes the message with `{0}`. Literal composite-format braces in player-authored messages are escaped with the shared `Sanitise()` helper while constructing the emote and restored when it is rendered. The shared emote renderer also reports malformed formats as emote errors rather than propagating a format exception into the player-input loop.
