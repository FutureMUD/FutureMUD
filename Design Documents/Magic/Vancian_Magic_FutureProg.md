...
Character queries and mutations use the canonical identity for persisted choices, ledger and progression. Physical access, acting-body requirements, costs and scroll potency use the actual actor where specified. Queries do not initialise capacity, refresh, write state, roll casting/control checks or invoke notification callbacks. Availability means a currently usable route; it does not resolve a target or promise a target-specific material/resource preflight will succeed. Game-authored candidate, eligibility and progression progs must be pure. Recursive policy evaluation fails closed.
