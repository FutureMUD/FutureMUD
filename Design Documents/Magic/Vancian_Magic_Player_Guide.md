...
Use an ordinal for an exact prepared copy or spontaneous slot. `next` selects the next matching unspent finite position within that named allowance. `atwill` is only for an at-will allowance. Follow it with the normal spell's target/additional arguments. Power comes from that route; you do not supply an extra power word. Spell-backed power verbs may use one uniquely matching Vancian allowance at that power; if more than one matches, use the explicit casting command to choose your expenditure.
