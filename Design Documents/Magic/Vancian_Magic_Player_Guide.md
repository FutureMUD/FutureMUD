...
Use an ordinal for an exact prepared copy or spontaneous slot. `next` selects the next matching unspent finite position within that named allowance. `atwill` is only for an at-will allowance. Follow it with the normal spell's target/additional arguments. Power comes from that route; you do not supply an extra power word. A separately granted and permitted spell-backed power invokes its backing spell through the normal power and spell rules, but neither selects nor spends a Vancian allowance. It remains usable after Vancian slots are exhausted; knowing the spell alone does not grant the power or bypass ordinary Vancian casting.

At matching level the default power is Standard. Each excess casting level adds one power step, capped at RecklesslyPowerful. A permitted cantrip prepared in a higher finite slot spends that slot and gets its upcast power. Subsequent at-will casts retain their ordinary power. Ordinary casting and scroll activation remain usable during combat; preparation and writing require you to be stationary and out of combat. At-will removes only finite-slot expenditure: normal spell resources, materials, restrictions and delays still apply.
...
Spell help lists costs separately for each currently available capability/allowance using the same spell level, casting level, caster level and power as casting. At-will uses its own base level. Unavailable routes show unresolved cost expressions; legacy costs have a separate label.
