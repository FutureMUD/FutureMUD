...
| --- | --- | --- |
| R1 | A separately granted, permitted spell-backed power remains an independent route after Vancian slots are exhausted and does not change the ledger. Ordinary Vancian casting still selects and spends its allowance; knowing a spell alone neither grants the power nor bypasses that requirement. | `VR.SpellBackedPower_RemainsIndependentAfterVancianSlotsAreExhausted`; `VR.VancianKnowledge_DoesNotGrantSpellBackedPowerOrFreeDirectCasting`; `VR.SpellBackedPower_LegitimateLegacyRouteStillUsesItsNormalCheckAndCosts` |
