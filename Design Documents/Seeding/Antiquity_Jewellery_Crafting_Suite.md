...
This document records the dedicated craft suite for the `SeedAntiquityJewellery` catalogue in `DatabaseSeeder/Seeders/ItemSeeder/ItemSeeder.Antiquity.cs`. Jewellery was previously treated as an older covered surface by the broad antiquity audit; the next-pass implementation now gives it a real craft source, upstream commodity chain, knowledge gate, and per-reference catalogue.
...
The implementation lives in `DatabaseSeeder/Seeders/ItemSeeder/ItemSeeder.Crafting.AntiquityJewellery.cs` and is registered from `SeedCrafts()` through `SeedAntiquityJewelleryCrafts()`.
