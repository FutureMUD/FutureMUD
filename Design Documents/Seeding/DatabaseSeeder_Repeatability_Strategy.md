...
| --- | ---: | --- | --- | --- | --- | --- | --- | --- | --- |
| `CoreDataSeeder` | 0 | No prerequisite beyond database itself | `MayAlreadyBeInstalled` if any account exists | None | Seeds canonical bootstrap records plus stock terrain foundations, terrain tags, and per-terrain stock forage profiles | Repairs missing stock terrain forage profiles and backfills empty or invalid stock terrain assignments while preserving valid custom profile assignments | `OneShot` / limited forage-profile repair | High | Keep terrain foundations in core ownership and create a broader canonical-record plan before expanding repeatability claims beyond forage-profile backfill |
...
| `ChargenSeeder` | 100 | Requires at least one account and the `Human` race | Deterministic stock-key check on canonical stages, helper progs, static settings, and default starting-location role | None | Upserts chargen resources, helper progs, special-application static settings, default starting-location role, and one canonical storyboard row per chargen stage | Repairs seeded package in place and collapses duplicate storyboard rows for the same stage | `Idempotent` / `RepairExisting` | Medium | Keep expanding focused rerun tests and preserve builder-authored storyboard definitions when a stage already has content |
| `CultureSeeder` | 101 | Requires `Human`, skill decorators, and chargen size progs | Deterministic stock-key check on seeded simple name cultures/profile markers and pack markers | None | Upserts simple name cultures, random profiles, languages, accents, scripts, additive script-language memberships, mutual intelligibilities, ethnicities, cultures, and stock blood/sweat materials by stable names | Repairs seeded stock records in place, replaces drifted keyed ethnicity-characteristic joins, and preserves cross-pack and builder-added script memberships | `Idempotent` / `RepairExisting` | Medium | Finish deeper race-specific Middle-earth rerun coverage and chargen-size-prog preservation tests; keep the language and heritage coverage matrices synchronized |
