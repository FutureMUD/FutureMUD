...
- `CoreDataSeeder` is now `Idempotent` / `RepairExisting` only for its foundation catalogues. A detected rerun skips account and game-identity questions and never recreates bootstrap accounts, characters, cells, shards, zones, channels, settings, or helper progs. It reconciles stock tags, materials, liquids, gases, terrain foundations and forage profiles, units, colours, default planes, and hearing profiles by stable stock identities. Material-tag reconciliation resolves the expected parent hierarchy as well as the leaf name, so independent stock taxonomies can safely share leaf names such as `Apiary Product`; terrain atmosphere backfill materializes stock terrain rows before resolving gas or liquid IDs so MySQL readers are not overlapped during rerun repairs.
...
- Human blood, sweat, and their dried residues resolve their solvent from the stock `water` liquid by name. Installation does not assume water has database ID 1; a missing water prerequisite produces an explicit error. This changes newly seeded references and requires no schema migration.
...
### Stock colour lookup compatibility

CoreDataSeeder now gives 34 secondary legacy Colour characteristic values unique builder lookup names such as `banana yellow (stock colour 284)`. The original primary lookup remains `banana yellow`. These are existing compatibility identities, not additional colours: all characteristic-value IDs, referenced Colour IDs, RGB/basic/fancy presentation data and profile definitions remain intact. Runtime colour descriptions use the referenced Colour, not the qualified characteristic lookup name, so the suffix is not garment prose.

Fresh source definitions use the unique names directly. The ordinary Core foundation rerun applies [narrow compatibility rules](../../DatabaseSeeder/Seeders/CoreDataSeeder.ColourLookups.cs) only when both old values, their stock Colour signatures and the parent definition match, with no customised flags/progs, duplicate targets or name collisions. Other records remain untouched and ItemSeeder's exact preflight still rejects unresolved ambiguities. Saved item XML and character selections retain their original value IDs. No migration, deletion, ID remapping or new installer package is introduced.

The four stock `Colour`/`Colour1`/`Colour2`/`Colour3` patterns now match complete variable names. A rerun anchors only their exact old stock patterns with the expected definition type/model/parent, preserving custom patterns. This prevents the general colour channel from capturing `finecolour` or a numbered channel. Existing Fine/Drab definitions and all IDs are retained. Runtime load arguments accept punctuation in quoted values and hyphenated unquoted lookup names; tests assert actual selection without random fallback.

### Clothing material-colour prerequisite stock

CoreDataSeeder also installs missing leather/wood/lacquer clothing colour profiles from existing Colour values. It validates all missing definitions before adding any, retains customised existing profiles, and never creates replacement colour identities. UsefulSeeder supplies the corresponding three Variable components and preserves installed component revisions on rerun; missing, ambiguous or incompatible profile prerequisites fail before that Variable batch. All three are standard-item package completeness markers: an otherwise complete older installation advertises missing stock rather than appearing current. The existing `items` question and all five replay profiles' `items=yes` answers remain unchanged. See the [native-material checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#native-material-colour-prerequisites).

### Human clothing prerequisite stock

HumanSeeder installs the 43 Industrialised/historical-correction wearable configurations and 109 consumed layer variants by stable component names. Fresh wear-stock installation and baseline upgrades use the same source definitions and produce identical graphs. Missing profiles/components are restored; existing clothing profiles, source wearable components and every existing variant revision are preserved, including builder customisations. This is install-missing behaviour within HumanSeeder's wider repair-capable package, not blanket ownership over customised clothing data. Thickness variants retain the actual current source's profiles, default, bulky/visibility flags and other supported configuration while changing layer consumption and, only when explicitly authored, bulk on a separately named identity. The two lounge jackets, rain shell and selected historical under/over-robes use non-bulky variants; other inherited flags remain unchanged. The entire missing variant batch is validated before insertion. Missing, ambiguous, non-current, invalid or conditionally wearable source components fail with a named prerequisite error, rather than creating invalid references or partially inserting that batch. No question, replay profile or selectable package is added. ItemSeeder must still validate actual revisions and full outfit composition before admission.
...
- Item reconciliation retains the revision already attached for a logical component ID; a newer current stock revision must not be added alongside it as a second component. Clothing preflight resolves the actual attached revision, not a same-name current replacement. Approved historical clothing dependencies retain their original aggregate owner, but their presence alone does not infer selection of that entire earlier-era package, including after their later-era admission is removed. Explicit answers, remembered era selection and other managed era content still determine genuine earlier-era installs.
- Clothing component, wearable-geometry and mandatory-layer checks use read-only snapshots before mutation. Invalid profiles, definite outfit layer/bulky conflicts and garments that cannot be worn alone fail without adopting records or writing default settings. These checks are not the remaining complete-inventory, anatomy/attachment or full-batch ownership proof; failed preflight still forbids every clothing application step.
...
- Clothing skins receive a read-only whole-batch ownership/signature check before clothing application, reused by the historical skin writer before changing its first batch entry. Exact unmanaged stock is adopted only during application. Managed logical IDs resolve compatible renamed/current revisions; duplicates, competing claims, missing-current states, unmanaged drift and preserved skins targeting another base reject. This does not close arbitrary custom-skin palette proof.
- Clothing outfits use a shared read-only whole-batch target/signature check before application and in the historical writer. Name/marker collisions, competing logical-ID ownership and unmanaged drift reject before earlier templates or provenance change. Compatible renamed templates, removed markers and customised defaults are preserved by managed identity. Live outfit signatures use managed skin IDs and stable keys rather than mutable skin names. Complete actual preserved-graph validation remains required; see the [outfit checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#outfit-batch-ownership-before-mutation).
- Normalized clothing recipes preflight their complete admitted craft/access-prog batch before clothing application and before the first adapter write. Unique current revisions and logical ownership resolve independently of mutable names; unmanaged rows need complete signatures. Shared identical planned gates are permitted, conflicting definitions or incompatible callable contracts reject, and fresh recipes need not have their future progs pre-created. The direct trait-gate writer validates all three targets before insertion. Valid customisations remain preserved, while source updates use tracked trait bindings. These checks do not certify every actual preserved recipe dependency or close database proof; see the [craft checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#craft-and-access-prog-ownership-before-mutation).
- New physical clothing bases receive complete target/signature ownership preflight and actual retained/projected component checks. Managed renames retain their logical IDs and are explicitly preserved, since canonical fingerprints do not include mutable unique names. The item writer rechecks the complete new-clothing ownership batch before its first application.
- Newly created outfits, progs and knowledge receive generated logical IDs during the first install's normal flush, before detachment. The complete pending batch validates exact entity/ownership identity, positive permanent keys and noncompeting claims before assigning any ID. A second save persists IDs inside the existing ItemSeeder transaction without changing fingerprints or application timestamps; failed saves retain pending state for retry. In-memory regressions do not prove MySQL rollback. Existing unresolvable legacy ownership is not guessed from names. See the [first-install checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#first-install-identities-and-new-base-ownership).
- Clothing reuse preflight projects the actual item writer. Exact untouched managed aggregates may replace obsolete component links, retaining attached revisions for continuing logical IDs. Missing-link repair stays additive; customised aggregates remain unchanged and must satisfy the intended clothing use as installed. Unresolved or duplicate links omitted by the name-only fingerprint block replacement. Only prototype relationships are replaced: component definitions and item instances are not deleted. Preflight performs no mutation, and full-batch ownership/database proof remains required.
