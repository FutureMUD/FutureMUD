...
- `Current State`: `UsefulSeeder` and `ItemSeeder` provide a large stock item and component baseline, and `ItemSeeder.Crafting` shows that the item seeder already touches crafting-adjacent support. `UsefulSeeder` now also covers a broader furniture-container catalogue, size-labelled door and gate component presets for `Tiny` through `Gigantic` exit door sizes, and a modern template slice with lithium battery ecosystems, grid outlets and generators, cellular and answering-machine endpoints, computer and network starter components, signal automation and security examples, gas containers, rebreathers, inhalers, defibrillators, and conditional external-organ support machines.
- `Runtime Breadth vs Seeder Breadth`: the engine still supports a much larger item-component catalogue, revisioned prototypes, skins, groups, implants, finished merchandise families, and rich crafting/project systems than stock seeding provides. Even after the UsefulSeeder furniture, door, and modern expansions, food presets, fax-machine ecosystems, breathing-filter cartridge families, and many profession-specific item lines remain intentionally unseeded.
- `Seeder Fit`: `Strong`
...
- `Current State`: `UsefulSeeder` provides its general starter individual-AI examples, while the repeatable `WildlifeCatalogueSeeder` provides production-ready `Wildlife - ...` and `Managed Animal - ...` individual AIs, finished wildlife group templates, ecology support progs, habitat tags, den/roost/nest anchors, and input-free shelter crafts. It generates an exhaustive normal-animal and explicitly eligible mythical-beast recommendation manifest; every social recommendation resolves to a seeded group template and every managed-eligible race has both wild and managed alternatives. `AIStorytellerSeeder` remains an adjacent optional system.
- `Runtime Breadth vs Seeder Breadth`: the package deliberately does not create map-tied NPC templates, live populations, patrol routes, or sapient social roles. It supplies shared ecological archetypes that become usable unchanged when attached to an NPC template and, for social animals, paired with a live group made from the recommendation.
- `Seeder Fit`: `Strong` for wildlife individual and group template packs; `Possible` for broader general AI packs
- `Template Strategy`: reusable behaviour packs, not full live NPC populations
- `Recommended Seeder Scope`: `Template Library`
- `Notes / Risks`: stock-owned wildlife rows are canonical and repaired on rerun, so builders should clone them before intentional changes. Stock terrain habitat tags are added rather than replacing builder tags. Shelter claims are saved effects: an individual or permitted same-group owner can claim a tagged anchor; stale claims become reclaimable after seven days, and unoccupied AI-created anchors morph after thirty days. Species-specific tuning, breeding, continent-scale migration, named populations, patrol routes, and sapient-social behavior remain appropriately game-authored.
