...
The [food and drink specification](./FutureMUD_Industrialised_Food_Drink_Design_Reference.md) replaces the former 50-row Industrial food quota. Gate 1 passed on 4 September 2026 for 45 Industrial reservations; Gate 2's approved counting correction classifies 39 as items and six as liquids, producing 589 Industrial-only ordinary prototypes (`550 + 39`). Shared food admissions and reused earlier stock are not Industrial-only rows. The strict dependency graph is implemented but awaits editorial Gate 2 acceptance; the 700 generated food rows remain rejected drafts.
