...
| Implemented infrastructure | Gate 2 passed on 2026-09-04. Wave 2 supplies normalized typed sources, source-preserving item/clothing graph audits, exact colour binding, complete approved dependency/reuse resolution, runtime-backed wear/layer proof, whole-batch ownership, preserved outfit/craft graph checks, rich persistence and version-3 provenance. |
...
| Verified production readiness | Not established. Gate 2 proves infrastructure and prerequisite resolution only; Gates 3–7 remain open for representative review, finished content, crafts/outfits, replay and production acceptance. |
...
Wave 2's historical corrections give 51 reused bases complete authored variable-colour prose and suitable Variable components: 39 textile corrections and [12 material-specific corrections](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#native-material-colour-prerequisites). Conventional colours remain outfit defaults. The dependency report has zero missing Variable capabilities, and complete real-stock component/profile binding is tested across all 364 approved designs. The stock-colour compatibility repair retains saved colour IDs and resolves legacy lookup/channel ambiguity. A subsequent [physical correction](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#stock-anatomy-ensemble-layering-and-safe-component-replacement) gives nine reused bases dedicated layer configurations and the alb an authored variable-colour default; all 134 proposals pass runtime footprint/layer tests on stock and equivalent derived wearer locations. Whole-batch item/skin/outfit/craft ownership and preserved outfit/craft graphs are checked before mutation. This is the accepted Gate 2 boundary, not acceptance of finished palettes, wearer sizing, relation capacity, recipes or relational-world replay; those belong to Gates 3–7.
...
The [typed catalogue](../../DatabaseSeeder/Seeders/IndustrialisedItemCatalogue.cs) now loads normalized clothing skin, colour, craft-product and ordered outfit-entry contracts. The older generic craft/outfit tables remain draft scaffolding and are not the completed clothing contract. Database resolution and persistence integration are tracked in the [Wave 2 evidence register](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md); typed records alone do not close those gaps.
...
Before bulk authoring, complete preflight and persistence for these requirements. The [sync script](../../scripts/sync-industrialised-item-catalogue.ps1) is a safe consumer of source TSVs and approved planning records: refresh writes the derived item, [clothing graph](./Industrialised_Clothing_Catalogue_Audit.tsv) and [full-inventory dependency](./Industrialised_Clothing_Dependency_Plan.tsv) audits only and never assembles or overwrites descriptions. The dependency plan covers all 364 approved bases and names unresolved stock, colour and physical work. `-Check` proves source/audit parity, scope reconciliation and structural validity, not complete physical resolution or this specification's production completeness.
...
| 2. Dependencies resolved | All materials, components, characteristics, tags and references resolve; skin/base compatibility, layering, admissions and supported claims validate before mutation | passed | Passed 2026-09-04. The complete 364-base/697-skin-brief/134-outfit planning graph has zero missing stock names, Variable capabilities or component-composition issues; all base geometries and planned outfit footprints/layers have runtime-backed proof, including nontrivial `CountsAs`; item/skin/outfit/craft ownership and preserved graphs fail before mutation. Exact evidence and later-gate boundaries are in the Wave 2 register |
