...
| Seeded reusable component prototypes | 4,557 |
| Materials | 977 |
| Liquids | 330 |
| Gases | 68 |
| Tags | 2,300 |
| ItemSeeder manifest entries | 42,711 |
...
The user experience remains entirely inside `ItemSeeder`. Later-era selection, named technology profiles, custom profile composition, scope selection and diagnostics are ItemSeeder questions. Implementation helpers and tabular catalogues may be separate files. Reusable component prototypes remain with their established owners: UsefulSeeder for general/modern utilities, CombatSeeder for combat stock and HumanSeeder for humanoid wear configurations. ItemSeeder declares and diagnoses those prerequisites but does not silently create them. Wave 2 supplies 43 missing/alternative/corrected wearable configurations and 109 layer variants; the stock-anatomy layer check now covers all 364 bases and 134 outfit proposals, but does not close the clothing dependency or production gates.
...
The [Wave 1 inventory and coverage review](./Industrialised_Clothing_Wave1_Evidence_and_Coverage.md) has user scope/count approval dated 2026-09-03 (Gate 1 passed): 251 new bases (223 shared, 20 Industrial-only, two Modern-only and six Information-only), 113 reused bases, 697 additional skin briefs and 134 outfit proposals across all four bands. Each base includes its complete standalone unskinned presentation; additional-skin totals exclude a duplicate plain default. This approved planning scope replaces the clothing allocation and yields 5,423 shared plus 600 Industrial-only ordinary prototypes (6,023 total); the eight later-only deltas are outside that Stage 2 total. Clothing infrastructure and complete prerequisite planning passed Gate 2 on 2026-09-04; these planning counts still do not replace source/test quotas or establish production acceptance. The user's colour clarification is incorporated: conventional colours are overridable outfit defaults, with no approved fixed-colour locks; Gates 3–7 remain open. The review records overlapping per-era counts, recipe obligations and required craft/lifecycle recalculation; 84 of the clothing outfits admit Industrial, and whole-programme loadouts still require reconciliation.

The [Industrialised food and drink specification](./FutureMUD_Industrialised_Food_Drink_Design_Reference.md) supersedes the 650 shared/50 Industrial food quotas. Gate 1 passed on 4 September 2026 for 464 reserved identities. Gate 2's authorised physical-kind correction classifies those as 397 items and 67 liquids: 226 shared and 39 Industrial ordinary items produce Stage 2 planning totals of 4,999 shared and 589 Industrial ordinary prototypes, while liquids remain separate. The exact matrix assigns consumers to all 307 adopted historical dependencies and retains 26 serving manifests. Gate 2 infrastructure is implemented but awaits editorial acceptance; the current 700 generated food rows remain rejected drafts.
...
Implement typed TSV loading, admission validation and the shared/Industrial catalogue, including lifecycle and craft links where they add meaningful play. The original 5,800/650 allocations are subject to the approved clothing reconciliation and the pending food Gate 1 reconciliation above. Generate review manifests and reconcile through ItemSeeder's existing provenance system.

Status: infrastructure implemented; production acceptance outstanding. The draft embedded source contains 5,800 `shared-industrialised` and 650 `industrial` ordinary rows across 24 domains, with recorded draft counts of 2,337 craft products (36.2%), 1,290 lifecycle participants (20.0%), 100 outfits and 40 canonical `vehicle_revolution_*` graphs. These are inventory/structural counts, not certification of substantive prose, meaningful craft/lifecycle coverage or physically complete outfits. The generated clothing rows specifically remain unaccepted drafts. Typed Industrialised preflight now runs before stock creation and consumes Stage 1 metadata. Clothing Wave 2 adds normalized source contracts, exact colour bindings, rich skin/outfit persistence, a prose-preserving normalized craft adapter, complete approved-inventory dependency proof, preserved outfit/craft graph validation and nonempty source-graph audit coverage. Clothing Gate 2 passed on 2026-09-04; Gates 3–7 still require the finished authored wardrobe, production recipes and outfits, repeatability and production acceptance. See the [Wave 2 evidence register](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md). The canonical manifest contract is version 3 and currently contains 42,754 aggregates; the dependency-reviewed food sources and header-only clothing sources add no accepted production rows.
...
- [Generated clothing source-graph audit](./Industrialised_Clothing_Catalogue_Audit.tsv)
