...
The [food and drink specification](./FutureMUD_Industrialised_Food_Drink_Design_Reference.md) replaces the remaining 650-row shared food quota. Gate 1 passed on 4 September 2026 for 262 shared reservations; Gate 2's approved counting correction classifies 226 as items and 36 as liquids, producing 4,999 shared ordinary prototypes (`4,773 + 226`). All 3,000 earlier food/liquid records are audited separately, and liquids, skins, crafts, failure products and serving manifests do not count as ordinary prototypes. The strict dependency graph is implemented but awaits editorial Gate 2 acceptance; existing generated rows remain rejected drafts.
...
[Clothing Wave 2](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md) adds normalized typed sources for authored skins, explicit base admissions, variable colour bindings and overridable outfit defaults, production-route and quality rationale, exact craft products and ordered outfit entries. Exact database colour resolution, rich skin/outfit persistence and a prose-preserving normalized craft adapter are connected. Derived audits account for nonempty source graphs without rewriting text. Complete inventory/reuse resolution, physical ensemble validation, full craft/batch preflight and nonempty canonical capture remain in progress. Fixed-colour exceptions remain unapproved and are rejected. Schema support or header-only tables do not prove Gate 2. Authored TSV text is the source of truth; generators must not assemble or overwrite it.
...
The shared module is not independently selectable. Current code installs admitted shared rows when Industrial is selected and retains later-only admissions for their eras. `scripts/sync-industrialised-item-catalogue.ps1 -Check` checks current source/audit parity, not editorial acceptance or the new clothing gates. Refresh now writes only the derived audit and leaves source rows and authored descriptions untouched. Existing preflight/provenance/profile-lock infrastructure must be extended and retested against the accepted replacement catalogue; its presence does not certify clothing completeness. Recalculate programme craft/lifecycle coverage on accepted ordinary bases, excluding skins and repeated recipe/outfit use.
