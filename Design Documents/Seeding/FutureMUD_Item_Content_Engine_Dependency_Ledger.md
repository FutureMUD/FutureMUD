...
|---|---|---|---|
| `clothing-authored-source` | Seeder/source integration — Gate 2 complete | Source-preserving item/clothing graph audits, normalized tables, exact colour binding, rich outfit persistence/fingerprints and normalized craft application are connected. All 113 reused identities resolve authoritative definitions with their original owners. Complete target/signature and preserved-graph preflight runs before mutation. Nonempty production capture remains Gate 4/6 evidence, not a Wave 2 dependency | Clothing Gates 4, 6 |
| `clothing-outfit-colour` | Infrastructure complete; production content open | Exact skins, palettes/characteristics, coordinated load arguments, placement and wear order are supported and fail closed. Conventional colours are overridable outfit defaults rather than base/skin locks. Finished coordinated outfits and reload inspection remain content/integration work | Clothing Gates 3, 5, 6 |
| `clothing-stock-colour-ambiguity` | Gate 2 complete | CoreDataSeeder assigns unique lookup names to 34 secondary legacy identities while retaining IDs and displayed colours. General/numbered patterns are anchored. Complete real-stock profiles bind for all 364 approved rows; exact custom ambiguities still fail closed | Re-open Gate 2 only if stock identity/profile facts change |
| `clothing-standalone-skin-palette` | Infrastructure complete; presentation review open | Standalone-base preflight rejects actual random profile values outside the authored palette, and material-native profiles are supplied. Exact skin/outfit/craft selections validate. Any finished skin narrowing the base palette must prove ordinary loading and later skin changes before acceptance | Clothing Gates 3, 5 |
| `clothing-production-products` | Craft infrastructure complete; recipe content open | Normalized rows compile to existing craft types with unchanged prose, exact products/skins, explicit timing and inherited/selected values. Preserved crafts validate their complete child graph and hook signatures before mutation. Narrow inherited palettes fail until the author selects an exact product value or supplies a supported filter; `UnusedInput` remains item-count based | Clothing Gates 3, 5 |
| `clothing-wearable-sizing` | Engine boundary — deferred | `IWearableSize` and size-rule types exist, but the current `Body.CanWear`/`IWearable` path does not consult an item instance's wearable size. Gate 2 therefore proves geometry, layering and nontrivial bodypart `CountsAs`, not size-tailored fit. Production prose must not promise enforced sizing unless a separately approved runtime integration is implemented | Does not block generic-size Gate 2 stock; blocks any mechanics-backed sizing claim |
...
Physical preflight validates actual component capabilities/revisions, Wearable XML and Direct/Shape profile geometry before mutation, rejecting definite layer and bulky conflicts. The complete 364-base geometry and 134-outfit runtime footprint/layer matrix passes across four stock-anatomy fixtures after dedicated layer corrections, both on designed-body locations and derived wearer locations that nontrivially `CountsAs` them; optional external non-wearable locations follow runtime compatibility. Race/gender/age sizing, exact attachment capacity and materialised finished outfits remain later content/integration work. The [ensemble checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#stock-anatomy-ensemble-layering-and-safe-component-replacement) records the tested boundary.

The [full dependency inventory](./Industrialised_Clothing_Dependency_Plan.tsv) supplies all 364 approved physical graphs and outfit consumers. HumanSeeder now supplies 43 added geometry configurations and 109 consumed layer variants, including a lowered hood and corrected narrow garters. There are zero missing stock names, Variable capabilities or component-composition issues. CoreDataSeeder supplies the distinct plant-pith prerequisite with [material evidence and explicit parameter estimates](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#pith-material-prerequisite); leather, wood and lacquer profiles supply the native-colour dependencies. Nine reused layer corrections and the alb's authored variable default supplement the earlier 51 variable-prose/component repairs. Conventional colours remain outfit defaults. Complete stock binding is tested; material-/skin-specific palette acceptance, actual wearer fit/attachments and full-world integration remain open. The audit's `physical-unverified` status does not certify complete garments or ensembles.

Skin target/signature ownership now has a shared read-only whole-batch check before clothing application and before the historical skin writer mutates its first entry. Fourteen focused regressions cover failed-preflight preservation, exact unmanaged adoption during apply, competing ownership/current revisions, source updates and compatible customised names/text. A preserved skin targeting another base fails rather than being silently rewritten; see the [skin checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#skin-batch-ownership-before-mutation).

Outfit target/signature ownership now uses equivalent whole-batch preflight and writer checks. Sixteen new cases cover later collisions without earlier mutation, exact adoption, stable managed identity after builder renames/marker removal, source updates and skin-name-independent dependency signatures; see the [outfit checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#outfit-batch-ownership-before-mutation).

The complete admitted normalized craft/access-prog batch now receives read-only target/signature preflight before clothing and recipe writes. Thirty new cases cover complete unmanaged adoption, unique current revisions, conflicting prog contracts/ownership, shared versus colliding planned gates, valid customisations, source updates and direct-writer ordering. The adapter's detached-trait update defect is repaired. Remaining item-batch ownership, actual preserved outfit/recipe graphs, arbitrary skin colour behaviour and final generated-ID reconciliation are still open; see the [craft checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#craft-and-access-prog-ownership-before-mutation). No engine subsystem or migration is implied by closing these seeder ownership seams.

The [first-install checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#first-install-identities-and-new-base-ownership) supersedes the new-item ownership and generated-ID gaps in the preceding checkpoint: new physical bases have complete ownership/current-revision checks and retained/projected component preflight, and generated outfit/prog/knowledge IDs are persisted during the first install. Renamed managed garments remain customized without duplicate creation. Preserved outfit and craft graph checks now complete the Gate 2 pre-mutation seam. Arbitrary finished skin palettes, exact attachments and fresh/populated relational-database proof remain Gates 3–6 work.
