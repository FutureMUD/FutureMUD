...
- `SeedMedievalFoodProductionFoundationItems` adds six food-production tool or apparatus prototypes.
- The combined item sources create **174 tool/workshop prototypes** and **50 intermediate stock prototypes**, for **224 total item prototypes**.
...
| Food-production tools and apparatus | 6 | `SeedMedievalFoodProductionFoundationItems` |
...
Every exact Medieval item input comes from an earlier phase. Functional tools resolve through seeded Historic Foundation, Primary Production, or earlier Medieval tool tags.

The food, beverage, and preservation foundation is now active with 17 apparatus crafts, 18 processing crafts, and 13 finished-product crafts. It supplies the missing lauter tun, shared pre-industrial food stocks, staple breads and pottages, meat and fish provisions, vegetable oil, amber ale, and red wine. Civilian transport and draft tack are supplied by the pre-industrial vehicle catalogue, while the shared leisure catalogue supplies board games, dice, counters, and simple toys across the three later pre-industrial eras. Wheelwright clamps, tenon cutters, brain-tanning buckets, earth rammers, and a separate dressing axe remain deferred until a dependent workflow justifies them. Any new terminal-source gap must still be routed to butchery, forage, agriculture, pastoral/apiary/sericulture, or Primary Production rather than invented by the Medieval item seeder.
