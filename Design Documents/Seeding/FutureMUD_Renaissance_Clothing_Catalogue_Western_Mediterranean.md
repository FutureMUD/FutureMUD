...
The Industrialised reuse pass supplies dedicated non-bulky layer components for the plain cassock and liturgical alb without changing their identities or economic/material fields. The alb override below stores a complete authored variable-colour description; white belongs to the corresponding historical outfit defaults. The six explicit row overrides are source-authoritative, not generated sentence fragments. See the [physical correction register](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#stock-anatomy-ensemble-layering-and-safe-component-replacement) for tested scope and remaining wearer-fit work.
...
These are complete authored source records, copied verbatim by the historical exporter. The five institutional textile records retain their original identities, materials, economics and wearable configurations; their standalone presentations now support variable colour. Conventional white for the surplice is an outfit default, not an item restriction. The academic robe's judicial skin likewise uses variable colour, with black selected only by the judicial example outfit. The legacy `linen_surplus` spelling remains a stable identity, not the public noun. These corrections do not certify all historical wear geometry or prices for later-era production acceptance.
...
| `renaissance_institution_academic_robe` | a long $colour academic robe | `robe` | `broadcloth` | `Normal` / `Good` | 1200.0g / 120.0m | `Holdable`<br>`Destroyable_Clothing`<br>`Wear_Long_Open_Robe`<br>`Armour_LightClothing`<br>`Insulation_Moderate`<br>`Variable_Colour` | `Era / Renaissance Era`<br>`Market / Clothing / Religious Clothing`<br>`Institution / Religious` | Full description: This long robe is made from dense $colour broadcloth and hangs open down the front. Its full sleeves descend from reinforced shoulders, with broad folds gathering where they join the body. Flat facings finish the open edges, and a deep turned hem gives the lower panels an even fall. The close, smooth surface is unadorned, without an attached hood, fur border or badge identifying a particular institution. [skinnable] |
| `renaissance_institution_full_cowl` | a hooded $colour wool cowl | `cowl` | `wool` | `Normal` / `Good` | 900.0g / 80.0m | `Holdable`<br>`Destroyable_Clothing`<br>`Wear_Cloak_(Closed)`<br>`Armour_LightClothing`<br>`Insulation_Moderate`<br>`Variable_Colour` | `Era / Renaissance Era`<br>`Market / Clothing / Religious Clothing`<br>`Institution / Religious` | Full description: This cowl is made from substantial $colour wool, with a deep hood joined to broad panels around the shoulders. The cloth falls in loose, rounded folds below the neck rather than following a fitted waist. Flat seams and a narrow turned hem finish the plain outer panels. The hood and body are one continuous garment, with no separate shoulder cape, contrasting lining or embroidered border. [skinnable] |
| `renaissance_institution_linen_surplus` | a loose $colour linen surplice | `surplice` | `linen` | `Normal` / `Good` | 650.0g / 60.0m | `Holdable`<br>`Destroyable_Clothing`<br>`Wear_Tabard`<br>`Armour_LightClothing`<br>`Insulation_Minor`<br>`Variable_Colour` | `Era / Renaissance Era`<br>`Market / Clothing / Religious Clothing`<br>`Institution / Religious` | Full description: This loose overgarment is made from $colour linen with a broad opening at the neck. Its ample body gathers into small folds below the shoulders, spreading into a straight, even lower hem. The wide sleeves hang freely from simple joins, with narrow turned edges at their openings. The cloth is unlined and closely woven, and its plain surface carries neither lace inserts nor an embroidered border. [skinnable] |
| `renaissance_institution_monastic_scapular` | a long $colour wool scapular | `scapular` | `wool` | `Normal` / `Good` | 700.0g / 55.0m | `Holdable`<br>`Destroyable_Clothing`<br>`Wear_Tabard`<br>`Armour_LightClothing`<br>`Insulation_Moderate`<br>`Variable_Colour` | `Era / Renaissance Era`<br>`Market / Clothing / Religious Clothing`<br>`Institution / Religious` | Full description: Two long panels of $colour wool hang from the joined shoulders of this scapular. A simple opening passes over the head, leaving the cloth to fall down the front and back with the sides open. The panels are broad and straight, their edges secured by narrow turned hems. There are no sleeves, pockets or attached hood, and the plain, softly napped surface has no badge or contrasting trim. [skinnable] |
| `renaissance_institution_plain_cassock` | a long $colour close-buttoned cassock | `cassock` | `broadcloth` | `Normal` / `Good` | 1050.0g / 100.0m | `Holdable`<br>`Destroyable_Clothing`<br>`Wear_Robe_Layer_0_75_NonBulky`<br>`Armour_LightClothing`<br>`Insulation_Moderate`<br>`Variable_Colour` | `Era / Renaissance Era`<br>`Market / Clothing / Religious Clothing`<br>`Institution / Religious` | Full description: This long cassock is cut from $colour broadcloth with a close-fitting upper body and a fuller lower skirt. A line of small covered buttons follows the front opening, rising toward a modest standing collar. The sleeves taper gently at the wrists, and the closely worked seams keep the shoulders smooth. Its dense outer surface has an even finish, with no attached sash, embroidered insignia or contrasting piping. [skinnable] |

| `renaissance_institution_liturgical_alb` | a long $colour linen alb | `robe` | `linen` | `Normal` / `Standard` | 980.0g / 33.0m | `Holdable`<br>`Destroyable_Clothing`<br>`Wear_Robe_Layer_0_5_NonBulky`<br>`Armour_LightClothing`<br>`Insulation_Minor`<br>`Variable_BasicColour` | `Era / Renaissance Era`<br>`Market / Clothing / Standard Clothing` | Full description: This long alb is made from $colour linen with a plain neckline and close, full-length sleeves. The body widens gently below the shoulders into loose vertical folds. Narrow seams lie flat against the unlined cloth, and a simply turned hem finishes the lower edge. Its smooth surface has no embroidered panels, lace inserts or attached girdle. [skinnable] |
