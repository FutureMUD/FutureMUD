...
- [MedievalClothing](../../DatabaseSeeder/Seeders/ItemSeeder.MedievalClothing.cs) retains the 408-item execution order. Its 33 approved reuse definitions, including the cotton/silk saris, short cotton bodice, leather smith apron, amice, wimple and garters, now call the [shared authoritative source records](../../DatabaseSeeder/Seeders/ItemSeeder.HistoricalClothingSources.Data.cs). Their generated outfit references are not a substitute for those definitions. The move preserves complete text and physical/economic values rather than assembling prose.
- [Pre-industrial aliases](../../DatabaseSeeder/Seeders/ItemSeeder.PreIndustrialBaseline.Aliases.cs) still own the shared belt/sash aliases; their two definitions use the same shared source records and established alias writer. Reuse their canonical targets; do not recreate legacy identities.
...
Wave 2 now resolves all 113 approved identities through [HistoricalClothingSources](../../DatabaseSeeder/Seeders/ItemSeeder.HistoricalClothingSources.cs): 33 retain the `medieval` owner, two `shared-preindustrial`, and 78 `outfits`. Only those approved references may use this lookup. Later admissions extend the existing aggregate instead of creating a second owner or prototype. Focused in-memory tests exercise fresh Industrial-only garters, read-only ownership conflicts, reruns and customization preservation; a complete source/owner and maintained resource-name check covers all 113. This is not yet full-inventory colour, wearability or database-install proof; historical fixed prose and missing variable bindings below still need resolution.
