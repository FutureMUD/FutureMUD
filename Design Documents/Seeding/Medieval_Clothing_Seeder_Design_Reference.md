...
- The ordered Medieval installer still creates those 408 identities. Thirty-three definitions reused by the Industrialised wardrobe now reside in [shared authoritative records](../../DatabaseSeeder/Seeders/ItemSeeder.HistoricalClothingSources.Data.cs) and are invoked at their original positions; other Medieval definitions remain direct calls. The source move itself preserved their text and physical/economic values. A subsequent colour correction gives 19 of these textile garments complete authored variable-colour prose and a stock Variable component; their identities, materials, wear profiles, weights, prices and quality remain unchanged. Existing example outfits retain their former white, black or red choices as overridable entry defaults, including separate white-ground/black-stripe channels on the tallit gadol. Colour words in the example outfit lists describe those selected instances, not restrictions on ordinary unskinned stock. The canonical item bullets below record the variable base descriptions. Later-era reuse still requires resolution of remaining legacy colour/finish and physical dependencies.
...
Subsequent physical corrections give tablet-woven garters narrow knee-band coverage, the linen cincture a thin waist profile, and the sticharion and Daoist cross-collar robe dedicated non-bulky robe variants. These replace only the four sources' wearable component choices; their identities, materials, prices, quality, weights and other components remain unchanged. Shared generic profiles are not retuned. The [Industrialised ensemble checkpoint](./Industrialised_Clothing_Wave2_Infrastructure_and_Gate2.md#stock-anatomy-ensemble-layering-and-safe-component-replacement) records the stock-anatomy checks and preserves actual attachment/wearer-fit proof as outstanding.
...
- The reused smith's apron and shared plain belt now use `Variable_LeatherColour`; the lacquered eboshi uses `Variable_LacquerColour`. These restrict random stock selection to suitable material appearances while still allowing multiple colours. Eboshi black is an overridable historical outfit default. The belt's `medieval_plain_leather_belt` alias and all three original physical/economic contracts are retained. Full authored paragraphs live in the shared authoritative records; they are copied verbatim by the exporter.
...
- Some legacy items outside the approved Industrialised reuse inventory still omit colour variables. That historical source state is not an approved later-era fixed-colour exception. Reused stock must have truthful material-native palettes rather than acquire arbitrary colours or retain a fixed conventional colour.
...
- `medieval_leather_smith_apron` - a $colour leather smith's apron; noun: `apron`; material: `leather`; size/quality: `Small`/`Standard`; weight/cost: 650g/28.0m; wear: `Wear_Apron`; variables: `Variable_LeatherColour`; matches: all urban smith, metalworker, saddler, leatherworker, and heavy craft variants across Europe, Byzantium, Rus / Novgorod, Magyar, Steppe Turkic / Mongol, Islamic, Indian, and East Asian towns.
...
- `medieval_latin_amice` - a $colour linen amice; noun: `amice`; material: `linen`; size/quality: `Small`/`Standard`; weight/cost: 120g/10.0m; wear: `Wear_Scarf`; variables: Variable_Colour; matches: Latin Christian priest, deacon, bishop, and higher liturgical vestment variants.
- `medieval_latin_linen_cincture` - a $colour linen cincture; noun: `cincture`; material: `linen`; size/quality: `Small`/`Standard`; weight/cost: 80g/6.0m; wear: `Wear_Sash`; variables: Variable_Colour; matches: Latin Christian priest, deacon, bishop, and alb-belted liturgical variants.
...
- `medieval_eastern_black_riassa` - a $colour wool riassa; noun: `riassa`; material: `wool`; size/quality: `Normal`/`Standard`; weight/cost: 850g/30.0m; wear: `Wear_Robe`; variables: Variable_Colour; matches: Eastern Christian monk, nun, priest, and non-liturgical clergy variants.
...
- `medieval_eastern_sticharion` - a $colour linen sticharion; noun: `sticharion`; material: `linen`; size/quality: `Normal`/`Good`; weight/cost: 480g/40.0m; wear: `Wear_Robe`; variables: Variable_Colour; matches: Eastern Christian deacon, priest, bishop, and altar-serving liturgical variants.
...
- `medieval_eastern_kamilavkion` - a $colour felt kamilavkion; noun: `kamilavkion`; material: `felt`; size/quality: `Small`/`Standard`; weight/cost: 170g/20.0m; wear: `Wear_Hat`; variables: Variable_Colour; matches: Eastern Christian priest, monk, and bishop headwear variants where appropriate.
- `medieval_islamic_plain_imam_qamis` - a plain $colour cotton qamis; noun: `qamis`; material: `cotton`; size/quality: `Normal`/`Standard`; weight/cost: 420g/18.0m; wear: `Wear_Robe`; variables: Variable_Colour; matches: Islamic imam, khatib, mosque teacher, and plain learned-clergy variants across Andalusian, Abbasid, Fatimid, Seljuk, and Indian Muslim settings.
...
- `medieval_jewish_tallit_gadol` - a $colour1 wool tallit gadol; noun: `tallit`; material: `wool`; size/quality: `Normal`/`Good`; weight/cost: 420g/40.0m; wear: `Wear_Mantle`; variables: Variable_2Colour; matches: Jewish synagogue prayer leader, scholar, worshipper, and Sabbath or festival prayer variants.
- `medieval_jewish_tallit_katan` - a $colour wool tallit katan; noun: `tallit`; material: `wool`; size/quality: `Small`/`Standard`; weight/cost: 230g/22.0m; wear: `Wear_Vest`; variables: Variable_Colour; matches: Jewish daily pious dress, prayer leader, scholar, and underlayer variants.
- `medieval_jewish_skullcap` - a $colour wool skullcap; noun: `skullcap`; material: `wool`; size/quality: `Small`/`Standard`; weight/cost: 60g/6.0m; wear: `Wear_Skullcap`; variables: Variable_Colour; matches: Jewish scholar, synagogue worshipper, prayer leader, and modest headwear variants.
...
- `medieval_hindu_white_priest_dhoti` - a $colour cotton dhoti; noun: `dhoti`; material: `cotton`; size/quality: `Normal`/`Standard`; weight/cost: 360g/16.0m; wear: `Wear_Loincloth`; variables: Variable_Colour; matches: Hindu temple priest, ritual assistant, and clean white priestly lower-drape variants in North Indian / Rajput and South Indian / Chola settings.
...
- `medieval_hindu_kaupina` - a plain $colour cotton kaupina; noun: `kaupina`; material: `cotton`; size/quality: `Small`/`Standard`; weight/cost: 90g/4.0m; wear: `Wear_Loincloth`; variables: Variable_Colour; matches: Hindu ascetic, renunciant, and austere underlayer variants.
...
- `medieval_jain_white_ascetic_robe` - a plain $colour cotton robe; noun: `robe`; material: `cotton`; size/quality: `Normal`/`Standard`; weight/cost: 420g/16.0m; wear: `Wear_Robe`; variables: Variable_Colour; matches: Jain Śvetāmbara monk, nun, and white-clad ascetic variants in Indian urban and temple settings.
- `medieval_jain_white_shoulder_wrap` - a $colour cotton shoulder wrap; noun: `wrap`; material: `cotton`; size/quality: `Small`/`Standard`; weight/cost: 160g/8.0m; wear: `Wear_Mantle`; variables: Variable_Colour; matches: Jain Śvetāmbara monk, nun, and white-clad ascetic upper-layer variants.
...
- `medieval_daoist_cross_collar_robe` - a $colour cross-collared cotton robe; noun: `robe`; material: `cotton`; size/quality: `Normal`/`Standard`; weight/cost: 520g/24.0m; wear: `Wear_Robe`; variables: Variable_Colour; matches: Daoist priest, nun, temple assistant, and everyday ritual-service variants in Song China settings.
...
- `medieval_daoist_ritual_cap` - a $colour cloth ritual cap; noun: `cap`; material: `cotton`; size/quality: `Small`/`Standard`; weight/cost: 90g/12.0m; wear: `Wear_Hat`; variables: Variable_Colour; matches: Daoist priest, nun, and temple ritual headwear variants.
- `medieval_shinto_white_joe_robe` - a $colour hemp jōe robe; noun: `jōe`; material: `hemp`; size/quality: `Normal`/`Good`; weight/cost: 520g/42.0m; wear: `Wear_Robe`; variables: Variable_Colour; matches: Shinto priest, shrine ritualist, and Heian / Kamakura Japanese shrine-service variants.
- `medieval_shinto_priest_hakama` - a pair of $colour hemp hakama; noun: `hakama`; material: `hemp`; size/quality: `Normal`/`Good`; weight/cost: 480g/34.0m; wear: `Wear_Trousers`; variables: Variable_Colour; matches: Shinto priest, shrine ritualist, and ceremonial Japanese shrine-service variants.
- `medieval_shinto_priest_eboshi` - a $colour lacquered eboshi; noun: `eboshi`; material: `silk`; size/quality: `Small`/`Good`; weight/cost: 110g/38.0m; wear: `Wear_Hat`; variables: `Variable_LacquerColour`; matches: Shinto priest, shrine ritualist, and formal Heian / Kamakura Japanese headwear variants.
- `medieval_shinto_miko_white_kosode` - a $colour hemp kosode; noun: `kosode`; material: `hemp`; size/quality: `Normal`/`Standard`; weight/cost: 420g/24.0m; wear: `Wear_Robe`; variables: Variable_Colour; matches: Shrine attendant, miko, female temple-helper, and Japanese shrine household-service variants.
- `medieval_shinto_miko_red_hakama` - a pair of pleated $colour hemp hakama; noun: `hakama`; material: `hemp`; size/quality: `Normal`/`Standard`; weight/cost: 460g/28.0m; wear: `Wear_Trousers`; variables: Variable_Colour; matches: Shrine attendant, miko, female temple-helper, and Japanese shrine-service variants.
