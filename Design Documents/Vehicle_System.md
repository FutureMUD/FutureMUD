...
The core database seeder supplies fast, indoor terrain defaults for this authoring surface: `Vehicle Interior`, `Vehicle Passenger Cabin`, `Vehicle Cargo Hold`, ship corridor/cabin/cargo/engine-room variants, and spaceship corridor/cabin/bridge/engineering/cargo/airlock variants. `Zero-G Spaceship Compartment` remains available when the hosted cell itself should resolve zero gravity. These are ordinary terrain choices rather than vehicle-type restrictions; builders can mix or replace them per compartment.
