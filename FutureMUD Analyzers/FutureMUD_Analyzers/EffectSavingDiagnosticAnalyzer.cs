...
public sealed class EffectSavingDiagnosticAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeSymbol(SymbolAnalysisContext context)
    {
        ...
        IPropertySymbol? savingProperty = namedTypeSymbol.GetMembers("SavingEffect")
        ...
        Location? location = namedTypeSymbol.Locations.FirstOrDefault(l => l.IsInSource);
        if (location is null)
        {
            return;
        }
    ...
    private static bool PropertyAlwaysReturnsTrue(IPropertySymbol propertySymbol)
    {
        foreach (SyntaxReference syntaxReference in propertySymbol.DeclaringSyntaxReferences)
        {
            ...
            List<ReturnStatementSyntax> returnStatements = getter.Body.DescendantNodes()
