...
public class CollectionExtensionsEdgeCaseTests
{
    ...
    {
        (int sum1, int sum2) = Array.Empty<int>().Sum2(x => x, x => x * 2);
    ...
    {
        int[] data = new[] { 1, 5, 3 };
        int result = data.FirstMax(x => x, 10);
    ...
    {
        int[] data = new[] { 1, -2, 3 };
        int result = data.FirstMin(x => x, 0);
    ...
    {
        string?[] data = { "a", "b" };
        List<string> result = data.WhereNotNull(x => x == "a" ? null : x).ToList();
    ...
    {
        string?[] data = { "a", "b" };
        List<string> result = data.SelectNotNull(x => x == "a" ? null : x!.ToUpperInvariant()).ToList();
        CollectionAssert.AreEqual(new[] { "B" }, result);
    }
    ...
    {
        int[] data = new[] { 1, 2, 3 };
        IEnumerable<IEnumerable<int>> result = data.FindSequenceConsecutive(x => x > 0, 5);
