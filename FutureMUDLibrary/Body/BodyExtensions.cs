...
namespace MudSharp.Body
{
    ...
    public static class BodyExtensions
    {
            ...
            IEnumerable<IBodypart> prune)
        {
            ...
            List<IBodypart> result = new(parts);
            foreach (IBodypart item in prune)
            {
        ...
        public static Orientation HeightToOrientation(double HitHeight, double TotalHeight, Orientation appendagesMightBeSubstitutedForThisOrientation)
        {
            ...
            {
                double ratio = HitHeight / TotalHeight;
                if (ratio < 0.15)
                {
                    return Orientation.Lowest;
                }
            ...
            Orientation value = NonAppendageHeightToOrientation();
        ...
        {
            int orientationRoll = RandomUtilities.Random(1, 100);
            int alignmentRoll = RandomUtilities.Random(1, 100);
            ...
            Alignment finalAlignment = FromAlignmentPolar((attackAlignment.ToAlignmentPolar(attackFacing) + alignmentOffset) % 8);
            ...
            Orientation finalOrientation = attackOrientation.RaiseUp(orientationOffset);
