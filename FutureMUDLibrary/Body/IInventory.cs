...
namespace MudSharp.Body
{
    ...
    public interface IInventory : IPerceiver
    {
        ...
        Dictionary<IGameItem, WearableItemCoverStatus> GetAllItemsCoverStatus(bool useIgnoreArmour);
    ...
    public static class InventoryExtensions
    {
            ...
            where T : IInventory
        {
            ...
            {
                foreach (IGameItem item in inv.HeldOrWieldedItems.Where(item => (ignoreCover || (inv.HoldOrWieldLocFor(item) as IGrab)?.ItemsVisible() != false) && perceiver.CanSee(item)))
                {
            ...
            foreach (IGameItem item in inv.OrderedWornItems)
            {
                ...
                List<Tuple<WearableItemCoverStatus, IGameItem>> cover = inv.CoverInformation(item).ToList();
                if (
                    cover.All(
                        x =>
                ...
                {
                    IEnumerable<Tuple<WearableItemCoverStatus, IGameItem>> coveredlocs =
                        cover.Where(
                            x =>
                                (x.Item1 == WearableItemCoverStatus.Covered) ||
                                (x.Item1 == WearableItemCoverStatus.TransparentlyCovered));
    ...
    public static class WieldExtensionClass
    {
            ...
            IInventory body) where T : IWield
        {
            ...
            List<IWieldItemWieldResult> reasons = wieldlocs.Select(x => x.CanWield(item, body)).ToList();
    ...
    public static class GrabExtensionClass
    {
        ...
        {
            List<WearlocGrabResult> results = holdlocs.Select(x => x.CanGrab(item, inventory)).ToList();
