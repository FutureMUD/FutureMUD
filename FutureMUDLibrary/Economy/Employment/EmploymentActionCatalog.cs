...
public static class EmploymentActionCatalog
{
	...
	[
		...
		Executable("purchase", EmploymentActionCategory.Purchasing,
			"tasks step purchase <quantity> <merchandise id|name> from <shop id|name|any> [max <amount>] [keyword <keywords>]",
			EmploymentActionStepType.Purchase,
			EmploymentAuthority.ApprovePurchases,
			Caps(EmploymentAICapability.CanPurchaseCommodities),
			"Buys stocked merchandise through the native shop transaction system using employer authorisation and reserved funds.",
			Aliases("buy"),
		...
		Executable("craft", EmploymentActionCategory.Production,
			...
			"Starts a native craft if the worker can currently perform it; output tracking remains a follow-up."),
		...
		Deferred("transfer", EmploymentActionCategory.Purchasing,
			...
			isFinancial: true),
		Deferred("physicalfloat", EmploymentActionCategory.Purchasing,
			"tasks step physicalfloat <bank|container> <amount>",
			EmploymentAuthority.WithdrawBusinessCash,
			"Physical teller trips, employee cash floats, and recovery flows are deferred behind the current register virtual-funds float adapter.",
			isFinancial: true),
