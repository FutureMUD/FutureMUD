...
public static class EmploymentActionCatalog
{
	...
	[
		Executable("getid", EmploymentActionCategory.Logistics,
			"tasks step getid <quantity> <item prototype ids...> from <here|cell ids...>",
		...
		Executable("gettag", EmploymentActionCategory.Logistics,
			"tasks step gettag <quantity> <tag> from <here|cell ids...>",
			EmploymentActionStepType.GetItemsByTag,
			EmploymentAuthority.ManageDeliveryRoutes,
			Caps(EmploymentAICapability.CanDeliverItems),
			"Collects a quantity of items matching an item tag.",
			Aliases("tag")),
		Executable("commodity", EmploymentActionCategory.Logistics,
			"tasks step commodity <weight> <material> [tag <tag>] from <here|cell ids...> [char <name>=<value> ...]",
			EmploymentActionStepType.GetCommodity,
			EmploymentAuthority.ManageDeliveryRoutes,
			Caps(EmploymentAICapability.CanDeliverItems),
			"Collects a commodity weight by material, optional tag, and optional characteristics.",
			Aliases("material")),
		Executable("deliver", EmploymentActionCategory.Logistics,
			"tasks step deliver to <here|cell id> [container <item id>|containertag <tag>]",
		...
		AuditOnly("purchase", EmploymentActionCategory.Purchasing,
			...
			isFinancial: true),
		AuditOnly("bankdeposit", EmploymentActionCategory.Purchasing,
			...
			"Records bank-deposit audit and ledger evidence without moving real bank funds.",
			Aliases("deposit"),
			requiresPaymentAuthorisation: true,
			isFinancial: true),
		AuditOnly("bankwithdraw", EmploymentActionCategory.Purchasing,
			"tasks step bankwithdraw <amount>",
			EmploymentActionStepType.BankWithdrawal,
			EmploymentAuthority.WithdrawBusinessCash,
			Caps(EmploymentAICapability.CanUseBankAccount, EmploymentAICapability.CanHandleCash),
			"Records bank-withdrawal audit and ledger evidence without moving real bank funds.",
		...
		Executable("authorise", EmploymentActionCategory.Authorisation,
			"tasks step authorise <description>",
			EmploymentActionStepType.CataloguedShell,
			EmploymentAuthority.ApprovePurchases,
			Caps(),
			"Records a durable auditable manager authorisation note.",
			Aliases("authorize")),
		Executable("reserve", EmploymentActionCategory.Authorisation,
			"tasks step reserve <description>",
			EmploymentActionStepType.CataloguedShell,
			EmploymentAuthority.ManageStockRules,
			Caps(),
			"Records a durable reservation token for later employment steps."),
		Executable("release", EmploymentActionCategory.Authorisation,
			"tasks step release <description>",
			EmploymentActionStepType.CataloguedShell,
			EmploymentAuthority.ManageStockRules,
			Caps(),
			"Records a durable release token for a prior employment reservation."),
		...
		Deferred("float", EmploymentActionCategory.Purchasing,
			...
			isFinancial: true),
		Deferred("return", EmploymentActionCategory.Logistics,
			"tasks step return <vehicle|animal|container> <target>",
			EmploymentAuthority.ManageDeliveryRoutes,
			"Returning vehicles, animals, and reusable containers is deferred until those subsystems expose safe task hooks.",
			Caps(EmploymentAICapability.CanDeliverItems)),
		Deferred("load", EmploymentActionCategory.Logistics,
			"tasks step load <items> into <vehicle|container>",
			EmploymentAuthority.ManageDeliveryRoutes,
			"Load optimisation is deferred until vehicle and container capacity planning is represented in task context.",
			Caps(EmploymentAICapability.CanDeliverItems)),
		Deferred("unload", EmploymentActionCategory.Logistics,
			"tasks step unload <vehicle|container> at <location>",
			EmploymentAuthority.ManageDeliveryRoutes,
			"Unload optimisation is deferred until vehicle and container task handling is represented.",
			Caps(EmploymentAICapability.CanDeliverItems)),
		Deferred("vehicle", EmploymentActionCategory.Logistics,
			"tasks step vehicle <reserve|load|return> ...",
			EmploymentAuthority.ManageDeliveryRoutes,
			"Vehicle-specific execution is deferred until vehicle routing and reservation hooks exist.",
