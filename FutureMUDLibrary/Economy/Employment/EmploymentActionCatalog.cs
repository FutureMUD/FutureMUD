...
public static class EmploymentActionCatalog
{
	...
	[
		...
		Executable("price", EmploymentActionCategory.Administration,
			"tasks step price merch <id|name> <amount> OR tasks step price market <host|market id|name> category <category> [supply <pct>] [demand <pct>] [flat <pct>] [name <text>] [duration <timespan>|until <date|never>]",
			EmploymentActionStepType.PriceChange,
			EmploymentAuthority.AdjustPrices,
			Caps(EmploymentAICapability.CanManagePrices),
			"Changes exact shop merchandise base prices or records category-level market price impacts through market influences.",
			Aliases("pricing")),
