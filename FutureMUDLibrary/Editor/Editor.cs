    ...
    public class Editor : IEditor
    {
        ...
        public void HandleCommand(string command)
        {
            if (command.Length > 0)
            {
                if (command[0] == '*')
                {
                    StringStack ss = new(command.RemoveFirstCharacter());
                    switch (ss.Pop().ToLowerInvariant())
                    {
                        case "len":
                        case "length":
                        case "chars":
                            int charCount = (int)(_lines.Sum(x => x.RawTextLength()) * CharacterLengthMultplier);
                        ...
                        case "recall":
                            ...
                            _lines.Clear();
                            _insertPosition = -1;
                            foreach (string addline in _recallText.Split('\n'))
                            {
                                _lines.Add(addline);
                            }
                            ...
                            return;
                        case "delete":
                            if (!int.TryParse(ss.Pop(), out int value))
                            {
                            ...
                            if (int.TryParse(ss.Pop(), out int value2))
                            {
                                if (value2 <= value)
                                {
