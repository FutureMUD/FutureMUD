...
public sealed record RecordedAudioSpeakerSnapshot(long? CharacterId, string Name, Gender Gender)
{
    ...
    public static RecordedAudioSpeakerSnapshot LoadFromXml(XElement element)
    {
        return new RecordedAudioSpeakerSnapshot(
            long.TryParse(element.Attribute("character")?.Value, out long characterId) && characterId > 0
                ? characterId
                : null,
            element.Attribute("name")?.Value ?? "Unknown Speaker",
            short.TryParse(element.Attribute("gender")?.Value, out short genderValue)
                ? (Gender)genderValue
                : Gender.Neuter);
    }
}
    ...
    RecordedAudioSpeakerSnapshot Speaker)
{
    ...
    public static RecordedAudioSegment LoadFromXml(XElement element)
    {
        return new RecordedAudioSegment(
            TimeSpan.FromMilliseconds(long.TryParse(element.Attribute("delayms")?.Value, out long delayMs) ? delayMs : 0L),
            TimeSpan.FromMilliseconds(long.TryParse(element.Attribute("durationms")?.Value, out long durationMs) ? durationMs : 0L),
            long.TryParse(element.Attribute("language")?.Value, out long languageId) ? languageId : 0L,
            long.TryParse(element.Attribute("accent")?.Value, out long accentId) ? accentId : 0L,
            element.Element("Text")?.Value ?? string.Empty,
            int.TryParse(element.Attribute("volume")?.Value, out int volume) && Enum.IsDefined(typeof(AudioVolume), volume)
                ? (AudioVolume)volume
                : AudioVolume.Decent,
            int.TryParse(element.Attribute("outcome")?.Value, out int outcome) && Enum.IsDefined(typeof(Outcome), outcome)
                ? (Outcome)outcome
                : Outcome.Pass,
    ...
    {
        ICharacter? speaker = spokenLanguage.Origin as MudSharp.Character.ICharacter;
        return new RecordedAudioSegment(
            delayBeforeSegment,
    ...
    public static TimeSpan EstimateDuration(string rawText)
    {
        ...
        int wordCount = rawText.Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Length;
