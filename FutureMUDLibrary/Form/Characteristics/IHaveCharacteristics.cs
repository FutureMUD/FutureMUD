...
namespace MudSharp.Form.Characteristics
{
    ...
    public static class IHaveCharacteristicsExtensions
    {
            ...
            IEnumerable<(ICharacteristicDefinition, ICharacteristicValue)> characteristics)
        {
            ...
            if (BasicFancyCharacteristicRegex.IsMatch(definition))
            {
                selection =
                    characteristics.Select(x => x.Item1)
                        .FirstOrDefault(
            ...
            bool ignoreObscurers = false) where T : IHaveCharacteristics
        {
            ...
            MatchCollection matches = CharacteristicRegex.Matches(description);
            ...
            IFuturemud gameworld, IRace race = null, ICulture culture = null, IEthnicity ethnicity = null, int age = 0, double height = 0.0)
        {
            ...
            description = CharacteristicRegex.Replace(description, m =>
            {
                ...
                {
                    return "$" + m.Groups[1].Value[1..] + m.Groups[2].Value +
...
: "";
            ...
            bool ignoreObscurers = false) where T : IHaveCharacteristics
        {
            ...
            IPerceivable ownerPerc = owner as IPerceivable;
            ICharacterTemplate ownerChargen = owner as ICharacterTemplate;
            Gendering gender = ownerPerc is not null ? ownerPerc.ApparentGender(voyeur) : Gendering.Get(ownerChargen.SelectedGender);
            description = PronounNumberRegex.Replace(description, m =>
            {
                switch (gender.Enum)
                {
            ...
            {
                IBody ownerbody = (owner as IBody) ?? (owner as IHaveABody)?.Body;
                if (ownerbody == null)
                {
                    return m.Value;
                }

                Tuple<ICharacteristicDefinition, CharacteristicDescriptionType> type = owner.GetCharacteristicDefinition(m.Groups["characteristic"].Value);
                if (type == null)
                {
                    return m.Value;
                }
                ...
                int ownercount = ownerbody.Bodyparts.Count(x => x.Shape == bodypartSpecificCharacteristicDefinition.TargetShape);
                string text;
            ...
            description = CharacteristicRegex.Replace(description, m =>
            {
                ...
                Tuple<ICharacteristicDefinition, CharacteristicDescriptionType> type = owner.GetCharacteristicDefinition(m.Groups[2].Value);
                // Check for ? modifier, which means this is a "not null or default" check
                if (m.Groups[1].Value == "?")
                {
                    if ((type.Item1 == null) ||
                ...
                IObscureCharacteristics obscurer = owner.GetObscurer(type.Item1, voyeur);
            ...
            description = ExtraVariableRegex.Replace(description, m =>
            {
                ...
                switch (m.Groups[2].Value)
                {
                    ...
                    case "himself":
                        return gender.Reflexive();
                    case "height":
                        ICharacter character = owner as ICharacter ?? (owner as IBody)?.Actor;
                    ...
                    case "male":
                        return gender.GenderClass();
                    case "race":
                        IHaveRace race = owner as IHaveRace;
                        return race?.Race?.Name.ToLowerInvariant() ?? "unknown";
                    case "culture":
                        IHaveCulture culture = owner as IHaveCulture;
                        return culture?.Culture?.Name.ToLowerInvariant() ?? "unknown";
                    case "ethnicity":
                        IHaveRace ethnicity = owner as IHaveRace;
                    ...
                    case "tattoos":
                        ...
                        if (character != null)
                        {
                            List<ITattoo> tattoos = character.Body.Tattoos
                        ...
                        ICharacterTemplate chargen = owner as ICharacterTemplate;
                        if (chargen != null)
                        {
                            List<(ITattooTemplate Tattoo, IBodypart Bodypart)> tattoos = chargen.SelectedDisfigurements
                    ...
                    case "scars":
                        ...
                        if (character != null)
                        {
                            List<IScar> scars = character.Body.Scars
                            .Where(x => character.Body.ExposedBodyparts.Contains(x.Bodypart))
                            .OrderByDescending(x => x.Size)
                            .ThenByDescending(x => x.Bodypart.IsVital)
                            .ToList();
