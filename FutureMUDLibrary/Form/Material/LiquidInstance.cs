...
namespace MudSharp.Form.Material
{
    public class LiquidInstance
    {
        ...
        {
            return OriginLiquid == other.OriginLiquid && MagicalChargesMatch(other);
        }
        ...
        public virtual XElement SaveToXml()
        {
            ...
            root.Add(System.Linq.Enumerable.Select(MagicalCharges,
                x => new XElement("Magic", new XAttribute("substance", x.Key), x.Value.Save())));
            return root;
        }
        ...
        public LiquidInstance(XElement root, IFuturemud gameworld)
        {
            ...
            FreshnessStage = Enum.TryParse<LiquidFreshnessStage>(root.Attribute("freshnessStage")?.Value, true, out var stage)
                ? stage
                : LiquidFreshnessStage.Fresh;
            foreach (var magic in root.Elements("Magic"))
            {
                MagicalCharges[(long)magic.Attribute("substance")] = MudSharp.Magic.SubstanceCharge.Load(magic.Element("Charge"));
            }
        }
