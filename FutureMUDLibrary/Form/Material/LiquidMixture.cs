...
namespace MudSharp.Form.Material
{
    ...
    public class LiquidMixture : IProgVariable
    {
        ...
        public LiquidMixture(XElement root, IFuturemud gameworld)
        {
            ...
            foreach (XElement item in root.Elements("Liquid"))
            {
        ...
        public IEnumerable<LiquidMixture> Split(int number)
        {
            ...
            double volumeEach = TotalVolume / number;
            List<LiquidMixture> list = new();
            for (int i = 0; i < number; i++)
            {
                LiquidMixture mixture = Clone();
        ...
        public static LiquidMixture CreateEmpty(IFuturemud gameworld)
        {
            return new(new LiquidInstance[] { }, gameworld);
        }
        ...
        public (bool Truth, ItemQuality Quality) CountsAs(ILiquid liquid)
        {
            ...
            double ratio = _instances.Where(x => x.Liquid.LiquidCountsAs(liquid)).Sum(x => x.Amount) / TotalVolume;
            if (ratio >= 0.95)
            {
        ...
        {
            foreach (LiquidInstance liquid in mixture._instances)
            {
                AddLiquid(liquid);
            }
        }
        ...
        {
            foreach (LiquidInstance instance in otherLiquid.Instances)
            {
        ...
        public LiquidMixture RemoveLiquidVolume(double volume)
        {
            ...
            List<LiquidInstance> newInstances = new();
        ...
        public INeedFulfiller GetNeedFulfiller()
        {
            ...
            double multiplier = Gameworld?.UnitManager?.BaseFluidToLitres ?? 1.0;
            double satiationPoints = 0.0, thirstPoints = 0.0, waterLitres = 0.0, alcoholLitres = 0.0;
            foreach (LiquidInstance liquid in Instances)
            {
        ...
        {
            foreach (LiquidInstance instance in _instances)
            {
        ...
        public string TasteString(ICharacter actor)
        {
            ...
            Dictionary<(string Taste, string Vague), double> tasteDictionary = new();
            foreach (LiquidInstance instance in _instances)
            {
                (string TasteText, string VagueTasteText) tt = (instance.Liquid.TasteText, instance.Liquid.VagueTasteText);
            ...
            double sum = _instances.Sum(x => x.Amount * x.Liquid.TasteIntensity);
            StringBuilder sb = new();
            foreach (KeyValuePair<(string Taste, string Vague), double> taste in tasteDictionary.OrderByDescending(x => x.Value))
            {
