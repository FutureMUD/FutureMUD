...
namespace MudSharp.Framework
{
    public class ReadOnlyCollectionDictionary<T, U> : IReadOnlyCollectionDictionary<T, U>
    {
        ...
        private class ReadonlyEnumerator : IEnumerator<KeyValuePair<T, IEnumerable<U>>>
        {
            ...
            {
                bool node = _internalEnumerator.MoveNext();
                if (!node)
                {
                    return false;
                }
        ...
        {
            Dictionary<T, IEnumerable<U>> dictionary = new();
            foreach ((T key, List<U> value) in _cd.ToDictionary())
            {
    ...
    public class CollectionDictionary<T, U> : ICollectionDictionary<T, U>
    {
        ...
        {
            _internal = new Dictionary<T, List<U>>();
            foreach (KeyValuePair<T, List<U>> value in values)
            {
        ...
        {
            foreach ((T Key, U Value) item in range)
            {
        ...
        {
            foreach (KeyValuePair<T, List<U>> item in _internal.ToList())
            {
    ...
    public static class CollectionDictionaryExtensions
    {
        ...
        {
            CollectionDictionary<TKey, TValue> dict = new();
            foreach ((TKey Key, TValue Value) item in enumerable)
            {
        ...
        {
            CollectionDictionary<TKey, TValue> dict = new();
            foreach (Tuple<TKey, TValue> item in enumerable)
            {
        ...
        {
            CollectionDictionary<TKeyR, TValueR> dict = new();
            foreach (KeyValuePair<TKey, List<TValue>> item in dictionary)
            {
                foreach (TValue sub in item.Value)
                {
