...
public static partial class DateUtilities
{
    ...
    public static bool TryParseDateTimeOrRelative(string text, IAccount account, bool allowRelative, out DateTime utcDateTime)
    {
        ...
        if (!TimeSpanParser.TryParse(text, TimeSpanParserUtil.Units.Seconds, TimeSpanParserUtil.Units.Weeks, out TimeSpan timespan))
        {
            return false;
        }
    ...
    public static DateOrder GetDateOrder(CultureInfo culture)
    {
        ...
        DateOrder mdOrder = DateOrder.DayFirst;
        if (!string.IsNullOrWhiteSpace(monthDayPattern))
        {
            DateOrder? order = DetermineMonthDayOrder(monthDayPattern);
            if (order.HasValue)
            {
                mdOrder = order.Value;
            }
        ...
        DateOrder ymOrder = DateOrder.DayFirst;
        if (!string.IsNullOrWhiteSpace(yearMonthPattern))
        {
            DateOrder? order = DetermineMonthDayOrder(yearMonthPattern);
            if (order.HasValue)
            {
                ymOrder = order.Value;
            }
        ...
        {
            DateOrder? order = DetermineMonthDayOrder(shortDatePattern);
            if (order.HasValue)
            {
                return order.Value;
            }
    ...
    private static DateOrder? DetermineMonthDayOrder(string pattern)
    {
        ...
        Regex regex = new(@"(?<specifier>d+|M+|y+)", RegexOptions.IgnoreCase);
