...
namespace MudSharp.Framework
{
    public static class NumberUtilities
    {
        ...
        public static bool TryParsePercentage(this string text, CultureInfo culture, out double value)
        {
            ...
            {
                if (!double.TryParse(text, out double fallback))
                {
                    return false;
                }
            ...
            bool result = double.TryParse(text.Replace(culture.NumberFormat.PercentSymbol, "").Trim(), NumberStyles.Any,
        ...
        public static bool TryParsePercentageDecimal(this string text, CultureInfo culture, out decimal value)
        {
            ...
            if (!text.Contains(culture.NumberFormat.PercentSymbol))
            {
                if (!decimal.TryParse(text, out decimal fallback))
                {
                    return false;
                }
            ...
            bool result = decimal.TryParse(text.Replace(culture.NumberFormat.PercentSymbol, "").Trim(), NumberStyles.Any,
                culture.NumberFormat, out value);
            if (result)
            {
                value = value / 100.0M;
        ...
        {
            bool reverseDirection = toInt < fromInt;
            List<int> results = new(toInt - fromInt + 1);
            if (reverseDirection)
            {
