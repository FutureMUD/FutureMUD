...
namespace MudSharp.Framework
{
    public class RankedRange<T>
    {
        ...
        public double NextUpperBound
        {
            get
            {
                Range<T> last = _range.LastOrDefault();
                if (last != null)
                {
        ...
        public double NextLowerBound
        {
            get
            {
                Range<T> first = _range.FirstOrDefault();
        ...
        {
            Range<T> range = _range.FirstOrDefault(x => x.Value.Equals(value));
            if (range is null)
            {
                return false;
            }
            ...
            int index = _range.IndexOf(range);
            Range<T> next = _range[index + 1];
