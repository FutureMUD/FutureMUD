...
namespace MudSharp.Framework
{
    public static partial class StringExtensions
    {
        ...
        {
            return input.Length > 1 ? input[1..].TrimStart() : "";
        }
        ...
        public static string RemoveFirstWord(this string input)
        {
            ...
            return firstSpace != -1 ? input[(firstSpace + 1)..].TrimStart() : "";
        }
        ...
        {
            Match match = IncrementRegex.Match(str);
            if (!match.Success)
            {
                return $"{str}1";
            }
        ...
        {
            Regex regex = new($"{name}(?<number>[0-9]+)?$", RegexOptions.IgnoreCase);
            List<Match> matches = names
        ...
        public static string ToBonusString(this double number, IFormatProvider formatProvider = null, uint decimalPlaces = 2, bool colour = true)
        {
            ...
            string format = $"+#,0{decimalBit};-#,0{decimalBit};+0{decimalBit}";
            string value = number.ToString(format, formatProvider);
            if (colour)
            {
                return number >= 0.00 ?
        ...
        public static string Wrap(this string text, int width, string indent = "")
        {
            ...
            bool endsWithNewline = text.Last() == '\n';

            StringBuilder sb = new();
            string[] strings = text.Replace("\t", "  ").Split(new[] { "\r\n", "\n" }, StringSplitOptions.None);
            ...
            foreach (string line in strings)
            {
                ...
                {
                    int splitIndex = LastWhiteSpaceIndex(remains, width - indent.Length);
                    sb.AppendLine(indent + remains[..(splitIndex + 1)].TrimEnd());
                    remains = remains[splitIndex..].TrimStart();
                }
        ...
        private static int LastWhiteSpaceIndex(string text, int width)
        {
            ...
            {
                char current = text[lastWhitespace - 1];
        ...
        public static string NormaliseOutputSentences(this string input)
        {
            ...
            foreach (string token in tokens)
            {
                ...
                {
                    // Process normal text
                    string processedText = ProcessText(token, ref sentenceEnded);
        ...
        private static string ProcessText(string text, ref bool sentenceEnded)
        {
            ...
            while (i < text.Length)
            {
                StringBuilder punctuationSequence = new();
        ...
        public static string ProperSentences(this string input)
        {
            ...
            List<char> chararray = new(input.Length);
            bool colour = false, mxp = false;
        ...
        public static IEnumerable<string> SplitStringsForDiscord(this string message)
        {
            ...
            {
                int lastNewline = message[..1950].LastIndexOf('\n');
                if (lastNewline == -1)
                {
                    list.Add(message[..1950]);
                    message = message[1950..];
                    continue;
                }

                list.Add(message[..lastNewline]);
        ...
        {
            StringBuilder sb = new();
            int distanceFromNewline = 0;
            StringBuilder sbLine = new();
            bool foundNonSpaceCharacters = false;
            bool appendedLonelyNewline = false;
            bool inAnsi = false;
        ...
        public static string ToTitleCaseAP(this string input)
        {
            ...
            HashSet<string> lowercaseWords = new(StringComparer.OrdinalIgnoreCase)
            {
            ...
            int totalWords = wordIndices.Count;
            ...
            for (int i = 0; i < wordIndices.Count; i++)
            {
                ...
                if (word.Contains('-'))
                {
                    ...
                    {
                        string subword = hyphenatedWords[j];
        ...
        private static string CapitalizeWord(string word)
        {
            ...
            if (word.ToUpper() == word || word[1..].Any(char.IsUpper))
            {
                return word;
            }
