...
namespace MudSharp.Framework
{
    public static partial class StringUtilities
    {
        ...
        int maxwidth, bool bColourTable = true, ANSIColour colour = null, int truncatableColumnIndex = -1, bool unicodeTable = false, bool denseTable = false)
        {
            ...
            string ursep = $"{(unicodeTable ? "╗" : "+")}{(bColourTable ? colour.Reset() : "")}";
            string llsep = $"{(bColourTable ? colour.ToString() : "")}{(unicodeTable ? "╚" : "+")}";
            string lrsep = $"{(unicodeTable ? "╝" : "+")}{(bColourTable ? colour.Reset() : "")}";
            string lmsep = $"{(bColourTable ? colour.ToString() : "")}{(unicodeTable ? "╠" : "+")}";
            string rmsep = $"{(bColourTable ? colour.ToString() : "")}{(unicodeTable ? "╣" : "+")}";
            string topsep = $"{(unicodeTable ? "╦" : "+")}";
            string midsep = $"{(unicodeTable ? "╬" : "+")}";
            string bottomsep = $"{(unicodeTable ? "╩" : "+")}";
        ...
        {
            IPerceivableHaveTraits pht = voyeur as IPerceivableHaveTraits;
            text = CheckReplacementRegex.Replace(text, match =>
            {
                ...
                ITraitDefinition trait = gameworld.Traits.GetByIdOrName(match.Groups["trait"].Value);
                if (trait is null)
                {
                    return match.Groups["text"].Value;
                }

                if (!double.TryParse(match.Groups["difficulty"].Value, out double value))
                {
                    return match.Groups["text"].Value;
                }
        ...
        {
            IHaveLanguage languagePerceiver = voyeur as IHaveLanguage;

            text = LanguageReplacementRegex.Replace(text, match =>
            {
                string[] split = match.Groups["details"].Value.Split(',');
                if (split.Length < 2 || languagePerceiver == null)
                {
                    return match.Groups["text"].Value;
                }

                ILanguage language = long.TryParse(split[0], out long value) ? gameworld.Languages.Get(value) : gameworld.Languages.GetByName(split[0]);
                if (language == null)
                {
                    return match.Groups["text"].Value;
                }

                IScript script = long.TryParse(split[1], out value) ? gameworld.Scripts.Get(value) : gameworld.Scripts.GetByName(split[1]);
                if (script == null)
                {
                    return match.Groups["text"].Value;
                }

                double requiredSkill = 0.0;
                WritingStyleDescriptors style = (WritingStyleDescriptors)gameworld.GetStaticInt("DefaultWritingStyleInText");
                Form.Colour.IColour colour = gameworld.Colours.Get(gameworld.GetStaticLong("DefaultWritingColourInText"));
                bool defaultStyleChanged = false;
                foreach (string item in split.Skip(2))
                {
                    Match im = LanguageReplacementAttributeRegex.Match(item);
                    if (!im.Success)
                    {
                        continue;
                    }
        ...
        private static Regex IsValidFormatStringRegex = new(@"(?<!\{)\{\d+(:[^}]+)?\}(?!\})", RegexOptions.Compiled);
        ...
        public static bool IsValidFormatString(this string formatString, int expectedParameterCount, ReadOnlySpan<bool> mandatoryMask)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                string numberPart = placeholder.Split(new[] { ':', '}' })[0].Trim('{');
            ...
            for (int i = 0; i < placeholdersPresent.Length; i++)
            {
