...
public static class ProgVariableTypeRegistry
{
	...
	public static string Describe(ProgVariableTypes type)
	{
		...
		var containerSuffix = string.Empty;
		var underlying = type.WithoutLiteral();
		...
		if (MetadataByType.TryGetValue(underlying, out var metadata))
		{
			return $"{metadata.DisplayName}{containerSuffix}";
		}

		var flags = underlying.GetFlags().ToList();
		if (!flags.Any())
		{
			return "Unknown Type";
		}

		return $"{string.Join(" ", flags.Select(x => MetadataByType.TryGetValue(x, out var item) ? item.DisplayName : x.ToStorageString()))}{containerSuffix}";
	}

	public static ProgTypeKind GetExactKind(ProgVariableTypes type)
	{
		return MetadataByType.TryGetValue(type, out var metadata) ? metadata.ExactKind : ProgTypeKind.Unknown;
	}
	...
	public static bool TryParse(string? value, out ProgVariableTypes type)
	{
		...
		{
			return TryParseStorageValue(value.Substring(3), out type);
		}
		...
		{
			var match = ModifierRegex.Match(value);
		...
		var condensed = value.Replace(" ", string.Empty);
	...
	{
		var tokens = new HashSet<string>(StringComparer.InvariantCultureIgnoreCase)
		{
			displayName,
		...
		var metadata = new TypeMetadata(type, displayName, ProgTypeKind.Unknown, singleBit, tokens.ToList());
