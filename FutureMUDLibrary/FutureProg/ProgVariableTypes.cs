...
public readonly struct ProgVariableTypes : IEquatable<ProgVariableTypes>
{
    ...
    public ProgVariableTypeCode LegacyCode => TryGetLegacyCode(out ProgVariableTypeCode code) ? code : ProgVariableTypeCode.Unknown;
    ...
    public string ToStorageString()
    {
        ...
        byte[] bytes = _mask.ToByteArray(isUnsigned: true, isBigEndian: true);
        string hex = Convert.ToHexString(bytes).TrimStart('0').ToLowerInvariant();
    ...
    public static ProgVariableTypes FromStorageString(string value)
    {
        ...
        if (TryParse(value, out ProgVariableTypes parsed))
        {
            return parsed;
        }
