...
namespace MudSharp.RPG.Checks
{
    ...
    public static class CheckExtensions
    {
        ...
        {
            int newVal = (int)outcome + degrees;
        ...
        {
            text = text.Trim();
            List<Difficulty> difficulties = Enum.GetValues(typeof(Difficulty)).OfType<Difficulty>().ToList();
            ...
            List<Tuple<Difficulty, string>> diffNames = difficulties.Select(x => Tuple.Create(x, Enum.GetName(typeof(Difficulty), x))).ToList();
        ...
        {
            text = text.Trim();
            string exploded = text.SplitCamelCase();
            List<Outcome> outcomes = Enum.GetValues(typeof(Outcome)).OfType<Outcome>().ToList();
            ...
            List<(Outcome Enum, string Name, string ExplodedName)> outcomeNames = outcomes.Select(x => (Enum: x, Name: Enum.GetName(typeof(Outcome), x), ExplodedName: Enum.GetName(typeof(Outcome), x).SplitCamelCase())).ToList();
        ...
        {
            int newVal = (int)difficulty + degrees;
