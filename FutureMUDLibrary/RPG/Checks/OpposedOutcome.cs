...
namespace MudSharp.RPG.Checks
{
    ...
    public class OpposedOutcome
    {
        ...
        /// <summary>
        /// Creates an OpposedOutcome that can result in a stalemate result
        /// </summary>
        /// <param name="proponent">The outcome for the proponent</param>
        /// <param name="opponent">The outcome for the opponent</param>
        public OpposedOutcome(Outcome proponent, Outcome opponent)
        {
            int difference = (int)proponent - (int)opponent;
            if (difference == 0)
            {
        ...
        /// <summary>
        /// Creates an OpposedOutcome that must NOT be a stalemate. Will ensure that at least one side wins.
        /// </summary>
        /// <param name="proponentOutcomes">The result against all possible difficulties for the proponent</param>
        /// <param name="opponentOutcomes">The result against all possible difficulties for the opponent</param>
        /// <param name="primaryProponent">The primary difficulty for the proponent</param>
        /// <param name="primaryOpponent">The primary difficulty for the opponent</param>
        ...
        {
            Difficulty proponentDifficulty = primaryProponent;
            Difficulty opponentDifficulty = primaryOpponent;
            bool? increaseDifficulty = default;
            while (true)
            {
                Outcome proponent = proponentOutcomes[proponentDifficulty].Outcome;
                Outcome opponent = opponentOutcomes[opponentDifficulty].Outcome;
                int difference = (int)proponent - (int)opponent;
                if (difference == 0)
                {
