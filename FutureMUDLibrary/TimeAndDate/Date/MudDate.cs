...
public class MudDate : IComparable
{
    ...
    protected bool InternalSubtractDays(ref int numberOfDays)
    {
        ...
        IEnumerable<Month> formerMonths = ThisYear.Months.TakeWhile(x => x != Month).Reverse();
        int passedDaysInYear = formerMonths.Sum(x => x.Days) + Day;
        if (numberOfDays < passedDaysInYear)
        {
            numberOfDays -= Day;
            foreach (Month month in formerMonths)
            {
    ...
    protected bool InternalAdvanceDays(ref int numberOfDays)
    {
        ...
        IEnumerable<Month> futureMonths = ThisYear.Months.SkipWhile(x => x != Month).Skip(1);
        int remainingDaysInYear = futureMonths.Sum(x => x.Days) + (Month.Days - Day);
        if (numberOfDays <= remainingDaysInYear)
        {
            numberOfDays -= Month.Days - Day;
            foreach (Month month in futureMonths)
            {
                if (numberOfDays <= month.Days)
                {
                    _month = month;
    ...
    public MudDate ConvertToOtherCalendar(ICalendar convert)
    {
        ...
        bool isFuture = this > Calendar.CurrentDate;
        int currentDaysDifference = DaysDifference(this, Calendar.CurrentDate);
        MudDate newDate = new(convert.CurrentDate);
    ...
    private static int DaysDifference(MudDate date1, MudDate date2)
    {
        ...
        {
            int daysThisYear = date1.DayNumberInYear();
            int daysCompareYear = date2.DayNumberInYear();
            int daysBetweenYears = date1.Calendar.CountDaysBetweenYears(date1.Year, date2.Year);
    ...
    public void AdvanceYears(int numberOfYears, bool normaliseDays)
    {
        ...
        Month newMonth = _thisYear.Months.Find(x => x.Alias == Month.Alias);
        if (newMonth == null)
        {
            ...
            if (normaliseDays)
            {
                ...
                int days = oldYear.Months.TakeWhile(x => x != month).Sum(x => x.Days) + day;
                foreach (Month tempMonth in _thisYear.Months)
                {
