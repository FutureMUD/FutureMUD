...
public class CommonUtilitiesTests
{
    ...
    {
        TestEnum value1 = TestEnum.Third | TestEnum.Fourth | TestEnum.Seventh;
        TestEnum value2 = TestEnum.Second;
        ...
        List<Enum> values = value1.GetFlags().ToList();
        ...
        List<Enum> values2 = value2.GetFlags().ToList();
        ...
        IEnumerable<TestEnum> values1a = value1.GetAllFlags();
    ...
    {
        if (!"Gigantic".TryParseEnum<SizeCategory>(out SizeCategory value))
        {
        ...
        if (!"Ground Level".TryParseEnum<RoomLayer>(out RoomLayer rlvalue))
        {
        ...
        if (!ProgVariableTypes.TryParse("Terrain", out ProgVariableTypes fpvalue))
        {
    ...
    public void TestCounter()
    {
        ...
        try
        {
            ...
            Counter<string> caseIntensitiveCounter = new(StringComparer.OrdinalIgnoreCase) { { "balloon", 4 } };
    ...
    {
        IncrementingCounter[,] array = new IncrementingCounter[10, 10];
        for (int i = 0; i < 10; i++)
        {
            for (int j = 0; j < 10; j++)
            {
        ...
        int count = array.ApplyFunctionToAdjacentsReturnCount(5, 5, value =>
        {
            value.Value++;
            return true;
        });
    ...
    public void TestFindCoordinates()
    {
        ...
        (int x, int y) = array.GetCoordsOfElement("twenty four");
    ...
    {
        BetterRandom rng = new(0, 8675309);
        List<int> results = new();
        Counter<bool> count = new();
        for (int i = 0; i < 50000; i++)
        {
            int result = rng.Next32i();
            results.Add(result);
            count[result >= 0] += 1;
        }
        ...
        BetterRandom rng2 = new(3, 8675309);
    ...
    {
        XElement element = XElement.Parse(@"<Strategy type=""hierarchy""><Member prog=""24"" ><Strategy type=""jail"" ><Minimum>1 weeks</Minimum><Maximum>2 months</Maximum></Strategy></Member><Member prog=""1"" ><Strategy type=""bond"" ><Length>12 months</Length></Strategy></Member></Strategy>");
    ...
    {
        string expression1 = "1d10";
        string expression2 = "4d6k3";
        string expression3 = "10d6e6";
        string expression4 = "3d6m8";
        string expression5 = "1d10e10M15";
        ...
        sb.AppendLine();
        foreach (KeyValuePair<int, int> value in counter.OrderBy(x => x.Key))
        {
    ...
    public void TestPassword()
    {
        ...
        string result = Encoding.UTF8.GetString(SHA384.Create().ComputeHash(Encoding.UTF8.GetBytes(password + salt)));
