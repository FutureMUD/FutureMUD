...
public class DiceTests
{
    ...
    {
        (int result, IEnumerable<int> rolls) = Dice.Roll("1d1r1", true);
        Assert.AreEqual(1, result);
        CollectionAssert.AreEqual(new List<int> { 1, 1 }, rolls.ToList());

        (int _, IEnumerable<int> emptyRolls) = Dice.Roll("1d1r1", false);
    ...
    {
        (int result, IEnumerable<int> rolls) = Dice.Roll("1d6R6", true);
    ...
    {
        int seed = FindSeedForExplode(2, 2);
        List<int> expectedRolls = GetExplodingRolls(seed, 1, 2, 2);
        SeedRandom(seed);

        (int result, IEnumerable<int> rolls) = Dice.Roll("1d2e2", true);
        List<int> rollList = rolls.ToList();
    ...
    private static int FindSeedForExplode(int sides, int threshold, int maxSeed = 10000)
    {
        for (int seed = 1; seed <= maxSeed; seed++)
        {
            ...
            int first = random.Next(1, sides + 1);
            int second = random.Next(1, sides + 1);
