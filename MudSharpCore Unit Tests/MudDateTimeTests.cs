...
public class MudDateTimeTests
{
    ...
    {
        Mock<ISaveManager> saveManagerMock = new();
    ...
    {
        bool assertionHit1 = false;
        bool assertionHit2 = false;
        DateListener listener2 = new(_testCalendar, 28, "jung", 34, _utcTimezone, 0, objs => assertionHit1 = true, new object[] { }, "Test");
        DateListener listener1 = new(_testCalendar, 1, "kepler", 34, _utcTimezone, 0, objs => assertionHit2 = true, new object[] { }, "Test");

        _testCalendar.CurrentDate.AdvanceDays(1);
        Assert.AreEqual(true, assertionHit1, "Didn't advance from 27th to 28th");

        MudDate expectedNextDate = _testCalendar.GetDate("1/kep/34");

        RecurringInterval daily = new()
        {
            IntervalAmount = 1,
            Modifier = 0,
    ...
    {
        bool assertionHit1 = false;
        DateListener listener1 = new(_testCalendar, 28, "jung", 34, _cstTimezone, 0, objs => assertionHit1 = true, new object[] { }, "Test");
        _testCalendar.CurrentDate.AdvanceDays(1);
    ...
    {
        Clock clock = new(XElement.Parse(@"<Clock>  <Alias>UTC</Alias>  <Description>Universal Time Clock</Description>  <ShortDisplayString>$j:$m:$s $i</ShortDisplayString>  <SuperDisplayString>$j:$m:$s $i $t</SuperDisplayString>  <LongDisplayString>$c $i</LongDisplayString>  <SecondsPerMinute>60</SecondsPerMinute>  <MinutesPerHour>60</MinutesPerHour>  <HoursPerDay>24</HoursPerDay>  <InGameSecondsPerRealSecond>2</InGameSecondsPerRealSecond>  <SecondFixedDigits>2</SecondFixedDigits>  <MinuteFixedDigits>2</MinuteFixedDigits>  <HourFixedDigits>0</HourFixedDigits>  <NoZeroHour>true</NoZeroHour>  <NumberOfHourIntervals>2</NumberOfHourIntervals>  <HourIntervalNames>    <HourIntervalName>a.m</HourIntervalName>    <HourIntervalName>p.m</HourIntervalName>  </HourIntervalNames>  <HourIntervalLongNames>    <HourIntervalLongName>in the morning</HourIntervalLongName>    <HourIntervalLongName>in the afternoon</HourIntervalLongName>  </HourIntervalLongNames>  <CrudeTimeIntervals>    <CrudeTimeInterval text=""night"" Lower=""-2"" Upper=""4""/>    <CrudeTimeInterval text=""morning"" Lower=""4"" Upper=""12""/>    <CrudeTimeInterval text=""afternoon"" Lower=""12"" Upper=""18""/>    <CrudeTimeInterval text=""evening"" Lower=""18"" Upper=""22""/>  </CrudeTimeIntervals></Clock>"), _gameworld)
        {
            Id = 1
        };
        Calendar calendar = new(XElement.Parse(@"<calendar>
  ...
  <intercalarymonths />
        ...
        MudDate cd = calendar.CurrentDate;

        cd.AdvanceDays(1);
        Assert.IsTrue(cd.Day == 16 && cd.Month.Alias == "january" && cd.Year == 2020, $"Advancing 1 day from 15th to 16th January failed: {cd}");

        cd.AdvanceDays(15);
        Assert.IsTrue(cd.Day == 31 && cd.Month.Alias == "january" && cd.Year == 2020, $"Advancing 15 days from 16th to 31st January failed: {cd}");

        cd.AdvanceDays(2);
        Assert.IsTrue(cd.Day == 2 && cd.Month.Alias == "february" && cd.Year == 2020, $"Advancing 2 days from 31st Jan to 2nd Feb failed: {cd}");

        cd.AdvanceDays(-2);
        Assert.IsTrue(cd.Day == 31 && cd.Month.Alias == "january" && cd.Year == 2020, $"Advancing -2 days to 31st Jan from 2nd Feb failed: {cd}");

        cd.AdvanceDays(-31);
        Assert.IsTrue(cd.Day == 31 && cd.Month.Alias == "december" && cd.Year == 2019, $"Advancing -31 days from 31st January to 31st December failed: {cd}");

        cd.AdvanceMonths(1, false, true);
        Assert.IsTrue(cd.Day == 31 && cd.Month.Alias == "january" && cd.Year == 2020, $"Advancing 1 month from 31st December to 31st January failed: {cd}");

        cd.AdvanceMonths(1, false, true);
        Assert.IsTrue(cd.Day == 29 && cd.Month.Alias == "february" && cd.Year == 2020, $"Advancing 1 month from 31st January to 29th February failed: {cd}");

        cd.AdvanceMonths(1, false, true);
        Assert.IsTrue(cd.Day == 29 && cd.Month.Alias == "march" && cd.Year == 2020, $"Advancing 1 month from 29th February to 29th March failed: {cd}");

        calendar.SetDate("31/01/2020");
        cd = calendar.CurrentDate;

        cd.AdvanceMonths(2, false, true);
        Assert.IsTrue(cd.Day == 31 && cd.Month.Alias == "march" && cd.Year == 2020, $"Advancing 2 months from 31st January to 31st March failed: {cd}");

        cd.AdvanceMonths(-4, false, true);
        Assert.IsTrue(cd.Day == 30 && cd.Month.Alias == "november" && cd.Year == 2019, $"Advancing -4 months from 31st March to 30th November failed: {cd}");

        cd.AdvanceYears(1, false);
        Assert.IsTrue(cd.Day == 30 && cd.Month.Alias == "november" && cd.Year == 2020, $"Advancing 1 year from 30 November 2019 to 30 November 2020 failed: {cd}");

        cd.AdvanceYears(2, false);
        Assert.IsTrue(cd.Day == 30 && cd.Month.Alias == "november" && cd.Year == 2022, $"Advancing 2 years from 30 November 2020 to 30 November 2022 failed: {cd}");

        cd.AdvanceYears(-10, false);
        Assert.IsTrue(cd.Day == 30 && cd.Month.Alias == "november" && cd.Year == 2012, $"Advancing -10 years from 30 November 2022 to 30 November 2012 failed: {cd}");

        calendar.SetDate("29/02/2020");
        cd = calendar.CurrentDate;

        cd.AdvanceYears(1, false);
        Assert.IsTrue(cd.Day == 28 && cd.Month.Alias == "february" && cd.Year == 2021, $"Advancing 1 year from 29 Feb 2020 to 28 Feb 2021 failed: {cd}");

        cd.AdvanceYears(-1, false);
        Assert.IsTrue(cd.Day == 28 && cd.Month.Alias == "february" && cd.Year == 2020, $"Advancing -1 year to 29 Feb 2020 from 28 Feb 2021 failed: {cd}");

        calendar.SetDate("29/02/2020");
        cd = calendar.CurrentDate;

        cd.AdvanceYears(1, true);
        Assert.IsTrue(cd.Day == 1 && cd.Month.Alias == "march" && cd.Year == 2021, $"Advancing 1 year (with date normalisation) from 29 Feb 2020 to 1st Mar 2021 failed: {cd}");

        calendar.SetDate("01/01/2020");
        cd = calendar.CurrentDate;

        cd.AdvanceDays(366);
        Assert.IsTrue(cd.Day == 1 && cd.Month.Alias == "january" && cd.Year == 2021, $"Advancing 366 days from 31 Jan 2020 to 31 Jan 2021 failed: {cd}");

        cd.AdvanceDays(-366);
        Assert.IsTrue(cd.Day == 1 && cd.Month.Alias == "january" && cd.Year == 2020, $"Advancing -366 days to 31 Jan 2020 from 31 Jan 2021 failed: {cd}");
    }

    [TestMethod]
    public void TestMudTimeSpan()
    {
        bool result = MudTimeSpan.TryParse("24:0:0", out MudTimeSpan mts);
    ...
    {
        Calendar calendar1 = new(XElement.Parse(@"<calendar>
   ...
   <intercalarymonths>
        ...
        Calendar calendar2 = new(XElement.Parse(@"<calendar>
