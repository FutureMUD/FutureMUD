...
public class PlanetaryMoonTests
{
        ...
        MoonPhase expectedPhase)
    {
        ...
        double dayNumber = context.Moon.CurrentDayNumber;
        (double rightAscension, double declination) = context.Moon.EquatorialCoordinates(dayNumber);
        CelestialInformation position = context.Moon.CurrentPosition(context.DefaultGeography);
    ...
    public void CurrentPosition_NonStandardClockUsesActualClockLengthForDirectionSampling()
    {
        ...
        try
        {
            ...
            for (int minute = 1; minute < 20; minute++)
            {
                ...
                CelestialInformation current = context.Moon.CurrentPosition(context.DefaultGeography);

                context.Clock.CurrentTime.SetTime((minute - 1) / 10, (minute - 1) % 10, 0);
                CelestialInformation previous = context.Moon.CurrentPosition(context.DefaultGeography);
