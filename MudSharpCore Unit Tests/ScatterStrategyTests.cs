...
public class ScatterStrategyUtilitiesTests
{
    ...
    {
        ScatterTestHelpers.CellNetworkBuilder builder = new();
        ScatterTestHelpers.CellNetworkBuilder.CellData origin = builder.AddCell("origin", (0, 0, 0));
        ScatterTestHelpers.CellNetworkBuilder.CellData north = builder.AddCell("north", (0, 1, 0));
        ScatterTestHelpers.CellNetworkBuilder.CellData east = builder.AddCell("east", (1, 0, 0));
        builder.ConnectTwoWay("origin", "north", CardinalDirection.North);
        builder.ConnectTwoWay("origin", "east", CardinalDirection.East);
        builder.Finalise();

        Mock<IPerceiver> target = ScatterTestHelpers.CreatePerceiver(origin.Cell.Object);
        IReadOnlyList<CellScatterInfo> infos = ScatterStrategyUtilities.GetCellInfos(target.Object, 2, true);

        Assert.AreEqual(3, infos.Count);
        CellScatterInfo originInfo = infos.Single(x => x.Cell == origin.Cell.Object);
        Assert.AreEqual(0, originInfo.Distance);
        Assert.AreEqual(CardinalDirection.Unknown, originInfo.DirectionFromOrigin);

        CellScatterInfo northInfo = infos.Single(x => x.Cell == north.Cell.Object);
        Assert.AreEqual(1, northInfo.Distance);
        Assert.AreEqual(CardinalDirection.North, northInfo.DirectionFromOrigin);

        CellScatterInfo eastInfo = infos.Single(x => x.Cell == east.Cell.Object);
    ...
    public void GetCellInfos_RespectsClosedDoors()
    {
        ...
        Mock<IPerceiver> target = ScatterTestHelpers.CreatePerceiver(origin.Cell.Object);
        IReadOnlyList<CellScatterInfo> blocked = ScatterStrategyUtilities.GetCellInfos(target.Object, 1, true);
        Assert.IsFalse(blocked.Any(x => x.Cell == north.Cell.Object));

        IReadOnlyList<CellScatterInfo> unblocked = ScatterStrategyUtilities.GetCellInfos(target.Object, 1, false);
...
public class BallisticScatterStrategyTests
{
    ...
    public void GetScatterTarget_PrefersForwardDirectionAndReturnsCellWhenNoTarget()
    {
        ...
        ScatterTestHelpers.CellNetworkBuilder.CellData origin = builder.AddCell("origin", (0, 0, 0));
        ScatterTestHelpers.CellNetworkBuilder.CellData north = builder.AddCell("north", (0, 1, 0));
        ScatterTestHelpers.CellNetworkBuilder.CellData south = builder.AddCell("south", (0, -1, 0));
        builder.ConnectTwoWay("origin", "north", CardinalDirection.North);
        builder.ConnectTwoWay("origin", "south", CardinalDirection.South);
        builder.Finalise();

        Mock<ICharacter> shooter = ScatterTestHelpers.CreateCharacter(origin.Cell.Object);
        Mock<IPerceiver> target = ScatterTestHelpers.CreatePerceiver(origin.Cell.Object);
        List<ICellExit> path = ScatterTestHelpers.CreatePath(CardinalDirection.North);
        HashSet<CardinalDirection> directionSet = ScatterTestHelpers.GetDirectionSet(path);
        IReadOnlyList<CellScatterInfo> cellInfos = ScatterStrategyUtilities.GetCellInfos(target.Object, 3, true);
        List<(CellScatterInfo Info, double Weight)> weights = ScatterTestHelpers.ComputeDirectionalWeights(typeof(BallisticScatterStrategy), cellInfos, directionSet,
...
public class ArcingScatterStrategyTests
{
    ...
    public void GetScatterTarget_PrefersCloserCellsWhenNoTarget()
    {
        ...
        Mock<ICharacter> shooter = ScatterTestHelpers.CreateCharacter(origin.Cell.Object);
        Mock<IPerceiver> target = ScatterTestHelpers.CreatePerceiver(origin.Cell.Object);
        IReadOnlyList<CellScatterInfo> cellInfos = ScatterStrategyUtilities.GetCellInfos(target.Object, 1, true);
        List<(CellScatterInfo Info, double Weight)> weights = ScatterTestHelpers.ComputeArcingWeights(cellInfos);
        ...
        RangedScatterResult result = ArcingScatterStrategy.Instance.GetScatterTarget(shooter.Object, target.Object, Array.Empty<ICellExit>());
...
public class LightScatterStrategyTests
{
    ...
    public void GetScatterTarget_FavorsContinuationAndReturnsCell()
    {
        ...
        List<ICellExit> path = ScatterTestHelpers.CreatePath(CardinalDirection.North);
        HashSet<CardinalDirection> directionSet = ScatterTestHelpers.GetDirectionSet(path);
        IReadOnlyList<CellScatterInfo> cellInfos = ScatterStrategyUtilities.GetCellInfos(target.Object, 5, true);
        List<(CellScatterInfo Info, double Weight)> weights = ScatterTestHelpers.ComputeDirectionalWeights(typeof(LightScatterStrategy), cellInfos, directionSet,
        path.Last().OutboundDirection);
...
public class SpreadScatterStrategyTests
{
    ...
    public void GetScatterTarget_DropsIntoOriginWhenNoTargets()
    {
        ...
        RangedScatterResult result = SpreadScatterStrategy.Instance.GetScatterTarget(shooter.Object, target.Object, Array.Empty<ICellExit>());
    ...
    public void GetScatterTarget_StrikesAvailableTarget()
    {
        ...
        Mock<ICharacter> shooter = ScatterTestHelpers.CreateCharacter(origin.Cell.Object);
        Mock<IPerceiver> target = ScatterTestHelpers.CreatePerceiver(origin.Cell.Object);
        Mock<ICharacter> victim = ScatterTestHelpers.CreateCharacter(origin.Cell.Object);
...
internal static class ScatterTestHelpers
{
    internal sealed class CellNetworkBuilder
    {
        ...
        {
            CellData origin = _cells[originName];
            CellData destination = _cells[destinationName];
            Mock<IExit> exit = new();
            ...
            Mock<ICellExit> forward = CreateExit(origin.Cell.Object, destination.Cell.Object, direction, exit.Object);
            Mock<ICellExit> reverse = CreateExit(destination.Cell.Object, origin.Cell.Object, direction.Opposite(), exit.Object);
        ...
        {
            foreach ((Mock<ICell> cell, List<ICellExit> exits) in _exits)
            {
    ...
    {
        MethodInfo method = strategyType.GetMethod("CellWeight", BindingFlags.Static | BindingFlags.NonPublic);
        List<(CellScatterInfo, double)> results = new();
        foreach (CellScatterInfo info in infos)
        {
    ...
    {
        MethodInfo method = typeof(ArcingScatterStrategy).GetMethod("CellWeight", BindingFlags.Static | BindingFlags.NonPublic);
        List<(CellScatterInfo, double)> results = new();
        foreach (CellScatterInfo info in infos)
        {
