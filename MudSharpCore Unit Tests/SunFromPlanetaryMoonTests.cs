...
public class SunFromPlanetaryMoonTests
{
        ...
        TimeOfDay expectedTimeOfDay)
    {
        ...
        double dayNumber = context.SunFromMoon.CurrentDayNumber;
        (double rightAscension, double declination) =
            CelestialTestAssertions.SunFromMoonEquatorialCoordinates(context.SunFromMoon, dayNumber);
        CelestialInformation position = context.SunFromMoon.CurrentPosition(context.ZeroGeography);
    ...
    public void CurrentPosition_ConjunctionCaseUsesRelativePositionAndRemainsStable()
    {
        ...
        (double rightAscension, double declination) =
            CelestialTestAssertions.SunFromMoonEquatorialCoordinates(context.SunFromMoon, dayNumber);
        double rootRightAscension = context.Sun.RightAscension(dayNumber);
        double rootDeclination = context.Sun.Declension(dayNumber);
    ...
    public void CurrentPosition_NonStandardClockUsesActualClockLengthForDirectionSampling()
    {
        ...
        try
        {
            ...
            for (int minute = 1; minute < 20; minute++)
            {
                ...
                CelestialInformation current = context.SunFromMoon.CurrentPosition(context.DefaultGeography);

                context.Clock.CurrentTime.SetTime((minute - 1) / 10, (minute - 1) % 10, 0);
                CelestialInformation previous = context.SunFromMoon.CurrentPosition(context.DefaultGeography);
