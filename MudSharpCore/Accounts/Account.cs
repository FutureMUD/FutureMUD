...
public sealed class Account : SaveableItem, IAccount
{
    ...
    public Account(MudSharp.Models.Account account, IFuturemud gameworld, IAuthority authority = null)
    {
        ...
        {
            AccountsChargenResources dbres = account.AccountsChargenResources.FirstOrDefault(x => x.ChargenResourceId == resource.Id);
            if (dbres == null)
            {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Account dbitem = FMDB.Context.Accounts.Find(Id);
