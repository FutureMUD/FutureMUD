...
public sealed class ArenaCombatantClass : SaveableItem, ICombatantClass
{
    ...
    public ArenaCombatantClass(CombatArena arena, string name, IFutureProg eligibilityProg)
    {
        ...
        using (new FMDB())
        {
            Models.ArenaCombatantClass dbItem = new()
            {
                ArenaId = arena.Id,
                Name = name,
                Description = string.Empty,
    ...
    private bool BuildingCommandLoader(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg? prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Character, [ProgVariableTypes.Location]).LookupProg();
        if (prog == null)
        {
            return false;
        }
    ...
    private bool BuildingCommandStageName(ICharacter actor, StringStack command)
    {
        ...
        IRandomNameProfile? profile = Gameworld.RandomNameProfiles.GetByIdOrName(command.SafeRemainingArgument);
        if (profile is null)
        {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.ArenaCombatantClass? dbItem = FMDB.Context.ArenaCombatantClasses.Find(Id);
            if (dbItem == null)
            {
                return;
            }
