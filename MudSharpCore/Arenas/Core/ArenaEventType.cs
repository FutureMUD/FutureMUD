...
public sealed class ArenaEventType : SaveableItem, IArenaEventType
{
        ...
        Func<long, ICombatantClass?> classLookup, IArenaEliminationStrategy? eliminationStrategy = null)
    {
        ...
        foreach (Models.ArenaEventTypeSide? side in model.ArenaEventTypeSides)
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine("Sides:");
        sb.AppendLine();
        foreach (IArenaEventTypeSide? side in _sides.OrderBy(x => x.Index))
        {
    ...
    private bool BuildingCommandRegistration(ICharacter actor, StringStack command)
    {
        ...
        if (!MudTimeSpan.TryParse(command.SafeRemainingArgument, actor, out MudTimeSpan? mts))
        {
    ...
    private bool BuildingCommandAutoSchedule(ICharacter actor, StringStack command)
    {
        ...
        string firstToken = command.PopForSwitch();
    ...
    private bool BuildingCommandBetting(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<BettingModel>(out BettingModel model))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandAppearance(ICharacter actor, StringStack command)
    {
        ...
        if (!Arena.Currency.TryGetBaseCurrency(command.SafeRemainingArgument, out decimal value))
        {
    ...
    private bool BuildingCommandScoringProg(ICharacter actor, StringStack command)
    {
        ...
        ProgVariableTypes requiredType = EliminationMode == ArenaEliminationMode.PointsElimination
            ? ProgVariableTypes.Number | ProgVariableTypes.Collection
            : ProgVariableTypes.Void;
        IFutureProg? prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, requiredType,
            ArenaProgParameters.EventProgParameterSets).LookupProg();
        if (prog == null)
        {
            return false;
    ...
    private bool BuildingCommandEloStyle(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<ArenaEloStyle>(out ArenaEloStyle style))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandEloKFactor(ICharacter actor, StringStack command)
    {
        ...
        if (!decimal.TryParse(command.SafeRemainingArgument, out decimal value) || value <= 0.0m)
        {
    ...
    private bool BuildingCommandElimination(ICharacter actor, StringStack command)
    {
        ...
        if (!TryParseEliminationMode(command.SafeRemainingArgument, out ArenaEliminationMode mode))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandSide(ICharacter actor, StringStack command)
    {
        ...
        if (!ArenaSideIndexUtilities.TryParseDisplayIndex(command.PopSpeech(), out int index))
        {
            actor.OutputHandler.Send("You must specify the numeric index of the side starting at 1.");
            return false;
        }

        IArenaEventTypeSide? side = _sides.FirstOrDefault(x => x.Index == index);
        if (side == null)
        {
    ...
    public IArenaEventType Clone(string newName, ICharacter originator)
    {
        ...
        using (new FMDB())
        {
            Models.ArenaEventType dbType = new()
            {
                ArenaId = Arena.Id,
                Name = newName,
                BringYourOwn = BringYourOwn,
            ...
            foreach (ArenaEventTypeSide side in _sides.OfType<ArenaEventTypeSide>())
            {
                Models.ArenaEventTypeSide dbSide = new()
                {
                    Index = side.Index,
                ...
                foreach (ICombatantClass cls in side.EligibleClasses)
                {
            ...
            ArenaEventType newType = new(dbType, Arena, Arena.GetCombatantClass, EliminationStrategy);
    ...
    public override void Save()
    {
        ...
        using (new FMDB())
        {
            Models.ArenaEventType? dbType = FMDB.Context.ArenaEventTypes.Find(Id);
            if (dbType == null)
            {
                return;
            }
