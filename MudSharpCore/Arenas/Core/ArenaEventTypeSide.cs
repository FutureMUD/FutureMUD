...
The NPC Loader prog is executed once and has the same parameters but returns a collection of characters loaded.";
    ...
    private bool BuildingCommandPolicy(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<ArenaSidePolicy>(out ArenaSidePolicy policy))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandRating(ICharacter actor, StringStack command)
    {
        ...
        if (!decimal.TryParse(first, out decimal min))
        {
        ...
        if (command.IsFinished || !decimal.TryParse(command.PopSpeech(), out decimal max))
        {
    ...
    private bool BuildingCommandRatingBound(ICharacter actor, StringStack command, bool isMinimum)
    {
        ...
        decimal? newMinimum = isMinimum ? value : MinimumRating;
        decimal? newMaximum = isMinimum ? MaximumRating : value;
    ...
    private bool BuildingCommandClass(ICharacter actor, StringStack command)
    {
        ...
        ICombatantClass? cls = EventType.Arena.CombatantClasses
            .FirstOrDefault(x => x.Name.EqualTo(command.SafeRemainingArgument));
        if (cls == null && long.TryParse(command.SafeRemainingArgument, out long id))
        {
    ...
    public override void Save()
    {
        ...
        using (new FMDB())
        {
            Models.ArenaEventTypeSide? dbSide = FMDB.Context.ArenaEventTypeSides.Find(Id);
            if (dbSide == null)
            {
                return;
            }
            ...
            foreach (ICombatantClass cls in _eligibleClasses)
            {
