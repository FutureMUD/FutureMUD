...
public sealed class CombatArena : SaveableItem, ICombatArena
{
    ...
    public CombatArena(Arena arena, IFuturemud gameworld)
    {
        ...
        foreach (ArenaManager? manager in arena.ArenaManagers)
        {
        ...
        foreach (Models.ArenaCombatantClass? combatant in arena.ArenaCombatantClasses)
        {
            _combatantClasses.Add(new ArenaCombatantClass(combatant, this));
        }

        foreach (Models.ArenaEventType? type in arena.ArenaEventTypes)
        {
            _eventTypes.Add(new ArenaEventType(type, this, GetCombatantClass));
        }

        Dictionary<long, ArenaEventType> typesById = _eventTypes.OfType<ArenaEventType>().ToDictionary(x => x.Id);
        foreach (Models.ArenaEvent? ev in arena.ArenaEvents)
        {
            if (!typesById.TryGetValue(ev.ArenaEventTypeId, out ArenaEventType? eventType))
            {
                continue;
            }

            ArenaEvent arenaEvent = new(ev, this, eventType);
            _events.Add(arenaEvent);
        }

        foreach (IArenaEvent evt in _events)
        {
        ...
        IEnumerable<IArenaReservation>? reservations = null)
    {
        ...
        ArenaEvent newEvent = new(concrete, this, scheduledFor, reservations);
    ...
    public (bool Truth, string Reason) EnsureFunds(decimal amount)
    {
        ...
        decimal available = AvailableFunds();
        if (available >= amount)
        {
            return (true, string.Empty);
        }
    ...
    public string Show(ICharacter actor)
    {
        ...
        {
            foreach (ICombatantClass combatantClass in CombatantClasses)
            {
        ...
        {
            foreach (IArenaEventType type in EventTypes)
            {
                sb.AppendLine(
    ...
    public string ShowToManager(ICharacter actor)
    {
        ...
        foreach (IArenaEventType type in EventTypes)
        {
            string autoSchedule = type.AutoScheduleEnabled && type.AutoScheduleInterval.HasValue &&
...
The phase transition prog is executed once for each side and passes in the following parameters:
	...
	#kNumber#0 #marenaid#0 - the id of the arena
	#kNumber#0 #mid#0 - the id of the event type
	#kText#0 #mevent#0 - the name of the event type
	#kText#0 #mphase#0 - the phase of the arena fight

The valid phases are #6Draft#0, #6Scheduled#0, #6RegistrationOpen#0, #6Preparing#0, #6Staged#0, #6Live#0, #6Resolving#0, #6Cleanup#0, #6Completed#0, and #6Aborted#0.";
    ...
    private bool BuildingCommandSignupEcho(ICharacter actor, StringStack command)
    {
        ...
        Emote emote = new(emoteText, actor);
        if (!emote.Valid)
        {
    ...
    private bool BuildingCommandManager(ICharacter actor, StringStack command)
    {
        switch (command.PopForSwitch())
        {
            case "add":
                ...
                if (!long.TryParse(command.SafeRemainingArgument, out long addId))
                {
                    actor.OutputHandler.Send("You must specify the numeric ID of the manager to add.");
                    return false;
                }

                ICharacter addCharacter = Gameworld.TryGetCharacter(addId, true);
            ...
            case "remove":
                ...
                if (!long.TryParse(command.SafeRemainingArgument, out long removeId))
                {
                    actor.OutputHandler.Send("You must specify the numeric ID of the manager to remove.");
                    return false;
                }

                ICharacter removeCharacter = Gameworld.TryGetCharacter(removeId, true);
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Arena? dbArena = FMDB.Context.Arenas.Find(Id);
            if (dbArena == null)
            {
                return;
            }
            ...
            if (_managersDirty)
            {
                ...
                foreach (long manager in _managerIds)
                {
