...
public class ArenaLifecycleService : IArenaLifecycleService
{
    ...
    public void Transition(IArenaEvent arenaEvent, ArenaEventState targetState)
    {
        ...
        ArenaEventState initialState = arenaEvent.State;
        ...
        for (ArenaEventState state = initialState; state < targetState;)
        {
            ArenaEventState nextState = GetNextState(state);
    ...
    {
        List<IArenaEvent> interruptedEvents = _gameworld.CombatArenas
        ...
        foreach (IArenaEvent? arenaEvent in interruptedEvents)
        {
