...
public class ArenaNpcService : IArenaNpcService
{
    ...
    public IEnumerable<ICharacter> AutoFill(IArenaEvent arenaEvent, int sideIndex, int slotsNeeded)
    {
        ...
        IArenaEventTypeSide? side = arenaEvent.EventType.Sides.FirstOrDefault(x => x.Index == sideIndex);
        if (side is null)
        {
            return Enumerable.Empty<ICharacter>();
        }
        ...
        int remaining = slotsNeeded - candidates.Count;
        if (remaining <= 0)
        {
            return candidates;
        }

        List<ICharacter> generated = side.NpcLoaderProg.ExecuteCollection<ICharacter>(
        ...
        foreach (ICharacter? npc in generated)
        {
            if (candidates.Count >= slotsNeeded)
            {
                break;
    ...
    private static void RestoreInventory(ICharacter npc, ArenaNpcPreparationEffect effect)
    {
        ...
        if (body is null)
        {
            foreach (ArenaNpcPreparationEffect.ArenaNpcItemSnapshot snapshot in effect.Items)
            {
    ...
    {
        foreach (ICombatantClass combatantClass in side.EligibleClasses)
        {
            try
            {
