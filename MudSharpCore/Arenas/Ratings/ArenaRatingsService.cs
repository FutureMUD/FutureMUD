...
public class ArenaRatingsService : IArenaRatingsService
{
    ...
    private void UpdateRatingsByCharacterId(IArenaEvent arenaEvent, IReadOnlyDictionary<long, decimal> deltasByCharacterId)
    {
        ...
        Dictionary<long, ArenaRatingParticipant> participantsByCharacterId = SelectRateableParticipants(arenaEvent)
        ...
        foreach ((long characterId, decimal delta) in deltasByCharacterId)
        {
            if (!participantsByCharacterId.TryGetValue(characterId, out ArenaRatingParticipant? participant))
            {
                continue;
            }
    ...
    public void ApplyDefaultElo(IArenaEvent arenaEvent)
    {
        ...
        List<ArenaRatingParticipant> participants = SelectRateableParticipants(arenaEvent);
        if (participants.Count < 2)
        {
            return;
        }

        (ArenaOutcome? outcome, IReadOnlyCollection<int>? winningSides) = ResolveOutcome(arenaEvent);
        ...
        IReadOnlyDictionary<long, decimal> deltaByCharacterId = strategy.CalculateDeltas(
            participants,
            outcome.Value,
            winningSet,
            kFactor,
    ...
    {
        ArenaOutcome? outcome = TryExtractOutcome(arenaEvent);
        IReadOnlyCollection<int>? winningSides = TryExtractWinningSides(arenaEvent);
    ...
    private static bool TryConvertOutcome(object? value, out ArenaOutcome outcome)
    {
        switch (value)
        {
            ...
            case ArenaOutcome typed:
                outcome = typed;
                return true;
            case string text when Enum.TryParse<ArenaOutcome>(text, true, out ArenaOutcome parsed):
                outcome = parsed;
                return true;
