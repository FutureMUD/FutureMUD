...
public class ScarTemplate : DisfigurementTemplate, IScarTemplate
{
    public ScarTemplate(MudSharp.Models.DisfigurementTemplate dbitem, IFuturemud gameworld) : base(dbitem, gameworld)
    {
        ...
        foreach (XElement element in root.Element("Damages").Elements())
        {
    ...
    public override string WhyCannotSubmit()
    {
        ...
        {
            return
                "You must change the short description of this scar to something different to the default placeholder.";
        }

        if (FullDescription.EqualTo(DefaultFullDescription))
        {
            return
                "You must change the full description of this scar to something different to the default placeholder.";
        }
    ...
    private bool BuildingCommandDamage(ICharacter actor, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<DamageType>(command.PopSpeech(), out DamageType value))
        {
            actor.OutputHandler.Send(
        ...
        if (!Utilities.TryParseEnum<WoundSeverity>(command.PopSpeech(), out WoundSeverity sValue))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandHealingChance(ICharacter actor, StringStack command, bool damageHealing)
    {
        if (command.IsFinished)
        {
    ...
    private bool BuildingCommandSurgery(ICharacter actor, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<SurgicalProcedureType>(command.PopSpeech(), out SurgicalProcedureType value))
        {
            actor.OutputHandler.Send(
