...
public class TattooTemplate : DisfigurementTemplate, ITattooTemplate
{
    ...
    {
        XElement root = XElement.Parse(dbitem.Definition);
        ...
        foreach (XElement element in root.Element("ChargenCosts")?.Elements() ?? Enumerable.Empty<XElement>())
        {
        ...
        foreach (XElement element in root.Element("Inks").Elements())
        {
        ...
        foreach (XElement element in root.Element("TextSlots")?.Elements("TextSlot") ?? Enumerable.Empty<XElement>())
        {
    ...
    public TattooTemplate(TattooTemplate rhs, IAccount originator, string name) : base(rhs, originator, name)
    {
        ...
        foreach (KeyValuePair<IChargenResource, int> cost in rhs.ChargenCosts)
        {
    ...
    public override string Show(ICharacter actor)
    {
        ...
        sb.AppendLine(
            $"Required Inks: {_inkColours.Select(x => $"{x.Key.Name} [w{x.Value:N2}]".ColourValue()).ListToString()}");
        sb.AppendLine(
    ...
    private bool BuildingCommandOverride(ICharacter actor, StringStack command)
    {
        ...
        string plain = command.PopSpeech();
        if (command.IsFinished)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandSkill(ICharacter actor, StringStack command)
    {
        ...
        IRangeTraitValueDecorator rtvd = TattooistTrait.Decorator as IRangeTraitValueDecorator;
        if ((rtvd == null || actor.IsAdministrator()) && double.TryParse(skillText, out double value))
        {
            if (value < 0)
            {
        ...
        string range = TattooistTrait.Decorator.OrderedDescriptors.FirstOrDefault(x => x.EqualTo(skillText));
    ...
    private bool BuildingCommandInk(ICharacter actor, StringStack command)
    {
        ...
        IColour colour = long.TryParse(command.PopSpeech(), out long value)
        ...
        if (!command.IsFinished)
        {
            if (!double.TryParse(command.PopSpeech(), out double dvalue) || dvalue <= 0.0)
            {
    ...
    private bool BuildingCommandKnowledge(ICharacter actor, StringStack command)
    {
        ...
        List<IKnowledge> knowledges = actor.IsAdministrator() ? Gameworld.Knowledges.ToList() : actor.Knowledges.ToList();
        IKnowledge knowledge = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandChargen(ICharacter actor, StringStack command)
    {
        ...
        Regex costsRegex = new("(?<amount>\\d+) (?<resource>\\D+)");
    ...
    private bool BuildingCommandSize(ICharacter actor, StringStack command)
    {
        ...
        if (!GameItemEnumExtensions.TryParseSize(command.PopSpeech(), out SizeCategory size))
        {
    ...
    public override string WhyCannotSubmit()
    {
        ...
        List<string> invalidSlots = GetInvalidTextSlotReferences().ToList();
        if (invalidSlots.Any())
        {
            return
    ...
    public IEnumerable<(IColour Colour, double Amount)> InkColours
    {
        get
        {
            double totalInkVolume = Gameworld.GetStaticDouble("TattooInkTotalPerSize") * (int)Size / TicksToCompleteTattoo;
            double totalInkWeights = _inkColours.Sum(x => x.Value);
    ...
    public string ResolveDescription(string description, IReadOnlyDictionary<string, ITattooTextValue> textValues)
    {
        ...
        {
            TattooTemplateTextSlot slot = _textSlots.FirstOrDefault(x => x.Name.EqualTo(match.Groups["name"].Value));
            if (slot == null)
            {
                return match.Value;
            }
            ...
            if (textValues != null &&
                textValues.TryGetValue(slot.Name, out ITattooTextValue value) &&
                value != null)
            {
    ...
    {
        string fallbackText = string.IsNullOrWhiteSpace(slot.DefaultText) ? "None".ColourError() : slot.DefaultText.ColourValue();
        return
    ...
    private bool BuildingCommandTextSlot(ICharacter actor, StringStack command)
    {
        if (command.IsFinished)
        {
            IEnumerable<string[]> rows = _textSlots.Select(x => new[]
            {
                x.Name.ColourName(),
        ...
        string firstArgument = command.PopSpeech();
        ...
        TattooTemplateTextSlot slot = _textSlots.FirstOrDefault(x => x.Name.EqualTo(firstArgument));
        if (slot == null)
        {
    ...
    private bool BuildingCommandTextSlotAdd(ICharacter actor, StringStack command)
    {
        ...
        if (command.IsFinished || !int.TryParse(command.PopSpeech(), out int maximumLength) || maximumLength <= 0)
        {
    ...
    private bool BuildingCommandTextSlotRemove(ICharacter actor, StringStack command)
    {
        ...
        TattooTemplateTextSlot slot = _textSlots.FirstOrDefault(x => x.Name.EqualTo(command.PopSpeech()));
        if (slot == null)
        {
            actor.OutputHandler.Send("There is no such tattoo text slot.");
            return false;
        ...
        List<string> references = GetTextSlotReferences().Where(x => x.EqualTo(slot.Name)).ToList();
        if (references.Any())
        {
            actor.OutputHandler.Send(
    ...
    {
        if (command.IsFinished || !bool.TryParse(command.PopSpeech(), out bool value))
        {
    ...
    private bool BuildingCommandTextSlotLanguage(ICharacter actor, TattooTemplateTextSlot slot, StringStack command)
    {
        ...
        ILanguage language = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandTextSlotScript(ICharacter actor, TattooTemplateTextSlot slot, StringStack command)
    {
        ...
        IScript script = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandTextSlotStyle(ICharacter actor, TattooTemplateTextSlot slot, StringStack command)
    {
        ...
        WritingStyleDescriptors style = WritingStyleDescriptors.None;
        foreach (string item in command.SafeRemainingArgument.Split(' ', StringSplitOptions.RemoveEmptyEntries))
        {
            foreach (string subItem in item.Split(',', StringSplitOptions.RemoveEmptyEntries))
            {
