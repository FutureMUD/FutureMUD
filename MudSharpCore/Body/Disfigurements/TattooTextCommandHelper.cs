...
public static class TattooTextCommandHelper
{
        ...
        out string errorMessage)
    {
        ...
        Dictionary<string, ITattooTextValue> values = new(StringComparer.InvariantCultureIgnoreCase);
        while (!command.IsFinished)
        {
            string raw = command.PopSpeech();
            ...
            string slotName = raw[..equalsIndex];
            ITattooTemplateTextSlot slot = template.GetTextSlot(slotName);
            if (slot == null)
            {
            ...
            if (allowCopyFromWriting && valueText.StartsWith("copy:", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                bool copiedUnreadably = !actor.CanRead(writing);
        ...
        List<string> missingRequired = template.TextSlots
