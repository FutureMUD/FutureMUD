...
public class TattooTextValue : ITattooTextValue
{
    ...
    {
        string details = $"{Language?.Id ?? 0},{Script?.Id ?? 0}";
        if (MinimumSkill > 0.0)
        {
            details += $",skill={MinimumSkill:F2}";
        }

        if (Style != WritingStyleDescriptors.None)
        {
            foreach (WritingStyleDescriptors flag in Style.GetFlags().OfType<WritingStyleDescriptors>().Where(x => x != WritingStyleDescriptors.None))
            {
