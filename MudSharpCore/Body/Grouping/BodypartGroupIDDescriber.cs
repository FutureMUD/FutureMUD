...
public class BodypartGroupIDDescriber : BodypartGroupDescriber
{
    ...
    private BodypartGroupIDDescriber(BodypartGroupIDDescriber rhs)
    {
        ...
        Comment = rhs.Comment;
        foreach (KeyValuePair<IBodypart, bool> item in rhs.Prototypes)
        {
    ...
    private void DoDatabaseInsert()
    {
        using (new FMDB())
        {
            ...
            foreach (KeyValuePair<IBodypart, bool> item in Prototypes)
            {
    ...
    public override BodypartGroupResult Match(IEnumerable<IBodypart> parts)
    {
        ...
        List<IBodypart> matches = parts.Where(part => Prototypes.Keys.Any(x => MatchesPrototype(part, x))).ToList();
    ...
    public override void FinaliseLoad(MudSharp.Models.BodypartGroupDescriber describer, IFuturemud gameworld)
    {
        ...
        foreach (BodypartGroupDescribersBodypartProtos part in describer.BodypartGroupDescribersBodypartProtos)
        {
