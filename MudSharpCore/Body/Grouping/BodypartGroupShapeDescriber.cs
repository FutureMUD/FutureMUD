...
public class BodypartGroupShapeDescriber : BodypartGroupDescriber
{
    public BodypartGroupShapeDescriber(MudSharp.Models.BodypartGroupDescriber rule, IFuturemud gameworld)
    {
        ...
        _id = rule.Id;
        foreach (BodypartGroupDescribersShapeCount shape in rule.BodypartGroupDescribersShapeCount)
        {
    ...
    private BodypartGroupShapeDescriber(BodypartGroupShapeDescriber rhs)
    {
        ...
        Comment = rhs.Comment;
        foreach (KeyValuePair<IBodypartShape, (int Min, int Max)> item in rhs.ShapeCount)
        {
    ...
    private bool BuildingCommandShape(ICharacter actor, StringStack ss)
    {
        ...
        IBodypartShape shape = Gameworld.BodypartShapes.GetByIdOrName(ss.PopSpeech());
        if (shape is null)
        {
        ...
        if (!int.TryParse(ss.PopSpeech(), out int min) || min < 0)
        {
        ...
        if (!int.TryParse(ss.PopSpeech(), out int max) || max < 0)
        {
            actor.OutputHandler.Send($"The text {ss.Last.ColourCommand()} is not a valid number 0 or greater.");
            return false;
        }
    ...
    {
        Models.BodypartGroupDescriber dbitem = FMDB.Context.BodypartGroupDescribers.Find(Id);
        ...
        foreach (KeyValuePair<IBodypartShape, (int Min, int Max)> item in ShapeCount)
        {
    ...
    private void DoDatabaseInsert()
    {
        using (new FMDB())
        {
            Models.BodypartGroupDescriber dbitem = new()
            {
                Comment = Comment,
                DescribedAs = DescribedAs,
                Type = "shape",
    ...
    public override BodypartGroupResult Match(IEnumerable<IBodypart> parts)
    {
        ...
        {
            IEnumerable<IBodypart> remains = RemainderShapes(parts);
