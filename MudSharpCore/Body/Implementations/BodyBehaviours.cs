...
public partial class Body
{
    ...
    public void Open(IOpenable openable, ICharacter openableOwner, IEmote playerEmote, bool useCouldLogic = false)
    {
        if (useCouldLogic)
        {
            ...
            {
                case WhyCannotOpenReason.Locked:
                    ILockable lockable = openable.Parent.GetItemType<ILockable>();
                    if (lockable == null)
                    {
                        return;
                    }

                    List<ILock> locks = lockable.Locks.ToList();
                    List<IGameItem> keysRequired = new();
                    foreach (IKey key in ExternalItems.SelectMany(x => x.ShallowAccessibleItems(Actor))
                    ...
                    IInventoryPlan plan = InventoryPlanFactory.CreatePhaseForEachItem(Actor, keysRequired, DesiredItemState.Held,
                        "key");
