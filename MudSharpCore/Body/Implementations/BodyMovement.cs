...
public partial class Body
{
    ...
    {
        double mult = Actor.CombinedEffectsOfType<IStaminaExpenditureEffect>()
    ...
    public void SpendStamina(double amount)
    {
        ...
        {
            foreach (ISecondWindMerit availableSecondWind in SecondWindsAvailable())
            {
    ...
    {
        List<ISecondWindMerit> availableSecondWind = Actor.Merits.OfType<ISecondWindMerit>().Where(x =>
    ...
    public double EncumbrancePercentage
    {
        get
        {
            double weight = ExternalItems.Sum(x => x.Weight);
            double limit = EncumbranceLimitExpression.Evaluate(Actor, null, TraitBonusContext.Encumbrance);
            if (limit <= 0.0)
            {
                return 0.0;
            }
    ...
    public void StaminaTenSecondHeartbeat()
    {
        ...
        if (CurrentStamina < MaximumStamina)
        {
            ...
            double mult = Actor.CombinedEffectsOfType<IStaminaRegenerationRateEffect>()
                            .Aggregate(1.0, (x, y) => x * y.Multiplier);
            GainStamina(regen * mult);
        }

        double staminaSpentRatio = _staminaSpentInCurrentPeriod / MaximumStamina;
    ...
    public void StaminaMinuteHeartbeat()
    {
        ...
        if (LongtermExertion > ExertionLevel.Normal && Race.SweatLiquid != null && !isDead)
        {
            ...
            double sweatTotal = Race.SweatRateInLitresPerMinute * multiplier;
            ...
            if (Race.SweatLiquid != null && sweatTotal > 0.0)
            {
                ...
                double bodypartHitChanceSum = Math.Max(1.0, Bodyparts.Sum(x => x.RelativeHitChance));

                foreach (IGrouping<IWear, (IGameItem Item, IWear Wearloc, IWearlocProfile Profile)> part in _wornItems.GroupBy(x => x.Wearloc))
                {
        ...
        {
            RPG.Checks.ICheck check = Gameworld.GetCheck(RPG.Checks.CheckType.ArmourUseCheck);
