...
public partial class Body : IHaveNeeds, IEat
{
    ...
    public (bool Success, string ErrorMessage) Eat(ICorpse corpse, double bites, IEmote playerEmote)
    {
        ...
        (bool Success, string ErrorMessage) result = CanEat(corpse, bites);
        if (!result.Success)
        {
        ...
        double bitesLeft = corpse.RemainingEdibleWeight / Actor.Race.BiteWeight;
    ...
    public (bool Success, string ErrorMessage) Eat(ISeveredBodypart bodypart, double bites, IEmote playerEmote)
    {
        ...
        (bool Success, string ErrorMessage) result = CanEat(bodypart, bites);
        if (!result.Success)
        {
            Actor.OutputHandler.Send(result.ErrorMessage);
            return result;
        ...
        double bitesLeft = bodypart.RemainingEdibleWeight / Actor.Race.BiteWeight;
        Actor.OutputHandler.Handle(
            new MixedEmoteOutput(new Emote(
                string.Format(Actor.Race.EatCorpseEmoteText, BiteDescriber(bites, bitesLeft)), Actor, Actor,
    ...
    public (bool Success, string ErrorMessage) Eat(string foragableYield, double bites, IEmote playerEmote)
    {
        ...
        (bool Success, string ErrorMessage) result = CanEat(foragableYield, bites);
        if (!result.Success)
        {
            Actor.OutputHandler.Send(result.ErrorMessage);
            return result;
        ...
        double bitesLeft = Actor.Location.GetForagableYield(foragableYield) / Actor.Race.BiteYield(foragableYield);
        (string emote, INeedFulfiller fulfiler, double yieldconsumed) = Actor.Race.EatYield(foragableYield, bites);
        Actor.OutputHandler.Handle(
    ...
    public bool SilentSwallow(ISwallowable swallowable)
    {
        ...
        IEdible edible = swallowable.Parent.GetItemType<IEdible>();
        if (edible != null)
        {
            FulfilNeeds(new NeedFulfiller
            {
