...
public partial class Body
{
    ...
    public IGameItem SeverBodypart(IBodypart bodypart)
    {
        ...
        HashSet<IBodypart> severedParts = Bodyparts.Where(x => x.DownstreamOfPart(bodypart)).ToHashSet();
        ...
        HashSet<IGameItem> heldOrWieldedItems =
            _wieldedItems.Where(x => severedParts.Any(y => y.CountsAs(x.Item2)))
                         ...
                         .ToHashSet();
        foreach (IGameItem item in heldOrWieldedItems)
        {
            Take(item);
        ...
        HashSet<IGameItem> wornItems =
            _wornItems.Where(x => severedParts.Contains(x.Item2) && x.Item3.Mandatory)
                      .Select(x => x.Item1)
                      .ToHashSet();
        foreach (IGameItem item in wornItems)
        {
            Take(item);
        }

        List<IWound> removedWounds =
            Wounds.Where(
        ...
        List<IBodypart> severedPartsAndOrgans = severedParts.SelectMany(x => x.Organs).Distinct().Concat(severedParts).ToList();
        List<IBone> severedBones = severedParts.SelectMany(x => x.Bones).Distinct().ToList();

        List<IImplant> removedImplants =
            _implants.Where(x => severedPartsAndOrgans.Any(y => y.CountsAs(x.TargetBodypart))).ToList();
        foreach (IImplant implant in removedImplants)
        {
            RemoveImplant(implant);
        }

        List<ITattoo> removedTattoos = _tattoos.Where(x => severedPartsAndOrgans.Any(y => y.CountsAs(x.Bodypart))).ToList();
        foreach (ITattoo tattoo in removedTattoos)
        {
        ...
        IGameItem newPart = BodypartGameItemComponentProto.CreateNewSeveredBodypart(Actor, severedParts, wornItems,
    ...
    public void RestoreBodypart(IBodypart bodypart)
    {
        if (_severedRoots.Remove(bodypart))
        {
            ...
            if (limb != null)
            {
                HashSet<long> bodypartIDs = Bodyparts.Where(x => limb.Parts.Contains(x)).Select(x => x.Id).ToHashSet();
                HashSet<IBodypart> baseBodyparts =
                    Prototype.BodypartsFor(Race, Gender.Enum).Where(x => limb.Parts.Contains(x)).ToHashSet();
                HashSet<IBodypart> missingParts = baseBodyparts.Where(x => !bodypartIDs.Contains(x.Id)).ToHashSet();
    ...
    public IGameItem ExciseOrgan(IOrganProto organ)
    {
        ...
        List<IWound> removedWounds = Wounds.Where(x => x.Bodypart == organ).ToList();
        foreach (IWound wound in removedWounds)
        {
            wound.Delete();
        }
        ...
        List<IImplant> removedImplants = _implants.Where(x => x.TargetBodypart == organ).ToList();
        foreach (IImplant implant in removedImplants)
        {
            RemoveImplant(implant);
        }

        IGameItem newPart = BodypartGameItemComponentProto.CreateNewSeveredBodypart(Actor, new[] { organ },
    ...
    public void RecalculatePartsAndOrgans()
    {
        ...
        HashSet<IBodypart> allParts = Prototype
        ...
        HashSet<IBodypart> newBodyparts = allParts.Where(x => x is IExternalBodypart).ToHashSet();
        HashSet<IOrganProto> organs = allParts.OfType<IOrganProto>()
        ...
        HashSet<IBone> bones = allParts.OfType<IBone>()
                            .Where(x => !severedParts.Contains(x) &&
                                        newBodyparts.Any(y =>
    ...
    public CanUseLimbResult CanUseLimb(ILimb limb)
    {
        ...
        CanUseLimbResult result = CanUseLimbResult.CanUse;
        List<ILimbIneffectiveEffect> limbEffects = EffectsOfType<ILimbIneffectiveEffect>().Where(x => x.Applies(limb) && x.AppliesToLimb(limb))
                                                                 .ToList();
    ...
    {
        bool result = CheckBodypartDamage();

        foreach (ILimb limb in Limbs)
        {
            CanUseLimbResult canUseLimbResult = EvaluateCanUseLimb(limb);
    ...
    {
        CanUseLimbResult result = CanUseLimbResult.CanUse;
        List<IAdditionalBodypartsMerit> merits = Merits.OfType<IAdditionalBodypartsMerit>().Where(x => x.Applies(this)).ToList();
        HashSet<IBodypart> baseBodyparts = Prototype
                            .BodypartsFor(Race, Gender.Enum)
                            .Concat(merits.SelectMany(x => x.AddedBodyparts(Actor)))
                            .Distinct()
        ...
        List<IWound> wounds = GetWoundsForLimb(limb).ToList();
        double maxHP = HealthStrategy.MaxHP(Actor);
    ...
    private bool LimbDamageEffects(ILimb limb, CanUseLimbResult why)
    {
        ...
        bool result = false;
        foreach (IGameItem item in limb.Parts.OfType<IGrab>().SelectMany(HeldOrWieldedItemsFor).ToList())
        {
            switch (why)
            {
                case CanUseLimbResult.CantUseDamage:
    ...
    {
        bool result = false;
        List<IGrouping<IBodypart, IWound>> woundsQuery = Wounds.Where(x => !(x.Bodypart is IOrganProto) && !(x.Bodypart is IBone))
                                .GroupBy(x => x.Bodypart).ToList();
        foreach (IGrouping<IBodypart, IWound> part in woundsQuery)
        {
            if (part.Key == null)
            {
                continue;
    ...
    {
        (Orientation, Alignment) roll = BodyExtensions.GetRandomHitLocation(orientation, alignment, facing, appendagesActive);
        List<IBodypart> eligableBodyparts =
            Bodyparts.Where(
    ...
    {
        double merits = Merits.Concat(Actor.Merits).Where(x => x.Applies(Actor)).OfType<IBodypartHealthMerit>()
    ...
    {
        foreach (BodiesImplants item in body.BodiesImplants.Where(x => _implants.All(y => y.Parent.Id != x.ImplantId)).ToList())
        {
            body.BodiesImplants.Remove(item);
        }

        foreach (Models.GameItem dbitem in _implants.Where(x => body.BodiesImplants.All(y => y.ImplantId != x.Parent.Id)).ToList()
    ...
    {
        foreach (BodiesImplants item in body.BodiesImplants)
        {
            IImplant implant = Gameworld.TryGetItem(item.Implant, true)?.GetItemType<IImplant>();
            if (implant != null)
            {
                implant.InstallImplant(this);
                _implants.Add(implant);
        ...
        foreach (IImplant item in Implants)
        {
    ...
    private void LoadTattoos(MudSharp.Models.Body body)
    {
        ...
        {
            XElement root = XElement.Parse(body.Tattoos);
            foreach (XElement tattoo in root.Elements("Tattoo"))
            {
    ...
    private void LoadScars(MudSharp.Models.Body body)
    {
        ...
        {
            XElement root = XElement.Parse(body.Scars);
            foreach (XElement scar in root.Elements("Scar"))
            {
