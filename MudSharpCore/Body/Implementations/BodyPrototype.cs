...
public class BodyPrototype : SaveableItem, IBodyPrototype
{
    ...
    public BodyPrototype(BodyProto proto, IFuturemud game)
    {
        ...
        {
            _validPositions = new List<IPositionState>();
            foreach (BodyProtosPositions position in proto.BodyProtosPositions)
            {
        ...
        foreach (BodypartGroupDescribersBodyProtos rule in proto.BodypartGroupDescribersBodyProtos)
        {
        ...
        foreach (Models.MoveSpeed speed in proto.MoveSpeeds)
        {
            MoveSpeed gspeed = new(speed);
        ...
        foreach (IPositionState state in _speeds.Select(x => x.Position).Distinct())
        {
    ...
    {
        List<IBodypart> allParts = Gameworld.BodypartPrototypes.Where(x => x.Body == this).ToList();
        List<IExternalBodypart> partsOnly = allParts.OfType<IExternalBodypart>().ToList();
    ...
    {
        foreach (IBodypart part in additions)
        {
    ...
    {
        foreach (IBodypart part in removals)
        {
    ...
    public IEnumerable<IBodypart> BodypartsFor(IRace race, Gender gender)
    {
        ...
        if (_countsAsId.HasValue)
        {
            IBodyPrototype parent = Gameworld.BodyPrototypes.Get(_countsAsId.Value);
        ...
        foreach (IBodypart part in race?.BodypartRemovals ?? Enumerable.Empty<IBodypart>())
        {
            parts.Remove(part);
        }
    ...
    public override void Save()
    {
        ...
        foreach (IPositionState position in _validPositions)
        {
...
#if DEBUG
        foreach (IExternalBodypart item in AllExternalBodyparts)
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        List<(IBodypart Part, bool FromParent)> parts = new();
        foreach (IBodypart part in AllBodypartsBonesAndOrgans)
        {
