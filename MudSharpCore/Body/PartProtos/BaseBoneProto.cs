...
public abstract class BaseBoneProto : BodypartPrototype, IBone
{
    ...
    {
        foreach (BoneOrganCoverage organ in proto.BoneOrganCoveragesBone)
        {
    ...
    public override string ShowToBuilder(ICharacter builder)
    {
        ...
        sb.AppendLine();
        (IBodypart Part, BodypartInternalInfo Info)[] parts = Body.AllBodyparts.Select(x => (Part: x, Info: x.BoneInfo.ValueOrDefault(this, null))).Where(x => x.Item2 is not null).ToArray();
    ...
    private bool BuildingCommandUncover(ICharacter builder, StringStack command)
    {
        ...
        IOrganProto organ = Body.Organs.FirstOrDefault(x => x.Name.EqualTo(name));
        if (organ == null)
        {
        ...
        using (new FMDB())
        {
            ...
            foreach (BoneOrganCoverage cover in FMDB.Context.BoneOrganCoverages.Where(x => x.BoneId == Id).ToList())
            {
    ...
    private bool BuildingCommandCover(ICharacter builder, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParsePercentage(builder.Account.Culture, out double hitchance) || hitchance <= 0.0)
        {
        ...
        if (_coveredOrgans.Any(x => x.Organ == organ))
        {
            ...
            using (new FMDB())
            {
                BoneOrganCoverage dbitem = FMDB.Context.BoneOrganCoverages.First(x => x.BoneId == Id && x.OrganId == organ.Id);
    ...
    private bool BuildingCommandRemoveInside(ICharacter builder, StringStack command)
    {
        ...
        string name = command.PopSpeech();
        IBodypart part = Body.AllBodyparts.FirstOrDefault(x => x.Name.EqualTo(name));
        if (part == null)
        {
    ...
    private bool BuildingCommandInside(ICharacter builder, StringStack command)
    {
        ...
        if (part.BoneInfo.ContainsKey(this))
        {
            ...
            using (new FMDB())
            {
                ...
                BodypartInternalInfos dbinternal = dbtarget.BodypartInternalInfosBodypartProto.First(x => x.InternalPartId == Id);
    ...
    private bool BuildingCommandPrimary(ICharacter builder, StringStack command)
    {
        ...
        foreach (KeyValuePair<IBone, BodypartInternalInfo> bone in part.BoneInfo)
        {
            if (bone.Key != this)
            {
        ...
        using (new FMDB())
        {
            foreach (BodypartInternalInfos dbitem in FMDB.Context.BodypartInternalInfos
