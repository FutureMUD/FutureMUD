...
public abstract partial class BodypartPrototype : LateKeywordedInitialisingItem, IBodypart
{
    ...
    public override object DatabaseInsert()
    {
        ...
        ILimb limb = Body.Limbs.FirstOrDefault(x => x.Parts.Contains(this));
        if (limb != null)
        {
    ...
    {
        BodypartProto dbitem = FMDB.Context.BodypartProtos.Find(Id);
        if (dbitem == null)
        {
    ...
    private bool BuildingCommandFemale(ICharacter builder, StringStack command)
    {
        using (new FMDB())
        {
            ...
            BodyProtosAdditionalBodyparts newAdditional = new()
            {
                BodypartId = Id,
                BodyProtoId = Body.Id,
                Usage = "female"
    ...
    private bool BuildingCommandCountsAs(ICharacter builder, StringStack command)
    {
        ...
        IBodypart target = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandImplantSpace(ICharacter builder, StringStack command)
    {
        ...
        double organs = OrganInfo.Where(x => x.Value.IsPrimaryInternalLocation).Sum(x => x.Key.ImplantSpaceOccupied);
    ...
    private bool BuildingCommandArmour(ICharacter builder, StringStack command)
    {
        ...
        IArmourType armour = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandLife(ICharacter builder, StringStack command)
    {
        ...
        if (!uint.TryParse(command.PopSpeech(), out uint value))
        {
    ...
    private bool BuildingCommandAlignment(ICharacter builder, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<Alignment>(command.PopSpeech(), out Alignment value))
        {
            builder.OutputHandler.Send(
    ...
    private bool BuildingCommandOrientation(ICharacter builder, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<Orientation>(command.PopSpeech(), out Orientation value))
        {
            builder.OutputHandler.Send(
    ...
    private bool BuildingCommandType(ICharacter builder, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<BodypartTypeEnum>(command.PopSpeech(), out BodypartTypeEnum type))
        {
            builder.OutputHandler.Send(
