...
public abstract class ExternalBodypartProto : BodypartPrototype, IExternalBodypart
{
    ...
    private bool BuildingCommandUpstream(ICharacter builder, StringStack command)
    {
        ...
        ILimb oldLimb = Body.Limbs.FirstOrDefault(x => x.Parts.Contains(this));
        ILimb newLimb;
        List<ILimb> newLimbs = Body.Limbs.Where(x => DownstreamOfPart(x.RootBodypart)).ToList();
        if (newLimbs.Count > 1)
        {
            ILimb bodyRootLimb = newLimbs.FirstOrDefault(x => x.RootBodypart.UpstreamConnection == null);
