...
public class GrabbingWieldingBodypartProto : GrabbingBodypartProto, IWield
{
    ...
    {
        StringBuilder sb = new(ShowToBuilderSkipThis(builder));
    ...
    public override bool PartDamageEffects(IBody owner, CanUseBodypartResult why)
    {
        ...
        List<IGameItem> items = owner.WieldedItemsFor(this).Concat(owner.HeldItemsFor(this)).ToList();
        if (!items.Any())
        {
            return result;
        }

        ILimb limb = owner.GetLimbFor(this);
