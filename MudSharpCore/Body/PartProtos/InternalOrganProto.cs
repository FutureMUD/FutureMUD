...
public abstract class InternalOrganProto : BodypartPrototype, IOrganProto
{
    ...
    public override string ShowToBuilder(ICharacter builder)
    {
        ...
        sb.AppendLine();
        (IBodypart Part, BodypartInternalInfo Info)[] parts = Body.AllBodyparts.Select(x => (Part: x, Info: x.OrganInfo.ValueOrDefault(this, null))).Where(x => x.Item2 is not null).ToArray();
        ...
        sb.AppendLine();
        (IBone Part, (IOrganProto Organ, BodypartInternalInfo Info) Info)[] bones = Body.AllBodypartsBonesAndOrgans.OfType<IBone>().Select(x => (Part: x, Info: x.CoveredOrgans.FirstOrDefault(y => y.Organ == this))).Where(x => x.Info.Organ is not null).ToArray();
        sb.AppendLine(StringUtilities.GetTextTable(
            from bone in bones
            select new List<string>
            {
    ...
    private bool BuildingCommandRemoveInside(ICharacter builder, StringStack command)
    {
        ...
        IExternalBodypart part = Body.AllExternalBodyparts.FirstOrDefault(x => x.Name.EqualTo(name));
        if (part == null)
        {
    ...
    private bool BuildingCommandInside(ICharacter builder, StringStack command)
    {
        ...
        if (part.OrganInfo.ContainsKey(this))
        {
            bool primary = part.OrganInfo[this].IsPrimaryInternalLocation;
    ...
    private bool BuildingCommandPrimary(ICharacter builder, StringStack command)
    {
        ...
        foreach (KeyValuePair<IOrganProto, BodypartInternalInfo> organ in part.OrganInfo)
        {
            if (organ.Key != this)
            {
