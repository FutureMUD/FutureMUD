...
public abstract class PositionState : FrameworkItem, IPositionState
{
    ...
    {
        return StateDictionary.TryGetValue(id, out IPositionState value) ? value : PositionUndefined.Instance;
    }

    [CanBeNull]
    public static IPositionState GetState(string stateText)
    {
        if (long.TryParse(stateText, out long value))
        {
            return GetState(value);
        }

        foreach (IPositionState position in StateDictionary.Values)
        {
        ...
        IPerceivable newTarget)
    {
        ...
        else
        {
            ...
            IChair newChair = newTargetItem?.GetItemType<IChair>();
