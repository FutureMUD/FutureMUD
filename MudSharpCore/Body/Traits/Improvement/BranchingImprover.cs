...
public class BranchingImprover : ClassicImprovement
{
    ...
    protected BranchingImprover(BranchingImprover rhs, string name)
    {
        ...
        using (new FMDB())
        {
            XElement definition = rhs.SaveDefinition();
            Models.Improver dbitem = new()
            {
                Name = name,
                Type = "branching",
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement element in root.Element("Branches").Elements())
        {
            ITraitDefinition trait = Gameworld.Traits.Get(long.Parse(element.Attribute("base").Value));
            ITraitDefinition branch = Gameworld.Traits.Get(long.Parse(element.Attribute("branch").Value));
    ...
    private bool BuildingCommandBranch(ICharacter actor, StringStack command)
    {
        ...
        ITraitDefinition baseTrait = Gameworld.Traits.GetByIdOrName(command.PopSpeech());
        ...
        if (!double.TryParse(command.PopSpeech(), out double branch))
        {
        ...
        if (!double.TryParse(command.PopSpeech(), out double open))
        {
            actor.OutputHandler.Send($"The text {command.Last.ColourCommand()} is not a valid number.");
            return false;
        }
