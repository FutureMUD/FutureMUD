...
public class DerivedAttributeDefinition : DerivedTraitDefinition, IAttributeDefinition
{
    ...
    protected void DoDatabaseInsert()
    {
        using (new FMDB())
        {
            Models.TraitExpression dbexpr = new()
            {
            ...
            TraitExpression expr = new(dbexpr, Gameworld);
            Gameworld.Add(expr);
            Expression = expr;

            Models.TraitDefinition dbitem = new()
            {
                Name = Name,
    ...
    private bool BuildingCommandAlias(ICharacter actor, StringStack command)
    {
        ...
        string alias = command.SafeRemainingArgument.ToLowerInvariant();
    ...
    private bool BuildingCommandExpression(ICharacter actor, StringStack command)
    {
        ...
        ITraitExpression expr = Gameworld.TraitExpressions.GetByIdOrName(command.SafeRemainingArgument);
        if (expr is null)
        {
