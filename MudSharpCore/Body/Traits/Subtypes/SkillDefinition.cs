...
public class SkillDefinition : TraitDefinition, ISkillDefinition
{
        ...
        : base(trait, game)
    {
        ...
        foreach (Models.TraitDefinitionsChargenResources item in trait.TraitDefinitionsChargenResources)
        {
    ...
    {
        Models.TraitDefinition dbitem = FMDB.Context.TraitDefinitions.Find(Id);
        dbitem.Name = Name;
    ...
    public override string Show(ICharacter voyeur)
    {
        ...
        if (ResourceCosts.Any())
        {
            ...
            sb.AppendLine();
            foreach ((IChargenResource resource, int cost) cost in ResourceCosts)
            {
                sb.AppendLine(
...
Note: Most often you will want to use the #3TRAITEXPRESSION#0 command to edit the existing trait expression rather than changing to a new one";
    ...
    private bool BuildingCommandExpression(ICharacter actor, StringStack command)
    {
        ...
        ITraitExpression expression = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandImprover(ICharacter actor, StringStack command)
    {
        ...
        IImprovementModel improver = long.TryParse(command.SafeRemainingArgument, out long value)
