...
public class NewSun : PerceivedItem, ICelestialObject
{
    ...
    {
        XElement element = root.Element("Illumination");
        if (element != null)
        {
        ...
        if (element != null)
        {
            foreach (XElement sub in element.Elements("Trigger"))
            {
                Triggers.Add(
        ...
        if (element != null)
        {
            foreach (XElement sub in element.Elements("Description"))
            {
    ...
    {
        double mean = MeanAnomaly(dayNumber);
        return (mean +
    ...
    {
        double semiMajorAxis = OrbitalSemiMajorAxis > 0.0 ? OrbitalSemiMajorAxis : 1.0;
        double trueAnomaly = TrueAnomaly(dayNumber);
        double denominator = 1.0 + OrbitalEccentricity * Math.Cos(trueAnomaly);
    ...
    public (double X, double Y, double Z) PositionVector(double dayNumber)
    {
        ...
        double rightAscension = RightAscension(dayNumber);
        double declination = Declension(dayNumber);
    ...
    {
        double eclipticLongitude = EclipticLongitudeOfSun(dayNumber);
    ...
    {
        double ha = HourAngle(dayNumber, coordinate);
    ...
    {
        double decl = Declension(dayNumber);
    ...
    private double OneMinuteTimeFraction
    {
        get
        {
            double minutesPerDay = (double)(Clock?.HoursPerDay ?? 0) * (Clock?.MinutesPerHour ?? 0);
    ...
    protected CelestialMoveDirection CurrentDirection(GeographicCoordinate geography)
    {
        ...
        double current = Altitude(dn, geography);
        double former = Altitude(dn - OneMinuteTimeFraction, geography);
    ...
    {
        CelestialInformation newStatus = CurrentPosition(geography);
