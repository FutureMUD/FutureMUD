...
public class PlanetaryMoon : PerceivedItem, ICelestialObject
{
    ...
    {
        double m = MeanAnomaly(dayNumber);
    ...
    {
        double semiMajorAxis = OrbitalSemiMajorAxis > 0.0 ? OrbitalSemiMajorAxis : 0.00257;
    ...
    public (double RA, double Dec) EquatorialCoordinates(double dayNumber)
    {
        ...
        double z = Math.Sin(wv) * Math.Sin(OrbitalInclination);
    ...
    public (double X, double Y, double Z) PositionVector(double dayNumber)
    {
        ...
        (double rightAscension, double declination) = EquatorialCoordinates(dayNumber);
    ...
    {
        (double ra, double dec) = EquatorialCoordinates(dayNumber);
    ...
    private double PhaseAngle()
    {
        ...
        double cycleDay = (CurrentCelestialDay - FullMoonReferenceDay).Modulus(CelestialDaysPerYear);
        return 2 * Math.PI * (cycleDay / CelestialDaysPerYear);
    }

    public MoonPhase CurrentPhase()
    {
        double frac = (CurrentCelestialDay - FullMoonReferenceDay).Modulus(CelestialDaysPerYear) / CelestialDaysPerYear;
