...
public partial class Character : PerceiverItem, ICharacter
{
    ...
    public Character(IFuturemud gameworld, ICharacterTemplate template)
    {
        ...
        foreach (IChargenRole role in template.SelectedRoles)
        {
            ...
            foreach (IGrouping<IClan, IRoleClanMembership> clan in template.SelectedRoles.SelectMany(x => x.ClanMemberships).GroupBy(x => x.Clan))
            {
                IRank rank = clan.Select(x => x.Rank).FirstMax(x => x.RankNumber);
            ...
            foreach (KeyValuePair<ITraitDefinition, (double amount, bool giveIfMissing)> adjustment in role.TraitAdjustments)
            {
    ...
    public override double IlluminationProvided
    {
        ...
        {
            double sum = 0.0;
            foreach (IProduceIllumination effect in CombinedEffectsOfType<IProduceIllumination>().Where(x => x.Applies()))
            {
...
#nullable restore
    ...
    public string ShowHealth(IPerceiver voyeur)
    {
        ...
        if (NeedsToBreathe)
        {
            List<Tuple<LungProto, double>> lungs = Body.Organs.OfType<LungProto>().Select(x => Tuple.Create(x, x.OrganFunctionFactor(Body)))
                            .ToList();
            List<Tuple<TracheaProto, double>> tracheas = Body.Organs.OfType<TracheaProto>().Select(x => Tuple.Create(x, x.OrganFunctionFactor(Body)))
                               .ToList();
            if (CanBreathe)
            {
            ...
            foreach (Tuple<LungProto, double> lung in lungs)
            {
                if (lung.Item2 > 0.95)
            ...
            foreach (Tuple<TracheaProto, double> trachea in tracheas)
            {
        ...
        (double floor, double ceiling) = Body.TolerableTemperatures(true);
        sb.AppendLine(
        ...
        BodyTemperatureStatus tempStatus = HealthStrategy.CurrentTemperatureStatus(this);
        ...
        foreach (ISecondWindMerit merit in Merits.OfType<ISecondWindMerit>().Where(x => x.Applies(this)))
        {
        ...
        //Can you see?
        IEnumerable<EyeProto> eyes = Body.Bodyparts.OfType<EyeProto>();
        foreach (EyeProto eye in eyes.Where(x => Body.AffectedBy<IBodypartIneffectiveEffect>(x)))
        {
            sb.AppendLine($"You cannot see out of your {eye.FullDescription()}.".Colour(Telnet.BoldRed));
        }

        foreach (EyeProto cybereye in eyes.Where(x =>
        ...
        foreach (EyeProto blindfolded in eyes.Where(x => Body.WornItemsFor(x).Any(y => y.IsItemType<IBlindfold>())))
        {
        ...
        //Can you hear?
        IEnumerable<EarProto> ears = Body.Organs.OfType<EarProto>();
        foreach (EarProto ear in ears.Where(x => Body.AffectedBy<IBodypartIneffectiveEffect>(x)))
        {
            sb.AppendLine($"You cannot hear out of your {ear.FullDescription()}.".Colour(Telnet.BoldRed));
        }

        foreach (EarProto cyberear in ears.Where(x =>
                     Body.Prosthetics.Any(y => x.DownstreamOfPart(y.TargetBodypart) && !y.Functional)))
        {
            sb.Append(
        ...
        double bloodRatio = Body.CurrentBloodVolumeLitres / Body.TotalBloodVolumeLitres;
        if (bloodRatio <= 0.65)
        {
        ...
        List<Tuple<HeartProto, double>> hearts = Body.Organs.OfType<HeartProto>().Select(x => Tuple.Create(x, x.OrganFunctionFactor(Body)))
                         .ToList();
        if (hearts.Any())
        {
            foreach (Tuple<HeartProto, double> heart in hearts)
            {
                if (heart.Item2 <= 0.0)
                {
                    sb.AppendLine(
        ...
        sb.AppendLine();
        foreach (IBodypart part in Body.Bodyparts.Where(x => x.Organs.Any() || x.IsVital).ToList())
        {
            List<IWound> wounds = Body.Wounds.Where(x => x.Bodypart == part).ToList();
            List<IWound> internalWounds = Body.Wounds
            ...
            double painRatio = wounds.Sum(x => x.CurrentPain) * 0.5 / Body.HitpointsForBodypart(part);
            double internalStressRatio = internalWounds.Sum(x => Math.Max(x.CurrentDamage, x.CurrentPain)) /
                                      Body.HitpointsForBodypart(part);
            double totalRatio = painRatio + internalStressRatio;
        ...
        foreach (ILimb limb in Body.Limbs)
        {
            CanUseLimbResult canUseLimb = Body.CanUseLimb(limb);
        ...
        foreach (IGrouping<IBodypart, (IInfection infection, IBodypart part)> infection in Body.PartInfections.Select(x => (infection: x,
        ...
        {
            double intensity = infection.Sum(x => x.infection.Intensity);
            if (intensity < 100.0)
            {
                continue;
            }
        ...
        foreach (IScoreAddendumEffect effect in CombinedEffectsOfType<IScoreAddendumEffect>().Where(x => x.ShowInHealth))
        {
    ...
    public string ShowScore(IPerceiver voyeur)
    {
        ...
        NeedsResult status = NeedsModel.Status;
        ...
        foreach (IScoreAddendumEffect effect in CombinedEffectsOfType<IScoreAddendumEffect>().Where(x => x.ShowInScore))
        {
            sb.AppendLine(effect.ScoreAddendum);
        }

        IGuardCharacterEffect guardCharacter = EffectsOfType<IGuardCharacterEffect>().FirstOrDefault();
        if (guardCharacter != null)
        {
            sb.AppendLine($"You are guarding {guardCharacter.Targets.Select(x => x.HowSeen(this)).ListToString()}.");
        }

        IGuardItemEffect guardItem = EffectsOfType<IGuardItemEffect>().FirstOrDefault();
        if (guardItem != null)
        {
            sb.AppendLine(
        ...
        List<ILegalAuthority> bondEffects = EffectsOfType<GoodBehaviourBond>().Select(x => x.Authority).Distinct().ToList();
        if (bondEffects.Any())
        {
        ...
        List<(ILegalAuthority Legal, (decimal Fine, MudDateTime DueDate) Owed)> fines = Gameworld.LegalAuthorities.Select(x => (Legal: x, Owed: x.FinesOwed(this))).Where(x => x.Owed.Fine > 0.0M).ToList();
        foreach ((ILegalAuthority authority, (decimal fine, MudDateTime date)) in fines)
        {
        ...
        if (ClanMemberships.Any())
        {
            ...
            sb.AppendLine();
            foreach (IClanMembership clan in ClanMemberships)
            {
                sb.AppendLine(
    ...
    {
        string stringValue = gameworld.GetStaticConfiguration("MaximumNumberOfAliasProg");
        ...
        if (stringValue != null && double.TryParse(stringValue, out double dvalue))
        {
    ...
    {
        Models.Character item = (MudSharp.Models.Character)dbitem;
        _id = item.Id;
        foreach (IClanMembership cm in ClanMemberships)
        {
            cm.MemberId = item.Id;
        }
    ...
    public override object DatabaseInsert()
    {
        ...
        foreach (IChargenRole role in Roles.Distinct())
        {
        ...
        foreach (IClanMembership clan in ClanMemberships)
        {
            Models.ClanMembership dbclan = new()
            {
                Character = dbitem,
                ClanId = clan.Clan.Id,
                RankId = clan.Rank.Id,
        ...
        foreach (ICharacterKnowledge knowledge in _characterKnowledges)
        {
            Models.CharacterKnowledge dbknow = new()
            {
                Character = dbitem,
        ...
        foreach (IAccent accent in Accents)
        {
            CharacterAccent dbaccent = new()
            {
                Character = dbitem,
                AccentId = accent.Id,
    ...
    {
        foreach (ICharacterKnowledge knowledge in _addedKnowledges)
        {
            if (knowledge.Id == 0)
            {
        ...
        foreach (ICharacterKnowledge knowledge in _removedKnowledges)
        {
            Models.CharacterKnowledge dbitem = FMDB.Context.CharacterKnowledges.Find(knowledge.Id);
            if (dbitem == null)
            {
                continue;
            }
        ...
        foreach (Models.CharacterKnowledge item in dbchar.CharacterKnowledges)
        {
    ...
    private void LoadFromDatabase(MudSharp.Models.Character character)
    {
        ...
        foreach (PerceiverMerit merit in character.PerceiverMerits)
        {
        ...
        foreach (Models.CharacterKnowledge knowledge in character.CharacterKnowledges.ToList())
        {
        ...
        {
            IAccent iAccent = Gameworld.Accents.Get(accent.AccentId);
        ...
        foreach (ILanguage language in Gameworld.Languages.Where(
        ...
        foreach (CharactersScripts script in character.CharactersScripts)
        {
            _scripts.Add(Gameworld.Scripts.Get(script.ScriptId));
        }

        foreach (IScript script in Gameworld.Scripts.Where(
        ...
        foreach (ActiveJob item in character.ActiveJobs)
        {
    ...
    {
        XElement root = XElement.Parse(character.NameInfo);
        _personalName = new PersonalName(root.Element("PersonalName").Element("Name"), Gameworld);
        Aliases = new List<IPersonalName>();
        foreach (XElement element in root.Element("Aliases").Elements())
        {
            Aliases.Add(new PersonalName(element, Gameworld));
        }

        int current = int.Parse(root.Element("CurrentName").Value);
        if (current == 0)
        {
            _currentName = PersonalName;
        }
    ...
    {
        double amount = (double)(decimal)LiverFunctionProg.Execute(character);
    ...
    public IEnumerable<IGameItem> DeepContextualItems
    {
        get
        {
            ...
            foreach (IGameItem item in items.ToArray())
            {
                if (item.GetItemType<IContainer>() is IContainer ic)
                {
                    foreach (IGameItem subitem in ic.Contents.Where(x => ic.CanTake(this, x, 0)))
                    {
                        items.Add(subitem);
                    }
                }
    ...
    public bool Feel(string feeling)
    {
        ...
        PlayerEmote feelEmote = new(feeling, this, permitSpeech: PermitLanguageOptions.IgnoreLanguage);
        if (!feelEmote.Valid)
        {
        ...
        AppendableEmoteOutput output = new MixedEmoteOutput(new Emote("@ feel|feels", this)).Append(feelEmote);
    ...
    public void EditorModeMulti(Action<IEnumerable<string>, IOutputHandler, object[]> postAction, Action<IOutputHandler, object[]> cancelAction, IEnumerable<string> editorTexts, double characterLengthMultiplier = 1.0, string recallText = null, EditorOptions options = EditorOptions.None, object[] suppliedArguments = null)
    {
        ...
        NonPlayerOutputHandler newHandler = new();
    ...
    public virtual bool Quit(bool silent = false)
    {
        ...
        if (Location != null)
        {
            ...
            foreach (INPC bodyguard in Location.LayerCharacters(RoomLayer).OfType<INPC>()
    ...
    public void LoginCharacter()
    {
        ...
        {
            List<IElection> elections = ClanMemberships
    ...
    public void SaveMinutes(MudSharp.Models.Character dbchar)
    {
        ...
        int oldMinutes = dbchar.TotalMinutesPlayed;
    ...
    {
        foreach (IEffect effect in CombinedEffects())
        {
    ...
    private bool MountedCanInteractWith(IGameItem item, MountedItemInteraction interaction, out string message)
    {
        ...
        switch (interaction)
        {
            ...
            case MountedItemInteraction.Manipulate:
                SizeCategory mountSize = mount.CurrentContextualSize(SizeContext.BeingRiddenAsMount);
                IContainer container = current.GetItemType<IContainer>();
    ...
    string IControllable.Prompt
    {
        get
        {
            ...
            if (Account.PromptType.HasFlag(PromptType.Classic))
            {
                ...
                string classic = HealthStrategy.ReportConditionPrompt(this, PromptType.Classic);
                if (Account.PromptType.HasFlag(PromptType.IncludeMagic) &&
                    Capabilities.Any(x => x.ShowMagicResourcesInPrompt))
                {
                    foreach (KeyValuePair<IMagicResource, double> resource in MagicResourceAmounts)
                    {
                        double current = resource.Value / resource.Key.ResourceCap(this);
            ...
            double staminaRatio = CurrentStamina / MaximumStamina;
            ...
            string currentstaminaString = CurrentStamina.ToString("N0", this);
            string maxstaminaString = MaximumStamina.ToString("N0", this);
            ...
            if (Account.PromptType.HasFlag(PromptType.Full))
            {
                ...
                if (Account.PromptType.HasFlag(PromptType.IncludeMagic) &&
                    Capabilities.Any(x => x.ShowMagicResourcesInPrompt))
                {
                    foreach (KeyValuePair<IMagicResource, double> amount in MagicResourceAmounts)
                    {
                        double cap = amount.Key.ResourceCap(this);
                        if (cap == 0)
                        {
                            continue;
                        }
    ...
    public void AddKnowledge(ICharacterKnowledge knowledge)
    {
        ...
        foreach (IScript script in Gameworld.Scripts.Where(x =>
    ...
    {
        ICharacterKnowledge removedKnowledge = _characterKnowledges.First(x => x.Knowledge.Id == knowledge.Id);
    ...
    {
        IEnumerable<IGrowableCharacteristicValue> selectables = Gameworld.CharacteristicValues.OfType<IGrowableCharacteristicValue>().Where(x =>
        ...
        IGrowableCharacteristicValue value)
    {
        ...
        ITag universal = Gameworld.Tags.Get(Gameworld.GetStaticLong("UniversalStyleToolTagId"));
        ...
        {
            ITag different = Gameworld.Tags.Get(Gameworld.GetStaticLong("DifferentGrowthStyleToolTagId"));
            ...
            EffectHandler.AddEffect(new StagedCharacterActionWithTarget(this, target,
                ...
                $"working on $1's {definition.Name.ToLowerInvariant()}.", new Action<IPerceivable>[]
                {
                    ...
                    perc =>
                    {
                        ...
                        {
                            RecentlyStyled effect = target.Body
    ...
    private void LoadOutfits(MudSharp.Models.Character dbchar)
    {
        ...
        XElement root = XElement.Parse(dbchar.Outfits);
        foreach (XElement element in root.Elements("Outfit"))
        {
