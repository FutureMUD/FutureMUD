...
public partial class Character
{
    ...
    protected void RevalidateCombatSettingsAfterInitialisation()
    {
        ...
        ICharacterCombatSettings resolved = ResolveCombatSettingsAfterInitialisation();
    ...
    private void ForceAcquireTargetCheckForOpponents()
    {
        if (Combat != null)
        {
            foreach (IPerceiver combatant in Combat.Combatants.Where(x => x.CombatTarget == this).ToList())
            {
    ...
    public override void AcquireTarget()
    {
        ...
        List<IPerceiver> targetingMe = Combat.Combatants
        ...
        if (targetingMe.Any())
        {
            ...
            {
                IPerceiver newTarget = targetingMe.Where(x => x.ColocatedWith(this)).GetRandomElement();
    ...
    public override bool CheckCombatStatus()
    {
        ...
        foreach (IPerceiver combatant in Combat.Combatants
    ...
    private bool WillAttackTarget(bool ignoreTemporary)
    {
        ...
        ICharacter charTarget = CombatTarget as ICharacter;
    ...
    public override Facing GetFacingFor(ICombatant opponent, bool reset = false)
    {
        ...
        IFixedFacingEffect returnValue = Effects.OfType<IFixedFacingEffect>().FirstOrDefault(x => x.AppliesTo(opponent));
        if (returnValue != null)
        {
            Facing value = returnValue.Facing;
            if (reset)
            {
    ...
    public override bool Engage(IPerceiver target, bool ranged)
    {
        ...
        {
            foreach (IPerceiver person in target.Combat.Combatants)
            {
                person.Send(
    ...
    {
        bool grappling = EffectsOfType<IGrappling>().Any(x => x.Target == CombatTarget) ||
    ...
    {
        foreach (Ally ally in dbchar.AlliesCharacter.Where(x => !AllyIDs.Contains(x.AllyId)).ToList())
        {
            dbchar.AlliesCharacter.Remove(ally);
        }

        foreach (long ally in AllyIDs.Where(x => dbchar.AlliesCharacter.All(y => y.AllyId != x)).ToList())
        {
        ...
        foreach (Ally ally in dbchar.AlliesCharacter)
        {
    ...
    {
        ICorpse corpse = (owner as ICharacter)?.Corpse;
        if (corpse == null)
        {
            Target_OnQuit(owner);
            return;
        ...
        IMortalPerceiver mp = (IMortalPerceiver)owner;
        LoseTarget(mp);
    ...
    private void CheckAllTargets()
    {
        ...
        foreach (IMortalPerceiver target in SeenTargets.ToList())
        {
            LoseTarget(target);
        }
    }
