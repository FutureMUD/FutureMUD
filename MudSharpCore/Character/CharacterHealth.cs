...
public partial class Character
{
    ...
    public virtual IGameItem Die()
    {
        ...
        if (deathBoard != null)
        {
            FutureProg.IFutureProg deathProg = Gameworld.FutureProgs.Get(Gameworld.GetStaticLong("PostToDeathsProg"));
            if (deathProg?.ExecuteBool(this) != false)
            {
                ICalendar calendar = Location.Calendars.First();
        ...
        foreach (ICharacter ch in (Combat?.Combatants.OfType<ICharacter>().Where(x => x.CombatTarget == this) ??
        ...
        IPerceiver combatTarget = CombatTarget;
        CombatTarget = null;
        ...
        if (Controller != null)
        {
            IOutputHandler oldOutputHandler = OutputHandler;
