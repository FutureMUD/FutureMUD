...
public partial class Character
{
    ...
    public override void DoFallDamage(double fallDistance)
    {
        ...
        foreach (LimbType target in targetLimbs)
        {
            ILimb limb = Body.Limbs.Where(x => x.LimbType == target).GetRandomElement();
            if (limb == null)
            {
                continue;
            }

            IBodypart bodypart = Body.BodypartsForLimb(limb).GetWeightedRandom(x => x.RelativeHitChance);
            Damage partDamage = new(damage) { Bodypart = bodypart };
    ...
    {
        IGrappling grappling = CombinedEffectsOfType<IGrappling>().FirstOrDefault();
        if (grappling?.TargetEffect.UnderControl == true && !IsEngagedInMelee)
        {
            ICharacter target = grappling.Target;
            RemoveEffect(grappling, true);
            Dragging drag = new(this, null, target);
    ...
    public bool Move(ICellExit exit, IEmote? emote = null, bool ignoreSafeMovement = false)
    {
        ...
        CanMoveResponse response = CanMove(exit, flags);
        if (!response.Result)
        {
        ...
        IMovement movement = MudSharp.Movement.Movement.CreateMovement(this, exit, emote, ignoreSafeMovement);
        if (movement is null)
        {
            return false;
        }

        foreach (ICharacter mover in movement.CharacterMovers)
        {
    ...
    private CanMoveResponse CanMoveInternal(CanMoveFlags flags, IPositionState movingPositionOverride)
    {
        ...
        bool dragging = CombinedEffectsOfType<Dragging>().Any();
        double staminaMultiplier = 1.0;
        bool ignoreTerrainStamina = false;
        ...
        double staminaCost = StaminaForMovement(movingPosition, CurrentSpeeds[movingPosition], staminaMultiplier,
        ...
        PerceivableRejectionResponse args = new();
        ...
        IPositionState fixedPosition)
    {
        ...
        (bool Success, IMoveSpeed FastestMoveSpeed) CanMovePositionGeneralLogic(IPositionState state)
        {
            ...
            {
                IMoveSpeed speed = Body.Speeds.Where(x => x.Position == state)
            ...
            IMoveSpeed cspeed = Body.Speeds.Where(x => x.Position == state).OrderByDescending(x => x.Multiplier)
                             .FirstOrDefault();
            if (cspeed == null)
            {
                return (false, null);
        ...
        {
            (bool Success, IMoveSpeed FastestMoveSpeed) general = CanMovePositionGeneralLogic(PositionProne.Instance);
            if (!general.Success)
            {
                return (false, null);
            }
    ...
    public double MoveSpeed(ICellExit exit)
    {
        ...
        {
            Difficulty armourUseDifficulty = ArmourUseCheckDifficulty(ArmourUseDifficultyContext.AidedWalking);
            Gameworld.GetCheck(CheckType.ArmourUseCheck).Check(this, armourUseDifficulty);
            ICheck check = Gameworld.GetCheck(CheckType.CrutchWalking);
            CheckOutcome crutchResult = check.Check(this, armourUseDifficulty);
        ...
        double multiplier = (exit?.Exit.TimeMultiplier ?? 1.0) +
                         meritMultiplier +
                         speed.Multiplier +
                         sightMultiplier +
        ...
        double result = baseSpeed * multiplier;
    ...
    public bool Move(string rawInput)
    {
        ...
        string target = ss.PopSafe();

        PlayerEmote emote = new(ss.PopParentheses(), this);
    ...
    public void ExecuteMove(IMovement movement, IMoveSpeed speedOverride = null)
    {
        ...
        switch (transition)
        {
            case CellMovementTransition.TreesToTrees:
                ...
                ICheck check = Gameworld.GetCheck(CheckType.ClimbTreetoTreeCheck);
                IWeatherEvent weather = Location.CurrentWeather(this);
                Dictionary<Difficulty, CheckOutcome> result = check.CheckAgainstAllDifficulties(this, Difficulty.VeryEasy, null,
                    externalBonus:
        ...
        HideInvis hideEffect = EffectsOfType<HideInvis>().FirstOrDefault();
    ...
    {
        Difficulty baseDifficulty = Difficulty.Automatic;
        switch (Encumbrance)
        {
    ...
    private (bool Truth, string Error) CheckGeneralMovementRestrictions(string actionVerb)
    {
        ...
        List<IEffect> blockers = CombinedEffectsOfType<IEffect>().Where(x => x.IsBlockingEffect("move")).ToList();
        if (blockers.Any())
        {
            return (false,
        ...
        List<string> itemBlockers = Body.AllItems.Where(x => x.PreventsMovement()).Select(x => x.WhyPreventsMovement(this))
                               .ToList();
        if (itemBlockers.Any())
        {
    ...
    private (bool Truth, string Error) CanFlyInternal(bool allowAlreadyFlying, IFlightEffect ignoredEffect = null)
    {
        ...
        List<IBodypart> wings = Body.Bodyparts.Where(x => x is WingProto).ToList();
        int workingWings = wings.Where(x => Body.CanUseBodypart(x) == CanUseBodypartResult.CanUse)
    ...
    public (bool Truth, string Error) CanLand()
    {
        ...
        (bool truth, string error) = CheckGeneralMovementRestrictions("land");
        if (!truth)
        {
            return (false, error);
        }
    ...
    public double FlyStaminaMultiplier()
    {
        ...
        ICheck check = Gameworld.GetCheck(CheckType.FlyCheck);
        ...
        CheckOutcome outcome = check.Check(this, difficulty);
    ...
    public void DoFlyHeartbeat()
    {
        ...
        double staminaCost = FlyStaminaCostPerTick();
    ...
    {
        (bool truth, string error) = CheckGeneralMovementRestrictions("ascend");
        if (!truth)
        {
            return (false, error);
        }
        ...
        Dragging dragging = CombinedEffectsOfType<Dragging>().FirstOrDefault();
        if (dragging != null &&
    ...
    {
        (bool truth, string error) = CheckGeneralMovementRestrictions("dive");
        if (!truth)
        {
            return (false, error);
        }
    ...
    void IFly.Ascend(IEmote actionEmote)
    {
        ...
        ConvertGrapplesToDrags();
        (bool can, string error) = ((IFly)this).CanAscend();
        if (!can)
        {
            OutputHandler.Send(error);
            return;
        ...
        RoomLayer desired = higherLayers.LowestLayer();
        Emote emote = new(Gameworld.GetStaticString("AscendFlyEmote"), this, this);
        bool breaching = RoomLayer.In(RoomLayer.InTrees, RoomLayer.HighInTrees) &&
    ...
    void IFly.Dive(IEmote actionEmote)
    {
        ...
        ConvertGrapplesToDrags();
        (bool can, string error) = ((IFly)this).CanDive();
        if (!can)
        {
            OutputHandler.Send(error);
            return;
        ...
        RoomLayer desired = lowerLayers.HighestLayer();
        Emote emote = new(Gameworld.GetStaticString("DiveFlyEmote"), this, this);
        bool leavingTrees = RoomLayer.In(RoomLayer.InTrees, RoomLayer.HighInTrees) &&
                           desired.In(RoomLayer.GroundLevel, RoomLayer.OnRooftops);
        bool enteringTrees = RoomLayer.In(RoomLayer.InAir, RoomLayer.HighInAir) &&
                            desired.In(RoomLayer.InTrees, RoomLayer.HighInTrees);
        if (leavingTrees)
        {
    ...
    {
        double staminaCost = Gameworld.GetStaticDouble("SwimSurfaceStamina");
        ICheck check = Gameworld.GetCheck(CheckType.SwimmingCheck);
        ITerrain terrain = Location.Terrain(this);
        double inventoryBuoyancy = Body.AllItems.Sum(x => x.Buoyancy(terrain.WaterFluid.Density));
        Difficulty difficulty =
    ...
    public void DoSwimHeartbeat()
    {
        ...
        IPerceiver dragTarget = EffectsOfType<Dragging>().FirstOrDefault()?.Target as IPerceiver;
        if (PositionState != PositionSwimming.Instance)
        {
            if (terrain.TerrainLayers.Any(x => x.IsLowerThan(RoomLayer)))
            {
                string emoteInsert = RoomLayer == RoomLayer.GroundLevel ? "below the surface" : "deeper underwater";
                OutputHandler.Handle(
        ...
        double staminaCost = Gameworld.GetStaticDouble("SwimStayAfloatStamina");
        ICheck check = Gameworld.GetCheck(CheckType.SwimStayAfloatCheck);
        double inventoryBuoyancy = Body.AllItems.Sum(x => x.Buoyancy(terrain.WaterFluid.Density));
        Difficulty difficulty =
            Difficulty.ExtremelyEasy.StageDown((int)(inventoryBuoyancy /
                                                     Gameworld.GetStaticDouble("FloatBuoyancyPerDifficulty")));
    ...
    (bool Truth, string Error) ISwim.CanAscend()
    {
        ...
        List<IEffect> blockers = CombinedEffectsOfType<IEffect>()
    ...
    void ISwim.Ascend(IEmote actionEmote)
    {
        ...
        ConvertGrapplesToDrags();
        (bool can, string error) = ((ISwim)this).CanAscend();
        if (!can)
        {
            OutputHandler.Send(error);
            return;
        ...
        List<RoomLayer> higherLayers = terrain.TerrainLayers.Where(x => x.IsHigherThan(RoomLayer)).ToList();
        RoomLayer desired = higherLayers.LowestLayer();
        Emote emote = new(Gameworld.GetStaticString("AscendEmote"), this, this);
        bool breaching = !Location.IsUnderwaterLayer(desired);
        if (breaching)
        {
    ...
    void ISwim.Dive(IEmote actionEmote)
    {
        ...
        ConvertGrapplesToDrags();
        (bool truth, string error) = ((ISwim)this).CanDive();
        if (!truth)
        {
            OutputHandler.Send(error);
            return;
        ...
        RoomLayer desired = terrain.TerrainLayers.Where(x => x.IsLowerThan(RoomLayer)).HighestLayer();

        bool alreadyUnderwater = Location.IsUnderwaterLayer(RoomLayer);
        Emote emote = alreadyUnderwater
    ...
    public bool DoClimbMovementCheck(Difficulty difficulty)
    {
        ...
        double staminaCost = Gameworld.GetStaticDouble("ClimbingTickStaminaUsage") * (EffectsOfType<Dragging>().Any()
            ? 1.0 + ((IHaveWeight)EffectsOfType<Dragging>().First().Target).Weight / Weight
            : 1.0);
        if (!CanSpendStamina(staminaCost))
        {
        ...
        SpendStamina(staminaCost);
        ICheck check = Gameworld.GetCheck(CheckType.ClimbCheck);
        IWeatherEvent weather = Location.CurrentWeather(this);
        Dictionary<Difficulty, CheckOutcome> allResults = check.CheckAgainstAllDifficulties(this, difficulty, null,
            externalBonus: (weather?.Precipitation.PrecipitationClimbingBonus() ?? 0.0) +
                           (weather?.Wind.WindClimbingBonus() ?? 0.0));
        CheckOutcome result = allResults[difficulty];
        ...
        {
            Difficulty compare = difficulty.StageDown(3);
            CheckOutcome compareOutcome = allResults[compare];
    ...
    {
        (bool truth, string error) = CheckGeneralMovementRestrictions("climb");
        if (!truth)
        {
            return (false, error);
        }
    ...
    public void ClimbUp(IEmote actionEmote = null)
    {
        ...
        bool result = DoClimbMovementCheck(climbExit != null ? climbExit.ClimbDifficulty : Difficulty.ExtremelyEasy);
        if (!AffectedBy<Immwalk>())
        {
        ...
        RoomLayer desiredLayer = higherLayers.LowestLayer();
        ...
        {
            IPositionState highestUpright = MostUprightMobilePosition();
    ...
    public void ClimbDown(IEmote actionEmote = null)
    {
        ...
        bool result = DoClimbMovementCheck(climbExit != null ? climbExit.ClimbDifficulty : Difficulty.Trivial);
        if (!AffectedBy<Immwalk>())
        {
            AddEffect(new BlockLayerChange(this), ClimbDelay);
        }
        ...
        RoomLayer desiredLayer = lowerLayers.HighestLayer();
