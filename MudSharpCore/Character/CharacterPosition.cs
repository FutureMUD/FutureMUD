...
public partial class Character
{
    ...
    public override IEnumerable<(IPerceivable Thing, Proximity Proximity)> LocalThingsAndProximities()
    {
        ...
        foreach (GuardingExit effect in CombinedEffectsOfType<GuardingExit>().Where(x => x.Exit.Exit.Door != null).ToList())
        {
            yield return (effect.Exit.Exit.Door.Parent, Proximity.Proximate);
        }
    }
    ...
    public void Awaken(IEmote emote = null)
    {
        ...
        {
            IDreamingEffect dream = EffectsOfType<IDreamingEffect>().First();
        ...
        {
            WellRested rested = EffectsOfType<WellRested>().First();
            rested.RenewRest();
        }
    }
    ...
    public void Sleep(IEmote emote = null)
    {
        ...
        int delay = Dice.Roll($"1d10+{50 + result.FailureDegrees() * 10}"); // TODO - soft coded somewhere?
