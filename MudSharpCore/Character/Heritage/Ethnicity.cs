...
public class Ethnicity : SaveableItem, IEthnicity
{
    public Ethnicity(MudSharp.Models.Ethnicity ethnicity, IFuturemud gameworld)
    {
        ...
        foreach (EthnicitiesCharacteristics item in ethnicity.EthnicitiesCharacteristics)
        {
            ICharacteristicDefinition definition = gameworld.Characteristics.Get(item.CharacteristicDefinitionId);
            ICharacteristicProfile profile = gameworld.CharacteristicProfiles.Get(item.CharacteristicProfileId);
        ...
        foreach (EthnicitiesNameCultures nc in ethnicity.EthnicitiesNameCultures)
        {
        ...
        {
            foreach (ICharacteristicDefinition item in ParentRace.Characteristics(Gender.Indeterminate))
            {
                ...
                ICharacteristicProfile profile = GetDefaultCharacteristicProfile(gameworld.CharacteristicProfiles, item);
                if (profile is null)
                {
                    continue;
                }
        ...
        foreach (EthnicitiesChargenResources item in ethnicity.EthnicitiesChargenResources)
        {
        ...
        foreach (ChargenAdvicesEthnicities item in ethnicity.ChargenAdvicesEthnicities)
        {
    ...
    {
        Gameworld = gameworld;
        IReadOnlyCollection<(ICharacteristicDefinition Characteristic, ICharacteristicProfile Profile)> characteristicSelections = GetCharacteristicProfileSelections(race, gameworld.CharacteristicProfiles);
        List<ICharacteristicDefinition> missingCharacteristics = characteristicSelections
        ...
        using (new FMDB())
        {
            ...
            foreach ((ICharacteristicDefinition Characteristic, ICharacteristicProfile Profile) selection in characteristicSelections)
            {
    ...
    public Ethnicity(IEthnicity rhs, string newName)
    {
        ...
        foreach (KeyValuePair<ICharacteristicDefinition, ICharacteristicProfile> characteristic in rhs.CharacteristicChoices)
        {
            CharacteristicChoices[characteristic.Key] = characteristic.Value;
        }

        foreach (ChargenResourceCost cost in rhs.Costs)
        {
            _costs.Add(new ChargenResourceCost
            {
        ...
        foreach (IChargenAdvice advice in rhs.ChargenAdvices)
        {
        ...
        using (new FMDB())
        {
            ...
            foreach (KeyValuePair<ICharacteristicDefinition, ICharacteristicProfile> characteristic in CharacteristicChoices)
            {
                dbitem.EthnicitiesCharacteristics.Add(new Models.EthnicitiesCharacteristics
                {
                    Ethnicity = dbitem,
            ...
            foreach (ChargenResourceCost cost in Costs)
            {
            ...
            foreach (IChargenAdvice advice in ChargenAdvices)
            {
    ...
    {
        List<ICharacteristicProfile> profiles = characteristicProfiles.ToList();
    ...
    {
        List<ICharacteristicProfile> profiles = characteristicProfiles
    ...
    public override void Save()
    {
        ...
        foreach (IChargenAdvice advice in _chargenAdvices)
        {
    ...
    private bool BuildingCommandNameCulture(ICharacter actor, StringStack command)
    {
        ...
        INameCulture culture = Gameworld.NameCultures.GetByIdOrName(command.PopSpeech());
    ...
    private bool BuildingCommandAdvice(ICharacter actor, StringStack command)
    {
        ...
        IChargenAdvice which = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandCharacteristic(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicDefinition choice = CharacteristicChoices.Keys.FirstOrDefault(x => x.Name.EqualTo(name)) ??
        ...
        string profileName = command.SafeRemainingArgument;
        ICharacteristicProfile profile = Gameworld.CharacteristicProfiles.Where(x => x.TargetDefinition == choice)
    ...
    private bool BuildingCommandGroup(ICharacter actor, StringStack command)
    {
        if (command.IsFinished)
        {
            IEnumerable<string> previous = Gameworld.Ethnicities.Where(x => x.ParentRace == ParentRace)
        ...
        string group = command.SafeRemainingArgument.TitleCase();
        EthnicGroup = group;
        Changed = true;
        actor.OutputHandler.Send(
    ...
    private bool BuildingCommandSubgroup(ICharacter actor, StringStack command)
    {
        ...
        if (command.IsFinished)
        {
            IEnumerable<string> previous = Gameworld.Ethnicities
    ...
    private bool BuildingCommandBloodModel(ICharacter actor, StringStack command)
    {
        ...
        IPopulationBloodModel model = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandTempCeiling(ICharacter actor, StringStack command)
    {
        ...
        double result = Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument,
            Framework.Units.UnitType.TemperatureDelta, out bool success);
        if (!success)
        {
            actor.OutputHandler.Send(
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        foreach (KeyValuePair<ICharacteristicDefinition, ICharacteristicProfile> choice in CharacteristicChoices.OrderBy(x => x.Key.Name))
        {
