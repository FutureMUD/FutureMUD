...
public partial class Race : SaveableItem, IRace
{
    ...
    public Race(MudSharp.Models.Race race, IFuturemud gameworld)
    {
        ...
        foreach (ChargenAdvicesRaces item in race.ChargenAdvicesRaces)
        {
        ...
        foreach (RacesAdditionalBodyparts bodypart in race.RacesAdditionalBodyparts)
        {
            IBodypart bp = BaseBody?.AllBodypartsBonesAndOrgans.FirstOrDefault(x => x.Id == bodypart.BodypartId) ??
        ...
        foreach (RacesAdditionalCharacteristics item in race.RacesAdditionalCharacteristics)
        {
            ICharacteristicDefinition characteristic = gameworld.Characteristics.Get(item.CharacteristicDefinitionId);
            switch (item.Usage)
            {
                case "base":
        ...
        foreach (RacesAttributes item in race.RacesAttributes)
        {
        ...
        foreach (RacesWeaponAttacks item in race.RacesWeaponAttacks)
        {
        ...
        foreach (RacesCombatActions item in race.RacesCombatActions)
        {
        ...
        foreach (RacesBreathableLiquids item in race.RacesBreathableLiquids)
        {
        ...
        foreach (RacesBreathableGases item in race.RacesBreathableGases)
        {
        ...
        foreach (RacesRemoveBreathableLiquids item in race.RacesRemoveBreathableLiquids)
        {
            _removeBreathableFluids.Add(Gameworld.Liquids.Get(item.LiquidId));
        }

        foreach (RacesRemoveBreathableGases item in race.RacesRemoveBreathableGases)
        {
        ...
        foreach (RaceEdibleForagableYields yield in race.RaceEdibleForagableYields)
        {
        ...
        foreach (RacesEdibleMaterials material in race.RacesEdibleMaterials)
        {
    ...
    public Race(Race rhs, string newName)
    {
        ...
        using (new FMDB())
        {
            ...
            foreach (IBodypart part in _maleOnlyAdditions)
            {
                dbitem.RacesAdditionalBodyparts.Add(new RacesAdditionalBodyparts
                {
                    Race = dbitem,
            ...
            foreach (IBodypart part in _femaleOnlyAdditions)
            {
                dbitem.RacesAdditionalBodyparts.Add(new RacesAdditionalBodyparts
                {
                    Race = dbitem,
            ...
            foreach (IBodypart part in _bodypartRemovals)
            {
                dbitem.RacesAdditionalBodyparts.Add(new RacesAdditionalBodyparts
                {
                    Race = dbitem,
            ...
            foreach (ICharacteristicDefinition item in _baseCharacteristics)
            {
            ...
            foreach (ICharacteristicDefinition item in _maleOnlyCharacteristics)
            {
                dbitem.RacesAdditionalCharacteristics.Add(new RacesAdditionalCharacteristics
                {
                    Race = dbitem,
            ...
            foreach (ICharacteristicDefinition item in _femaleOnlyCharacteristics)
            {
                dbitem.RacesAdditionalCharacteristics.Add(new RacesAdditionalCharacteristics
                {
                    Race = dbitem,
            ...
            foreach (EdibleMaterial item in _edibleMaterials)
            {
            ...
            foreach (ChargenResourceCost item in _costs)
            {
            ...
            foreach (INaturalAttack attack in _naturalWeaponAttacks)
            {
                RacesWeaponAttacks dbattack = new();
            ...
            foreach (IAuxiliaryCombatAction action in _auxiliaryCombatActions)
            {
            ...
            foreach (EdibleForagableYield item in _edibleForagableYields)
            {
    ...
    {
        List<ICharacteristicDefinition> returnVar = new(_baseCharacteristics);
        if (ParentRace != null)
        {
    ...
    public IEnumerable<(Gender Gender, ICharacteristicDefinition Definition)> GenderedCharacteristics
    {
        get
        {
            List<(Gender, ICharacteristicDefinition)> returnVar = new();
            if (ParentRace is not null)
            {
    ...
    {
        List<IRace> children = Gameworld.Races.Where(x => x.SameRace(this)).Except(this).ToList();
    ...
    public override void Save()
    {
        ...
        foreach (IAuxiliaryCombatAction action in _auxiliaryCombatActions)
        {
            RacesCombatActions dbaction = new()
            {
                CombatActionId = action.Id,
                RaceId = Id
            };
    ...
    {
        EdibleForagableYield efy = EdibleForagableYields.First(x => x.YieldType.EqualTo(yieldType));
        double totalYield = efy.YieldPerBite * bites;
    ...
    {
        EdibleMaterial em = EdibleMaterials.First(x => x.Material == material);
        double totalKGs = BiteWeight * bites;
    ...
    {
        int age = template.SelectedBirthday.Calendar.CurrentDate.YearsDifference(template.SelectedBirthday);
        return AgeCategory(age);
    }
