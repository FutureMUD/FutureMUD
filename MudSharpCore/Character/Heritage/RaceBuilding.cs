...
public partial class Race
{
    ...
    private bool BuildingCommandBodypartHealth(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParsePercentage(actor.Account.Culture, out double value) || value <= 0.0)
        {
    ...
    private bool BuildingCommandCost(ICharacter actor, StringStack command, bool isCost)
    {
        ...
        IChargenResource which = long.TryParse(command.PopSpeech(), out long value)
        ...
        {
            int amount = _costs.RemoveAll(x => x.Resource == which);
            if (amount == 0)
            {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int cost))
        {
    ...
    private bool BuildingCommandAdvice(ICharacter actor, StringStack command)
    {
        ...
        CharacterCreation.IChargenAdvice which = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandEdibleYield(ICharacter actor, StringStack command)
    {
        ...
        string yield = command.PopSpeech().ToLowerInvariant();
        switch (yield)
        {
            case "add":
    ...
    private bool BuildingCommandYieldAdd(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double hunger))
        {
        ...
        if (!double.TryParse(command.PopSpeech(), out double thirst))
        {
            actor.OutputHandler.Send("That is not a valid number of hours.");
            return false;
        }
    ...
    private bool BuildingCommandEdibleYieldWater(ICharacter actor, StringStack command, string yield)
    {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(command.SafeRemainingArgument, UnitType.FluidVolume, actor, out double value))
        {
    ...
    private bool BuildingCommandEdibleYieldAlcohol(ICharacter actor, StringStack command, string yield)
    {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(command.SafeRemainingArgument, UnitType.Mass, actor, out double value))
        {
            actor.OutputHandler.Send("That is not a valid quantity.");
            return false;
        }
    ...
    {
        string property = command.Last.ToLowerInvariant().CollapseString();
        if (command.IsFinished)
        {
            switch (property)
            {
        ...
        EdibleMaterial em = _edibleMaterials.First(x => x.Material == material);
        Changed = true;
        switch (property)
        {
            case "water":
    ...
    {
        List<IRace> children = Gameworld.Races.Where(x => x.ParentRace == this).ToList();
        if (!children.Any())
        {
        ...
        IAttributeDefinition attribute = _attributes.GetByIdOrName(command.SafeRemainingArgument);
        if (attribute is null)
        {
    ...
    private bool BuildingCommandCharacteristicDemote(ICharacter actor, StringStack command)
    {
        ...
        Form.Characteristics.ICharacteristicDefinition characteristic = _baseCharacteristics.Concat(_maleAndFemaleCharacteristics)
    ...
    private bool BuildingCommandCharacteristicAddOrSet(ICharacter actor, StringStack command, Gender gender)
    {
        ...
        Form.Characteristics.ICharacteristicDefinition characteristic = Gameworld.Characteristics
                                      .GetByIdOrName(command.SafeRemainingArgument);
        if (characteristic is null)
        {
        ...
        foreach (IRace race in Gameworld.Races.Where(x => x.SameRace(this)))
        {
    ...
    private bool BuildingCommandArmourQuality(ICharacter actor, StringStack command)
    {
        ...
        if (!Enum.TryParse<ItemQuality>(command.SafeRemainingArgument, out ItemQuality quality))
        {
    ...
    private bool BuildingCommandArmourType(ICharacter actor, StringStack command)
    {
        ...
        IArmourType armour = Gameworld.ArmourTypes.GetByIdOrName(command.SafeRemainingArgument);
        if (armour is null)
        {
    ...
    private bool BuildingCommandBloodModel(ICharacter actor, StringStack command)
    {
        ...
        Health.IBloodModel model = Gameworld.BloodModels.GetByIdOrName(command.SafeRemainingArgument);
        if (model is null)
        {
    ...
    private bool BuildingCommandBreathingModel(ICharacter actor, StringStack command)
    {
        ...
        try
        {
            IBreathingStrategy model = GetBreathingStrategy(command.SafeRemainingArgument.ToLowerInvariant().CollapseString());
    ...
    private bool BuildingCommandButcheryProfile(ICharacter actor, StringStack command)
    {
        ...
        IRaceButcheryProfile profile = Gameworld.RaceButcheryProfiles.GetByIdOrName(command.SafeRemainingArgument);
        if (profile is null)
        {
    ...
    {
        List<Gender> allowedGenders = new()
        {
            Gender.Male,
            Gender.Female,
            Gender.NonBinary,
    ...
    {
        if (command.IsFinished || !command.SafeRemainingArgument.TryParsePercentage(out double value) || value < 0.0)
        {
    ...
    private bool BuildingCommandHealthModel(ICharacter actor, StringStack command)
    {
        ...
        Health.IHealthStrategy model = Gameworld.HealthStrategies.GetByIdOrName(command.SafeRemainingArgument);
        if (model is null)
        {
    ...
    private bool BuildingCommandCorpseModel(ICharacter actor, StringStack command)
    {
        ...
        Health.ICorpseModel model = Gameworld.CorpseModels.GetByIdOrName(command.SafeRemainingArgument);
        if (model is null)
        {
    ...
    private bool BuildingCommandBody(ICharacter actor, StringStack command)
    {
        ...
        IBodyPrototype body = Gameworld.BodyPrototypes.GetByIdOrName(command.SafeRemainingArgument);
        if (body is null)
        {
    ...
    private bool BuildingCommandParent(ICharacter actor, StringStack command)
    {
        ...
        if (command.SafeRemainingArgument.EqualTo("none"))
        {
            ...
            actor.AddEffect(new Accept(actor, new GenericProposal
            {
                ...
                AcceptAction = text =>
                {
                    ...
                    foreach (IAttributeDefinition attribute in ParentRace.Attributes)
                    {
                        if (!_attributes.Contains(attribute))
                        {
                            _attributes.Add(attribute);
                    ...
                    foreach (IBodypart item in ParentRace.BodypartAdditions)
                    {
                    ...
                    foreach (IBodypart item in ParentRace.MaleOnlyAdditions)
                    {
                    ...
                    foreach (IBodypart item in ParentRace.FemaleOnlyAdditions)
                    {
                    ...
                    foreach (IBodypart item in ParentRace.BodypartRemovals)
                    {
        ...
        IRace race = Gameworld.Races.GetByIdOrName(command.SafeRemainingArgument);
        if (race is null)
        {
    ...
    {
        foreach (ICharacter character in Gameworld.Characters)
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        foreach (IFluid item in _breathableFluids)
        {
            sb.AppendLine($"\t{item.Name.Colour(item.DisplayColour)} @ {CanBreatheFluid(item).RateMultiplier.ToStringP2Colour(actor)}");
        }

        foreach (IFluid item in ParentRace?.BreathableFluids ?? Enumerable.Empty<IFluid>())
        {
        ...
        foreach (IFluid item in _removeBreathableFluids)
        {
        ...
        sb.AppendLine();
        List<Form.Characteristics.ICharacteristicDefinition> allCharacteristics = Characteristics(Gender.Indeterminate).ToList();
        List<Form.Characteristics.ICharacteristicDefinition> maleCharacteristics = Characteristics(Gender.Male).ToList();
        List<Form.Characteristics.ICharacteristicDefinition> femaleCharacteristics = Characteristics(Gender.Female).ToList();
        foreach (Form.Characteristics.ICharacteristicDefinition item in allCharacteristics)
        {
        ...
        if (_costs.Any())
        {
            foreach (ChargenResourceCost cost in _costs)
            {
                sb.AppendLine(
        ...
        {
            foreach (CharacterCreation.IChargenAdvice advice in _chargenAdvices)
            {
                sb.AppendLine();
                sb.AppendLine(
