...
public class NameCulture : SaveableItem, INameCulture
{
    ...
    public NameCulture(MudSharp.Models.NameCulture culture, IFuturemud gameworld)
    {
        ...
        _id = culture.Id;
        _name = culture.Name;
        foreach (Models.RandomNameProfile item in culture.RandomNameProfiles)
        {
            RandomNameProfile profile = new(item, this);
    ...
    protected void LoadFromXml(XElement root)
    {
        ...
        if (element != null)
        {
            foreach (XElement sub in element.Elements("Pattern"))
            {
                _styles.Add(
        ...
        if (element != null)
        {
            foreach (XElement sub in element.Elements("Element"))
            {
    ...
    public IPersonalName GetPersonalName(string pattern, bool nonSaving = false)
    {
        try
        {
            ...
            Dictionary<NameUsage, List<string>> elements = new();
            foreach (NameUsage item in Enum.GetValues(typeof(NameUsage)))
            {
                string name = Enum.GetName(typeof(NameUsage), item).ToLowerInvariant();
    ...
    {
        Models.NameCulture dbitem = FMDB.Context.NameCultures.Find(Id);
        dbitem.Name = _name;
...
For example, #3?Nickname[ a.k.a ""$Nickname""][]#0";
    ...
    private bool BuildingCommandRegex(ICharacter actor, StringStack command)
    {
        ...
        Regex regex = new(command.SafeRemainingArgument, RegexOptions.IgnoreCase);
        try
        {
            regex.Match("");
        }
        ...
        foreach (NameCultureElement element in NameCultureElements)
        {
    ...
    private bool BuildingCommandPattern(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<NameStyle>(out NameStyle style))
        {
            actor.OutputHandler.Send(
        ...
        Regex regex = new("\\$(?<enum>[a-zA-Z]+)");
        string pattern = command.SafeRemainingArgument;
        List<NameUsage> elements = new();
        int index = 0;
        pattern = regex.Replace(pattern, m =>
        {
            if (!m.Groups["enum"].Value.TryParseEnum<NameUsage>(out NameUsage usage))
            {
    ...
    private bool BuildingCommandElement(ICharacter actor, StringStack command)
    {
        ...
        switch (cmd)
        {
            case "add":
                ...
                if (!command.SafeRemainingArgument.TryParseEnum<NameUsage>(out NameUsage usage))
                {
                    actor.OutputHandler.Send(
            ...
            case "del":
                ...
                if (!command.SafeRemainingArgument.TryParseEnum<NameUsage>(out NameUsage remove))
                {
                    actor.OutputHandler.Send(
        ...
        if (!cmd.TryParseEnum<NameUsage>(out NameUsage which))
        {
            actor.OutputHandler.Send(
        ...
        NameCultureElement element = _nameCultureElements.FirstOrDefault(x => x.Usage == which);
        if (element == null)
        {
            actor.OutputHandler.Send(
        ...
        switch (command.PopSpeech().ToLowerInvariant())
        {
            ...
            case "min":
                ...
                if (!int.TryParse(command.SafeRemainingArgument, out int min) || min < 0)
                {
            ...
            case "max":
                ...
                if (!int.TryParse(command.SafeRemainingArgument, out int max) || max < 1)
                {
    ...
    {
        NameCultureElement element = (NameCultureElement)args[0];
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach (KeyValuePair<NameStyle, Tuple<string, List<NameUsage>>> pattern in _styles.OrderBy(x => x.Key))
        {
            sb.AppendLine(
