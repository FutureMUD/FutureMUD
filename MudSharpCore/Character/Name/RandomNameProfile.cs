...
public class RandomNameProfile : SaveableItem, IEditableItem, IRandomNameProfile
{
    ...
    public RandomNameProfile(MudSharp.Models.RandomNameProfile profile, INameCulture culture)
    {
        ...
        foreach (Models.RandomNameProfilesDiceExpressions item in profile.RandomNameProfilesDiceExpressions)
        {
            _nameUsageDiceExpressionsDictionary.Add((NameUsage)item.NameUsage, item.DiceExpression);
        }

        foreach (Models.RandomNameProfilesElements item in profile.RandomNameProfilesElements)
        {
    ...
    public RandomNameProfile(INameCulture culture, Gender gender, string name)
    {
        ...
        foreach (NameCultureElement usage in culture.NameCultureElements)
        {
        ...
        using (new FMDB())
        {
            Models.RandomNameProfile dbitem = new()
            {
                Name = name,
                Gender = (short)gender,
            ...
            foreach (KeyValuePair<NameUsage, string> expression in _nameUsageDiceExpressionsDictionary)
            {
    ...
    public RandomNameProfile(RandomNameProfile rhs, string newName)
    {
        ...
        using (new FMDB())
        {
            ...
            foreach (KeyValuePair<NameUsage, string> expression in rhs._nameUsageDiceExpressionsDictionary)
            {
                dbitem.RandomNameProfilesDiceExpressions.Add(new Models.RandomNameProfilesDiceExpressions
                { DiceExpression = expression.Value, NameUsage = (int)expression.Key, RandomNameProfile = dbitem });
            }

            foreach (KeyValuePair<NameUsage, List<(string Value, int Weight)>> usage in rhs._randomNamesDictionary)
            {
                foreach ((string Value, int Weight) element in usage.Value)
                {
            ...
            foreach (Models.RandomNameProfilesDiceExpressions item in dbitem.RandomNameProfilesDiceExpressions)
            {
                _nameUsageDiceExpressionsDictionary.Add((NameUsage)item.NameUsage, item.DiceExpression);
            }

            foreach (Models.RandomNameProfilesElements item in dbitem.RandomNameProfilesElements)
            {
                _randomNamesDictionary.Add((NameUsage)item.NameUsage, (item.Name, item.Weighting));
            }
        }
    ...
    {
        List<string> allResults = new();
        Dictionary<NameUsage, List<string>> resultsDictionary = new();
    ...
    {
        Models.RandomNameProfile dbitem = FMDB.Context.RandomNameProfiles.Find(Id);
        dbitem.Name = Name;
        ...
        foreach (KeyValuePair<NameUsage, List<(string Value, int Weight)>> usage in _randomNamesDictionary)
        {
            foreach ((string Value, int Weight) element in usage.Value)
            {
                dbitem.RandomNameProfilesElements.Add(new Models.RandomNameProfilesElements
    ...
    public bool BuildingCommand(ICharacter actor, StringStack command)
    {
        ...
        string name = command.Last;
        NameCultureElement element = Culture.NameCultureElements.FirstOrDefault(x => x.Name.EqualTo(name)) ??
                      (name.TryParseEnum<NameUsage>(out NameUsage usage)
    ...
    private bool BuildingCommandElement(ICharacter actor, StringStack command, NameCultureElement element)
    {
        ...
        switch (command.PopSpeech().ToLowerInvariant())
        {
            ...
            case "del":
                ...
                int count = _randomNamesDictionary[element.Usage].RemoveAll(x => x.Value.EqualTo(name));
                if (count == 0)
                {
                    actor.OutputHandler.Send(
    ...
    private bool BuildingCommandGender(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<Gender>(out Gender gender))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandDice(ICharacter actor, StringStack command)
    {
        ...
        NameCultureElement element = Culture.NameCultureElements.FirstOrDefault(x => x.Name.EqualTo(name)) ??
                      (name.TryParseEnum<NameUsage>(out NameUsage theEnum)
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach (KeyValuePair<NameUsage, string> chance in _nameUsageDiceExpressionsDictionary)
        {
        ...
        foreach (KeyValuePair<NameUsage, List<(string Value, int Weight)>> usage in _randomNamesDictionary)
        {
            ...
            sb.AppendLine();
            double totalWeight = (double)usage.Value.Sum(x => x.Weight);
            string[] strings = usage.Value.OrderByDescending(x => x.Weight).Select(x =>
            ...
            int longest = strings.Max(x => x.Length);
