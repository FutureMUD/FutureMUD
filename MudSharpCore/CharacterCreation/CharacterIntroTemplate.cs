...
public class CharacterIntroTemplate : SaveableItem, ICharacterIntroTemplate
{
    ...
    private CharacterIntroTemplate(CharacterIntroTemplate rhs, string newName)
    {
        ...
        using (new FMDB())
        {
            Models.CharacterIntroTemplate dbitem = new()
            {
                Name = Name,
    ...
    public CharacterIntroTemplate(MudSharp.Models.CharacterIntroTemplate template, IFuturemud gameworld)
    {
        ...
        foreach (XElement item in root.Elements("Echo"))
        {
    ...
    {
        Models.CharacterIntroTemplate dbitem = FMDB.Context.CharacterIntroTemplates.Find(Id);
        dbitem.Name = Name;
    ...
    private bool BuildingCommandEchoText(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int pos1))
        {
    ...
    private bool BuildingCommandEchoSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int pos2))
        {
            actor.OutputHandler.Send($"The text {command.Last.ColourCommand()} is not a valid number.");
            return false;
        }
    ...
    private bool BuildingCommandProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(Gameworld, actor, command.SafeRemainingArgument, ProgVariableTypes.Boolean, [ProgVariableTypes.Toon]).LookupProg();
        if (prog is null)
        {
            return false;
        }
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach ((string Echo, TimeSpan Delay) echo in Echoes.Zip(Delays, (e, d) => (Echo: e, Delay: d)))
        {
            sb.AppendLine();
