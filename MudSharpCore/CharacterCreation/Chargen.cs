...
public partial class Chargen : FrameworkItem, IChargen
{
        ...
        Action<string> responseCallback)
    {
        ...
        using (new FMDB())
        {
            Account dbaccount = FMDB.Context.Accounts.FirstOrDefault(x => x.Id == reviewerAccountId);
            if (dbaccount == null)
            {
                responseCallback(
            ...
            Models.Chargen dbitem = FMDB.Context.Chargens.FirstOrDefault(x => x.Id == requestedChargen);
            if (dbitem == null)
            {
                responseCallback(
        ...
        PermissionLevel reviewerAuthority = reviewerAccount.Authority?.Level ?? PermissionLevel.Guest;
        ...
        {
            IChargenRole blockingRole = chargen.SelectedRoles.FirstOrDefault(x =>
    ...
    public void RecalculateCurrentCosts()
    {
        ...
        {
            foreach ((IChargenResource resource, int cost) in screen.ChargenCosts(this))
            {
    ...
    public string DisplayForReviewForDiscord(IAccount character, PermissionLevel permission)
    {
        ...
        int age = SelectedBirthday.Calendar.CurrentDate.YearsDifference(SelectedBirthday);
        sb.AppendLine(
        ...
        foreach (Tuple<string, string> note in SelectedNotes)
        {
            sb.AppendLine();
    ...
    public long ApproveApplicationExternal()
    {
        ...
        using (new FMDB())
        {
            ...
            Character.Character character = new(Gameworld, this);
            ...
            foreach (Tuple<string, string> item in SelectedNotes)
            {
            ...
            Account dbaccount = FMDB.Context.Accounts.Find(Account.Id);
            if (dbaccount == null)
            {
                throw new ApplicationException("Account was not found in the database in Chargen ApproveCharacter.");
            }

            foreach (KeyValuePair<IChargenResource, int> cost in ApplicationCosts)
            {
        ...
        IOutputHandler handler)
    {
        ...
        using (new FMDB())
        {
            ...
            Character.Character newChar = new(Gameworld, this);
            Gameworld.SaveManager.Flush();
            //var id = Character.Character.CreateCharacterFromTemplate(this, Account);
            Models.Character dbchar = FMDB.Context.Characters.Find(newChar.Id);
            long id = newChar.Id;
            if (approver != null)
            {
            ...
            foreach (KeyValuePair<IChargenResource, int> cost in ApplicationCosts.Where(x => x.Value > 0))
            {
                AccountsChargenResources dbaccountresource =
                    dbaccount.AccountsChargenResources.FirstOrDefault(
                        x => x.ChargenResourceId == cost.Key.Id);
    ...
    public string HandleCommand(string command)
    {
        ...
        string result = CurrentScreen.HandleCommand(command);
        RecalculateCurrentCosts();
    ...
    public void ResetStage(ChargenStage stage)
    {
        ...
        _completedStages.Remove(stage);
        List<KeyValuePair<ChargenStage, IEnumerable<ChargenStage>>> invalidatedStages =
            Gameworld.ChargenStoryboard.StageDependencies.Where(
                x => _completedStages.Contains(x.Key) && x.Value.Any(y => y == stage)).ToList();
        foreach (KeyValuePair<ChargenStage, IEnumerable<ChargenStage>> otherStage in invalidatedStages)
        {
    ...
    private void LoadFromXml(XElement root)
    {
        using (new FMDB())
        {
            ...
            foreach (XElement completed in root.Element("CompletedStages").Elements("Stage"))
            {
            ...
            XElement snElement = root.Element("SelectedName");
            ...
            XElement element = root.Element("SkillValues");
            if (element != null)
            {
                foreach (XElement item in element.Elements("Skill"))
                {
                    ITraitDefinition skill = Gameworld.Traits.Get(long.Parse(item.Attribute("ID").Value));
                    if (skill == null)
                    {
                        continue;
                    }
            ...
            foreach (XElement item in root.Element("SelectedNotes").Elements("Note"))
            {
            ...
            {
                foreach (XElement item in root.Element("Rejections").Elements("Rejection"))
                {
            ...
            if (element != null)
            {
                foreach (XElement role in element.Elements("Role"))
                {
            ...
            if (element != null)
            {
                foreach (XElement merit in element.Elements("Merit"))
                {
            ...
            if (element != null)
            {
                foreach (XElement knowledge in element.Elements("Knowledge"))
                {
            ...
            if (element != null)
            {
                foreach (XElement item in element.Elements("Tattoo"))
                {
                    ITattooTemplate tattoo = Gameworld.DisfigurementTemplates.Get(long.Parse(item.Attribute("id").Value)) as ITattooTemplate;
                    long bodypartId = long.Parse(item.Attribute("bodypart").Value);
                    ...
                    IReadOnlyDictionary<string, ITattooTextValue> textValues = (IReadOnlyDictionary<string, ITattooTextValue>)item.Element("TextValues")?
                        .Elements("TextValue")
    ...
    {
        return Traits.Any(x => x.Definition == trait);
    }
