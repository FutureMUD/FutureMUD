...
public class ChargenAdvice : SaveableItem, IChargenAdvice
{
    ...
    public ChargenAdvice(IFuturemud gameworld, string title, string text, ChargenStage stage)
    {
        ...
        using (new FMDB())
        {
            Models.ChargenAdvice dbitem = new()
            {
    ...
    {
        Models.ChargenAdvice dbitem = FMDB.Context.ChargenAdvices.Find(Id);
    ...
    private bool BuildingCommandRole(ICharacter actor, StringStack command)
    {
        ...
        IChargenRole role = Gameworld.Roles.GetByIdOrName(command.SafeRemainingArgument);
        if (role is null)
        {
    ...
    private bool BuildingCommandStage(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<ChargenStage>(out ChargenStage stage))
        {
            actor.OutputHandler.Send(
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach (IRace item in Gameworld.Races.Where(x => x.ChargenAdvices.Contains(this)))
        {
            sb.AppendLine($"\tRace #{item.Id.ToString("N0", actor)} - {item.Name}".ColourValue());
        }

        foreach (IEthnicity item in Gameworld.Ethnicities.Where(x => x.ChargenAdvices.Contains(this)))
        {
            sb.AppendLine($"\tEthnicity #{item.Id.ToString("N0", actor)} - {item.Name}".ColourValue());
        }

        foreach (ICulture item in Gameworld.Cultures.Where(x => x.ChargenAdvices.Contains(this)))
        {
            sb.AppendLine($"\tCulture #{item.Id.ToString("N0", actor)} - {item.Name}".ColourValue());
        }

        foreach (IChargenRole item in Gameworld.Roles.Where(x => x.ChargenAdvices.Contains(this)))
        {
