...
public abstract class ChargenScreenStoryboard : SaveableItem, IChargenScreenStoryboard
{
    ...
    protected void SaveAfterTypeChange()
    {
        using (new FMDB())
        {
            Models.ChargenScreenStoryboard dbitem = FMDB.Context.ChargenScreenStoryboards.Find(Id);
    ...
    public abstract class ChargenScreen : IChargenScreen
    {
        ...
        protected string DisplayChargenAdvice()
        {
            ...
            foreach (IChargenAdvice advice in Chargen.SelectedRace?.ChargenAdvices.Where(x =>
            ...
            foreach (IChargenAdvice advice in Chargen.SelectedEthnicity?.ChargenAdvices.Where(x =>
                         x.TargetStage == AssociatedStage &&
                         x.ShouldShowAdviceProg?.ExecuteBool(Chargen) != false) ?? Enumerable.Empty<ChargenAdvice>())
            {
                sb.AppendLine();
            ...
            foreach (IChargenAdvice advice in Chargen.SelectedCulture?.ChargenAdvices.Where(x =>
                         x.TargetStage == AssociatedStage &&
                         x.ShouldShowAdviceProg?.ExecuteBool(Chargen) != false) ?? Enumerable.Empty<ChargenAdvice>())
            {
                sb.AppendLine();
            ...
            foreach (IChargenAdvice advice in Chargen.SelectedRoles.SelectMany(x => x.ChargenAdvices).Distinct().Where(x =>
                         x.TargetStage == AssociatedStage && x.ShouldShowAdviceProg?.ExecuteBool(Chargen) != false))
            {
                sb.AppendLine();
                sb.AppendLine(advice.AdviceTitle.GetLineWithTitle(Chargen.Account.LineFormatLength,
    ...
    protected string ShowHeader(ICharacter voyeur)
    {
        ...
        int order = Gameworld.ChargenStoryboard.OrderOf(Stage);
        if (order == 0)
        {
    ...
    public sealed override void Save()
    {
        ...
        foreach (ChargenStage dependency in Gameworld.ChargenStoryboard.StageDependencies[Stage])
        {
