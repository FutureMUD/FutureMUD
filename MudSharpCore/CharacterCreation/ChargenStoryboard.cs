...
public class ChargenStoryboard : IChargenStoryboard
{
        ...
        : this()
    {
        ...
        using (new FMDB())
        {
            List<Tuple<IChargenScreenStoryboard, int>> storyboardList = new();
            foreach (Models.ChargenScreenStoryboard stage in FMDB.Context.ChargenScreenStoryboards.Include(x => x.DependentStages).AsNoTracking())
            {
                ChargenStage stageEnum = (ChargenStage)Convert.ToInt32(stage.ChargenStage);
                ChargenScreenStoryboardFactory factory = ChargenStageFactories[stageEnum]
                ...
                IChargenScreenStoryboard storyboard = factory.CreateNew(Gameworld, stage);
                _defaultNextStage[stageEnum] = (ChargenStage)Convert.ToInt32(stage.NextStage);
                foreach (ChargenScreenStoryboardDependentStage dependency in stage.DependentStages)
                {
    ...
    {
        ChargenStage movingStagePredecessor = _defaultNextStage.FirstOrDefault(x => x.Value == stage).Key;
        ChargenStage movingStageFollower = _defaultNextStage[stage];
        ChargenStage afterStageOldFollower = _defaultNextStage[afterStage];
        ...
        foreach (IChargenScreenStoryboard ss in _stageScreenMap.Values)
        {
            ss.Changed = true;
        }
    }
    ...
    {
        IChargenScreenStoryboard newScreen = _chargenNameFactoryEnumerable.First(x => x.Name.EqualTo(newType)).Factory
