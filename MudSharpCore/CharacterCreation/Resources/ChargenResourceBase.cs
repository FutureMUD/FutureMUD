...
public abstract class ChargenResourceBase : SaveableItem, IChargenResource
{
    ...
    {
        ChargenResource dbitem = FMDB.Context.ChargenResources.Find(Id);
        dbitem.Name = Name;
...
Note - for the cap formula, use #6variable#0 to substitute the referenced resource, and use a fixed value of #6-1#0 to signify that there is no cap.";
    ...
    private bool BuildingCommandAlias(ICharacter actor, StringStack command)
    {
        ...
        string alias = command.SafeRemainingArgument.TitleCase();
    ...
    private bool BuildingCommandPermission(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<PermissionLevel>(out PermissionLevel value))
        {
    ...
    private bool BuildingCommandTime(ICharacter actor, StringStack command)
    {
        ...
        if (!MudTimeSpan.TryParse(command.SafeRemainingArgument, actor, out MudTimeSpan value))
        {
    ...
    private bool BuildingCommandControlProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Boolean, [ProgVariableTypes.Character]).LookupProg();
        if (prog is null)
        {
            return false;
        }
    ...
    private bool BuildingCommandResource(ICharacter actor, StringStack command)
    {
        ...
        IChargenResource resource = Gameworld.ChargenResources.GetByIdOrName(command.SafeRemainingArgument);
        if (resource is null)
        {
