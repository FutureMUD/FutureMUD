...
public class RealtimeRegeneratingResource : ChargenResourceBase
{
    ...
    private void HeartbeatManagerOnFuzzyMinuteHeartbeat()
    {
        ...
        using (new FMDB())
        {
            List<Models.Account> accounts = FMDB.Context.Accounts
                               .Include(x => x.AccountsChargenResources)
                               .ToList();
            Dictionary<long, IAccount> accountLookup = Futuremud.Games.First().Accounts.ToDictionary(x => x.Id);
            DateTime now = DateTime.UtcNow;
            foreach (Models.Account dbaccount in accounts)
            {
                AccountsChargenResources resource = dbaccount.AccountsChargenResources.FirstOrDefault(x => x.ChargenResourceId == Id);
                if (resource is null)
                {
                ...
                IAccount account = accountLookup[dbaccount.Id];
