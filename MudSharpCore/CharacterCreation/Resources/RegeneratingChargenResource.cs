...
public class RegeneratingChargenResource : ChargenResourceBase
{
    ...
    public override void UpdateOnSave(ICharacter character, int oldMinutes, int newMinutes)
    {
        ...
        DateTime lastAward = character.Account.AccountResourcesLastAwarded.ValueOrDefault(this, null) ??
        ...
        double intervalCount = (DateTime.UtcNow - lastAward).Ticks / (double)AwardInterval.Ticks;
        if (intervalCount > 0 &&
