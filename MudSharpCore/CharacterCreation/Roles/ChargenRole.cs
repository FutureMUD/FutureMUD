...
internal class ChargenRole : SaveableItem, IChargenRole
{
    ...
    private ChargenRole(ChargenRole rhs, string name)
    {
        ...
        using (new FMDB())
        {
            ...
            foreach (KeyValuePair<IChargenResource, int> item in rhs.Costs)
            {
                ChargenRolesCost newItem = new();
            ...
            foreach (KeyValuePair<ICurrency, decimal> item in rhs.StartingCurrency)
            {
                ChargenRolesCurrency newItem = new();
                newItem.ChargenRole = dbitem;
            ...
            foreach (KeyValuePair<ITraitDefinition, (double amount, bool giveIfMissing)> item in rhs.TraitAdjustments)
            {
                ChargenRolesTrait newItem = new();
                newItem.ChargenRole = dbitem;
            ...
            foreach (IRoleClanMembership item in rhs.ClanMemberships)
            {
                ChargenRolesClanMemberships newItem = new();
                newItem.ChargenRole = dbitem;
                ...
                foreach (IAppointment appointment in item.Appointments)
                {
                    ChargenRolesClanMembershipsAppointments newAppointment = new();
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.ChargenRole dbitem = FMDB.Context.ChargenRoles.Find(Id);
            dbitem.Name = Name;
            ...
            foreach (IChargenAdvice item in _chargenAdvices)
            {
                dbitem.ChargenAdvicesChargenRoles.Add(new ChargenAdvicesChargenRoles
                { ChargenAdviceId = item.Id, ChargenRole = dbitem });
            }
            ...
            foreach (string account in RequiredApprovers)
            {
                dbitem.ChargenRolesApprovers.Add(
                    new ChargenRolesApprovers
                    {
            ...
            foreach (KeyValuePair<IChargenResource, int> item in Costs)
            {
                ChargenRolesCost newItem = new();
                newItem.ChargenRole = dbitem;
                newItem.ChargenResourceId = item.Key.Id;
            ...
            foreach (KeyValuePair<ICurrency, decimal> item in StartingCurrency)
            {
                ChargenRolesCurrency newItem = new();
                newItem.ChargenRole = dbitem;
                newItem.CurrencyId = item.Key.Id;
            ...
            foreach (KeyValuePair<ITraitDefinition, (double amount, bool giveIfMissing)> item in TraitAdjustments)
            {
                ChargenRolesTrait newItem = new();
                newItem.ChargenRole = dbitem;
                newItem.TraitId = item.Key.Id;
            ...
            foreach (ChargenRolesClanMemberships membership in dbitem.ChargenRolesClanMemberships)
            {
            ...
            foreach (IRoleClanMembership item in ClanMemberships)
            {
                ChargenRolesClanMemberships newItem = new();
                newItem.ChargenRole = dbitem;
                newItem.ClanId = item.Clan.Id;
    ...
    private void LoadFromDatabase(MudSharp.Models.ChargenRole role)
    {
        ...
        foreach (ChargenAdvicesChargenRoles item in role.ChargenAdvicesChargenRoles)
        {
    ...
    public string Poster
    {
        get
        {
            using (new FMDB())
            {
                Models.Account dbaccount = FMDB.Context.Accounts.Find(PosterId);
    ...
    private bool BuildingCommandType(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.SafeRemainingArgument.TryParseEnum<ChargenRoleType>(out ChargenRoleType type))
        {
    ...
    {
        ICharacter actor = (ICharacter)parameters[0];
    ...
    private bool BuildingCommandCost(ICharacter actor, StringStack ss)
    {
        ...
        string resourceText = ss.PopSpeech();
        IChargenResource resource =
    ...
    private bool BuildingCommandProg(ICharacter actor, StringStack ss)
    {
        ...
        IFutureProg prog = long.TryParse(argText, out long value)
    ...
    private bool BuildingCommandApprovers(ICharacter actor, StringStack ss)
    {
        ...
        List<string> names = ss.SafeRemainingArgument.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).ToList();
        using (new FMDB())
        {
            HashSet<string> accnames = FMDB.Context.Accounts.Select(x => x.Name).AsNoTracking().ToHashSet();
    ...
    private bool BuildingCommandViewPermission(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.PopSpeech().TryParseEnum<PermissionLevel>(out PermissionLevel level))
        {
            actor.Send("That is not a valid permission level. See {0}.",
                "show permissions".Colour(Telnet.Yellow).FluentTagMXP("send", "href='show permissions'"));
            return false;
    ...
    private bool BuildingCommandClans(ICharacter actor, StringStack ss)
    {
        ...
        IClan clan = ClanModule.GetTargetClan(actor, ss.PopSpeech(), false);
        if (clan is null)
        {
    ...
    private bool BuildingCommandClanPaygrade(ICharacter actor, StringStack ss, IClan clan)
    {
        ...
        IPaygrade paygrade = long.TryParse(text, out long id) ?
        ...
        IRoleClanMembership membership = ClanMemberships.First(x => x.Clan == clan);
    ...
    {
        string text = ss.SafeRemainingArgument;
        IAppointment appointment = long.TryParse(text, out long id) ?
        ...
        IRoleClanMembership membership = ClanMemberships.FirstOrDefault(x => x.Clan == clan);
    ...
    {
        string text = ss.SafeRemainingArgument;
        IRank rank = long.TryParse(text, out long id) ?
        ...
        IRank minimumRank = membership.Appointments.Select(x => x.MinimumRankToHold).DefaultIfEmpty(clan.Ranks.MinBy(x => x.RankNumber)).MaxBy(x => x.RankNumber);
    ...
    private bool BuildingCommandTraits(ICharacter actor, StringStack ss)
    {
        ...
        ITraitDefinition trait = Gameworld.Traits.GetByName(ss.PopSpeech());
        if (trait is null)
        {
    ...
    private bool BuildingCommandCurrency(ICharacter actor, StringStack ss)
    {
        ...
        ICurrency currency = Gameworld.Currencies.GetByName(ss.PopSpeech());
        if (currency is null)
        {
        ...
        if (!currency.TryGetBaseCurrency(ss.SafeRemainingArgument, out decimal amount))
        {
    ...
    private bool BuildingCommandMerit(ICharacter actor, StringStack command)
    {
        ...
        IMerit merit = Gameworld.Merits.GetByIdOrName(command.SafeRemainingArgument);
        if (merit is null)
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        if (ClanMemberships.Any())
        {
            ...
            sb.AppendLine();
            foreach (IRoleClanMembership clan in ClanMemberships)
            {
                sb.AppendLine(
        ...
        if (TraitAdjustments.Any())
        {
            ...
            sb.AppendLine();
            foreach (KeyValuePair<ITraitDefinition, (double amount, bool giveIfMissing)> adjust in TraitAdjustments)
            {
                sb.AppendLine(
        ...
        if (AdditionalMerits.Any())
        {
            ...
            sb.AppendLine();
            foreach (IMerit merit in AdditionalMerits)
            {
