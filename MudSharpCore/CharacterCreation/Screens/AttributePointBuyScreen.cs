...
public class AttributePointBuyScreenStoryboard : ChargenScreenStoryboard
{
        ...
        : base(dbitem, gameworld)
    {
        ...
        MaximumBoostsProg = long.TryParse(definition.Element("MaximumBoostsProg").Value, out long value)
    ...
    internal class AttributePointBuyScreen : ChargenScreen
    {
        ...
        public void Reset()
        {
            foreach (IAttributeDefinition attribute in Chargen.SelectedRace.Attributes.Where(x => x.TraitType == TraitType.Attribute))
            {
        ...
        {
            List<ITrait> selectedAttributes = new();
        ...
        public override string Display()
        {
            ...
            (int boosts, int remaining) = Storyboard.EvaluateBoosts(Chargen);
            ...
            if (boosts > 0)
            {
                IEnumerable<(IChargenResource Resource, int Cost)> costs = Storyboard.ChargenCosts(Chargen);
                sb.AppendLine(
        ...
        public override string HandleCommand(string command)
        {
            ...
            {
                (int _, int remaining) = Storyboard.EvaluateBoosts(Chargen);
                if (remaining > 0)
                {
                    return
            ...
            IAttributeDefinition which = _baseValues.Keys.FirstOrDefault(
