...
public class DisfigurementPickerScreenStoryboard : ChargenScreenStoryboard
{
        ...
        dbitem, gameworld)
    {
        ...
        foreach (XElement prosthesis in definition.Element("Prostheses")?.Elements() ?? Enumerable.Empty<XElement>())
        {
            IGameItemProto item = Gameworld.ItemProtos.Get(long.Parse(prosthesis.Element("Item")?.Value ??
                                                           throw new ApplicationException(
            ...
            Counter<IChargenResource> costs = new();
            foreach (XElement cost in prosthesis.Element("Costs")?.Elements() ?? Enumerable.Empty<XElement>())
            {
            ...
            XElement element = prosthesis.Element("CanSelectProg");
            if (element == null)
            {
                throw new ApplicationException(
            ...
            IFutureProg prog = long.TryParse(element.Value, out long progId)
    ...
    private bool BuildingCommandProstheticProg(ICharacter actor, StringStack command)
    {
        ...
        ChargenProsthesisSelection prosthetic = PickableProstheses.First(x => x.Item.Id == value);
    ...
    private bool BuildingCommandProstheticCosts(ICharacter actor, StringStack command)
    {
        ...
        {
            if (!int.TryParse(command.PopSpeech(), out int amount))
            {
            ...
            IChargenResource resource = Gameworld.ChargenResources.GetByIdOrNames(command.PopSpeech());
            if (resource is null)
            {
        ...
        prosthetic.ResourceCosts.Clear();
        foreach (KeyValuePair<IChargenResource, int> cost in costs)
        {
    ...
    private bool BuildingCommandProstheticAdd(ICharacter actor, StringStack command)
    {
        ...
        ProstheticGameItemComponentProto prostheticProto = proto.GetItemType<ProstheticGameItemComponentProto>();
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(Gameworld, actor, command.PopSpeech(),
            ProgVariableTypes.Boolean, new List<ProgVariableTypes>
            {
                ProgVariableTypes.Chargen
            }).LookupProg();
    ...
    {
        Counter<IChargenResource> resources = new();
        foreach (IGameItemProto prosthesis in chargen.SelectedProstheses)
        {
            foreach (KeyValuePair<IChargenResource, int> resource in PickableProstheses.First(x => x.Item == prosthesis).ResourceCosts)
            {
                resources[resource.Key] += resource.Value;
            }
        }

        foreach ((IDisfigurementTemplate Disfigurement, IBodypart Bodypart) disfigurement in chargen.SelectedDisfigurements)
        {
            foreach (KeyValuePair<IChargenResource, int> resource in disfigurement.Disfigurement.ChargenCosts)
            {
                resources[resource.Key] += resource.Value;
            }
        }

        foreach (ISelectedTattoo tattoo in chargen.SelectedTattoos)
        {
            foreach (KeyValuePair<IChargenResource, int> resource in tattoo.Tattoo.ChargenCosts)
            {
                resources[resource.Key] += resource.Value;
            }
        }
    ...
    internal class DisfigurementPickerScreen : ChargenScreen
    {
        ...
        private string DisplayTattoos()
        {
            ...
            {
                IReadOnlyDictionary<string, ITattooTextValue> previewValues = (IReadOnlyDictionary<string, ITattooTextValue>)_selectedTattooTextValues;
                sb.AppendLine(
                ...
                {
                    ITattooTemplateTextSlot slot = _pendingTattooTextSlots.Peek();
                    sb.AppendLine(
        ...
        private string DisplayProsthetics()
        {
            ...
            sb.AppendLine();
            List<ChargenProsthesisSelection> pickables = Storyboard.PickableProstheses
                                      .Where(x =>
                                      ...
                                      .ToList();
            int i = 1;
            foreach (ChargenProsthesisSelection pickable in pickables)
            {
                sb.AppendLine(
        ...
        private string HandleCommandProsthetics(string command)
        {
            ...
            if (cmd.EqualTo("clear"))
            {
                ...
                IGameItemProto targetItem = _selectedProstheses.GetFromItemListByKeyword(ss.PopSpeech(), null);
                if (targetItem == null)
                {
        ...
        private string HandleCommandTattoos(string command)
        {
            if (_selectedTattoo != null)
            {
                ...
                if (_pendingTattooTextSlots.Any())
                {
                    ...
                    int maxLength = MaximumTattooTextLength(slot);
                    ...
                    ISelectedTattoo finalTattoo = FinaliseSelectedTattoo();
                    UpdateChargen();
                    return
                ...
                IBodypart bodypart = BodypartsForTattoo(_selectedTattoo).FirstOrDefault(x => x.Name.EqualTo(command));
                if (bodypart == null)
                {
                    return
                ...
                foreach (ITattooTemplateTextSlot slot in _selectedTattoo.TextSlots.Where(x => x.RequiredCustomText))
                {
                ...
                ISelectedTattoo tattoo = FinaliseSelectedTattoo();
                UpdateChargen();
                return
            ...
            if (command.EqualTo("shuffle"))
            {
                ...
                List<ITattooTemplate> filtered = new();
                foreach (ITattooTemplate tattoo in _tattooTemplates)
                {
        ...
        {
            SelectedTattoo selection = new(_selectedTattoo, _selectedTattooBodypart,
        ...
        private string HandleCommandBodyparts(string command)
        {
            ...
            if (command.EqualTo("parts"))
            {
                ...
                void DescribeLevel(IBodypart proto, int level)
                {
                    ...
                    foreach (IBodypart branch in parts.Where(x => x.UpstreamConnection == proto).ToList())
                    {
            ...
            List<IExternalBodypart> allparts = Chargen.SelectedRace.BaseBody
                                  .BodypartsFor(Chargen.SelectedRace, Chargen.SelectedGender)
                                  .OfType<IExternalBodypart>()
                                  .ToList();
            IExternalBodypart part = allparts.FirstOrDefault(x => x.Name.EqualTo(command)) ??
