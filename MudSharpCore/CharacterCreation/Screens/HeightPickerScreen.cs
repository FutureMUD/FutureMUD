...
public class HeightPickerScreenStoryboard : ChargenScreenStoryboard
{
        ...
        : base(dbitem, gameworld)
    {
        ...
        MaximumHeightProg = long.TryParse(definition.Element("MaximumHeightProg").Value, out long value)
    ...
    internal class HeightPickerScreen : ChargenScreen
    {
        ...
        public override string HandleCommand(string command)
        {
            ...
            double height = Storyboard.Gameworld.UnitManager.GetBaseUnits(command, UnitType.Length, out bool success);
            if (!success)
            {
                return "That is not a valid height.";
            }

            double minimumHeight = Convert.ToDouble(Storyboard.MinimumHeightProg.Execute(Chargen));
            double maximumHeight = Convert.ToDouble(Storyboard.MaximumHeightProg.Execute(Chargen));
