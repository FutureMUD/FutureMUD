...
internal class KnowledgePickerBySkillScreenStoryboard : ChargenScreenStoryboard
{
        ...
        : base(dbitem, gameworld)
    {
        ...
        XElement element = definition.Element("NumberOfPicksProg");
        NumberOfPicksProg = long.TryParse(element?.Value ?? "0", out long value)
    ...
    {
        List<IKnowledge> freeKnowledges = ((IList<IProgVariable>)FreeKnowledgesProg.Execute(chargen)).OfType<IKnowledge>()
            .ToList();
        foreach (IChargenResource resource in Gameworld.ChargenResources)
        {
            int sum = 0;
            foreach (IKnowledge knowledge in chargen.SelectedKnowledges)
            {
...
Enter the name or number of the knowledge you would like to select, and #3done#0 to finish.".SubstituteANSIColour();
        ...
        public override string HandleCommand(string command)
        {
            ...
            {
                string which = command.RemoveFirstWord();
            ...
            int picks = Storyboard.NumberOfPicksProg.ExecuteInt(Chargen, SkillEnumerator.Current.Trait);
