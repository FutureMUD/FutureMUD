...
public class NotePickerScreenStoryboard : ChargenScreenStoryboard
{
        ...
        : base(dbitem, gameworld)
    {
        ...
        List<(string, string, IFutureProg)> notes = definition
                    .Elements("Note")
                    .Select(note => (
                        note.Attribute("Name").Value,
                        note.Element("Blurb")?.Value,
                        int.TryParse(note.Element("Prog")?.Value, out int value)
    ...
    private void PostAddNote(string text, IOutputHandler handler, object[] args)
    {
        ...
        IFutureProg prog = (IFutureProg)args[2];
    ...
    private bool BuildingCommandNoteRemove(ICharacter actor, StringStack command)
    {
        ...
        (string Name, string Blurb, IFutureProg Prog) note = NotesBlurbs.FirstOrDefault(x => x.Name.EqualTo(name));
        if (note == default)
        {
    ...
    {
        string which = (string)args[1];
        (string Name, string Blurb, IFutureProg Prog) note = NotesBlurbs.FirstOrDefault(x => x.Name.EqualTo(which));
        if (note == default)
        {
    ...
    public override string Show(ICharacter voyeur)
    {
        ...
        {
            foreach ((string name, string blurb, IFutureProg prog) in NotesBlurbs)
            {
                sb.AppendLine();
