...
public class RolePickerScreenStoryboard : ChargenScreenStoryboard
{
        ...
        : base(dbitem, gameworld)
    {
        ...
        XElement element = definition.Element("IntroductionBlurb");
        if (element != null)
        {
        ...
        if (element != null)
        {
            foreach (XElement sub in element.Elements("RoleType"))
            {
                ChargenRoleType type = int.TryParse(sub.Attribute("Type").Value, out int value)
        ...
        if (element != null)
        {
            foreach (XElement sub in element.Elements("Role"))
            {
        ...
        foreach (ChargenRoleType type in Enum.GetValues<ChargenRoleType>().Except(ChargenRoleType.StartingLocation))
        {
    ...
    public override string Show(ICharacter voyeur)
    {
        ...
        int i = 1;
        foreach (ChargenRoleType type in TypeOrder)
        {
            sb.AppendLine();
    ...
    public override IEnumerable<(IChargenResource Resource, int Cost)> ChargenCosts(IChargen chargen)
    {
        ...
        {
            int sum = 0;
            foreach (IChargenRole role in chargen.SelectedRoles)
            {
    ...
    internal class RolePickerScreen : ChargenScreen
    {
        ...
        public override string Display()
        {
            ...
            {
                int index = 1;
                int longestName = _roles.Max(x => x.Name.Length);
                int columns = Math.Min(Chargen.Account.LineFormatLength / longestName, 3);
                return
        ...
        public override string HandleCommand(string command)
        {
            ...
            if (_selectedRole == null)
            {
                if (int.TryParse(command, out int value))
                {
                    ...
                    {
                        string currentStage = Storyboard.TypeNames[CurrentStage];
                        if (AdvanceStage())
                        {
...
The valid types are:
    ...
    public override bool BuildingCommand(ICharacter actor, StringStack command)
    {
        ...
        if (command.Last.TryParseEnum<ChargenRoleType>(out ChargenRoleType roleType))
        {
    ...
    private bool BuildingCommandSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<ChargenRoleType>(out ChargenRoleType type1))
        {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<ChargenRoleType>(out ChargenRoleType type2))
        {
    ...
    {
        ChargenRoleType roleType = (ChargenRoleType)args[1];
