...
public class SkillCostPickerScreenStoryboard : ChargenScreenStoryboard
{
        ...
        : base(dbitem, gameworld)
    {
        ...
        NumberOfFreeSkillPicksProg =
            long.TryParse(definition.Element("NumberOfFreeSkillPicksProg").Value, out long value)
    ...
    public override IEnumerable<(IChargenResource Resource, int Cost)> ChargenCosts(IChargen chargen)
    {
        ...
        {
            int sum = 0;
            foreach (ITraitDefinition skill in nonFreeSkills)
            {
    ...
    internal class SkillCostPickerScreen : ChargenScreen
    {
        ...
        protected string DisplaySkillBoostStage()
        {
            ...
            foreach (ITraitDefinition skill in Chargen.SelectedSkills)
            {
                skillStrings.Add(
        ...
        {
            List<ITraitDefinition> selectedSkills = Chargen.SelectedSkills.Except(FreeSkills).ToList();
            List<ITraitDefinition> languages = CurrentSelectables.Where(x => x.Group.EqualTo("language")).ToList();
            List<ITraitDefinition> nonLanguages = CurrentSelectables.Where(x => !languages.Contains(x)).ToList();
            if (languages.Any())
            {
...
The following arguments can all accept an option number of times to do the action:
	...
	#6-all [<times>]#0 - remove x boosts from all skills".SubstituteANSIColour();
            ...
            {
                foreach (KeyValuePair<ITraitDefinition, int> skill in SelectedBoosts.ToList())
                {
            ...
            List<ITraitDefinition> skillsToChange = new();
            if (groupReference)
            {
            ...
            {
                ITraitDefinition skill = Chargen.SelectedSkills.FirstOrDefault(
            ...
            foreach (ITraitDefinition skill in skillsToChange)
            {
        ...
        private string HandleCommandSkillPick(string command)
        {
            ...
            {
                StringStack ss = new(lcommand.RemoveFirstWord());
                if (ss.IsFinished)
                {
                    return "What skill do you want to see help for?";
                }

                string argument = ss.SafeRemainingArgument;
                ITraitDefinition hskill =
...
Note, the boost cost expression can use the following parameters:
    ...
    private bool BuildingCommandResource(ICharacter actor, StringStack command)
    {
        ...
        IChargenResource resource = Gameworld.ChargenResources.GetByIdOrNames(command.SafeRemainingArgument);
        if (resource is null)
        {
