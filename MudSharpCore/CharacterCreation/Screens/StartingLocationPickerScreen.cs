...
public class StartingLocationPickerScreenStoryboard : ChargenScreenStoryboard
{
        ...
        dbitem, gameworld)
    {
        ...
        XElement element = definition.Element("Blurb");
        if (element != null)
        {
            Blurb = element.Value;
        }
        ...
        if (element != null)
        {
            ...
            foreach (XElement location in element.Elements("Location"))
            {
    ...
    public override string Show(ICharacter voyeur)
    {
        ...
        int i = 1;
        foreach (StartingLocation location in Locations)
        {
            sb.AppendLine();
    ...
    internal class StartingLocationPickerScreen : ChargenScreen
    {
        ...
        public override string HandleCommand(string command)
        {
            ...
            SelectedLocation = int.TryParse(command, out int value)
    ...
    private bool BuildingCommandLocationRoom(ICharacter actor, StringStack command)
    {
        ...
        string locationText = command.PopSpeech();
        StartingLocation which = Locations.FirstOrDefault(x => x.Name.EqualTo(locationText)) ??
    ...
    private bool BuildingCommandLocationRemove(ICharacter actor, StringStack command)
    {
        ...
        StartingLocation which = Locations.FirstOrDefault(x => x.Name.EqualTo(command.SafeRemainingArgument)) ??
                    Locations.FirstOrDefault(x =>
    ...
    {
        StartingLocation which = (StartingLocation)args[1];
        which.Blurb = text;
        Changed = true;
        handler.Send(
    ...
    private bool BuildingCommandLocationAdd(ICharacter actor, StringStack command)
    {
        ...
        IChargenRole role = Gameworld.Roles.GetByIdOrName(command.PopSpeech());
        if (role is null)
        {
...
{text.Wrap((int)args[0]).SubstituteANSIColour()}");
    ...
    private bool BuildingCommandLocationOnCommenceProg(ICharacter actor, StringStack command)
    {
        ...
        StartingLocation which = Locations.FirstOrDefault(x => x.Name.EqualTo(text)) ??
                    Locations.FirstOrDefault(x =>
