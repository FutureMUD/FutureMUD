...
internal class WeightPickerScreenStoryboard : ChargenScreenStoryboard
{
        ...
        : base(dbitem, gameworld)
    {
        ...
        MaximumWeightProg = long.TryParse(definition.Element("MaximumWeightProg").Value, out long value)
    ...
    internal class WeightPickerScreen : ChargenScreen
    {
        ...
        public override string HandleCommand(string command)
        {
            ...
            double weight = Storyboard.Gameworld.UnitManager.GetBaseUnits(command, UnitType.Mass, out bool success);
            if (!success)
            {
                return "That is not a valid weight.";
            }

            double minimumWeight = Convert.ToDouble(Storyboard.MinimumWeightProg.Execute(Chargen));
            double maximumWeight = Convert.ToDouble(Storyboard.MaximumWeightProg.Execute(Chargen));
