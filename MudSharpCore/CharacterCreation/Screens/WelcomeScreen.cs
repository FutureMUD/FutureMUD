...
internal class WelcomeScreenStoryboard : ChargenScreenStoryboard
{
        ...
        gameworld)
    {
        ...
        foreach (XElement item in definition.Elements("Blurb"))
        {
    ...
    public override string Show(ICharacter voyeur)
    {
        ...
        int i = 1;
        foreach (string blurb in _blurbs)
        {
            sb.AppendLine();
    ...
    {
        if (command.IsFinished || !int.TryParse(command.PopSpeech(), out int index1) || index1 <= 0)
        {
    ...
    private void PostEditBlurb(string text, IOutputHandler handler, object[] args)
    {
        ...
        {
            ICharacter ch = (ICharacter)args[2];
    ...
    private bool BuildingCommandBlurbReorder(ICharacter actor, StringStack command)
    {
        ...
        if (command.IsFinished || !int.TryParse(command.PopSpeech(), out int index2) || index2 <= 0)
        {
            actor.OutputHandler.Send(
