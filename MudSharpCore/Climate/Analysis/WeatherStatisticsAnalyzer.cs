...
internal sealed class WeatherTransitionSnapshot
{
		...
		IReadOnlyDictionary<(long SeasonId, long EventId), IReadOnlyList<(long EventId, double Chance)>> transitions)
	{
		...
		foreach (var item in maximumAdditionalChangeChanceFromStableWeather)
		{
			_maximumAdditionalChangeChanceFromStableWeather[item.Key] = item.Value;
		}

		foreach (var item in incrementalAdditionalChangeChanceFromStableWeather)
		{
			_incrementalAdditionalChangeChanceFromStableWeather[item.Key] = item.Value;
		}

		foreach (var item in baseChangeChance)
		{
	...
	public IEnumerable<long> SeasonEventIds(long seasonId)
	{
		...
		foreach (var key in _baseChangeChance.Keys.Where(x => x.SeasonId == seasonId))
		{
			ids.Add(key.EventId);
		}

		foreach (var kvp in _transitions.Where(x => x.Key.SeasonId == seasonId))
		{
		...
		Random random)
	{
		...
		var hasData = _baseChangeChance.ContainsKey((currentSeasonId, currentWeatherEventId.Value)) &&
					  _transitions.ContainsKey((currentSeasonId, currentWeatherEventId.Value));

		var currentEvent = GetEvent(currentWeatherEventId.Value);
		var forceChange = !hasData || currentEvent is null || !currentEvent.PermittedTimesOfDay.Contains(currentTime);

		var baseChance = _baseChangeChance.GetValueOrDefault((currentSeasonId, currentWeatherEventId.Value));
		var maxAdditional = _maximumAdditionalChangeChanceFromStableWeather.GetValueOrDefault(currentSeasonId);
		var incrementalAdditional = _incrementalAdditionalChangeChanceFromStableWeather.GetValueOrDefault(currentSeasonId);
		var shouldChange = forceChange ||
		...
		if (_cachedTransitions.TryGetValue((currentSeasonId, currentWeatherEventId.Value, currentTime), out var options) &&
			options.Length > 0)
		{
	...
	{
		if (!_seasonEventCache.TryGetValue((seasonId, time), out var options))
		{
			return null;
		}
	...
	private void RecalculateCaches()
	{
		...
		foreach (var kvp in _transitions)
		{
			foreach (var time in _timesOfDay)
			{
				var values = kvp.Value
					.Where(x => _events.TryGetValue(x.EventId, out var weatherEvent) && weatherEvent.PermittedTimesOfDay.Contains(time))
					.ToArray();
		...
		var temp = new Dictionary<(long SeasonId, TimeOfDay Time), List<(long EventId, double Chance)>>();
		foreach (var kvp in _baseChangeChance)
		{
			if (!_events.TryGetValue(kvp.Key.EventId, out var weatherEvent))
			{
				continue;
			}

			foreach (var time in weatherEvent.PermittedTimesOfDay)
			{
				var key = (kvp.Key.SeasonId, time);
...
internal static class WeatherStatisticsMath
{
	public static double Percentile(IReadOnlyList<double> values, double percentile)
	{
		...
		var clamped = Math.Max(0.0, Math.Min(1.0, percentile));
...
internal sealed class HourlyBlockTracker
{
	...
	private void FlushCurrentBlock(IReadOnlyDictionary<long, SeasonAccumulator> accumulators)
	{
		...
		if (accumulators.TryGetValue(_seasonId, out var accumulator))
		{
			if (!accumulator.HourlyStatistics.TryGetValue(_hour, out var hourly))
			{
...
internal sealed class WeatherStatisticsAnalyzer
{
	public WeatherStatisticsResult Analyze(WeatherStatisticsRequest request)
	{
		...
		var transitionSnapshot = LoadTransitionSnapshot(request.Controller);
		var simulationContext = BuildSimulationContext(request.Controller);
		var seasonInfo = request.Controller.RegionalClimate.Seasons
			.Distinct()
			.ToDictionary(x => x.Id);
		...
		var initialSeasonId = request.Controller.CurrentSeason?.Id ?? seasonInfo.Values.First().Id;
		if (!seasonInfo.ContainsKey(initialSeasonId))
		{
			initialSeasonId = seasonInfo.Values.First().Id;
		}

		var simulationRequest = new WeatherStatisticsSimulationRequest
		{
			Years = request.Years,
		...
		string controllerName = "Simulation")
	{
		...
		try
		{
			...
			var seasonAccumulators = request.SeasonInfo.Values.ToDictionary(x => x.Id, _ => new SeasonAccumulator());
			var hourlyTracker = new HourlyBlockTracker();
			var state = new WeatherSimulationState
			{
			...
			{
				var initialSeason = request.SimulationContext.SeasonRotation.Get(request.SimulationContext.GetCelestialDay());
			...
			var expectedMinutesPerYear = (long)Math.Max(1.0,
			...
			var maxMinutes = Math.Max(2_000_000L, expectedMinutesPerYear * (request.Years + request.BurnInYears + 2L) * 2L);
			var collectData = request.BurnInYears == 0;
			var burnInYears = 0;
			var measuredYears = 0;
			var simulatedMinutes = 0L;
			...
			var rows = BuildRows(request.SeasonInfo, seasonAccumulators, request.TransitionSnapshot,
	...
	private static List<WeatherStatisticsCsvRow> BuildRows(
		...
		int hoursPerDay)
	{
		var rows = new List<WeatherStatisticsCsvRow>();
		foreach (var season in seasonInfo.Values.OrderBy(x => x.CelestialDayOnset).ThenBy(x => x.Name))
		{
			if (!seasonAccumulators.TryGetValue(season.Id, out var stats))
			{
				continue;
			}

			var totalMinutes = stats.TotalMinutes;
			var eventIds = transitionSnapshot.SeasonEventIds(season.Id)
			...
			{
				var eventName = transitionSnapshot.GetEvent(eventId)?.Name ?? eventId.ToString(CultureInfo.InvariantCulture);
	...
	private static WeatherTransitionSnapshot LoadTransitionSnapshot(IWeatherController controller)
	{
		using (new FMDB())
		{
			var dbModel = FMDB.Context.ClimateModels
			...
			foreach (var season in dbModel.ClimateModelSeasons)
			{
				...
				foreach (var seasonEvent in season.SeasonEvents)
				{
					...
					var parsedTransitions = ParseTransitions(seasonEvent.Transitions)
			...
			var eventInfo = new Dictionary<long, WeatherEventInfo>();
			foreach (var eventId in eventIds)
			{
				var weatherEvent = controller.Gameworld.WeatherEvents.Get(eventId);
				if (weatherEvent is null)
				{
					continue;
				}
	...
	internal static List<(long EventId, double Chance)> ParseTransitions(string xml)
	{
		...
		foreach (var element in root.Elements())
		{
			...
			if (!double.TryParse(chanceText, NumberStyles.Float | NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out var chance))
			{
				continue;
			}
	...
	private static WeatherSimulationContext BuildSimulationContext(IWeatherController controller)
	{
		...
		var baseTemperatures = controller.RegionalClimate.HourlyBaseTemperaturesBySeason
			.ToDictionary(x => (x.Key.Season.Id, x.Key.DailyHour), x => x.Value);
		foreach (var season in controller.RegionalClimate.Seasons)
		{
			for (var hour = 0; hour < controller.FeedClock.HoursPerDay; hour++)
			{
		...
		var feedClockState = new FeedClockState(
		...
		var realSecondsPerFeedMinute = controller.FeedClock.SecondsPerMinute / Math.Max(0.000_001, controller.FeedClock.InGameSecondsPerRealSecond);
		var seasonYearLength = controller.RegionalClimate.SeasonRotation.Ceiling;
		...
		var celestialModels = LoadCelestialModels(controller.Celestial);
		ICelestialSimulation? celestialSimulation = null;
		if (controller.Celestial is not null)
		{
			var (simulation, actions) = BuildCelestialSimulation(controller.Celestial, celestialModels);
			celestialSimulation = simulation;
			cleanupActions.AddRange(actions);
		}
		var runtime = new RuntimeSimulationState(feedClockState, celestialSimulation, realSecondsPerFeedMinute,
		...
		IReadOnlyDictionary<long, MudSharp.Models.Celestial> celestialModels)
	{
		switch (source)
		{
			...
			{
				var (clonedMoon, clonedClock, _, cleanupActions) = ClonePlanetaryMoon(moon);
				return (new ClockDrivenCelestialSimulation(clonedMoon, new[] { clonedClock }), cleanupActions);
			}
			case SunFromPlanetaryMoon sunFromMoon:
			{
				var (clonedMoon, moonClock, _, moonCleanupActions) = ClonePlanetaryMoon(sunFromMoon.Moon);
				var (clonedSun, sunClock, _, sunCleanupActions) = CloneNewSun(sunFromMoon.Sun, celestialModels);
				var clonedObject = new SunFromPlanetaryMoon(clonedMoon, clonedSun);
			...
			case PlanetFromMoon planetFromMoon:
			{
				var (daySource, dayCleanupActions) = BuildCelestialSimulation(planetFromMoon.Moon, celestialModels);
		...
		IReadOnlyDictionary<long, MudSharp.Models.Celestial> celestialModels)
	{
		...
		var clonedCalendar = CloneCalendar(source.Calendar, clonedClock);
		var clonedSun = new NewSun(XElement.Parse(dbCelestial.Definition), clonedClock, source.Gameworld);
		SetNoSave(clonedSun);
	...
	private static (PlanetaryMoon Moon, Clock Clock, ICalendar Calendar, List<Action> CleanupActions) ClonePlanetaryMoon(PlanetaryMoon source)
	{
		var clonedClock = CloneClock(source.Clock);
		var clonedCalendar = CloneCalendar(source.Calendar, clonedClock);
		var clonedMoon = new PlanetaryMoon(clonedCalendar, clonedClock);
	...
	private static Clock CloneClock(IClock sourceClock)
	{
		...
		var primary = concreteClock.PrimaryTimezone;
		if (primary is null)
		{
			throw new ApplicationException($"The clock {sourceClock.Name} has no primary timezone.");
		}
		var clonedPrimary = new MudTimeZone(0, primary.OffsetHours, primary.OffsetMinutes,
			primary.Description, primary.Alias);
		SetNoSave(clonedPrimary);
		var clonedClock = new Clock(concreteClock.SaveToXml(), concreteClock.Gameworld, clonedPrimary,
		...
		foreach (var timezone in concreteClock.Timezones.Where(x => x.Id != primary.Id))
		{
			var clonedTimezone = new MudTimeZone(0, timezone.OffsetHours,
	...
	private static Calendar CloneCalendar(ICalendar sourceCalendar, IClock feedClock)
	{
		...
		var clonedCalendar = new Calendar(concreteCalendar.SaveToXml(), concreteCalendar.Gameworld);
	...
	private static Action CreateClockMinutesCleanup(IClock clock, object target, string methodName)
	{
		try
		{
			...
			var handler = (ClockEventHandler)Delegate.CreateDelegate(typeof(ClockEventHandler), target, method);
	...
	private static TimeOfDay DetermineTimeOfDayByHour(int localHour, int hoursPerDay)
	{
		...
		var fraction = localHour / (double)hoursPerDay;
		if (fraction < 0.20)
		{
			return TimeOfDay.Night;
		}
	...
	private sealed class ClockDrivenCelestialSimulation : ICelestialSimulation
	{
		...
		public bool AdvanceByRealSeconds(double realSeconds)
		{
			...
			var day = _celestial.CurrentCelestialDay;
			var wrapped = day + 0.000_001 < _previousDay;
			_previousDay = day;
			return wrapped;
		}
		...
		private sealed class ClockAdvanceState
		{
			...
			{
				_fractionalInGameSeconds += realSeconds * _inGameSecondsPerRealSecond;
				var wholeSeconds = (int)Math.Floor(_fractionalInGameSeconds);
				if (wholeSeconds <= 0)
				{
					return;
				}
	...
	private sealed class CompositeCelestialSimulation : ICelestialSimulation
	{
		...
		public bool AdvanceByRealSeconds(double realSeconds)
		{
			var wrapped = _daySource.AdvanceByRealSeconds(realSeconds);
	...
	private sealed class FeedClockState
	{
		...
		public int LocalHour
		{
			get
			{
				var localMinutes = _minutes + _targetOffsetMinutes - _primaryOffsetMinutes;
				var hourCarry = (int)Math.Floor(localMinutes / (double)MinutesPerHour);
				var localHours = _hours - _primaryOffsetHours + _targetOffsetHours + hourCarry;
