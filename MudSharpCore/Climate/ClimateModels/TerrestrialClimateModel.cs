...
public class TerrestrialClimateModel : ClimateModelBase
{
    ...
    private void RecalculateCaches()
    {
        ...
        foreach (KeyValuePair<(ISeason Season, IWeatherEvent OldEvent), List<(IWeatherEvent NewEvent, double Chance)>> kvp in _newWeatherEventChances)
        {
            ...
            {
                (IWeatherEvent NewEvent, double Chance)[] arr = kvp.Value.Where(x => x.NewEvent.PermittedTimesOfDay.Contains(time)).ToArray();
        ...
        Dictionary<(ISeason, TimeOfDay), List<(IWeatherEvent, double)>> temp = new();
        foreach (KeyValuePair<(ISeason Season, IWeatherEvent Event), double> kvp in _weatherEventChangeChance)
        {
            foreach (TimeOfDay time in kvp.Key.Event.PermittedTimesOfDay)
            {
                (ISeason Season, TimeOfDay time) key = (kvp.Key.Season, time);
                if (!temp.TryGetValue(key, out List<(IWeatherEvent, double)> list))
                {
        ...
        foreach (KeyValuePair<(ISeason, TimeOfDay), List<(IWeatherEvent, double)>> kvp in temp)
        {
            (IWeatherEvent, double)[] arr = kvp.Value.ToArray();
            ...
            TimeOfDay currentTime, int consecutiveUnchangedPeriods)
    {
        ...
        {
            if (_cachedTransitions.TryGetValue((currentSeason, currentWeather, currentTime), out (IWeatherEvent NewEvent, double Chance)[] options) &&
                options.Length > 0)
            {
    ...
    public override IEnumerable<IWeatherEvent> PermittedTransitions(IWeatherEvent currentEvent, ISeason currentSeason,
            TimeOfDay timeOfDay)
    {
        return _cachedTransitions.TryGetValue((currentSeason, currentEvent, timeOfDay), out (IWeatherEvent NewEvent, double Chance)[] list)
    ...
    private bool BuildingCommandCloneSeason(ICharacter actor, StringStack command)
    {
        ...
        ISeason clone = Gameworld.Seasons.GetByIdOrName(command.SafeRemainingArgument);
        if (clone is null)
        {
        ...
        ISeason season = Gameworld.Seasons.GetByIdOrName(command.SafeRemainingArgument);
        if (season is null)
        {
            actor.OutputHandler.Send($"The text {command.SafeRemainingArgument.ColourCommand()} does not represent a valid season.");
            return false;
        ...
        foreach (IWeatherEvent weather in WeatherEvents)
        {
    ...
    private bool BuildingCommandSeason(ICharacter actor, StringStack command)
    {
        ...
        ISeason season = Seasons.GetByIdOrName(command.PopSpeech());
        if (season is null)
        {
        ...
        foreach (IWeatherEvent item in WeatherEvents)
        {
            sb.AppendLine();
            ...
            sb.AppendLine();
            double total = _newWeatherEventChances[(season, item)].Sum(x => x.Chance);
            foreach ((IWeatherEvent NewEvent, double Chance) trans in _newWeatherEventChances[(season, item)].OrderByDescending(x => x.Chance))
            {
    ...
    private bool BuildingCommandSeasonTransition(ICharacter actor, StringStack command, ISeason season)
    {
        ...
        IWeatherEvent weather = Gameworld.WeatherEvents.GetByIdOrName(command.PopSpeech());
        if (weather is null)
        {
        ...
        IWeatherEvent newWeather = Gameworld.WeatherEvents.GetByIdOrName(command.PopSpeech());
        ...
        Changed = true;
        RecalculateCaches();
        double total = _newWeatherEventChances[(season, weather)].Sum(x => x.Chance);
    ...
    private bool BuildingCommandRemoveSeason(ICharacter actor, StringStack command)
    {
        ...
        ISeason season = Seasons.GetByIdOrName(command.SafeRemainingArgument);
        if (season is null)
        {
            actor.OutputHandler.Send($"The text {command.SafeRemainingArgument.ColourCommand()} does not represent a valid season.");
            return false;
    ...
    private void DoDatabaseInsert()
    {
        using (new FMDB())
        {
            ...
            foreach (ISeason season in Seasons)
            {
                ClimateModelSeason dbseason = new()
                {
                    ClimateModel = dbitem,
                ...
                foreach (IWeatherEvent weather in WeatherEvents)
                {
                    ...
                    ClimateModelSeasonEvent dbevent = new()
                    {
    ...
    public TerrestrialClimateModel(TerrestrialClimateModel rhs, string name)
    {
        ...
        foreach (ISeason season in rhs.Seasons)
        {
            ...
            foreach (IWeatherEvent weather in rhs.WeatherEvents)
            {
    ...
    public TerrestrialClimateModel(Models.ClimateModel model, IFuturemud gameworld)
    {
        ...
        foreach (ClimateModelSeason dbseason in model.ClimateModelSeasons)
        {
            ISeason season = gameworld.Seasons.Get(dbseason.SeasonId);
            ...
            foreach (ClimateModelSeasonEvent dbevent in dbseason.SeasonEvents)
            {
                IWeatherEvent we = gameworld.WeatherEvents.Get(dbevent.WeatherEventId);
                _weatherEventChangeChance[(season, we)] = dbevent.ChangeChance;
                List<(IWeatherEvent, double)> transitions = new();
                foreach (XElement transition in XElement.Parse(dbevent.Transitions).Elements())
                {
                    IWeatherEvent other = gameworld.WeatherEvents.Get(long.Parse(transition.Attribute("id").Value));
                    if (other == we)
                    {
                        continue;
                    }
    ...
    {
        ClimateModel dbitem = FMDB.Context.ClimateModels.Find(Id);
        dbitem.Name = Name;
