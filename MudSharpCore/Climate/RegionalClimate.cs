...
public class RegionalClimate : SaveableItem, IRegionalClimate
{
    public RegionalClimate(MudSharp.Models.RegionalClimate climate, IFuturemud gameworld)
    {
        ...
        foreach (RegionalClimatesSeason season in climate.RegionalClimatesSeasons)
        {
            ISeason gseason = gameworld.Seasons.Get(season.SeasonId);
            _seasons.Add(gseason);
            XElement definition = XElement.Parse(season.TemperatureInfo);
            foreach (XElement element in definition.Elements("Value"))
            {
    ...
    private RegionalClimate(RegionalClimate rhs, string name)
    {
        ...
        foreach (KeyValuePair<(ISeason Season, int DailyHour), double> item in rhs._hourlyBaseTemperaturesBySeason)
        {
    ...
    private void DoDatabaseInsert()
    {
        using (new FMDB())
        {
            Models.RegionalClimate dbitem = new()
            {
                Name = Name,
    ...
    private bool BuildingCommandTemperatureFluctuation(ICharacter actor, StringStack command)
    {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(command.SafeRemainingArgument, UnitType.Temperature, actor, out double value) || value < 0.0)
        {
    ...
    private bool BuildingCommandTemperatures(ICharacter actor, StringStack command)
    {
        ...
        ISeason season = _seasons.GetByIdOrName(command.PopSpeech());
        if (season is null)
        {
        ...
        {
            if (!Gameworld.UnitManager.TryGetBaseUnits(command.PopParentheses(), UnitType.Temperature, actor, out double value))
            {
        ...
        List<int> hours = _hourlyBaseTemperaturesBySeason
    ...
    private bool BuildingCommandTemperature(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int hour) || hour < 0)
        {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(command.SafeRemainingArgument, UnitType.Temperature, actor, out double value))
        {
    ...
    private bool BuildingCommandSeason(ICharacter actor, StringStack command)
    {
        ...
        if (_seasons.Contains(season))
        {
            ...
            foreach (KeyValuePair<(ISeason Season, int DailyHour), double> item in _hourlyBaseTemperaturesBySeason.Where(x => x.Key.Season == season).AsEnumerable())
            {
        ...
        {
            _seasons.Add(season);
            IClock clock = Gameworld.Clocks.First();
            for (int i = 0; i < clock.HoursPerDay; i++)
            {
    ...
    private bool BuildingCommandClimateModel(ICharacter actor, StringStack command)
    {
        ...
        IClimateModel model = Gameworld.ClimateModels.GetByIdOrName(command.SafeRemainingArgument);
        if (model is null)
        {
    ...
    public string Show(ICharacter voyeur)
    {
        ...
        sb.AppendLine();
        foreach (BoundRange<ISeason> range in SeasonRotation.Ranges)
        {
        ...
        sb.AppendLine();
        List<int> hours = _hourlyBaseTemperaturesBySeason.Select(x => x.Key.DailyHour).Distinct().OrderBy(x => x).ToList();
        sb.AppendLine(StringUtilities.GetTextTable(
    ...
    {
        Models.RegionalClimate dbitem = FMDB.Context.RegionalClimates.Find(Id);
        dbitem.Name = Name;
