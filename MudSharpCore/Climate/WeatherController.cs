...
public class WeatherController : SaveableItem, IWeatherController
{
    ...
    public WeatherController(IFuturemud gameworld, string name, IRegionalClimate climate, IZone zone)
    {
        ...
        using (new FMDB())
        {
            Models.WeatherController dbitem = new()
            {
                Name = Name,
    ...
    public void SetWeather(IWeatherEvent newEvent)
    {
        ...
        ConsecutiveUnchangedPeriods = 0;
        Changed = true;
        string echo = newEvent.DescribeTransitionTo(oldEvent)?.SubstituteANSIColour();
    ...
    public void HandleWeatherTick()
    {
        ...
        if (++MinuteCounter >= RegionalClimate.ClimateModel.MinuteProcessingInterval)
        {
            ...
            else
            {
                ...
                {
                    weatherChanged = true;
                    string echo = weather.DescribeTransitionTo(CurrentWeatherEvent)?.SubstituteANSIColour();
                    IWeatherEvent oldWeather = CurrentWeatherEvent;
        ...
        else
        {
            ...
            {
                string echo = CurrentWeatherEvent.RandomFlavourEcho();
                if (!string.IsNullOrEmpty(echo))
                {
                    WeatherEcho?.Invoke(this, echo);
                }
    ...
    private void UpdateCurrentSeason()
    {
        ...
        double seasonYearLength = RegionalClimate.SeasonRotation.Ceiling > 0.0
            ? RegionalClimate.SeasonRotation.Ceiling
            : Celestial?.CelestialDaysPerYear ?? 365.0;
        double celestialDay = Celestial?.CurrentCelestialDay ?? 0.0;
    ...
    {
        Models.WeatherController dbitem = FMDB.Context.WeatherControllers.Find(Id);
    ...
    private bool BuildingCommandRadius(ICharacter actor, StringStack command)
    {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(command.SafeRemainingArgument, UnitType.Length, actor, out double value))
        {
            actor.OutputHandler.Send($"The text {command.SafeRemainingArgument.ColourCommand()} is not a valid length measurement.");
            return false;
        }

        double valueAsMetres = Gameworld.UnitManager.BaseHeightToMetres * value;
    ...
    private bool BuildingCommandCelestial(ICharacter actor, StringStack command)
    {
        ...
        ICelestialObject celestial = Gameworld.CelestialObjects.GetByIdOrName(command.SafeRemainingArgument);
    ...
    private bool BuildingCommandLatitude(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.SafeRemainingArgument, out double value) || value >= 90 || value <= -90)
        {
    ...
    private bool BuildingCommandLongitude(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.SafeRemainingArgument, out double value) || value >= 180 || value <= -180)
        {
    ...
    private bool BuildingCommandTimezone(ICharacter actor, StringStack command)
    {
        ...
        IMudTimeZone timezone = FeedClock.Timezones.GetByIdOrNames(command.SafeRemainingArgument);
        if (timezone is null)
        {
    ...
    private bool BuildingCommandClock(ICharacter actor, StringStack command)
    {
        ...
        IClock clock = Gameworld.Clocks.GetByIdOrName(command.SafeRemainingArgument);
        if (clock is null)
        {
