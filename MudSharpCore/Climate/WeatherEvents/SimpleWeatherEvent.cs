...
public class SimpleWeatherEvent : WeatherEventBase
{
    public SimpleWeatherEvent(Models.WeatherEvent weather, IFuturemud gameworld) : base(weather, gameworld)
    {
        ...
        XElement transition = definition.Element("Transitions");
        _defaultTransitionEcho = transition.Element("Default").Value;
        foreach (XElement element in transition.Elements("Transition"))
        {
    ...
    private SimpleWeatherEvent(SimpleWeatherEvent rhs, string name) : base(rhs.Gameworld, name)
    {
        ...
        foreach (KeyValuePair<long, string> item in rhs._transitionEchoOverrides)
        {
    ...
    private bool BuildingCommandEcho(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double chance) || chance <= 0.0)
        {
        ...
        string echo = command.SafeRemainingArgument.ProperSentences();
        _randomEchoes.Add((echo, chance));
        double total = _randomEchoes.Sum(x => x.Chance);
    ...
    private bool BuildingCommandRemoveEcho(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int value) || value < 1 || value > _randomEchoes.Count)
        {
        ...
        (string Echo, double Chance) echo = _randomEchoes[value - 1];
    ...
    public override string Show(ICharacter actor)
    {
        ...
        {
            foreach (KeyValuePair<long, string> item in _transitionEchoOverrides)
            {
                IWeatherEvent we = Gameworld.WeatherEvents.Get(item.Key);
                if (we is null)
                {
                    continue;
                }
        ...
        if (total > 0.0)
        {
            foreach ((string Echo, double Chance) echo in _randomEchoes)
            {
    ...
    {
        WeatherEvent dbitem = FMDB.Context.WeatherEvents.Find(Id);
        dbitem.Name = Name;
