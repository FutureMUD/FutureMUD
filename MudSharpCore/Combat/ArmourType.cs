...
public class ArmourType : SaveableItem, IArmourType
{
    public ArmourType(IFuturemud gameworld, string name)
    {
        ...
        foreach (DamageType dt in Enum.GetValues(typeof(DamageType)).OfType<DamageType>().ToList())
        {
    ...
    public ArmourType(MudSharp.Models.ArmourType type, IFuturemud gameworld)
    {
        ...
        XElement definition = XElement.Parse(type.Definition);
        XElement element = definition.Element("DissipateExpressions");
        foreach (XElement item in element?.Elements("Expression") ?? Enumerable.Empty<XElement>())
        {
        ...
        if (element != null)
        {
            foreach (XElement item in element.Elements("Transform"))
            {
    ...
    private ArmourType(ArmourType rhs, string newName)
    {
        ...
        foreach (KeyValuePair<DamageType, Expression> item in rhs.AbsorbExpressions)
        {
            AbsorbExpressions[item.Key] = new Expression(item.Value.OriginalExpression);
        }
        foreach (KeyValuePair<DamageType, Expression> item in rhs.AbsorbExpressionsPain)
        {
            AbsorbExpressionsPain[item.Key] = new Expression(item.Value.OriginalExpression);
        }
        foreach (KeyValuePair<DamageType, Expression> item in rhs.AbsorbExpressionsStun)
        {
            AbsorbExpressionsStun[item.Key] = new Expression(item.Value.OriginalExpression);
        }
        foreach (KeyValuePair<DamageType, Expression> item in rhs.DissipateExpressions)
        {
            DissipateExpressions[item.Key] = new Expression(item.Value.OriginalExpression);
        }
        foreach (KeyValuePair<DamageType, Expression> item in rhs.DissipateExpressionsPain)
        {
            DissipateExpressionsPain[item.Key] = new Expression(item.Value.OriginalExpression);
        }
        foreach (KeyValuePair<DamageType, Expression> item in rhs.DissipateExpressionsStun)
        {
            DissipateExpressionsStun[item.Key] = new Expression(item.Value.OriginalExpression);
        }

        foreach (KeyValuePair<DamageType, (WoundSeverity Threshold, DamageType Transform)> item in rhs.DamageTypeTransformations)
        {
    ...
    {
        Models.ArmourType dbitem = FMDB.Context.ArmourTypes.Find(Id);
        dbitem.MinimumPenetrationDegree = (int)MinimumPenetrationDegree;
        dbitem.BaseDifficultyDegrees = BaseDifficultyBonus;
        dbitem.Name = Name;
        dbitem.StackedDifficultyDegrees = StackedDifficultyBonus;
        ...
        ref List<IWound> wounds)
    {
        ...
        DamageType newDamageType = damage.DamageType;
        ...
        bool passive)
    {
        ...
        ISolid solid = armour.Parent.Material as ISolid;
        double strength = 0.0;
        switch (damage.DamageType)
        {
            case DamageType.Slashing:
        ...
        List<IWound> armourWounds = passive
    ...
    public string Show(ICharacter voyeur)
    {
        ...
        sb.AppendLine();
        foreach (KeyValuePair<DamageType, (WoundSeverity Threshold, DamageType Transform)> item in DamageTypeTransformations)
        {
            sb.AppendLine(
	...
	#3strength#0 - either the shear or tensile strength of the material depending on the damage type, in pascals"
        ...
        sb.AppendLine();
        foreach (KeyValuePair<DamageType, Expression> item in DissipateExpressions.OrderBy(x => x.Key))
        {
        ...
        sb.AppendLine();
        foreach (KeyValuePair<DamageType, Expression> item in AbsorbExpressions.OrderBy(x => x.Key))
        {
            sb.AppendLine($"\t{item.Key.Describe().ColourValue()}: {item.Value.OriginalExpression.ColourCommand()}");
        }
        ...
        sb.AppendLine();
        foreach (KeyValuePair<DamageType, Expression> item in DissipateExpressionsStun.OrderBy(x => x.Key))
        {
            sb.AppendLine($"\t{item.Key.Describe().ColourValue()}: {item.Value.OriginalExpression.ColourCommand()}");
        }
        ...
        sb.AppendLine();
        foreach (KeyValuePair<DamageType, Expression> item in AbsorbExpressionsStun.OrderBy(x => x.Key))
        {
            sb.AppendLine($"\t{item.Key.Describe().ColourValue()}: {item.Value.OriginalExpression.ColourCommand()}");
        }
        ...
        sb.AppendLine();
        foreach (KeyValuePair<DamageType, Expression> item in DissipateExpressionsPain.OrderBy(x => x.Key))
        {
            sb.AppendLine($"\t{item.Key.Describe().ColourValue()}: {item.Value.OriginalExpression.ColourCommand()}");
        }
        ...
        sb.AppendLine();
        foreach (KeyValuePair<DamageType, Expression> item in AbsorbExpressionsPain.OrderBy(x => x.Key))
        {
            sb.AppendLine($"\t{item.Key.Describe().ColourValue()}: {item.Value.OriginalExpression.ColourCommand()}");
        }
...
Additionally, absorb formulas can use the following parameter:
    ...
    private bool BuildingCommandTransform(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.PopSpeech().TryParseEnum<DamageType>(out DamageType fromDamage))
        {
        ...
        if (!ss.PopSpeech().TryParseEnum<DamageType>(out DamageType toDamage))
        {
            actor.OutputHandler.Send($"The text {ss.Last.ColourCommand()} is not a valid damage type.");
            return false;
        }
        ...
        if (!ss.SafeRemainingArgument.TryParseEnum<WoundSeverity>(out WoundSeverity severity))
        {
...
You can use the following parameters in this formula:
	...
	#6organic#0 - 1 if armour material is organic, 0 if not".SubstituteANSIColour());
        ...
        Expression expression = new(ss.SafeRemainingArgument);
    ...
    private bool BuildingCommandStackedDifficulty(ICharacter actor, StringStack ss)
    {
        ...
        if (!double.TryParse(ss.SafeRemainingArgument, out double value))
        {
