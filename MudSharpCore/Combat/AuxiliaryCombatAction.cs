...
internal class AuxiliaryCombatAction : CombatAction, IAuxiliaryCombatAction
{
    ...
    {
        Type iType = typeof(IAuxiliaryEffect);
        foreach (
            Type type in
        ...
        {
            MethodInfo method = type.GetMethod("RegisterTypeHelp", BindingFlags.Static | BindingFlags.Public);
    ...
    protected AuxiliaryCombatAction(AuxiliaryCombatAction rhs)
    {
        ...
        using (new FMDB())
        {
            Models.CombatAction dbitem = new();
    ...
    private void LoadFromDatabase(Models.CombatAction dbitem)
    {
        ...
        XElement root = XElement.Parse(dbitem.AdditionalInfo);
        foreach (XElement effect in root.Elements("Effect"))
        {
    ...
    {
        Models.CombatAction dbitem = FMDB.Context.CombatActions.Find(Id);
...
#nullable enable
    ...
    {
        Func<AuxiliaryCombatAction, ICharacter, StringStack, IAuxiliaryEffect?>? parser = _builderParsers[ss.PopForSwitch()];
        if (parser is null)
        {
    ...
    protected void SaveMoveSpecificData(Models.CombatAction dbitem)
    {
        ...
        foreach (IAuxiliaryEffect effect in _auxiliaryEffects)
        {
    ...
    private bool BuildingCommandSet(ICharacter actor, StringStack command)
    {
        ...
        IAuxiliaryEffect effect = _auxiliaryEffects.ElementAt(value - 1);
    ...
    private bool BuildingCommandTypeHelp(ICharacter actor, StringStack command)
    {
        ...
        string? type = _builderTypeHelps[command.SafeRemainingArgument.CollapseString()];
        if (type is null)
        {
            actor.OutputHandler.Send($"There is no such auxiliary effect type. The valid types are {_builderTypeNames.Select(x => x.ColourValue()).ListToString()}.");
            return false;
    ...
    private bool BuildingCommandAddEffect(ICharacter actor, StringStack command)
    {
        ...
        IAuxiliaryEffect? type = CreateEffectFromBuilderInput(this, actor, command);
        if (type is null)
        {
            return false;
        }
