...
internal class DefenderAdvantage : IAuxiliaryEffect
{
    ...
    {
        AuxiliaryCombatAction.RegisterBuilderParser("defenderadvantage", (_, actor, ss) =>
        {
            IFuturemud gameworld = actor.Gameworld;
            if (ss.IsFinished)
            {
            ...
            ITraitDefinition? trait = gameworld.Traits.GetByIdOrName(ss.PopSpeech());
            if (trait is null)
            {
            ...
            if (!ss.PopSpeech().TryParseEnum<Difficulty>(out Difficulty difficulty))
            {
            ...
            if (ss.IsFinished)
            {
                actor.OutputHandler.Send($"What amount of offensive bonus/penalty do you want to apply to the defender per degree of failure on defense? Each {gameworld.GetStaticDouble("CheckBonusPerDifficultyLevel").ToString("N2", actor).ColourValue()} bonus represents one degree of difficulty.");
                return null;
            }

            if (!double.TryParse(ss.PopSpeech(), out double offense))
            {
            ...
            if (ss.IsFinished)
            {
                actor.OutputHandler.Send($"What amount of defense bonus/penalty do you want to apply to the defender per degree of failure on defense? Each {gameworld.GetStaticDouble("CheckBonusPerDifficultyLevel").ToString("N2", actor).ColourValue()} bonus represents one degree of difficulty.");
                return null;
            }

            if (!double.TryParse(ss.PopSpeech(), out double defense))
            {
                actor.OutputHandler.Send($"The value {ss.Last.ColourCommand()} is not a valid number.");
                return null;
            }
            ...
            $@"The Defender Advantage effect type applies a bonus or penalty to the person using the move based on how successful an opposed check is.

The syntax to create this type is as follows:

	#3auxiliary set add defenderadvantage <defense trait> <difficulty> <offense bonus> <defense bonus>#0
...
{BuildingHelpText}",
    ...
    private bool BuildingCommandAllowPositives(ICharacter actor)
    {
        ...
        actor.OutputHandler.Send($"This effect will {AllowPositives.NowNoLonger()} apply positive modifiers when the attacker is unsuccessful.");
        return true;
    }
    ...
    private bool BuildingCommandAllowNegatives(ICharacter actor)
    {
        ...
        actor.OutputHandler.Send($"This effect will {AllowNegatives.NowNoLonger()} apply negative modifiers when the attacker is successful.");
        return true;
    }
    ...
    private bool BuildingCommandBonusOffense(ICharacter actor, StringStack command)
    {
        ...
        sb.AppendLine($"\t{OpposedOutcomeDegree.Total.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? OffenseBonusPerDegree * -5.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Major.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? OffenseBonusPerDegree * -4.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Moderate.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? OffenseBonusPerDegree * -3.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Minor.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? OffenseBonusPerDegree * -2.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Marginal.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? OffenseBonusPerDegree * -1.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{"Tie".Colour(Telnet.Yellow)}: {Difficulty.Normal.DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Marginal.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? OffenseBonusPerDegree * 1.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Minor.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? OffenseBonusPerDegree * 2.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Moderate.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? OffenseBonusPerDegree * 3.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Major.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? OffenseBonusPerDegree * 4.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Total.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? OffenseBonusPerDegree * 5.0 : 0.0).DescribeColoured()}");
    ...
    private bool BuildingCommandBonusDefense(ICharacter actor, StringStack command)
    {
        ...
        sb.AppendLine($"\t{OpposedOutcomeDegree.Total.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? DefenseBonusPerDegree * -5.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Major.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? DefenseBonusPerDegree * -4.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Moderate.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? DefenseBonusPerDegree * -3.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Minor.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? DefenseBonusPerDegree * -2.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Marginal.DescribeColour()} {"Opponent".Colour(Telnet.Red)}: {Difficulty.Normal.ApplyBonus(AllowPositives ? DefenseBonusPerDegree * -1.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{"Tie".Colour(Telnet.Yellow)}: {Difficulty.Normal.DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Marginal.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? DefenseBonusPerDegree * 1.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Minor.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? DefenseBonusPerDegree * 2.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Moderate.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? DefenseBonusPerDegree * 3.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Major.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? DefenseBonusPerDegree * 4.0 : 0.0).DescribeColoured()}");
        sb.AppendLine($"\t{OpposedOutcomeDegree.Total.DescribeColour()} {"Attacker".Colour(Telnet.Green)}: {Difficulty.Normal.ApplyBonus(AllowNegatives ? DefenseBonusPerDegree * 5.0 : 0.0).DescribeColoured()}");
        actor.OutputHandler.Send(sb.ToString());
        return true;
    }
