...
public abstract class CombatAction : SaveableItem
{
    ...
    private bool BuildingCommandPosition(ICharacter actor, StringStack command)
    {
        ...
        IPositionState state = PositionState.GetState(command.SafeRemainingArgument);
        if (state == null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandDelay(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.SafeRemainingArgument, out double delay))
        {
    ...
    private bool BuildingCommandIntention(ICharacter actor, StringStack command)
    {
        ...
        CombatMoveIntentions intentions = Intentions;
        while (!command.IsFinished)
        {
            if (!CombatExtensions.TryParseCombatMoveIntention(command.PopSpeech(), out CombatMoveIntentions value))
            {
    ...
    private bool BuildingCommandRecover(ICharacter actor, StringStack command)
    {
        ...
        if (!CheckExtensions.GetDifficulty(command.PopSpeech(), out Difficulty success))
        {
            actor.OutputHandler.Send(
        ...
        if (!CheckExtensions.GetDifficulty(command.PopSpeech(), out Difficulty failure))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandExertion(ICharacter actor, StringStack command)
    {
        ...
        if (!Enum.TryParse<ExertionLevel>(command.PopSpeech(), true, out ExertionLevel result))
        {
            actor.OutputHandler.Send(
