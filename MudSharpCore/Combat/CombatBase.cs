...
public abstract class CombatBase : ICombat
{
    ...
    public static void SetupCombat(IFuturemud gameworld)
    {
        using (new FMDB())
        {
            string expression = gameworld.GetStaticConfiguration("RecoveryTimeExpressionId");
    ...
    public void MergeCombat(ICombat oldCombat)
    {
        ...
        foreach (IPerceiver person in oldCombat.Combatants)
        {
            JoinCombat(person);
        }
    }
    ...
    public virtual string DescribeFor(ICharacter voyeur)
    {
        ...
        if (Combatants.Contains(voyeur))
        {
            ...
            ICombatStrategy strategy = CombatStrategyFactory.GetStrategy(voyeur.CombatStrategyMode);
            string why = strategy.WhyWontAttack(voyeur);
            if (!string.IsNullOrEmpty(why))
            {
                sb.AppendLine(why);
            }

            string whyTarget = voyeur.CombatTarget is ICharacter tch ? strategy.WhyWontAttack(voyeur, tch) : string.Empty;
        ...
        sb.AppendLine();
        foreach (IPerceiver combatant in Combatants)
        {
    ...
    {
        foreach (IPerceiver combatant in Combatants.ToList())
        {
...
#if DEBUG
            Console.WriteLine(
                $"Character {character.CurrentName.GetName(NameStyle.SimpleFull)} used {move.StaminaCost:N2} stamina on {move.Description}.");
#endif
...
#if DEBUG
            Console.WriteLine(
                $"Character {tcharacter.CurrentName.GetName(NameStyle.SimpleFull)} used {targetResponse.StaminaCost:N2} stamina on {targetResponse.Description}.");
#endif
    ...
    public virtual void CombatAction(IPerceiver perceiver)
    {
        ...
        ICombatMove move = perceiver.ChooseMove();
        CombatAction(perceiver, move);
        if (move != null)
        {
            CheckCombatEndingConditions(move.Assailant);
            foreach (ICharacter target in move.CharacterTargets)
            {
                if (target == null)
                {
                    continue;
    ...
    {
        List<IPerceiver> results = new();
        results.Add(combatant);
        while (true)
        {
            bool found = false;
            foreach (IPerceiver other in Combatants)
            {
