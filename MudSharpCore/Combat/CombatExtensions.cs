...
public static class CombatExtensions
{
    ...
    public static double ApplyMovementSpeedCheck(this ICharacter character, double baseSpeed, bool isFleeingRole)
    {
        ...
        ICharacter mover = character.GetCombatMover();
        CheckType checkType = DetermineMovementCheckType(isFleeingRole, mover != character);
        if (checkType == CheckType.None)
        {
            return baseSpeed;
        }

        ICheck check = character.Gameworld.GetCheck(checkType);
        if (check.Type == CheckType.None)
        {
            return baseSpeed;
        }

        CheckOutcome outcome = check.Check(character, Difficulty.Normal);
        int degrees = outcome.CheckDegrees();
    ...
    public static bool TryParseDefenseType(string text, out DefenseType type)
    {
        ...
        List<DefenseType> values = Enum.GetValues(typeof(DefenseType)).OfType<DefenseType>().ToList();
    ...
    public static bool TryParseCombatMoveIntention(string text, out CombatMoveIntentions intention)
    {
        ...
        List<CombatMoveIntentions> values = Enum.GetValues(typeof(CombatMoveIntentions)).OfType<CombatMoveIntentions>().ToList();
        if (values.Any(x => x.Describe().StartsWith(text, StringComparison.InvariantCultureIgnoreCase)))
        {
            intention =
                values.FirstOrDefault(
    ...
    public static bool TryParseCombatStrategyMode(string text, out CombatStrategyMode intention)
    {
        ...
        List<CombatStrategyMode> values = Enum.GetValues(typeof(CombatStrategyMode)).OfType<CombatStrategyMode>().ToList();
        if (values.Any(x => x.Describe().StartsWith(text, StringComparison.InvariantCultureIgnoreCase)))
        {
            intention =
                values.FirstOrDefault(
    ...
    public static bool TryParseBuiltInMoveType(string text, out BuiltInCombatMoveType move)
    {
        ...
        List<BuiltInCombatMoveType> values = Enum.GetValues(typeof(BuiltInCombatMoveType)).OfType<BuiltInCombatMoveType>().ToList();
        if (values.Any(x => x.Describe().StartsWith(text, StringComparison.InvariantCultureIgnoreCase)))
        {
            move =
                values.FirstOrDefault(
    ...
    public static bool IsMagicAttack(this BuiltInCombatMoveType type)
    {
        switch (type)
        {
