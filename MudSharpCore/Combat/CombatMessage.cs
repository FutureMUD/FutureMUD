...
public class CombatMessage : SaveableItem, ICombatMessage
{
    public CombatMessage(IFuturemud gameworld)
    {
        ...
        using (new FMDB())
        {
            Models.CombatMessage dbitem = new()
            {
                Priority = 0,
    ...
    private void LoadFromDatabase(MudSharp.Models.CombatMessage message)
    {
        ...
        foreach (long attack in message.CombatMessagesWeaponAttacks.Select(x => x.WeaponAttackId).ToList())
        {
            WeaponAttackIds.Add(attack);
        }

        foreach (long move in message.CombatMessagesCombatActions.Select(x => x.CombatActionId).ToList())
        {
    ...
    public void Delete()
    {
        ...
        if (_id != 0)
        {
            using (new FMDB())
            {
                ...
                Models.CombatMessage dbitem = FMDB.Context.CombatMessages.Find(Id);
                if (dbitem != null)
                {
    ...
    public override void Save()
    {
        ...
        foreach (long item in WeaponAttackIds)
        {
            dbitem.CombatMessagesWeaponAttacks.Add(new CombatMessagesWeaponAttacks
            { CombatMessage = dbitem, WeaponAttackId = item });
        }
        foreach (long item in AuxiliaryActionIds)
        {
            dbitem.CombatMessagesCombatActions.Add(new CombatMessagesCombatActions
            {
                CombatMessage = dbitem,
    ...
    public string ShowBuilder(ICharacter actor)
    {
        ...
        if (WeaponAttackIds.Any())
        {
            ...
            foreach (IWeaponAttack attack in WeaponAttackIds.SelectNotNull(x => Gameworld.WeaponAttacks.Get(x)))
            {
                IWeaponType weapon = Gameworld.WeaponTypes.FirstOrDefault(x => x.Attacks.Contains(attack));
        ...
        if (AuxiliaryActionIds.Any())
        {
            ...
            foreach (IAuxiliaryCombatAction action in AuxiliaryActionIds.SelectNotNull(x => Gameworld.AuxiliaryCombatActions.Get(x)))
            {
    ...
    {
        if (command.IsFinished || !command.SafeRemainingArgument.TryParsePercentage(actor.Account.Culture, out double value) || value < 0 || value > 1)
        {
    ...
    private bool BuildingCommandType(ICharacter actor, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<BuiltInCombatMoveType>(command.PopSpeech(), out BuiltInCombatMoveType value))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandOutcome(ICharacter actor, StringStack command)
    {
        ...
        if (!CheckExtensions.GetOutcome(command.PopSpeech(), out Outcome outcome))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandMessage(ICharacter actor, StringStack command)
    {
        ...
        string emote = command.SafeRemainingArgument
                           .Replace("\\n", "\n")
                           .Replace("\\b", "\b");
        (string help, bool valid) = VetEmote(Type, emote);
        if (!valid)
        {
    ...
    private bool BuildingCommandProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = long.TryParse(command.PopSafe(), out long value)
    ...
    private bool BuildingCommandAttack(ICharacter actor, StringStack command)
    {
        ...
        IWeaponAttack attack = actor.Gameworld.WeaponAttacks.Get(value);
        if (attack == null)
        {
    ...
    private bool BuildingCommandMove(ICharacter actor, StringStack command)
    {
        ...
        IAuxiliaryCombatAction move = actor.Gameworld.AuxiliaryCombatActions.Get(value);
        if (move == null)
        {
