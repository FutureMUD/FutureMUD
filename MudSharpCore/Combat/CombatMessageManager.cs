...
public class CombatMessageManager : ICombatMessageManager
{
    ...
    private void LoadCombatMessages(IFuturemud gameworld)
    {
        using (new FMDB())
        {
            List<Models.CombatMessage> messages = FMDB.Context.CombatMessages.ToList();
            foreach (Models.CombatMessage message in messages)
            {
        ...
        List<Tuple<BuiltInCombatMoveType, Outcome>> missingMessages = new();
        List<Outcome> outcomeValues =
            Enum.GetValues(typeof(Outcome))
                .Cast<Outcome>()
                .Where(x => x != Outcome.None && x != Outcome.NotTested)
                .ToList();
        foreach (BuiltInCombatMoveType typeValue in Enum.GetValues(typeof(BuiltInCombatMoveType)).Cast<BuiltInCombatMoveType>())
        {
        ...
        if (missingMessages.Any())
        {
            ...
            foreach (Tuple<BuiltInCombatMoveType, Outcome> message in missingMessages)
            {
