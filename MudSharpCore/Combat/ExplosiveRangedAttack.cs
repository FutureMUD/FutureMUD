...
public class ExplosiveRangedAttack : RangedNaturalAttackBase, IExplosiveRangedAttack
{
	...
	{
		ExplosionSize = Enum.TryParse<SizeCategory>(root.Element("ExplosionSize")?.Value ?? "", true, out var size)
			? size
			: SizeCategory.Normal;
		MaximumProximity = Enum.TryParse<Proximity>(root.Element("MaximumProximity")?.Value ?? "", true, out var proximity)
			? proximity
			: Proximity.Proximate;
	}
	...
	{
		if (!command.SafeRemainingArgument.TryParseEnum<SizeCategory>(out var value))
		{
