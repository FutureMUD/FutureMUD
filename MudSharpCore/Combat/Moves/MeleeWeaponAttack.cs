...
public class MeleeWeaponAttack : WeaponAttackMove
{
    ...
    public override CombatMoveResult ResolveMove(ICombatMove defenderMove)
    {
        ...
        WardDefenseMove ward = defenderMove as WardDefenseMove;
    ...
    {
        double blockBonus = BlockMove.GetBlockBonus(this, defenderMove.Alignment, defenderMove.Shield);
        double targetBonus = Assailant.GetBonusForDefendersFromTargeting();
        Difficulty defenderDifficulty = Attack.Profile.BaseBlockDifficulty.StageUp(defenderMove.DifficultStageUps);
        Dictionary<Difficulty, CheckOutcome> blockCheck = Gameworld.GetCheck(defenderMove.Check)
        ...
        int bsuccess = blockCheck[defenderDifficulty].SuccessDegrees();
        if (bsuccess > 0)
        {
            double adv = bsuccess switch
            {
        ...
        OpposedOutcome result = new(attackRoll, blockCheck, CheckDifficulty, defenderDifficulty);
#if DEBUG
        Console.WriteLine(
        ...
        if (result.Outcome == OpposedOutcomeDirection.Proponent)
        {
            ...
            string blockEmote = Gameworld.CombatMessageManager.GetFailMessageFor(defenderMove.Assailant, Assailant,
        ...
        {
            string blockEmote = Gameworld.CombatMessageManager.GetMessageFor(defenderMove.Assailant, Assailant,
                defenderMove.Shield.Parent, Attack, defenderMove.MoveType, blockCheck[defenderDifficulty], null);
            Assailant.OutputHandler.Handle(
                new EmoteOutput(
                    new Emote(
        ...
        Difficulty recovery = attackRoll[CheckDifficulty].IsPass()
    ...
    {
        double targetBonus = Assailant.GetBonusForDefendersFromTargeting();
        Difficulty defenderDifficulty = Attack.Profile.BaseParryDifficulty.StageUp(parry.DifficultStageUps);
        Dictionary<Difficulty, CheckOutcome> parryCheck = Gameworld.GetCheck(defenderMove.Check)
                                  .CheckAgainstAllDifficulties(defenderMove.Assailant, defenderDifficulty,
        ...
        double parryDelay = Gameworld.GetStaticDouble("ParryDelaySeconds") *
        ...
        int advantageLoss = parryCheck[defenderDifficulty].CheckDegrees() -
        ...
        OpposedOutcome result = new(attackRoll, parryCheck, CheckDifficulty, defenderDifficulty);
#if DEBUG
        Console.WriteLine(
        ...
        {
            string parryEmote = Gameworld.CombatMessageManager.GetMessageFor(defenderMove.Assailant, Assailant,
                parry.Weapon.Parent, Attack, parry.MoveType, parryCheck[defenderDifficulty], null);
            Assailant.OutputHandler.Handle(
                new EmoteOutput(
            ...
            if (!parry.Weapon.Parent.Destroyed && result.Degree == OpposedOutcomeDegree.Total)
            {
                ...
                {
                    CheckOutcome secondCheck = Gameworld.GetCheck(defenderMove.Check)
    ...
    {
        bool echoSlipped = false;
        double targetBonus = Assailant.GetBonusForDefendersFromTargeting();
        Difficulty defenderDifficulty = Attack.Profile.BaseDodgeDifficulty.StageUp(dodge.DifficultStageUps);
        ...
        double dodgeDelay = Gameworld.GetStaticDouble("DodgeDelaySeconds") *
                           (1.0 - dodgeCheck[defenderDifficulty].CheckDegrees() / 6.0);
        Gameworld.Scheduler.DelayScheduleType(defenderMove.Assailant, ScheduleType.Combat,
                TimeSpan.FromSeconds(dodgeDelay));
        int dsuccess = dodgeCheck[defenderDifficulty].SuccessDegrees();
        if (dsuccess > 0)
        {
            double adv = dsuccess switch
            {
        ...
        if (dodgeCheck[defenderDifficulty].Outcome == Outcome.MajorFail &&
            RandomUtilities.DoubleRandom(0.0, 1.0) < Gameworld.GetStaticDouble("DodgeMajorFailFallChance") &&
            defenderMove.Assailant.PositionState.Upright)
        {
            echoSlipped = true;
            defenderMove.Assailant.DoCombatKnockdown();
        }
        OpposedOutcome result = new(attackRoll, dodgeCheck, CheckDifficulty, defenderDifficulty);
#if DEBUG
        Console.WriteLine(
            ...
            result.Outcome == OpposedOutcomeDirection.Stalemate)
        {
            ...
            double finalAngle = Attack.Profile.BaseAngleOfIncidence * angleMultiplier;
            (IDamage damage, IDamage selfDamage) = GetDamagePlusSelfDamageForWeapon(Weapon, attackRoll[CheckDifficulty],
                result.Degree, defenderHaveWounds, TargetBodypart, finalAngle);

            string dodgeEmote = Gameworld.CombatMessageManager.GetFailMessageFor(defenderMove.Assailant, Assailant,
        ...
        {
            string dodgeEmote = Gameworld.CombatMessageManager.GetMessageFor(defenderMove.Assailant, Assailant,
                Weapon.Parent, Attack, dodge.MoveType, dodgeCheck[defenderDifficulty], null);
            Assailant.OutputHandler.Handle(
                new EmoteOutput(
            ...
            if (Assailant.State != CharacterState.Dead && result.Degree == OpposedOutcomeDegree.Total)
            {
                ...
                {
                    Facing previousFacing = defenderMove.Assailant.GetFacingFor(Assailant);
                    ImproveCombatPosition(defenderMove.Assailant, Assailant);
                    Facing newFacing = defenderMove.Assailant.GetFacingFor(Assailant);
    ...
    {
        WorsenCombatPosition(defenderMove.Assailant, Assailant);
        OpposedOutcome result = new(attackRoll[CheckDifficulty], Outcome.NotTested);
#if DEBUG
        Console.WriteLine(
            $"MeleeWeaponAttack HelplessDefenseMove Outcome: {result.Degree.Describe()} to {result.Outcome.Describe()}");
#endif
        (IDamage damage, IDamage selfDamage) = GetDamagePlusSelfDamageForWeapon(Weapon, attackRoll[CheckDifficulty], result.Degree,
