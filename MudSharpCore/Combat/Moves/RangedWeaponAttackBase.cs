...
public abstract class RangedWeaponAttackBase : CombatMoveBase, IRangedWeaponAttackMove
{
    ...
    public override CombatMoveResult ResolveMove(ICombatMove defenderMove)
    {
        ...
        {
            IGameItem targetItem = target as IGameItem;
            positionBonus += GetSizeDifficulty(targetItem.Size);
        }

        double noPressureBonus = 0.0;
        ...
        if (Weapon.ReadyToFire == false)
        {
            ...
            //skillups.
            CheckOutcome failOutcome = new() { Outcome = Outcome.MajorFail };
        ...
        ICharacter targetChar = (ICharacter)target;
        ...
        {
            CheckOutcome dodgeResult = Gameworld.GetCheck(dodge.Check).Check(targetChar, Weapon.BaseDodgeDifficulty, Assailant);
            OpposedOutcome opposed = new(results.Item1, dodgeResult);
        ...
        {
            CheckOutcome blockResult = Gameworld.GetCheck(block.Check).Check(targetChar, block.CheckDifficulty, Assailant);
