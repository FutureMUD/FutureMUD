...
public class StruggleMove : CombatMoveBase
{
    ...
    public override CombatMoveResult ResolveMove(ICombatMove defenderMove)
    {
        ...
        IDragging drag = Assailant.CombinedEffectsOfType<DragTarget>().FirstOrDefault()?.Drag;
        if (drag != null)
        {
    ...
    {
        Difficulty baseDifficulty = Difficulty.Normal;
        // TODO - effects
        // TODO - merits
    ...
    private CombatMoveResult StruggleFreeFromGrapple(IBeingGrappled grapple)
    {
        ...
        ICheck check = Assailant.Gameworld.GetCheck(CheckType.StruggleFreeFromGrapple);
        CheckOutcome result = check.Check(Assailant, grapple.StruggleDifficulty, grapple.Grappling.Owner);
        ICheck opponentCheck = Assailant.Gameworld.GetCheck(CheckType.OpposeStruggleFreeFromGrapple);
        CheckOutcome opponentResult = grapple.Grappling.CharacterOwner.CurrentStamina <= 0.0
        ...
        OpposedOutcome opposed = new(result, opponentResult);
    ...
    private CombatMoveResult StruggleFreeFromDrag(IDragging drag)
    {
        ...
        ICheck struggleCheck = Assailant.Gameworld.GetCheck(CheckType.StruggleFreeFromDrag);
        CheckOutcome result = struggleCheck.Check(Assailant, Difficulty.Normal);
        Outcome opponentResult = Outcome.MajorFail;
        ICheck opponentCheck = Assailant.Gameworld.GetCheck(CheckType.OpposeStruggleFreeFromDrag);
        foreach (Dragger dragger in drag.Draggers)
        {
        ...
        OpposedOutcome opposed = new(result.Outcome, opponentResult);
        if (opposed.Outcome == OpposedOutcomeDirection.Proponent)
        {
            Assailant.OutputHandler.Handle(new EmoteOutput(
                new Emote(
