...
public abstract class WeaponAttackMove : CombatMoveBase, IWeaponAttackMove
{
    ...
    {
        double damageTypeMultiplier = 0.0;
        switch (damageType)
        {
        ...
        double attackerAverageHardness = weapon.Parent.Material.Density;

        double defenderAverageHardness = target is ICharacter tch && targetBodypart != null
    ...
    {
        double attackerAverageHardness = Assailant.Body.WornItemsProfilesFor(attackingBodypart)
                                               .Where(x => !x.Item2.NoArmour && x.Item1.IsItemType<IArmour>())
                                               .SelectNotNull(x => x.Item1.Material as ISolid)
    ...
    {
        List<IWeaponAttack> weaponAttacks = wardResult.WardWeapon?.WeaponType.UsableAttacks(
        ...
        List<INaturalAttack> naturalAttacks = counterAttacker.Race.UsableNaturalWeaponAttacks(counterAttacker, target, false,
            BuiltInCombatMoveType.WardFreeUnarmedAttack).ToList();

        //Check attack preference flags
        List<IWeaponAttack> preferredWeapon = weaponAttacks
        ...
        List<INaturalAttack> preferredNatural = naturalAttacks
        ...
        IWeaponAttack wAttack = weaponAttacks?.GetWeightedRandom(x => x.Weighting);
        INaturalAttack nAttack = naturalAttacks.GetWeightedRandom(x => x.Attack.Weighting);
    ...
    {
        IBodypart targetBodypart = aggressor.Body.RandomBodyPartGeometry(weaponAttack.Orientation,
            weaponAttack.Alignment, Facing.Front);
        CheckOutcome attackRoll = Gameworld.GetCheck(CheckType.MeleeWeaponCheck)
                                  .Check(defender, weaponAttack.Profile.BaseAttackerDifficulty,
                                      wardResult.WardWeapon.WeaponType.AttackTrait, aggressor);
        CheckOutcome defenseRoll = Gameworld.GetCheck(CheckType.DodgeCheck)
                                   .Check(aggressor, weaponAttack.Profile.BaseDodgeDifficulty, defender);
        OpposedOutcome result = new(attackRoll, defenseRoll);
        string attackEmote = Gameworld.CombatMessageManager
...
#endif
        int finalDegree = result.Outcome == OpposedOutcomeDirection.Opponent ? 0 : (int)result.Degree;
    ...
    {
        IBodypart targetBodypart = aggressor.Body.RandomBodyPartGeometry(naturalAttack.Attack.Orientation,
            naturalAttack.Attack.Alignment, Facing.Front);
        CheckOutcome attackRoll = Gameworld.GetCheck(CheckType.NaturalWeaponAttack)
        ...
        string attackEmote = string.Format(Gameworld.CombatMessageManager
                                                 .GetMessageFor(defender, aggressor,
        ...
        if (result.Outcome == OpposedOutcomeDirection.Opponent)
        {
            ...
            string dodgeEmote = Gameworld.CombatMessageManager.GetMessageFor(
        ...
        {
            string dodgeEmote = Gameworld.CombatMessageManager.GetFailMessageFor(
    ...
    {
        Difficulty warderDifficulty = ward.GetWarderDifficulty(this);
        Dictionary<Difficulty, CheckOutcome> wardCheck = Gameworld.GetCheck(CheckType.Ward)
        ...
        string wardEmote = Gameworld.CombatMessageManager.GetMessageFor(ward.Assailant, Assailant,
            ward.WardWeapon?.Parent, Attack, BuiltInCombatMoveType.WardDefense, wardCheck[warderDifficulty], null);
        Difficulty wardeeDifficulty = ward.GetWardeeDifficulty(this);
        Dictionary<Difficulty, CheckOutcome> wardOpponentCheck = Gameworld.GetCheck(CheckType.WardDefense)
                                         .CheckAgainstAllDifficulties(Assailant, wardeeDifficulty, null,
                                             ward.Assailant);
        OpposedOutcome result = new(wardCheck, wardOpponentCheck, warderDifficulty, wardeeDifficulty);
        ...
        {
            CheckOutcome wardIgnoreCheck = Gameworld.GetCheck(CheckType.WardIgnore)
    ...
    public virtual void ResolveBloodSpray(CombatMoveResult result)
    {
        ...
        foreach (IWound curWound in result.WoundsCaused)
        {
            ...
            {
                IBody bloodSource = (curWound.Parent as ICharacter)?.Body;
    ...
    {
        ICharacter target = move.Assailant;
        if (target.Body == null)
        {
            return;
        }
        ...
        bool defending = !(move is TooExhaustedMove) &&
        ...
        {
            Facing facing = Assailant.GetFacingFor(target, true);
