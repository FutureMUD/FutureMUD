...
public class ProgCombat : CombatBase
{
    ...
    public override bool LeaveCombat(IPerceiver character)
    {
        ...
        foreach (IPerceiver combatant in Combatants.Where(x => x.CombatTarget == character).ToList())
        {
    ...
    {
        foreach (IPerceiver combatant in _combatants.ToList())
        {
            if (echo)
            {
    ...
    public override void TruceRequested(IPerceiver combatant)
    {
        ...
        else
        {
            ...
            {
                IPerceiver oldTarget = combatant.CombatTarget;
                IPerceiver nextTarget = Combatants.FirstOrDefault(x =>
            ...
            if (combatant.EffectsOfType<ICombatTruceEffect>().Any())
            {
                ...
                foreach (ICharacter ch in Combatants.OfType<ICharacter>().Where(x => x.CombatTarget == combatant).ToList())
                {
    ...
    public override string LDescAddendumFor(ICombatant combatant, IPerceiver voyeur)
    {
        ...
        IPerceivable perceivable = combatant as IPerceivable;
        string direction =
