...
For information on emote syntax see #3help emote#0.";
    ...
    private bool BuildingCommandType(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<CoverType>(out CoverType type))
        {
    ...
    private bool BuildingCommandExtent(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<CoverExtent>(out CoverExtent extent))
        {
    ...
    private bool BuildingCommandDesc(ICharacter actor, StringStack command)
    {
        ...
        NoFormatEmote emote = new(text, new DummyPerceiver(), null);
        if (!emote.Valid)
        {
    ...
    private bool BuildingCommandAction(ICharacter actor, StringStack command)
    {
        ...
        string text = command.SafeRemainingArgument;
        Emote emote = new(text, new DummyPerceiver(), null);
        if (!emote.Valid)
        {
            actor.OutputHandler.Send(emote.ErrorMessage);
            return false;
    ...
    {
        Models.RangedCover dbitem = FMDB.Context.RangedCovers.Find(Id);
        dbitem.Name = Name;
