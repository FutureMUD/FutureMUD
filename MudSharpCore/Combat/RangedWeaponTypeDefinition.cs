...
public class RangedWeaponTypeDefinition : SaveableItem, IRangedWeaponType
{
    ...
    private void DoDatabaseInsert()
    {
        using (new FMDB())
        {
            Models.RangedWeaponTypes dbitem = new();
            SaveToDBItem(dbitem);
    ...
    {
        Models.RangedWeaponTypes dbitem = FMDB.Context.RangedWeaponTypes.Find(Id);
        SaveToDBItem(dbitem);
        Changed = false;
    }
...
For the damage formula you can use the following parameters:
    ...
    private bool BuildingCommandDamageBonus(ICharacter actor, StringStack ss)
    {
        ...
        TraitExpression expression = new(ss.SafeRemainingArgument, Gameworld);
    ...
    private bool BuildingCommandLoadType(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.SafeRemainingArgument.TryParseEnum<AmmunitionLoadType>(out AmmunitionLoadType value))
        {
    ...
    private bool BuildingCommandAmmoType(ICharacter actor, StringStack ss)
    {
        ...
        string grade = ss.SafeRemainingArgument.ToLowerInvariant();
        SpecificAmmunitionGrade = ss.SafeRemainingArgument;
        Changed = true;
        List<IRangedWeaponType> sameTypes = Gameworld.RangedWeaponTypes.Where(x => x.RangedWeaponType == RangedWeaponType).ToList();
        bool warning = sameTypes.Any() && sameTypes.All(x => !x.SpecificAmmunitionGrade.EqualTo(grade)) && Gameworld.AmmunitionTypes.Where(x => x.RangedWeaponTypes.Contains(RangedWeaponType)).All(x => !x.SpecificType.EqualTo(grade));
    ...
    private bool BuildingCommandOperateTrait(ICharacter actor, StringStack ss)
    {
        ...
        ITraitDefinition trait = Gameworld.Traits.GetByIdOrName(ss.SafeRemainingArgument);
        if (trait is null)
        {
    ...
    private bool BuildingCommandAimLossPerShot(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.SafeRemainingArgument.TryParsePercentage(actor.Account.Culture, out double value))
        {
    ...
    private bool BuildingCommandAimDifficulty(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.SafeRemainingArgument.TryParseEnum<Difficulty>(out Difficulty value))
        {
    ...
    private bool BuildingCommandClass(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.SafeRemainingArgument.TryParseEnum<WeaponClassification>(out WeaponClassification value))
        {
    ...
    private bool BuildingCommandType(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.SafeRemainingArgument.TryParseEnum<RangedWeaponType>(out RangedWeaponType value))
        {
