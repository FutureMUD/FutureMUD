...
public class ClinchStrategy : StandardMeleeStrategy
{
    ...
    {
        List<IWeaponAttack> attacks = new();
        IMeleeWeapon weapon = null;
        List<IMeleeWeapon> weapons = new();
        bool hadPossibleAttacks = false;
        foreach (MeleeAttackOrderPreference preference in ch.CombatSettings.MeleeAttackOrderPreferences)
        {
            switch (preference)
            {
                case MeleeAttackOrderPreference.Weapon:
                    ...
                    if (!weapons.Any())
                    {
                        List<IMeleeWeapon> wieldableClinchItems = ch
                        ...
                        List<IMeleeWeapon> drawableClinchItems = ch.Body.ExternalItems
        ...
        List<IWeaponAttack> preferredAttacks = attacks.Where(x => x.Intentions.HasFlag(ch.CombatSettings.PreferredIntentions)).ToList();
    ...
    public bool HasViableClinchAttack(ICharacter ch)
    {
        ...
        IEnumerable<INaturalAttack> natAttacks = ch.Race.UsableNaturalWeaponAttacks(ch, ch.CombatTarget, false,
