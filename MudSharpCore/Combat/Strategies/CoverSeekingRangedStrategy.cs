...
public abstract class CoverSeekingRangedStrategy : RangeBaseStrategy
{
        ...
        IProvideCover coverItem)
    {
        ...
        {
            List<IRangedWeapon> weapons = GetReadyRangedWeapons(assailant).Concat(GetNotReadyButLoadableWeapons(assailant))
                                                          .ToList();
            if (weapons.Any())
            {
    ...
    protected CombatantCover ScoutCover(ICharacter ch)
    {
        ...
        List<(IRangedCover Cover, IProvideCover Provider, double Fitness)> fitness = new();
        fitness.AddRange(
        ...
        (IRangedCover Cover, IProvideCover Provider, double Fitness) random = fitness.Where(x => x.Fitness > 0.0).WhereMax(x => x.Fitness).GetRandomElement();
    ...
    protected override ICombatMove HandleCombatMovement(IPerceiver combatant)
    {
        ...
        if (combatant is ICharacter ch)
        {
            ...
            {
                CombatantCover cover = ScoutCover(ch);
                if (cover != null)
                {
