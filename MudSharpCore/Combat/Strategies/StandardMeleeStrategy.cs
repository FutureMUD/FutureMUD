...
public class StandardMeleeStrategy : StrategyBase
{
        ...
        IPerceiver assailant)
    {
        ...
        Difficulty finalBlockDifficulty = move.Attack.Profile.BaseBlockDifficulty.ApplyBonus(shieldBonus);
        Difficulty finalParryDifficulty =
            move.Attack.Profile.BaseParryDifficulty.ApplyBonus(parry?.WeaponType.ParryBonus ?? 0.0);
        Difficulty finalDodgeDifficulty = move.Attack.Profile.BaseDodgeDifficulty;
        ...
        double finalParryTargetNumber = defender.Gameworld.GetCheck(CheckType.ParryCheck).TargetNumber(defender, finalParryDifficulty, parry?.WeaponType.ParryTrait, assailant);
        ...
        bool canBlock = shield is not null && !clinching && move.Attack.Profile.BaseBlockDifficulty != Difficulty.Impossible && defender.CanSpendStamina(BlockMove.MoveStaminaCost(assailant, defender, shield));
        bool canParry = parry is not null && !defender.EffectsOfType<IWardBeatenEffect>().Any() && !clinching && move.Attack.Profile.BaseParryDifficulty != Difficulty.Impossible && defender.CanSpendStamina(ParryMove.MoveStaminaCost(assailant, defender, parry));
        bool canDodge = defender.RidingMount is null && move.Attack.Profile.BaseDodgeDifficulty != Difficulty.Impossible && defender.CanSpendStamina(DodgeMove.MoveStaminaCost(defender));
    ...
    protected ICombatMove CheckFixedAttacks(ICharacter ch)
    {
        ...
        List<IMeleeWeapon> weapons = ch.Body.WieldedItems.SelectNotNull(x => x.GetItemType<IMeleeWeapon>()).ToList();
        List<IImplantMeleeWeapon> implants = ch.Body.Implants
                         .OfType<IImplantMeleeWeapon>()
                         .Where(x => x.WeaponIsActive)
                         .ToList();
        List<IProstheticMeleeWeapon> prosthetics = ch.Body.Prosthetics
        ...
        {
            foreach (MeleeAttackOrderPreference preference in ch.CombatSettings.MeleeAttackOrderPreferences)
            {
                switch (preference)
                {
                    case MeleeAttackOrderPreference.Weapon:
                        foreach (IMeleeWeapon weapon in weapons)
                        {
                            CombatCoupDeGrace cdge = ch.EffectsOfType<CombatCoupDeGrace>().FirstOrDefault(x => x.Weapon == weapon);
                            if (cdge != null)
                            {
                    ...
                    case MeleeAttackOrderPreference.Implant:
                        foreach (IImplantMeleeWeapon implant in implants)
                        {
                            CombatCoupDeGrace cdge = ch.EffectsOfType<CombatCoupDeGrace>().FirstOrDefault(x => x.Weapon == implant);
                            if (cdge != null)
                            {
                                return new CoupDeGrace(cdge.Attack, (ICharacter)ch.CombatTarget)
                                {
                    ...
                    case MeleeAttackOrderPreference.Prosthetic:
                        foreach (IProstheticMeleeWeapon prosthetic in prosthetics)
                        {
                            CombatCoupDeGrace cdge = ch.EffectsOfType<CombatCoupDeGrace>()
        ...
        FixedCombatMoveType effect = ch.EffectsOfType<FixedCombatMoveType>().FirstOrDefault();
        if (effect == null)
        {
            return null;
        }

        List<Tuple<IMeleeWeapon, List<IWeaponAttack>>> weaponAttacks = weapons.Select(x => Tuple.Create(x,
                                       x.WeaponType
        ...
        List<Tuple<IImplantMeleeWeapon, List<IWeaponAttack>>> implantAttacks = implants.Select(x => Tuple.Create(x,
                                         x.WeaponType
                                          ...
                                          .ToList()))
                                     .Where(x => x.Item2.Any()).ToList();
        List<Tuple<IProstheticMeleeWeapon, List<IWeaponAttack>>> prostheticAttacks = prosthetics.Select(x => Tuple.Create(x,
                                               x.WeaponType
                                                .UsableAttacks(ch, x.Parent, ch.CombatTarget, x.HandednessForWeapon(ch),
                                                    false,
                                                    effect.FixedTypes.ToArray())
                                                ...
                                                .ToList()))
                                           .Where(x => x.Item2.Any()).ToList();
        List<INaturalAttack> unarmedAttacks = ch.Race.UsableNaturalWeaponAttacks(ch, ch.CombatTarget, false, effect.FixedTypes.ToArray())
                               .Where(x => ch.CanSpendStamina(NaturalAttackMove.MoveStaminaCost(ch, x.Attack)))
                               .ToList();
        ICharacter charTarget = ch.CombatTarget as ICharacter;
        IGameItem itemTarget = ch.CombatTarget as IGameItem;
            ...
            foreach (MeleeAttackOrderPreference preference in ch.CombatSettings.MeleeAttackOrderPreferences)
            {
                switch (preference)
                {
                    case MeleeAttackOrderPreference.Weapon:
                        ...
                        Tuple<IMeleeWeapon, List<IWeaponAttack>> attack = weaponAttacks.GetWeightedRandom(x => x.Item2.Sum(y => y.Weighting));
                        IMeleeWeapon attackWeapon = attack.Item1;
                        IWeaponAttack attackProfile = attack.Item2.GetWeightedRandom(x => x.Weighting);
                    ...
                    case MeleeAttackOrderPreference.Implant:
                        ...
                        Tuple<IImplantMeleeWeapon, List<IWeaponAttack>> implantAttack = implantAttacks.GetWeightedRandom(x => x.Item2.Sum(y => y.Weighting));
                    ...
                    case MeleeAttackOrderPreference.Prosthetic:
                        ...
                        Tuple<IProstheticMeleeWeapon, List<IWeaponAttack>> prostheticAttack = prostheticAttacks.GetWeightedRandom(x => x.Item2.Sum(y => y.Weighting));
        ...
        {
            INaturalAttack attack = unarmedAttacks.GetWeightedRandom(x => x.Attack.Weighting);
            if (!effect.Indefinite)
            {
                ch.RemoveEffect(effect);
            }
    ...
    {
        ICharacter tch = combatant.CombatTarget as ICharacter;
            ...
            combatant.Combat.Combatants.Except(tch).All(x => x.CombatTarget != combatant))
        {
            ...
            {
                List<IWeaponAttack> cdgeAttacks = weapon.WeaponType.UsableAttacks(combatant, weapon.Parent, combatant.CombatTarget,
        ...
        BuiltInCombatMoveType[] attackTypes = CombatExtensions.StandardMeleeWeaponAttacks;
        ...
        IEnumerable<IWeaponAttack> possibleAttacks =
    ...
    {
        List<(IMeleeWeapon Weapon, IWeaponAttack Attack, double Weight)> possibleWeaponsAndAttacks =
            new();
        List<(IMeleeWeapon Weapon, IWeaponAttack Attack, double Weight)> weaponsAndAttacks =
            new();
        int iterations = 0;
        foreach (MeleeAttackOrderPreference preference in combatant.CombatSettings.MeleeAttackOrderPreferences.AsEnumerable().Reverse())
        {
            ...
            iterations++;
            int multiplier = iterations; // TODO - straight up multiplier or more complex?
            List<(IMeleeWeapon Weapon, IWeaponAttack Attack, double Weighting)> possibleAttacksThisIteration = weaponsForThisIteration
                                               .Select(x => (Weapon: x,
        ...
        {
            List<(IMeleeWeapon Weapon, IWeaponAttack Attack, double Weight)> onTargetAttacks = weaponsAndAttacks.Where(x =>
        ...
        List<(IMeleeWeapon Weapon, IWeaponAttack Attack, double Weight)> preferredAttacks =
        ...
        (IMeleeWeapon Weapon, IWeaponAttack Attack, double Weight) attack = weaponsAndAttacks.GetWeightedRandom(x => x.Weight);
        if (attack == default)
        {
            return null;
        }
    ...
    protected virtual ICombatMove AttemptUseNaturalAttack(ICharacter combatant)
    {
        ...
        BuiltInCombatMoveType[] attackTypes = new[]
        {
        ...
        List<INaturalAttack> preferredAttacks =
    ...
    public bool HasViableMeleeAttack(ICharacter combatant)
    {
        ...
        BuiltInCombatMoveType[] naturalTypes = new[]
        {
                        BuiltInCombatMoveType.NaturalWeaponAttack,
                        BuiltInCombatMoveType.StaggeringBlowUnarmed,
                        BuiltInCombatMoveType.UnbalancingBlowUnarmed,
