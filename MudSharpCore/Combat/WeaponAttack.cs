...
public class WeaponAttack : CombatAction, IWeaponAttack
{
    ...
    protected WeaponAttack(IFuturemud gameworld, BuiltInCombatMoveType type)
    {
        ...
        using (new FMDB())
        {
            Models.WeaponAttack dbitem = new()
            {
    ...
    {
        Models.WeaponAttack dbitem = FMDB.Context.WeaponAttacks.Find(Id);
    ...
    private bool BuildingCommandStun(ICharacter actor, StringStack command)
    {
        ...
        ITraitExpression expression = long.TryParse(command.PopSpeech(), out long value)
        ...
        actor.OutputHandler.Send(
            $"This attack now uses trait expression #{expression.Id.ToString("N0", actor)} for stun:  {expression}");
        return true;
    }
    ...
    private bool BuildingCommandPain(ICharacter actor, StringStack command)
    {
        ...
        Changed = true;
        actor.OutputHandler.Send(
            $"This attack now uses trait expression #{expression.Id.ToString("N0", actor)} for pain:  {expression}");
        return true;
    }
    ...
    private bool BuildingCommandDamage(ICharacter actor, StringStack command)
    {
        ...
        Changed = true;
        actor.OutputHandler.Send(
            $"This attack now uses trait expression #{expression.Id.ToString("N0", actor)} for damage:  {expression}");
        return true;
    }
    ...
    private bool BuildingCommandVerb(ICharacter actor, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<MeleeWeaponVerb>(command.PopSpeech(), out MeleeWeaponVerb value))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandDamagetype(ICharacter actor, StringStack command)
    {
        ...
        if (!WoundExtensions.TryGetDamageType(command.PopSpeech(), out DamageType damage))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandOrientation(ICharacter actor, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<Orientation>(command.PopSpeech(), out Orientation orientation))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandAlignment(ICharacter actor, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<Alignment>(command.PopSpeech(), out Alignment alignment))
        {
            actor.OutputHandler.Send(
                $"What alignment on the target should this attack come from when being used from the right hand towards two entities facing each other?\nValid values are: {Enum.GetNames(typeof(Alignment)).Select(x => x.Colour(Telnet.Green)).ListToString()}");
            return false;
    ...
    private bool BuildingCommandWeapon(ICharacter actor, StringStack command)
    {
        ...
        {
            foreach (IWeaponType type in Gameworld.WeaponTypes.Where(x => x.Attacks.Contains(this)))
            {
        ...
        IWeaponType weapontype = long.TryParse(command.SafeRemainingArgument, out long value)
        ...
        foreach (IRace race in Gameworld.Races)
        {
    ...
    public string ShowBuilder(ICharacter actor)
    {
        ...
        List<ICombatMessage> messages = Gameworld.CombatMessageManager.CombatMessages.Where(x => x.CouldApply(this))
                                .OrderByDescending(x => x.Priority).ThenByDescending(x => x.Outcome ?? Outcome.None)
                                .ThenBy(x => x.WeaponAttackProg != null).ToList();
        int i = 1;
        foreach (ICombatMessage message in messages)
        {
            sb.AppendLine(
    ...
    public IWeaponAttack CloneWeaponAttack()
    {
        using (new FMDB())
        {
            ...
            FMDB.Context.SaveChanges();
            IWeaponAttack newAttack = LoadWeaponAttack(dbnew, Gameworld);
            weaponType?.AddAttack(newAttack);
            foreach (ICombatMessage message in Gameworld.CombatMessageManager.CombatMessages.Where(
