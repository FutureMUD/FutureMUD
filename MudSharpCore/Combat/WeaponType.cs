...
public class WeaponType : SaveableItem, IWeaponType
{
    ...
    public WeaponType(MudSharp.Models.WeaponType type, IFuturemud game)
    {
        ...
        foreach (Models.WeaponAttack attack in type.WeaponAttacks)
        {
    ...
    public WeaponType(WeaponType rhs, string newName)
    {
        ...
        using (new FMDB())
        {
            ...
            List<Models.WeaponAttack> newAttacks = new();
            foreach (IWeaponAttack attack in rhs.Attacks)
            {
                Models.WeaponAttack dbattack = FMDB.Context.WeaponAttacks.Find(attack.Id);
                if (dbattack is null)
                {
                    continue;
                }

                Models.WeaponAttack newdbattack = new()
                {
                    WeaponTypeId = _id,
                    Verb = dbattack.Verb,
                ...
                foreach (CombatMessagesWeaponAttacks message in dbattack.CombatMessagesWeaponAttacks)
                {
            ...
            FMDB.Context.SaveChanges();
            foreach (Models.WeaponAttack attack in newAttacks)
            {
                IWeaponAttack gattack = WeaponAttack.LoadWeaponAttack(attack, Gameworld);
    ...
    public override void Save()
    {
        ...
        using (new FMDB())
        {
            Models.WeaponType dbitem = FMDB.Context.WeaponTypes.Find(Id);
            ...
            HashSet<long> attackIds = _attacks.Select(x => x.Id).ToHashSet();
            foreach (Models.WeaponAttack attack in FMDB.Context.WeaponAttacks)
            {
    ...
    private bool BuildingCommandClassification(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<WeaponClassification>(out WeaponClassification value) ||
