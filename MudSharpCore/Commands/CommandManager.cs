...
public class CommandManager : ICommandManager
{
    ...
    {
        foreach (
            KeyValuePair<string, IExecutable> command in other.Commands.Where(command => HasPermissionUpTo >= command.Value.PermissionRequired))
        {
    ...
    {
        IExecutable command = LocateCommand(ref playerInput);
    ...
    protected IExecutable LocateCommand(ref string input)
    {
        ...
        _commands.TryGetValue(splitCommands.PopSpeech().ToLowerInvariant(), out IExecutable command);
        ...
        {
            KeyValuePair<string, IExecutable> abbreviation =
...
public class CommandManager<T> : ICommandManager<T>
{
    ...
    {
        foreach (
            KeyValuePair<string, IExecutable<T>> command in other.TCommands.Where(command => HasPermissionUpTo >= command.Value.PermissionRequired))
        {
    ...
    public virtual bool Execute(T argument, string playerInput, CharacterState state = CharacterState.Any,
        PermissionLevel permissionLevel = PermissionLevel.Any, IOutputHandler outputHandler = null)
    {
        IExecutable<T> command = LocateCommand(argument, ref playerInput);
...
public class CharacterCommandManager : CommandManager<ICharacter>, ICharacterCommandManager
{
    ...
    public override IExecutable<ICharacter> LocateCommand(ICharacter reference, ref string input)
    {
        ...
        string commandText = splitCommands.PopSpeech().ToLowerInvariant();
        _tCommands.TryGetValue(commandText, out IExecutable<ICharacter> command);
        CheckCommandConditions(ref command, reference, commandText);

        if (command == null)
        {
            ISocial social = Socials?.FirstOrDefault(x => x.Applies(reference, splitCommands.Last, false));
            if (social != null)
            {
        ...
        if (command == null && !string.IsNullOrWhiteSpace(splitCommands.Last))
        {
            IEnumerable<KeyValuePair<string, IExecutable<ICharacter>>> abbrevCommands =
                _tCommands.Concat(
            ...
            KeyValuePair<string, IExecutable<ICharacter>> abbreviation =
        ...
        PermissionLevel permissionLevel = PermissionLevel.Any, IOutputHandler outputHandler = null)
    {
        ...
        {
            IEffect delayingEffect = argument.Effects.FirstOrDefault(x =>
    ...
    public override bool Execute(ICharacter argument, string playerInput, CharacterState state = CharacterState.Any,
        PermissionLevel permissionLevel = PermissionLevel.Any, IOutputHandler outputHandler = null)
    {
        IExecutable<ICharacter> command = LocateCommand(argument, ref playerInput);
        if (command != null)
        {
