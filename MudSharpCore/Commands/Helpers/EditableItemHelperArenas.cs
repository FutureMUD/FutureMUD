...
public partial class EditableItemHelper
{
    public static EditableItemHelper CombatArenaHelper { get; } = new()
    {
        ...
        EditableNewAction = (actor, input) =>
        {
            ...
            IEconomicZone zone = actor.Gameworld.EconomicZones.GetByIdOrName(input.PopSpeech());
            if (zone == null)
            {
            ...
            ICurrency currency = actor.Gameworld.Currencies.GetByIdOrName(input.SafeRemainingArgument);
            if (currency == null)
            {
            ...
            CombatArena arena = new(actor.Gameworld, name, zone, currency);
    ...
    public static EditableItemHelper CombatantClassHelper { get; } = new()
    {
        ...
        EditableNewAction = (actor, input) =>
        {
            ...
            ICombatArena arena = actor.Gameworld.CombatArenas.GetByIdOrName(input.PopSpeech());
            if (arena == null)
            {
            ...
            ArenaCombatantClass cls = new(concreteArena, name, prog);
            actor.RemoveAllEffects<BuilderEditingEffect<ICombatantClass>>();
    ...
    public static EditableItemHelper ArenaEventTypeHelper { get; } = new()
    {
        ...
        EditableNewAction = (actor, input) =>
        {
            ...
            if (!int.TryParse(input.SafeRemainingArgument, out int sideCount) || sideCount <= 0)
            {
                actor.OutputHandler.Send("You must specify a positive whole number of sides.");
                return;
            }

            TimeSpan registrationDuration = TimeSpan.FromMinutes(10);
            TimeSpan preparationDuration = TimeSpan.FromMinutes(2);
            TimeSpan timeLimit = TimeSpan.FromMinutes(15);
            ...
            using (new FMDB())
            {
                ...
                for (int index = 0; index < sideCount; index++)
                {
                    ...
                    foreach (ICombatantClass cls in arena.CombatantClasses)
                    {
        ...
        EditableCloneAction = (actor, input) =>
        {
            ...
            string which = input.PopSpeech();
            IArenaEventType template = actor.Gameworld.CombatArenas.SelectMany(x => x.EventTypes)
            ...
            IArenaEventType clone = template.Clone(name, actor);
            actor.RemoveAllEffects<BuilderEditingEffect<IArenaEventType>>();
    ...
    public static EditableItemHelper ArenaEventHelper { get; } = new()
    {
        ...
        EditableNewAction = (actor, input) =>
        {
            ...
            IArenaEventType eventType = arena.EventTypes.FirstOrDefault(x => x.Name.EqualTo(typeText)) ??
            ...
            if (!DateUtilities.TryParseDateTimeOrRelative(input.SafeRemainingArgument, actor.Account, true, out DateTime when))
            {
                actor.OutputHandler.Send("That is not a valid date/time.");
                return;
            }

            IArenaEvent evt = arena.CreateEvent(eventType, when);
            actor.RemoveAllEffects<BuilderEditingEffect<IArenaEvent>>();
