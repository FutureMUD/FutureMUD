    ...
    static EditableRevisableItemHelper()
    {
        ...
        NpcTemplateHelper = new EditableRevisableItemHelper
        {
            ...
            DeleteEditableItemAction = item =>
            {
                using (new FMDB())
                {
                    Models.NpcTemplate dbproto = FMDB.Context.NpcTemplates.Find(item.Id, item.RevisionNumber);
                    if (dbproto != null)
                    {
            ...
            CustomSearch = (protos, keyword, gameworld) =>
            {
                ...
                {
                    keyword = keyword[1..];
                    IArtificialIntelligence ai = gameworld.AIs.GetByIdOrName(keyword);
                    if (ai is null)
                    {
                        return protos;
                    }
        ...
        GameItemHelper = new EditableRevisableItemHelper
        {
            ...
            DeleteEditableItemAction = item =>
            {
                using (new FMDB())
                {
                    Models.GameItemProto dbproto = FMDB.Context.GameItemProtos.Find(item.Id, item.RevisionNumber);
                    if (dbproto != null)
                    {
            ...
            EditableNewAction = (actor, input) =>
            {
                ...
                if (cmd.Length == 0)
                {
                    GameItemProto item = new(actor.Gameworld, actor.Account);
        ...
        ForagableHelper = new EditableRevisableItemHelper
        {
            ...
            DeleteEditableItemAction = item =>
            {
                using (new FMDB())
                {
                    Models.Foragable dbproto = FMDB.Context.Foragables.Find(item.Id, item.RevisionNumber);
                    if (dbproto != null)
                    {
            ...
            EditableNewAction = (actor, input) =>
            {
                Foragable foragable = new(actor.Account);
        ...
        ForagableProfileHelper = new EditableRevisableItemHelper
        {
            ...
            DeleteEditableItemAction = item =>
            {
                using (new FMDB())
                {
                    Models.ForagableProfile dbproto = FMDB.Context.ForagableProfiles.Find(item.Id, item.RevisionNumber);
                    if (dbproto != null)
                    {
            ...
            EditableNewAction = (actor, input) =>
            {
                ForagableProfile item = new(actor.Account);
                actor.Gameworld.Add(item);
        ...
        CraftHelper = new EditableRevisableItemHelper
        {
            ...
            DeleteEditableItemAction = item =>
            {
                using (new FMDB())
                {
                    Models.Craft dbproto = FMDB.Context.Crafts.Find(item.Id, item.RevisionNumber);
                    if (dbproto != null)
                    {
                    ...
                    switch (char.ToLowerInvariant(keyword[0]))
                    {
                        ...
                        case '-':
                            return protos
                                   ...
                                   .ToList<IEditableRevisableItem>();
                        case '&':
                            ITag tag = gameworld.Tags.GetByIdOrName(key2);
                            if (tag is null)
                            {
                                return [];
                            }
                            ...
                            return protos
                                   ...
                                   .Where(x =>
                                    ...
                                    .ToList<IEditableRevisableItem>();
                        case '*':
                            if (!long.TryParse(key2, out long id))
                            {
                                return [];
                            }
                            return protos
                                   ...
                                   .ToList<IEditableRevisableItem>();
                        case '^':
                            Form.Material.ILiquid liquid = gameworld.Liquids.GetByIdOrName(key2);
                            if (liquid is null)
                            {
                                return [];
                            }
        ...
        ProjectHelper = new EditableRevisableItemHelper
        {
            ...
            DeleteEditableItemAction = item =>
            {
                using (new FMDB())
                {
                    Models.Project dbproto = FMDB.Context.Projects.Find(item.Id, item.RevisionNumber);
                    if (dbproto != null)
                    {
            ...
            EditableNewAction = (actor, input) =>
            {
                ...
                IProject item = ProjectFactory.CreateProject(actor.Account, actor.Gameworld, input.PopSpeech());
                if (item == null)
                {
                    actor.OutputHandler.Send(
        ...
        TattooHelper = new EditableRevisableItemHelper
        {
            ...
            EditableNewAction = (actor, input) =>
            {
                ...
                List<ITattooTemplate> tattoos = actor.IsAdministrator()
                ...
                TattooTemplate tattoo = new(actor.Account, name);
        ...
        ItemSkinHelper = new EditableRevisableItemHelper
        {
            ...
            DeleteEditableItemAction = item =>
            {
                using (new FMDB())
                {
                    foreach (IGameItem gitem in item.Gameworld.Items)
                    {
                        if (gitem.Skin == item)
                        {
                            gitem.Skin = null;
                    ...
                    Models.GameItemSkin dbproto = FMDB.Context.GameItemSkins.Find(item.Id, item.RevisionNumber);
                    if (dbproto != null)
                    {
            ...
            EditableNewAction = (actor, input) =>
            {
                ...
                GameItemSkin skin = new(actor.Account, actor.Gameworld, proto, name);
