...
#FSub-Component Butcher Phases#0
    ...
    private static void ButcheringClone(ICharacter actor, StringStack ss, bool copyproducts)
    {
        ...
        IRaceButcheryProfile profile = long.TryParse(ss.PopSpeech(), out long value)
        ...
        string name = ss.SafeRemainingArgument.ToLowerInvariant().TitleCase();
        ...
        IRaceButcheryProfile newprofile = profile.Clone(name, copyproducts);
    ...
    private static void ButcheringEdit(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IRaceButcheryProfile> editing = actor.CombinedEffectsOfType<BuilderEditingEffect<IRaceButcheryProfile>>()
                               .FirstOrDefault();
            if (editing == null)
            {
        ...
        if (cmd.EqualTo("new"))
        {
            ...
            RaceButcheryProfile newprofile = new(actor.Gameworld, name);
            actor.Gameworld.Add(newprofile);
            actor.RemoveAllEffects<BuilderEditingEffect<IRaceButcheryProfile>>();
            actor.AddEffect(new BuilderEditingEffect<IRaceButcheryProfile>(actor) { EditingItem = newprofile });
            actor.OutputHandler.Send(
        ...
        IRaceButcheryProfile profile = long.TryParse(cmd, out long value)
            ? actor.Gameworld.RaceButcheryProfiles.Get(value)
    ...
    private static void ButcheringShow(ICharacter actor, StringStack ss)
    {
        ...
        IRaceButcheryProfile profile = long.TryParse(ss.SafeRemainingArgument, out long value)
            ? actor.Gameworld.RaceButcheryProfiles.Get(value)
...
You can use the following options with this command:
    ...
    private static void ButcheryProductClone(ICharacter actor, StringStack ss)
    {
        ...
        IButcheryProduct profile = long.TryParse(ss.PopSpeech(), out long value)
        ...
        IButcheryProduct newprofile = profile.Clone(name);
        actor.Gameworld.Add(newprofile);
    ...
    private static void ButcheryProductEdit(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IButcheryProduct> editing = actor.CombinedEffectsOfType<BuilderEditingEffect<IButcheryProduct>>()
                               .FirstOrDefault();
            if (editing == null)
            {
        ...
        if (cmd.EqualTo("new"))
        {
            ...
            Body.IBodyPrototype body = actor.Gameworld.BodyPrototypes.GetByIdOrName(ss.PopSpeech());
            if (body is null)
            {
            ...
            Body.IBodypart part = body.AllBodyparts.GetByIdOrName(ss.PopSpeech());
            if (part is null)
            {
            ...
            GameItems.IGameItemProto proto = actor.Gameworld.ItemProtos.GetByIdOrName(ss.PopSpeech());
            if (proto is null)
            {
            ...
            ButcheryProduct newprofile = new(name, body, part, proto, quantity);
            actor.Gameworld.Add(newprofile);
            actor.RemoveAllEffects<BuilderEditingEffect<IButcheryProduct>>();
            actor.AddEffect(new BuilderEditingEffect<IButcheryProduct>(actor) { EditingItem = newprofile });
            actor.OutputHandler.Send(
        ...
        IButcheryProduct profile = long.TryParse(cmd, out long value)
            ? actor.Gameworld.ButcheryProducts.Get(value)
    ...
    private static void ButcheryProductShow(ICharacter actor, StringStack ss)
    {
        ...
        IButcheryProduct profile = long.TryParse(ss.SafeRemainingArgument, out long value)
            ? actor.Gameworld.ButcheryProducts.Get(value)
...
The syntax for this command is as follows:
    ...
    private static void EconomicZoneClone(ICharacter actor, StringStack command)
    {
        ...
        IEconomicZone clone = zone.Clone(name);
    ...
    {
        BuilderEditingEffect<IEconomicZone> editing = actor.CombinedEffectsOfType<BuilderEditingEffect<IEconomicZone>>().FirstOrDefault();
        if (editing == null)
        {
    ...
    private static void EconomicZoneShow(ICharacter actor, StringStack command)
    {
        ...
        IEconomicZone target = long.TryParse(command.SafeRemainingArgument, out long id)
    ...
    private static void EconomicZoneEdit(ICharacter actor, StringStack command)
    {
        ...
        if (command.PeekSpeech().EqualTo("new"))
        {
            ...
            Construction.IZone zone = long.TryParse(command.PopSpeech(), out long value)
            ...
            EconomicZone economicZone = new(actor.Gameworld, zone, name);
    ...
    {
        List<IEconomicZone> zones = actor.Gameworld.EconomicZones.ToList();
