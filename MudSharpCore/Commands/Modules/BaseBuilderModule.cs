...
internal abstract class BaseBuilderModule : Module<ICharacter>
{
        ...
        EditableRevisableItemHelper helper)
    {
        ...
        else if (long.TryParse(cmd, out long id))
        {
            if (!int.TryParse(input.SafeRemainingArgument, out int revision))
            {
    ...
    {
        IEnumerable<IEditableRevisableItem> protos = helper.GetAllEditableItems(character).Where(x => x.Status == RevisionStatus.PendingRevision);
    ...
    public static void GenericReviewAll(ICharacter character, StringStack input, EditableRevisableItemHelper helper)
    {
        ...
        IEditableRevisableItem[] editableItems = protos as IEditableRevisableItem[] ?? protos.ToArray();
        ...
        int count = editableItems.Length;
        using (new FMDB())
        {
...
If you do not wish to approve or decline, you may type {"abort edit".Colour(Telnet.Yellow)}.");
    ...
    public static void GenericReviewHistory(ICharacter character, StringStack input, EditableRevisableItemHelper helper)
    {
        ...
        IEnumerable<IEditableRevisableItem> protos = helper.GetAllEditableItemsByIdFunc(character, value);
        if (!protos.Any())
        {
    ...
    {
        IEditableRevisableItem proto = long.TryParse(input.Last, out long value)
    ...
    {
        List<IEditableRevisableItem> protos = helper.GetAllEditableItems(character).ToList();
        ...
        while (!input.IsFinished)
        {
            ...
            switch (cmd)
            {
                ...
                case "mine":
                    using (new FMDB())
                    {
                        Models.Account dbaccount = FMDB.Context.Accounts.Find(character.Account.Id);
                        if (dbaccount == null)
                        {
                            character.OutputHandler.Send("There is no such account.");
                            return;
    ...
    public static void GenericEditDefault(ICharacter character, StringStack input, EditableItemHelper helper)
    {
        ...
        IEditableItem proto = helper.GetEditableItemByIdOrNameFunc(character, cmd);
    ...
    {
        List<IEditableItem> protos = helper.GetAllEditableItems(character).ToList();
    ...
    protected static void Item_Load(ICharacter actor, StringStack input)
    {
        ...
        {
            string skinText = input.PopSpeech()[1..];

            if (long.TryParse(skinText, out long skinid))
            {
        ...
        if (quantity > 1)
        {
            ...
            {
                IGameItem item = proto.CreateNew(actor, skin, quantity, input.SafeRemainingArgument).First();
            ...
            else
            {
                ...
                List<IGameItem> items = proto.CreateNew(actor, skin, quantity, input.SafeRemainingArgument).ToList();
        ...
        {
            IGameItem item = proto.CreateNew(actor, skin, 1, input.SafeRemainingArgument).First();
            actor.Gameworld.Add(item);
