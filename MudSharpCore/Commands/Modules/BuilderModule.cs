...
You must use one of the following subcommands of this command:
    ...
    {
        List<ICharacteristicDefinition> characteristics = actor.Gameworld.Characteristics.ToList();
    ...
    private static void CharacteristicDefinitionShow(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicDefinition definition = actor.Gameworld.Characteristics.GetByIdOrName(command.SafeRemainingArgument);
        if (definition == null)
        {
    ...
    private static void Characteristic_Definition_Add(ICharacter actor, StringStack command, bool client, bool bodypart)
    {
        ...
        if (client)
        {
            ...
            ICharacteristicDefinition parent = actor.Gameworld.Characteristics.GetByIdOrName(command.SafeRemainingArgument);
            if (parent == null)
            {
                actor.OutputHandler.Send("There is no such characteristic definition.");
                return;
            ...
            if (bodypart)
            {
                BodypartSpecificClientCharacteristicDefinition child = new(name, name.ToLowerInvariant(),
            ...
            {
                ClientCharacteristicDefinition child = new(name, name.ToLowerInvariant(),
        ...
        if (bodypart)
        {
            BodypartSpecificCharacteristicDefinition definition = new(name, name.ToLowerInvariant(),
        ...
        {
            CharacteristicDefinition definition = new(name, name.ToLowerInvariant(), $"The {name} characteristic",
                ctype,
                actor.Gameworld);
            actor.OutputHandler.Send(
    ...
    private static void CharacteristicValueClone(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicValue clone = actor.Gameworld.CharacteristicValues.GetByIdOrName(command.PopSpeech());
        if (clone == null)
        {
        ...
        ICharacteristicValue newValue = clone.Clone(command.SafeRemainingArgument);
    ...
    private static void CharacteristicValueShow(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicValue value = actor.Gameworld.CharacteristicValues.GetByIdOrName(command.SafeRemainingArgument);
        if (value == null)
        {
            actor.OutputHandler.Send("There is no such characteristic value.");
            return;
    ...
    {
        List<ICharacteristicValue> values = actor.Gameworld.CharacteristicValues.ToList();
        while (!command.IsFinished)
        {
            string cmd = command.PopSpeech();
            if (cmd.Length == 0)
            {
                continue;
            }
            ...
            {
                cmd = cmd[1..];
                ICharacteristicProfile profile = actor.Gameworld.CharacteristicProfiles.GetByIdOrName(cmd);
                if (profile == null)
                {
                    actor.OutputHandler.Send(
    ...
    private static void Characteristic_Value_Add(ICharacter actor, StringStack command)
    {
        string name = command.PopSpeech();
        ICharacteristicDefinition definition = null;
        definition = long.TryParse(name, out long value)
        ...
        string basic = command.PopSpeech();
        switch (definition.Type)
        {
            ...
            case CharacteristicType.Multiform:
                ...
                string fancy = command.PopSpeech();
            ...
            case CharacteristicType.Growable:
                ...
                if (command.IsFinished || !int.TryParse(command.PopSpeech(), out int length))
                {
    ...
    {
        string name = command.PopSpeech();
        ICharacteristicValue cvalue = actor.Gameworld.CharacteristicValues.GetByIdOrName(name);
    ...
    private static void CharacteristicProfileClone(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicProfile clone = actor.Gameworld.CharacteristicProfiles.GetByIdOrName(command.PopSpeech());
        if (clone == null)
        {
        ...
        ICharacteristicProfile newProfile = clone.Clone(name);
    ...
    private static void CharacteristicProfileSet(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicProfile which = actor.Gameworld.CharacteristicProfiles.GetByIdOrName(command.PopSpeech());
        if (which == null)
        {
            actor.OutputHandler.Send("There is no such characteristic profile.");
            return;
    ...
    {
        List<ICharacteristicProfile> characteristics = actor.Gameworld.CharacteristicProfiles.ToList();
        while (!command.IsFinished)
        {
            string cmd = command.PopSpeech();
            if (cmd[0] == '+' && cmd.Length > 1)
    ...
    private static void CharacteristicProfileNew(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicDefinition definition = actor.Gameworld.Characteristics.GetByIdOrName(command.PopSpeech());
        if (definition == null)
        {
            actor.OutputHandler.Send("There is no such characteristic definition.");
            return;
        ...
        string typeText = command.PopForSwitch();
        ...
        {
            case "standard":
                CharacteristicProfile profile = new(name, definition, "Standard");
...
The syntax for this command is as follows:
    ...
    private static void DreamRemove(ICharacter actor, StringStack ss)
    {
        ...
        IDream dream = actor.Gameworld.Dreams.GetByIdOrName(ss.PopSpeech());
        if (dream is null)
        {
        ...
        ICharacter tch;
        if (long.TryParse(ss.SafeRemainingArgument, out long value))
        {
...
The syntax to use this command is as follows:
    ...
    private static void TagParent(ICharacter actor, StringStack ss)
    {
        ...
        ITag parent = actor.Gameworld.Tags.GetByIdOrName(ss.SafeRemainingArgument);
        if (parent is null)
        {
        ...
        string old = tag.FullName.ColourName();
        tag.Parent = parent;
    ...
    private static void TagRename(ICharacter actor, StringStack ss)
    {
        ...
        string name = ss.SafeRemainingArgument.TitleCase().Trim();
    ...
    private static void TagView(ICharacter actor, StringStack command)
    {
        ...
        List<ITag> tags = actor.Gameworld.Tags.FindMatchingTags(command.SafeRemainingArgument);
        if (tags.Count == 0)
        {
        ...
        ITag targetTag = tags.Single();
        List<ITag> children = actor.Gameworld.Tags.Except(targetTag).Where(x => x.IsA(targetTag)).ToList();
        StringBuilder sb = new();

        void DescribeLevel(ITag tag, int level)
        {
            sb.AppendLine(new string('\t', level) + $"[{tag.Id.ToString("N0", actor)}] {tag.Name}");
            foreach (ITag branch in children.Where(x => x.Parent == tag).ToList())
            {
    ...
    private static void TagInsert(ICharacter actor, StringStack command)
    {
        ...
        string tagName = command.PopSpeech();
        ...
        ITag parentTag = actor.Gameworld.Tags.GetByIdOrName(command.SafeRemainingArgument);
        if (parentTag == null)
        {
        ...
        List<ITag> children = actor.Gameworld.Tags.Where(x => x.Parent == parentTag).ToList();
        Tag newTag = new(tagName, parentTag, actor.Gameworld);
    ...
    private static void TagRemove(ICharacter actor, StringStack command)
    {
        ...
        int childrenCount = actor.Gameworld.Tags.Except(targetTag).Count(x => x.IsA(targetTag));
        actor.Send(
        ...
        actor.AddEffect(new Accept(actor, new GenericProposal(
            message =>
            {
                ...
                List<ITag> tagsToDelete =
                    actor.Gameworld.Tags.Where(x => x.IsA(targetTag)).ToList();
                foreach (ITag tag in tagsToDelete)
                {
    ...
    {
        BuilderEditingEffect<IWearProfile> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<IWearProfile>>().FirstOrDefault();
        if (effect is null)
        {
    ...
    private static void WearProfileAdd(ICharacter actor, StringStack ss)
    {
        ...
        using (new FMDB())
        {
            ...
            IWearProfile newWearProfile = GameItems.Inventory.WearProfile.LoadWearProfile(dbitem, actor.Gameworld);
            actor.Gameworld.Add(newWearProfile);

            Models.GameItemComponentProto dbcomponent = new()
            {
            ...
            IGameItemComponentProto component = actor.Gameworld.GameItemComponentManager.GetProto(dbcomponent, actor.Gameworld);
    ...
    private static void WearProfileClone(ICharacter actor, StringStack ss)
    {
        ...
        IWearProfile profile = actor.Gameworld.WearProfiles.GetByIdOrName(ss.PopSpeech());
        if (profile is null)
        {
        ...
        IWearProfile newItem = profile.Clone(name);
    ...
    private static void WearProfileEdit(ICharacter actor, StringStack ss)
    {
        ...
        IWearProfile profile = actor.Gameworld.WearProfiles.GetByIdOrName(ss.SafeRemainingArgument);
        if (profile is null)
        {
    ...
    {
        List<IWearProfile> profiles = actor.Gameworld.WearProfiles.ToList();
...
7) #3min(99, (2 * dex:4) + (1 * str:1) + (2 * wil:5))#0
    ...
    private static void TraitExpressionTest(ICharacter actor, StringStack ss)
    {
        ...
        ITraitExpression te = actor.Gameworld.TraitExpressions.GetByIdOrName(ss.PopSpeech());
        if (te is null)
        {
        ...
        {
            foreach (string item in te.NonTraitParameters)
            {
                if (ss.IsFinished)
                {
    ...
    private static void TraitExpressionView(ICharacter actor, StringStack ss)
    {
        ...
        {
            expression = long.TryParse(ss.PopSpeech(), out long value)
    ...
    {
        List<ITraitExpression> expressions = actor.Gameworld.TraitExpressions.ToList();
        // Filters
        while (!ss.IsFinished)
        {
            string cmd = ss.PopSpeech();
    ...
    private static void TraitExpressionClone(ICharacter actor, StringStack ss)
    {
        ...
        ITraitExpression expression = long.TryParse(ss.PopSpeech(), out long value)
            ? actor.Gameworld.TraitExpressions.Get(value)
            : actor.Gameworld.TraitExpressions.GetByName(ss.Last);
        if (expression == null)
        {
        ...
        TraitExpression newexpr = new((TraitExpression)expression);
    ...
    private static void TraitExpressionEdit(ICharacter actor, StringStack ss)
    {
        ...
        ITraitExpression expr = long.TryParse(ss.PopSpeech(), out long value)
            ? actor.Gameworld.TraitExpressions.Get(value)
            : actor.Gameworld.TraitExpressions.GetByName(ss.Last);
        if (expr == null)
        {
    ...
    {
        string name = "Unnamed Trait Expression";
        if (!ss.IsFinished)
        {
        ...
        TraitExpression newexpr = new(actor.Gameworld, name);
...
The syntax for this command is as follows:
    ...
    private static void GasShow(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IGas> effect = actor.EffectsOfType<BuilderEditingEffect<IGas>>().FirstOrDefault();
            if (effect == null)
            {
                actor.OutputHandler.Send(
    ...
    private static void GasNew(ICharacter actor, StringStack ss)
    {
        ...
        string name = ss.PopSpeech().ToLowerInvariant();
    ...
    private static void GasClone(ICharacter actor, StringStack ss)
    {
        ...
        IGas gas = actor.Gameworld.Gases.GetByIdOrName(ss.PopSpeech());
        if (gas == null)
        {
            actor.OutputHandler.Send("There is no such gas.");
            return;
    ...
    {
        IEnumerable<IGas> gases = actor.Gameworld.Gases.AsEnumerable();
        while (!command.IsFinished)
        {
            // Filter
            string cmd = command.PopSpeech();
...
#9Note#0: Liquid properties marked with a #B*#0 above are currently not used by the engine but will see inclusion in the future.";
    ...
    {
        IEnumerable<ILiquid> liquids = actor.Gameworld.Liquids.AsEnumerable();
        while (!command.IsFinished)
        {
            // Filter
            string cmd = command.PopSpeech();
    ...
    private static void LiquidShow(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<ILiquid> effect = actor.EffectsOfType<BuilderEditingEffect<ILiquid>>().FirstOrDefault();
            if (effect == null)
            {
                actor.OutputHandler.Send(
    ...
    private static void LiquidNew(ICharacter actor, StringStack ss)
    {
        ...
        Liquid newLiquid = new(name, actor.Gameworld);
    ...
    private static void LiquidClone(ICharacter actor, StringStack ss)
    {
        ...
        ILiquid liquid = actor.Gameworld.Liquids.GetByIdOrName(ss.PopSpeech());
        if (liquid == null)
        {
            actor.OutputHandler.Send("There is no such liquid.");
            return;
        ...
        ILiquid newMaterial = liquid.Clone(name);
...
The syntax for this command is as follows:
    ...
    private static void MaterialShow(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<ISolid> effect = actor.EffectsOfType<BuilderEditingEffect<ISolid>>().FirstOrDefault();
            if (effect == null)
            {
                actor.OutputHandler.Send(
        ...
        ISolid material = actor.Gameworld.Materials.GetByIdOrName(targetText);
        if (material == null)
        {
    ...
    private static void MaterialNew(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.PopSpeech().TryParseEnum<MaterialBehaviourType>(out MaterialBehaviourType behaviour))
        {
            actor.OutputHandler.Send(
        ...
        Solid newMaterial = new(name, behaviour, actor.Gameworld);
        actor.OutputHandler.Send(
    ...
    private static void MaterialClone(ICharacter actor, StringStack ss)
    {
        ...
        ISolid newMaterial = material.Clone(name, description);
        actor.OutputHandler.Send(
    ...
    {
        IEnumerable<ISolid> materials = actor.Gameworld.Materials.AsEnumerable();
        while (!command.IsFinished)
        {
            if (!command.PopSpeech().TryParseEnum<MaterialBehaviourType>(out MaterialBehaviourType materialType))
            {
...
Also, as an admin you should see the two related commands #3GIVETATTOO#0 and #3FINISHTATTOO#0.";
    ...
    private static void TattooContinue(ICharacter actor, StringStack ss)
    {
        ...
        ITattoo tattoo = target.Body.Tattoos.Where(x => target.Body.ExposedBodyparts.Contains(x.Bodypart))
        ...
        IInventoryPlan plan = TattooNeedlePlan.CreatePlan(actor);
        ...
        void BeginTattooAction()
        {
            ...
            IInventoryPlan inkplan = tattoo.TattooTemplate.GetInkPlan(actor);
    ...
    private static void TattooInscribe(ICharacter actor, StringStack ss)
    {
        ...
        if (!TattooTextCommandHelper.TryParseTextValues(actor, template, ss, true, out List<ITattooTextValue> textValues,
                out bool hasUnreadableCopyPenalty, out string errorMessage))
        {
        ...
        void BeginTattooAction()
        {
            ...
            IInventoryPlan inkplan = template.GetInkPlan(actor);
            if (inkplan.PlanIsFeasible() != InventoryPlanFeasibility.Feasible)
            {
                actor.OutputHandler.Send("You do not have the inks you require to work on that tattoo.");
                return;
            ...
            plan.FinalisePlanNoRestore();
            inkplan.FinalisePlanNoRestore();
            ITattoo tattoo = template.ProduceTattoo(actor, target, bodypart, textValues, hasUnreadableCopyPenalty);
    ...
    private static void TattooClone(ICharacter actor, StringStack command)
    {
        ...
        ITattooTemplate newTattoo = (ITattooTemplate)tattoo.Clone(actor.Account, name);
        actor.Gameworld.Add(newTattoo);
        actor.RemoveAllEffects(x => x.IsEffectType<BuilderEditingEffect<IDisfigurementTemplate>>());
        actor.AddEffect(new BuilderEditingEffect<IDisfigurementTemplate>(actor) { EditingItem = newTattoo });
        actor.OutputHandler.Send(
...
The list of filters you can use with #3bodypart list#0 are as follows:
    ...
    {
        IEnumerable<IBodypart> parts = actor.Gameworld.BodypartPrototypes.AsEnumerable();
    ...
    private static IBodypart GetBodypartFromBuilderInput(ICharacter actor, StringStack ss)
    {
        ...
        {
            IBodyPrototype body = actor.Gameworld.BodyPrototypes.GetByIdOrName(cmd1);
            if (body == null)
            {
    ...
    {
        BuilderEditingEffect<IBodypart> effect = actor.EffectsOfType<BuilderEditingEffect<IBodypart>>().FirstOrDefault();
        if (effect == null)
        {
        ...
        IBodypart newPart = effect.EditingItem.Clone(name);
    ...
    private static void BodypartShow(ICharacter actor, StringStack ss)
    {
        ...
        IBodypart bodypart = GetBodypartFromBuilderInput(actor, ss);
        if (bodypart is null)
        {
            return;
        }
...
You can also edit the following specific properties:
    ...
    {
        OverrideTerrain effect = actor.Location.EffectsOfType<OverrideTerrain>().FirstOrDefault();
        if (effect is null)
        {
    ...
    private static void TerrainOverride(ICharacter actor, StringStack ss)
    {
        ...
        ITerrain terrain = actor.Gameworld.Terrains.GetByIdOrName(ss.PopSpeech());
        if (terrain is null)
        {
    ...
    {
        string terrains = JsonSerializer.Serialize(actor.Gameworld.Terrains.Select(x =>
            new
            {
                Id = x.Id,
                Name = x.Name,
    ...
    {
        BuilderEditingEffect<ITerrain> effect = actor.EffectsOfType<BuilderEditingEffect<ITerrain>>().FirstOrDefault();
        if (ss.IsFinished)
        {
            if (effect == null)
            {
        ...
        ITerrain terrain = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void TerrainClone(ICharacter actor, StringStack ss)
    {
        ...
        ITerrain clone = long.TryParse(ss.PopSpeech(), out long value)
            ? actor.Gameworld.Terrains.Get(value)
            : actor.Gameworld.Terrains.GetByName(ss.Last);
        if (clone == null)
        {
        ...
        Terrain terrain = new(clone, name);
    ...
    private static void TerrainNew(ICharacter actor, StringStack ss)
    {
        ...
        Terrain terrain = new(actor.Gameworld, name);
        actor.Gameworld.Add(terrain);
        actor.RemoveAllEffects<BuilderEditingEffect<ITerrain>>();
        actor.AddEffect(new BuilderEditingEffect<ITerrain>(actor) { EditingItem = terrain });
        actor.OutputHandler.Send(
...
You can use the following syntax with this command:
    ...
    private static void UnitTest(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.PopSpeech().TryParseEnum<UnitType>(out UnitType type))
        {
        ...
        if (!actor.Gameworld.UnitManager.TryGetBaseUnits(ss.SafeRemainingArgument, type, actor, out double value))
        {
...
You can filter #3healthstrategy list#0 by type, owner type, or with #6+keyword#0 / #6-keyword#0 name filters.";
    ...
    protected static void HealthStrategy(ICharacter actor, string input)
    {
        ...
        if (ss.PeekSpeech().EqualTo("typehelp"))
        {
            ...
            (string blurb, string help) = BaseHealthStrategy.GetTypeInfoFor(type);
            StringBuilder sb = new();
