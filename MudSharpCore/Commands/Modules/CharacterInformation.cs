...
Additionally, admins can use the following options:
	...
	#3replace#0 - replace the short description with the name".SubstituteANSIColour());
                ...
                switch (ss.PopSpeech().ToLowerInvariant())
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            { "full", "brief", "fullbrief", "classic", "language", "stealth", "position" };
                            if (actor.Capabilities.Any())
                            {
                                options.Add("magic");
                            }
            ...
            case "width":
                ...
                if (!int.TryParse(ss.PopSpeech(), out int outerWrap))
                {
                ...
                if (!int.TryParse(ss.PopSpeech(), out int innerWrap))
                {
                    actor.Send("You must enter a number for your wrap width.");
                    return;
                }
            ...
            case "page":
                ...
                if (!int.TryParse(ss.PopSpeech(), out int pageLength))
                {
            ...
            case "culture":
                ...
                string culturePick = ss.PopSpeech();
            ...
            case "timezone":
                ...
                return;
            case "units":
                List<string> unitSystems = actor.Gameworld.UnitManager.Units.Select(x => x.System).Distinct().ToList();
                if (ss.IsFinished)
                {
                ...
                string unitChoice = ss.PopSpeech();
            ...
            case "writing":
                ...
                WritingStyleDescriptors newStyle = WritingStyleDescriptors.None;
                string text = ss.PopSpeech();
                WritingStyleDescriptors value = WritingStyleDescriptors.None.Parse(text);
            ...
            case "currency":
                ...
                ICurrency currency = actor.Gameworld.Currencies.GetByIdOrName(ss.SafeRemainingArgument);
                if (currency is null)
                {
    ...
    protected static void Consider(ICharacter actor, string input)
    {
        ...
        ICharacter target = actor.TargetActor(arg);
        if (target == null)
        {
        ...
        Gendering gender = target.ApparentGender(actor);
        StringBuilder sb = new();
        ...
        string consider = target.GetConsiderString(actor);
    ...
    protected static void SkillCategories(ICharacter actor, string input)
    {
        ...
        foreach (IGrouping<string, ITrait> group in actor.TraitsOfType(TraitType.Skill).Where(x => !x.Hidden).GroupBy(x => x.Definition.Group)
                                   .OrderBy(x => x.Key))
        {
            if (sb.Length > 0)
            {
            ...
            sb.AppendLine();
            List<string> categorySkills = new();
            foreach (ITrait skill in group.OrderBy(x => x.Definition.Name))
            {
                categorySkills.Add(
    ...
    protected static void SkillAudit(ICharacter actor, string input)
    {
        ...
        foreach (IGrouping<string, ISkill> group in actor.TraitsOfType(TraitType.Skill).Where(x => !x.Hidden).OfType<ISkill>()
                                   .GroupBy(x => x.Definition.Group).OrderBy(x => x.Key))
        {
            ...
            sb.AppendLine($"Category: {group.Key.TitleCase()}".GetLineWithTitle(actor.LineFormatLength,
                actor.Account.UseUnicode, Telnet.Green, Telnet.Cyan));
            sb.AppendLine();
            foreach (ISkill skill in group.OrderBy(x => x.Definition.Name))
            {
...
Additionally, admins can use these forms:
    ...
    protected static void Knowledges(ICharacter actor, string input)
    {
        ...
        {
            string targetText = ss.PopSpeech().RemoveFirstCharacter();
            target = long.TryParse(targetText, out long value)
        ...
        IEnumerable<ICharacterKnowledge> knowledges = target.CharacterKnowledges;
...
For admins, the syntax is simply #3score#0 to see your own score or #3score <target>#0 to see someone else's score."
    ...
    protected static void Score(ICharacter actor, string input)
    {
        ...
        {
            ICharacter targetActor = actor.TargetActor(ss.SafeRemainingArgument);
            if (targetActor == null)
            {
...
The syntax is simply #3attributes#0 or #3attributes <target>#0 to see someone else's attributes.")]
    protected static void Attributes(ICharacter actor, string input)
    {
        ...
        sb.AppendLine();
        foreach (IAttribute attribute in who.Body.Traits.OfType<IAttribute>().OrderBy(x => x.AttributeDefinition.DisplayOrder))
        {
    ...
    internal class HeightTest : TestType
    {
            ...
            Difficulty difficulty)
        {
            ...
            foreach (Difficulty thisDifficulty in AllDifficulties)
            {
...
You can also use this command to test against someone else. This always echoes.
    ...
    protected static void Test(ICharacter actor, string input)
    {
        ...
        string topLevelCommand = ss.PopSpeech();
        bool echo = "silentcommand".StartsWith(topLevelCommand, StringComparison.InvariantCultureIgnoreCase);
        ...
        if (cmd.EqualToAny("vs", "versus", "against"))
        {
            ...
            ICharacter target = actor.TargetActor(cmd);
            if (target == null)
            {
            ...
            OpposedOutcome outcome = new(playerOutcomes, targetOutcomes, diff, diff);
            CheckOutcome playerOutcome = playerOutcomes[outcome.ProponentDifficulty];
            CheckOutcome targetOutcome = targetOutcomes[outcome.OpponentDifficulty];
            OpposedOutcome nonExperimentalOutcome = new(playerOutcomes[diff], targetOutcomes[diff]);
            ...
            foreach (ICharacter witness in actor.Location?.LayerCharacters(actor.RoomLayer).Where(x => !x.IsAdministrator()) ??
            ...
            StringBuilder vsAdminOutput = new();
 ...
 Data for $4:
	...
	Rolls vs {Math.Round(targetOutcome.TargetNumber, 2).ToString().Colour(Telnet.BoldYellow)}: {targetOutcome.Rolls.Select(x => Math.Round(x, 2).ToString()
            ...
            foreach (ICharacter admin in actor.Location?.LayerCharacters(actor.RoomLayer).Where(x => x.IsAdministrator()) ??
                                  Enumerable.Empty<ICharacter>())
            {
        ...
        else
        {
            ...
            {
                foreach (ICharacter witness in actor.Location?.LayerCharacters(actor.RoomLayer)
            ...
            //Generate admin output
            List<IPerceivable> perceivables = new() { actor };
            int perceivable = 1;
            int bonusPerDifficulty = Futuremud.Games.First().GetStaticInt("CheckBonusPerDifficultyLevel");
            int bonusAdjustment = (int)(outcome.FinalBonus / bonusPerDifficulty);
            StringBuilder adminOutput = new();
            adminOutput.Append(
            ...
            {
                adminOutput.Append("\n\tActive Bonuses: ");
                foreach (Tuple<string, double> item in outcome.ActiveBonuses)
                {
    ...
    protected static void Plan(ICharacter actor, string input)
    {
        ...
        switch (ss.PopSpeech().ToLowerInvariant())
        {
            ...
            default:
                ...
                ICharacter target = actor.Gameworld.Characters.GetByPersonalName(ss.Last);
                if (target == null)
                {
    ...
    protected static void UnDub(ICharacter actor, string command)
    {
        ...
        IDub targetDub = actor.Dubs.GetFromItemListByKeyword(ss.PopSpeech(), actor);
        if (targetDub == null)
        {
        ...
        using (new FMDB())
        {
            ...
            Models.Dub dbitem = FMDB.Context.Dubs.Find(targetDub.Id);
            if (dbitem != null)
            {
    ...
    protected static void Dub(ICharacter actor, string command)
    {
        ...
        string keywordText = ss.PopSpeech();
    ...
    protected static void Dubs(ICharacter actor, string command)
    {
        ...
        while (!ss.IsFinished)
        {
            string filterText = ss.PopSpeech();
            dubs =
                dubs.Where(
    ...
    protected static void Body(ICharacter actor, string command)
    {
        ...
        if (!ss.IsFinished)
        {
            if (ss.SafeRemainingArgument.EqualTo("limbs"))
            {
                ...
                foreach (ILimb thelimb in actor.Body.Limbs.OrderBy(x => x.LimbType.LimbOrder()))
                {
        ...
        void DescribeLevel(IBodypart proto, int level)
        {
            ...
            foreach (IBodypart branch in parts.Where(x => x.UpstreamConnection?.CountsAs(proto) == true).ToList())
            {
            ...
            actor.Knowledges.Any(x => actor.Gameworld.SurgicalProcedures.Any(y => y.KnowledgeRequired == x))))
        {
            ...
            IOrganProto[] organs = (limb is null
                ? target.Body.Organs
            ...
            if (organs.Any())
            {
                ...
                foreach (IOrganProto organ in organs)
                {
...
You can use the following options with this command:
    ...
    protected static void Uncovered(ICharacter actor, string command)
    {
        ...
        List<IBodypart> bodyparts = target.Body.Bodyparts.ToList();
        List<(IGameItem Item, IWear Wearloc, IWearlocProfile Profile)> coverInfo = target.Body.WornItemsFullInfo.ToList();
        List<IBodypart> uncoveredParts = new();
        Dictionary<IBodypart, bool> transparentDictionary = new();
        ...
        foreach (IBodypartWithInventoryDisplayOrder part in uncoveredParts.OfType<IBodypartWithInventoryDisplayOrder>().OrderBy(x => x.DisplayOrder))
        {
            sb.AppendLine(
