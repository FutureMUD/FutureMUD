...
Filters for list:
    ...
    private static void ChargenChangeType(ICharacter actor, StringStack ss)
    {
        ...
        List<(string Name, string Description, string HelpText, ChargenStage Stage)> types = ChargenStoryboard.ChargenStageTypeInfos.Where(x => x.Stage == stage).ToList();
        if (types.Count == 1)
        {
        ...
        string typeName = ss.SafeRemainingArgument;
        (string Name, string Description, string HelpText, ChargenStage Stage) type = types.FirstOrDefault(x => x.Name.StartsWith(typeName, StringComparison.InvariantCultureIgnoreCase));
        if (type == default)
        {
            actor.OutputHandler.Send(
...
You are now editing this new storyboard.");
    ...
    private static void ChargenTypeHelp(ICharacter actor, StringStack ss)
    {
        ...
        string typeName = ss.SafeRemainingArgument;
        (string Name, string Description, string HelpText, ChargenStage Stage) type = ChargenStoryboard.ChargenStageTypeInfos.FirstOrDefault(x =>
...
{type.HelpText}".SubstituteANSIColour());
    ...
    private static void ChargenShow(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IChargenScreenStoryboard> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<IChargenScreenStoryboard>>()
                              .FirstOrDefault();
            if (effect is null)
            {
        ...
        IChargenScreenStoryboard screen =
    ...
    private static void ChargenReorder(ICharacter actor, StringStack ss)
    {
        ...
        IChargenScreenStoryboard otherScreen =
            actor.Gameworld.ChargenStoryboard.StageScreenMap.Values.GetByIdOrName(ss.SafeRemainingArgument);
        ...
        foreach (ChargenStage stage in actor.Gameworld.ChargenStoryboard.DefaultOrder)
        {
    ...
    private static void ChargenView(ICharacter actor, StringStack ss)
    {
        ...
        using (new FMDB())
        {
            Models.Chargen dbitem = FMDB.Context.Chargens.Find(value);
            if (dbitem is null)
            {
    ...
    private static void ChargenList(ICharacter actor, StringStack ss)
    {
        ...
        while (!ss.IsFinished)
        {
            ...
            if (cmd.TryParseEnum<ChargenState>(out ChargenState state))
            {
...
You can use the following commands to work with merits:
    ...
    private static void MeritTypeHelp(ICharacter actor, StringStack command)
    {
        ...
        (string Type, string Blurb, string HelpText) type = MeritFactory.TypeHelps.FirstOrDefault(x => x.Type.EqualTo(command.SafeRemainingArgument));
