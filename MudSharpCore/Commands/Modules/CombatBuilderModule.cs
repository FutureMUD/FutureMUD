...
You can use the following arguments to refine the list command:
    ...
    private static void WeaponAttackAudit(ICharacter actor, StringStack ss)
    {
        ...
        List<(BuiltInCombatMoveType, Outcome)> missingMessages = new();
        List<Outcome> outcomeValues =
            Enum.GetValues(typeof(Outcome))
                .Cast<Outcome>()
                .Where(x => x != Outcome.None && x != Outcome.NotTested)
                .ToList();
        List<BuiltInCombatMoveType> typeValues = Enum.GetValues(typeof(BuiltInCombatMoveType)).Cast<BuiltInCombatMoveType>().ToList();
        // Make sure there is a universal fallback message for every attack type
        foreach (BuiltInCombatMoveType typeValue in typeValues)
        {
        ...
        if (missingMessages.Count > 0)
        {
            ...
            foreach ((BuiltInCombatMoveType type, Outcome outcome) in missingMessages)
            {
    ...
    private static void WeaponAttackView(ICharacter actor, StringStack ss)
    {
        ...
        {
            attack = long.TryParse(ss.PopSpeech(), out long value)
	...
	#3*<movetype>#0 - shows only attacks of the specified move type".SubstituteANSIColour());
        ...
        IEnumerable<IWeaponAttack> attacks = actor.Gameworld.WeaponAttacks.AsEnumerable();
        while (!ss.IsFinished)
        {
            ...
            if (arg[0] == '*' && arg.Length > 1)
            {
                if (!arg[1..].TryParseEnum<BuiltInCombatMoveType>(out BuiltInCombatMoveType value))
                {
                    actor.OutputHandler.Send(
                        $"There is no such built-in combat move type as {arg[1..].ColourCommand()}.\nThe valid types are as follows:\n\n{Enum.GetValues<BuiltInCombatMoveType>().Select(x => x.DescribeEnum(false, Telnet.Cyan)).ListToLines(true)}");
                    return;
                }
            ...
            IWeaponType weaponType = actor.Gameworld.WeaponTypes.FirstOrDefault(x => x.Name.EqualTo(arg)) ??
    ...
    private static void WeaponAttackClone(ICharacter actor, StringStack ss)
    {
        ...
        IWeaponAttack newattack = attack.CloneWeaponAttack();
    ...
    private static void WeaponAttackNew(ICharacter actor, StringStack ss)
    {
        ...
        if (!CombatExtensions.TryParseBuiltInMoveType(ss.PopSpeech(), out BuiltInCombatMoveType value) ||
        ...
        IWeaponAttack newAttack = Combat.WeaponAttack.NewWeaponAttack(value, actor.Gameworld);
        actor.Gameworld.Add(newAttack);
        actor.RemoveAllEffects(x => x.IsEffectType<BuilderEditingEffect<IWeaponAttack>>());
        actor.AddEffect(new BuilderEditingEffect<IWeaponAttack>(actor) { EditingItem = newAttack });
        actor.OutputHandler.Send(
    ...
    private static void WeaponAttackNatural(ICharacter actor, StringStack ss)
    {
        ...
        IWeaponAttack attack = long.TryParse(ss.PopSpeech(), out value)
            ? actor.Gameworld.WeaponAttacks.Get(value)
            : actor.Gameworld.WeaponAttacks.GetByName(ss.Last);
        if (attack == null)
        {
        ...
        if (ss.Peek().EqualToAny("remove", "rem", "delete", "del"))
        {
            ...
            Body.IExternalBodypart bodypart = race.BaseBody.AllExternalBodyparts.GetFromItemListByKeyword(ss.PopSpeech(), actor);
            if (bodypart == null)
            {
        ...
        Body.IExternalBodypart targetbodypart = race.BaseBody.AllExternalBodyparts.GetFromItemListByKeyword(ss.PopSpeech(), actor);
        ...
        foreach (INaturalAttack similar in race.NaturalWeaponAttacks
...
The syntax for this command is as follows:
    ...
    {
        StringStack ss = new(text.RemoveFirstWord());
    ...
    private static void AuxiliaryNatural(ICharacter actor, StringStack ss)
    {
        ...
        IAuxiliaryCombatAction attack = long.TryParse(ss.PopSpeech(), out value)
    ...
    private static void AuxiliaryClone(ICharacter actor, StringStack ss)
    {
        ...
        IAuxiliaryCombatAction action = long.TryParse(ss.PopSpeech(), out long value)
            ? actor.Gameworld.AuxiliaryCombatActions.Get(value)
            : actor.Gameworld.AuxiliaryCombatActions.GetByName(ss.Last);
        if (action == null)
        {
        ...
        IAuxiliaryCombatAction newAction = action.Clone();
    ...
    private static void AuxiliaryShow(ICharacter actor, StringStack ss)
    {
        ...
        {
            action = long.TryParse(ss.PopSpeech(), out long value)
                ? actor.Gameworld.AuxiliaryCombatActions.Get(value)
                : actor.Gameworld.AuxiliaryCombatActions.GetByName(ss.Last);
            if (action == null)
            {
	...
	#3-key#0 - excludes weapon types whose names have the keyword".SubstituteANSIColour());
        ...
        IEnumerable<IAuxiliaryCombatAction> actions = actor.Gameworld.AuxiliaryCombatActions.AsEnumerable();
        while (!ss.IsFinished)
        {
            string arg = ss.PopSpeech();
            if (arg[0] == '+' && arg.Length > 1)
    ...
    private static void AuxiliaryNew(ICharacter actor, StringStack ss)
    {
        ...
        AuxiliaryCombatAction newAction = new(name, actor.Gameworld);
        actor.Gameworld.Add(newAction);
        actor.RemoveAllEffects(x => x.IsEffectType<BuilderEditingEffect<IAuxiliaryCombatAction>>());
        actor.AddEffect(new BuilderEditingEffect<IAuxiliaryCombatAction>(actor) { EditingItem = newAction });
...
You can also use the following options to filter searches:
    ...
    private static void CombatMessageView(ICharacter actor, StringStack ss)
    {
        ...
        {
            message = long.TryParse(ss.PopSpeech(), out long value)
	...
	#6&<type>#0 - filters messages for a particular type");
        ...
        IEnumerable<ICombatMessage> messages = actor.Gameworld.CombatMessageManager.CombatMessages.AsEnumerable();
        while (!ss.IsFinished)
        {
            ...
            {
                text = text[1..];
                IWeaponAttack attack = long.TryParse(text, out long value)
                    ? actor.Gameworld.WeaponAttacks.Get(value)
            ...
            {
                text = text[1..];
                if (!text.TryParseEnum<BuiltInCombatMoveType>(out BuiltInCombatMoveType type))
                {
                    actor.OutputHandler.Send(
            ...
            if (!text.TryParseEnum<MeleeWeaponVerb>(out MeleeWeaponVerb verb))
            {
                actor.OutputHandler.Send(
    ...
    private static void CombatMessageClone(ICharacter actor, StringStack ss)
    {
        ...
        ICombatMessage message = long.TryParse(ss.PopSpeech(), out long value)
            ? actor.Gameworld.CombatMessageManager.CombatMessages.FirstOrDefault(x => x.Id == value)
            : default;
        if (message == null)
        {
        ...
        CombatMessage newmessage = new((CombatMessage)message);
    ...
    private static void CombatMessageDelete(ICharacter actor, StringStack ss)
    {
        ...
        ICombatMessage cm = actor.EffectsOfType<BuilderEditingEffect<ICombatMessage>>().First().EditingItem;
    ...
    {
        CombatMessage newmessage = new(actor.Gameworld);
        actor.Gameworld.CombatMessageManager.AddCombatMessage(newmessage);
        actor.RemoveAllEffects(x => x.IsEffectType<BuilderEditingEffect<ICombatMessage>>());
        actor.AddEffect(new BuilderEditingEffect<ICombatMessage>(actor) { EditingItem = newmessage });
        actor.OutputHandler.Send(
