    ...
    protected static void Burden(ICharacter actor, string command)
    {
        ...
        switch (ss.PopSpeech().ToLowerInvariant())
        {
            ...
            case "defense":
                ...
                break;
            case var degText when int.TryParse(degText, out int value):
                degrees = value;
                offense = true;
                defense = true;
                if (value < 0 || value > 10)
...
You can use the command in one of three ways:
    ...
    protected static void Release(ICharacter actor, string command)
    {
        ...
        PlayerEmote emote = new(ss.PopParentheses(), actor);
        if (!emote.Valid)
        {
        ...
        IGrappling grapple = actor.EffectsOfType<IGrappling>().FirstOrDefault();
        Dragging drag = actor.EffectsOfType<Dragging>().FirstOrDefault();
        IEffect effect = (IEffect)grapple ?? drag;
        ICharacter target = grapple?.Target ?? drag.Target as ICharacter;
...
The syntax is one of the following:
    ...
    {
        List<IEffect> blockingEffects = actor.CombinedEffectsOfType<IEffect>()
        ...
        if (ss.IsFinished)
        {
            ...
            {
                WarnedByEnforcer wbe = actor.EffectsOfType<WarnedByEnforcer>().First();
        ...
        else
        {
            ...
            (bool truth, string error) = CanBeSurrenderedTo(target, actor);
            if (!truth)
            {
        ...
        if (target.Combat != null)
        {
            ...
            Grappling grapple = new(target, actor);
            target.AddEffect(grapple);
            foreach (Body.ILimb limb in actor.Body.Limbs.Where(x => actor.Body.BodypartsForLimb(x).Any()).ToList())
            {
        ...
        {
            Dragging drag = new(target, null, actor);
    ...
    protected static void Grapple(ICharacter actor, string command)
    {
        ...
        CombatStrategyMode desiredStrategy = CombatStrategyMode.GrappleForControl;
        switch (ss.PopSpeech().ToLowerInvariant())
        {
            case "kill":
            case "strangle":
...
The syntax for the smash command is as follows:
    ...
    protected static void Smash(ICharacter actor, string command)
    {
        ...
        Body.IBodypart part = actor.Body.Prototype.DefaultDoorSmashingPart;
        bool designatedPart = false;
        if (!ss.IsFinished)
        {
            ss.PopSpeech();
            if (ss.IsFinished)
            ...
            if (targetWeapon != null)
            {
                ...
                List<IWeaponAttack> attacks = targetAsWeapon.WeaponType.UsableAttacks(actor, targetWeapon,
                ...
                IWeaponAttack attack = attacks.Where(x => actor.CanSpendStamina(x.StaminaCost))
        ...
        {
            IMeleeWeapon possibleWeapon = actor.Body.WieldedItems
        ...
        if (targetAsWeapon != null)
        {
            ...
            MeleeWeaponSmashItemAttack move = new(attack)
            {
                Assailant = actor,
                Target = targetLock?.Parent ?? targetItem,
                ParentItem = targetLock == null ? null : targetItem,
                Weapon = targetAsWeapon
            };
        ...
        IEnumerable<INaturalAttack> nattacks =
        ...
        UnarmedSmashItemAttack nmove = new(nattack.Attack)
        {
            Assailant = actor,
            Target = targetLock?.Parent ?? targetItem,
            ParentItem = targetLock == null ? null : targetItem,
        ...
        CombatMoveResult nresult = nmove.ResolveMove(null);
...
The syntax:
    ...
    {
        StringStack ss = new(command.RemoveFirstWord());
        List<IMeleeWeapon> weapons = actor.Body.WieldedItems.SelectNotNull(x => x.GetItemType<IMeleeWeapon>()).ToList();
        ...
        if (ss.Peek().Equals("list", StringComparison.InvariantCultureIgnoreCase))
        {
            ...
            StringBuilder sb = new();
            foreach (IMeleeWeapon item in weapons)
            {
                if (sb.Length > 0)
                {
                    sb.AppendLine();
                ...
                sb.AppendLine();
                foreach (
                    IFixedBodypartWeaponAttack attack in
        ...
        string attackText = ss.PopSpeech();
...
The syntax is #3rescue <target>#0 or #3rescue none#0 to stop trying to rescue them.", AutoHelp.HelpArgOrNoArg)]
    ...
    {
        Rescue effect = actor.EffectsOfType<Rescue>().FirstOrDefault();
        StringStack ss = new(command.RemoveFirstWord());
        ...
        if (actor.Combat == null)
        {
            ...
            List<IPerceiver> potentialTargets =
...
Similarly to reverse the above you can use the following:
    ...
    protected static void Guard(ICharacter actor, string command)
    {
        ...
        if (ss.IsFinished)
        {
            IGuardCharacterEffect guardCharacter = actor.EffectsOfType<IGuardCharacterEffect>().FirstOrDefault();
            ...
            IGuardItemEffect guardItem = actor.EffectsOfType<IGuardItemEffect>().FirstOrDefault();
            if (guardItem != null)
            {
                actor.Send(
            ...
            IGuardExitEffect guardExit = actor.EffectsOfType<IGuardExitEffect>().FirstOrDefault();
            if (guardExit != null)
            {
                StringBuilder sb = new();
                ...
                if (guardExit.Exemptions.Any())
                {
                    ...
                    foreach ((long Id, string Description) exemption in guardExit.Exemptions)
                    {
        ...
        IGuardExitEffect guardExitEffect = actor.EffectsOfType<IGuardExitEffect>().FirstOrDefault();
        if (targetText.EqualTo("permit"))
        {
            ...
            {
                string dubText = ss.SafeRemainingArgument[1..];
                ...
                IDub targetDub = actor.Dubs.GetFromItemListByKeyword(dubText, actor);
                if (targetDub == null)
                {
            ...
            ICharacter targetCharacter = actor.TargetActor(ss.SafeRemainingArgument);
        ...
        IPerceivable target = actor.TargetLocal(targetText);
        if (target == null)
        {
        ...
        {
            IGuardCharacterEffect guardEffect = targetActor.EffectsOfType<IGuardCharacterEffect>().FirstOrDefault();
            if (guardEffect == null)
            {
...
The syntax for this command is #3support <target>#0.", AutoHelp.HelpArgOrNoArg)]
    protected static void Support(ICharacter actor, string command)
    {
        ...
        ICharacter target = actor.TargetAlly(ss.PopSpeech()) ?? actor.TargetActor(ss.Last);
        if (target == null)
        {
...
#6location#0 room, #6perceiver#0 attacker, #6perceiver#0 target", AutoHelp.HelpArg)]
    ...
    protected static void Peace(ICharacter actor, string command)
    {
        ...
        if (ss.IsFinished)
        {
            List<ICombat> combats = actor.Location.Characters.SelectNotNull(x => x.Combat).Distinct().ToList();
            foreach (ICombat combat in combats)
            {
        ...
        if (ss.Peek().StartsWith("permanent", StringComparison.InvariantCultureIgnoreCase))
        {
            ...
            if (!ss.IsFinished)
            {
                prog = long.TryParse(ss.PopSpeech(), out long value)
...
The syntax is as follows:
    ...
    protected static void Load(ICharacter actor, string command)
    {
        ...
        IRangedWeapon targetWeapon = target.GetItemType<IRangedWeapon>();
        if (targetWeapon == null)
        {
    ...
    protected static void Aim(ICharacter actor, string command)
    {
        ...
        else
        {
            ...
            if (targetText.EqualToAny("me", "self") && actor.Combat == null)
            {
                ...
                Body.IBodypart bodypart = actor.Body.GetTargetBodypart(ss.PopSpeech());
                if (bodypart == null)
                {
    ...
    protected static void Fire(ICharacter actor, string command)
    {
        ...
        OutOfCombatAim aiming = actor.EffectsOfType<OutOfCombatAim>().FirstOrDefault();
        if (aiming == null)
        {
        ...
        actor.Aim = aiming.Aim;
        RangedWeaponAttackMove attack = new(actor, aiming.Target, aiming.Weapon);
    ...
    protected static void Cover(ICharacter actor, string command)
    {
        ...
        {
            CombatantCover cover = TakeCover.GetCoverFor(actor, !ss.IsFinished);
            if (cover == null)
            {
                actor.Send(
    ...
    protected static void Charge(ICharacter actor, string command)
    {
        ...
        StringStack ss = new(command.RemoveFirstWord());
        ICharacter target = actor.CombatTarget as ICharacter;
        if (!ss.IsFinished)
        {
  ...
  #3target none#0 - stops targeting any specific bodypart", AutoHelp.HelpArg)]
    protected static void Target(ICharacter actor, string command)
    {
        ...
        if (ss.IsFinished)
        {
            ...
            {
                IPerceiver target = actor.CombatTarget ?? actor.EffectsOfType<OutOfCombatAim>().FirstOrDefault()?.Target ??
        ...
        ICharacter tch = actor.CombatTarget as ICharacter ??
        ...
        Body.IBodypart part = tch.Body.GetTargetBodypart(ss.SafeRemainingArgument);
        if (part == null)
        {
    ...
    {
        IEnumerable<ICharacterCombatSettings> settings = GetSettingsForCharacter(actor, command.PopSpeech());
    ...
    protected static void CombatShow(ICharacter actor, StringStack command)
    {
        ...
        {
            setting = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    protected static void CombatClone(ICharacter actor, StringStack command)
    {
        ...
        IEnumerable<ICharacterCombatSettings> potentialSettings = GetSettingsForCharacter(actor, string.Empty);
        ICharacterCombatSettings setting = long.TryParse(command.PopSpeech(), out long value)
        ...
        string newName = command.SafeRemainingArgument;
        ...
        CharacterCombatSettings newSetting = new(actor, (CharacterCombatSettings)setting, newName);
...
The syntax to use this command is as follows:");
    ...
    private static void CombatSet(ICharacter actor, StringStack ss)
    {
        ...
        IEnumerable<ICharacterCombatSettings> potentialSettings = GetSettingsForCharacter(actor, string.Empty);
        ICharacterCombatSettings setting = long.TryParse(ss.PopSpeech(), out long value)
            ? potentialSettings.FirstOrDefault(x => x.Id == value)
            : potentialSettings.FirstOrDefault(
...
".SubstituteANSIColour());
    ...
    private static void CombatConfigSwapOrder(ICharacter actor, StringStack command)
    {
        ...
        if (!Enum.TryParse<MeleeAttackOrderPreference>(command.PopSpeech(), true, out MeleeAttackOrderPreference pref1))
        {
            actor.OutputHandler.Send(
        ...
        if (!Enum.TryParse<MeleeAttackOrderPreference>(command.PopSpeech(), true, out MeleeAttackOrderPreference pref2))
        {
            actor.OutputHandler.Send(
    ...
    {
        if (command.IsFinished ||
            !command.SafeRemainingArgument.TryParsePercentage(actor.Account.Culture, out double value))
        {
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.AutomaticallyMoveTowardsTarget);
        if (truth == null)
        {
            return;
        }
    ...
    {
        bool? truth = GetTruthValue(actor, command, !actor.CombatSettings.PreferNonContactClinchBreaking);
        if (truth == null)
        {
            return;
        }
    ...
    private static void CombatConfigForbid(ICharacter actor, StringStack command)
    {
        ...
        if (!CombatExtensions.TryParseCombatMoveIntention(command.PopSpeech(), out CombatMoveIntentions intention))
        {
            actor.Send(StringUtilities.HMark +
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.PreferFavouriteWeapon);
        if (truth == null)
        {
            return;
        }
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.PreferShieldUse);
        if (truth == null)
        {
            return;
        }
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.FallbackToUnarmedIfNoWeapon);
        if (truth == null)
        {
            return;
        }
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.AttackHelpless);
        if (truth == null)
        {
            return;
        }
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.AttackCriticallyInjured);
        if (truth == null)
        {
            return;
        }
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.SkirmishToOtherLocations);
        if (truth == null)
        {
            return;
        }
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.PreferToFightArmed);
        if (truth == null)
        {
            return;
        }
    ...
    private static void CombatConfigPriorityProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Number,
    ...
    private static void CombatConfigCharacter(ICharacter actor, StringStack command)
    {
        ...
        ICharacter target = actor.Gameworld.Actors.GetByName(command.PopSpeech()) ??
    ...
    {
        List<WeaponClassification> classifications = Enum.GetValues(typeof(WeaponClassification)).OfType<WeaponClassification>().ToList();
    ...
    {
        bool? truth = GetTruthValue(actor, command, actor.CombatSettings.ManualPositionManagement);
        if (truth == null)
        {
            return;
        }
    ...
    private static void CombatConfigMeleeStrategy(ICharacter actor, StringStack command)
    {
        List<CombatStrategyMode> meleeStrategies = Enum.GetValues(typeof(CombatStrategyMode)).OfType<CombatStrategyMode>()
                                  ...
                                  .ToList();
        string validStrategyText = StringUtilities.GetTextTable(
        ...
        string stratName = command.SafeRemainingArgument;
        if (!stratName.TryParseEnum<CombatStrategyMode>(out CombatStrategyMode value))
        {
            actor.OutputHandler.Send(
    ...
    {
        List<CombatStrategyMode> rangeStrategies = Enum.GetValues(typeof(CombatStrategyMode)).OfType<CombatStrategyMode>()
    ...
    private static void CombatConfigName(ICharacter actor, StringStack command)
    {
        ...
        string oldName = actor.CombatSettings.Name;
    ...
    private static void CombatConfigGrapple(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<GrappleResponse>(out GrappleResponse value))
        {
            actor.OutputHandler.Send(
    ...
    protected static void CombatPreferredDefense(ICharacter actor, StringStack command)
    {
        ...
        if (!CombatExtensions.TryParseDefenseType(command.SafeRemainingArgument, out DefenseType type))
        {
