...
internal class CommunicationsModule : Module<ICharacter>
{
    ...
    protected static void Feel(ICharacter actor, string input)
    {
        ...
        PlayerEmote feelEmote = new(ss.RemainingArgument, actor);
        if (!feelEmote.Valid)
        {
    ...
    protected static void Languages(ICharacter actor, string input)
    {
        ...
        sb.AppendLine();
        foreach (ILanguage lang in actor.Languages)
        {
    ...
    protected static void PreferAccent(ICharacter actor, string input)
    {
        ...
        string accentText = ss.PopSpeech();
        string languageText = ss.PopSpeech();

        IAccent accent = actor.Accents.FirstOrDefault(x => x.Name.EqualTo(accentText) &&
    ...
    protected static void Accents(ICharacter actor, string input)
    {
        ...
        {
            ILanguage lang = actor.Body.Languages.FirstOrDefault(x =>
        ...
        sb.AppendLine();
        foreach (IGrouping<ILanguage, IAccent> language in accents.GroupBy(x => x.Language))
        {
            IAccent preferred = actor.PreferredAccent(language.Key);
...
If you care about getting grammatically correct echoes to yourself (for log purposes etc), consider the difference between the below two examples:
    ...
    protected static void SingTo(ICharacter actor, string input)
    {
        ...
        IPerceivable ptarget = actor.Target(target);
        if (ptarget == null)
        {
...
If exchanges have been linked together, you can dial another exchange by using its full prefix as an area code. Calls will ring out automatically after the destination exchange's configured maximum number of rings.
  ...
  The syntax is:
    ...
    protected static void Dial(ICharacter actor, string input)
    {
        ...
        if (!TryGetTelephone(actor, ss.PopSpeech(), "use", out IGameItem item, out ITelephone telephone, out string error))
        {
            actor.OutputHandler.Send(error);
            return;
        }
        ...
        string number = ss.SafeRemainingArgument;
...
The syntax is:
    ...
    protected static void Fax(ICharacter actor, string input)
    {
        ...
        if (!TryGetFaxMachine(actor, ss.PopSpeech(), "use", out IGameItem item, out IFaxMachine faxMachine, out string error))
        {
            actor.OutputHandler.Send(error);
            return;
        }
        ...
        IGameItem documentItem = actor.TargetHeldItem(ss.SafeRemainingArgument);
        ...
        IReadable readable = documentItem.GetItemType<IReadable>();
        if (readable == null)
        {
...
The syntax is:
    ...
    protected static void Answer(ICharacter actor, string input)
    {
        ...
        if (!TryGetTelephone(actor, ss.SafeRemainingArgument, "answer", out IGameItem item, out ITelephone telephone, out string error))
        {
            actor.OutputHandler.Send(error);
            return;
        }
...
The syntax is:
    ...
    protected static void PickUp(ICharacter actor, string input)
    {
        ...
        if (!TryGetTelephone(actor, ss.SafeRemainingArgument, "pick up", out IGameItem item, out ITelephone telephone,
                out string error))
        {
            actor.OutputHandler.Send(error);
            return;
...
The syntax is:
    ...
    protected static void HangUp(ICharacter actor, string input)
    {
        ...
        if (!TryGetTelephone(actor, ss.SafeRemainingArgument, "hang up", out IGameItem item, out ITelephone telephone,
                out string error))
        {
            actor.OutputHandler.Send(error);
            return;
        ...
        out ITelephone telephone, out string error)
    {
        ...
        (bool truth, string manipulationError) = actor.CanManipulateItem(item);
        if (!truth)
        {
            telephone = null;
...
You can use the following syntax with this command:
    ...
    private static void BoardLook(ICharacter actor, StringStack input)
    {
        ...
        {
            IGameItem target = actor.TargetLocalItem(input.SafeRemainingArgument);
            if (target is null)
            {
    ...
    private static void BoardDelete(ICharacter actor, StringStack input)
    {
        ...
        if (!int.TryParse(input.SafeRemainingArgument, out int value))
        {
            actor.OutputHandler.Send("That is not a valid number.");
            return;
        }

        IBoardPost post = board.Board.Posts.ElementAtOrDefault(value - 1);
        if (post is null)
        {
    ...
    private static void BoardCreate(ICharacter actor, StringStack ss)
    {
        ...
        ICalendar calendar = actor.Gameworld.Calendars.GetByIdOrNames(ss.PopSpeech());
        if (calendar is null)
        {
        ...
        using (new FMDB())
        {
            ...
            Board board = new(dbBoard, actor.Gameworld);
    ...
    private static void PostAction(string text, IOutputHandler handler, object[] args)
    {
        ...
        string title = ((string)args[1]).TitleCase();
        IBoardItem board = (IBoardItem)args[2];
    ...
    private static void BoardView(ICharacter actor, StringStack input)
    {
        ...
        if (!long.TryParse(input.SafeRemainingArgument, out long value))
        {
            actor.Send("That is not a valid ID for a post to view.");
            return;
        }

        IBoardPost post = actor.Gameworld.Boards.SelectMany(x => x.Posts).FirstOrDefault(x => x.Id == value);
        if (post == null)
        {
    ...
    private static void BoardList(ICharacter actor, StringStack input)
    {
        ...
        IBoard board = actor.Gameworld.Boards.GetByName(input.PopSpeech());
        if (board == null)
        {
        ...
        List<IBoardPost> offsetPosts = board.Posts.Reverse().Skip((int)offset).ToList();
        List<IBoardPost> posts = offsetPosts.Take(actor.Account.PageLength - 3).ToList();
        StringBuilder sb = new();
        sb.AppendLine($"The {board.Name.Colour(Telnet.Green)} board has the following posts:");
        foreach (IBoardPost post in posts)
        {
            string leadin =
    ...
    protected static void Boards(ICharacter actor, string input)
    {
        ...
        foreach (IBoard board in actor.Gameworld.Boards)
        {
