...
The syntax is as follows:
    ...
    protected static void Crimes(ICharacter actor, string input)
    {
        ...
        if (ss.IsFinished)
        {
            List<(ILegalAuthority Authority, List<ICrime> Known, List<ICrime> Unknown, List<ICrime> Resolved)> legalAuthorities = actor.Gameworld.LegalAuthorities
            ...
            foreach ((ILegalAuthority authority, List<ICrime> known, List<ICrime> unknown, List<ICrime> resolved) in legalAuthorities)
            {
                sb.AppendLine(
                    $"You have committed the following crimes in the {authority.Name.ColourName()} jurisdiction:");
                sb.AppendLine();
                List<ICrime> combined = known
                               .Concat(unknown)
                               .Concat(resolved)
        ...
        if (target == null)
        {
            if (actor.IsAdministrator() && long.TryParse(ss.Last, out long id))
            {
        ...
        foreach ((ILegalAuthority authority, List<ICrime> known, List<ICrime> unknown, List<ICrime> resolved) in crimes)
        {
            ...
            sb.AppendLine();
            List<ICrime> combined = new(known);
...
The syntax for this command is as follows:
    ...
    protected static void Rapsheet(ICharacter actor, string input)
    {
        ...
        StringBuilder sb = new();
        List<(ILegalAuthority Authority, List<ICrime> Known, List<ICrime> Resolved)> crimes = jurisdictions
        ...
        foreach ((ILegalAuthority authority, List<ICrime> known, List<ICrime> resolved) in crimes)
        {
            ...
            sb.AppendLine($"The crimes of {target.HowSeen(actor)} in the {authority.Name.ColourName()} jurisdiction:");
            sb.AppendLine();
            List<ICrime> combined = known.Concat(resolved).OrderBy(x => x.RealTimeOfCrime).ToList();
            sb.AppendLine(StringUtilities.GetTextTable(
                from crime in combined
                select new List<string>
                {
 ...
 The syntax is as follows:
    ...
    protected static void Report(ICharacter actor, string input)
    {
        ...
        if (ss.IsFinished)
        {
            ...
            foreach (ICrime accusable in accusableCrimes)
            {
                sb.AppendLine(
        ...
        ICrime crime = accusableCrimes.FirstOrDefault(x => x.Id == value);
        if (crime == null)
        {
            actor.OutputHandler.Send(
...
The syntax to use this command is:
    ...
    {
        List<ILegalAuthority> legals = actor.Gameworld.LegalAuthorities
                          .Where(x =>
        ...
        ICharacter who = actor.TargetActor(ss.PopSpeech());
        if (who == null)
        {
            if (actor.IsAdministrator() && long.TryParse(ss.Last, out long id))
            {
        ...
        if (ss.IsFinished)
        {
            IEnforcementAuthority enf = legal.GetEnforcementAuthority(actor);
            actor.OutputHandler.Send($"What crime do you want to accuse {who.HowSeen(actor)} of?\nYou could accuse the following crimes: {legal.Laws
        ...
        if (!actor.IsAdministrator())
        {
            IEnforcementAuthority enforcer = legal.GetEnforcementAuthority(actor);
...
#6Note: Admins can use character ID instead of a target keyword in the above.#0", AutoHelp.HelpArg)]
    protected static void Forgive(ICharacter actor, string input)
    {
        ...
        List<ICrime> crimes = jurisdictions.SelectMany(x => x.KnownCrimesForIndividual(who)).ToList();
        if (actor.IsAdministrator())
        {
        ...
        if (ss.PeekSpeech().EqualTo("all"))
        {
            ...
            foreach (ILegalAuthority jurisdiction in jurisdictions)
            {
        ...
        ICrime forgivencrime = crimes.FirstOrDefault(x => x.Id == value);
...
The syntax is #3plead guilty#0 or #3plead innocent#0.", AutoHelp.HelpArgOrNoArg)]
    ...
    {
        StringStack ss = new(input.RemoveFirstWord());
        ConsideringPlea pleaEffect = actor.EffectsOfType<ConsideringPlea>().FirstOrDefault();
        ...
        OnTrial trialEffect = actor.EffectsOfType<OnTrial>(x => x.LegalAuthority == pleaEffect.LegalAuthority).FirstOrDefault();
        ...
        string plea = ss.PopSpeech().ToLowerInvariant().CollapseString();
...
The syntax for this command is simply #3argue defense#0 or #3argue prosecution#0.", AutoHelp.HelpArg)]
    protected static void Argue(ICharacter actor, string input)
    {
        ...
        bool defense = ss.PopSpeech().EqualToAny("defense", "defence");
        if (defense)
        {
...
You can use the following options for punishments, can can include multiples in your conviction:
    ...
    {
        StringStack ss = new(input.RemoveFirstWord());
        List<ILegalAuthority> authorities = actor.Gameworld.LegalAuthorities.WhereNotNull(x => x.GetEnforcementAuthority(actor))
                               .ToList();
        ILegalAuthority jurisdiction = authorities.FirstOrDefault(x => x.CourtLocation == actor.Location);
        ...
        List<ICrime> crimes = jurisdiction.KnownCrimesForIndividual(target).ToList();
        if (crimes.Count == 0)
        {
        ...
        ICrime targetCrime = crimes.GetByIdOrName(ss.PopSpeech());
        ...
        PunishmentOptions options = targetCrime.Law.PunishmentStrategy.GetOptions(target, targetCrime);
        if (ss.IsFinished)
        {
        ...
        PunishmentResult punishment = new();
        while (!ss.IsFinished)
        {
            switch (ss.PopForSwitch())
            {
                ...
                case "fine":
                    ...
                    if (!jurisdiction.Currency.TryGetBaseCurrency(amountText, out decimal amount))
                    {
                ...
                case "imprisonment":
                    ...
                    if (!MudTimeSpan.TryParse(ss.PopSpeech(), actor, out MudTimeSpan ts))
                    {
...
The syntax is #3crimeinfo <id>#0.", AutoHelp.HelpArg)]
    protected static void CrimeInfo(ICharacter actor, string input)
    {
        ...
        ICrime crime = actor.Gameworld.Crimes.GetById(ss.SafeRemainingArgument);
        if (crime is null ||
        (
...
There are the following syntaxes for this command:
    ...
    protected static void TakeCustody(ICharacter actor, string input)
    {
        ...
        InCustodyOfEnforcer existing = target.CombinedEffectsOfType<InCustodyOfEnforcer>()
    ...
    protected static void EndCustody(ICharacter actor, string input)
    {
        ...
        foreach (InCustodyOfEnforcer effect in effects)
        {
...
The syntax is either #3patrols#0 or #3patrols <jurisdiction>#0 if you are an enforcer for multiple jurisdictions.", AutoHelp.HelpArg)]
    protected static void Patrols(ICharacter actor, string input)
    {
        ...
        if (!authorities.Any())
        {
            ...
            ILegalAuthority authority = actor.Gameworld.LegalAuthorities.GetByIdOrName(ss.SafeRemainingArgument);
            if (authority == null)
            {
        ...
        foreach (ILegalAuthority authority in authorities)
        {
            ...
            sb.AppendLine();
            foreach (IPatrol patrol in authority.Patrols)
            {
                sb.AppendLine(
    ...
    protected static void PermitWork(ICharacter actor, string input)
    {
        ...
        bool property = ss.PopSpeech().EqualTo("permitworkproperty");
        ICharacter target = actor.TargetActor(ss.PopSpeech());
        if (target == null)
        {
        ...
        if (!TimeSpanParser.TryParse(ss.SafeRemainingArgument, Units.Hours, Units.Hours, out TimeSpan timespan))
        {
            actor.OutputHandler.Send(
...
The syntax for this command is simply #3requesttrial#0.", AutoHelp.HelpArg)]
    protected static void RequestTrial(ICharacter actor, string input)
    {
        ...
        IEnumerable<AwaitingSentencing> sentences = actor.EffectsOfType<AwaitingSentencing>();
...
The syntax is as follows:
    ...
    protected static void BailOut(ICharacter actor, string input)
    {
        ...
        {
            ss.PopSpeech();
            AwaitingSentencing effect = actor.EffectsOfType<AwaitingSentencing>().FirstOrDefault();
            if (effect is null)
            {
            ...
            ILegalAuthority authority = effect.LegalAuthority;
            ICurrency currency = authority.Currency;

            List<ICrime> crimes = authority.KnownCrimesForIndividual(actor).ToList();
            ...
            string bailText = string.Empty;
            decimal bail = crimes.Sum(x => authority.BailCalculationProg?.Execute<decimal?>(actor, x) ?? 0.0M);
            if (bail > 0.0M)
            {
                if (!ss.IsFinished)
                {
                    ...
                    (bool truth, string withdrawError) = account.CanWithdraw(bail, true);
                    if (!truth)
                    {
                ...
                else
                {
                    ...
                    List<ICurrencyPile> piles = container.Contents.RecursiveGetItems<ICurrencyPile>(false).ToList();
                    Dictionary<ICurrencyPile, Dictionary<ICoin, int>> found = currency.FindCurrency(piles, bail);
                    decimal value = found.Sum(x => x.Value.Sum(y => y.Value * y.Key.Value));
                    if (value < bail)
                    {
                        actor.OutputHandler.Send(
                    ...
                    decimal change = value - bail;

                    foreach (KeyValuePair<ICurrencyPile, Dictionary<ICoin, int>> item in found.Where(item =>
        ...
        ILegalAuthority jurisdiction = actor.Gameworld.LegalAuthorities.FirstOrDefault(x => x.PrisonLocation == actor.Location);
        if (jurisdiction is null)
        {
        ...
        if (whoText.EqualTo("list"))
        {
            ...
            sb.AppendLine();
            foreach (ICharacter prisoner in prisoners)
            {
                List<ICrime> crimes = jurisdiction.KnownCrimesForIndividual(prisoner).ToList();
                decimal prisonerBail = jurisdiction.KnownCrimesForIndividual(prisoner).Sum(x =>
        ...
        ICharacter who = prisoners.GetFromItemListByKeywordIncludingNames(whoText, actor);
        if (who is null)
        {
            actor.OutputHandler.Send(
        ...
        if (ss.IsFinished)
        {
            ...
            (bool truth, string withdrawError) = account.CanWithdraw(calculatedBail, true);
            if (!truth)
            {
                actor.OutputHandler.Send(withdrawError);
                return;
...
The syntax is simply #3returnbail#0.", AutoHelp.HelpArg)]
    protected static void ReturnBail(ICharacter actor, string input)
    {
        ...
        OnBail effect = actor.EffectsOfType<OnBail>(x => x.LegalAuthority == jurisdiction).FirstOrDefault();
        if (effect is null)
        {
            actor.OutputHandler.Send(
        ...
        actor.OutputHandler.Handle(new EmoteOutput(new Emote(actor.Gameworld.GetStaticString("ReturnFromBailEmote"),
            actor, actor)));
        decimal bailReturned = jurisdiction.KnownCrimesForIndividual(actor).Sum(x => x.CalculatedBail);
        ...
        if (bailReturned > 0.0M)
        {
            IGameItem bundle =
                jurisdiction.PrisonerBelongingsStorageLocation.GameItems.FirstOrDefault(x =>
                    x.AffectedBy<PrisonerBelongings>(actor));
            IGameItem money = GameItems.Prototypes.CurrencyGameItemComponentProto.CreateNewCurrencyPile(jurisdiction.Currency,
...
The syntax is as follows:
    ...
    {
        StringStack ss = new(input.RemoveFirstWord());
        ICharacter who = actor;
        List<(INPC NPC, LawyerAI AI)> courtLawyers = actor.Gameworld.Actors
                                    .OfType<INPC>()
        ...
        if (ss.IsFinished || ss.PeekSpeech().EqualTo("me"))
        {
            ...
            AwaitingSentencing sentencingEffect = actor.EffectsOfType<AwaitingSentencing>().FirstOrDefault();
            OnBail bailEffect = actor.EffectsOfType<OnBail>().FirstOrDefault();
        ...
        List<(INPC NPC, LawyerAI AI)> availableLawyers = courtLawyers.Where(x => x.AI.AvailableToHire(x.NPC, jurisdiction, false)).ToList();
        if (ss.IsFinished || ss.SafeRemainingArgument.EqualTo("list"))
        {
            ...
            StringBuilder sb = new("The following lawyers are available for hire:");
            sb.AppendLine();
            sb.AppendLine(StringUtilities.GetTextTable(
        ...
        INPC target = availableLawyers.Select(x => x.NPC).GetFromItemListByKeywordIncludingNames(ss.PopSpeech(), actor);
        if (target is null)
        {
        ...
        (INPC _, LawyerAI ai) = availableLawyers.First(x => x.NPC == target);
        decimal fee = Math.Truncate(ai.FeeProg.ExecuteDecimal(who, target));
        string actionText;
        IBankAccount lawyerBankAccount = ai.BankAccountProg?.Execute<IBankAccount>(target);
        if (!ss.IsFinished)
        {
            (IBankAccount account, string error) = Bank.FindBankAccount(ss.SafeRemainingArgument, null, actor);
            if (account is null)
            ...
            (bool truth, string withdrawError) = account.CanWithdraw(fee, true);
            if (!truth)
            {
                actor.OutputHandler.Send(withdrawError);
                return;
...
The syntax is as follows:
    ...
    protected static void PayFine(ICharacter actor, string input)
    {
        ...
        (decimal fine, MudDateTime _) = jurisdiction.FinesOwed(actor);
        ...
        List<ICrime> crimesWithFines = jurisdiction.ResolvedCrimesForIndividual(actor).Where(x => x.FineRecorded > 0.0M && !x.FineHasBeenPaid).ToList();
        ...
        {
            ICrime crime = crimesWithFines.GetByIdOrName(ss.PopSpeech());
            if (crime is null)
            {
        ...
        if (!ss.IsFinished)
        {
            ...
            (bool truth, string withdrawError) = account.CanWithdraw(fine, true);
            if (!truth)
            {
                actor.OutputHandler.Send(withdrawError);
                return;
        ...
        (fine, MudDateTime date) = jurisdiction.FinesOwed(actor);
        if (fine > 0.0M)
        {
