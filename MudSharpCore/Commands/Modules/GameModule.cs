...
internal class GameModule : Module<ICharacter>
{
    ...
    protected static void SkillLevels(ICharacter actor, string command)
    {
        ...
        List<ITraitDefinition> traits = actor.IsAdministrator()
            ? actor.Gameworld.Traits.ToList()
            : actor.Traits.Select(x => x.Definition).ToList();
        ITraitDefinition trait = traits.FirstOrDefault(x => x.Name.EqualTo(traitName)) ??
        ...
        sb.AppendLine(":");
        foreach (string level in trait.Decorator.OrderedDescriptors)
        {
...
This command exists to let players coordinate between one another, but overuse of this command is discouraged.", AutoHelp.HelpArg)]
    protected static void Notify(ICharacter actor, string command)
    {
        ...
        {
            List<INotifyEffect> effects = actor.EffectsOfType<INotifyEffect>().ToList();
            if (!effects.Any())
            {
            ...
            INotifyEffect effect = effects.First();
        ...
        if (clan == null)
        {
            IEnumerable<ICharacter> characters = actor.Gameworld.Characters.Except(actor).GetAllByName(ss.Last);
...
For a full list of combat flags, see #3SHOW COMBATFLAGS#0", AutoHelp.HelpArg)]
    protected static void Attacks(ICharacter actor, string input)
    {
        ...
        if (ss.IsFinished)
        {
            ...
            foreach (IMeleeWeapon weapon in actor.Body.WieldedItems.SelectNotNull(x => x.GetItemType<IMeleeWeapon>()))
            {
                AddAttacksForWeapon(weapon);
            }

            foreach (IImplantMeleeWeapon weapon in actor.Body.Implants.OfType<IImplantMeleeWeapon>())
            {
                AddAttacksForWeapon(weapon);
            }
        ...
        IMeleeWeapon targetAsWeapon = target.GetItemType<IMeleeWeapon>();
    ...
    protected static void Keyword(ICharacter actor, string input)
    {
        ...
        string[] strings = input.Split(".".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
        ...
        i = 1;
        foreach (ICharacter item in matchingChars)
        {
            sb.AppendLine($"\t{i++}: {item.HowSeen(actor)}");
        }
        ...
        i = 1;
        foreach (IGameItem item in matchingPersonalObjects.Concat(matchingRoomObjects))
        {
            sb.AppendLine($"\t{i++}: {item.HowSeen(actor)}");
        }
        ...
        i = 1;
        foreach (IGameItem item in matchingPersonalObjects)
        {
            sb.AppendLine($"\t{i++}: {item.HowSeen(actor)}");
        }
        ...
        i = 1;
        foreach (IGameItem item in matchingRoomObjects)
        {
            sb.AppendLine($"\t{i++}: {item.HowSeen(actor)}");
        }
    ...
    protected static void Statistics(ICharacter actor, string input)
    {
        ...
        using (new FMDB())
        {
            Models.WeeklyStatistic weeklyStats = actor.Gameworld.GameStatistics.GetOrCreateWeeklyStatistic();
    ...
    protected static void Who(ICharacter actor, string input)
    {
        List<ICharacter> whocharacters =
            actor.Gameworld.Characters
                 ...
                 .ToList();
        int count = whocharacters.Count;
        int guestCount = actor.Gameworld.Characters.Count(x => x.IsGuest);
        ...
        foreach (IClanMembership clan in actor.ClanMemberships.Where(x => x.Clan.ShowClanMembersInWho))
        {
            ...
            List<ICharacter> clanMembers = whocharacters.Where(x => x.ClanMemberships.Any(y => y.Clan == clan.Clan)).ToList();
        ...
        List<ICharacter> availableAdmins = actor.Gameworld.Characters.Where(x => x.AffectedBy<IAdminAvailableEffect>()).ToList();
...
The syntax for this command is as follows:
    ...
    protected static void Petition(ICharacter actor, string input)
    {
        ...
        string petitionText = ss.SafeRemainingArgument.ProperSentences().Fullstop();
        ...
        {
            List<ICharacter> staff = actor.Gameworld.Actors.Where(x => x.IsAdministrator() || x.AffectedBy<Switched>()).ToList();
            staff.Handle(
...
You can use the #3landmarks#0 syntax to see all landmarks that you know, and #3landmarks <which>#0 to see detailed information about a landmark, including directions to get there from where you are.", AutoHelp.HelpArg)]
    protected static void Landmarks(ICharacter actor, string input)
    {
        ...
        LandmarkEffect landmark = landmarks.First(x => x.Owner == target);
        ...
        int index = 1;
        foreach ((FutureProg.IFutureProg Prog, string Text) text in landmark.LandmarkDescriptionTexts.Where(x => x.Prog.ExecuteBool(actor)))
        {
            sb.AppendLine();
...
The syntax is #3tagsearch <tag>#0.", AutoHelp.HelpArgOrNoArg)]
    ...
    {
        string cmd = input.RemoveFirstWord();
        ITag tag = actor.Gameworld.Tags.GetByIdOrName(cmd);
        ...
        List<IGameItemProto> items = actor.Gameworld.ItemProtos.Where(x =>
    ...
    protected static void Learn(ICharacter actor, string input)
    {
        ...
        NoAccentGain accent = actor.EffectsOfType<NoAccentGain>().FirstOrDefault();
        List<INoTraitGainEffect> traits = actor.EffectsOfType<INoTraitGainEffect>().ToList();
        ...
        {
            List<(ISkillDefinition Skill, int Attempts)> skills = branch.GetSkills().ToList();
            if (skills.Any())
            {
            ...
            List<(IKnowledge Knowledge, double Lessons)> knowledges = branch.GetKnowledges().ToList();
            if (knowledges.Any())
            {
                foreach ((IKnowledge Knowledge, double Lessons) knowledge in knowledges)
                {
                    double ratio = knowledge.Lessons / knowledge.Knowledge.LearnerSessionsRequired;
                    if (ratio >= 0.75)
                    {
                        sb.AppendLine(
        ...
        List<IAccent> realAccents = accent?.Accents.Where(x => actor.Languages.Contains(x.Language)).ToList() ??
                          new List<IAccent>();
        if (realAccents.Any())
        {
            sb.AppendLine("You are not gaining any familiarity with the following accents:");

            foreach (IAccent item in realAccents)
            {
                sb.AppendLine(
...
You can also type 'forage' on its own to see what kinds of yields you can search for in the area.", AutoHelp.HelpArg)]
    ...
    {
        IForagableProfile profile = actor.Location.ForagableProfile;
        if (profile == null)
        {
    ...
    protected static void Implant(ICharacter actor, string input)
    {
        ...
        List<IImplantNeuralLink> dni = actor.Body.Implants.OfType<IImplantNeuralLink>().ToList();
        if (!dni.Any())
        {
            actor.OutputHandler.Send(
        ...
        {
            foreach (IImplantNeuralLink implant in dni)
            {
    ...
    protected static void Clean(ICharacter actor, string input)
    {
        ...
        IPerceivable target = actor.Target(ss.Last);
        if (target == null)
        {
    ...
    {
        List<IProposalEffect> effects = actor.EffectsOfType<IProposalEffect>().ToList();
        if (!effects.Any())
        {
        ...
        StringBuilder sb = new("You have the following pending transactions:\n\n");
        foreach (IProposalEffect effect in effects)
        {
    ...
    {
        IProposalEffect effect = actor.EffectsOfType<IProposalEffect>().FirstOrDefault();
        if (effect == null)
        {
            actor.OutputHandler.Send("You have no pending transactions.");
            return;
    ...
    {
        IProposalEffect item = actor.EffectsOfType<IProposalEffect>().GetFromItemListByKeyword(ss.PopSpeech(), actor);
        if (item == null)
        {
        ...
        string comment = ss.IsFinished ? "" : ss.SafeRemainingArgument;
    ...
    public static IComparer<string> CommandGroupComparer =>
        C5.ComparerFactory<string>.CreateComparer(
            (group1, group2) =>
            {
                int result1 = GetCommandGroupScore(group1);
                int result2 = GetCommandGroupScore(group2);
    ...
    protected static void _Commands(ICharacter actor, string input)
    {
        ...
        {
            StringBuilder sb = new();
            foreach (IGrouping<string, string> group in actor.CommandTree.Commands.ReportCommandsInGroups(
        ...
        Queue<ICleanableEffect> effectQueue, Queue<IPerceivable> itemQueue)
    {
        ...
        return item =>
        {
                  ...
                  )
            {
                if (!effectQueue.Any())
                {
                    ...
                    if (itemQueue.Any())
                    {
                        ...
                        IPerceivable newItem = itemQueue.Dequeue();
                        Queue<ICleanableEffect> newEffectQueue = GetCleanableEffectQueue(newItem, actor, false);
            ...
            IGameItem toolItem = effect[0].CleaningToolTag != null
            ...
            if (effect[0].LiquidRequired != null)
            {
                ...
                List<ILiquidContainer> potentialLiquidItems =
                    actor.ContextualItems.SelectNotNull(y => y.GetItemType<ILiquidContainer>())
                         ...
                         .ToList();
                Dictionary<ILiquidContainer, double> actualLiquidItems = new();
                Dictionary<LiquidMixture, double> actualLiquids = new();
                double targetAmount = effect[0].LiquidAmountConsumed;
                foreach (ILiquidContainer lcon in potentialLiquidItems)
                {
                    if (lcon.LiquidMixture.TotalVolume >= targetAmount)
                    {
                        actualLiquidItems[lcon] = targetAmount;
                        LiquidMixture newLiquid = lcon.RemoveLiquidAmount(targetAmount, actor, "clean");
                ...
                foreach (KeyValuePair<LiquidMixture, double> liquidUsed in actualLiquids)
                {
                ...
                List<IPerceivable> perceiverArgs = new()
                { actor, gitem, toolItem };
                ...
                foreach (KeyValuePair<LiquidMixture, double> liquid in actualLiquids)
                {
    ...
    private static Queue<ICleanableEffect> GetCleanableEffectQueue(IPerceivable target, ICharacter actor,
        bool echoFailure)
    {
        List<ICleanableEffect> originalCleanableEffects =
                target.EffectsOfType<ICleanableEffect>().Where(x => x.Applies(actor)).ToList();
        List<ICleanableEffect> cleanableEffects = originalCleanableEffects.ToList();
    ...
    {
        Queue<ICleanableEffect> effectQueue = GetCleanableEffectQueue(target, actor, true);
        if (!effectQueue.Any())
        {
            return;
        }
    ...
    {
        List<IGameItem> localItems = actor.Location.LayerGameItems(actor.RoomLayer)
                              .Where(x => x.Effects.Any(y => y is ICleanableEffect)).ToList();
        List<IGameItem> personalItems = actor.Body.ExternalItems.Where(x => x.Effects.Any(y => y is ICleanableEffect)).ToList();
        List<IGameItem> potentialItems = onlyMine ? personalItems : personalItems.Concat(localItems).ToList();
        ...
        List<IGameItem> cleanableItems = potentialItems.Where(x => GetCleanableEffectQueue(x, actor, false).Any()).ToList();
        ...
        Queue<ICleanableEffect> effectQueue = GetCleanableEffectQueue(cleanableItems.First(), actor, false);
        Queue<IPerceivable> itemQueue = new(cleanableItems.Skip(1));
        actor.AddEffect(
