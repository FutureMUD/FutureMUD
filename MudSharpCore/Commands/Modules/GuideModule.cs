...
The syntax to edit roles is as follows:
    ...
    {
        StringStack ss = new(input.RemoveFirstWord());
        string starget = ss.PopSpeech();
        IEnumerable<ICharacter> characters = actor.PermissionLevel > PermissionLevel.Guide
            ? actor.Gameworld.Characters
            : actor.Gameworld.Characters.Where(x => x.AffectedBy<INewPlayerEffect>());
        ICharacter target = (actor.PermissionLevel > PermissionLevel.Guide ? characters.GetByPersonalName(starget) : null) ??
    ...
    private static void ApplicationView(ICharacter character, StringStack command)
    {
        using (new FMDB())
        {
            ...
            chargen = long.TryParse(cmd, out long value)
            ...
            CharacterCreation.Chargen loadedChargen = new(chargen, character.Gameworld, chargen.Account);
    ...
    {
        IEnumerable<IChargenRole> roles = actor.Gameworld.Roles.Where(x => actor.PermissionLevel >= x.MinimumPermissionToView);
        while (!ss.IsFinished)
        {
            string text = ss.PopSpeech();
            if (text.TryParseEnum<ChargenRoleType>(out ChargenRoleType type))
            {
    ...
    private static void RoleShow(ICharacter actor, StringStack ss)
    {
        ...
        IEnumerable<IChargenRole> roles = actor.Gameworld.Roles.Where(x => x.MinimumPermissionToView <= actor.PermissionLevel);
    ...
    private static void RoleEdit(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IChargenRole> editing = actor.CombinedEffectsOfType<BuilderEditingEffect<IChargenRole>>().FirstOrDefault();
            if (editing is null)
            {
        ...
        IChargenRole role = actor.Gameworld.Roles.GetByIdOrName(ss.SafeRemainingArgument);
        if (role is null)
        {
    ...
    private static void RoleSet(ICharacter actor, StringStack ss)
    {
        ...
        IChargenRole role = editing.EditingItem;
    ...
    private static void RoleClone(ICharacter actor, StringStack ss)
    {
        ...
        IChargenRole role = actor.Gameworld.Roles.GetByIdOrName(ss.PopSpeech());
        if (role is null)
        {
            actor.OutputHandler.Send("There is no such role.");
            return;
        ...
        IChargenRole newRole = role.Clone(ss.SafeRemainingArgument.TitleCase());
    ...
    private static void RoleCreate(ICharacter actor, StringStack ss)
    {
        ...
        if (!ss.PopSpeech().TryParseEnum<ChargenRoleType>(out ChargenRoleType type))
        {
        ...
        ChargenRole role = new(actor.Account, name, type, actor.Gameworld);
