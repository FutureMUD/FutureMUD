...
internal class HealthModule : Module<ICharacter>
{
    ...
    protected static void Defibrillate(ICharacter actor, string command)
    {
        ...
        IDefibrillator defib = actor.Body.HeldOrWieldedItems.SelectNotNull(x => x.GetItemType<IDefibrillator>()).FirstOrDefault();
        if (defib == null)
        {
    ...
    protected static void Relocate(ICharacter actor, string command)
    {
        ...
        IBodypart targetPart = target.Body.GetTargetPart(ss.PopSpeech());
        if (targetPart == null)
        {
        ...
        {
            List<BoneFracture> wounds = target.Wounds.OfType<BoneFracture>().Where(x => candidateBones.Contains(x.Bone)).ToList();
            string displayPart = targetPart.FullDescription();
            if (!wounds.Any())
            {
            ...
            BoneFracture wound = candidateBones
...
Options:
    ...
    protected static void CleanWounds(ICharacter actor, string command)
    {
        ...
        WoundSeverity severity = WoundSeverity.None;
        bool useItems = true;
        while (!ss.IsFinished)
        {
            ...
            if (actor != target && !target.WillingToPermitMedicalIntervention(actor))
            {
                target.AddEffect(new Accept(target, new GenericProposal
                {
                    AcceptAction = text =>
                    {
                        ...
                        (bool canStillClean, CleaningWounds.PeekCanCleanReason stillReason) = CleaningWounds.PeekCanClean(actor, target, severity, useItems);
                        if (canStillClean)
                        {
    ...
    protected static void Suture(ICharacter actor, string command)
    {
        ...
        IInventoryPlan inventoryPlan = actor.Gameworld.SutureInventoryPlanTemplate.CreatePlan(actor);
    ...
    protected static void Tend(ICharacter actor, string command)
    {
        ...
        IInventoryPlan inventoryPlan = actor.Gameworld.TendInventoryPlanTemplate.CreatePlan(actor);
        switch (inventoryPlan.PlanIsFeasible())
        {
            case InventoryPlanFeasibility.NotFeasibleNotEnoughHands:
...
The syntaxes available include:
    ...
    protected static void Repair(ICharacter actor, string command)
    {
        ...
        if (ss.Peek().EqualTo("with"))
        {
            ...
            IGameItem kitItem = actor.TargetHeldItem(ss.PopSpeech());
            if (kitItem == null)
            {
        ...
        else if (ss.Peek().EqualTo("why"))
        {
            ...
            {
                (bool success, string message) = kit.CanRepair(wound);
                if (success)
                {
                    sb.AppendLine(
        ...
        ICharacter tch = target as ICharacter;
        List<IWound> twounds =
            (tch != null
    ...
    protected static void Dislodge(ICharacter actor, string command)
    {
        ...
        List<IWound> wounds = target.VisibleWounds(actor, WoundExaminationType.Examination).Where(x => x.Lodged != null)
                           .ToList();
        if (!wounds.Any())
        {
            actor.Send(
        ...
        List<(IWound Wound, IGameItem Lodged, ILodgeConsequence Consequence)> woundsAndLodged = wounds
        ...
        IGameItem targetLodged = woundsAndLodged.Select(x => x.Lodged).GetFromItemListByKeyword(ss.PopSpeech(), actor);
        ...
        (IWound Wound, IGameItem Lodged, ILodgeConsequence Consequence) targetWound = woundsAndLodged.First(x => x.Lodged == targetLodged);
        ...
        void RemoveLodged()
        {
            ...
            ICheck check = actor.Gameworld.GetCheck(CheckType.RemoveLodgedObjectCheck);
            CheckOutcome result = check.Check(actor, targetWound.Consequence?.DifficultyToRemove ?? Difficulty.Normal,
                target);
            IDamage damage = targetWound.Consequence?.GetDamageOnRemoval(targetWound.Wound, result);
            if (damage != null)
            {
        ...
        void BeginRemove()
        {
            CharacterActionWithTarget effect = new(actor, target, perceivable => RemoveLodged(),
    ...
    {
        List<(IWound Wound, IGameItem Lodged, ILodgeConsequence Consequence)> woundsAndLodged = target.Wounds.Where(x => x.Lodged != null).Select(x =>
...
The syntax is as follows:
    ...
    {
        List<IKnowledge> surgeryKnowledges = actor.Gameworld.SurgicalProcedures.Select(x => x.KnowledgeRequired).Distinct().ToList();
    ...
    private static void SurgeryEditNew(ICharacter actor, StringStack ss)
    {
        ...
        IBodyPrototype body = actor.Gameworld.BodyPrototypes.GetByIdOrName(ss.PopSpeech());
        if (body is null)
        {
        ...
        IKnowledge knowledge = actor.Gameworld.Knowledges.GetByIdOrName(ss.PopSpeech());
        ...
        if (ss.IsFinished)
        {
            List<string> existing = actor.Gameworld.SurgicalProcedures.Select(x => x.MedicalSchool).Distinct().ToList();
            if (existing.Any())
            {
    ...
    {
        BuilderEditingEffect<ISurgicalProcedure> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<ISurgicalProcedure>>().FirstOrDefault();
        if (effect is null)
        {
    ...
    private static void SurgeryEdit(ICharacter actor, StringStack ss)
    {
        ...
        ISurgicalProcedure surgery = actor.Gameworld.SurgicalProcedures.GetByIdOrName(ss.SafeRemainingArgument);
        if (surgery is null)
        {
    ...
    protected static void Triage(ICharacter actor, string command)
    {
        ...
        List<ISurgicalProcedure> procedures =
    ...
    {
        StringBuilder sb = new();
        List<string> wounds = actor.VisibleWounds(actor, WoundExaminationType.Self)
                          .Select(x => (Wound: x,
        ...
        if (actor.Body.SeveredRoots.Any())
        {
            ...
            List<IBodypart> insignificantSevers = actor.Body.SeveredRoots.Where(x => !x.Significant).ToList();
    ...
    private static void SurgeryPerform(ICharacter actor, StringStack ss)
    {
        ...
        string targetProcedure = ss.PopSpeech();
        ISurgicalProcedure procedure =
        ...
        {
            IPerceivable targetPerceivable = actor.TargetLocal(ss.PopSpeech());
        ...
        StringStack remains = new(ss.RemainingArgument);
    ...
    private static void SurgeryShow(ICharacter actor, StringStack ss)
    {
        ...
        string targetProcedure = ss.SafeRemainingArgument;
        ISurgicalProcedure procedure =
        ...
        if (procedure.Phases.Any())
        {
            ...
            int phaseNumber = 1;
            foreach (SurgicalProcedurePhase phase in procedure.Phases)
            {
                sb.AppendLine(
    ...
    protected static void Cure(ICharacter actor, string command)
    {
        ...
        IPerceivable target = ss.IsFinished ? actor : actor.Target(ss.PopSpeech());
        ...
        if (ss.Peek().EqualTo("infections"))
        {
            ...
            foreach (IInfection infection in tch.Body.PartInfections.ToList())
            {
        ...
        if (ss.Peek().EqualTo("bones"))
        {
            ...
            foreach (BoneFracture wound in tch.Body.Wounds.OfType<BoneFracture>().ToList())
            {
        ...
        TreatmentType cureType = TreatmentType.Mend;
        ...
        foreach (IWound wound in targetAsHaveWounds.Wounds.Where(x => x.Bodypart == part || part == null).ToList())
        {
...
Your options are to name the specific bodypart, #3random#0 for a random part, #3all#0 to hit all their external bodyparts, #3*limb#0 to hit all bodyparts on a particular limb, or #3none#0 if you're targeting an item.".SubstituteANSIColour().Wrap(actor.InnerLineFormatLength));
        ...
        ICharacter targetChar = target as ICharacter;
        ...
        bool all = false;
        ILimb limb = null;
        List<IBodypart> targetBodyparts = new();
        string bpTarget = ss.PopSpeech();
        if (string.IsNullOrEmpty(bpTarget))
        {
            IBodypart targetBodypart = targetChar?.Body.RandomBodypart;
        ...
        if (weapon == null)
        {
            ...
            if (!WoundExtensions.TryGetDamageType(ss.PopSpeech(), out DamageType damageType))
            {
            ...
            {
                foreach (IBodypart bodypart in targetBodyparts)
                {
                    finalDamage = new Damage
                    {
                        ActorOrigin = actor,
        ...
        {
            IWeaponAttack attack = ss.IsFinished
    ...
    protected static void Health(ICharacter actor, string command)
    {
        ...
        if (targetAsCharacter != null)
        {
            ...
            double bac = 10.0 * targetAsCharacter.NeedsModel.AlcoholLitres /
        ...
        BoneFracture[] fractures = targetWoundable.Wounds.OfType<BoneFracture>().ToArray();
        IWound[] nonFractures = targetWoundable.Wounds.Except(fractures).ToArray();
        ...
        if (targetAsCharacter != null && targetAsCharacter.Body.PartInfections.Any())
        {
            ...
            foreach (IInfection infection in targetAsCharacter.Body.PartInfections)
            {
    ...
    protected static void Sever(ICharacter actor, string input)
    {
        ...
        string targetBodypartText = ss.PopSpeech();
        IBodypart targetBodypart =
    ...
    protected static void InstallImplant(ICharacter actor, string input)
    {
        ...
        if (implant.TargetBodypart == null)
        {
            ...
            IBodypart targetPart = target.Body.GetTargetBodypart(ss.PopSpeech());
            if (targetPart == null)
            {
    ...
    protected static void PowerImplant(ICharacter actor, string input)
    {
        ...
        IImplant implant = target.Body.Implants.Select(x => x.Parent).GetFromItemListByKeyword(ss.PopSpeech(), actor)
        ...
        IImplantPowerSupply implantPower = implant.Parent.GetItemType<IImplantPowerSupply>();
        ...
        IImplantPowerPlant powerplant = target.Body.Implants.Select(x => x.Parent).GetFromItemListByKeyword(ss.PopSpeech(), actor)
    ...
    protected static void ConnectImplant(ICharacter actor, string input)
    {
        ...
        IImplantNeuralLink neural = target.Body.Implants.Select(x => x.Parent).GetFromItemListByKeyword(ss.PopSpeech(), actor)
        ...
        foreach (IImplantNeuralLink otherneural in target.Body.Implants.OfType<IImplantNeuralLink>().Except(neural))
        {
    ...
    protected static void Infect(ICharacter actor, string input)
    {
        ...
        Construction.ITerrain terrain = target.Location.Terrain(target);
        if (ss.IsFinished)
        {
            if (targetWound)
            {
                IWound wound = target.Body.Wounds.Where(x => x.Infection == null && x.EligableForInfection())
                                  .GetRandomElement();
            ...
            IBodypart bodypart = target.Body.Bodyparts.Concat(target.Body.Organs).GetRandomElement();
        ...
        if (targetWound)
        {
            IWound wound = target.Body.Wounds.GetFromItemListByKeyword(ss.PopSpeech(), actor);
            if (wound == null)
            {
        ...
        IBodypart targetBodypart = target.Body.Bodyparts.Concat(target.Body.Organs)
