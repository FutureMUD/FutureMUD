...
internal class HelpModule : Module<ICharacter>
{
    ...
    {
        ICharacter character = (ICharacter)parameters[0];
        using (new FMDB())
        {
            Models.Helpfile dbhelpfile = new()
            {
    ...
    private static void HEditNew(ICharacter actor, StringStack command)
    {
        ...
        string categoryText = command.PopSpeech();
        string subcategoryText = command.PopSpeech();
    ...
    private static void HEditDelete(ICharacter actor, StringStack command)
    {
        ...
        IHelpfile helpfile = GetHelpfile(actor, command, true);
        if (helpfile == null)
        {
    ...
    private static void HEditName(ICharacter actor, StringStack command)
    {
        ...
        IHelpfile helpfile = GetHelpfile(actor, command, false);
        if (helpfile == null)
        {
        ...
        string name = command.SafeRemainingArgument.Trim();
    ...
    private static void HEditKeywords(ICharacter actor, StringStack command)
    {
        ...
        IHelpfile helpfile = GetHelpfile(actor, command);
        if (helpfile == null)
        {
            actor.Send("There is no such helpfile for you to edit.");
            return;
    ...
    private static void HEditProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = long.TryParse(command.Last, out long value)
    ...
    private static void HEditTextPost(string text, IOutputHandler handler, params object[] parameters)
    {
        ...
        IEditableHelpfile helpfile = ((IHelpfile)parameters[1]).GetEditableHelpfile;
    ...
    {
        IHelpfile helpfile = (IHelpfile)parameters[1];
    ...
    private static void HEditExtraTextAddPost(string text, IOutputHandler handler, object[] parameters)
    {
        ICharacter actor = (ICharacter)parameters[0];
        IEditableHelpfile helpfile = ((IHelpfile)parameters[1]).GetEditableHelpfile;
        IFutureProg prog = (IFutureProg)parameters[2];
    ...
    private static void HEditExtraTextRemove(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int index) || index < 1 || index > helpfile.AdditionalTexts.Count())
        {
    ...
    private static void HEditExtraTextEditPost(string text, IOutputHandler handler, object[] parameters)
    {
        ...
        ICharacter actor = (ICharacter)parameters[0];
        IEditableHelpfile helpfile = ((IHelpfile)parameters[1]).GetEditableHelpfile;
        Tuple<IFutureProg, string> additionalText = (Tuple<IFutureProg, string>)parameters[2];
        int index = helpfile.AdditionalTexts.IndexOf(additionalText);
    ...
    private static void HEditExtraTextEdit(ICharacter actor, StringStack command)
    {
        ...
        Tuple<IFutureProg, string> additionalText = helpfile.AdditionalTexts.ElementAtOrDefault(value - 1);
    ...
    private static void HEditExtraTextProg(ICharacter actor, StringStack command)
    {
        ...
        IEditableHelpfile ehelp = helpfile.GetEditableHelpfile;
        int index = ehelp.AdditionalTexts.IndexOf(additionalText);
    ...
    private static void HEditView(ICharacter actor, StringStack command)
    {
        ...
        int count = 1;
        foreach (Tuple<IFutureProg, string> additional in helpfile.AdditionalTexts)
        {
            sb.AppendLine();
    ...
    {
        string helpName = useFullArgument ? command.SafeRemainingArgument : command.PopSpeech();
        IHelpfile helpfile = actor.Gameworld.Helpfiles.GetByIdOrName(helpName);
...
The syntax for this is as follows:
    ...
    {
        StringBuilder sb = new();
        List<IHelpfile> helpfiles = actor.Gameworld.Helpfiles.ToList();
    ...
    {
        IHelpfile exactMatch = actor.Gameworld.Helpfiles.FirstOrDefault(x =>
        ...
        List<IHelpfile> helpFilesMatched = new[] { exactMatch }.ToList();
    ...
    {
        StringStack ss = new(command.RemoveFirstWord());
        List<IHelpfile> allHelpfiles = actor.Gameworld.Helpfiles.Where(x => x.CanView(actor)).ToList();
        bool isAdmin = actor.IsAdministrator();
        List<ICommandHelpInfo> builtInHelp = actor.Gameworld
                               ...
                               .ToList();
        List<IHelpInformation> combined = allHelpfiles.OfType<IHelpInformation>().Concat(builtInHelp).ToList();
        List<string> categories = allHelpfiles
...
{categories.ArrangeStringsOntoLines()}"
        ...
        IHelpInformation exactMatch = combined.FirstOrDefault(x => x.CanView(actor) &&
        ...
        List<IHelpInformation> helpfilesMatched = exactMatch != null
    ...
    private static bool HandleHelpOnCommand(ICharacter actor, StringStack ss, string cmd, List<IHelpfile> allHelpfiles, List<ICommandHelpInfo> builtInHelp)
    {
        ...
        List<(string Name, string TagLine, string SubCategory, string Keywords)> helpOutput = helpfiles
