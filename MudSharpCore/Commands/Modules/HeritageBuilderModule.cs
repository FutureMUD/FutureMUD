...
internal class HeritageBuilderModule : BaseBuilderModule
{
    ...
    {
        string which = input.RemoveFirstWord();
        List<ILanguage> languages = actor.IsAdministrator() ? actor.Gameworld.Languages.ToList() : actor.Languages.ToList();
        ILanguage language = languages.FirstOrDefault(x => x.Name.EqualTo(which)) ??
        ...
        List<IScript> scripts = actor.Gameworld.Scripts.Where(x => x.DesignedLanguages.Contains(language)).ToList();
        if (!scripts.Any())
        {
        ...
        List<ILanguage> mutuals = actor.Gameworld.Languages.Where(x => language.MutualIntelligability(x) != Difficulty.Impossible)
                           .ToList();
        if (!mutuals.Any())
        {
            sb.AppendLine("\tNone that you are aware of.");
        ...
        {
            foreach (ILanguage mutual in mutuals)
            {
                sb.AppendLine(
    ...
    {
        BuilderEditingEffect<ILanguage> editing = actor.CombinedEffectsOfType<BuilderEditingEffect<ILanguage>>().FirstOrDefault();
        if (editing == null)
        {
    ...
    private static void LanguageView(ICharacter actor, StringStack ss)
    {
        ...
        ILanguage language = long.TryParse(ss.SafeRemainingArgument, out long value)
    ...
    private static void LanguageEdit(ICharacter actor, StringStack ss)
    {
        ...
        if (ss.PeekSpeech().EqualTo("new"))
        {
            ...
            ITraitDefinition trait = long.TryParse(ss.SafeRemainingArgument, out long tvalue)
            ...
            Language newLanguage = new(actor.Gameworld, trait, name);
    ...
    private static void AccentEdit(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IAccent> editing = actor.CombinedEffectsOfType<BuilderEditingEffect<IAccent>>().FirstOrDefault();
            if (editing == null)
            {
                actor.OutputHandler.Send(
        ...
        if (ss.PeekSpeech().EqualTo("new"))
        {
            ...
            ILanguage language = long.TryParse(ss.PopSpeech(), out long value)
                ? actor.Gameworld.Languages.Get(value)
            ...
            Accent newAccent = new(actor.Gameworld, language, name);
        ...
        IAccent accent = long.TryParse(ss.SafeRemainingArgument, out long evalue)
    ...
    private static void AccentShow(ICharacter actor, StringStack ss)
    {
        ...
        IAccent accent = long.TryParse(ss.SafeRemainingArgument, out long value)
    ...
    protected static void PlayerAccent(ICharacter actor, string input)
    {
        ...
        if (which.Contains('.'))
        {
            string[] split = which.Split(new[] { '.' }, 2);
            string lang = split[0];
            List<ILanguage> languages = actor.IsAdministrator() ? actor.Gameworld.Languages.ToList() : actor.Languages.ToList();
        ...
        List<IAccent> accents = actor.IsAdministrator() ? actor.Gameworld.Accents.ToList() : actor.Accents.ToList();
        if (language != null)
        {
            accents = accents.Where(x => x.Language == language).ToList();
        }

        IAccent accent = accents.FirstOrDefault(x => x.Name.EqualTo(which)) ??
    ...
    {
        BuilderEditingEffect<IScript> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<IScript>>().FirstOrDefault();
        if (effect == null)
        {
    ...
    private static void ScriptShow(ICharacter actor, StringStack ss)
    {
        ...
        IScript script = long.TryParse(ss.SafeRemainingArgument, out long value)
    ...
    private static void ScriptEdit(ICharacter actor, StringStack ss)
    {
        ...
        if (cmd.EqualTo("new"))
        {
            ...
            IKnowledge knowledge = long.TryParse(ss.SafeRemainingArgument, out long value)
            ...
            Script newScript = new(actor.Gameworld, name, knowledge);
        ...
        IScript script = long.TryParse(cmd, out long lvalue)
    ...
    protected static void PlayerScript(ICharacter actor, string command)
    {
        ...
        ILanguage language = actor.CurrentWritingLanguage;
        if (!ss.IsFinished)
        {
            text = ss.PopSpeech();
    ...
    {
        BuilderEditingEffect<IKnowledge> editing = actor.CombinedEffectsOfType<BuilderEditingEffect<IKnowledge>>().FirstOrDefault();
        if (editing == null)
        {
    ...
    private static void KnowledgeEdit(ICharacter actor, StringStack ss)
    {
        ...
        if (cmd.EqualTo("new"))
        {
            ...
            Knowledge newKnowledge = new(name);
        ...
        IKnowledge knowledge = long.TryParse(cmd, out long value)
            ? actor.Gameworld.Knowledges.Get(value)
...
The syntax for this command is as follows:
    ...
    {
        List<IRandomNameProfile> profiles = actor.Gameworld.RandomNameProfiles.ToList();
    ...
    private static void RandomNameEdit(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IRandomNameProfile> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<IRandomNameProfile>>().FirstOrDefault();
            if (effect == null)
            {
        ...
        if (ss.PeekSpeech().EqualTo("new"))
        {
            ...
            INameCulture culture = long.TryParse(ss.PopSpeech(), out long profileid)
            ...
            if (!ss.SafeRemainingArgument.TryParseEnum<Gender>(out Gender gender))
            {
                actor.OutputHandler.Send(
            ...
            RandomNameProfile newProfile = new(culture, gender, name);
        ...
        IRandomNameProfile randomProfile = long.TryParse(ss.SafeRemainingArgument, out long value)
    ...
    private static void RandomNameClone(ICharacter actor, StringStack ss)
    {
        ...
        IRandomNameProfile profile = long.TryParse(ss.PopSpeech(), out long value)
            ? actor.Gameworld.RandomNameProfiles.Get(value)
        ...
        RandomNameProfile clone = new((RandomNameProfile)profile, name);
...
For example, #3?Nickname[ a.k.a ""$Nickname""][]#0";
    ...
    private static void NameCultureClone(ICharacter actor, StringStack ss)
    {
        ...
        INameCulture nameCulture = long.TryParse(ss.PopSpeech(), out long value)
        ...
        NameCulture clone = new(nameCulture, name);
        actor.Gameworld.Add(clone);
        actor.OutputHandler.Send(
    ...
    private static void NameCultureShow(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<INameCulture> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<INameCulture>>().FirstOrDefault();
            if (effect == null)
            {
        ...
        INameCulture nameCulture = long.TryParse(ss.SafeRemainingArgument, out long value)
            ? actor.Gameworld.NameCultures.Get(value)
    ...
    private static void NameCultureEdit(ICharacter actor, StringStack ss)
    {
        ...
        if (ss.PeekSpeech().EqualTo("new"))
        {
            ...
            NameCulture newName = new(actor.Gameworld, name);
...
The correct syntax for this command is as follows:
    ...
    private static void CultureClone(ICharacter actor, StringStack ss)
    {
        ...
        ICulture culture = long.TryParse(ss.PopSpeech(), out long value)
        ...
        Culture clone = new(culture, name);
        actor.Gameworld.Add(clone);
        actor.OutputHandler.Send(
    ...
    private static void CultureShow(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<ICulture> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<ICulture>>().FirstOrDefault();
            if (effect == null)
            {
        ...
        ICulture culture = long.TryParse(ss.SafeRemainingArgument, out long value)
            ? actor.Gameworld.Cultures.Get(value)
    ...
    private static void CultureEdit(ICharacter actor, StringStack ss)
    {
        ...
        if (ss.PeekSpeech().EqualTo("new"))
        {
            ...
            Culture newCulture = new(actor.Gameworld, name);
...
The correct syntax for this command is as follows:
    ...
    private static void EthnicityClone(ICharacter actor, StringStack ss)
    {
        ...
        IEthnicity ethnicity = long.TryParse(ss.PopSpeech(), out long value)
        ...
        Ethnicity clone = new(ethnicity, name);
        actor.Gameworld.Add(clone);
        actor.OutputHandler.Send(
    ...
    private static void EthnicityShow(ICharacter actor, StringStack ss)
    {
        if (ss.IsFinished)
        {
            BuilderEditingEffect<IEthnicity> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<IEthnicity>>().FirstOrDefault();
            if (effect == null)
            {
        ...
        IEthnicity ethnicity = long.TryParse(ss.SafeRemainingArgument, out long value)
            ? actor.Gameworld.Ethnicities.Get(value)
    ...
    private static void EthnicityEdit(ICharacter actor, StringStack ss)
    {
        ...
        if (ss.PeekSpeech().EqualTo("new"))
        {
            ...
            IRace race = long.TryParse(ss.SafeRemainingArgument, out long raceid)
            ...
            List<Form.Characteristics.ICharacteristicDefinition> missingCharacteristics = MudSharp.Character.Heritage.Ethnicity.GetCharacteristicProfileSelections(race, actor.Gameworld.CharacteristicProfiles)
    ...
    {
        List<IEthnicity> ethnicities = actor.Gameworld.Ethnicities.ToList();
        if (!ss.IsFinished)
        {
            IRace race = long.TryParse(ss.SafeRemainingArgument, out long raceid)
                ? actor.Gameworld.Races.Get(raceid)
