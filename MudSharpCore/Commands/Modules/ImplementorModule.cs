...
public class ImplementorModule : Module<ICharacter>
{
    ...
    protected static void CommandLevels(ICharacter actor, string input)
    {
        ...
        PermissionLevel minimum = PermissionLevel.Any;
        if (!ss.IsFinished)
        {
        ...
        foreach (IGrouping<PermissionLevel, IExecutable<ICharacter>> group in actor.CommandTree.Commands.TCommands.Values
    ...
    protected static void CurrencyTest(ICharacter actor, string input)
    {
        ...
        {
            ss.PopSpeech();
            decimal amount = actor.Currency.GetBaseCurrency(ss.SafeRemainingArgument, out bool success);
            if (success)
            {
        ...
        {
            if (!decimal.TryParse(ss.PopSpeech(), out decimal amount))
            {
    ...
    protected static void TestGPT(ICharacter actor, string input)
    {
        ...
        using (new FMDB())
        {
            ...
            if (long.TryParse(cmd, out long id))
            {
            ...
            string prompt = ss.SafeRemainingArgument;
    ...
    private static void DebugOrphans(ICharacter actor)
    {
        ...
        List<IGameItem> items = new(ids.Count);
        ...
        List<IActiveCraftGameItemComponent> crafts = items.SelectNotNull(x => x.GetItemType<IActiveCraftGameItemComponent>()).ToList();
        List<IRangedWeapon> ranged = items.SelectNotNull(x => x.GetItemType<IRangedWeapon>()).ToList();
        List<Health.IWound> wounds = items.SelectMany(x => x.Wounds).Concat(actor.Gameworld.Characters.SelectMany(x => x.Wounds)).Where(x => x.Lodged is not null).ToList();
    ...
    {
        Dictionary<IGameItem, WearableItemCoverStatus> covers = actor.Body.GetAllItemsCoverStatus(true);
        foreach (IGameItem item in actor.Body.WornItems)
        {
            List<Tuple<WearableItemCoverStatus, IGameItem>> cover = actor.Body.CoverInformation(item).ToList();
    ...
    private static void DebugBackup(ICharacter actor)
    {
        try
        {
            string destination = new DatabaseUpgradeCoordinator().CreateBackup(new DatabaseUpgradeRequest
            {
...
{ss.RemainingArgument.ColourCommand()}");
    ...
    private static void Debug_AddTime(ICharacter actor, StringStack ss)
    {
        ...
        if (!TimeSpan.TryParse(ss.SafeRemainingArgument, out TimeSpan timespan))
        {
            actor.OutputHandler.Send(
    ...
    private static void DebugCleanupOrphanedItems(ICharacter actor)
    {
        ...
        void DoCleanupOrphanedItems()
        {
            ...
            List<long> onlinePCIDs = actor.Gameworld.Characters.Select(x => x.Id).ToList();
            actor.Gameworld.SystemMessage("Loading offline PCs so their inventory is accounted for...", true);
            using (new FMDB())
            {
                IOrderedQueryable<Models.Character> PCsToLoad =
                ...
                {
                    bool any = false;
                    foreach (Models.Character pc in PCsToLoad.Skip(i++ * 10).Take(10).ToList())
                    {
                        any = true;
                        ICharacter character = actor.Gameworld.TryGetCharacter(pc.Id, true);
            ...
            HashSet<ICharacter> corpsePCs = new();
            foreach (ICorpse item in actor.Gameworld.Items.SelectNotNull(x => x.GetItemType<ICorpse>()).ToList())
            {
            ...
            List<long> loadedItemIDs = actor.Gameworld.Items.Select(x => x.Id).ToList();
            using (new FMDB())
            {
    ...
    {
        using StreamWriter html = new("ProgCollectionHelps.html");
        html.WriteLine("<html>");
...
And the result would be a number with the value of 33.</p>");
        ...
        foreach (CollectionExtensionFunctionCompilerInformation compiler in CollectionExtensionFunction.FunctionCompilerInformations
    ...
    {
        using StreamWriter html = new("ProgTypeHelps.html");
        html.WriteLine("<html>");
        html.WriteLine(@"<head><style>
table, th, td {
  border: 1px solid black;
...
div.function-generalhelp {
  color: darkred;
        ...
        foreach (ProgVariableTypes type in ProgVariableTypes.Anything
                                                    .GetAllFlags()
                                                    ...
                                                    .ToList())
        {
            ...
            foreach (KeyValuePair<string, ProgVariableTypes> item in info.PropertyTypeMap)
            {
    ...
    {
        using StreamWriter html = new("ProgFunctionsAlphabetically.html");
        html.WriteLine("<html>");
        html.WriteLine(@"<head><style>
table, th, td {
  border: 1px solid black;
...
div.function-generalhelp {
  color: darkred;
        ...
        foreach (FunctionCompilerInformation function in infos.OrderBy(x => x.FunctionName))
        {
            ...
            else
            {
                ...
                foreach (ProgVariableTypes item in function.Parameters)
                {
            ...
            {
                IFunction compiled = function.CompilerFunction(new List<IFunction>(), gameworld);
            ...
            int parameterCount = parameterNames.Count();
    ...
    {
        using StreamWriter html = new("ProgFunctionsByCategory.html");
        html.WriteLine("<html>");
        html.WriteLine(@"<head><style>
table, th, td {
  border: 1px solid black;
...
div.function-generalhelp {
  color: darkred;
        ...
        html.WriteLine("<body>\n<h1>FutureMUD Function Reference</h1>");
        foreach (IGrouping<string, FunctionCompilerInformation> group in infos.GroupBy(x => x.Category).OrderBy(x => x.Key))
        {
            html.WriteLine(
                $"<details><summary>{group.Key.TitleCase()}</summary>");
            foreach (FunctionCompilerInformation function in group.OrderBy(x => x.FunctionName))
            {
                html.WriteLine("  <div class=\"function-box\">");
                html.WriteLine($"    <h3>{function.FunctionName.ToUpperInvariant()}</h3>");
                List<string> parameterNames;
    ...
    private static void DebugWeatherStats(ICharacter actor, StringStack ss)
    {
        ...
        IWeatherController controller = actor.Gameworld.WeatherControllers.GetByIdOrName(ss.PopSpeech());
        ...
        int burnInYears = 2;
        int? seed = null;
        string outputBase = $"WeatherStats-{controller.Id}-{DateTime.UtcNow:yyyyMMddHHmmss}";
        ...
        while (!ss.IsFinished)
        {
            switch (ss.PopForSwitch())
            {
                ...
                case "burn":
                    ...
                    break;
                case "seed":
                    if (ss.IsFinished || !int.TryParse(ss.PopSpeech(), out int seedValue))
                    {
        ...
        string fullBasePath = GetWeatherStatisticsOutputBasePath(outputBase);
        ...
        try
        {
            ...
            WeatherStatisticsResult result = analyzer.Analyze(new WeatherStatisticsRequest
            {
            ...
            string directory = Path.GetDirectoryName(fullBasePath);
            ...
            List<(string Label, string Description, string Path, List<WeatherStatisticsCsvRow> Rows)> outputs = new()
            {
                (
                    "Weather Events",
            ...
            foreach ((string Label, string Description, string Path, List<WeatherStatisticsCsvRow> Rows) output in outputs)
            {
    ...
    {
        using FileStream file = new(fullPath, FileMode.Create, FileAccess.Write, FileShare.Read);
        using StreamWriter writer = new(file, Encoding.UTF8);
        foreach ((string Label, string Value) entry in BuildWeatherStatisticsHeaderEntries(fullPath, outputDescription, controller, result, seed, generatedAtUtc))
        {
    ...
    private static IEnumerable<(string Label, string Value)> BuildWeatherStatisticsHeaderEntries(
        ...
        DateTime generatedAtUtc)
    {
        List<ICelestialObject> celestials = controller.Celestial is null
            ? new List<ICelestialObject>()
            : FlattenCelestialObjects(controller.Celestial).ToList();
        List<string> celestialClocks = celestials
            ...
            .OrderBy(x => x)
            .ToList();
        List<string> celestialCalendars = celestials
            ...
            .OrderBy(x => x)
            .ToList();
        List<string> seasonDescriptions = controller.RegionalClimate.Seasons
            .OrderBy(x => x.CelestialDayOnset)
            .ThenBy(x => x.Name)
            .Select(DescribeItem)
            .ToList();
        List<string> weatherEvents = result.Rows
    ...
    {
        List<ICelestialObject> results = new();
    ...
    private static string DescribeItem(object item)
    {
        ...
        string name = type.GetProperty("Name")?.GetValue(item)?.ToString() ?? type.Name;
        string id = type.GetProperty("Id")?.GetValue(item)?.ToString();
    ...
    {
        List<string> items = descriptions
    ...
    private static void DebugDescriptions(ICharacter actor, StringStack ss)
    {
        ...
        ICharacter target = ss.IsFinished ? actor : actor.TargetActor(ss.PopSpeech());
        ...
        using FileStream file = new($"Debug Descriptions {DateTime.Now.ToFileTime()}.csv", FileMode.Create,
            FileAccess.Write);
        StreamWriter writer = new(file);
        writer.WriteLine("id,text,pattern");
        foreach (
            IEntityDescriptionPattern pattern in patterns.Where(x => x.IsValidSelection(target)).ToList())
        {
            writer.WriteLine(
    ...
    private static void DebugGuests(ICharacter actor, StringStack ss)
    {
        ...
        if (!int.TryParse(ss.PopSpeech(), out int number))
        {
    ...
    private static void DebugSun(ICharacter actor, StringStack ss)
    {
        ...
        ICelestialObject sun = long.TryParse(ss.PopSpeech(), out long value)
        ...
        if (ss.IsFinished || !int.TryParse(ss.SafeRemainingArgument, out int time))
        {
    ...
    private static void DebugCelestials(ICharacter actor)
    {
        ...
        for (int i = 0; i < 1000000; i++)
        {
            ...
            CelestialInformation celestialInfo = celestial.CurrentPosition(zone.Geography);
            double illumination = celestial.CurrentIllumination(zone.Geography);
            MudDateTime datetime = new(zone.Date(calendar), zone.Time(clock), timezone);
    ...
    private static void DebugCombatSpeed(ICharacter actor, StringStack ss)
    {
        ...
        if (!double.TryParse(ss.SafeRemainingArgument, out double value) || value <= 0.0)
        {
        ...
        using (new FMDB())
        {
            StaticConfiguration setting = FMDB.Context.StaticConfigurations.Find("CombatSpeedMultiplier");
    ...
    private static IEnumerable<ICharacter> EnsureAllPCsAreLoaded(IFuturemud gameworld)
    {
        ...
        gameworld.ForceOutgoingMessages();
        HashSet<ICharacter> corpsePCs = new();
        foreach (ICorpse item in gameworld.Items.SelectNotNull(x => x.GetItemType<ICorpse>()).ToList())
        {
            corpsePCs.Add(item.OriginalCharacter);
        }
...
#3GPT set <which> prompt#0 - drops into an editor to write the prompt";
    ...
    private static void GPTQuery(ICharacter actor, StringStack ss)
    {
        ...
        {
            Models.GPTThread thread;
            if (long.TryParse(ss.PopSpeech(), out long id))
            {
                thread = FMDB.Context.GPTThreads.Find(id);
            }
            else
    ...
    private static void GPTSetName(ICharacter actor, GPTThread thread, StringStack ss)
    {
        ...
        string name = ss.SafeRemainingArgument.Trim();
    ...
    private static void PostAction(string text, IOutputHandler handler, object[] args)
    {
        using (new FMDB())
        {
            GPTThread thread = FMDB.Context.GPTThreads.Find((long)args[0]);
            if (thread is null)
            {
    ...
    {
        if (ss.IsFinished || !double.TryParse(ss.SafeRemainingArgument, out double value) || value < 0)
        {
    ...
    {
        IEnumerable<string> models = OpenAIHandler.GPTModels().Result;
        if (ss.IsFinished)
        {
        ...
        string modelText = ss.SafeRemainingArgument;
        string model = models.FirstOrDefault(x => x.EqualTo(modelText)) ??
        ...
        using (new FMDB())
        {
            ...
            actor.AddEffect(new Accept(actor, new GenericProposal
            {
                ...
                AcceptAction = text =>
                {
                    ...
                    using (new FMDB())
                    {
                        GPTThread dbitem = FMDB.Context.GPTThreads.Find(thread.Id);
                        if (dbitem == null)
                        {
                            return;
                        }
    ...
    private static void GPTCreate(ICharacter actor, StringStack ss)
    {
        ...
        if (!double.TryParse(ss.PopSpeech(), out double temperature) || temperature < 0)
        {
