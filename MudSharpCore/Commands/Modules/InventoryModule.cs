...
The syntax is either: 
    ...
    protected static void Swap(ICharacter actor, string input)
    {
        ...
        IGameItem targetTwo = actor.TargetHeldItem(ss.PopSpeech());
...
The syntax is as follows: 
    ...
    protected static void Restrain(ICharacter actor, string input)
    {
        ...
        List<IWearProfile> profiles = wearable.Profiles
...
The syntax is as follows:
    ...
    protected static void Unwield(ICharacter actor, string input)
    {
        ...
        IGameItem targetItem = actor.Body.WieldedItems.GetFromItemListByKeyword(targetText, actor);
        if (targetItem == null)
        {
    ...
    protected static void Wield(ICharacter actor, string input)
    {
        ...
        string targetText = ss.PopSpeech();
        IGameItem targetItem = actor.TargetHeldItem(targetText);
        if (targetItem == null)
        {
        ...
        string emoteText = ss.PopParentheses();
        bool manual2hand = false;
        IWield specificHand = null;
        if (!ss.IsFinished)
        {
            string shandText = ss.PopSpeech();
    ...
    protected static void Put(ICharacter actor, string input)
    {
        ...
        Match weightMatch = PutWeightRegex.Match(input.RemoveFirstWord());
        ...
        string containerOwnerText = weightMatch.Success ? weightMatch.Groups["owner"].Value :
            match.Success ? match.Groups["owner"].Value :
            currencyMatch.Groups["owner"].Value;

        string containerText = weightMatch.Success
        ...
        (bool truth, string error) = actor.CanManipulateItem(containerItem);
        if (!truth)
        {
        ...
        {
            double amount = actor.Gameworld.UnitManager.GetBaseUnits(weightMatch.Groups["weight"].Value,
                Framework.Units.UnitType.Mass, out bool success);
            if (!success)
            {
            ...
            IGameItem targetItem = actor.Body.TargetHeldItem(weightMatch.Groups["item"].Value);
            if (targetItem == null)
            {
            ...
            {
                int quantity = (int)(targetItem.Weight > 0.0 && targetItem.Quantity > 0
            ...
            IGameItem item = targetItem.PeekSplitByWeight(amount);
            ...
            {
                int lesserAmount = containerItem.GetItemType<IContainer>().CanPutAmount(item);
        ...
        else if (match.Success)
        {
            ...
            IGameItem targetItem = actor.Body.TargetHeldItem(match.Groups["item"].Value);
            if (targetItem == null)
            {
                actor.OutputHandler.Send("You do not have anything like that to put in " +
                                         containerItem.HowSeen(actor.Body) + ".");
        ...
        {
            string currencyText = currencyMatch.Groups["currency"].Value.Strip(x => x == '\"');
            decimal targetCurrency = actor.Currency.GetBaseCurrency(currencyText, out bool success);
            if (!success)
            {
    ...
    protected static void Take(ICharacter actor, string input)
    {
        ...
        Match match = TakeCommandRegex.Match(text);
        if (!match.Success)
        {
        ...
        ICharacter target = actor.TargetActorOrCorpse(match.Groups["target"].Value);
        if (target == null)
        {
        ...
        if (match.Groups["targetcontainer"].Length > 0)
        {
            ...
            IContainer targetContainer = targetContainerItem.GetItemType<IContainer>();
        ...
        if (targetContainerItem == null)
        {
            ...
            IGameItem newItem = target.Body.Take(targetItem, quantity);
    ...
    protected static void Get(ICharacter actor, string input)
    {
        ...
        Match match = GetCommandRegex.Match(input.RemoveFirstWord());
        Match currencyMatch = GetCurrencyRegex.Match(input.RemoveFirstWord());
        Match weightMatch = GetWeightRegex.Match(input.RemoveFirstWord());
        ...
        string containerText = weightMatch.Success ? weightMatch.Groups["container"].Value :
            match.Success ? match.Groups[3].Value : currencyMatch.Groups["container"].Value;
        if (containerText.Length > 0)
        {
            IGameItem containerItem = actor.TargetItem(containerText);
            if (containerItem == null)
            {
            ...
            if (weightMatch.Success)
            {
                ...
                IGameItem targetItem = container.Contents.GetFromItemListByKeyword(weightMatch.Groups["item"].Value, actor);
                if (targetItem == null)
                {
            ...
            {
                IGameItem targetItem = container.Contents.GetFromItemListByKeyword(match.Groups[2].Value, actor);
                actor.Body.Get(targetItem, containerItem, quantity, emote);
            }
            else
            {
        ...
        else
        {
            if (weightMatch.Success)
            {
                ...
                IGameItem targetItem = actor.TargetLocalItem(weightMatch.Groups["item"].Value);
                if (targetItem == null)
                {
            ...
            {
                IGameItem targetItem = actor.TargetLocalItem(match.Groups[2].Value);
                if (targetItem == null)
                {
    ...
    protected static void Drop(ICharacter actor, string input)
    {
        ...
        Match match = DropCommandRegex.Match(input.RemoveFirstWord());
        Match currencyMatch = DropCurrencyRegex.Match(input.RemoveFirstWord());
        Match weightMatch = DropWeightRegex.Match(input.RemoveFirstWord());
        if (!match.Success && !currencyMatch.Success && !weightMatch.Success)
        {
            actor.OutputHandler.Send(
        ...
        {
            List<IGameItem> items = actor.Body.HeldItems.ToList();
            foreach (IGameItem item in items.Where(item => !actor.Body.CanDrop(item, 0)))
            {
        ...
        if (weightMatch.Success)
        {
            ...
            IGameItem targetItem = actor.TargetHeldItem(weightMatch.Groups["item"].Value);
            if (targetItem == null)
            {
            ...
            IGameItem item = targetItem.DropByWeight(actor.Location, amount);
            if (CrimeExtensions.HandleCrimesAndLawfulActing(actor, CrimeTypes.Littering, null, item))
            {
                return;
            }
        ...
        {
            IGameItem targetItem = actor.TargetHeldItem(match.Groups[3].Value);
            if (targetItem == null)
            {
    ...
    protected static void Give(ICharacter actor, string input)
    {
        ...
        Match weightMatch = GiveWeightRegex.Match(input.RemoveFirstWord());
        if (!match.Success && !currencyMatch.Success && !weightMatch.Success)
        {
            actor.OutputHandler.Send(
        ...
        string targetText = weightMatch.Success ? weightMatch.Groups["target"].Value :
        ...
        else if (match.Success)
        {
            ...
            IGameItem targetItem = actor.TargetHeldItem(match.Groups[2].Value);
            if (targetItem == null)
            {
    ...
    protected static void Wear(ICharacter actor, string input)
    {
        ...
        IGameItem targetItem = actor.TargetHeldItem(match.Groups[1].Value);
        if (targetItem == null)
        {
    ...
    protected static void Remove(ICharacter actor, string input)
    {
        ...
        IGameItem targetItem = actor.TargetTopLevelWornItem(target);
        if (targetItem == null)
        {
...
The syntax is simply #3wearall#0.", AutoHelp.HelpArg)]
    protected static void WearAll(ICharacter actor, string command)
    {
        ...
        List<IGameItem> items = actor.Location
        ...
        foreach (IGameItem item in items)
        {
            ...
            if (item.GetItemType<IBeltable>() is { } beltable)
            {
                IBelt belt = actor.Body.ExternalItems.SelectNotNull(x => x.GetItemType<IBelt>()).FirstOrDefault(x => x.CanAttachBeltable(beltable) == IBeltCanAttachBeltableResult.Success);
                if (belt is not null)
                {
            ...
            if (item.GetItemType<IWieldable>() is { } wieldable)
            {
                ISheath sheath = actor.Body.ExternalItems.SelectNotNull(x => x.GetItemType<ISheath>()).FirstOrDefault(x => actor.Body.CanSheathe(item, x.Parent));
            ...
            List<IContainer> containers = actor.Body.ExternalItems.SelectNotNull(x => x.GetItemType<IContainer>()).ToList();
...
The possible syntaxes for this command are:
        ...
        out IGameItem problemItem)
    {
        ...
        List<Tuple<WearableItemCoverStatus, IGameItem>> itemsCoveringItem = actor.Body.CoverInformation(item)
        ...
        foreach (Tuple<WearableItemCoverStatus, IGameItem> otherItem in itemsCoveringItem)
        {
    ...
    private static void StripSelf(ICharacter actor, StringStack ss, PlayerEmote emote)
    {
        ...
        {
            double seconds = 0.0;
            HashSet<IGameItem> considered = new();
            ...
            void ProcessSecondsForItem(IGameItem item)
            {
                ...
                considered.Add(item);
                IWearable wear = item.GetItemType<IWearable>();
                if (wear == null)
                {
                    return;
                }
                ...
                foreach (KeyValuePair<IWear, IWearlocProfile> part in wear.CurrentProfile.AllProfiles)
                {
                    IEnumerable<IGameItem> items = actor.Body.WornItemsFor(part.Key).SkipWhile(x => x != item).Skip(1);
                    foreach (IGameItem otherItem in items)
                    {
        ...
        if (ss.Peek().EqualTo("covering"))
        {
            ...
            IGameItem coverItem = actor.Body.TargetWornItem(ss.PopSpeech());
            if (coverItem == null)
            {
            ...
            List<Tuple<WearableItemCoverStatus, IGameItem>> coverItemInfo = actor.Body.CoverInformation(coverItem).ToList();
        ...
        {
            ss.PopSpeech();
            IGameItem targetContainerItem = actor.TargetLocalOrHeldItem(ss.PopSpeech());
            if (targetContainerItem == null)
            {
        ...
        {
            foreach (IGameItem item in coveringItems)
            {
                if (!RecursiveRemoveItem(actor, item, container, out IGameItem problem))
                {
    ...
    private static void StripOther(ICharacter actor, ICharacter target, StringStack ss, PlayerEmote emote)
    {
        ...
        TimeSpan TimeSpanForStrip(List<IGameItem> strippedItems)
        {
            ...
            void ProcessSecondsForItem(IGameItem item)
            {
                ...
                foreach (KeyValuePair<IWear, IWearlocProfile> part in wear.CurrentProfile.AllProfiles)
                {
                    List<IGameItem> items = target.Body.WornItemsFor(part.Key).SkipWhile(x => x != item).Skip(1).ToList();
                    foreach (IGameItem otherItem in items)
                    {
                        ProcessSecondsForItem(otherItem);
                    }
        ...
        void BeginStrip(bool grantTemporaryConsent = false)
        {
            ...
            actor.AddEffect(new SimpleCharacterAction(actor, perc =>
            {
                foreach (IGameItem item in coveringItems)
                {
                    if (!StripOtherRecursiveRemoveItem(item, container, out IGameItem problem))
                    {
                        actor.Send($"You were unable to remove {problem.HowSeen(actor)} and anything below it.");
                        return;
                    }
...
The valid syntaxes are:
    ...
    private static void OutfitClone(ICharacter actor, StringStack ss)
    {
        ...
        IOutfit outfit = actor.EffectsOfType<BuilderEditingEffect<IOutfit>>().First().EditingItem;
        ...
        IOutfit newOutfit = outfit.CopyOutfit(actor, name);
    ...
    private static void OutfitRemove(ICharacter actor, StringStack ss)
    {
        ...
        IOutfit outfit = actor.Outfits.FirstOrDefault(x => x.Name.EqualTo(name)) ??
        ...
        List<IGameItem> items = actor.Body.DirectWornItems.Where(x => outfit.Items.Any(y => y.Id == x.Id)).Reverse().ToList();
        if (!items.Any())
        {
        ...
        actor.AddEffect(new SimpleCharacterAction(actor, perc =>
        {
            foreach (IGameItem outfitItem in items)
            {
                List<Tuple<WearableItemCoverStatus, IGameItem>> coverItemInfo = actor.Body.CoverInformation(outfitItem).Where(x => x.Item2 != null).ToList();
                List<IGameItem> coveringItems = new();
                if (coverItemInfo.Any())
                {
                ...
                foreach (IGameItem item in coveringItems)
                {
                    long? containerId = outfit.Items.FirstOrDefault(x => x.Id == item.Id)?.PreferredContainerId;
                    IContainer container = actor.ContextualItems.FirstOrDefault(
                                             x => x.Id == containerId &&
    ...
    private static void OutfitWear(ICharacter actor, StringStack ss)
    {
        ...
        bool force = !ss.IsFinished && ss.PopSpeech().EqualTo("!");
        int i = 1;
        InventoryPlanTemplate plan = new(actor.Gameworld,
        ...
        IInventoryPlan charPlan = plan.CreatePlan(actor);
        ...
        string missingText = infeasible.Select(x =>
    ...
    private static void OutfitTeach(ICharacter actor, StringStack ss)
    {
        ...
        target.AddEffect(new Accept(target, new GenericProposal
        {
            AcceptAction = text =>
            {
                ...
                IOutfit newOutfit = outfit.CopyOutfit(target, newName);
    ...
    private static void OutfitEdit(ICharacter actor, StringStack ss)
    {
        ...
        if (name.EqualTo("new"))
        {
            ...
            Outfit newOutfit = new(actor, name);
            actor.AddOutfit(newOutfit);
