...
Note: The following two subcommands take a long time and can sometimes cause issues with performance afterwards because of the aggressive loading of things that they do. You should generally plan to reboot the MUD immediately after running these.
	...
	#3comp update all#0 - same as comp update, but also loads all characters so that their inventories are included";
        ...
        {
            bool includeOffline = input.PopSpeech().Equals("all", StringComparison.InvariantCultureIgnoreCase);
            actor.Send(
            ...
            actor.AddEffect(new Accept(actor, new GenericProposal(
                text =>
                {
                    ...
                    actor.Gameworld.Scheduler.AddSchedule(new Schedule(() =>
                    {
                        ...
                        using (new FMDB())
                        {
                            foreach (IGameItemProto item in actor.Gameworld.ItemProtos)
                            {
        ...
        private static void Component_Types(ICharacter actor, StringStack input)
        {
            ...
            while (!input.IsFinished)
            {
                ...
                switch (txt[0])
                {
                    case '+':
                        types = types.Where(x =>
                            x.Blurb.Contains(txt[1..], StringComparison.InvariantCultureIgnoreCase)).ToList();
                        break;
                    case '-':
                        types = types.Where(x =>
                            !x.Blurb.Contains(txt[1..], StringComparison.InvariantCultureIgnoreCase)).ToList();
...
{type.Help}");
        ...
        {
            IEnumerable<IGameItemComponentProto> protos = actor.Gameworld.ItemComponentProtos.Where(x => x.Status == RevisionStatus.PendingRevision);
        ...
        private static void Component_Review_History(ICharacter actor, StringStack input)
        {
            ...
            List<IGameItemComponentProto> protos = actor.Gameworld.ItemComponentProtos.GetAll(value);
            if (!protos.Any())
            {
        ...
        private static void Component_Review_All(ICharacter actor, StringStack input)
        {
            ...
            List<IGameItemComponentProto> editableItems = protos.ToList();
            int count = editableItems.Count;
            using (new FMDB())
            {
                Dictionary<long, Models.Account> accounts = editableItems.Select(x => x.BuilderAccountID).Distinct()
        ...
        {
            string cmd = input.PopForSwitch();
            if (cmd.Length == 0)
            {
                BuilderEditingEffect<IGameItemComponentProto> effect = actor.CombinedEffectsOfType<BuilderEditingEffect<IGameItemComponentProto>>().FirstOrDefault();
        ...
        {
            IGameItemComponentProto item = actor.EditingItem<IGameItemComponentProto>();
            if (item is null)
            {
                actor.OutputHandler.Send("You are not currently editing any item component.");
                return;
            ...
            using (new FMDB())
            {
                Models.GameItemComponentProto dbproto = FMDB.Context.GameItemComponentProtos.Find(item.Id, item.RevisionNumber);
                if (dbproto != null)
                {
                    actor.Gameworld.SaveManager.Flush();
        ...
        private static void Component_Edit_Submit(ICharacter actor, StringStack input)
        {
            ...
            string comment = input.IsFinished ? "" : input.SafeRemainingArgument;
        ...
        private static void Component_Edit_New(ICharacter actor, StringStack input)
        {
            ...
            IGameItemComponentProto result = actor.Gameworld.GameItemComponentManager.GetProto(cmd, actor.Gameworld, actor.Account);
            if (result == null)
            {
        ...
        private static void Component_Edit_Default(ICharacter actor, StringStack input)
        {
            ...
            {
                if (!int.TryParse(cmd, out int revision))
                {
                    actor.OutputHandler.Send(
        ...
        {
            List<IGameItemComponentProto> protos = actor.Gameworld.ItemComponentProtos.Where(x => x.Status == RevisionStatus.Current).ToList();
            ...
            while (!input.IsFinished)
            {
                ...
                switch (cmd)
                {
                    ...
                    case "mine":
                        using (new FMDB())
                        {
                            Models.Account dbaccount = FMDB.Context.Accounts.Find(actor.Account.Id);
                            if (dbaccount == null)
                            {
                                actor.OutputHandler.Send("There is no such account.");
                                return;
                    ...
                    default:
                        if (cmd[0] == '+')
                        {
                            string subcmd = cmd.RemoveFirstCharacter();
                            if (subcmd.Length == 0)
                            {
        ...
        {
            string cmd = input.PopSpeech();
            if (!long.TryParse(cmd, out long vnum))
            {
...
#6*<room id>#0 - searches for groups with forms that explicitly contain the specified room",
        ...
        protected static void ItemGroup(ICharacter actor, string command)
        {
            ...
            IGameItemGroup itemgroup = long.TryParse(cmd, out long value)
        ...
        {
            IEnumerable<IGameItemGroup> itemgroups = actor.Gameworld.ItemGroups.AsEnumerable();
            while (!command.IsFinished)
            {
                ...
                {
                    case '*':
                        if (!long.TryParse(text[1..], out id))
                        {
                            actor.OutputHandler.Send($"There is no room with ID {text[1..].ColourValue()}.");
                            return;
                        }
            ...
            List<IGameItemProto> protos = actor.Gameworld.ItemProtos.GetAllApprovedOrMostRecent(true).ToList();
            actor.OutputHandler.Send(StringUtilities.GetTextTable(
        ...
        private static void ItemGroupShow(ICharacter actor, StringStack command)
        {
            ...
            IGameItemGroup itemgroup = long.TryParse(command.PopSpeech(), out long value)
                ? actor.Gameworld.ItemGroups.Get(value)
                : actor.Gameworld.ItemGroups.FirstOrDefault(
        ...
        private static void ItemGroupNew(ICharacter actor, StringStack command)
        {
            ...
            using (new FMDB())
            {
                ...
                FMDB.Context.SaveChanges();
                IGameItemGroup newItem = GameItemGroupFactory.CreateGameItemGroup(dbitem, actor.Gameworld);
...
{GenericReviewableSearchList}";
        ...
        protected static void ItemSkin(ICharacter actor, string input)
        {
            ...
            {
                List<IGameItemProto> protos = actor.Gameworld.ItemProtos
