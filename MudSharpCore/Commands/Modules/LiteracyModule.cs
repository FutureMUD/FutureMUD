...
public class LiteracyModule : Module<ICharacter>
{
	...
	private static bool BreakSealForAccess(ICharacter actor, GameItems.IGameItem target, string reason)
	{
		var sealable = target.GetItemType<ISealable>();
		if (sealable is null || !sealable.IsSealed)
		{
			return true;
		}
		...
		sealable.BreakSeal(actor, reason);
		return true;
	}
...
The syntax is simply #3scripts#0.", AutoHelp.HelpArg)]
    protected static void Scripts(ICharacter actor, string command)
    {
        ...
        foreach (IScript script in actor.Scripts)
        {
            sb.AppendLine(
...
For graffiti, see the #3look#0 command instead.", AutoHelp.HelpArgOrNoArg)]
    protected static void Read(ICharacter actor, string command)
    {
        ...
        GameItems.IGameItem target = actor.TargetItem(ss.PopSpeech());
        if (target == null)
        {
        ...
        IReadable targetAsReadable = target.GetItemType<IReadable>();
        ...
        IOpenable targetAsOpenable = target.GetItemType<IOpenable>();
		...
		if (!BreakSealForAccess(actor, target, "read"))
		{
			return;
		}
        ...
        {
            int textNumber = 1;
            foreach (ICanBeRead item in targetAsReadable.Readables)
            {
        ...
        ICanBeRead readable = targetAsReadable.Readables.ElementAtOrDefault(value - 1);
        if (readable == null)
        {
            actor.Send(
...
The syntax is as follows:
    ...
    protected static void Write(ICharacter actor, string command)
    {
        ...
        IWritingImplement implement = actor.Body.HeldOrWieldedItems.SelectNotNull(x => x.GetItemType<IWritingImplement>())
                             .FirstOrDefault();
        if (implement == null)
        {
            actor.Send(
        ...
        actor.EditorMode((text, handler, pars) =>
            {
                ...
                SimpleWriting writing = new(actor.Gameworld, actor, text)
                {
				...
				if (!BreakSealForAccess(actor, target, "written on"))
				{
					actor.Gameworld.SaveManager.Abort(writing);
					return;
				}
...
Both of these will drop you into an editor to describe your graffiti and any text on it.", AutoHelp.HelpArgOrNoArg)]
    protected static void Graffiti(ICharacter actor, string command)
    {
        ...
        GameItems.IGameItem target = actor.TargetItem(targetText);
        if (target is null)
        {
        ...
        string sdesc = ss.SafeRemainingArgument;
        if (sdesc.Length > 80)
        {
        ...
        IWritingImplement implement = actor.Body.HeldOrWieldedItems
...
To draw on locations or things that aren't normally meant for writing and drawing, see the #3graffiti#0 command.", AutoHelp.HelpArgOrNoArg)]
    protected static void DrawPicture(ICharacter actor, string command)
    {
        ...
        if (!ss.PopSpeech().TryParseEnum<DrawingSize>(out DrawingSize size))
        {
            actor.OutputHandler.Send(
        ...
        DummyDrawing dummy = new() { DrawingSize = size, ImplementType = implement.WritingImplementType, Author = actor };
        ...
        actor.EditorMode((text, handler, pars) =>
        {
			...
			if (!BreakSealForAccess(actor, target, "drawn on"))
			{
				actor.Gameworld.SaveManager.Abort(drawing);
				return;
			}
...
The syntax is #3title <item> <title>#0.", AutoHelp.HelpArgOrNoArg)]
    protected static void Title(ICharacter actor, string command)
    {
		...
		if (!BreakSealForAccess(actor, target, "retitled"))
		{
			return;
		}
