...
You can also use this command to remove similarly large items from containers. In this case the syntax is #3haul out <item> <container>#0.",
    ...
    protected static void Haul(ICharacter actor, string command)
    {
        ...
        IGameItem targetItem = actor.TargetLocalItem(ss.PopSpeech());
        if (targetItem == null)
        {
        ...
        IGameItem targetContainer = actor.TargetLocalItem(ss.PopSpeech());
        ...
        IContainer container = targetContainer.GetItemType<IContainer>();
        if (container == null)
        {
        ...
        {
            double dragCapacity = dragging.TheDrag.Draggers.Sum(x =>
    ...
    private static void HaulOut(ICharacter actor, StringStack ss)
    {
        ...
        string targetItemText = ss.PopSpeech();
        if (ss.IsFinished)
        {
        ...
        IContainer targetAsContainer = targetContainer.GetItemType<IContainer>();
        ...
        IGameItem targetItem = targetAsContainer.Contents.GetFromItemListByKeyword(targetItemText, actor);
        if (targetItem == null)
        {
...
The syntax is as follows:
    ...
    protected static void Style(ICharacter actor, string command)
    {
        ...
        {
            actor.Send(
                $"Which characteristic of theirs did you want to style? Available options are: {target.CharacteristicDefinitions.Where(x => x.Type == CharacteristicType.Growable).Select(x => $"{x.Name.Colour(Telnet.Green)} ({x.Pattern})").ListToString()}.");
            return;
        }
        ...
        List<IGrowableCharacteristicValue> possibleStyles = target.PossibleStyles(definition).ToList();
        if (ss.IsFinished)
        {
            ...
            IGrowableCharacteristicValue current = target.Body.GetCharacteristic(definition, null) as IGrowableCharacteristicValue;
            ITag universal = actor.Gameworld.Tags.Get(actor.Gameworld.GetStaticLong("UniversalStyleToolTagId"));
            ITag different = actor.Gameworld.Tags.Get(actor.Gameworld.GetStaticLong("DifferentGrowthStyleToolTagId"));
        ...
        IGrowableCharacteristicValue style = possibleStyles.FirstOrDefault(x => x.GetValue.EqualTo(text));
        if (style == null)
        {
            actor.Send(
...
Note - you can use the #3stop#0 command to stop dragging someone", AutoHelp.HelpArgOrNoArg)]
    protected static void Drag(ICharacter actor, string command)
    {
        ...
        if (ss.Peek().EqualTo("aid") || ss.Peek().EqualTo("help"))
        {
            ...
            ICharacter help = actor.TargetActor(ss.PopSpeech());
            if (help == null)
            {
            ...
            IDragging drag = help.EffectsOfType<Dragging>().FirstOrDefault() ?? help.EffectsOfType<DragHelper>().First().Drag;
            ...
            if (ss.Peek().EqualTo("by"))
            {
                ...
                if (tChar != null)
                {
                    IGameItem item = tChar.Body.WornItems.Concat(tChar.Body.Wounds.Select(x => x.Lodged))
                ...
                {
                    IGameItem tItem = drag.Target as IGameItem;
                    IGameItem item = tItem.AttachedAndConnectedItems.GetFromItemListByKeyword(ss.PopSpeech(), actor);
                    if (item == null)
                    {
                        actor.Send(
        ...
        IPerceivable target = actor.TargetLocal(ss.PopSpeech());
        if (target == null)
        {
        ...
        if (ss.Peek().EqualTo("by"))
        {
            ...
            {
                IGameItem tItem = target as IGameItem;
                IGameItem item = tItem.AttachedAndConnectedItems.GetFromItemListByKeyword(ss.PopSpeech(), actor);
                if (item == null)
                {
                    actor.Send(
...
The syntax is simply #3struggle#0, which can be used on a short delay and costs you stamina to attempt.", AutoHelp.HelpArg)]
    protected static void Struggle(ICharacter actor, string command)
    {
        ...
        IDragging drag = actor.EffectsOfType<DragTarget>().FirstOrDefault()?.Drag;
...
The syntax is #3flip <item>#0 or #3flip coin#0. You must be holding a single coin to use the flip coin version of the command.", AutoHelp.HelpArg)]
    protected static void Flip(ICharacter actor, string command)
    {
        ...
        {
            ss.PopSpeech();
            ICurrencyPile currency = actor.Body.HeldOrWieldedItems
            ...
            if (cheat)
            {
                ICheck check = actor.Gameworld.GetCheck(CheckType.CheatAtCoinFlip);
        ...
        IFlip flippable = target.GetItemType<IFlip>();
        if (flippable == null)
        {
...
The syntax to use this command is #3tear <item>#0.", AutoHelp.HelpArgOrNoArg)]
    protected static void Tear(ICharacter actor, string command)
    {
        ...
        ITearable tearable = target.GetItemType<ITearable>();
        if (tearable == null)
        {
        ...
        (bool truth, string error) = actor.CanManipulateItem(target);
        if (!truth)
        {
        ...
        IGameItem tornItem = tearable.Tear(actor, emote);
        if (tornItem != null)
        {
...
There are a couple of ways you can use this command:
    ...
    {
        ITurnable turnable = actor.Body.HeldOrWieldedItems.SelectNotNull(x => x.GetItemType<ITurnable>()).FirstOrDefault();
        StringStack ss = new(command.RemoveFirstWord());
        if (ss.IsFinished)
        {
            if (turnable == null)
        ...
        (bool truth, string error) = actor.CanManipulateItem(item);
        if (!truth)
        {
            actor.OutputHandler.Send(error);
            return;
        ...
        {
            extent = extent[1..];
            if (!double.TryParse(extent, out double value))
            {
        ...
        if (!double.TryParse(extent, out double dvalue))
        {
...
If you don't specify an amount, you will use the whole amount of the cream.", AutoHelp.HelpArgOrNoArg)]
    protected static void Apply(ICharacter character, string command)
    {
        ...
        IGameItem item = character.TargetHeldItem(ss.PopSpeech());
        if (item == null)
        {
        ...
        IApply applicable = item.GetItemType<IApply>();
        if (applicable == null)
        {
        ...
        ICharacter targetCharacter = character.TargetActor(ss.PopSpeech());
        ...
        Body.IBodypart bodypart = targetCharacter.Body.GetTargetBodypart(ss.PopSpeech());
        if (bodypart == null)
        {
        ...
        string emote = ss.PopParentheses();
        double amount = 0.0;
        if (!ss.IsFinished)
        {
            amount = character.Gameworld.UnitManager.GetBaseUnits(ss.SafeRemainingArgument, UnitType.Mass, out bool success);
            if (!success)
            {
...
The syntax is #3inject <item> <target> <bodypart> [<amount>]#0.", AutoHelp.HelpArgOrNoArg)]
    protected static void Inject(ICharacter character, string command)
    {
        ...
        IGameItem target = character.TargetHeldItem(ss.PopSpeech());
        if (target == null)
        {
        ...
        IInject targetInjectable = target.GetItemType<IInject>();
...
You can use the following syntaxes with this command:
    ...
    protected static void Feed(ICharacter character, string command)
    {
        ...
        if (ss.PeekSpeech().EqualTo("from"))
        {
            ...
            IGameItem container = character.TargetHeldItem(ss.PopSpeech());
            if (container == null)
            {
            ...
            ILiquidContainer liquidContainer = container.GetItemType<ILiquidContainer>();
            ...
            void ExecuteForceLiquid()
            {
                ...
                LiquidMixture liquid = liquidContainer.LiquidMixture;
        ...
        ISwallowable swallow = item.GetItemType<ISwallowable>();
        IEdible edible = item.GetItemType<IEdible>();
        ...
        void ExecuteForceFeed()
        {
            ...
            IGameItem newItem = item.DropsWhole(1) ? item : item.Get(target.Body, 1);
...
The syntax is as follows:
    ...
    protected static void Swallow(ICharacter character, string command)
    {
        ...
        Match match = EatRegex.Match(command.RemoveFirstWord());
        if (!match.Success)
        {
        ...
        {
            IGameItem tableItem = character.TargetLocalItem(match.Groups["table"].Value);
            if (tableItem == null)
            {
        ...
        IGameItem target = container == null
        ...
        ISwallowable targetAsSwallowable = target.GetItemType<ISwallowable>();
        ...
        PlayerEmote emote = new(match.Groups["emote"].Value, character);
        if (!emote.Valid)
        {
    ...
    {
        double bites = match.Groups["bites"].Length > 0 ? double.Parse(match.Groups["bites"].Value) : 1;
        PlayerEmote emote = new(match.Groups["emote"].Value, character);
        if (!emote.Valid)
        {
            character.Send(emote.ErrorMessage);
        ...
        string yield = match.Groups["food"].Value.ToLowerInvariant();
...
Additionally, if you can eat foragable yields, the syntax is as per below:
    ...
    protected static void Eat(ICharacter character, string command)
    {
        ...
        IEdible targetAsFood = target.GetItemType<IEdible>();
        ICorpse targetAsCorpse = target.GetItemType<ICorpse>();
        ISeveredBodypart targetAsBodypart = target.GetItemType<ISeveredBodypart>();
        ...
        double bites = match.Groups["all"].Length > 0 ? 0 :
...
#6Note - for the amount, you can either use a quantity with units (e.g. 30ml/1oz) or if you use a number, it's a multiplier of the default sip amount.#0";
    ...
    {
        string text = command.RemoveFirstWord();
        ...
        ILiquidContainer targetAsDrink = target.GetItemType<ILiquidContainer>();
...
The syntax to use this command is as follows:
    ...
    protected static void Fill(ICharacter character, string command)
    {
        ...
        ILiquidContainer targetAsContainer = target.GetItemType<ILiquidContainer>();
        if (targetAsContainer == null)
        {
        ...
        ILiquidContainer containerAsContainer = container.GetItemType<ILiquidContainer>();
    ...
    protected static void FillGas(ICharacter character, string command)
    {
        ...
        IGasContainer targetContainer = target.GetItemType<IGasContainer>();
        if (targetContainer == null)
        {
        ...
        IGasContainer sourceContainer = source.GetItemType<IGasContainer>();
...
The syntax is as follows:
    ...
    protected static void Empty(ICharacter actor, string command)
    {
        ...
        ILiquidContainer liquidContainer = target.GetItemType<ILiquidContainer>();
        if (liquidContainer != null)
        {
            if (!liquidContainer.IsOpen)
            {
            ...
            double amount = liquidContainer.LiquidMixture.TotalVolume;
            if (!ss.IsFinished)
            {
        ...
        IContainer container = target.GetItemType<IContainer>();
        if (container == null)
        {
            actor.OutputHandler.Send(
        ...
        if (!ss.IsFinished)
        {
            string secondTargetText = ss.PopSafe();
            IGameItem secondTarget = actor.TargetItem(secondTargetText);
            ...
            (bool truth, string error) = actor.CanManipulateItem(secondTarget);
            if (!truth)
            {
                actor.OutputHandler.Send(error);
                return;
            ...
            IContainer secondContainer = secondTarget.GetItemType<IContainer>();
...
The syntax is as follows:
    ...
    protected static void Pour(ICharacter actor, string command)
    {
        ...
        ILiquidContainer fromItemContainer = fromItem.GetItemType<ILiquidContainer>();
        ...
        (bool truth, string error) = actor.CanManipulateItem(intoItem);
        if (!truth)
        {
            actor.Send(error);
            return;
        ...
        ILiquidContainer intoItemContainer = intoItem.GetItemType<ILiquidContainer>();
...
The syntax is as follows:
    ...
    {
        Match match = SpillCommandRegex.Match(command.RemoveFirstWord());
        if (!match.Success)
        {
        ...
        IGameItem vessel = character.TargetHeldItem(match.Groups["vessel"].Value);
        if (vessel == null)
        {
        ...
        ILiquidContainer vesselContainer = vessel.GetItemType<ILiquidContainer>();
        ...
        {
            ICheck check1 = character.Gameworld.GetCheck(CheckType.SpillLiquidOnPerson);
            double proximityBonus = character.GetProximity(charTarget).In(Proximity.Intimate, Proximity.Immediate)
                ? character.Gameworld.GetStaticDouble("SpillCharacterProximityBonus")
                : 0.0;
            Dictionary<Difficulty, CheckOutcome> results1 =
                check1.CheckAgainstAllDifficulties(character, Difficulty.Normal, null, charTarget, proximityBonus);

            ICheck check2 = character.Gameworld.GetCheck(CheckType.DodgeSpillLiquidOnPerson);
            double unawareBonus = charTarget.IsBlocked("general", "move").Truth
                ? 0.0
                : character.Gameworld.GetStaticDouble("SpillCharacterUnawareBonus");
            Dictionary<Difficulty, CheckOutcome> results2 =
                check2.CheckAgainstAllDifficulties(charTarget, Difficulty.Normal, null, character, unawareBonus);

            OpposedOutcome outcome = new(results1, results2, Difficulty.Normal, Difficulty.Normal);
            if (outcome.Outcome == OpposedOutcomeDirection.Opponent)
            {
    ...
    protected static void Puff(ICharacter character, string command)
    {
        ...
        IPuffable puffable = target.GetItemType<IPuffable>();
        if (puffable == null)
        {
    ...
    protected static void Light(ICharacter character, string command)
    {
        ...
        IGameItem target = character.TargetItem(ss.PopSpeech());
        if (target == null)
        {
        ...
        string ignitionText = ss.PopSpeech();
        IPerceivable ignition = character.Target(ignitionText);
    ...
    protected static void Install(ICharacter character, string command)
    {
        ...
        string installText = ss.PopSpeech();
        if (ss.IsFinished)
        {
        ...
        IGameItem target = character.TargetHeldItem(installText);
        if (target == null)
        {
        ...
        ILock targetAsLock = target.GetItemType<ILock>();
    ...
    private static void InstallDoor(ICharacter character, StringStack ss, IGameItem doorItem, IDoor door)
    {
        ...
        void doorInstallCompletionAction(IPerceivable perceivable)
        {
            if (exit.Exit.Door == null)
            {
                ...
                foreach (ILock lo in door.Locks.ToList())
                {
        ...
        ILockable lockable)
    {
        ...
        {
            IGameItem targetLock = lockable.Locks.Select(x => x.Parent).GetFromItemListByKeyword(ss.PopSpeech(), character);
            if (targetLock == null)
            {
    ...
    protected static void Junk(ICharacter character, string command)
    {
        ...
        IGameItem item = character.TargetHeldItem(cmd);
        if (item == null)
        {
    ...
    protected static void Open(ICharacter actor, string command)
    {
        ...
        {
            IGameItem doorItem = cellExits.SelectNotNull(x => x.Exit.Door)
                                    .Select(x => x.Parent)
            ...
            {
                IGameItem item = actor.TargetItem(cmd);
                if (item != null)
                {
                    openable = item.GetItemType<IOpenable>();
                    if (openable == null)
        ...
        (bool truth, string error) = actor.CanManipulateItem(openable.Parent);
        if (!truth)
        {
            actor.OutputHandler.Send(error);
            return;
...
The syntax is as follows:
    ...
    protected static void Attach(ICharacter actor, string command)
    {
        ...
        IGameItem targetItem = actor.TargetHeldItem(cmd);
        if (targetItem == null)
        {
        ...
        IBelt targetBeltAsBelt = targetBelt.GetItemType<IBelt>();
    ...
    private static void AttachProsthetic(ICharacter actor, IGameItem targetItem, StringStack ss)
    {
        ...
        IProsthetic targetProsthetic = targetItem.GetItemType<IProsthetic>();
        ...
        bool delayed)
    {
        ...
        List<Body.IBodypart> affectedParts =
    ...
    protected static void Detach(ICharacter actor, string command)
    {
        ...
        if (targetBelt?.GetItemType<IBelt>() == null)
        {
            ...
            ICharacter targetActor = actor.TargetActor(cmd);
            if (targetActor != null)
            {
                targetBelt =
        ...
        IGameItem targetItem = targetBeltAsBelt.ConnectedItems.Select(x => x.Parent).GetFromItemListByKeyword(cmd, actor);
        if (targetItem == null)
        {
    ...
    protected static void Close(ICharacter actor, string command)
    {
        ...
        else
        {
            ...
            {
                IGameItem item = actor.Body.TargetItem(cmd);
                if (item != null)
                {
                    openable = item.GetItemType<IOpenable>();
                    if (openable == null)
...
The syntax is as follows:
    ...
    protected static void Lock(ICharacter actor, string command)
    {
        ...
        (bool truth, string error) = actor.CanManipulateItem(targetItem);
        if (!truth)
        {
            actor.OutputHandler.Send(error);
            return;
        ...
        IKey whichKey = null;
        List<IKey> heldKeys = actor.Body.HeldOrWieldedItems.SelectNotNull(x => x.GetItemType<IKey>()).ToList();
        if (theLock is not null)
        {
            ...
            foreach (IKey key in heldKeys.Where(key => theLock.CanLock(actor, key)))
            {
                whichKey = key;
                break;
            }
        ...
        foreach (ILock item in lockable.Locks.Where(x => !x.IsLocked).ToList())
        {
            ...
            foreach (IKey key in heldKeys.Where(x => item.CanLock(actor, x)))
            {
        ...
        if (lockable is ILock selfLock)
        {
            ...
            foreach (IKey key in heldKeys.Where(x => selfLock.CanLock(actor, x)))
            {
...
The syntax is as follows:
    ...
    protected static void Unlock(ICharacter actor, string command)
    {
        ...
        if (theLock is not null)
        {
            ...
            foreach (IKey key in heldKeys.Where(key => theLock.CanUnlock(actor, key)))
            {
                whichKey = key;
                break;
            }
        ...
        foreach (ILock item in lockable.Locks.Where(x => x.IsLocked).ToList())
        {
            ...
            foreach (IKey key in heldKeys.Where(x => item.CanUnlock(actor, x)))
            {
        ...
        if (lockable is ILock selfLock)
        {
            ...
            foreach (IKey key in heldKeys.Where(x => selfLock.CanUnlock(actor, x)))
            {
...
The syntax for this command is as follows:
    ...
    protected static void Connect(ICharacter actor, string command)
    {
        ...
        IGameItem target1 = actor.TargetItem(ss.PopSpeech());
        if (target1 == null)
        {
        ...
        List<IConnectable> target1Connectables = target1.GetItemTypes<IConnectable>().ToList();
        ...
        IGameItem target2 = actor.TargetItem(ss.PopSpeech());
        if (target2 == null)
        {
        ...
        List<IConnectable> target2Connectables = target2.GetItemTypes<IConnectable>().ToList();
        ...
        (bool truth, string error) = actor.CanManipulateItem(target1);
        if (!truth)
        {
            actor.OutputHandler.Send(error);
            return;
        ...
        foreach (IConnectable connectable1 in target1Connectables)
        {
            foreach (IConnectable connectable2 in target2Connectables)
            {
    ...
    protected static void Disconnect(ICharacter actor, string command)
    {
        ...
        IGameItem target1 = actor.TargetItem(ss.PopSafe());
        if (target1 == null)
        {
        ...
        IConnectable target1AsConnectable = target1.GetItemType<IConnectable>();
        ...
        ICharacter targetActor = null;
        IGameItem target2 = actor.TargetItem(ss.PopSafe());
        if (target2 == null)
        {
            targetActor = actor.TargetActor(ss.Last);
            if (targetActor != null)
        ...
        IConnectable target2AsConnectable = target2.GetItemType<IConnectable>();
    ...
    protected static void Select(ICharacter actor, string command)
    {
        ...
        PlayerEmote emote = null;
        string argumentText = "";
        Match regexResult = ArgumentsAndEmoteRegex.Match(ss.SafeRemainingArgument ?? "");
        ...
        StringStack argSS = new(argumentText);
        argSS.PopSpeechAll();
        ...
        (bool truth, string error) = actor.CanManipulateItem(selectable.Parent);
        if (!truth)
        {
            actor.OutputHandler.Send(error);
            return;
...
The syntax is as follows:
    ...
    protected static void Switch(ICharacter actor, string command)
    {
        ...
        List<ISwitchable> switchables = target
        ...
        string switchText = ss.PopSpeech();
        ...
        actor.OutputHandler.Handle(
            new MixedEmoteOutput(new Emote($"@ switch|switches $0 to '{ss.Last.ToLowerInvariant()}'", actor,
                target)).Append(emote));
        foreach (ISwitchable switchable in canSwitchables)
        {
...
#nullable enable
    private static ILock? GetTargetLock(ICharacter actor, IGameItem target, string lockName)
    {
        ...
        {
            ILockable lockable = target.GetItemType<ILockable>();
...
#nullable restore
    ...
    private static void LockSmithSet(ICharacter actor, StringStack ss)
    {
        ...
        string targetName = ss.PopSpeech();
        IGameItem target = actor.TargetItem(targetName);
        if (target == null)
        {
        ...
        (bool truth, string error) = actor.CanManipulateItem(targetLock.Parent);
        if (!truth)
        {
            actor.OutputHandler.Send(error);
            return;
        ...
        void finalAction(IPerceivable perceivable)
        {
            ...
            CheckOutcome finalOutcome = actor.Gameworld.GetCheck(CheckType.LocksmithingCheck)
    ...
    private static void LockSmithPair(ICharacter actor, StringStack ss)
    {
        ...
        IGameItem targetKeyItem = actor.TargetHeldItem(keyName);
        ...
        IKey targetKey = targetKeyItem.GetItemType<IKey>();
        ...
        void finalAction(IPerceivable perceivable)
        {
            ...
            CheckOutcome checkOutcome = actor.Gameworld.GetCheck(CheckType.LocksmithingCheck)
    ...
    private static void LockSmithUnlock(ICharacter actor, StringStack ss)
    {
        ...
        string targetName = ss.PopSpeech();
        string lockName = ss.IsFinished ? string.Empty : ss.PopSpeech();
        ...
        ILocksmithingTool lockTool = actor.Body.HeldItems.SelectNotNull(x => x.GetItemType<ILocksmithingTool>()).FirstOrDefault();
        ...
        void finalAction(IPerceivable perceivable)
        {
            ...
            CheckOutcome outcome = actor.Gameworld.GetCheck(CheckType.LocksmithingCheck)
    ...
    protected static void Sharpen(ICharacter actor, string command)
    {
        ...
        IGameItem sharpener = actor.TargetItem(ss.PopSpeech());
        if (sharpener == null)
        {
        ...
        (bool truth, string error) = actor.CanManipulateItem(sharpener);
        if (!truth)
        {
            actor.OutputHandler.Send(error);
            return;
        ...
        ISharpen sharpenerAsSharpen = sharpener.GetItemType<ISharpen>();
    ...
    protected static void Lob(ICharacter actor, string command)
    {
        ...
        Difficulty difficulty = actor.ArmourUseCheckDifficulty(ArmourUseDifficultyContext.LobItem);
        string tossEmote;
        Outcome minimumSuccessThreshold = Outcome.MinorPass;
        if (exit is not null)
        {
        ...
        ICheck check = actor.Gameworld.GetCheck(CheckType.TossItemCheck);
        CheckOutcome result = check.Check(actor, difficulty);
        ...
        if (result.Outcome < minimumSuccessThreshold)
        {
            ...
            if (exit == null)
            {
                List<RoomLayer> intermediates = actor.Location.Terrain(actor).TerrainLayers
                                         .IntermediateLayers(actor.RoomLayer, layer).ToList();
                if (intermediates.Any())
                {
                    RoomLayer random = intermediates.GetRandomElement();
                    actor.OutputHandler.Handle(new EmoteOutput(new Emote(
    ...
    protected static void Bundle(ICharacter actor, string command)
    {
        ...
        while (!ss.IsFinished)
        {
            IGameItem target = actor.TargetLocalOrHeldItem(ss.PopSpeech());
            if (target == null)
            {
                actor.OutputHandler.Send(
        ...
        IGameItem bundle = PileGameItemComponentProto.CreateNewBundle(items);
    ...
    protected static void Roll(ICharacter actor, string command)
    {
        ...
        PileGameItemComponent targetBundle = target.GetItemType<PileGameItemComponent>();
        ...
        IDice targetDice = target.GetItemType<IDice>();
        ...
        foreach (IDice die in dice)
        {
            for (int i = 0; i < die.Parent.Quantity; i++)
            {
