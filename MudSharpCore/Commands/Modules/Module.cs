...
public abstract class Module<T> : IModule
{
    ...
    {
        foreach (Tuple<IEnumerable<string>, Command<T>> dissection in
    ...
    {
        PlayerCommand playerCommand = (PlayerCommand)commandMethod.GetCustomAttributes(typeof(PlayerCommand), false)[0];
        RequiredCharacterState requiredCharacterState =
        ...
        DelayBlock delayBlock = (DelayBlock)commandMethod.GetCustomAttributes(typeof(DelayBlock), false).FirstOrDefault();
        IExecutable<T>.CommandGenericMethodDelegate method =
        ...
        HelpInfo helpInfoData = (HelpInfo)commandMethod.GetCustomAttribute(typeof(HelpInfo), false);
        ConditionalCommandAppearance conditions =
        ...
        CommandDisplayOptions commandDisplayOptions = displayOptions?.Options ?? CommandDisplayOptions.None;
        CharacterState requiredState = requiredCharacterState?.State ?? CharacterState.Any;
        PermissionLevel permission = commandPermission?.PermissionLevel ?? PermissionLevel.Any;
    ...
    public override string ToString()
    {
        ...
        foreach (string command in Commands.ReportCommands(PermissionLevel.Any, default))
        {
