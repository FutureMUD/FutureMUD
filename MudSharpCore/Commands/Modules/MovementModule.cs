...
internal class MovementModule : Module<ICharacter>
{
	...
	{
		actor.Send("The follow command is temporarily disabled. See the PARTY command in the meantime.");
		return;
		/*
		var ss = new StringStack(input.RemoveFirstWord());
		var starget = ss.Pop();

		if (starget.Equals("help", StringComparison.InvariantCultureIgnoreCase) ||
		    starget.Equals("?", StringComparison.InvariantCultureIgnoreCase)) {
		    actor.Send(
		        $"The syntax is {"follow <target>".Colour(Telnet.Yellow)} or {"follow self".Colour(Telnet.Yellow)} to stop following.");
		    return;
		}

		var target = string.IsNullOrEmpty(starget) ? actor : actor.TargetActor(starget);
		...
		if (target == actor) {
		    ...
		    actor.Send("You stop following {0}.", actor.Following);
		    actor.Follow(null);
		    return;
		}

		actor.Follow(target);
		actor.Send("You will now try to follow {0}.", actor.Following);
		*/
	}
		...
		else
		{
			...
			switch (cmd)
			{
				...
				case "eject":
					...
					break;
				default:
					actor.OutputHandler.Send("That is not a valid option to use with the party command.");
					return;
			}
		}
	}
