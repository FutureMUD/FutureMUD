...
internal class MovementModule : Module<ICharacter>
{
    ...
    protected static void Move(ICharacter actor, string input)
    {
        ...
        PlayerEmote emote = new(ss.PopParentheses(), actor.Body);
...
The syntax for this command is as follows:
    ...
    protected static void Party(ICharacter actor, string input)
    {
        ...
        if (cmd.Length == 0)
        {
            ...
            StringBuilder sb = new("Your party consists of the following:\n\n");
        ...
        switch (cmd)
        {
            case "speed":
                ...
                IMoveSpeed speed = actor.Speeds.GetByIdOrName(cmd);
                if (speed is null)
                {
    ...
    protected static void Speed(ICharacter actor, string input)
    {
        ...
        if (cmd.Length == 0)
        {
            ...
            foreach (IPositionState position in actor.Speeds.Select(x => x.Position).Distinct())
            {
                sb.AppendLine($"\tWhen {position.DescribeLocationMovementParticiple.TitleCase().ColourValue()}");
                foreach (IMoveSpeed item in actor.Speeds.Where(x => x.Position == position)
        ...
        IMoveSpeed speed = actor.Speeds.FirstOrDefault(x => x.Name.EqualTo(cmd)) ??
