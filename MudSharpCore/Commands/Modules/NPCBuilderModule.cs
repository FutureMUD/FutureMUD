...
namespace MudSharp.Commands.Modules
{
    internal class NPCBuilderModule : BaseBuilderModule
    {
        ...
        private static void NPCMake(ICharacter character, StringStack command)
        {
            ...
            {
                ICharacter target = character.Gameworld.TryGetCharacter(value, true);
                if (target == null)
                {
                ...
                SimpleNPCTemplate template = new(character.Gameworld, character.Account,
            ...
            ICharacter tch = character.TargetActorOrCorpse(command.Last);
            if (tch == null)
            {
            ...
            SimpleNPCTemplate ctemplate = new(character.Gameworld, character.Account, tch.GetCharacterTemplate(),
                command.PopSpeech());
...
The core syntax to use this command is as follows:
	...
	#3npc list [<filters>]#0 - lists all NPC prototypes. See below for filters:
        ...
        private static void NPCInstances(ICharacter actor, StringStack ss)
        {
            ...
            INPCTemplate template = actor.Gameworld.NpcTemplates.GetByIdOrName(ss.SafeRemainingArgument);
            if (template is null)
            {
                actor.OutputHandler.Send("There is no such NPC Template.");
                return;
            ...
            List<INPC> npcs = actor.Gameworld.NPCs.OfType<INPC>().Where(x => x.Template.Id == template.Id).ToList();
...
You can use the following options:
	...
	#3gait set emote <#> maxalert <alertness>#0 - sets maximum group alertness";
        ...
        private static void GroupAITemplateView(ICharacter actor, StringStack ss)
        {
            if (ss.IsFinished)
            {
                IGroupAITemplate editing = actor.CombinedEffectsOfType<BuilderEditingEffect<IGroupAITemplate>>().FirstOrDefault()
                                   ?.EditingItem;
                if (editing == null)
                {
            ...
            IGroupAITemplate item = long.TryParse(ss.PopSpeech(), out long value)
        ...
        private static void GroupAITemplateClone(ICharacter actor, StringStack ss)
        {
            ...
            IGroupAITemplate original = long.TryParse(ss.PopSpeech(), out long value)
                ? actor.Gameworld.GroupAITemplates.Get(value)
                : actor.Gameworld.GroupAITemplates.GetByName(ss.Last);
            if (original == null)
            {
            ...
            string nameText = ss.PopSpeech().TitleCase();
            ...
            GroupAITemplate newItem = new((GroupAITemplate)original, nameText);
            actor.RemoveAllEffects<BuilderEditingEffect<IGroupAITemplate>>();
        ...
        private static void GroupAITemplateEdit(ICharacter actor, StringStack ss)
        {
            ...
            IGroupAITemplate template = long.TryParse(ss.PopSpeech(), out long value)
                ? actor.Gameworld.GroupAITemplates.Get(value)
                : actor.Gameworld.GroupAITemplates.GetByName(ss.Last);
            if (template == null)
            {
        ...
        private static void GroupAITemplateNew(ICharacter actor, StringStack ss)
        {
            ...
            GroupAITemplate template = new(actor.Gameworld, nameText);
            actor.RemoveAllEffects<BuilderEditingEffect<IGroupAITemplate>>();
            actor.AddEffect(new BuilderEditingEffect<IGroupAITemplate>(actor) { EditingItem = template });
            actor.OutputHandler.Send(
...
You can use the following sub-commands:
	...
	#3group setalertness <which> <alertness>#0 - overrides the alertness level of a group";
        ...
        private static void GroupSetAlertness(ICharacter actor, StringStack ss)
        {
            ...
            IGroupAI group = long.TryParse(ss.PopSpeech(), out long value)
            ...
            if (!ss.PopSpeech().TryParseEnum<GroupAlertness>(out GroupAlertness alertness))
            {
                actor.OutputHandler.Send(
        ...
        private static void GroupSetAction(ICharacter actor, StringStack ss)
        {
            ...
            if (!ss.PopSpeech().TryParseEnum<GroupAction>(out GroupAction action))
            {
                actor.OutputHandler.Send(
        ...
        private static void GroupNew(ICharacter actor, StringStack ss)
        {
            ...
            (bool success, string error) = template.IsValidForCreatingGroups;
            if (!success)
            {
            ...
            GroupAI group = new(template, name);
            actor.OutputHandler.Send(
...
The following options are available as filters with the #3list#0 subcommand:
	...
	#6-<keyword>#0 - only show AIs without the keyword in the name";
        ...
        private static void AITypeHelp(ICharacter actor, StringStack ss)
        {
            ...
            (bool success, string text) = ArtificialIntelligenceBase.GetTypeHelp(type);
            if (!success)
            {
        ...
        private static void AITypes(ICharacter actor)
        {
            ...
            foreach (string type in ArtificialIntelligenceBase.AITypes)
            {
        ...
        private static void AIRemove(ICharacter actor, StringStack ss)
        {
            ...
            IArtificialIntelligence ai = actor.Gameworld.AIs.GetByIdOrName(ss.PopSpeech());
            if (ai is null)
            {
                actor.OutputHandler.Send("There is no such AI.");
                return;
