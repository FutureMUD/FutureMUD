...
The syntax is simply #3survey#0.", AutoHelp.HelpArg)]
    protected static void Survey(ICharacter actor, string input)
    {
        ...
        IProperty property = actor.Gameworld.Properties.FirstOrDefault(x => x.PropertyLocations.Contains(actor.Location));
        if (property != null)
        {
        ...
        double illumination = actor.Location.CurrentIllumination(actor);
        ...
        Difficulty spotDifficulty = terrain.SpotDifficulty;
        ...
        if (actor.IsAdministrator())
        {
            ...
            else
            {
                ...
                foreach (string type in actor.Location.ForagableTypes)
                {
                    double yield = actor.Location.GetForagableYield(type);
                    if (yield > 0.0)
                    {
            ...
            {
                foreach (ICharacter owner in territoryOwners)
                {
                    sb.AppendLine(
        ...
        {
            foreach (EdibleForagableYield yield in actor.Race.EdibleForagableYields)
            {
                double amount = actor.Location.GetForagableYield(yield.YieldType);
                int bites = (int)(amount / yield.YieldPerBite);
                if (bites > 0.0)
                {
                    sb.AppendLine(
...
See also: HELP EVALUATE, HELP SEARCH, HELP SCAN",
    ...
    protected static void Look(ICharacter actor, string input)
    {
        ...
        IPerceivable target = actor.Target(arg);
        ...
        if (target == null)
        {
            ...
            List<IGameItemGroup> groups = actor.Location.LayerGameItems(actor.RoomLayer).Where(x => actor.CanSee(x))
                              ...
                              .ToList();
            List<PuddleGameItemComponent> puddles = localItems.SelectNotNull(x => x.GetItemType<PuddleGameItemComponent>()).ToList();
            ...
            ITag commodityTag = actor.Gameworld.Tags.Get(actor.Gameworld.GetStaticLong("PuddleResidueTagId"));
            List<ICommodity> residues = (commodityTag is not null ? localItems
            ...
            IGameItemGroup targetGroup = groups.GetFromItemListByKeyword(arg, actor);
            if (targetGroup == null)
            {
        ...
        if (!ss.IsFinished)
        {
            string text = ss.PopSafe();
            // look <item> <lock>
            if (!lookin && ((target as IGameItem)?.IsItemType<ILockable>() ?? false))
            {
                ILockable targetLockable = (target as IGameItem).GetItemType<ILockable>();
                IGameItem targetlock = targetLockable.Locks.Select(x => x.Parent).GetFromItemListByKeyword(text, actor);
                if (targetlock != null)
                {
            ...
            if (target is ICharacter targetActor)
            {
                if (text.EqualTo("tattoos"))
                {
                    if (!ss.IsFinished)
                    {
                        IBodypart targetPart = targetActor.Body.GetTargetPart(ss.PopSpeech());
    ...
    private static void DisplayForScan(ICharacter actor, IEnumerable<ICharacter> characters, StringBuilder sb,
        bool useLayerTags)
    {
        foreach (IMovement move in characters.SelectNotNull(x => x.Movement).Distinct().Where(x => x.SeenBy(actor)))
        {
            ...
            sb.AppendLine();
            foreach (ICharacter ch in move.CharacterMovers.Where(x => actor.CanSee(x)))
            {
                actor.SeeTarget(ch);
            }
        }

        foreach (ICharacter ch in characters.Where(x => x.Movement == null))
        {
    ...
    private static void DisplayForScan(ICharacter actor, IEnumerable<IGameItem> items, StringBuilder sb,
        bool useLayerTags)
    {
        IEnumerable<IGrouping<RoomLayer, IGameItem>> layerItems = items.GroupBy(x => x.RoomLayer);
        foreach (IGrouping<RoomLayer, IGameItem> layerItem in layerItems)
        {
            ...
            {
                foreach (IGrouping<IGameItemGroup, IGameItem> group in layerItem.GroupBy(x => x.ItemGroup).OrderBy(x => x.Key == null))
                {
                    if (group.Key != null)
                    {
...
See also the #3scan#0, #3longscan#0 and #3search#0 commands.", AutoHelp.HelpArg)]
    protected static void QuickScan(ICharacter actor, string input)
    {
        ...
        {
            string sizeArg = ss.PopSpeech();
            exitArg = ss.IsFinished ? "" : ss.SafeRemainingArgument;

            if (sizeArg.TryParseEnum<SizeCategory>(out SizeCategory setSize))
            {
                userSetSize = setSize;
            }
            else
        ...
        ICheck check = actor.Gameworld.GetCheck(CheckType.QuickscanPerceptionCheck);
        Dictionary<Difficulty, CheckOutcome> allOutcomes = check.CheckAgainstAllDifficulties(actor, Difficulty.Normal, null);
        Difficulty difficulty = actor.Location.SpotDifficulty(actor).StageDown(1);
        CheckOutcome outcome = allOutcomes[difficulty];
        SizeCategory minSize = actor.CurrentContextualSize(SizeContext.None);
        ...
        foreach (ICellExit exit in exits)
        {
            ...
            List<IGameItem> items = location.GameItems.Where(x =>
...
See also the #3quickscan#0, #3longscan#0 and #3search#0 commands.", AutoHelp.HelpArg)]
    ...
    protected static void Scan(ICharacter actor, string input)
    {
        ...
        ICheck check = actor.Gameworld.GetCheck(CheckType.ScanPerceptionCheck);
        Dictionary<Difficulty, CheckOutcome> allOutcomes = check.CheckAgainstAllDifficulties(actor, Difficulty.Normal, null);
        Difficulty difficulty = actor.Location.SpotDifficulty(actor);
        CheckOutcome outcome = allOutcomes[difficulty];
        SizeCategory minSize = actor.CurrentContextualSize(SizeContext.None).ChangeSize(-2);
        switch (outcome.Outcome)
        {
            case Outcome.MinorPass:
                minSize = minSize.ChangeSize(-1);
...
See also the #3quickscan#0, #3scan#0 and #3search#0 commands.",
    ...
    protected static void LongScan(ICharacter actor, string input)
    {
        ...
        ICheck check = actor.Gameworld.GetCheck(CheckType.LongscanPerceptionCheck);
        Dictionary<Difficulty, CheckOutcome> allOutcomes = check.CheckAgainstAllDifficulties(actor, Difficulty.Normal, null);
        Difficulty difficulty = actor.Location.SpotDifficulty(actor);
        CheckOutcome outcome = allOutcomes[difficulty];
        SizeCategory minSize = actor.CurrentContextualSize(SizeContext.None).ChangeSize(-3);
        switch (outcome.Outcome)
        {
            case Outcome.MinorPass:
                minSize = minSize.ChangeSize(-1);
...
Note: The targets are ordered in the same order that they appear in the #3targets#0 command for the purposes of working out the best keywords.",
    ...
    protected static void Point(ICharacter actor, string input)
    {
        ...
        else
        {
            ...
            string distanceDescriptor = "extremely far";
