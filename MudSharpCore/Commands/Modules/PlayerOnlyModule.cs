...
The syntax for this command is as follows:
  ...
  #3journal write <title>#0 - drops you into an editor to write a journal entry";
    ...
    protected static void Journal(ICharacter actor, string command)
    {
        ...
        using (new FMDB())
        {
            Models.Account account = FMDB.Context.Accounts.Find(actor.Account.Id);
            if (account == null)
            {
            ...
            switch (ss.PopSpeech().ToLowerInvariant())
            {
                case "read":
                    ...
                    string firstArgument = readArguments.PopSpeech();
                    if (!int.TryParse(firstArgument, out index))
                    {
                        ...
                        List<(Models.Character Character, string Name)> pastCharacters = FMDB.Context.Characters
                    ...
                    List<Models.AccountNote> notes = FMDB.Context.AccountNotes
                    ...
                    Models.AccountNote note = notes[index - 1];
                ...
                case "write":
                    ...
                    string title = ss.SafeRemainingArgument.TitleCase();
                ...
                case "history":
                    if (ss.IsFinished)
                    {
                        List<string> whoHasNotes = FMDB.Context.AccountNotes
                    ...
                    string targetName = ss.SafeRemainingArgument;
                    List<(Models.Character Character, string Name)> formerCharacters = FMDB.Context.Characters
                                               .Where(x => x.AccountId == account.Id)
                                               .Distinct()
                                               .AsEnumerable()
                                               .Select(x => (Character: x,
                    ...
                    Models.Character lookupCharacter =
    ...
    private static void TypoWritePost(string message, IOutputHandler handler, object[] arguments)
    {
        ...
        IBoard board = actor.Gameworld.Boards.Get(actor.Gameworld.GetStaticLong("TyposBoardId"));
        if (board is null)
        {
            using (new FMDB())
            {
                ...
                Models.StaticConfiguration dbitem = FMDB.Context.StaticConfigurations.Find("TyposBoardId");
                if (dbitem == null)
                {
    ...
    protected static void Roles(ICharacter actor, string command)
    {
        ...
        if (!actor.IsAdministrator() || ss.IsFinished)
        {
            ...
            foreach (IChargenRole role in actor.Roles.OrderBy(x => x.RoleType))
            {
        ...
        foreach (IChargenRole role in target.Roles.OrderBy(x => x.RoleType))
        {
            sb.AppendLine($"\t[{role.RoleType}] {role.Name.TitleCase()}");
        }
    ...
    protected static void Names(ICharacter actor, string command)
    {
        ...
        if (!actor.IsAdministrator() || ss.IsFinished)
        {
            ...
            else
            {
                ...
                foreach (IPersonalName item in actor.Aliases)
                {
                    sb.AppendLine(
        ...
        else
        {
            ...
            ICharacter target = actor.TargetActor(targetText) ?? actor.Gameworld.Characters.GetByName(targetText);
            if (target == null)
            {
            ...
            else
            {
                ...
                foreach (IPersonalName item in target.Aliases)
                {
                    sb.AppendLine(
                        $"\t{item.GetName(NameStyle.FullName).Colour(Telnet.Cyan)}{(!item.GetName(NameStyle.SimpleFull).Equals(item.GetName(NameStyle.FullWithNickname), StringComparison.InvariantCultureIgnoreCase) ? ", a.k.a. " + item.GetName(NameStyle.FullWithNickname).Colour(Telnet.Cyan) : "")}");
                }
...
The syntax is #3INTRODUCE ME#0 or #3INTRODUCE <person>#0. You can append a bracketed emote to this command.",
    ...
    {
        StringStack ss = new(command.RemoveFirstWord());
        ICharacter target = actor.TargetActorOrCorpse(ss.PopSafe());
        if (target == null)
        {
        ...
        using (new FMDB())
        {
            ...
            FMDB.Context.SaveChanges();
            foreach ((ICharacter tch, Models.Dub dub) in addedDubs)
            {
    ...
    protected static void Social(ICharacter actor, string command)
    {
        ...
        string socialText = ss.PopSpeech();
        ISocial social = actor.CommandTree.Commands.Socials.FirstOrDefault(x => x.Applies(actor, socialText, false));
        if (social == null)
        {
    ...
    {
        bool overrideEthnicNameCulture = false;
        ...
        int limit = Character.Character.MaximumNumberOfAliases(actor);
        ...
        string nameText = command.SafeRemainingArgument.Trim();
        INameCulture nc =
        ...
        IPersonalName newName = nc.GetPersonalName(nameText, true);
        if (newName == null)
        {
            IRandomNameProfile exampleRN = actor.Gameworld.RandomNameProfiles.Where(x => x.Culture == nc && x.IsCompatibleGender(actor.Gender.Enum) && x.IsReady).GetRandomElement();
            StringBuilder sb = new();
        ...
        string newNameText = newName.GetName(NameStyle.FullName);
        if (
    ...
    private static void AliasRemove(ICharacter actor, StringStack command)
    {
        ...
        IPersonalName alias = actor.Aliases.FirstOrDefault(x => x.GetName(NameStyle.FullName).EqualTo(nameText));
        if (alias is null)
        {
    ...
    private static void AliasSelect(ICharacter actor, StringStack command)
    {
        ...
        string aliasText = command.SafeRemainingArgument;
        IPersonalName alias = actor.Aliases.GetName(aliasText);
        if (alias == null)
        {
