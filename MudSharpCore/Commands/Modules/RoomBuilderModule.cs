...
There is also a universal optional argument which must come first in the form of #3prog=someprog#0. This allows you to specify a prog that accepts either a single location or a collection of locations as an argument, and the template will execute that prog on the generated cells. You may specify this option multiple times but they must always be the first arguments in the list, before any template specific ones.";
    ...
    private static void CellLandmarkText(ICharacter actor, StringStack ss)
    {
        ...
        if (ss.IsFinished)
        {
            ...
            int i = 1;
            foreach ((IFutureProg Prog, string Text) text in existing.LandmarkDescriptionTexts)
            {
                sb.AppendLine();
        ...
        switch (ss.PopForSwitch())
        {
            ...
            case "new":
                ...
                IFutureProg prog = new ProgLookupFromBuilderInput(actor, ss.SafeRemainingArgument, ProgVariableTypes.Boolean, [ProgVariableTypes.Character]).LookupProg();
                if (prog is null)
                {
                    return;
                }
            ...
            case "text":
                ...
                if (!int.TryParse(ss.PopSpeech(), actor, out int value) || value < 1 || value > existing.LandmarkDescriptionTexts.Count)
                {
...
Enter your text below:");
            ...
            case "swap":
                ...
                if (!int.TryParse(ss.PopSpeech(), actor, out int value1) || value1 < 1 || value1 > existing.LandmarkDescriptionTexts.Count)
                {
                    actor.OutputHandler.Send($"The text {ss.Last.ColourCommand()} is not a valid number between {1.ToStringN0Colour(actor)} and {existing.LandmarkDescriptionTexts.Count.ToStringN0Colour(actor)}.");
                    return;
                }
                ...
                if (!int.TryParse(ss.PopSpeech(), actor, out int value2) || value2 < 1 || value2 > existing.LandmarkDescriptionTexts.Count)
                {
                    actor.OutputHandler.Send($"The text {ss.Last.ColourCommand()} is not a valid number between {1.ToStringN0Colour(actor)} and {existing.LandmarkDescriptionTexts.Count.ToStringN0Colour(actor)}.");
                    return;
                }
            ...
            case "rem":
                ...
                (IFutureProg Prog, string Text) old = existing.LandmarkDescriptionTexts.ElementAt(value - 1);
...
{Accept.StandardAcceptPhrasing}".SubstituteANSIColour());
            ...
            actor.AddEffect(new Accept(actor, new GenericProposal
            {
                AcceptAction = text =>
                {
                    (bool Truth, IEnumerable<string> Errors) result = location.ProposeDelete();
                    if (!result.Truth)
                    {
                        StringBuilder sb = new();
                        ...
                        sb.AppendLine();
                        int count = 1;
                        foreach (string issue in result.Errors)
                        {
    ...
    private static void CellDig(ICharacter actor, StringStack input)
    {
        ...
        if (!Constants.CardinalDirectionStringToDirection.TryGetValue(input.PopSpeech(), out CardinalDirection direction))
        {
    ...
    private static void CellNDig(ICharacter actor, StringStack input)
    {
        ...
        INonCardinalExitTemplate template = actor.Gameworld.NonCardinalExitTemplates.GetByIdOrName(match.Groups["template"].Value);
        if (template == null)
        {
    ...
    private static void CellNewTemplate(ICharacter actor, StringStack input)
    {
        ...
        while (!input.IsFinished && CellNewTemplateOptionsRegex.IsMatch(input.PeekSpeech()))
        {
            ...
            {
                case "prog":
                    IFutureProg prog = long.TryParse(match.Groups["value"].Value, out value)
        ...
        (bool success, string error, IEnumerable<object> args) = template.TryArguments(actor, input);
        if (!success)
        {
            actor.Send(error);
            return;
...
Possible filter options include:
        ...
        while (!ss.IsFinished)
        {
            ...
            switch (arg[0])
            {
                ...
                case '-':
                    ...
                    continue;
                default:
                    zone = long.TryParse(arg, out long value)
    ...
    {
        StringStack ss = new(input.RemoveFirstWord());
        IZone zone = long.TryParse(ss.SafeRemainingArgument, out long value)
            ? actor.Gameworld.Zones.Get(value)
...
See the #3CELL#0 command for more information about #3CELL PACKAGES#0.";
    ...
    protected static void ZoneEditName(ICharacter actor, IEditableZone zone, StringStack command)
    {
        ...
        string nameText = command.SafeRemainingArgument;
    ...
    protected static void ZoneEditElevation(ICharacter actor, IEditableZone zone, StringStack command)
    {
        ...
        string amountText = command.SafeRemainingArgument;
        double heightInBaseUnits = actor.Gameworld.UnitManager.GetBaseUnits(amountText, UnitType.Length, out bool success);
        if (!success)
        {
    ...
    protected static void ZoneEditLatitude(ICharacter actor, IEditableZone zone, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double latitude))
        {
    ...
    protected static void ZoneEditLongitude(ICharacter actor, IEditableZone zone, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double longitude))
        {
    ...
    protected static void ZoneEditAmbientLight(ICharacter actor, IEditableZone zone, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double level))
        {
            actor.OutputHandler.Send(
    ...
    protected static void ZoneEditTimezone(ICharacter actor, IEditableZone zone, StringStack command)
    {
        ...
        string clockText = command.PopSpeech();
        if (command.IsFinished)
        {
        ...
        string timezoneText = command.PopSpeech();

        IClock clock = long.TryParse(clockText, out long value)
        ...
        IMudTimeZone timeZone = clock.Timezones.GetByIdOrNames(timezoneText);
        if (timeZone == null)
        {
    ...
    protected static void ZoneEdit(ICharacter actor, StringStack command)
    {
        ...
        IZone izone = long.TryParse(command.PopSpeech(), out long value)
            ? actor.Gameworld.Zones.Get(value)
            : actor.Gameworld.Zones.FirstOrDefault(
        ...
        IEditableZone zone = izone.GetEditableZone;
        if (command.IsFinished)
        {
    ...
    private static void ZoneEditWeather(ICharacter actor, IEditableZone zone, StringStack command)
    {
        ...
        Climate.IWeatherController wc = actor.Gameworld.WeatherControllers.GetByIdOrName(command.SafeRemainingArgument);
        if (wc is null)
        {
    ...
    protected static void ZoneCreate(ICharacter actor, StringStack command)
    {
        ...
        string zoneName = command.PopSpeech().TitleCase();
        ...
        IShard shard = long.TryParse(command.PopSpeech(), out long value)
        ...
        Dictionary<IClock, IMudTimeZone> timezones = new();
        foreach (IClock clock in shard.Clocks)
        {
            if (command.IsFinished)
            {
    ...
    private static void ZoneShow(ICharacter actor, StringStack ss)
    {
        ...
        IZone zone = long.TryParse(ss.PopSpeech(), out long value)
            ? actor.Gameworld.Zones.Get(value)
...
Note: reverse any condition with a ! (e.g. !dawn, !snow, !*rain, !summer)");
        ...
        void GPTCallback(string text)
        {
            ...
            actor.AddEffect(new Accept(actor, new GenericProposal
            {
                ...
                AcceptAction = text =>
                {
                    if (!int.TryParse(text, out int value) || value < 1 || value > descriptions.Length)
                    {
        ...
        switch (actor.Gameworld.GetStaticConfiguration("DescSuggestionAIModel").ToLowerInvariant())
        {
            case "openai":
                ...
                break;
            case "anthropic":
                string anthropicModel = Futuremud.Games.First().GetStaticConfiguration("AnthropicDefaultModel");
    ...
    private static void CellSetAtmosphere(ICharacter actor, StringStack input)
    {
        ...
        IFluid fluid;
        string text = input.SafeRemainingArgument;
        if (isGas)
        {
    ...
    private static void CellSetRegister(ICharacter actor, StringStack command)
    {
        ...
        string variableName = command.PopSpeech();
    ...
    private static void CellSetRegisterDelete(ICharacter actor, StringStack command)
    {
        ...
        string whichVariable = command.PopSpeech().ToLowerInvariant();
    ...
    private static void CellSetDoor(ICharacter actor, StringStack input)
    {
        ...
        if (text.Equals("clear", StringComparison.InvariantCultureIgnoreCase))
        {
            ...
            else
            {
                ...
                foreach (RoomLayer block in newExit.BlockedLayers.ToList())
                {
        ...
        {
            IEnumerable<SizeCategory> sizes = Enum.GetValues(typeof(SizeCategory)).OfType<SizeCategory>();
    ...
    private static void CellEditLightMultiplier(ICharacter actor, StringStack input)
    {
        ...
        if (!double.TryParse(input.PopSpeech(), out double value))
        {
            actor.OutputHandler.Send(
    ...
    private static void CellEditTerrain(ICharacter actor, StringStack input)
    {
        ...
        string terrainText = input.SafeRemainingArgument;
        ITerrain terrain = actor.Gameworld.Terrains.GetByIdOrName(terrainText);
        if (terrain == null)
        {
    ...
    private static void CellEditHearingProfile(ICharacter actor, StringStack input)
    {
        ...
        string profileText = input.SafeRemainingArgument;
        Form.Audio.IHearingProfile profile = actor.Gameworld.HearingProfiles.GetByIdOrName(profileText);
        if (profile == null)
        {
...
{(from exit in exits
  select exit.BuilderInformationString(actor)
	...
	#3unblock <id|direction> <layer>#0 - removes a block on an exit from appearing in a specified layer".SubstituteANSIColour());
        ...
        IExit texit = actor.Gameworld.ExitManager.GetExitByID(value);
        if (texit == null)
        {
    ...
    private static void CellExitHide(ICharacter actor, StringStack input)
    {
        ...
        IFutureProg prog = actor.Gameworld.FutureProgs.GetByIdOrName(input.SafeRemainingArgument);
        if (prog == null)
        {
    ...
    private static void CellExitUnblock(ICharacter actor, StringStack input)
    {
        ...
        if (!input.PopSpeech().TryParseEnum<RoomLayer>(out RoomLayer layer))
        {
            actor.OutputHandler.Send(
    ...
    private static void CellExitClimb(ICharacter actor, StringStack input)
    {
        ...
        if (!CheckExtensions.GetDifficulty(input.SafeRemainingArgument, out Difficulty difficulty))
        {
    ...
    private static void CellEditNlink(ICharacter actor, StringStack input)
    {
        ...
        INonCardinalExitTemplate template = long.TryParse(match.Groups[1].Value, out long value)
...
#3cell package history <id>#0 - shows the building/review history of a particular cell package"
        ...
        while (!input.IsFinished)
        {
            ...
            switch (cmd)
            {
                ...
                case "by":
                    ...
                    using (new FMDB())
                    {
                        Models.Account dbaccount = FMDB.Context.Accounts.FirstOrDefault(x => x.Name == cmd);
                        if (dbaccount == null)
                        {
    ...
    private static void CellPackageReviewDefault(ICharacter actor, StringStack input)
    {
        ...
        {
            if (!int.TryParse(cmd, out int revnum))
            {
    ...
    private static void ShardCreate(ICharacter actor, StringStack input)
    {
        ...
        string nameText = input.PopSpeech();
        if (input.IsFinished)
        {
        ...
        ISkyDescriptionTemplate template = long.TryParse(input.PopSpeech(), out long value)
        ...
        Construction.Shard newShard = new(actor.Gameworld, template, nameText);
    ...
    private static void ShardEditClocks(ICharacter actor, StringStack input, IShard shard)
    {
        ...
        List<IClock> clocks = new();
        while (!input.IsFinished)
        {
            IClock clock = long.TryParse(input.PopSpeech(), out long value)
                ? actor.Gameworld.Clocks.Get(value)
                : actor.Gameworld.Clocks.FirstOrDefault(
                    x => x.Name.Equals(input.Last, StringComparison.InvariantCultureIgnoreCase)) ??
                actor.Gameworld.Clocks.FirstOrDefault(
    ...
    private static void ShardEditCalendars(ICharacter actor, StringStack input, IShard shard)
    {
        ...
        List<ICalendar> calendars = new();
        while (!input.IsFinished)
        {
            ICalendar calendar = long.TryParse(input.PopSpeech(), out long value)
    ...
    private static void ShardEditCelestials(ICharacter actor, StringStack input, IShard shard)
    {
        ...
        List<ICelestialObject> celestials = new();
        while (!input.IsFinished)
        {
            ICelestialObject celestial = long.TryParse(input.PopSpeech(), out long value)
    ...
    private static void ShardEdit(ICharacter actor, StringStack input)
    {
        ...
        IShard shard = long.TryParse(input.PopSpeech(), out long value)
            ? actor.Gameworld.Shards.Get(value)
            : actor.Gameworld.Shards.FirstOrDefault(
                x => x.Name.Equals(input.Last, StringComparison.InvariantCultureIgnoreCase));
        if (shard == null)
    ...
    private static void AreaList(ICharacter actor, string target)
    {
        ...
        {
            IZone zone = actor.Gameworld.Zones.FirstOrDefault(x =>
...
The syntax for working with areas is as follows:
    ...
    private static void AreaWeather(ICharacter actor, StringStack ss)
    {
        ...
        Climate.IWeatherController controller = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void AreaEdit(ICharacter actor, StringStack ss)
    {
        ...
        IArea area = long.TryParse(ss.PopSpeech(), out long value)
