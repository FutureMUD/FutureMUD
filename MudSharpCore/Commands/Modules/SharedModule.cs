...
This command is useful when you write-up a bunch of room creation commands in a text file to paste into the MUD at once, so you can refer to the rooms that you create rather than having to presuppose what the room ID will be.#0",
    ...
    protected static void Goto(ICharacter actor, string input)
    {
        ...
        ICell destination;
        if (long.TryParse(cmd, out long roomid))
        {
    ...
    protected static void Echo(ICharacter actor, string input)
    {
        ...
        Difficulty difficulty = Difficulty.Automatic;
        StringStack ss = new(input.RemoveFirstWord());
        ...
        Match match = _echoOnFailureRegex.Match(ss.RemainingArgument);
        if (match.Success)
        {
        ...
        PlayerEmote primaryEmote = new(primaryText, actor, false);
        ...
        PlayerEmote secondaryEmote = new(secondaryText, actor, false);
        ...
        EmoteOutput primaryOutput = new(primaryEmote);
        EmoteOutput secondaryOutput = new(secondaryEmote);

        actor.OutputHandler.Send("You send out an echo:");
        actor.OutputHandler.Send(primaryOutput);
        ICheck audioCheck = actor.Gameworld.GetCheck(CheckType.GenericListenCheck);
        ICheck visualCheck = actor.Gameworld.GetCheck(CheckType.GenericSpotCheck);
        foreach (ICharacter person in actor.Location.Characters.Except(actor))
        {
    ...
    protected static void ZEcho(ICharacter actor, string input)
    {
        ...
        string echo = input.RemoveFirstWord();
    ...
    protected static void PEcho(ICharacter actor, string input)
    {
        ...
        foreach (string starget in ss.PopSpeech().Split(','))
        {
        ...
        PlayerEmote emoteData = new(ss.RemainingArgument.SubstituteANSIColour(), actor.Body);
        if (emoteData.Valid)
        {
