...
public class ShowModule : Module<ICharacter>
{
    ...
    {
        List<IBloodtype> types = actor.Gameworld.Bloodtypes.ToList();
        if (!ss.IsFinished)
        {
            IBloodModel model = actor.Gameworld.BloodModels.GetByIdOrName(ss.SafeRemainingArgument);
            if (model is null)
            {
    ...
    {
        List<IPopulationBloodModel> pbms = actor.Gameworld.PopulationBloodModels.ToList();
        // TODO - filters
    ...
    {
        List<IBloodModel> models = actor.Gameworld.BloodModels.ToList();
        actor.OutputHandler.Send(StringUtilities.GetTextTable(
            from item in models
            select new List<string>
            {
    ...
    {
        List<IBloodtypeAntigen> antigens = actor.Gameworld.BloodtypeAntigens.ToList();
        actor.OutputHandler.Send(StringUtilities.GetTextTable(
    ...
    {
        List<IMoveSpeed> speeds = actor.Gameworld.MoveSpeeds
                          .OrderBy(x => x.Id)
                          .ToList();
        ...
        while (!ss.IsFinished)
        {
            ...
            switch (cmd[0])
            {
                ...
                case '*':
                    ...
                    continue;
                default:
                    IBodyPrototype body = actor.Gameworld.BodyPrototypes.GetByIdOrName(cmd);
                    if (body is null)
                    {
    ...
    private static void Show_CheckTemplates(ICharacter actor, StringStack ss)
    {
        using (new FMDB())
        {
            List<CheckTemplate> templates = FMDB.Context.CheckTemplates.Include(x => x.CheckTemplateDifficulties).ToList();
            actor.OutputHandler.Send(StringUtilities.GetTextTable(
    ...
    {
        List<ICheck> checks = actor.Gameworld.Checks.ToList();
        // TODO filters
    ...
    private static void Show_Characters(ICharacter actor, StringStack ss)
    {
        ...
        List<ICharacter> characters = actor.Gameworld.Characters.Concat(actor.Gameworld.CachedActors).Distinct()
            ...
            if (cmd.Length > 1)
            {
                ...
                switch (cmd[0])
                {
                    ...
                    case '*':
                        ...
                        continue;
                    case '>':
                        if (DateTime.TryParse(cmdSub, actor, out DateTime dt))
                        {
                    ...
                    case '<':
                        ...
                        continue;
                    case '$':
                        if (int.TryParse(cmdSub, out int minutes))
                        {
                            int result = minutes;
    ...
    private static void Show_Body(ICharacter actor, StringStack ss)
    {
        ...
        IBodyPrototype body = actor.Gameworld.BodyPrototypes.GetByIdOrName(ss.SafeRemainingArgument);
        if (body is null)
        {
    ...
    private static void Show_Skies(ICharacter actor, StringStack ss)
    {
        ...
        List<Construction.ISkyDescriptionTemplate> templates = actor.Gameworld.SkyDescriptionTemplates.ToList();
        StringBuilder sb = new();
    ...
    private static void Show_Sky(ICharacter actor, StringStack ss)
    {
        ...
        Construction.ISkyDescriptionTemplate sky = actor.Gameworld.SkyDescriptionTemplates.GetByIdOrName(ss.SafeRemainingArgument);
        if (sky is null)
        {
            actor.OutputHandler.Send(
        ...
        sb.AppendLine();
        foreach (Range<string> description in sky.SkyDescriptions.Ranges)
        {
            sb.AppendLine();
            sb.AppendLine(
    ...
    private static void Show_Accounts(ICharacter actor, StringStack ss)
    {
        ...
        List<IAccount> accounts = actor.Gameworld.Accounts.ToList();
        while (!ss.IsFinished)
        {
    ...
    public static void Show_Timezones(ICharacter actor, StringStack command)
    {
        ...
        {
            IClock clock = actor.Gameworld.Clocks.GetByIdOrNames(command.SafeRemainingArgument);
            if (clock is null)
            {
    ...
    private static void Show_Calendar(ICharacter actor, StringStack command)
    {
        ...
        ICalendar calendar = actor.Gameworld.Calendars.GetByIdOrNames(command.PopSpeech());
        if (calendar is null)
        {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int year))
        {
    ...
    private static void Show_Decorator(ICharacter actor, StringStack command)
    {
        ...
        ITraitValueDecorator decorator = actor.Gameworld.TraitDecorators.GetByIdOrName(command.SafeRemainingArgument);
    ...
    private static void Show_Climate(ICharacter actor, StringStack command)
    {
        ...
        Climate.IRegionalClimate climate = long.TryParse(command.PopSpeech(), out long value)
    ...
    private static void Show_WeatherController(ICharacter actor, StringStack command)
    {
        ...
        Climate.IWeatherController controller = long.TryParse(command.PopSpeech(), out long value)
    ...
    private static void Show_Race(ICharacter actor, StringStack ss)
    {
        ...
        IRace race = long.TryParse(ss.PopSpeech(), out long value)
        ...
        sb.AppendLine();
        foreach (INaturalAttack item in race.NaturalWeaponAttacks)
        {
            sb.AppendLine(
    ...
    private static void Show_AutoArea(ICharacter actor, StringStack ss)
    {
        ...
        IAutobuilderArea template = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void Show_AutoRoom(ICharacter actor, StringStack ss)
    {
        ...
        IAutobuilderRoom template = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void Show_AI(ICharacter actor, StringStack ss)
    {
        ...
        IArtificialIntelligence ai = actor.Gameworld.AIs.GetByIdOrName(ss.SafeRemainingArgument);
        if (ai is null)
        {
    ...
    private static void Show_Event(ICharacter actor, StringStack command)
    {
        ...
        EventInfoAttribute info = @event.GetAttribute<EventInfoAttribute>();
        StringBuilder sb = new();
        sb.AppendLine($"Event [{@event.DescribeEnum()}]".GetLineWithTitleInner(actor, Telnet.Cyan, Telnet.BoldWhite));
        sb.AppendLine();
        sb.AppendLine("Parameters:");
        sb.AppendLine();
        foreach ((string type, string name) parameter in info?.Parameters ?? Enumerable.Empty<(string, string)>())
        {
    ...
    {
        ITraitValueDecorator handwritingDecorator =
    ...
    private static void Show_Drug(ICharacter actor, StringStack ss)
    {
        ...
        IDrug drug = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void Show_Drugs(ICharacter actor, StringStack ss)
    {
        ...
        List<IDrug> drugs = actor.Gameworld.Drugs.ToList();
        // TODO - filtering
        actor.Send(StringUtilities.GetTextTable(
            drugs.Select(x => new[]
            {
    ...
    private static void Show_Weapon(ICharacter actor, StringStack ss)
    {
        ...
        IWeaponType weapon = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void Show_Weapons(ICharacter actor, StringStack ss)
    {
        ...
        List<IWeaponType> weapons = actor.Gameworld.WeaponTypes.ToList();
    ...
    private static void Show_RangedWeapon(ICharacter actor, StringStack ss)
    {
        ...
        IRangedWeaponType weapon = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void Show_RangedWeapons(ICharacter actor, StringStack ss)
    {
        ...
        List<IRangedWeaponType> weapons = actor.Gameworld.RangedWeaponTypes.ToList();
        // TODO - filtering criteria
        actor.Send(StringUtilities.GetTextTable(
            from weapon in weapons
            select new[]
    ...
    private static void Show_Armours(ICharacter actor, StringStack ss)
    {
        ...
        List<IArmourType> armours = actor.Gameworld.ArmourTypes.ToList();
        // TODO - filtering criteria
        actor.Send(StringUtilities.GetTextTable(
    ...
    private static void Show_Armour(ICharacter actor, StringStack ss)
    {
        ...
        IArmourType armour = long.TryParse(ss.PopSpeech(), out long value)
    ...
    private static void Show_Pattern(ICharacter actor, StringStack ss)
    {
        ...
        IEntityDescriptionPattern pattern = actor.Gameworld.EntityDescriptionPatterns.Get(value);
        if (pattern == null)
        {
        ...
        switch (ss.PopSpeech().ToLowerInvariant())
        {
            ...
            case "npc":
                ...
                CharacterCreation.ICharacterTemplate charTemplate = template.GetCharacterTemplate();
    ...
    private static void Show_Patterns(ICharacter actor, StringStack ss)
    {
        ...
        double height = 0.0;
        string whoFor = string.Empty;
        int age = 0;

        List<IEntityDescriptionPattern> patterns = actor.Gameworld.EntityDescriptionPatterns.ToList();
    ...
    private static void Show_Ethnicities(ICharacter actor, StringStack input)
    {
        ...
        IUneditableAll<IEthnicity> ethnicities = actor.Gameworld.Ethnicities;
        // todo - filters
    ...
    private static void Show_Ethnicity(ICharacter actor, StringStack input)
    {
        ...
        IEthnicity ethnicity = actor.Gameworld.Ethnicities.GetByIdOrName(input.SafeRemainingArgument);
    ...
    private static void Show_Cultures(ICharacter actor, StringStack input)
    {
        ...
        IUneditableAll<ICulture> cultures = actor.Gameworld.Cultures;
        // todo - filters
        actor.Send(StringUtilities.GetTextTable(
    ...
    private static void Show_Culture(ICharacter actor, StringStack input)
    {
        ...
        ICulture culture = long.TryParse(input.SafeRemainingArgument, out long value)
    ...
    private static void Show_Account(ICharacter actor, StringStack input)
    {
        ...
        using (new FMDB())
        {
            Models.Account dbaccount = long.TryParse(input.PopSpeech(), out long value)
    ...
    internal static string BuildAccountInfo(IAccount viewer, MudSharp.Models.Account dbaccount, IAccount account)
    {
        ...
        int totalminutes = dbaccount.Characters.Sum(x => x.TotalMinutesPlayed);
        ...
        if (account.AccountResources.Any(x => x.Value > 0))
        {
            ...
            sb.AppendLine();
            foreach (KeyValuePair<IChargenResource, double> item in account.AccountResources.Where(x => x.Value > 0))
            {
        ...
        List<Models.Character> characters = FMDB.Context.Characters.Where(x => x.AccountId == dbaccount.Id).ToList();
        List<Chargen> chargens = FMDB.Context.Chargens.Where(x => x.AccountId == dbaccount.Id).ToList();
        if (characters.Any() || chargens.Any())
        {
            sb.AppendLine();
            sb.AppendLine("Characters:");
            sb.AppendLine();
            foreach (Models.Character item in characters.Where(x => x.Status == 2))
            {
            ...
            foreach (Chargen item in chargens.Where(x => x.Status < 2))
            {
                XElement definition = XElement.Parse(item.Definition);
                Gender gender = (Gender)short.Parse(definition.Element("SelectedGender").Value);
                XElement nameElement = definition.Element("SelectedName");
                ...
                else
                {
                    ...
                    PersonalName name = new(nameCulture, nameElement.Element("Name"));
                    sb.AppendLine(
    ...
    private static void Show_Materials(ICharacter actor, StringStack input)
    {
        ...
        if (!input.IsFinished)
        {
            if (!input.PopSpeech().TryParseEnum<MaterialBehaviourType>(out MaterialBehaviourType materialType))
            {
    ...
    public static void Show_Terrain(ICharacter actor, StringStack input)
    {
        ...
        List<Construction.ITerrain> terrains = actor.Gameworld.Terrains.ToList();
    ...
    {
        IFutureProg prog;
        if (long.TryParse(input.PopSpeech(), out long id))
        {
            prog = actor.Gameworld.FutureProgs.Get(id);
        }
        else
        {
            List<IFutureProg> progs = actor.Gameworld.FutureProgs.Get(input.Last);
            if (progs.Count > 1)
            {
                actor.OutputHandler.Send(
        ...
        bool raw = input.Peek().EqualTo("raw");
    ...
    private static void Show_CharacteristicValues(ICharacter actor, StringStack input)
    {
        ...
        IEnumerable<ICharacteristicValue> values = actor.Gameworld.CharacteristicValues.Where(definition.IsValue);
        actor.OutputHandler.Send(
            StringUtilities.GetTextTable(
    ...
    public static void Show_Skills(ICharacter actor, StringStack input)
    {
        ...
        IEnumerable<ITraitDefinition> skills = actor.Gameworld.Traits.Where(x => x.TraitType == TraitType.Skill);
    ...
    private static void Show_Attributes(ICharacter actor, StringStack input)
    {
        ...
        string cmd = input.PopSpeech().ToLowerInvariant();
        IEnumerable<ITraitDefinition> attributes = actor.Gameworld.Traits.Where(x => x.TraitType == TraitType.Attribute);
        if (!string.IsNullOrEmpty(cmd))
        {
    ...
    private static void Show_Languages(ICharacter actor, StringStack input)
    {
        ...
        List<ILanguage> languages = actor.Gameworld.Languages.ToList();
        actor.OutputHandler.Send(
            StringUtilities.GetTextTable(
    ...
    private static void Show_Accents(ICharacter actor, StringStack input)
    {
        ...
        string cmd = input.PopSpeech().ToLowerInvariant();
        List<IAccent> accents = actor.Gameworld.Accents.ToList();
                ...
                case '+':
                    accents = accents.Where(x =>
                        x.Group.ToLowerInvariant()
                         .StartsWith(cmd[1..], StringComparison.InvariantCultureIgnoreCase)).ToList();
                    break;
                case '-':
                    accents = accents.Where(x =>
                        !x.Group.ToLowerInvariant()
    ...
    private static void Show_Scripts(ICharacter actor, StringStack input)
    {
        ...
        string cmd = input.PopSpeech().ToLowerInvariant();
        List<IScript> scripts = actor.Gameworld.Scripts.ToList();
        while (!string.IsNullOrEmpty(cmd))
        {
    ...
    {
        List<Form.Colour.IColour> colours = actor.Gameworld.Colours.ToList();
        //while (!input.IsFinished) {
    ...
    private static void Show_WearProfile(ICharacter actor, StringStack input)
    {
        ...
        IWearProfile profile = actor.Gameworld.WearProfiles.GetByIdOrName(input.SafeRemainingArgument);
        if (profile == null)
        {
    ...
    private static void Show_StackDecorators(ICharacter actor, StringStack input)
    {
        ...
        List<IStackDecorator> decorators = actor.Gameworld.StackDecorators.ToList();
    ...
    private static void Show_Knowledges(ICharacter actor, StringStack input)
    {
        ...
        List<IKnowledge> knowledges = actor.Gameworld.Knowledges.ToList();
        if (!input.IsFinished)
        {
            string category = input.PopSpeech();
            knowledges =
            ...
            if (!input.IsFinished)
            {
                string subcategory = input.PopSpeech();
                knowledges =
                    knowledges.Where(
                                  x =>
    ...
    private static void Show_Tags(ICharacter actor, StringStack command)
    {
        ...
        List<ITag> roots = actor.Gameworld.Tags.Where(x => x.Parent == null).ToList();
        actor.Send(StringUtilities.GetTextTable(
            from root in roots
    ...
    private static void Show_Ammo(ICharacter actor, StringStack command)
    {
        ...
        IAmmunitionType ammo = actor.Gameworld.AmmunitionTypes.GetByIdOrName(command.SafeRemainingArgument);
        if (ammo is null)
        {
            actor.OutputHandler.Send(
    ...
    private static void Show_Ammos(ICharacter actor, StringStack command)
    {
        ...
        IEnumerable<IAmmunitionType> ammos = actor.Gameworld.AmmunitionTypes.AsEnumerable();
        while (!command.IsFinished)
        {
            string filter = command.PopSpeech();
            if (filter.TryParseEnum<RangedWeaponType>(out RangedWeaponType rwt))
            {
    ...
    private static void Show_Merits(ICharacter actor, StringStack command)
    {
        ...
        List<ICharacterMerit> merits = actor.Gameworld.Merits.OfType<ICharacterMerit>().ToList();
        // TODO - filters
        actor.OutputHandler.Send(StringUtilities.GetTextTable(
