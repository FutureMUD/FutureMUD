...
As an admin, you can also do #3weather transition <event>#0 to force a transition to a particular weather event at the next tick.";
    ...
    private static void WeatherTransition(ICharacter actor, StringStack ss)
    {
        ...
        if (ss.IsFinished)
        {
            List<IWeatherEvent> newWeather = actor.Location.WeatherController.RegionalClimate.ClimateModel.PermittedTransitions(
            ...
            sb.AppendLine();
            foreach (IWeatherEvent item in newWeather)
            {
                sb.AppendLine(
        ...
        IEnumerable<IWeatherEvent> choices = actor.Location.WeatherController.RegionalClimate.ClimateModel.PermittedTransitions(
            actor.Location.WeatherController.CurrentWeatherEvent, actor.Location.WeatherController.CurrentSeason,
            actor.Location.CurrentTimeOfDay);
        IWeatherEvent choice = long.TryParse(ss.PopSpeech(), out long value)
    ...
    {
        IWeatherEvent weather = actor.Location.CurrentWeather(actor);
        double temperature = actor.Location.CurrentTemperature(actor);
        ISeason season = actor.Location.CurrentSeason(actor);
        (double floor, double ceiling) = actor.Body.TolerableTemperatures(true);
        (double floorNatural, double ceilingNatural) = actor.Body.TolerableTemperatures(false);
        StringBuilder sb = new();
