...
public partial class Clan : SaveableItem, IClan
{
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Clan clan = FMDB.Context.Clans.Find(Id);
            clan.Name = Name;
    ...
    public void RemoveMembership(IClanMembership membership)
    {
        ...
        List<IAppointment> oldAppointments = membership.Appointments.ToList();
        membership.Appointments.Clear();
        foreach (IAppointment appointment in oldAppointments)
        {
    ...
    {
        string stringValue = gameworld.GetStaticConfiguration("PlayersCanCreateClansProg");
    ...
    {
        foreach (IClanMembership member in Memberships)
        {
    ...
    {
        get => _clanBankAccount ??= Gameworld.BankAccounts.Get(_clanBankAccountId ?? 0L);
        set
        {
    ...
    public ICalendar Calendar
    {
        ...
        set
        {
            if (_calendar is not null && _calendar != value && value is not null)
            {
                ...
                foreach (IElection election in Appointments.SelectMany(x => x.Elections))
                {
    ...
    public virtual void FinaliseLoad(MudSharp.Models.Clan clan, IEnumerable<Models.ClanMembership> memberships)
    {
...
#endif
        foreach (Models.Paygrade item in clan.Paygrades)
        {
...
#endif
        int rankNumber = 0;
        foreach (Models.Rank item in clan.Ranks.OrderBy(x => x.RankNumber))
        {
...
#endif
        Dictionary<IAppointment, Models.Appointment> staging = new();
        foreach (Models.Appointment item in clan.Appointments)
        {
            Appointment newAppointment = new(item, this);
...
#endif
        foreach (KeyValuePair<IAppointment, Models.Appointment> item in staging)
        {
        ...
        if (!IsTemplate)
        {
            foreach (IAppointment item in Appointments)
            {
                IEnumerable<Models.Election> elections = FMDB.Context.Elections.Where(x => x.AppointmentId == item.Id)
    ...
    public void Disband(ICharacter disbander)
    {
        ...
        using (new FMDB())
        {
            ...
            Models.Clan dbclan = FMDB.Context.Clans.Find(Id);
            if (dbclan == null)
            {
        ...
        EmoteOutput emote = new(new Emote($"@ have|has disbanded the clan \"{FullName}\", effective immediately.",
            disbander));
        foreach (IClanMembership member in Memberships)
        {
    ...
    {
        List<IRank> ordered = Ranks.OrderBy(x => x.RankNumber).ToList();
        IRank newRank =
    ...
    public void DeleteAppointment(IAppointment appointment)
    {
        ...
        foreach (IExternalClanControl item in ExternalControls.ToList())
        {
