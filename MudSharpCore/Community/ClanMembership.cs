...
public class ClanMembership : SaveableItem, IClanMembership, ILazyLoadDuringIdleTime
{
        ...
        : this()
    {
        ...
        foreach (ClanMembershipsAppointments item in membership.ClanMembershipsAppointments)
        {
            Appointments.Add(Clan.Appointments.First(x => item.AppointmentId == x.Id));
        }

        foreach (ClanMembershipBackpay item in membership.ClanMembershipsBackpay)
        {
    ...
    public ClanPrivilegeType NetPrivileges => Rank.Privileges |
                   (Appointments.Any()
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.ClanMembership dbitem = FMDB.Context.ClanMemberships.Find(Clan.Id, MemberId);
            if (dbitem == null)
            {
                Changed = false;
                return;
            ...
            foreach (KeyValuePair<ICurrency, decimal> item in BackPayDiciontary)
            {
                ClanMembershipBackpay dbbackpay = new();
    ...
    private decimal MaximumBackpayForCurrency(ICurrency currency)
    {
        ...
        foreach (IAppointment position in Appointments)
        {
