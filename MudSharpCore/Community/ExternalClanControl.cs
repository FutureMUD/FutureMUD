...
public class ExternalClanControl : SaveableItem, IExternalClanControl
{
    public ExternalClanControl(MudSharp.Models.ExternalClanControl control, IFuturemud gameworld)
    {
        ...
        foreach (ExternalClanControlsAppointment appointment in control.ExternalClanControlsAppointments)
        {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.ExternalClanControl dbitem = FMDB.Context.ExternalClanControls.Find(VassalClan.Id, LiegeClan.Id,
            ...
            foreach (IClanMembership item in Appointees)
            {
                ExternalClanControlsAppointment appointee = new()
                {
    ...
    public void Delete()
    {
        ...
        using (new FMDB())
        {
            ...
            Models.ExternalClanControl dbitem = FMDB.Context.ExternalClanControls.Find(VassalClan.Id, LiegeClan.Id, ControlledAppointment.Id);
            if (dbitem != null)
            {
