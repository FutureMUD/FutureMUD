...
public class Rank : SaveableItem, IRank
{
        ...
        : this()
    {
        ...
        foreach (RanksPaygrade item in rank.RanksPaygrades.OrderBy(x => x.Order))
        {
            Paygrades.Add(clan.Paygrades.First(x => x.Id == item.PaygradeId));
        }

        foreach (RanksTitle item in rank.RanksTitles.OrderBy(x => x.Order))
        {
            TitlesAndProgs.Add(Tuple.Create(Gameworld.FutureProgs.Get(item.FutureProgId ?? 0), item.Title));
        }

        foreach (RanksAbbreviations item in rank.RanksAbbreviations.OrderBy(x => x.Order))
        {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Rank dbitem = FMDB.Context.Ranks.Find(Id);
            dbitem.Name = Name;
            ...
            int index = 0;
            foreach (IPaygrade paygrade in Paygrades)
            {
                RanksPaygrade pg = new()
                {
                    Order = index++,
            ...
            {
                RanksAbbreviations abbreviation = new()
                {
    ...
    IEnumerable<string> IRank.Abbreviations => AbbreviationsAndProgs.Select(x => x.Item2);
    ...
    IEnumerable<string> IRank.Titles => TitlesAndProgs.Select(x => x.Item2);
