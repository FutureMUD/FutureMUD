...
public class AdjacentFeature : Feature
{
    ...
    public AdjacentFeature(XElement root, IFuturemud gameworld) : base(root, gameworld)
    {
        ...
        foreach (XElement element in root.Element("Adjacents")?.Elements() ?? Enumerable.Empty<XElement>())
        {
            CardinalDirection direction = (CardinalDirection)int.Parse(element.Attribute("direction")?.Value ??
    ...
    {
        XElement @base = base.SaveToXml();
        @base.Add(new XElement("IgnoreExits", IgnoreExits));
        @base.Add(new XElement("AdjacentFeatureTag", new XCData(AdjacentFeatureTag)));
        XElement adjacent = new("Adjacents");
        foreach (KeyValuePair<CardinalDirection, string> item in DirectionAdjacencyTags)
        {
    ...
    private bool BuildingCommandAdjacentTag(ICharacter actor, StringStack command)
    {
        ...
        foreach (CardinalDirection direction in Constants.CardinalDirections.Except(CardinalDirection.Unknown))
        {
