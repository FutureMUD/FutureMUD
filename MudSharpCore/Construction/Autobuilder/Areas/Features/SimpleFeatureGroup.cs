...
public class SimpleFeatureGroup : TerrainFeatureGroup
{
    ...
    public SimpleFeatureGroup(XElement root, IFuturemud gameworld)
    {
        ...
        XElement element = root.Element("Features");
        if (element == null)
        {
    ...
    public override void ApplyTerrainFeatures(ICell[,] cellMap, List<string>[,] featureMap)
    {
        ...
        while (howMany > 0)
        {
            ...
            Feature feature = Features.GetWeightedRandom(x => x.Weighting);
            if (feature.MaximumCount > 0)
            {
                int count = counter.Count(feature.Name);
    ...
    public override string Show(ICharacter builder)
    {
        ...
        int i = 1;
        double total = Features.Sum(x => x.Weighting);
        foreach (Feature feature in Features)
        {
            sb.AppendLine(
        ...
        StringStack command)
    {
        ...
        if (!int.TryParse(command.Last, out int index) || index < 1 || index > Features.Count)
        {
        ...
        StringStack command)
    {
        ...
        Feature which = Features[index - 1];
        actor.OutputHandler.Send(
        ...
        StringStack command)
    {
        ...
        string tag = command.SafeRemainingArgument.TitleCase();
        ITag framework = actor.Gameworld.Tags.FirstOrDefault(x => x.Name.CollapseString().EqualTo(tag));
        Feature item = null;
        switch (type)
        {
            case "simple":
    ...
    private bool BuildingCommandDensity(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double min) || min < 0.0)
        {
