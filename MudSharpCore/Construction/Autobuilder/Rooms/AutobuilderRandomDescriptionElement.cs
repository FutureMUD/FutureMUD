...
public class AutobuilderRandomDescriptionElement : IAutobuilderRandomDescriptionElement
{
    ...
    public AutobuilderRandomDescriptionElement(XElement root, IFuturemud gameworld)
    {
        ...
        foreach (string tag in root.Element("Tags").Value.Split(","))
        {
    ...
    {
        List<ITerrain> terrains = new();
        while (!command.IsFinished)
        {
            ITerrain terrain = actor.Gameworld.Terrains.GetByIdOrName(command.PopSpeech());
            if (terrain == null)
            {
        ...
        foreach (ITerrain terrain in terrains)
        {
    ...
    protected virtual bool BuildingCommandName(ICharacter actor, StringStack command)
    {
        ...
        string name = command.SafeRemainingArgument.TitleCase().SanitiseExceptNumbered(1);
        RoomNameText = name;
        actor.OutputHandler.Send(
    ...
    private bool BuildingCommandWeight(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.SafeRemainingArgument, out double value) || value <= 0)
        {
