...
public class AutobuilderRandomDescriptionGroup : IAutobuilderRandomDescriptionElement
{
    ...
    {
        List<IAutobuilderRandomDescriptionElement> valid = _subElements.Where(x => x.Applies(terrain, tags)).ToList();
        List<IAutobuilderRandomDescriptionElement> mandatory = valid.Where(x => x.MandatoryIfValid).ToList();
    ...
    private bool BuildingCommandElement(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(text, out int value) || value < 1 || value > _subElements.Count)
        {
            actor.OutputHandler.Send(
    ...
    public virtual string Show(ICharacter actor)
    {
        ...
        int i = 1;
        foreach (IAutobuilderRandomDescriptionElement element in _subElements)
        {
