...
public class AutobuilderRoadRandomDescriptionElement : AutobuilderRandomDescriptionElement
{
    ...
    {
        bool @base = base.BuildingCommandTag(actor, command);
        if (Tags.Any())
        {
            TagRegex = new Regex($"{Tags.First()}=(?<directions>.+)");
        }
    ...
    {
        bool @base = base.BuildingCommandName(actor, command);
        if (!@base)
        {
    ...
    {
        bool @base = base.BuildingCommandText(actor, command);
        if (!@base)
        {
            actor.OutputHandler.Send(@"You can also use the following road-tag related options:
    ...
    {
        string matchingTag = tags.FirstOrDefault(x => TagRegex.IsMatch(x));
        if (matchingTag == null)
        {
            return (RoomNameText, Text);
        }

        List<CardinalDirection> directions = TagRegex.Match(matchingTag).Groups["directions"].Value.Split(',')
