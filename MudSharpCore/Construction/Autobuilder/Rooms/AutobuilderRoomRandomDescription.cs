...
public class AutobuilderRoomRandomDescription : AutobuilderRoomBase
{
        ...
        name)
    {
        ...
        foreach (KeyValuePair<ITerrain, (AutobuilderRoomInfo, Expression)> info in rhs.TerrainInfos)
        {
        ...
        using (new FMDB())
        {
            AutobuilderRoomTemplate dbitem = new()
            {
                Name = _name,
    ...
    protected override XElement SaveToXml()
    {
        ...
        foreach (KeyValuePair<ITerrain, (AutobuilderRoomInfo, Expression)> terrain in TerrainInfos)
        {
            XElement subElement = terrain.Value.Item1.SaveToXml();
...
Commands pertaining to random description elements that get added to the base descriptions:
    ...
    public override string Show(ICharacter builder)
    {
        ...
        sb.AppendLine();
        int i = 1;
        foreach (IAutobuilderRandomDescriptionElement item in RandomDescriptionElements)
        {
    ...
    private bool BuildingCommandElement(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(text, out int value) || value < 1 || value > RandomDescriptionElements.Count)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandForagable(ICharacter actor, StringStack command)
    {
        ...
        IForagableProfile fp = Gameworld.ForagableProfiles.GetByIdOrName(command.SafeRemainingArgument);
        if (fp == null)
        {
    ...
    private bool BuildingCommandTerrain(ICharacter actor, StringStack command)
    {
        ...
        ITerrain terrain = Gameworld.Terrains.GetByIdOrName(command.PopSpeech());
        if (terrain == null)
        {
        ...
        if (command.IsFinished)
        {
            ...
            {
                (AutobuilderRoomInfo, Expression) info = TerrainInfos[terrain];
                StringBuilder sb = new();
    ...
    private bool BuildingCommandDefaultTerrain(ICharacter actor, StringStack command)
    {
        ...
        ITerrain terrain = Gameworld.Terrains.GetByIdOrName(command.SafeRemainingArgument);
        if (terrain == null)
        {
    ...
    private bool BuildingCommandLight(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParsePercentage(out double percentage))
        {
    ...
    {
        ITerrain terrain = (ITerrain)arg2[0];
    ...
    {
        ITerrain terrain = (ITerrain)arg3[0];
        if (TerrainInfos.ContainsKey(terrain))
        {
            TerrainInfos[terrain] = (
