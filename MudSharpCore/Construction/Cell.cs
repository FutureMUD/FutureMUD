...
public partial class Cell : Location, IDisposable, ICell
{
    ...
    public Cell(ICellOverlayPackage package, IRoom room, bool temporary = false) : base(room.Gameworld)
    {
        ...
        using (new FMDB())
        {
            ...
            _id = dbCell.Id;
            CellOverlay newOverlay = new(this, package);
            dbCell.CurrentOverlayId = newOverlay.Id;
            FMDB.Context.SaveChanges();
            SetupCell(dbCell);

            List<IHook> hooks = Gameworld.DefaultHooks.Where(x => x.Applies(this, "Cell")).Select(x => x.Hook).ToList();
            if (hooks.Any())
            {
        ...
        room.Gameworld)
    {
        ...
        using (new FMDB())
        {
            ...
            FMDB.Context.SaveChanges();
            _id = dbCell.Id;
            CellOverlay newOverlay = new(this, package, templateCell);
            dbCell.CurrentOverlayId = newOverlay.Id;
            FMDB.Context.SaveChanges();
            SetupCell(dbCell);
    ...
    {
        IEditableCellOverlay overlay = _overlays.FirstOrDefault(x => x.Package == package);
        if (overlay == null)
        {
    ...
    {
        IEditableCellOverlay overlay = _overlays.Find(x => x.Id == id);
        if (overlay is null)
        {
            return;
        }
    ...
    public void Destroy(ICell fallbackCell)
    {
        ...
        using (new FMDB())
        {
            ...
            {
                Gameworld.SaveManager.Flush();
                Models.Cell dbitem = FMDB.Context.Cells.Find(Id);
                if (dbitem != null)
                {
    ...
    public void HandleAudioEcho(string audioText, AudioVolume volume, IPerceiver source, RoomLayer originalLayer, bool ignoreOriginLayer = true)
    {
        ...
        foreach (ICell location in vicinity)
        {
            ...
            List<ICellExit> directions = location.ExitsBetween(this, 10).ToList();
    ...
    {
        foreach (CellsTags tag in cell.CellsTags)
        {
