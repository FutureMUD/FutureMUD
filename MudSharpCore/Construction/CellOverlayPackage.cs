...
public class CellOverlayPackage : Framework.Revision.EditableItem, ICellOverlayPackage
{
        ...
        : base(originator)
    {
        ...
        using (new FMDB())
        {
            try
            {
                Models.CellOverlayPackage dbpack = new();
    ...
    public override string Show(ICharacter actor)
    {
        using (new FMDB())
        {
            ...
            sb.AppendLine();
            List<ICellOverlay> overlays = Gameworld.Cells.SelectMany(x => x.Overlays).Where(x => x.Package == this).ToList();
            sb.AppendLine(
            ...
            foreach (ICellOverlay overlay in overlays)
            {
                sb.AppendLine(
    ...
    public override IEditableRevisableItem CreateNewRevision(ICharacter initiator)
    {
        using (new FMDB())
        {
            Models.CellOverlayPackage dbnew = new()
            {
                Id = Id,
                RevisionNumber =
            ...
            Models.CellOverlayPackage dbold = FMDB.Context.CellOverlayPackages
            ...
            foreach (Models.CellOverlay overlay in dbold.CellOverlays)
            {
                Models.CellOverlay newOverlay = new()
                {
                ...
                foreach (CellOverlayExit exit in overlay.CellOverlaysExits)
                {
            ...
            FMDB.Context.SaveChanges();
            CellOverlayPackage package = new(dbnew, Gameworld);
            Gameworld.Add(package);
            foreach (Models.CellOverlay overlay in dbnew.CellOverlays)
            {
                ICell cell = Gameworld.Cells.Get(overlay.CellId);
                if (cell is null)
                {
                    continue;
                }
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.CellOverlayPackage dbpack = FMDB.Context.CellOverlayPackages.Find(Id, RevisionNumber);
            dbpack.Name = Name;
