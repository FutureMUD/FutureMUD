...
public static partial class DirectionExtensions
{
    ...
    public static string DescribeDirection(this IEnumerable<ICellExit> exits)
    {
        ...
        (int Northness, int Southness, int Westness, int Eastness, int Upness, int Downness) counts = exits.CountDirections();
        return
    ...
    public static string DescribeDirectionsToFrom(this IEnumerable<ICellExit> exits)
    {
        ...
        int distance = exits.Select(x => x.OutboundDirection).DistanceAsCrowFlies();

        string distanceDescription = "extremely far";
        switch (distance)
        {
            case 1:
