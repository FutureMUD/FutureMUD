...
public class Room : Location, IDisposable, IRoom
{
    ...
    {
        Action action = DestroyRoomWithDatabaseAction(fallbackCell);
    ...
    {
        List<Action> cellActions = new();
        foreach (ICell cell in _cells.ToList())
        {
        ...
        return () =>
        {
            ...
            {
                Gameworld.SaveManager.Flush();
                foreach (Action action in cellActions)
                {
                    action?.Invoke();
                }
