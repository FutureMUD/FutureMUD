...
public class Shard : Location, IEditableShard
{
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Shard dbitem = FMDB.Context.Shards.Find(Id);
            dbitem.Name = Name;
            ...
            foreach (ICalendar item in Calendars)
            {
                ShardsCalendars newItem = new();
            ...
            foreach (IClock item in Clocks)
            {
                ShardsClocks newItem = new();
            ...
            foreach (ICelestialObject item in Celestials)
            {
                ShardsCelestials newItem = new();
    ...
    private void LoadFromDB(MudSharp.Models.Shard plane)
    {
        ...
        foreach (ShardsClocks clock in plane.ShardsClocks)
        {
            _clocks.Add(Gameworld.Clocks.Get(clock.ClockId));
        }

        foreach (ShardsCalendars calendar in plane.ShardsCalendars)
        {
            _calendars.Add(Gameworld.Calendars.Get(calendar.CalendarId));
        }

        foreach (ShardsCelestials celestial in plane.ShardsCelestials)
        {
