...
public partial class Terrain : SaveableItem, ITerrain
{
    ...
    protected void LoadFromDB(Models.Terrain terrain)
    {
		...
		EnvironmentalMagicProfileId = terrain.EnvironmentalMagicProfileId;
        ...
        foreach (TerrainsRangedCovers cover in terrain.TerrainsRangedCovers)
        {
        ...
        StringStack ss = new(terrain.TerrainBehaviourMode);
    ...
    {
        ILiquid fluid = long.TryParse(ss.PopSpeech(), out long value)
    ...
    public Terrain(ITerrain rhs, string name)
    {
        ...
        using (new FMDB())
        {
            ...
            Models.Terrain rhsItem = FMDB.Context.Terrains.Find(rhs.Id);
    ...
    {
        Models.Terrain dbitem = FMDB.Context.Terrains.Find(Id);
        dbitem.Name = Name;
    ...
    public string Show(ICharacter actor)
    {
		...
		sb.AppendLine($"Default Environmental Profile: {DescribeEnvironmentalProfile(actor)}");
        sb.AppendLine(
    ...
    public bool BuildingCommand(ICharacter actor, StringStack command)
    {
        switch (command.PopSpeech().ToLowerInvariant())
        {
			...
			case "environment":
				return BuildingCommandEnvironment(actor, command);
            case "agriculture":
            case "agricultural":
            case "farm":
            case "field":
	...
	#3environment <profile|none>#0 - sets or removes the default environmental magic profile
    ...
    {
        string model = command.PopForSwitch();
        ILiquid liquid = null;
        switch (model)
        {
            case "outdoors":
...
The following additional models require you to specify a liquid to go with them:
    ...
    {
        string text = command.SafeRemainingArgument;
        if (!int.TryParse(text, out int value) || value < 0 || value > 255)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandInfection(ICharacter actor, StringStack command)
    {
        ...
        if (!Utilities.TryParseEnum<InfectionType>(command.PopSpeech(), out InfectionType theEnum))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandDefault(ICharacter actor, StringStack command)
    {
        ...
        DefaultTerrain = true;
        Changed = true;
        foreach (ITerrain terrain in Gameworld.Terrains.Except(this))
        {
    ...
    private bool BuildingCommandCover(ICharacter actor, StringStack command)
    {
        ...
        IRangedCover cover = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandWeather(ICharacter actor, StringStack command)
    {
        ...
        IWeatherController controller = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandForage(ICharacter actor, StringStack command)
    {
        ...
        {
            var changed = _foragableProfile is not null || _foragableProfileId != 0;
        ...
        if ((_foragableProfileId != 0 ? _foragableProfileId : _foragableProfile?.Id ?? 0) != profile.Id ||
            _foragableProfile is not null && !ReferenceEquals(_foragableProfile, profile))
        {
            _foragableProfile = profile;
    ...
    private bool BuildingCommandAtmosphere(ICharacter actor, StringStack command)
    {
        ...
        switch (command.PopSpeech().ToLowerInvariant())
        {
            ...
            case "gas":
                ...
                IGas gas = long.TryParse(command.PopSpeech(), out long value)
            ...
            case "liquid":
                ...
                ILiquid liquid = long.TryParse(command.PopSpeech(), out value)
                    ? Gameworld.Liquids.Get(value)
    ...
    private bool BuildingCommandName(ICharacter actor, StringStack command)
    {
        ...
        string name = command.PopSpeech().Trim().TitleCase();
