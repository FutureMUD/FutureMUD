...
public class Zone : Location, IEditableZone
{
    ...
    public Zone(MudSharp.Models.Zone zone, IFuturemud game) : base(game)
    {
        ...
        foreach (ZonesTimezones timezone in zone.ZonesTimezones)
        {
        ...
        foreach (IClock clock in Clocks.Where(x => !TimeZones.ContainsKey(x)))
        {
    ...
    public TimeOfDay CurrentTimeOfDay
    {
        get
        {
            CelestialInformation sunPosition = Celestials.FirstOrDefault(x => x.CelestialAngleIsUsedToDetermineTimeOfDay)
    ...
    {
        return TimeZones.TryGetValue(whichClock ?? Clocks.FirstOrDefault(), out IMudTimeZone value)
            ? value
