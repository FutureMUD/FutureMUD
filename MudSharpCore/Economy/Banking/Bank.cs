...
#nullable enable
    public static (IBankAccount? Account, string Error) FindBankAccount(string accTarget, IBank? homeBank, ICharacter? character)
    {
        ...
        {
            string[] split = accTarget.Split(':');
            IBank? bankTarget = Futuremud.Games.First().Banks.GetByName(split[0]) ??
        ...
        if (homeBank != null)
        {
            _ = int.TryParse(accTarget, out int accn);

            IBankAccount? accountTarget = homeBank.BankAccounts.FirstOrDefault(x =>
...
#nullable restore
    ...
    public Bank(Models.Bank bank, IFuturemud gameworld)
    {
        ...
        foreach (BankCurrencyReserve item in bank.BankCurrencyReserves)
        {
            CurrencyReserves[Gameworld.Currencies.Get(item.CurrencyId)] += item.Amount;
        }

        foreach (BankExchangeRate rate in bank.BankExchangeRates)
        {
            ExchangeRates
        ...
        foreach (Models.BankAccountType type in bank.BankAccountTypes)
        {
            _bankAccountTypes.Add(new BankAccountType(type, this));
        }

        foreach (Models.BankAccount account in bank.BankAccounts)
        {
            _bankAccounts.Add(new BankAccount(account, this));
        }

        foreach (BankManager manager in bank.BankManagers)
        {
    ...
    {
        Models.Bank dbitem = FMDB.Context.Banks.Find(Id);
        dbitem.Name = Name;
        ...
        foreach (KeyValuePair<ICurrency, decimal> item in CurrencyReserves)
        {
        ...
        foreach (KeyValuePair<(ICurrency From, ICurrency To), decimal> item in ExchangeRates)
        {
        ...
        foreach (long item in _bankManagerIds)
        {
        ...
        foreach (long item in _branchIds)
        {
    ...
    private bool BuildingCommandManager(ICharacter actor, StringStack command)
    {
        ...
        ICharacter target = actor.TargetActor(command.SafeRemainingArgument);
        if (target == null)
        {
    ...
    private bool BuildingCommandAccountType(ICharacter actor, StringStack command)
    {
        ...
        IBankAccountType type = BankAccountTypes.GetByNameOrAbbreviation(command.Last);
        if (type == null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandAccountTypeShow(ICharacter actor, StringStack command)
    {
        ...
        IBankAccountType type = BankAccountTypes.GetByNameOrAbbreviation(command.SafeRemainingArgument);
        if (type == null)
        {
    ...
    private bool BuildingCommandAccounTypeNew(ICharacter actor, StringStack command)
    {
        ...
        BankAccountType newAccount = new(this, name);
    ...
    private bool BuildingCommandZone(ICharacter actor, StringStack command)
    {
        ...
        IEconomicZone zone = Gameworld.EconomicZones.GetByIdOrName(command.SafeRemainingArgument);
        if (zone == null)
        {
    ...
    private bool BuildingCommandCode(ICharacter actor, StringStack command)
    {
        ...
        string code = command.SafeRemainingArgument;
    ...
    {
        List<IBankAccount> accounts = BankAccounts.ToList();
        while (!command.IsFinished)
        {
            ...
            {
                cmd = cmd[1..];
                IBankAccountType type = BankAccountTypes.GetByNameOrAbbreviation(cmd);
                if (type is null)
                {
    ...
    private void ManagerCommandAccount(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int accn) || accn <= 0)
        {
            actor.OutputHandler.Send("The account number you specify must be a number greater than zero.");
            return;
        }

        IBankAccount account = BankAccounts.FirstOrDefault(x => x.AccountNumber == accn);
        if (account is null)
        {
    ...
    {
        bool newMonth = EconomicZone.FinancialPeriodReferenceCalendar.CurrentDate.Day == 1 &&
                       !EconomicZone.FinancialPeriodReferenceCalendar.CurrentDate.IsIntercalaryDay()
            ;
        foreach (IBankAccount account in _bankAccounts)
        {
    ...
    private void ManagerCommandDeposit(ICharacter actor, StringStack command)
    {
        ...
        if (!actor.Currency.TryGetBaseCurrency(command.SafeRemainingArgument, out decimal amount))
        {
            actor.OutputHandler.Send(
        ...
        {
            Dictionary<ICurrencyPile, Dictionary<ICoin, int>> targetCoins = actor.Currency.FindCurrency(
        ...
        string moneyDescription = actor.Currency.Describe(amount, CurrencyDescriptionPatternType.Short)
                                    .ColourValue();
    ...
    private void ManagerCommandRollover(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int accn) || !_bankAccounts.Any(x => x.AccountNumber == accn))
        {
            actor.OutputHandler.Send("That is not a valid account number.");
            return;
        }

        IBankAccount account = _bankAccounts.First(x => x.AccountNumber == accn);
        if (account.AccountStatus != BankAccountStatus.Active)
        {
            actor.OutputHandler.Send(
        ...
        if (!int.TryParse(command.PopSpeech(), out int accn2) || !_bankAccounts.Any(x => x.AccountNumber == accn2))
        {
            actor.OutputHandler.Send("The rolled-over to account is not a valid account number.");
            return;
        }

        IBankAccount account2 = _bankAccounts.First(x => x.AccountNumber == accn2);
    ...
    private void ManagerCommandCredit(ICharacter actor, StringStack command)
    {
        ...
        if (!PrimaryCurrency.TryGetBaseCurrency(command.PopSpeech(), out decimal amount))
        {
    ...
    private void ManagerCommandAuditLogs(ICharacter actor, StringStack command)
    {
        ...
        sb.AppendLine();
        foreach (IBankManagerAuditLog item in AuditLogs.OrderByDescending(x => x.DateTime))
        {
            if (actor.IsAdministrator())
            {
                sb.AppendLine(
    ...
    private void ManagerCommandBalance(ICharacter actor, StringStack command)
    {
        ...
        foreach (KeyValuePair<ICurrency, decimal> currency in CurrencyReserves)
        {
            sb.AppendLine(
    ...
    private void ManagerCommandExchange(ICharacter actor, StringStack command)
    {
        ...
        ICurrency from = Gameworld.Currencies.GetByIdOrName(command.PopSpeech());
        if (from == null)
        {
        ...
        ICurrency to = Gameworld.Currencies.GetByIdOrName(command.PopSpeech());
        if (to == null)
        {
            actor.OutputHandler.Send($"There is no such currency as {command.Last.ColourCommand()}.");
            return;
        ...
        if (!decimal.TryParse(command.SafeRemainingArgument, out decimal rate))
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach (KeyValuePair<(ICurrency From, ICurrency To), decimal> currency in ExchangeRates)
        {
            sb.AppendLine(
        ...
        foreach (IBankAccountType type in _bankAccountTypes)
        {
            int count = _bankAccounts.Count(x => x.BankAccountType == type);
            sb.AppendLine(
    ...
    private void InitialiseAuditLogs()
    {
        ...
        using (new FMDB())
        {
            Models.Bank dbbank = FMDB.Context.Banks.Find(Id);
            foreach (Models.BankManagerAuditLog item in dbbank.BankManagerAuditLogs.AsEnumerable())
            {
    ...
    {
        (bool baseTruth, string baseError) = CanOpenNewBankAccounts(actor);
        if (!baseTruth)
        {
