...
public class BankAccountType : SaveableItem, IBankAccountType
{
    ...
    public BankAccountType(IBank bank, string name)
    {
        ...
        using (new FMDB())
        {
            Models.BankAccountType dbitem = new()
            {
                BankId = Bank.Id,
                Name = name,
    ...
    {
        Models.BankAccountType dbitem = FMDB.Context.BankAccountTypes.Find(Id);
        dbitem.Name = Name;
...
#1* Note - character|shop|clan argument only required if supplied prog is ambiguous#0".SubstituteANSIColour());
    ...
    private bool BuildingCommandCanOpenProg(ICharacter actor, StringStack command)
    {
        ...
        string arg1 = command.PopSpeech();
        string arg2 = command.SafeRemainingArgument;
        IFutureProg prog;
    ...
    {
        if (command.IsFinished || !command.SafeRemainingArgument.TryParsePercentageDecimal(out decimal value))
        {
    ...
    private bool BuildingCommandOverdrawnInterest(ICharacter actor, StringStack command, bool annual)
    {
        ...
        int days = Bank.EconomicZone.FinancialPeriodReferenceCalendar.Months.Sum(x => x.NormalDays);
        int months = Bank.EconomicZone.FinancialPeriodReferenceCalendar.Months.Count;
        int avgMonthDays = days / months;

        if (annual)
        {
            decimal daily = ((decimal)Math.Pow((double)(1.0M + value), 1.0 / months) - 1.0M) / avgMonthDays;
        ...
        decimal yearly = (decimal)Math.Pow((double)(value * avgMonthDays), months);
        actor.OutputHandler.Send(
    ...
    public string Show(ICharacter actor)
    {
        ...
        int daysInYear = Bank.EconomicZone.FinancialPeriodReferenceCalendar.Months.Sum(x => x.NormalDays);
        sb.AppendLine(
    ...
    public IBankAccount OpenAccount(IFrameworkItem owner)
    {
        using (new FMDB())
        {
            Models.BankAccount newAccount = new()
            {
                BankId = Bank.Id,
    ...
    public string ShowToCustomer(ICharacter actor)
    {
        ...
        IGameItemProto proto = Gameworld.ItemProtos.Get(PaymentItemPrototype ?? 0);
    ...
    {
        decimal fee = amount * WithdrawalFleeRate + WithdrawalFleeFlat;
        if (fee != 0.0M)
        {
    ...
    {
        decimal fee = amount * OverdrawFeeRate + OverdrawFeeFlat;
        if (fee != 0.0M)
        {
    ...
    {
        decimal fee = amount * DepositFeeRate + DepositFeeFlat;
        if (fee != 0.0M)
        {
    ...
    {
        decimal fee = amount * TransferFeeRate + TransferFeeFlat;
        if (fee != 0.0M)
        {
    ...
    {
        decimal fee = amount * TransferFeeOtherBankRate + TransferFeeOtherBankFlat;
        if (fee != 0.0M)
        {
    ...
    public void DoDailyAccountFees(IBankAccount account)
    {
        if (account.CurrentBalance < 0.0M)
        {
            decimal overdrawnfee = account.CurrentBalance * DailyOverdrawnInterestRate + DailyOverdrawnFee;
        ...
        decimal interest = DailyInterestRate * account.CurrentBalance;
        if (interest != 0.0M)
        {
