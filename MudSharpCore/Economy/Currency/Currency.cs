...
public class Currency : SaveableItem, ICurrency
{
    ...
    {
        foreach (Models.CurrencyDivision item in currency.CurrencyDivisions)
        {
            _currencyDivisions.Add(new CurrencyDivision(Gameworld, item, this));
        }

        foreach (Models.CurrencyDescriptionPattern item in currency.CurrencyDescriptionPatterns.OrderBy(x => x.Order))
        {
        ...
        foreach (Models.Coin item in currency.Coins)
        {
            Coin coin = new(Gameworld, item, this);
            _coins.Add(coin);
            Gameworld.Add(coin);
        }
    }
    ...
    private Currency(Currency rhs, string name)
    {
        ...
        using (new FMDB())
        {
            ...
            Dictionary<long, Models.CurrencyDivision> divisionMap = new();
            foreach (ICurrencyDivision division in rhs.CurrencyDivisions)
            {
                Models.CurrencyDivision dbdivision = new()
                {
                    Name = division.Name,
    ...
    public Dictionary<ICoin, int> FindCoinsForAmount(decimal amount, out bool exactMatch)
    {
        ...
        Dictionary<ICoin, int> results = new();
        foreach (ICoin coin in Coins.Where(x => x.UseForChange).OrderByDescending(x => x.Value))
        {
            int coinAmount = (int)(remainingAmount / coin.Value);
            if (coinAmount > 0)
            {
        ...
        decimal amount)
    {
        ...
        IOrderedEnumerable<ICoin> availableCoins =
            targetPiles.SelectMany(x => x.Coins).Select(x => x.Item1).Distinct().OrderByDescending(x => x.Value);
        Dictionary<ICurrencyPile, Dictionary<ICoin, int>> results = new();
        foreach (ICoin coin in availableCoins)
        {
            foreach (
            Tuple<ICurrencyPile, Tuple<ICoin, int>> pile in
            targetPiles.Where(x => x.Coins.Any(y => y.Item1 == coin))
                       .Select(x => Tuple.Create(x, x.Coins.First(y => y.Item1 == coin))))
            {
                int coinAmount = Math.Min((int)(amount / coin.Value), pile.Item2.Item2);
                if (coinAmount > 0)
                {
        ...
        foreach (ICoin coin in availableCoins.Reverse())
        {
            ICurrencyPile targetPile =
    ...
    {
        Models.Currency dbitem = FMDB.Context.Currencies.Find(Id);
        dbitem.Name = Name;
    ...
    private bool BuildingCommandRemovePattern(ICharacter actor, StringStack command)
    {
        ...
        ICurrencyDescriptionPattern pattern = PatternDictionary.Values.SelectMany(x => x).GetById(command.SafeRemainingArgument);
        if (pattern is null)
        {
    ...
    private bool BuildingCommandAddPattern(ICharacter actor, StringStack command)
    {
        ...
        CurrencyDescriptionPattern pattern = new(this, type);
    ...
    private bool BuildingCommandPattern(ICharacter actor, StringStack command)
    {
        ...
        ICurrencyDescriptionPattern pattern = PatternDictionary.Values.SelectMany(x => x).GetById(command.PopSpeech());
        if (pattern is null)
        {
    ...
    private bool BuildingCommandRemoveDivision(ICharacter actor, StringStack command)
    {
        ...
        ICurrencyDivision division = CurrencyDivisions.GetByIdOrName(command.PopSpeech());
        if (division is null)
        {
    ...
    private bool BuildingCommandAddDivision(ICharacter actor, StringStack command)
    {
        ...
        string name = command.PopSpeech().ToLowerInvariant();
        if (command.IsFinished)
        {
        ...
        if (!decimal.TryParse(command.SafeRemainingArgument, out decimal value))
        {
            actor.OutputHandler.Send($"The text {command.SafeRemainingArgument.ColourCommand()} is not a valid number.");
            return false;
        }

        CurrencyDivision division = new(Gameworld, name, value, this);
    ...
    {
        if (command.IsFinished || !decimal.TryParse(command.SafeRemainingArgument, out decimal value) || value < 0.0M)
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        foreach (ICurrencyDivision division in _currencyDivisions)
        {
        ...
        foreach (KeyValuePair<CurrencyDescriptionPatternType, List<ICurrencyDescriptionPattern>> patternType in PatternDictionary)
        {
            sb.AppendLine(StringUtilities.GetTextTable(
