...
public class CurrencyDescriptionPattern : SaveableItem, ICurrencyDescriptionPattern
{
        ...
        CurrencyDescriptionPatternType type)
    {
        ...
        foreach (Models.CurrencyDescriptionPatternElement item in pattern.CurrencyDescriptionPatternElements.OrderBy(x => x.Order))
        {
    ...
    public CurrencyDescriptionPattern(ICurrency parent, CurrencyDescriptionPatternType type)
    {
        ...
        using (new FMDB())
        {
            Models.CurrencyDescriptionPattern dbitem = new()
            {
                Type = (int)Type,
        ...
        switch (type)
        {
            ...
            case CurrencyDescriptionPatternType.Short:
            case CurrencyDescriptionPatternType.ShortDecimal:
                foreach (ICurrencyDivision division in parent.CurrencyDivisions)
                {
    ...
    {
        Models.CurrencyDescriptionPattern dbitem = FMDB.Context.CurrencyDescriptionPatterns.Find(Id);
    ...
    {
        List<ICurrencyDescriptionPatternElement> remove = _elements.Where(x => x.TargetDivision == division).ToList();
        foreach (ICurrencyDescriptionPatternElement item in remove)
        {
            item.Delete();
    ...
    {
        List<ICurrencyDescriptionPatternElement> list = new();
        int i = 0;
        foreach (ICurrencyDescriptionPatternElement item in _elements)
        {
    ...
    public string Describe(decimal value)
    {
        ...
        foreach (ICurrencyDescriptionPatternElement item in _elements.OrderBy(x => x.Order))
        {
            if (value / item.TargetDivision.BaseUnitConversionRate >= 1.0M || item == _elements.Last() ||
                item.ShowIfZero)
            {
                decimal ivalue = value / item.TargetDivision.BaseUnitConversionRate;
    ...
    private bool BuildingCommandRemoveElement(ICharacter actor, StringStack command)
    {
        ...
        ICurrencyDescriptionPatternElement element = Elements.GetByIdOrOrder(command.PopSpeech());
        if (element is null)
        {
    ...
    private bool BuildingCommandAddElement(ICharacter actor, StringStack command)
    {
        ...
        ICurrencyDivision division = Currency.CurrencyDivisions.GetByIdOrName(command.PopSpeech());
        if (division is null)
        {
        ...
        string plural = command.PopSpeech().ToLowerInvariant();
        if (command.IsFinished)
        {
        ...
        string pattern = command.SafeRemainingArgument;
        ...
        CurrencyDescriptionPatternElement element = new(this, pattern, plural, division);
    ...
    private bool BuildingCommandApplicabilityProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor.Gameworld, actor, command.SafeRemainingArgument,
    ...
    private bool BuildingCommandOrder(ICharacter actor, StringStack command)
    {
        ...
        int currentIndex = Currency.PatternDictionary[Type].IndexOf(this);
        Currency.PatternDictionary[Type].RemoveAt(currentIndex);
        Currency.PatternDictionary[Type].Insert(value - 1, this);
        for (int i = 0; i < Currency.PatternDictionary[Type].Count; i++)
        {
