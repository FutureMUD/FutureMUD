...
public class CurrencyDescriptionPatternElement : SaveableItem, ICurrencyDescriptionPatternElement
{
        ...
        CurrencyDescriptionPattern parentPattern, ICurrency parent)
    {
        ...
        foreach (CurrencyDescriptionPatternElementSpecialValues item in element.CurrencyDescriptionPatternElementSpecialValues)
        {
        ...
        string plural, ICurrencyDivision targetDivision)
    {
        ...
        using (new FMDB())
        {
            Models.CurrencyDescriptionPatternElement dbitem = new()
            {
                Order = Order,
    ...
    public string Describe(decimal amount)
    {
        ...
        bool remainder = amount != originalAmount;

        string pattern = remainder || string.IsNullOrEmpty(AlternatePattern) ? Pattern : AlternatePattern;
        string value;
    ...
    {
        Models.CurrencyDescriptionPatternElement dbitem = FMDB.Context.CurrencyDescriptionPatternElements.Find(Id);
        dbitem.Order = Order;
    ...
    {
        if (command.IsFinished || !decimal.TryParse(command.PopSpeech(), out decimal value))
        {
    ...
    private bool BuildingCommandOrder(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int value) || value < 1 || value > _parent.Elements.Count())
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        {
            foreach ((decimal value, string special) in SpecialValues)
            {
