...
public abstract class JobListingBase : SaveableItem, IJobListing
{
    ...
    private bool BuildingCommandMaximumDuration(ICharacter actor, StringStack command)
    {
        ...
        if (!MudTimeSpan.TryParse(command.SafeRemainingArgument, actor, out MudTimeSpan? timespan))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandPaygrade(ICharacter actor, StringStack command)
    {
        ...
        IClanMembership? membership = actor.ClanMemberships.FirstOrDefault(x => x.Clan == ClanMembership);
        ...
        IPaygrade? paygrade = ClanRank.Paygrades.GetByIdOrName(command.SafeRemainingArgument);
        if (paygrade is null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandAppointment(ICharacter actor, StringStack command)
    {
        ...
        IAppointment? appointment = ClanMembership.Appointments.GetByIdOrName(command.SafeRemainingArgument);
    ...
    private bool BuildingCommandRank(ICharacter actor, StringStack command)
    {
        ...
        IRank? rank = ClanMembership.Ranks.GetByIdOrName(command.SafeRemainingArgument);
        if (rank is null)
        {
            actor.OutputHandler.Send(
                $"There is no rank identified by the text {command.SafeRemainingArgument.ColourCommand()} in the {ClanMembership.FullName.ColourName()} clan.");
    ...
    private bool BuildingCommandClan(ICharacter actor, StringStack command)
    {
        ...
        IClan? clan = ClanModule.GetTargetClan(actor, command.SafeRemainingArgument);
        if (clan is null)
        {
    ...
    private bool BuildingCommandMaxEmployees(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int number) || number < 1)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandBankAccount(ICharacter actor, StringStack command)
    {
        ...
        (IBankAccount? account, string? error) = Bank.FindBankAccount(command.SafeRemainingArgument, null, actor);
        if (account is null)
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        IFrameworkItem employer = Employer;
        ...
        sb.AppendLine();
        DecimalCounter<ICurrency> reserves = new();
        ...
        foreach (KeyValuePair<ICurrency, decimal> money in MoneyPaidIn)
        {
        ...
        List<KeyValuePair<ICurrency, decimal>> owed = ActiveJobs.SelectMany(x => x.BackpayOwed).ToList();
        DecimalCounter<ICurrency> finalOwed = new();
        foreach (KeyValuePair<ICurrency, decimal> debt in owed)
        {
        ...
        DecimalCounter<ICurrency> dailyWages = new();
        foreach (IActiveJob job in ActiveJobs)
        {
    ...
    public (bool Truth, string Error) IsEligibleForJob(ICharacter actor)
    {
        ...
        double totalEffort = actor.ActiveJobs.Where(x => !x.IsJobComplete).Sum(x => x.FullTimeEquivalentRatio) +
    ...
    public string ShowToPlayer(ICharacter actor)
    {
        ...
        if (MaximumNumberOfSimultaneousEmployees > 0)
        {
            int positions = MaximumNumberOfSimultaneousEmployees - ActiveJobs.Count(x => !x.IsJobComplete);
        ...
        (bool truth, string? error) = IsEligibleForJob(actor);
        if (truth)
        {
    ...
    {
        DecimalCounter<ICurrency> counter = new();
        if (ClanMembership is null)
        {
            return counter;
        }
    ...
    public DecimalCounter<ICurrency> NetFinancialPosition
    {
        get
        {
            DecimalCounter<ICurrency> counter = new();
            counter += MoneyPaidIn;
            foreach (IActiveJob job in _activeJobs)
            {
    ...
    public double DaysOfSolvency
    {
        get
        {
            double days = double.PositiveInfinity;
            foreach (KeyValuePair<ICurrency, decimal> item in NetFinancialPosition)
            {
                decimal totalDaily = _activeJobs.Sum(x => x.DailyPay()[item.Key]);
                if (totalDaily == 0.0M)
                {
                    continue;
                }

                double currencyDays = (double)(item.Value / totalDaily);
