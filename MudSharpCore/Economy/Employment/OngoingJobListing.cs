...
public class OngoingJobListing : JobListingBase, IOngoingJobListing
{
    ...
    public OngoingJobListing(JobListing dbitem, IFuturemud gameworld) : base(dbitem, gameworld)
    {
        ...
        {
            ActiveOngoingJob job = new(dbjob, this, Gameworld);
            Gameworld.Add(job);
            _activeJobs.Add(job);
        }
    ...
    public void RegisterEvents()
    {
        ...
        Gameworld.HeartbeatManager.FuzzyThirtySecondHeartbeat += DoJobHeartbeat;
        MudDateTime referenceTime = PayInterval.GetNextAdjacentToCurrent(PayReference);
    ...
    public void CheckPayroll()
    {
        ...
        {
            foreach (IActiveJob job in ActiveJobs)
            {
    ...
    private void PartialPayrollForJob(IActiveJob job, double portion)
    {
        ...
        decimal pay = PayExpression.EvaluateDecimalWith(("effort", job.CurrentPerformance)) * (decimal)portion;
        ...
        if (pay > 0)
        {
            ...
            {
                decimal max = BankAccount.MaximumWithdrawal();
                if (max >= pay)
                {
                    job.RevenueEarned[PayCurrency] += pay;
    ...
    {
        MudDateTime now = PayReference.Calendar.CurrentDateTime;
        foreach (IActiveJob? job in ActiveJobs.ToList())
        {
            if (job.IsJobComplete)
            {
            ...
            foreach (KeyValuePair<ICurrency, decimal> item in job.BackpayOwed.ToList())
            {
                decimal owed = item.Value;
                if (owed <= 0.0M)
                {
                    continue;
                }
                ...
                if (MoneyPaidIn[item.Key] >= 0.0M)
                {
                    ...
                    {
                        decimal amount = MoneyPaidIn[item.Key];
    ...
    {
        DoJobLifecycleCheck();
        double decay = Gameworld.GetStaticDouble("OngoingJobPerformanceDecayPerHour") / 120.0;
        foreach (IActiveJob? job in ActiveJobs.ToList())
        {
            if (job.IsJobComplete)
            {
...
#1Note: Use the variable #6effort#1 in the pay expression for job project effort.#0";
    ...
    private bool BuildingCommandProject(ICharacter actor, StringStack command)
    {
        ...
        List<IProject> projects = Gameworld.Projects.Where(x => x.AppearInJobsList && x.Status == RevisionStatus.Current)
                                .ToList();
        IProject? project = projects.GetByIdOrName(command.SafeRemainingArgument);
        if (project is null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandEasyPay(ICharacter actor, StringStack command)
    {
        ...
        if (!PayCurrency.TryGetBaseCurrency(command.PopSpeech(), out decimal baseAmount) || baseAmount < 0.0M)
        {
            actor.OutputHandler.Send(
        ...
        if (!PayCurrency.TryGetBaseCurrency(command.SafeRemainingArgument, out decimal bonusAmount) ||
            bonusAmount < 0.0M)
        {
            actor.OutputHandler.Send(
                $"The text {command.Last.ColourCommand()} is not a valid amount of {PayCurrency.Name.ColourValue()}.");
    ...
    private bool BuildingCommandDate(ICharacter actor, StringStack command)
    {
        ...
        if (!MudDateTime.TryParse(command.SafeRemainingArgument, EconomicZone.FinancialPeriodReferenceCalendar,
                EconomicZone.FinancialPeriodReferenceClock, actor, out MudDateTime? dt, out string? error))
        {
            MudDate date = PayReference.Date;
            MudTime time = PayReference.Time;
            IMudTimeZone tz = PayReference.TimeZone;
            actor.OutputHandler.Send(
        ...
        MudDateTime referenceTime = PayInterval.GetNextAdjacentToCurrent(dt);
        PayReference = referenceTime;
        Changed = true;
        if (referenceTime != dt)
        {
    ...
    private bool BuildingCommandCurrency(ICharacter actor, StringStack command)
    {
        ...
        ICurrency? currency = Gameworld.Currencies.GetByIdOrName(command.SafeRemainingArgument);
        if (currency is null)
        {
    ...
    private bool BuildingCommandExpression(ICharacter actor, StringStack command)
    {
        ...
        ExpressionEngine.Expression expr = new(command.SafeRemainingArgument);
        if (expr.HasErrors())
        {
    ...
    {
        decimal minimum = Convert.ToDecimal(PayExpression.EvaluateWith(("effort", 0.0M)));
        decimal maximum = Convert.ToDecimal(PayExpression.EvaluateWith(("effort", 1000000.0M)));
        if (minimum != maximum)
        {
            return
    ...
    public override IActiveJob ApplyForJob(ICharacter actor)
    {
        MudDateTime? endingDate = MaximumDuration is not null
            ...
            : default;
        ActivePersonalProject? project = PersonalProject is not null ? new ActivePersonalProject(PersonalProject, actor) : default;
        ...
        bool hasClan = actor.ClanMemberships.Any(x => !x.IsArchivedMembership && x.Clan == ClanMembership);
        if (ClanMembership is not null)
        {
            ...
            if (membership is null)
            {
                ...
                IPaygrade? paygrade = ClanPaygrade ?? rank.Paygrades.OrderBy(x => x.PayAmount).FirstOrDefault();
                using (new FMDB())
                {
                    ...
                    Community.ClanMembership newMembership = new(dbitem, ClanMembership, Gameworld);
        ...
        ActiveOngoingJob job = new(this, actor,
