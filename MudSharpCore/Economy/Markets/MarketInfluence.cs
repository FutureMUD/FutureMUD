...
public class MarketInfluence : SaveableItem, IMarketInfluence
{
        ...
        [CanBeNull] MudDateTime appliesUntil)
    {
        ...
        foreach (MarketImpact impact in template.MarketImpacts)
        {
    ...
    private MarketInfluence(MarketInfluence rhs, string newName)
    {
        ...
        foreach (MarketImpact impact in rhs.MarketImpacts)
        {
            _marketImpacts.Add(impact with { });
        }
    ...
    public MarketInfluence(IMarket market, Models.MarketInfluence influence)
    {
        ...
        foreach (XElement impact in XElement.Parse(influence.Impacts).Elements("Impact"))
        {
    ...
    {
        Models.MarketInfluence dbitem = FMDB.Context.MarketInfluences.Find(Id);
        dbitem.Name = Name;
    ...
    private bool BuildingCommandDuration(ICharacter actor, StringStack command)
    {
        ...
        if (!TimeSpan.TryParse(command.SafeRemainingArgument, out TimeSpan value))
        {
    ...
    private bool BuildingCommandUntil(ICharacter actor, StringStack command)
    {
        ...
        if (!MudDateTime.TryParse(command.SafeRemainingArgument, Market.EconomicZone.FinancialPeriodReferenceCalendar, Market.EconomicZone.FinancialPeriodReferenceClock, actor, out MudDateTime date, out string error))
        {
    ...
    private bool BuildingCommandImpact(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParsePercentage(actor.Account.Culture, out double supply))
        {
        ...
        if (!command.PopSpeech().TryParsePercentage(actor.Account.Culture, out double demand))
        {
            actor.OutputHandler.Send($"The text {command.Last.ColourCommand()} is not a valid percentage.");
            return false;
        }
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        foreach (MarketImpact impact in _marketImpacts)
        {
    ...
    {
        MudDateTime now = Market.EconomicZone.FinancialPeriodReferenceCalendar.CurrentDateTime;
        if (AppliesFrom > now)
        {
            Delete();
            return;
