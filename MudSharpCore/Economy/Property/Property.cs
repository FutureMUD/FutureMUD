...
public partial class Property : SaveableItem, IProperty
{
    ...
    public Property(Models.Property property, IFuturemud gameworld)
    {
        ...
        foreach (Models.PropertyLeaseOrder order in property.LeaseOrders)
        {
            PropertyLeaseOrder newOrder = new(order, this);
        ...
        foreach (Models.PropertyLease lease in property.PropertyLeases)
        {
            PropertyLease newLease = new(lease, this);
    ...
    {
        Models.Property dbitem = FMDB.Context.Properties.Find(Id);
        dbitem.EconomicZoneId = EconomicZone.Id;
        dbitem.ApplyCriminalCodeInProperty = ApplyCriminalCodeInProperty;
        dbitem.DetailedDescription = DetailedDescription;
        dbitem.Name = _name;
...
** This version is necessary if the currencies are different between the two economic zones";
    ...
    private bool BuildingCommandEconomicZone(ICharacter actor, StringStack command)
    {
        ...
        IEconomicZone zone = Gameworld.EconomicZones.GetByIdOrName(command.PopSpeech());
        if (zone == null)
        {
        ...
        if (!decimal.TryParse(command.SafeRemainingArgument, out decimal multiplier) || multiplier <= 0.0M)
        {
        ...
        foreach (IPropertyLeaseOrder order in ExpiredLeaseOrders)
        {
    ...
    private bool BuildingCommandAddCell(ICharacter actor)
    {
        ...
        IProperty existing = Gameworld.Properties.FirstOrDefault(x => x.PropertyLocations.Contains(actor.Location));
        if (existing != null)
        {
            actor.OutputHandler.Send(
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach (IPropertyOwner owner in PropertyOwners)
        {
    ...
    {
        ICharacter controller = AutomaticShopController();
        foreach (IShop shop in PropertyShops)
        {
            shop.ClearEmployees();
    ...
    public void RekeyAllLocks()
    {
        ...
        foreach (ICell cell in PropertyLocations)
        {
            ...
            foreach (ICellExit exit in cell.ExitsFor(null, true))
            {
                ...
                {
                    foreach (ILock lockComp in door.Locks)
                    {
                        lockComp.Pattern = pattern;
                    }
                }
        ...
        List<IPropertyKey> existingKeys = _propertyKeys.ToList();
        _propertyKeys.Clear();
        foreach (IPropertyKey key in existingKeys)
        {
            GameItems.IGameItemProto proto = key.GameItem.Prototype;
            GameItems.IGameItem newItem = proto.CreateNew();
            IKey keyComp = newItem.GetItemType<IKey>();
            keyComp?.Pattern = pattern;
            PropertyKey newKey = new(Gameworld, this, key.Name, newItem,
    ...
    public string PreviewProperty(ICharacter voyeur)
    {
        ...
        bool owner = voyeur.IsAdministrator() || _propertyOwners.Any(x =>
        ...
        {
            sb.AppendLine("\nKeys:");
            foreach (IPropertyKey key in _propertyKeys)
            {
                sb.AppendLine(
        ...
        {
            sb.AppendLine("\nOwners:");
            foreach (IPropertyOwner powner in PropertyOwners)
            {
                sb.AppendLine(
        ...
        if (SaleOrder != null)
        {
            ...
            else if (owner)
            {
                ...
                else
                {
                    ...
                    List<IPropertyOwner> missing = SaleOrder.PropertyOwnerConsent.Where(x => !x.Value).Select(x => x.Key).ToList();
                    sb.AppendLine(
        ...
        if (LeaseOrder != null)
        {
            ...
            else if (owner)
            {
                ...
                {
                    List<IPropertyOwner> missing = LeaseOrder.PropertyOwnerConsent.Where(x => !x.Value).Select(x => x.Key).ToList();
                    sb.AppendLine(
                        $"Consent still required from {missing.Select(x => x.Describe(voyeur)).ListToString()}");
                }
            }
        ...
        if (Lease != null)
        {
            ICharacter leaseCh = Lease.Leaseholder as ICharacter;
            IClan leaseClan = Lease.Leaseholder as IClan;
    ...
    {
        IPropertySaleOrder saleOrder = SaleOrder;
    ...
    {
        IPropertyOwner newPropertyOwner = _propertyOwners.FirstOrDefault(x => x.Owner == newOwnerItem);
