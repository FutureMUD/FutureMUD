...
public class PropertyLease : SaveableItem, IPropertyLease
{
        ...
        IPropertyLeaseOrder leaseOrder)
    {
        ...
        using (new FMDB())
        {
            Models.PropertyLease dbitem = new()
            {
    ...
    {
        AutoRenew = false;
        foreach (IPropertyKey key in Property.PropertyKeys.Where(x => !x.IsReturned).ToList())
        {
    ...
    {
        Models.PropertyLease dbitem = FMDB.Context.PropertyLeases.Find(Id);
