...
internal abstract class ShopperBase : SaveableItem, IShopper
{
    ...
    {
        Type pType = typeof(IShopper);
        foreach (
            Type type in Futuremud.GetAllTypes().Where(x => x.GetInterfaces().Contains(pType)))
        {
            MethodInfo method = type.GetMethod("RegisterLoaders", BindingFlags.Static | BindingFlags.Public);
            method?.Invoke(null, []);
        }
    }
    ...
    {
        return _typeHelps.TryGetValue(type, out string help) ? (_typeBlurbs[type], help) : ("An invalid type", "");
    }
    ...
    protected void FlushLogEntries()
    {
        ...
        string mudnow = EconomicZone.FinancialPeriodReferenceCalendar.CurrentDateTime.GetDateTimeString();
        using (new FMDB())
        {
            foreach ((string Type, string Message) entry in _queuedLogEntries)
            {
    ...
    {
        Shopper dbitem = FMDB.Context.Shoppers.Find(Id);
        dbitem.Name = Name;
...
{SubtypeHelpText}";
    ...
    private bool BuildingCommandNextDate(ICharacter actor, StringStack command)
    {
        ...
        if (!MudDateTime.TryParse(command.SafeRemainingArgument, EconomicZone.FinancialPeriodReferenceCalendar, EconomicZone.FinancialPeriodReferenceClock, actor, out MudDateTime dt, out string error))
        {
