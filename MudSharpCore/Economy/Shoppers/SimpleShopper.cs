...
internal class SimpleShopper : ShopperBase
{
    ...
    private bool BuildingCommandShopWeightProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Number, [ProgVariableTypes.Shop]).LookupProg();
        if (prog is null)
        {
            return false;
        }
    ...
    private bool BuildingCommandShopProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Boolean, [ProgVariableTypes.Shop]).LookupProg();
        if (prog is null)
        {
            return false;
        }
    ...
    private bool BuildingCommandBudget(ICharacter actor, StringStack command)
    {
        ...
        if (!EconomicZone.Currency.TryGetBaseCurrency(command.SafeRemainingArgument, out decimal value) || value < 0.0M)
        {
    ...
    public override void DoShop()
    {
        ...
        List<(IGameItem Item, IMerchandise Merchandise, decimal Price)> allItems = shop
                       .AllMerchandiseForVirtualShoppers
                       .Where(x => WillBuyItemProg.ExecuteBool(x.Item, x.Merchandise, x.Price))
                       .ToList();
        decimal budget = BudgetPerShop;
        ...
        while (budget > 0.0M)
        {
            ...
            (IGameItem Item, IMerchandise Merchandise, decimal Price) item = allItems
            ...
            {
                int maxQuantity = (int)Math.Floor(item.Price / item.Item.Quantity);
                if (maxQuantity > 1)
                {
