...
public class Merchandise : LateInitialisingItem, IMerchandise
{
    ...
    {
        Models.Merchandise dbitem = FMDB.Context.Merchandises.Find(Id);
    ...
    private bool BuildingCommandContainer(ICharacter actor, StringStack command)
    {
        ...
        IGameItem targetItem = actor.TargetLocalItem(command.PopSpeech());
        if (targetItem == null)
        {
    ...
    private bool BuildingCommandReorder(ICharacter actor, StringStack command)
    {
        ...
        decimal price;
        if (command.PeekSpeech().TryParsePercentageDecimal(actor.Account.Culture, out decimal percentage))
        {
        ...
        {
            price = Shop.Currency.GetBaseCurrency(command.PopSpeech(), out bool success);
            if (!success)
            {
                actor.OutputHandler.Send("That is not a valid price.");
                return false;
    ...
    {
        if (command.IsFinished || !command.SafeRemainingArgument.TryParsePercentage(actor.Account.Culture, out double value) || value < 0.0 || value > 1.0)
        {
    ...
    {
        if (command.IsFinished || !command.SafeRemainingArgument.TryParsePercentageDecimal(actor.Account.Culture, out decimal value) || value < 0.0M)
        {
    ...
    public void ShowToBuilder(ICharacter actor)
    {
        ...
        List<IShopDeal> matchingDeals = Shop.Deals
        ...
        if (matchingDeals.Any())
        {
            sb.AppendLine();
            sb.AppendLine("Deals:");
            foreach (IShopDeal deal in matchingDeals)
            {
                string dealType = deal is ShopDeal shopDeal ? shopDeal.DescribeType(actor) : deal.DealType.DescribeEnum().ColourName();
                string expiry = deal.Expiry.Date is null
                    ? "Never".ColourValue()
