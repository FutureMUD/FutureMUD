...
public abstract partial class Shop
{
        ...
        ShopDealApplicability applicability = ShopDealApplicability.Sell)
    {
        ...
        decimal baseUnitPrice = applicability switch
        {
            ShopDealApplicability.Buy => merchandise.EffectivePrice * merchandise.BaseBuyModifier,
            _ => merchandise.EffectivePrice
        };

        List<IShopDeal> applicableDeals = Deals
            .Where(x => x.Applies(merchandise, actor, quantity, applicability, now))
            .ToList();
        List<IShopDeal> cumulativeDeals = applicableDeals
            .Where(x => x.IsCumulative)
            .ToList();
        List<IShopDeal> selectedDeals = new(cumulativeDeals);
        List<IShopDeal> nonCumulativeDeals = applicableDeals
            .Where(x => !x.IsCumulative)
            .ToList();
        ...
        decimal totalAdjustment = selectedDeals.Sum(x => x.PriceAdjustmentPercentage);
        decimal adjustedUnitPrice = Math.Max(0.0M, baseUnitPrice * (1.0M + totalAdjustment));
        decimal unitTax = applicability == ShopDealApplicability.Buy
            ? 0.0M
            : Math.Round(
        ...
        decimal totalPretax = Math.Truncate(adjustedUnitPrice * quantity);
        decimal includedTax = Math.Truncate(unitTax * quantity);
        decimal totalPrice = totalPretax + includedTax;
    ...
    {
        IShopPriceCalculation calculation = GetPriceCalculation(purchaser, merchandise, 1);
    ...
    {
        List<IShopDeal> deals = merchandise is null
