...
public class ShopDeal : LateInitialisingItem, IShopDeal
{
    ...
    private bool BuildingCommandType(ICharacter actor, StringStack command)
    {
        ...
        switch (command.PopSpeech().ToLowerInvariant())
        {
            case "sale":
                ...
                return true;
            case "volume":
                if (command.IsFinished || !int.TryParse(command.SafeRemainingArgument, out int quantity) || quantity < 2)
                {
    ...
    private bool BuildingCommandTarget(ICharacter actor, StringStack command)
    {
        ...
        switch (command.PopSpeech().ToLowerInvariant())
        {
            ...
            case "merchandise":
                ...
                IMerchandise? merch = Shop.Merchandises.GetById(command.Last) ??
    ...
    {
        if (command.IsFinished || !command.SafeRemainingArgument.TryParsePercentageDecimal(actor.Account.Culture, out decimal value))
        {
    ...
    private bool BuildingCommandExpires(ICharacter actor, StringStack command)
    {
        ...
        if (!MudDateTime.TryParse(command.SafeRemainingArgument, Shop.EconomicZone.FinancialPeriodReferenceCalendar,
                Shop.EconomicZone.FinancialPeriodReferenceClock, actor, out MudDateTime? value, out string? error))
        {
    ...
    {
        if (command.IsFinished || !MudTimeSpan.TryParse(command.SafeRemainingArgument, actor, out MudTimeSpan? value))
        {
