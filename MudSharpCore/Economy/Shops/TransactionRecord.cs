...
public class TransactionRecord : LateInitialisingItem, ITransactionRecord
{
    ...
    {
        ShopTransactionRecord dbitem = FMDB.Context.ShopTransactionRecords.Find(Id);
        ...
        dbitem.MerchandiseId = Merchandise?.Id;
        Changed = false;
    }

    public override object DatabaseInsert()
    {
        ShopTransactionRecord dbitem = new();
    ...
    public TransactionRecord(ShopTransactionType type, ICurrency currency, IShop shop, MudDateTime mudtime,
        ICharacter thirdparty, decimal pretaxvalue, decimal tax, IMerchandise merchandise)
    {
    ...
    public TransactionRecord(Models.ShopTransactionRecord record, IShop shop, IFuturemud gameworld)
    {
        ...
        Tax = record.Tax;
        Merchandise = shop.Merchandises.Get(record.MerchandiseId ?? 0L);
    }
        ...
        get
        {
            switch (TransactionType)
            {
                case ShopTransactionType.Sale:
                    return PretaxValue;
                case ShopTransactionType.Refund:
                    return PretaxValue;
