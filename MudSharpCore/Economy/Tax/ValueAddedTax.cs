...
public class ValueAddedTax : SalesTaxBase
{
    ...
    {
        XElement definition = XElement.Parse(tax.Definition);
        Rate = decimal.Parse(definition.Element("Rate").Value);
        foreach (XElement element in definition.Element("ExemptTags").Elements())
        {
    ...
    {
        return saleValue * Rate;
    }

    public override void Save()
    {
        EconomicZoneTax dbitem = FMDB.Context.EconomicZoneTaxes.Find(Id);
        dbitem.Name = Name;
    ...
    private bool BuildingCommandRate(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParsePercentageDecimal(out decimal value))
        {
    ...
    public override string Show(ICharacter actor)
    {
        ...
        foreach (ITag tag in ExemptTags)
        {
