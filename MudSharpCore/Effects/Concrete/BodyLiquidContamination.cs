...
public class BodyLiquidContamination : Effect, ILiquidContaminationEffect, IDescriptionAdditionEffect, IEffectAddsWeight
{
    ...
    {
        BodyLiquidContamination existing = actor.CombinedEffectsOfType<BodyLiquidContamination>().FirstOrDefault(x => bodyparts.All(y => x.Bodyparts.Contains(y)));
        ...
        BodyLiquidContamination effect = new(actor.Body, mixture, bodyparts);
    ...
    {
        ItemSaturationLevel saturation = BodyOwner.SaturationLevelForLiquid(Bodyparts);
        switch (saturation)
        {
        ...
        string partDescription = BodyOwner.DescribeBodypartGroup(Bodyparts);
    ...
    private void ContaminatingLiquidOnLiquidMixtureChanged(LiquidMixture mixture)
    {
        ...
        (double coat, double _) = BodyOwner.LiquidAbsorbtionAmounts;
        ItemSaturationLevel saturation = BodyOwner.SaturationLevel;
