...
public class Dragging : Effect, IDragging
{
    ...
    {
        ICharacter actor = (ICharacter)owner;
    ...
    private void Dragger_StateChanged(IPerceivable owner)
    {
        if (!CharacterState.Able.HasFlag(CharacterOwner.State))
        {
            ...
            foreach (ICharacter helper in Helpers)
            {
    ...
    {
        foreach (ICharacter dragger in Draggers.Select(x => x.Character))
        {
            dragger.Send(
    ...
    {
        ICharacter tChar = (ICharacter)Target;
    ...
    public override void RemovalEffect()
    {
        foreach (ICharacter helper in _helpers.ToList())
        {
            RemoveHelper(helper);
