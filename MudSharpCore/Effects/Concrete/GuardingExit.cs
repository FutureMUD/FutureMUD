...
public class GuardingExit : Effect, IGuardExitEffect
{
    ...
    protected GuardingExit(XElement effect, IPerceivable owner) : base(effect, owner)
    {
        ...
        XElement root = effect.Element("Element");
        long exitId = long.Parse(root.Element("Exit").Value);
        ...
        foreach (XElement item in root.Element("Exemptions").Elements())
        {
    ...
    {
        List<DummyKeywordedItem<long>> options = new(
            from kvp in _permittedExemptionCharacterLastDescs
            select new DummyKeywordedItem<long>(kvp.Key, item => kvp.Value.Split(' ', '-').ToList())
        );

        DummyKeywordedItem<long> option = options.GetFromItemListByKeyword(playerInput, null);
        if (option == null)
        {
