...
public class IncreasedBranchChance : Effect, IEffectSubtype
{
    ...
    public IncreasedBranchChance(XElement effect, IPerceivable owner) : base(effect, owner)
    {
        ...
        foreach (XElement item in root.Element("Skills")?.Elements("Skill") ?? Enumerable.Empty<XElement>())
        {
        ...
        foreach (XElement item in root.Element("Knowledges")?.Elements("Knowledge") ?? Enumerable.Empty<XElement>())
        {
        ...
        {
            foreach (ISkill item in ch.TraitsOfType(TraitType.Skill).OfType<ISkill>())
            {
            ...
            foreach (IKnowledge item in ch.Knowledges)
            {
