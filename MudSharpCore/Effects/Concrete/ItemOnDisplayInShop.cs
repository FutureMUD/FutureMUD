...
public class ItemOnDisplayInShop : Effect, IDescriptionAdditionEffect, IHandleEventsEffect
{
    ...
    {
        IMerchandise merch = Merchandise;
        if (merch == null)
        {
            return string.Empty;
        }

        (bool truth, string reason) = Shop.CanBuy(voyeur as ICharacter, merch, 1, null);
        if (!truth)
        {
        ...
        decimal price = Shop.PriceForMerchandise(voyeur as ICharacter, merch, 1);
        return
