    ...
    ISDescAdditionEffect
{
    ...
    private void ContaminatingLiquidOnLiquidMixtureChanged(LiquidMixture mixture)
    {
        ...
        IGameItem ownerItem = (IGameItem)Owner;
        ...
        if (mixture.TotalVolume > maxAbsorbed)
        {
                ...
                !ownerItem.Location.IsSwimmingLayer(ownerItem.RoomLayer))
            {
                ...
                IPerceiver locationPerceivable = (IPerceiver)ownerItem.LocationLevelPerceivable;
                LiquidMixture excess = new(mixture);
    ...
    {
        foreach (LiquidInstance instance in ContaminatingLiquid.Instances.Where(x => x.Liquid.Solvent != null).ToList())
        {
            double totalCleanableAmount =
    ...
    public bool CleanWithLiquid(LiquidMixture liquid, double amount)
    {
        ...
        List<LiquidInstance> instancesToClean = new();
        foreach (LiquidInstance instance in _contaminatingLiquid.Instances.ToList())
        {
        ...
        double totalRemovableVolume = instancesToClean.Sum(x => x.Amount);
        foreach (LiquidInstance instance in instancesToClean)
        {
    ...
    {
        LiquidSurfaceReactionHelper.ApplyToItem((IGameItem)Owner, ContaminatingLiquid).ProcessPassiveWounds();
        ...
        double dried = Math.Max(0.02 / Gameworld.UnitManager.BaseFluidToLitres, ContaminatingLiquid.TotalVolume * 0.1);
        List<(LiquidInstance Instance, double)> ratios = ContaminatingLiquid.Instances
    ...
    public string AddendumText
    {
        ...
        {
            IGameItem ownerItem = (IGameItem)Owner;
            Dictionary<ILiquid, double> liquids = new();
            foreach (LiquidInstance liquid in ContaminatingLiquid.Instances)
            {
                ILiquid existing = liquids.Keys.FirstOrDefault(x =>
            ...
            List<string> texts = new(liquids.Keys.Count);
            foreach (KeyValuePair<ILiquid, double> instance in liquids)
            {
                ItemSaturationLevel saturation = ownerItem.SaturationLevelForLiquid(instance.Value);
                switch (saturation)
                {
                    case ItemSaturationLevel.Dry:
                        continue;
    ...
    public double LiquidAmountConsumed
    {
        get
        {
            ILiquid required = LiquidRequired;
