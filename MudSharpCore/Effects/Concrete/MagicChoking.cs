...
public class MagicChoking : ConcentrationConsumingEffect, IMagicEffect, ICheckBonusEffect
{
    ...
    public override void ExpireEffect()
    {
        if (Power.TargetGetsResistanceCheck)
        {
            ...
            Dictionary<Difficulty, CheckOutcome> results = check.CheckAgainstAllDifficulties(CharacterOwner, Power.SkillCheckDifficulty,
                Power.SkillCheckTrait, CharacterTarget);
            ICheck resistCheck = Gameworld.GetCheck(CheckType.ResistMagicChokePower);
            Dictionary<Difficulty, CheckOutcome> resistResults = resistCheck.CheckAgainstAllDifficulties(CharacterTarget, Power.ResistCheckDifficulty,
                null, CharacterOwner);
            OpposedOutcome outcome = new(results, resistResults, Power.SkillCheckDifficulty,
