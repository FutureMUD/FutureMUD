...
public class MurderousRage : Effect, IRageEffect, IScoreAddendumEffect
{
    ...
    public override void ExpireEffect()
    {
        ...
        List<ICharacter> potentialTargets = CharacterOwner.Combat?.Combatants.Except(CharacterOwner).OfType<ICharacter>().Where(x =>
        ...
        {
            ICharacter target = potentialTargets.GetRandomElement();
        ...
        {
            IGameItem target = roomItems.GetRandomElement() ?? doorItems.GetRandomElement();
            CharacterOwner.Send(
                $"Without a suitable outlet for your rage, you feel compelled to smash up {target.HowSeen(CharacterOwner)}!");
            IWeaponAttack attack = weapon?.WeaponType
            ...
            INaturalAttack uattack = CharacterOwner.Race
