...
public class Nausea : Effect, ICheckBonusEffect, IScoreAddendumEffect
{
    ...
    public double BloodAlcoholContent
    {
        ...
        set
        {
            double oldValue = _bloodAlcoholContent;
    ...
    public double GetHungerThirstMultiplier
    {
        get
        {
            IBody bodyOwner = (IBody)Owner;
            double hungerMultiplier = 0.0;
    ...
    private string StatusDescription
    {
        get
        {
            double multiplier = GetHungerThirstMultiplier;
            double effectiveValue = _intensityPerGramMass * GetIntensityFromBAC(BloodAlcoholContent) * multiplier;
    ...
    {
        double multiplier = GetHungerThirstMultiplier;
        double oldEffectiveValue = (oldIntensity + GetIntensityFromBAC(oldBac)) * multiplier;
        double effectiveValue = (_intensityPerGramMass + GetIntensityFromBAC(BloodAlcoholContent)) * multiplier;
    ...
    public double IntensityPerGramMass
    {
        ...
        set
        {
            double oldValue = _intensityPerGramMass;
    ...
    public override void ExpireEffect()
    {
        ...
        if (multiplier > 0.4)
        {
            ...
            string contents = "the contents of &0's stomach";
            switch (bodyOwner.NeedsModel.Status & NeedsResult.HungerOnly)
            {
                case NeedsResult.AbsolutelyStuffed:
            ...
            string bloodDesc = "";
            double digestiveBlood = bodyOwner.EffectsOfType<IInternalBleedingEffect>()
            ...
            foreach (IInternalBleedingEffect effect in bodyOwner.EffectsOfType<IInternalBleedingEffect>()
                                            .Where(x => x.Organ is StomachProto || x.Organ is TracheaProto).ToList())
            {
                effect.BloodlossTotal = effect.BloodlossTotal / 2;
            }

            string ejecta = new[] { contents, bloodDesc }.ListToString();
