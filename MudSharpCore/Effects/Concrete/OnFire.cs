...
public class OnFire : Effect
{
	...
	{
		var existing = owner.EffectsOfType<OnFire>().FirstOrDefault();
		...
		var effect = new OnFire(owner, profile);
	...
	private void ApplyThermalLoad()
	{
		...
		var thermal = ch.CombinedEffectsOfType<ThermalImbalance>().FirstOrDefault();
		if (thermal is null)
		{
	...
	private IEnumerable<IWound> ApplyBurnDamage()
	{
		switch (Owner)
		{
			case ICharacter ch:
			{
				var bodypart = ch.Body.RandomBodyPartGeometry(Orientation.Centre, Alignment.Front, Body.Facing.Front, false);
	...
	private void AttemptSpread()
	{
		...
		foreach (var target in positionable.LocalThingsAndProximities()
