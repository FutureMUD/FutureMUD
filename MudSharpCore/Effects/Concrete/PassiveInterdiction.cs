...
public class PassiveInterdiction : Effect, IRangedObstructionEffect, IRemoveOnMovementEffect
{
        ...
        ILDescSuffixEffect, IScoreAddendumEffect
    {
        ...
        {
            List<PassivelyInterceding> effects = interposer.EffectsOfType<PassivelyInterceding>().ToList();
            if (!effects.Any())
            ...
            decimal score = effects.Sum(x => 1.0M / (interposer.CurrentContextualSize(SizeContext.RangedTarget) -
    ...
    public bool IsObstructedFrom(IPerceivable target, IPerceivable intercessor, [CanBeNull] IPerceivable attackOrigin)
    {
        ...
        SizeCategory targetSize = (target as IHaveContextualSizeCategory)?.CurrentContextualSize(SizeContext.RangedTarget) ??
                         GameItems.SizeCategory.Normal;
        SizeCategory intercessorSize =
            (intercessor as IHaveContextualSizeCategory)?.CurrentContextualSize(SizeContext.RangedTarget) ??
            GameItems.SizeCategory.Normal;
        int diff = (int)targetSize - (int)intercessorSize;
        double chance = 0.0;
        if (diff <= -2)
        {
            chance = 1.0;
