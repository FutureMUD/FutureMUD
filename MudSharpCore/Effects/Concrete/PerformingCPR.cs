...
public class PerformingCPR : CharacterActionWithTarget
{
    ...
    public override void ExpireEffect()
    {
        double targetHeartFunction = TargetCharacter.Body.Organs.OfType<HeartProto>()
                                                 ...
                                                 .DefaultIfEmpty(0)
                                                 .Sum();
        ICheck check = Gameworld.GetCheck(CheckType.PerformCPR);
        CheckOutcome result = check.Check(CharacterOwner, targetHeartFunction <= 0.0 ? Difficulty.Hard : Difficulty.Normal,
            TargetCharacter);
