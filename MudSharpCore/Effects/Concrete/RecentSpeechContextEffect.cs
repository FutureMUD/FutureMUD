...
public class RecentSpeechContextEffect : Effect, IRecentSpeechContextEffect
{
    ...
    public RecentSpeechContextEffect(XElement effect, IPerceivable owner)
        : base(effect, owner)
    {
        XElement? root = effect.Element("Effect")?.Element("Events");
        if (root is null)
        {
            return;
        }

        foreach (XElement element in root.Elements("Event"))
        {
            if (!TryParseEvent(element, out RecentSpeechContextEvent? parsed))
            {
                continue;
            }
    ...
    {
        DateTime timestampUtc = EnsureUtc(realTimeTimestampUtc);
        string speakerName = speaker.PersonalName.GetName(NameStyle.FullName)
            .IfNullOrWhiteSpace(speaker.HowSeen(null, colour: false, flags: PerceiveIgnoreFlags.TrueDescription));
        string? targetDescription = target switch
        {
            null => null,
        ...
        int maximumEvents, TimeSpan maximumSeparation)
    {
        ...
        DateTime latestUtc = EnsureUtc(latestTimestampUtc);
        DateTime earliestUtc = latestUtc - maximumSeparation;
        lock (_eventsLock)
        {
            PruneStaleEvents(latestUtc);
            List<RecentSpeechContextEvent> results = _events
    ...
    private static bool TryParseEvent(XElement element, out RecentSpeechContextEvent parsed)
    {
        ...
        if (!long.TryParse(element.Element("SpeakerId")?.Value, out long speakerId))
        {
            return false;
        }

        AudioVolume volume = AudioVolume.Decent;
        if (int.TryParse(element.Element("Volume")?.Value, out int volumeValue) &&
        ...
        long? targetId = null;
        if (long.TryParse(element.Element("TargetId")?.Value, out long parsedTargetId))
        {
    ...
    {
        DateTime earliest = referenceUtc - MaximumStoredAge;
        int removed = _events.RemoveAll(x => x.RealTimeTimestampUtc < earliest);
        if (removed > 0)
        {
            Changed = true;
        }
