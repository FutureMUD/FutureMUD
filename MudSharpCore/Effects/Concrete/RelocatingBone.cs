...
public class RelocatingBone : CharacterActionWithTarget, IAffectProximity
{
    ...
    {
        BoneFracture wound = TargetCharacter.Wounds.Where(x => x.Bodypart == Bone).OfType<BoneFracture>()
        ...
        Difficulty difficulty = wound.CanBeTreated(TreatmentType.Relocation);
        ...
        ICheck check = Gameworld.GetCheck(RPG.Checks.CheckType.RelocateBoneCheck);
        CheckOutcome result = check.Check(CharacterOwner, difficulty, Target);
