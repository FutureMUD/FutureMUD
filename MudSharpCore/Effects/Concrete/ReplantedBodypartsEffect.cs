...
public class ReplantedBodypartsEffect : Effect, IReplantedBodypartsEffect
{
    ...
    public override void ExpireEffect()
    {
        if (ChecksAtCurrentDifficulty++ >= 20)
        {
            ...
            {
                base.ExpireEffect();
                IBody bOwner = Owner as IBody;
                ILimb limb = bOwner.GetLimbFor(Bodypart);
    ...
    private void CheckInfections()
    {
        ...
        ICheck check = Gameworld.GetCheck(CheckType.ReplantedBodypartRejectionCheck);
        CheckOutcome result = check.Check(bodyOwner.Actor, ResistRejectionDifficulty, default(IPerceivable), null,
            Owner.EffectsOfType<IImmuneBonusEffect>().Where(x => x.Applies()).Select(x => x.ImmuneBonus).Sum());
        if (result.Outcome == Outcome.MajorFail)
        {
            List<IBodypart> eligableParts =
                bodyOwner.Bodyparts.Where(x => x.DownstreamOfPart(Bodypart) || x == Bodypart).ToList();
            List<IOrganProto> eligableOrgans =
            ...
            Construction.ITerrain terrain = bodyOwner.Location.Terrain(bodyOwner.Actor);
