...
public class Searching : CharacterAction
{
    ...
    private void SearchAction(IPerceivable dummy)
    {
        ...
        foreach (ICharacter tch in hiders)
        {
            if (!_searchHitsDictionary.ContainsKey(tch))
            {
                double skill = tch.EffectsOfType<IHideEffect>().Select(x => x.EffectiveHideSkill).Max();
        ...
        foreach (GameItems.IGameItem item in hiddenItems)
        {
            if (!_searchHitsDictionary.ContainsKey(item))
            {
                double skill = item.EffectsOfType<IItemHiddenEffect>().Select(x => x.EffectiveHideSkill).Max();
