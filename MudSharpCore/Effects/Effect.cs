...
public abstract partial class Effect : FrameworkItem, IEffect
{
    ...
    {
        bool scheduled = scheduledEffects.ContainsKey(this);
    ...
    {
        foreach (Type type in Futuremud.GetAllTypes().Where(x => x.IsSubclassOf(typeof(Effect))))
        {
            MethodInfo method = type.GetMethod("InitialiseEffectType", BindingFlags.Public | BindingFlags.Static);
