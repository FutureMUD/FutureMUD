...
public class SimpleHearingProfile : HearingProfile
{
    ...
    private bool BuildingCommandDefault(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<Difficulty>(out Difficulty diff))
        {
    ...
    private bool BuildingCommandDifficulty(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<AudioVolume>(out AudioVolume volume))
        {
            actor.OutputHandler.Send("That is not a valid volume.");
            return false;
        }

        if (!command.PopSpeech().TryParseEnum<Proximity>(out Proximity prox))
        {
    ...
    public override void Initialise(MudSharp.Models.HearingProfile profile, IFuturemud game)
    {
        ...
        XElement element = root.Element("DefaultDifficulty");
        if (element != null)
        {
        ...
        if (element != null)
        {
            foreach (XElement sub in element.Elements("Difficulty"))
            {
                _difficultyMap.Add(
