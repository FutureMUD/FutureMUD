...
public class CharacteristicDefinitionRemovalProposal : Proposal, IProposal
{
    ...
    public override void Accept(string message = "")
    {
        using (new FMDB())
        {
            ...
            Models.CharacteristicDefinition dbitem = FMDB.Context.CharacteristicDefinitions.Find(_definition.Id);
            if (dbitem != null)
            {
        ...
        foreach (
            ICharacteristicValue value in _proponent.Gameworld.CharacteristicValues.Where(x => x.Definition == _definition).ToList())
        {
            _proponent.Gameworld.Destroy(value);
        }

        foreach (
            ICharacteristicProfile value in
            _proponent.Gameworld.CharacteristicProfiles.Where(x => x.TargetDefinition == _definition).ToList())
        {
            _proponent.Gameworld.Destroy(value);
        }

        foreach (IBody ch in _proponent.Gameworld.Bodies.Where(x => x.CharacteristicDefinitions.Contains(_definition))
                )
        {
            ch.ExpireDefinition(_definition);
        }

        foreach (VariableGameItemComponentProto comp in _proponent.Gameworld.ItemComponentProtos.OfType<VariableGameItemComponentProto>())
        {
            comp.ExpireDefinition(_definition);
        }

        foreach (IVariable item in _proponent.Gameworld.Items.SelectNotNull(x => x.GetItemType<IVariable>()))
        {
