...
public class CharacteristicProfile : FrameworkItem, ICharacteristicProfile
{
    ...
    public CharacteristicProfile(string name, ICharacteristicDefinition target, string type, string definition = null)
    {
        using (new FMDB())
        {
            Models.CharacteristicProfile dbitem = new()
            {
                Name = name,
    ...
    protected virtual void LoadFromXml(XElement root)
    {
        ...
        {
            ICharacteristicValue item = long.TryParse(value.Value, out long val)
    ...
    private void BuildingCommandValue(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicValue value = Gameworld.CharacteristicValues.GetByIdOrName(command.SafeRemainingArgument);
        if (value == null)
        {
    ...
    public virtual string Show(ICharacter actor)
    {
        ...
        foreach (ICharacteristicValue value in Values)
        {
    ...
    public void Save()
    {
        using (new FMDB())
        {
            Models.CharacteristicProfile dbprofile = FMDB.Context.CharacteristicProfiles.Find(Id);
            dbprofile.Name = Name;
