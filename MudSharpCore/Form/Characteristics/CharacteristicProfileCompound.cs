...
public class CharacteristicProfileCompound : CharacteristicProfile
{
    ...
    public override IEnumerable<ICharacteristicValue> Values => Profiles.SelectMany(x => x.Values).Distinct().WhereNotNull(x => x).ToList();
    ...
    private void BuildingCommandProfile(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicProfile profile = Gameworld.CharacteristicProfiles.GetByIdOrName(command.SafeRemainingArgument);
        if (profile == null)
        {
    ...
    {
        foreach (XElement item in root.Elements("Profile"))
        {
            if (long.TryParse(item.Value, out long value))
            {
