...
public class CharacteristicValueRemovalProposal : Proposal, IProposal
{
    ...
    public override void Accept(string message = "")
    {
        using (new FMDB())
        {
            ...
            Models.CharacteristicValue dbitem = FMDB.Context.CharacteristicValues.Find(_value.Id);
            if (dbitem != null)
            {
        ...
        foreach (
            ICharacteristicProfile value in _proponent.Gameworld.CharacteristicProfiles.Where(x => x.ContainsCharacteristic(_value)))
        {
