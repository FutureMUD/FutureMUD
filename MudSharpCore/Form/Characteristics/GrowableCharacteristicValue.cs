...
public class GrowableCharacteristicValue : CharacteristicValue, IGrowableCharacteristicValue
{
    ...
    {
        Basic = value.Value;
        string[] split = value.AdditionalValue.Split(new char[] { ' ' }, 4);
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.CharacteristicValue dbvalue = FMDB.Context.CharacteristicValues.Find(Id);
...
Note: When using the STYLE command people can apply any style with an equal or lesser growth number essentially at will, whereas there is a timer after each style before they can apply a longer style and it must always be a style 1 larger than the current length.";
    ...
    public override void BuildingCommand(ICharacter actor, StringStack command)
    {
        ...
        {
            case "basic":
                string basic = command.SafeRemainingArgument;
                ...
                return;
            case "fancy":
                string fancy = command.SafeRemainingArgument;
                ...
                return;
            case "tag":
                string id = command.SafeRemainingArgument;
            ...
            case "growth stage":
            case "stage":
                string stage = command.SafeRemainingArgument;
                ...
                if (!int.TryParse(stage, out int value))
                {
                ...
                return;
            case "difficulty":
                string difftext = command.SafeRemainingArgument;
