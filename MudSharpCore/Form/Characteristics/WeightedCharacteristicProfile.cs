...
internal class WeightedCharacteristicProfile : CharacteristicProfile
{
    ...
    protected void EnsureValuesLoaded()
    {
        ...
        foreach ((long id, double weight) in _idsAndWeights)
        {
    ...
    public override string Show(ICharacter actor)
    {
        ...
        EnsureValuesLoaded();
        double sum = _valuesAndWeights.Sum(x => x.Weight);
        foreach ((ICharacteristicValue value, double weight) in _valuesAndWeights.OrderByDescending(x => x.Weight))
        {
            sb.AppendLine(
    ...
    protected void BuildingCommandValue(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicValue value = Gameworld.CharacteristicValues.GetByIdOrName(command.PopSpeech());
        if (value == null)
        {
        ...
        if (!double.TryParse(command.SafeRemainingArgument, out double weight))
        {
            actor.OutputHandler.Send(
        ...
        if (_valuesAndWeights.Any(x => x.Value == value))
        {
            int index = _valuesAndWeights.FindIndex(x => x.Value == value);
