...
public class Colour : SaveableItem, IColour
{
    ...
    {
        Models.Colour dbitem = FMDB.Context.Colours.Find(Id);
        dbitem.Name = Name;
        dbitem.Red = Red;
        dbitem.Green = Green;
        dbitem.Blue = Blue;
    ...
    private bool BuildingCommandBasic(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<BasicColour>(out BasicColour value))
        {
    ...
    private bool BuildingCommandRed(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int value) || value < 0 || value > 255)
        {
