...
public class Liquid : Fluid, ILiquid
{
    ...
    private bool BuildingCommandTaste(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double intensity) || intensity < 0)
        {
        ...
        string taste = command.PopSpeech().ProperSentences();
        string vague = taste;
        if (!command.IsFinished)
        {
    ...
    {
        Models.Liquid dbitem = FMDB.Context.Liquids.Find(Id);
