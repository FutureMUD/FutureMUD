...
public class BodypartShape : SaveableItem, IBodypartShape
{
    public BodypartShape(IFuturemud gameworld, string name)
    {
        ...
        using (new FMDB())
        {
            Models.BodypartShape dbitem = new()
            {
                Name = name
            };
    ...
    {
        Models.BodypartShape dbitem = FMDB.Context.BodypartShapes.Find(Id);
        dbitem.Name = Name;
        Changed = false;
    }
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("Usage:");
        foreach (IBodypart bodypart in Gameworld.BodypartPrototypes)
        {
