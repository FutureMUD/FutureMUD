...
public class EntityDescriptionPattern : SaveableItem, IEntityDescriptionPattern
{
    private void DoDatabaseInsert()
    {
        using (new FMDB())
        {
            Models.EntityDescriptionPattern dbitem = new()
            {
                Type = (int)Type,
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.EntityDescriptionPattern dbitem = FMDB.Context.EntityDescriptionPatterns.Find(Id);
    ...
    private bool BuildingCommandProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Boolean, [ProgVariableTypes.Toon]).LookupProg();
        if (prog is null)
        {
            return false;
        }
