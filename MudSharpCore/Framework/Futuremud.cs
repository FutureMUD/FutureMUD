...
public sealed partial class Futuremud : IFuturemud, IDisposable
{
    ...
    {
        Assembly databaseAssembly = Assembly.GetAssembly(typeof(FuturemudDatabaseContext));
        Assembly libraryAssembly = Assembly.GetAssembly(typeof(IFuturemud));
    ...
    public MaintenanceModeSetting MaintenanceMode
    {
        ...
        set
        {
            ...
            using (new FMDB())
            {
                Models.StaticConfiguration dbitem = FMDB.Context.StaticConfigurations.Find("MaintenanceMode");
                if (dbitem == null)
                {
    ...
    {
        Type pType = typeof(IPerceivable);
        foreach (
            Type type in GetAllTypes().Where(x => x.GetInterfaces().Contains(pType)))
        {
            MethodInfo method = type.GetMethod("RegisterPerceivableType", BindingFlags.Static | BindingFlags.Public);
    ...
    {
        string nameLower = loginName.ToLowerInvariant();

        using (new FMDB())
        {
            Models.Account accountLookup = (from dbAccount in FMDB.Context.Accounts
            ...
            IAccount account = Accounts.Get(accountLookup.Id);
    ...
    public void StartGameLoop()
    {
        ...
        finally
        {
            using (new FMDB())
            {
                ...
                foreach (Models.Character dbchar in Characters.SelectNotNull(ch => FMDB.Context.Characters.Find(ch.Id)))
                {
    ...
    public IAccount TryAccount(long accountid)
    {
        ...
        using (new FMDB())
        {
            ...
            Account newAccount = new(dbaccount, this,
    ...
    {
        CharacterPersonalNameLookup result =
    ...
    private void CheckNewPlayerHints()
    {
        foreach (ICharacter character in Characters)
        {
            ...
            NewPlayerHintsShown effect = character.CombinedEffectsOfType<NewPlayerHintsShown>().FirstOrDefault();
            if (effect is null)
            {
            ...
            INewPlayerHint hint = NewPlayerHints
    ...
    public void PreloadAccounts()
    {
        ...
        using (new FMDB())
        {
            foreach (Models.Account dbaccount in FMDB.Context.Accounts.ToList())
            {
    ...
    public IEnumerable<ICharacter> LoadAllPlayerCharacters()
    {
        ...
        HashSet<long> onlinePCIDs = Characters.Select(x => x.Id).Concat(_cachedActors.Select(x => x.Id)).Distinct().ToHashSet();
        using (new FMDB())
        {
            List<long> PCsToLoad =
    ...
    public void PreloadCharacterNames()
    {
        ...
        using (new FMDB())
        {
            foreach (Models.Character dbcharacter in FMDB.Context.Characters
    ...
    {
        string realMessage = $"{"[System Message]".Colour(Telnet.Green)} {message}\n";
        foreach (ICharacter person in Actors.Where(filterFunc))
        {
    ...
    {
        string realMessage = $"{"[Debug Message]".Colour(Telnet.BoldCyan)} {message}\n";
        foreach (ICharacter person in Actors.Where(x => x.AffectedBy<DebugMode>()))
        {
            person.OutputHandler?.Send(realMessage, nopage: true);
        }
    ...
    {
        string systemMessage = "[System Message] ".Colour(Telnet.Green);
        foreach (ICharacter person in Actors.Where(x => filterFunc(x) && message.ShouldSee(x)))
        {
    ...
    internal void FinalisePostCharacterLoadObjects()
    {
        ...
        foreach (IPostCharacterLoadFinalisable item in _postCharacterLoadFinalisables.ToList())
        {
    ...
    public ICharacter TryGetCharacter(long id, bool useCachedValues = false)
    {
        ...
        using (new FMDB())
        {
            Models.Character dbchar = (from ch in FMDB.Context.Characters
                                       /*
            ...
            Models.Guest dbguest = dbchar.Guest;
    ...
    public IGameItem TryGetItem(long id, bool addToGameworld = false)
    {
        ...
        Models.GameItem dbitem = _bootTimeCachedGameItems.ContainsKey(id) ? _bootTimeCachedGameItems[id] : null;
        if (dbitem != null)
        {
    ...
    {
        foreach (ITemporalListener listener in _listeners.Where(x => x.PertainsTo(obj)).ToArray())
        {
    ...
    public ICharacterCommandTree RetrieveAppropriateCommandTree(ICharacter character)
    {
        ...
        {
            ICharacterCommandTree guestPermission = _actorCommandTrees.FirstOrDefault(x => x.PermissionLevel == PermissionLevel.Guest);
        ...
        foreach (ICharacterCommandTree tree in _actorCommandTrees.Where(x => x.PermissionLevel <= character.PermissionLevel))
        {
            if (biggestTree == null)
            {
                biggestTree = tree;
