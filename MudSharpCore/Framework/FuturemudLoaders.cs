...
public sealed partial class Futuremud : IFuturemudLoader, IFuturemud, IDisposable
{
    ...
    void IFuturemudLoader.LoadFromDatabase()
    {
        ...
        IFuturemudLoader game = (IFuturemudLoader)this;

        ConsoleUtilities.WriteLine("\n#EEnsuring that Database migrations are applied...#0");
        DatabaseUpgradeCoordinator upgradeCoordinator = new();
        ...
        {
#if DEBUG
            StreamWriter sqlwriter = new("SqlDebugLog.txt");
#endif
            ...
            StaticConfiguration maintenanceSetting = FMDB.Context.StaticConfigurations.Find("MaintenanceMode");
            ...
            Type pType = typeof(ICombat);
            foreach (
                Type type in GetAllTypes().Where(x => x.GetInterfaces().Contains(pType))
            )
            {
                MethodInfo method = type.GetMethod("SetupCombat", BindingFlags.Static | BindingFlags.Public);
        ...
        Scheduler.AddSchedule(new RepeatingSchedule<IFuturemud>(this, this, fm =>
            {
                foreach (IActiveProject project in ActiveProjects.ToList())
                {
        ...
        Scheduler.AddSchedule(new RepeatingSchedule<IFuturemud>(this, this, fm =>
        {
            foreach (INPCSpawner spawner in fm.NPCSpawners)
            {
                spawner.CheckSpawn();
            }
...
For information on the syntax to use in emotes (such as those included in brackets after your communication command), see #3help emote#0.".SubstituteANSIColour(),
    ...
    void IFuturemudLoader.LoadNewPlayerHints()
    {
        ...
        List<Models.NewPlayerHint> hints = FMDB.Context.NewPlayerHints
            .AsNoTracking()
            .ToList();
        foreach (Models.NewPlayerHint hint in hints)
        {
...
#endif
        int count = _newPlayerHints.Count;
    ...
    void IFuturemudLoader.LoadAIStorytellers()
    {
        ...
        List<Models.AIStorytellerReferenceDocument> referenceDocuments = FMDB.Context.AIStorytellerReferenceDocuments
            .AsNoTracking()
            .ToList();
        foreach (Models.AIStorytellerReferenceDocument item in referenceDocuments)
        {
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int referenceCount = _aiStorytellerReferenceDocuments.Count;
        ...
        List<Models.AIStoryteller> storytellers = FMDB.Context.AIStorytellers
            .AsNoTracking()
            .ToList();
        foreach (Models.AIStoryteller item in storytellers)
        {
            _aiStorytellers.Add(new MudSharp.RPG.AIStorytellers.AIStoryteller(item, this));
        }

        List<Models.AIStorytellerSituation> situations = FMDB.Context.AIStorytellerSituations
            .AsNoTracking()
            .ToList();
        foreach (Models.AIStorytellerSituation item in situations)
        {
            RPG.AIStorytellers.AIStoryteller storyteller = _aiStorytellers.Get(item.AIStorytellerId) as MudSharp.RPG.AIStorytellers.AIStoryteller;
            if (storyteller is null)
            {
                continue;
            }

            RPG.AIStorytellers.AIStorytellerSituation situation = new(item, storyteller);
            storyteller.RegisterLoadedSituation(situation);
        }

        List<Models.AIStorytellerCharacterMemory> memories = FMDB.Context.AIStorytellerCharacterMemories
            .AsNoTracking()
            .ToList();
        foreach (Models.AIStorytellerCharacterMemory item in memories)
        {
            RPG.AIStorytellers.AIStoryteller storyteller = _aiStorytellers.Get(item.AIStorytellerId) as MudSharp.RPG.AIStorytellers.AIStoryteller;
            if (storyteller is null)
            {
            ...
            RPG.AIStorytellers.AIStorytellerCharacterMemory memory = new(item, storyteller);
        ...
        foreach (RPG.AIStorytellers.AIStoryteller storyteller in _aiStorytellers.OfType<MudSharp.RPG.AIStorytellers.AIStoryteller>())
        {
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int storytellerCount = _aiStorytellers.Count;
    ...
    void IFuturemudLoader.LoadTracks()
    {
        ...
        sw.Start();
#endif
        List<Models.Track> tracks = FMDB.Context.Tracks
                              .AsNoTracking()
                              .ToList();
        foreach (Models.Track item in tracks)
        {
    ...
    void IFuturemudLoader.LoadMarkets()
    {
        ...
        List<MarketCategory> categories = FMDB.Context.MarketCategories
                        .AsNoTracking()
                        .ToList();
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = _marketCategories.Count;
        ...
        sw.Restart();
#endif
        List<MarketInfluenceTemplate> templates = FMDB.Context.MarketInfluenceTemplates.AsNoTracking().ToList();
        foreach (MarketInfluenceTemplate item in templates)
        {
        ...
        sw.Restart();
#endif
        List<Market> markets = FMDB.Context.Markets
        ...
        sw.Restart();
#endif
        List<MarketPopulation> populations = FMDB.Context.MarketPopulations
                              .AsNoTracking()
                              .ToList();
        ...
        sw.Restart();
#endif
        List<Shopper> shoppers = FMDB.Context.Shoppers
                              .AsNoTracking()
                              .ToList();
        foreach (Shopper item in shoppers)
        {
    ...
    void IFuturemudLoader.LoadLegal()
    {
        ...
        sw.Start();
#endif
        List<MudSharp.Models.LegalAuthority> authorities = FMDB.Context.LegalAuthorities
                              .Include(x => x.Laws)
                              ...
                              .AsNoTracking()
                              .ToList();
        foreach (Models.LegalAuthority item in authorities)
        {
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = _legalAuthorities.Count;
        ConsoleUtilities.WriteLine("Loaded #2{0:N0}#0 {1}.", count, count == 1 ? "Legal Authority" : "Legal Authorities");

#if DEBUG
        sw.Restart();
#endif
        List<MudSharp.Models.WitnessProfile> profiles = FMDB.Context.WitnessProfiles
                           ...
                           .ThenInclude(x => x.LegalClass)
                           .AsNoTracking().ToList();
        foreach (Models.WitnessProfile item in profiles)
        {
    ...
    void IFuturemudLoader.LoadGrids()
    {
        ...
        sw.Start();
#endif
        List<Grid> grids = FMDB.Context.Grids.AsNoTracking().ToList();
    ...
    void IFuturemudLoader.LoadEconomy()
    {
        ...
        sw.Start();
#endif
        List<MudSharp.Models.EconomicZone> zones = FMDB.Context.EconomicZones
                        ...
                        .AsNoTracking()
                        .ToList();
        foreach (Models.EconomicZone zone in zones)
        {
        ...
        sw.Restart();
#endif
        List<Bank> banks = FMDB.Context.Banks
        ...
        List<Arena> combatArenas = FMDB.Context.Arenas
                                     ...
                                     .AsNoTracking()
                                     .ToList();
        foreach (Arena arena in combatArenas)
        {
        ...
        sw.Restart();
#endif
        List<Property> properties = FMDB.Context.Properties
        ...
        sw.Restart();
#endif
        List<Models.AuctionHouse> auctionhouses = FMDB.Context.AuctionHouses
                                .AsNoTracking()
                                .ToList();
        foreach (Models.AuctionHouse auctionhouse in auctionhouses)
        {
        ...
        sw.Restart();
#endif
        List<Shop> shops = FMDB.Context.Shops
        ...
        sw.Restart();
#endif
        List<Estate> estates = FMDB.Context.Estates
    ...
    void IFuturemudLoader.LoadClimate()
    {
        ...
        sw.Start();
#endif
        List<Season> seasons = FMDB.Context.Seasons.AsNoTracking().ToList();
        ...
        sw.Start();
#endif
        List<WeatherEvent> weatherevents = FMDB.Context.WeatherEvents.AsNoTracking().ToList();
        ...
        sw.Start();
#endif
        List<ClimateModel> models = FMDB.Context.ClimateModels
            .Include(x => x.ClimateModelSeasons)
            .ThenInclude(x => x.SeasonEvents)
            .AsSplitQuery()
            .AsNoTracking()
            .ToList();
        foreach (ClimateModel model in models)
        {
        ...
        sw.Start();
#endif
        List<RegionalClimate> regionals = FMDB.Context.RegionalClimates
                            .Include(x => x.RegionalClimatesSeasons)
                            .AsNoTracking().ToList();
        foreach (RegionalClimate regional in regionals)
        {
        ...
        sw.Start();
#endif
        List<WeatherController> controllers = FMDB.Context.WeatherControllers
                              .AsNoTracking().ToList();
        foreach (WeatherController controller in controllers)
        {
    ...
    void IFuturemudLoader.LoadMagic()
    {
        ...
        sw.Start();
#endif
        List<MagicSchool> schools = FMDB.Context.MagicSchools.AsNoTracking().ToList();
        foreach (MagicSchool school in schools)
        {
        ...
        sw.Start();
#endif
        List<MagicResource> resources = FMDB.Context.MagicResources.AsNoTracking().ToList();
        foreach (MagicResource resource in resources)
        {
        ...
        sw.Start();
#endif
        List<MagicGenerator> generators = FMDB.Context.MagicGenerators.AsNoTracking().ToList();
        foreach (MagicGenerator generator in generators)
        {
        ...
        sw.Start();
#endif
        List<MagicPower> powers = FMDB.Context.MagicPowers.AsNoTracking().ToList();
        ...
        sw.Start();
#endif
        List<MagicSpell> spells = FMDB.Context.MagicSpells.AsNoTracking().ToList();
        ...
        sw.Start();
#endif
        List<MagicCapability> capabilities =
            FMDB.Context.MagicCapabilities
                .AsNoTracking().ToList();
        foreach (MagicCapability capability in capabilities)
        {
    ...
    void IFuturemudLoader.LoadChargenAdvices()
    {
        ...
        sw.Start();
#endif
        List<Models.ChargenAdvice> advices = FMDB.Context.ChargenAdvices.AsNoTracking().ToList();
        foreach (Models.ChargenAdvice item in advices)
        {
    ...
    void IFuturemudLoader.LoadCharacterIntroTemplates()
    {
        ...
        sw.Start();
#endif
        List<Models.CharacterIntroTemplate> templates = FMDB.Context.CharacterIntroTemplates
                            .AsNoTracking().ToList();
        foreach (Models.CharacterIntroTemplate item in templates)
        {
    ...
    void IFuturemudLoader.LoadBloodtypes()
    {
        ...
        sw.Start();
#endif
        List<BloodtypeAntigen> antigens = FMDB.Context.BloodtypeAntigens.AsNoTracking().ToList();
        foreach (BloodtypeAntigen item in antigens)
        {
        ...
        sw.Restart();
#endif
        List<BloodModel> bloodModels = FMDB.Context.BloodModels
                              .Include(x => x.BloodModelsBloodtypes)
                              .AsNoTracking().ToList();
        foreach (BloodModel item in bloodModels)
        {
        ...
        sw.Restart();
#endif
        List<PopulationBloodModel> popBloodModels = FMDB.Context.PopulationBloodModels
                                 .Include(x => x.PopulationBloodModelsBloodtypes)
                                 .AsNoTracking().ToList();
        foreach (PopulationBloodModel item in popBloodModels)
        {
    ...
    void IFuturemudLoader.LoadAutobuilderTemplates()
    {
        ...
        List<AutobuilderRoomTemplate> roomTemplates = FMDB.Context.AutobuilderRoomTemplates.AsNoTracking().ToList();
        foreach (AutobuilderRoomTemplate item in roomTemplates)
        {
            _autobuilderRooms.Add(Construction.Autobuilder.AutobuilderFactory.LoadRoom(item, this));
        }

        int count = _autobuilderRooms.Count;
        ConsoleUtilities.WriteLine("Loaded #2{0:N0}#0 {1}.", count, count == 1 ? "Room Template" : "Room Templates");

        ConsoleUtilities.WriteLine("\nLoading #5Area Templates...#0");
        List<AutobuilderAreaTemplate> areaTemplates = FMDB.Context.AutobuilderAreaTemplates.AsNoTracking().ToList();
        foreach (AutobuilderAreaTemplate item in areaTemplates)
        {
        ...
        if (_autobuilderAreas.Count < 6)
        {
            ...
            {
                AutobuilderAreaTemplate dbitem = new()
                {
                    Name = "Rectangle",
    ...
    void IFuturemudLoader.LoadButchering()
    {
        ...
        sw.Start();
#endif
        List<ButcheryProducts> products = FMDB.Context.ButcheryProducts
                           .Include(x => x.ButcheryProductsBodypartProtos)
                           .Include(x => x.ButcheryProductItems)
                           .AsNoTracking()
                           .ToList();
        foreach (ButcheryProducts product in products)
        {
        ...
        sw.Restart();
#endif
        List<Models.RaceButcheryProfile> profiles = FMDB.Context.RaceButcheryProfiles
                           ...
                           .AsNoTracking()
                           .ToList();
        foreach (Models.RaceButcheryProfile profile in profiles)
        {
    ...
    void IFuturemudLoader.LoadTraitExpressions()
    {
        ...
        sw.Start();
#endif
        List<Models.TraitExpression> expressions = FMDB.Context.TraitExpressions
                              .Include(x => x.TraitExpressionParameters)
                              .AsNoTracking().ToList();
        foreach (Models.TraitExpression item in expressions)
        {
    ...
    void IFuturemudLoader.LoadCrafts()
    {
        ...
        sw.Start();
#endif
        List<Models.Craft> crafts = FMDB.Context.Crafts
                         ...
                         .AsNoTracking()
                         .ToList();
        foreach (Models.Craft item in crafts)
        {
    ...
    void IFuturemudLoader.LoadScripts()
    {
        ...
        sw.Start();
#endif
        List<Models.Script> scripts = FMDB.Context.Scripts
                          .Include(x => x.ScriptsDesignedLanguages)
                          .AsNoTracking().ToList();
        foreach (Models.Script item in scripts)
        {
    ...
    void IFuturemudLoader.LoadDrawings()
    {
        ...
        sw.Start();
#endif
        List<Drawing> drawings = FMDB.Context.Drawings.AsNoTracking().ToList();
    ...
    void IFuturemudLoader.LoadWritings()
    {
        ...
        sw.Start();
#endif
        List<Writing> writings = FMDB.Context.Writings.AsNoTracking().ToList();
        foreach (Writing item in writings)
        {
    ...
    void IFuturemudLoader.LoadCover()
    {
        ...
        sw.Start();
#endif
        List<Models.RangedCover> covers = FMDB.Context.RangedCovers.AsNoTracking().ToList();
        foreach (Models.RangedCover cover in covers)
        {
    ...
    void IFuturemudLoader.LoadSurgery()
    {
        ...
        sw.Start();
#endif
        List<Models.SurgicalProcedure> surgeries = FMDB.Context.SurgicalProcedures
                            .Include(x => x.SurgicalProcedurePhases)
                            .AsNoTracking().ToList();
        foreach (Models.SurgicalProcedure item in surgeries)
        {
    ...
    void IFuturemudLoader.LoadKnowledges()
    {
        ...
        sw.Start();
#endif
        List<Models.Knowledge> knowledges = FMDB.Context.Knowledges.Include(x => x.KnowledgesCosts).AsNoTracking().ToList();
        foreach (Models.Knowledge item in knowledges)
        {
    ...
    void IFuturemudLoader.LoadDrugs()
    {
        ...
        sw.Start();
#endif
        List<Models.Drug> drugs = FMDB.Context.Drugs
    ...
    void IFuturemudLoader.LoadCharacterCombatSettings()
    {
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = combatSettings.Count;
        ConsoleUtilities.WriteLine("Loaded #2{0:N0}#0 {1}.", count,
    ...
    void IFuturemudLoader.LoadArmourTypes()
    {
        ...
        sw.Start();
#endif
        List<Models.ArmourType> armourTypes = (from type in FMDB.Context.ArmourTypes.AsNoTracking() select type).ToList();
        foreach (Models.ArmourType type in armourTypes)
        {
    ...
    void IFuturemudLoader.LoadWeaponTypes()
    {
        ...
        sw.Start();
#endif
        List<Models.WeaponAttack> attacks = (from type in FMDB.Context.WeaponAttacks
                                        .AsNoTracking()
                                             select type).ToList();
        foreach (Models.WeaponAttack type in attacks)
        {
        ...
        List<Models.CombatAction> actions = (from action in FMDB.Context.CombatActions.AsNoTracking() select action).ToList();
        foreach (Models.CombatAction action in actions)
        {
        ...
        sw.Restart();
#endif
        List<Models.WeaponType> weaponTypes = (from type in FMDB.Context.WeaponTypes
                                            .Include(x => x.WeaponAttacks)
                                            .AsNoTracking()
                                               select type).ToList();
        foreach (Models.WeaponType type in weaponTypes)
        {
        ...
        sw.Restart();
#endif
        List<RangedWeaponTypes> rangedTypes = (from type in FMDB.Context.RangedWeaponTypes.AsNoTracking() select type).ToList();
        foreach (RangedWeaponTypes type in rangedTypes)
        {
        ...
        sw.Restart();
#endif
        List<AmmunitionTypes> ammoTypes = (from type in FMDB.Context.AmmunitionTypes.AsNoTracking() select type).ToList();
        foreach (AmmunitionTypes type in ammoTypes)
        {
    ...
    void IFuturemudLoader.LoadShieldTypes()
    {
        ...
        sw.Start();
#endif
        List<Models.ShieldType> shieldTypes = (from type in FMDB.Context.ShieldTypes.AsNoTracking() select type).ToList();
        foreach (Models.ShieldType type in shieldTypes)
        {
    ...
    void IFuturemudLoader.LoadDreams()
    {
        ...
        sw.Start();
#endif
        List<Models.Dream> dreams = (from dream in FMDB.Context.Dreams
                                        .Include(x => x.DreamPhases)
                                        .Include(x => x.DreamsAlreadyDreamt)
                                        .Include(x => x.DreamsCharacters)
                                        .AsNoTracking()
                                     select dream).ToList();
        foreach (Models.Dream dream in dreams)
        {
    ...
    void IFuturemudLoader.LoadMerits()
    {
        ...
        List<Merit> merits = (from merit in FMDB.Context.Merits
    ...
    void IFuturemudLoader.LoadHealthStrategies()
    {
        ...
        List<HealthStrategy> strategies = (from strategy in FMDB.Context.HealthStrategies
                                               .AsNoTracking()
                                           select strategy).ToList();
        foreach (HealthStrategy strategy in strategies)
        {
    ...
    void IFuturemudLoader.LoadForagables()
    {
        ...
        sw.Start();
#endif
        List<Models.Foragable> foragables = (from foragable in FMDB.Context.Foragables
                                                .Include(x => x.EditableItem)
                                                .AsNoTracking()
                                             select foragable).ToList();
        foreach (Models.Foragable foragable in foragables)
        {
        ...
        sw.Restart();
#endif
        List<Models.ForagableProfile> foragableProfiles =
        ...
        foreach (Models.ForagableProfile profile in foragableProfiles)
        {
    ...
    void IFuturemudLoader.LoadCorpseModels()
    {
        ...
        List<Models.CorpseModel> models = (from model in FMDB.Context.CorpseModels.AsNoTracking() select model).ToList();
        foreach (Models.CorpseModel model in models)
        {
    ...
    void IFuturemudLoader.LoadGameItemGroups()
    {
        ...
        sw.Start();
#endif
        List<ItemGroup> groups = (from item in FMDB.Context.ItemGroups.Include(x => x.ItemGroupForms).AsNoTracking()
                                  select item).ToList();

        foreach (ItemGroup item in groups)
        {
    ...
    void IFuturemudLoader.LoadSkyDescriptionTemplates()
    {
        ...
        sw.Start();
#endif
        List<Models.SkyDescriptionTemplate> templates = (from item in FMDB.Context.SkyDescriptionTemplates
                                          .Include(x => x.SkyDescriptionTemplatesValues)
                                          .AsNoTracking()
                                                         select item).ToList();

        foreach (Models.SkyDescriptionTemplate item in templates)
        {
    ...
    void IFuturemudLoader.LoadRoles()
    {
        ...
        sw.Start();
#endif
        List<Models.ChargenRole> roles = (from item in FMDB.Context.ChargenRoles
        ...
        foreach (Models.ChargenRole item in roles)
        {
    ...
    void IFuturemudLoader.LoadMaterials()
    {
...
#endif
        ConsoleUtilities.WriteLine("\nLoading #5Solids#0...");
        List<Material> materials = (from item in FMDB.Context.Materials
        ...
        ConsoleUtilities.WriteLine("\nLoading #5Liquids#0...");
        List<Models.Liquid> liquids = (from item in FMDB.Context.Liquids
                                        .Include(x => x.LiquidsTags)
                                        .AsNoTracking()
                                       select item).ToList();

        foreach (Models.Liquid item in liquids)
        {
        ...
        ConsoleUtilities.WriteLine("\nLoading #5Gases#0...");
        List<Models.Gas> gases = (from item in FMDB.Context.Gases
                                      .Include(x => x.GasesTags)
                                      .AsNoTracking()
                                  select item).ToList();

        foreach (Models.Gas item in gases)
        {
    ...
    void IFuturemudLoader.LoadSocials()
    {
        ...
        sw.Start();
#endif
        List<Models.Social> socials = (from item in FMDB.Context.Socials.AsNoTracking()
                                       select item).ToList();

        foreach (Models.Social item in socials)
        {
            _socials.Add(new Social(item, this));
        }

        foreach (ICharacterCommandTree tree in _actorCommandTrees)
        {
    ...
    void IFuturemudLoader.LoadChargenResources()
    {
        ...
        sw.Start();
#endif
        List<ChargenResource> resources = (from item in FMDB.Context.ChargenResources.AsNoTracking()
                                           select item).ToList();

        Dictionary<IChargenResource, ChargenResource> stagingTable = new();
        foreach (ChargenResource item in resources)
        {
            IChargenResource newItem = ChargenResourceBase.LoadFromDatabase(this, item);
            stagingTable.Add(newItem, item);
            _chargenResources.Add(newItem);
        }

        foreach (KeyValuePair<IChargenResource, ChargenResource> item in stagingTable)
        {
    ...
    void IFuturemudLoader.LoadHeightWeightModels()
    {
        ...
        sw.Start();
#endif
        List<Models.HeightWeightModel> models = (from item in FMDB.Context.HeightWeightModels.AsNoTracking()
                                                 select item).ToList();
        foreach (Models.HeightWeightModel item in models)
        {
    ...
    void IFuturemudLoader.LoadStaticValues()
    {
        ...
        List<StaticString> strings = (from item in FMDB.Context.StaticStrings.AsNoTracking()
                                      select item).ToList();

        _staticStrings.Clear();
        foreach (StaticString item in strings)
        {
        ...
        List<StaticConfiguration> settings = (from item in FMDB.Context.StaticConfigurations.AsNoTracking()
                                              select item).ToList();

        _staticConfigurations.Clear();
        foreach (StaticConfiguration item in settings)
        {
    ...
    void IFuturemudLoader.LoadChannels()
    {
        ...
        sw.Start();
#endif
        List<Models.Channel> channels = (from channel in FMDB.Context.Channels
    ...
    void IFuturemudLoader.LoadNonCardinalExitTemplates()
    {
        ...
        sw.Start();
#endif
        List<Models.NonCardinalExitTemplate> templates = (from template in FMDB.Context.NonCardinalExitTemplates.AsNoTracking()
                                                          select template).ToList();

        foreach (Models.NonCardinalExitTemplate template in templates)
        {
    ...
    void IFuturemudLoader.LoadNPCTemplates()
    {
        ...
        sw.Start();
#endif
        List<NpcTemplate> templates = (from template in FMDB.Context.NpcTemplates
    ...
    void IFuturemudLoader.LoadNPCs()
    {
        ...
        sw.Restart();
#endif
        List<Guest> guests = (from guest in FMDB.Context.Guests select guest).ToList();


        foreach (Guest guest in guests)
        {
            ICharacter newGuest = TryGetCharacter(guest.CharacterId);
        ...
        foreach (ILegalAuthority authority in _legalAuthorities)
        {
        ...
        List<Models.NPCSpawner> spawners = (from spawner in FMDB.Context.NpcSpawners select spawner).ToList();

        foreach (Models.NPCSpawner spawner in spawners)
        {
    ...
    void IFuturemudLoader.LoadGroupAIs()
    {
        ...
        sw.Start();
#endif
        List<GroupAiTemplate> templates = (from template in FMDB.Context.GroupAiTemplates.AsNoTracking() select template).ToList();

        foreach (GroupAiTemplate template in templates)
        {
        ...
        List<GroupAi> ais = FMDB.Context.GroupAis.AsNoTracking().ToList();
        foreach (GroupAi ai in ais)
        {
    ...
    void IFuturemudLoader.LoadAIs()
    {
        ...
        List<ArtificialIntelligence> ais = (from ai in FMDB.Context.ArtificialIntelligences.AsNoTracking()
                                            select ai).ToList();
    ...
    void IFuturemudLoader.LoadHooks()
    {
...
#endif
        HookBase.SetupHooks();
        List<Hooks> hooks = (from hook in FMDB.Context.Hooks.AsNoTracking()
                             select hook).ToList();
        ...
        List<Models.DefaultHook> defaultHooks = (from hook in FMDB.Context.DefaultHooks.AsNoTracking() select hook).ToList();
        foreach (Models.DefaultHook hook in defaultHooks)
        {
    ...
    void IFuturemudLoader.LoadCurrencies()
    {
        ...
        sw.Start();
#endif
        List<Models.Currency> currencies = (from currency in
    ...
    void IFuturemudLoader.LoadEntityDescriptionPatterns()
    {
        ...
        sw.Start();
#endif
        List<Models.EntityDescriptionPattern> patterns = (from pattern in FMDB.Context.EntityDescriptionPatterns.AsNoTracking()
                                                          select pattern).ToList();

        foreach (Models.EntityDescriptionPattern pattern in patterns)
        {
    ...
    void IFuturemudLoader.LoadHelpFiles()
    {
        ...
        sw.Start();
#endif
        List<Models.Helpfile> helpfiles = (from helpfile in FMDB.Context.Helpfiles
                                              .Include(x => x.HelpfilesExtraTexts)
                                              .AsNoTracking()
                                           select helpfile).ToList();

        foreach (Models.Helpfile helpfile in helpfiles)
        {
    ...
    void IFuturemudLoader.LoadFutureProgs()
    {
        ...
        sw.Start();
#endif
        List<Models.FutureProg> progs = (from prog in FMDB.Context.FutureProgs
                                      .Include(x => x.FutureProgsParameters)
                                      .AsNoTracking()
                                         select prog).ToList();

        foreach (Models.FutureProg prog in progs)
        {
    ...
    void IFuturemudLoader.LoadScriptedEvents()
    {
        ...
        sw.Start();
#endif
        List<ScriptedEvent> scripteds = (from scripted in FMDB.Context.ScriptedEvents
        ...
        foreach (ScriptedEvent scripted in scripteds)
        {
    ...
    void IFuturemudLoader.LoadCharacteristics()
    {
        ...
        CharacteristicDefinition relativeHeightDefinition =
            new(FMDB.Context.CharacteristicDefinitions.First(
                x => (CharacteristicType)x.Type == CharacteristicType.RelativeHeight), this);
        List<Models.CharacteristicValue> heightValues =
            FMDB.Context.CharacteristicValues.AsNoTracking().Where(
                x => x.DefinitionId == relativeHeightDefinition.Id).ToList();
        foreach (
            Models.CharacteristicValue item in
            heightValues.OrderBy(x => double.Parse(x.Value)))
        {
            CharacteristicValue newValue = new(item.Name, relativeHeightDefinition, item.Value,
        ...
        List<Models.CharacteristicDefinition> definitions =
            (from definition in
        ...
        foreach (Models.CharacteristicDefinition definition in definitions)
        {
        ...
        List<Models.CharacteristicValue> values = (from value in FMDB.Context.CharacteristicValues.AsNoTracking()
                                                   where value.DefinitionId != relativeHeightDefinition.Id
                                                   select value).ToList();
        foreach (Models.CharacteristicValue value in values)
        {
            ICharacteristicValue cvalue = CharacteristicValue.LoadValue(value, this);
            if (cvalue == null)
            {
        ...
        List<Models.CharacteristicProfile> profiles = (from profile in FMDB.Context.CharacteristicProfiles.AsNoTracking() select profile).ToList();
        foreach (Models.CharacteristicProfile profile in profiles)
        {
    ...
    void IFuturemudLoader.LoadUnits()
    {
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = UnitManager.Units.Count();
    ...
    void IFuturemudLoader.LoadStackDecorators()
    {
        ...
        sw.Start();
#endif
        List<Models.StackDecorator> decorators = (from decorator in FMDB.Context.StackDecorators.AsNoTracking()
                                                  select decorator).ToList();

        foreach (Models.StackDecorator decorator in decorators)
        {
        ...
        int count = _stackDecorators.Count;
    ...
    void IFuturemudLoader.LoadHearingProfiles()
    {
        ...
        sw.Start();
#endif
        List<HearingProfile> profiles = (from profile in FMDB.Context.HearingProfiles.AsNoTracking()
                                         select profile).ToList();
        ...
        foreach (KeyValuePair<long, Tuple<HearingProfile, Form.Audio.HearingProfiles.HearingProfile>> profile in stagingtable)
        {
    ...
    void IFuturemudLoader.LoadLanguageDifficultyModels()
    {
        ...
        sw.Start();
#endif
        List<LanguageDifficultyModels> models = (from model in FMDB.Context.LanguageDifficultyModels.AsNoTracking()
                                                 select model).ToList();
        foreach (LanguageDifficultyModels model in models)
        {
    ...
    void IFuturemudLoader.LoadLanguages()
    {
        ...
        sw.Start();
#endif
        List<Models.Language> languages = (from language in FMDB.Context.Languages
                                              .Include(x => x.Accents)
    ...
    void IFuturemudLoader.LoadAuthorityGroups()
    {
        ...
        sw.Start();
#endif
        List<AuthorityGroup> authorityGroups = (from authorityGroup in FMDB.Context.AuthorityGroups.AsNoTracking()
                                                select authorityGroup).ToList();
        foreach (AuthorityGroup authorityGroup in authorityGroups)
        {
            Authority newAuthority = new(authorityGroup);
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = authorityGroups.Count;
    ...
    void IFuturemudLoader.LoadTerrains()
    {
        ...
        sw.Start();
#endif
        List<Models.Terrain> terrains = (from terrain in FMDB.Context.Terrains.Include(x => x.TerrainsRangedCovers).AsNoTracking()
                                         select terrain).ToList();
        foreach (Models.Terrain terrain in terrains)
        {
    ...
    void IFuturemudLoader.LoadWorld()
    {
        ...
        sw.Start();
#endif
        List<Models.Shard> shards = (from plane in FMDB.Context.Shards
                                        ...
                                        .AsNoTracking()
                                     select plane).ToList();
        foreach (Models.Shard shard in shards)
        {
        ...
        sw.Restart();
#endif
        List<Models.Zone> zones = (from zone in FMDB.Context.Zones
                                      .Include(x => x.HooksPerceivables)
                                      .Include(x => x.ZonesTimezones)
                                      .AsNoTracking()
                                   select zone).ToList();
        foreach (Models.Zone zone in zones)
        {
    ...
    void IFuturemudLoader.LoadBodies()
    {
        ...
        sw.Start();
#endif
        List<BodypartGroupDescriber> rules = (from rule in FMDB.Context.BodypartGroupDescribers
        ...
        Dictionary<IBodypartGroupDescriber, BodypartGroupDescriber> bodypartStagingTable = new();
        foreach (BodypartGroupDescriber rule in rules)
        {
            IBodypartGroupDescriber newRule = Body.Grouping.BodypartGroupDescriber.LoadDescriber(rule, this);
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = _bodypartGroupDescriptionRules.Count;
        ConsoleUtilities.WriteLine("Loaded #2{0:N0}#0 {1}.", count,
        ...
        List<BodypartProto> bpprotos = FMDB.Context.BodypartProtos
                           ...
                           .AsSplitQuery()
                           .ToList();
        foreach (BodypartProto bpproto in bpprotos)
        {
            _bodypartPrototypes.Add(BodypartPrototype.LoadFromDatabase(bpproto, this));
        }

        // Setup Organ Info
        foreach (BodypartProto organ in bpprotos.Where(x => x.IsOrgan == 1).ToList())
        {
            IOrganProto iorgan = (IOrganProto)BodypartPrototypes.Get(organ.Id);
            foreach (BodypartInternalInfos info in organ.BodypartInternalInfosInternalPart)
            {
                IBodypart part = BodypartPrototypes.Get(info.BodypartProtoId);
        ...
        // Setup Bone Info
        foreach (BodypartProto bone in bpprotos.Where(x => ((BodypartTypeEnum)x.BodypartType).IsBone()).ToList())
        {
            IBone ibone = (IBone)BodypartPrototypes.Get(bone.Id);
            ibone.PostLoadProcessing(null, bone);
            foreach (BodypartInternalInfos info in bone.BodypartInternalInfosInternalPart)
            {
                IBodypart part = BodypartPrototypes.Get(info.BodypartProtoId);
                part.LinkBone(
                    ibone,
        ...
        // Link bodyparts
        foreach (BodypartProto part in bpprotos.Where(x => x.IsOrgan == 0 && !((BodypartTypeEnum)x.BodypartType).IsBone())
                                     .ToList())
        {
            IBodypart thisPart = BodypartPrototypes.Get(part.Id);
            if (part.BodypartProtoBodypartProtoUpstreamChildNavigation.Count <= 0)
            {
                continue;
            }

            IBodypart targetUpstream =
        ...
        sw.Restart();
#endif
        Dictionary<IBodyPrototype, BodyProto> bodyStaging = new();
        List<BodyProto> bodies = (from body in FMDB.Context.BodyProtos
                                       .Include(x => x.BodyProtosAdditionalBodyparts)
                                       .Include(x => x.BodypartGroupDescribersBodyProtos)
                     ...
                     .AsNoTracking()
                     .ToList();
        foreach (BodyProto dbbody in bodies)
        {
            BodyPrototype body = new(dbbody, this);
        ...
        foreach (BodypartProto part in bpprotos)
        {
        ...
        foreach (IBodyPrototype body in BodyPrototypes)
        {
        ...
        sw.Restart();
#endif
        List<Models.Limb> limbs = (from limb in FMDB.Context.Limbs
                                      .Include(x => x.LimbsSpinalParts)
                                      .Include(x => x.LimbsBodypartProto)
                                   select limb).ToList();
        foreach (Models.Limb limb in limbs)
        {
        ...
        foreach (KeyValuePair<IBodypartGroupDescriber, BodypartGroupDescriber> item in bodypartStagingTable)
        {
        ...
        foreach (IBodypart part in BodypartPrototypes)
        {
            ILimb partlimb = part.Body.Limbs.FirstOrDefault(x => x.Parts.Contains(part));
            if (partlimb == null)
            {
                List<ILimb> potentialLimbs = part.Body.Limbs.Where(x => part.DownstreamOfPart(x.RootBodypart)).ToList();
                if (potentialLimbs.Count > 1)
                {
                    ILimb bodyRootLimb = potentialLimbs.FirstOrDefault(x => x.RootBodypart.UpstreamConnection == null);
    ...
    void IFuturemudLoader.LoadBodypartShapes()
    {
        ...
        sw.Start();
#endif
        List<Models.BodypartShape> shapes = (from shape in FMDB.Context.BodypartShapes.AsNoTracking()
                                             select shape).ToList();

        foreach (Models.BodypartShape shape in shapes)
        {
        ...
        int count = _bodypartShapes.Count;
    ...
    void IFuturemudLoader.LoadClans()
    {
        ...
        List<Models.ClanMembership> memberships = (from membership in FMDB.Context.ClanMemberships
...
#endif
        Dictionary<IClan, Clan> staging = new();
        foreach (Clan clan in clans)
        {
            IClan newClan = Community.Clan.ClanFactory.LoadClan(clan, this);
            _clans.Add(newClan);
...
#endif
        foreach (KeyValuePair<IClan, Clan> clan in staging)
        {
...
#endif
        IQueryable<Models.ExternalClanControl> externals = from external
                            in FMDB.Context.ExternalClanControls
                                   .Include(x => x.ExternalClanControlsAppointments)
                                   .AsNoTracking()
                                                           select external;

        foreach (Models.ExternalClanControl external in externals)
        {
    ...
    void IFuturemudLoader.LoadColours()
    {
        ...
        sw.Start();
#endif
        List<Models.Colour> colours = FMDB.Context.Colours.AsNoTracking().ToList();
        foreach (Models.Colour colour in colours)
        {
    ...
    void IFuturemudLoader.LoadTags()
    {
        ...
        List<Models.Tag> tags = (from tag in FMDB.Context.Tags
                                 select tag).ToList();

        Dictionary<Models.Tag, ILoadingTag> staging = new();
        ...
        foreach (KeyValuePair<Models.Tag, ILoadingTag> tag in staging)
        {
    ...
    void IFuturemudLoader.LoadWearProfiles()
    {
        ...
        sw.Start();
#endif
        List<Models.WearProfile> profiles = (from profile in FMDB.Context.WearProfiles.AsNoTracking()
                                             select profile).ToList();
        foreach (Models.WearProfile profile in profiles)
        {
    ...
    void IFuturemudLoader.LoadGameItemComponentProtos()
    {
        ...
        sw.Start();
#endif
        List<Models.GameItemComponentProto> protos = (from proto in FMDB.Context.GameItemComponentProtos
                                        .Include(x => x.EditableItem)
                                        .AsNoTracking()
                                                      select proto).ToList();
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = _itemComponentProtos.Count();
        ConsoleUtilities.WriteLine("Loaded #2{0:N0}#0 {1}.", count,
    ...
    void IFuturemudLoader.LoadGameItemProtos()
    {
        ...
        sw.Start();
#endif
        List<Models.GameItemProto> protos = (from proto in FMDB.Context.GameItemProtos
                                        .Include(x => x.EditableItem)
                                        ...
                                        .AsNoTracking()
                                             select proto).ToList();
        foreach (Models.GameItemProto proto in protos)
        {
        ...
        {
            MethodInfo method = type.GetMethod("InitialiseItemType", BindingFlags.Public | BindingFlags.Static);
            method?.Invoke(null, new object[] { this });
        }
    ...
    void IFuturemudLoader.LoadGameItemSkins()
    {
        ...
        sw.Start();
#endif
        List<Models.GameItemSkin> skins = (from skin in FMDB.Context.GameItemSkins
                                      .Include(x => x.EditableItem)
                                      .AsNoTracking()
                                           select skin).ToList();
        foreach (Models.GameItemSkin skin in skins)
        {
    ...
    void IFuturemudLoader.LoadImprovementModels()
    {
        ...
        sw.Start();
#endif
        List<Improver> models = (from model in FMDB.Context.Improvers.AsNoTracking() select model).ToList();
        foreach (Improver model in models)
        {
        ...
        ConsoleUtilities.WriteLine($"Duration: #2{sw.ElapsedMilliseconds}ms#0");
#endif
        int count = _improvementModels.Count;
    ...
    void IFuturemudLoader.LoadTraits()
    {
        ...
        sw.Start();
#endif
        List<Models.TraitDefinition> traits = (from trait in FMDB.Context.TraitDefinitions
                                        .Include(x => x.TraitDefinitionsChargenResources)
                                        .AsNoTracking()
                                               select trait).ToList();
        foreach (Models.TraitDefinition trait in traits)
        {
    ...
    void IFuturemudLoader.LoadTraitDecorators()
    {
        ...
        sw.Start();
#endif
        List<TraitDecorator> decorators = (from decorator in FMDB.Context.TraitDecorators.AsNoTracking() select decorator).ToList();
        foreach (TraitDecorator decorator in decorators)
        {
    ...
    void IFuturemudLoader.LoadEthnicities()
    {
        ...
        sw.Start();
#endif
        List<Models.Ethnicity> ethnicities = (from ethnicity in FMDB.Context.Ethnicities
        ...
        foreach (Models.Ethnicity ethnicity in ethnicities)
        {
    ...
    void IFuturemudLoader.LoadNameCultures()
    {
        ...
        sw.Start();
#endif
        List<Models.NameCulture> namecultures =
        ...
        foreach (Models.NameCulture nameculture in namecultures)
        {
            _nameCultures.Add(new NameCulture(nameculture, this));
        }

        int count = namecultures.Count;
    ...
    void IFuturemudLoader.LoadCultures()
    {
        ...
        List<Models.Culture> cultures = (from culture in FMDB.Context.Cultures
        ...
        foreach (Models.Culture culture in cultures)
        {
    ...
    void IFuturemudLoader.LoadRaces()
    {
        ...
        sw.Start();
#endif
        List<Models.Race> races = (from race in FMDB.Context.Races
        ...
        foreach (Tuple<Race, Models.Race> race in staging)
        {
    ...
    void IFuturemudLoader.LoadProjects()
    {
        ...
        sw.Start();
#endif
        List<Project> projects = FMDB.Context.Projects
                           .Include(x => x.EditableItem)
        ...
        sw.Restart();
#endif
        List<ActiveProject> actives = FMDB.Context.ActiveProjects
                          .Include(x => x.ActiveProjectLabours)
                          .Include(x => x.ActiveProjectMaterials)
                          .Include(x => x.Characters)
                          .AsNoTracking().ToList();
        foreach (ActiveProject active in actives)
        {
    ...
    void IFuturemudLoader.LoadJobs()
    {
        ...
        sw.Start();
#endif
        List<JobListing> listings = FMDB.Context
                           .JobListings
                           .Include(x => x.ActiveJobs)
                           .AsNoTracking()
                           .ToList();
        foreach (JobListing listing in listings)
        {
    ...
    void IFuturemudLoader.LoadCalendars()
    {
        ...
        sw.Start();
#endif
        List<Models.Calendar> calendars = (from calendar in FMDB.Context.Calendars.AsNoTracking()
                                           select calendar).ToList();
        foreach (Models.Calendar calendar in calendars)
        {
    ...
    void IFuturemudLoader.LoadClocks()
    {
        ...
        List<Models.Clock> clocks = (from clock in FMDB.Context.Clocks
                                        .Include(x => x.Timezones)
                                        .AsNoTracking()
                                     select clock).ToList();
        foreach (Models.Clock clock in clocks)
        {
    ...
    void IFuturemudLoader.LoadChecks()
    {
        ...
        sw.Start();
#endif
        List<Check> checks = (from check in FMDB.Context.Checks
        ...
        {
            ICheck fmcheck = StandardCheck.LoadCheck(check, this);
            _checksIDs[fmcheck.Type] = fmcheck.Id;
            _checks.Add(fmcheck);
        }

        foreach (CheckType checkType in Enum.GetValues<CheckType>())
        {
            ...
            if (!_checksIDs.ContainsKey(checkType))
            {
                ...
                ICheck fmcheck = StandardCheck.LoadCheck(newCheck, this);
                _checksIDs[fmcheck.Type] = fmcheck.Id;
                _checks.Add(fmcheck);
            }
        }
        ...
        foreach (CheckType value in Enum.GetValues(typeof(CheckType)).OfType<CheckType>())
        {
    ...
    void IFuturemudLoader.LoadProgSchedules()
    {
        ...
        List<Models.ProgSchedule> schedules = (from schedule in FMDB.Context.ProgSchedules.AsNoTracking() select schedule).ToList();
        foreach (Models.ProgSchedule schedule in schedules)
        {
    ...
    void IFuturemudLoader.LoadCelestials()
    {
        ...
        List<Models.Celestial> celestials = (from celestial in FMDB.Context.Celestials.AsNoTracking()
                                             select celestial).ToList();
        foreach (Models.Celestial celestial in celestials)
        {
