...
public class GameStatistics : IGameStatistics
{
    ...
    {
        DateTime currentWeekStart = DateTime.UtcNow.GetStartOfWeek();
        WeeklyStatistic dbitem = FMDB.Context.WeeklyStatistics.FirstOrDefault(x => x.Start == currentWeekStart);
        if (dbitem == null)
        {
            ...
            foreach (Account account in FMDB.Context.Accounts)
            {
    ...
    {
        WeeklyStatistic dbitem = GetOrCreateWeeklyStatistic();
    ...
    {
        List<ICharacter> players = _parent.Characters.Where(x => !x.IsAdministrator() && !x.IsGuest)
                             .Concat(_parent.NPCs.Where(x => x.CombinedEffectsOfType<ICountForWho>().Any())).ToList();
        int count = players.Count;
        int admins = _parent.Characters.Count(x => x.IsAdministrator());
        int guestCount = _parent.Characters.Count(x => x.IsGuest);
        int availableAdmins = _parent.Characters.Count(x => x.AffectedBy<IAdminAvailableEffect>());
        int idlers = players.Count(x => (x.CharacterController?.InactivityMilliseconds ?? 0) > 600000);
        int locations = players.Select(x => x.Location).Distinct().Count();

        using (new FMDB())
        {
            PlayerActivitySnapshot snapshot = new()
            {
    ...
    {
        string recordPlayers = game.GetStaticConfiguration("RecordOnlinePlayers");
        if (recordPlayers != null)
        {
            RecordOnlinePlayers = int.Parse(recordPlayers);
        }

        string recordPlayersDateTime = game.GetStaticConfiguration("RecordOnlinePlayersDateTime");
    ...
    public bool UpdateOnlinePlayers()
    {
        ...
        int count = _parent.Characters.Count(x => !x.IsAdministrator() && !x.IsGuest);
        ...
        using (new FMDB())
        {
            StaticConfiguration dbitem = FMDB.Context.StaticConfigurations.Find("RecordOnlinePlayers");
            if (dbitem == null)
            {
