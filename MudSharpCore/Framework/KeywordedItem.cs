...
public static class KeywordedItemExtensions
{
        ...
        bool abbreviated = true) where T : IKeyworded
    {
        ...
        string[] strings = pattern.Split(".".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
        ...
        if (strings[0][0] == '#' && int.TryParse(strings[0].RemoveFirstCharacter(), out int number))
        {
            List<string> keywords = strings.Skip(1).ToList();
            if (!keywords.Any())
            {
    ...
    {
        IHavePersonalName ihpn = item as IHavePersonalName;
        IFrameworkItem ifi = item as IFrameworkItem;
        string pn = ihpn?.PersonalName.GetName(NameStyle.FullName);
        ...
        bool abbreviated = true) where T : IKeyworded
    {
        ...
        if (int.TryParse(strings[0], out number))
        {
            List<string> keystrings = strings.Skip(1).ToList();
            return (from item in itemList
...
public abstract class KeywordedItem : FrameworkItem, IKeywordedItem
{
    ...
    {
        IEnumerable<string> keywords = GetKeywordsFor(voyeur);
        if (!abbreviated)
        {
    ...
    protected IEnumerable<string> GetKeywordsFromSDesc(string sdesc)
    {
        ...
        ExplodedString explode = new(sdesc);
        foreach (string keyword in explode.Words)
        {
