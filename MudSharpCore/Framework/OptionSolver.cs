...
public class OptionSolver<T1, T2>
{
    ...
    {
        List<Solution<T1, T2>> consideredProblemSets = new();
        Solution<T1, T2> currentSolution = new(Problems.Select(x => (x, x.BestOption)).ToList());
        ...
        IOption<T2> clash = currentSolution.FirstClashingOption();
        while (true)
        {
            bool clashResolved = false;
            List<(IChoice<T1, T2> Choice, IOption<T2> Option)> clashingSolutions = currentSolution.Solutions.Where(x => x.Option.IsClash(clash)).ToList();
            foreach ((IChoice<T1, T2> Choice, IOption<T2> Option) choiceSolution in clashingSolutions)
            {
                foreach (IOption<T2> option in choiceSolution.Choice.Options.Where(x => !x.IsClash(clash)))
                {
                    Solution<T1, T2> newSolution = new(currentSolution, choiceSolution.Choice, option);
