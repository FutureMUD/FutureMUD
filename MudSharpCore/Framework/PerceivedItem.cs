...
public abstract class PerceivedItem : LateKeywordedInitialisingItem, IPerceivable
{
    ...
    {
        bool cansee = voyeur?.CanSee(this, flags) ?? true;

        switch (type)
        {
            case DescriptionType.Short:
                string output = cansee ? (_shortDescriptionPattern?.Pattern ?? _shortDescription).FluentProper(proper) :
    ...
    public virtual IEnumerable<IWound> ExplosionEmantingFromPerceivable(IExplosiveDamage damage)
    {
        ...
        IEnumerable<IGrouping<Proximity, IPerceivable>> things = LocalThingsAndProximities().GroupBy(x => x.Proximity, x => x.Thing);
        foreach (IGrouping<Proximity, IPerceivable> proximity in things)
        {
            ...
            foreach (IPerceivable thing in proximity)
            {
    ...
    {
        Dictionary<IEffect, TimeSpan> scheduledEffects = new();
    ...
    {
        foreach (HooksPerceivable hook in hooks)
        {
            IHook ihook = Gameworld.Hooks.Get(hook.HookId);
            if (ihook != null)
            {
                InstallHook(ihook);
            }
        ...
        List<IDefaultHook> defaultHooks = Gameworld.DefaultHooks.Where(x => x.Applies(this, perceiverType)).ToList();
        foreach (IDefaultHook hook in defaultHooks)
        {
    ...
    {
        foreach ((IEffect effect, TimeSpan timespan) in _cachedEffects)
        {
    ...
    protected void LoadEffects(XElement effects)
    {
        ...
        {
            IEffect newEffect = Effect.LoadEffect(effect, this);
            ...
            double remaining = double.Parse(effect.Element("Remaining").Value);
            if (remaining > 0)
            {
    ...
    public bool InVicinity(IPerceivable target)
    {
        ...
        List<IPerceivable> outwardsList = new();
        List<IPerceivable> inwardsList = new();
        ...
        List<IPerceivable> currentPath, List<IPerceivable> previous)
    {
        ...
        ITable table = (origin as IGameItem)?.GetItemType<ITable>();
        if (table != null)
        {
        ...
        IChair chair = (origin as IGameItem)?.GetItemType<IChair>();
        if (chair != null)
        {
    ...
    public virtual void SetEmote(IEmote emote)
    {
        ...
        {
            foreach (IPerceivable target in PositionEmote.Targets)
            {
        ...
        if (emote != null)
        {
            foreach (IPerceivable target in emote.Targets)
            {
                target.InvalidPositionTargets += Emote_InvalidPositionTargets;
            }
        }
