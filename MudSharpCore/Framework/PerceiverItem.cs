...
public abstract class PerceiverItem : PerceivedItem, IPerceiver
{
    ...
    {
        IAimInformation aim = (IAimInformation)sender;
    ...
    public virtual double GetBonusForDefendersFromTargeting()
    {
        ...
        (double min, double max) = tch.Body.Bodyparts.Select(x => x.RelativeHitChance).MinMax(x => x > 0.0);
    ...
    public void FallToGround()
    {
        ...
        while (true)
        {
            bool result = FallOneLayer(ref fallDistance);
            if (dragTarget != null)
            {
                double otherFallDistance = fallDistance;
    ...
    {
        ICharacter ch = this as ICharacter;
        IGameItem item = this as IGameItem;
