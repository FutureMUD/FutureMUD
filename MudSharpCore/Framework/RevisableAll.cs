...
public class RevisableAll<T> : IRevisableAll<T>, IUneditableRevisableAll<T> where T : class, IRevisableItem
{
    ...
    {
        List<T> items = _iterlist.Where(x => x.Id == id && filter(x)).ToList();
    ...
    {
        return _idlookup.TryGetValue(Tuple.Create(id, revision), out T value) ? value : null;
    }

    public T Get(long id, int revision, Func<T, bool> filter)
    {
        if (_idlookup.TryGetValue(Tuple.Create(id, revision), out T value))
        {
            if (filter(value))
            {
                return value;
    ...
    {
        List<T> matches = _iterlist.Where(x => x.Name.EqualTo(name, ignoreCase)).ToList();
    ...
    {
        List<T> list = new();
        foreach (KeyValuePair<long, List<T>> item in _groupingLookup)
        {
    ...
    {
        List<T> list = new();
        foreach (List<T> item in _groupingLookup.Values)
        {
            if (item.Count == 0)
            {
                continue;
            ...
            foreach (T rev in item.OrderByDescending(x => x.RevisionNumber))
            {
