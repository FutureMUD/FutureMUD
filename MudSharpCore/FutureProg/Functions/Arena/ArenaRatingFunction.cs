...
internal class ArenaRatingFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        ICombatantClass? combatantClass = ResolveCombatantClass(ParameterFunctions[1].Result?.GetObject);
    ...
    private ICombatantClass? ResolveCombatantClassByText(string text)
    {
        ...
        if (long.TryParse(text, out long idMatch))
        {
            return ResolveCombatantClassById(idMatch);
        }

        List<ICombatantClass> classes = _gameworld.CombatArenas
            .SelectMany(x => x.CombatantClasses)
            .DistinctBy(x => x.Id)
            .ToList();
