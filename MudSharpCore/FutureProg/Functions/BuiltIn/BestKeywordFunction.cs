...
internal class BestKeywordFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        {
            ICharacter target = (ICharacter)ParameterFunctions[1].Result;
            if (target == null)
            {
            ...
            IEnumerable<string> keywords = target.GetKeywordsFor(source);
            List<ICharacter> targets = source.Location.LayerCharacters(source.RoomLayer).Except(source)
