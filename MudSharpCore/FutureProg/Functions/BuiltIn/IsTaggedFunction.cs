...
internal class IsTaggedFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        {
            List<ITag> matchedtags = Gameworld.Tags.FindMatchingTags(ParameterFunctions[1].Result.GetObject.ToString());
            if (matchedtags.Count == 0)
            {
                ErrorMessage =
                    $"There was no tag found with reference {ParameterFunctions[1].Result.GetObject}";
                return StatementResult.Error;
            }
            ...
            {
                ErrorMessage =
                    $"Multiple tags found with reference {ParameterFunctions[1].Result.GetObject}";
                return StatementResult.Error;
            }
        ...
        if (tag == null)
        {
            ErrorMessage = $"There was no tag found with reference {ParameterFunctions[1].Result.GetObject}";
            return StatementResult.Error;
        }
