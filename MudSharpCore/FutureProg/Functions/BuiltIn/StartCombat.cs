...
internal class StartCombat : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        string reference = ParameterFunctions[0].Result?.GetObject?.ToString() ?? "";
        string description = ParameterFunctions[1].Result?.GetObject?.ToString() ??
                          "This is a fight to the death, with no rules or holds barred";
        string gerund = ParameterFunctions[2].Result?.GetObject?.ToString() ?? "fighting";
        bool friendly = Convert.ToBoolean(ParameterFunctions[3].Result?.GetObject ?? false);
        ICharacter fighter1 = (ICharacter)ParameterFunctions[4].Result?.GetObject;
        ICharacter fighter2 = (ICharacter)ParameterFunctions[5].Result?.GetObject;
        string progname = ParameterFunctions[6].Result?.GetObject?.ToString();
        IFutureProg joinprog = Gameworld.FutureProgs.GetByName(progname ?? "");
        progname = ParameterFunctions[7].Result?.GetObject?.ToString();
        IFutureProg leaveprog = Gameworld.FutureProgs.GetByName(progname ?? "");
        progname = ParameterFunctions[8].Result?.GetObject?.ToString();
        IFutureProg endprog = Gameworld.FutureProgs.GetByName(progname ?? "");
        progname = ParameterFunctions[9].Result?.GetObject?.ToString();
        IFutureProg moveprog = Gameworld.FutureProgs.GetByName(progname ?? "");
        progname = ParameterFunctions[10].Result?.GetObject?.ToString();
        IFutureProg hitprog = Gameworld.FutureProgs.GetByName(progname ?? "");
