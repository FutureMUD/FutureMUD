...
internal class GetTraitCapFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        {
            ICharacterTemplate chargen = (ICharacterTemplate)target;
            if (trait.TraitType == TraitType.Attribute)
            {
                ITrait attr = chargen.SelectedAttributes.FirstOrDefault(x => x.Definition == trait);
                Result = attr == null ? new NumberVariable(0.0M) : new NumberVariable(attr.Definition.MaxValue);
            }
            else
            {
                ISkillDefinition skill = chargen.SkillValues.FirstOrDefault(x => x.Item1 == trait).Item1 as ISkillDefinition;
