...
internal class TraitBonusFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        ICharacter target = ParameterFunctions[0].Result as ICharacter;
        if (target == null)
        {
        ...
        ITraitDefinition trait = (ITraitDefinition)ParameterFunctions[1].Result;
        if (trait == null)
        {
        ...
        decimal dBonus = (decimal)(ParameterFunctions[2].Result?.GetObject ?? 0.0M);
        string reference = (string)(ParameterFunctions[3].Result?.GetObject ?? "");
        ...
        {
            target.RemoveAllEffects<ProgTraitBonusEffect>(x => x.OriginalReference.EqualTo(reference), true);
            ProgTraitBonusEffect effect = new(target, trait, (double)dBonus, reference, filterProg);
