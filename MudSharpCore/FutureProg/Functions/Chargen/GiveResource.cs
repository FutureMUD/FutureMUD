...
namespace MudSharp.FutureProg.Functions.Chargen
{
    internal class GiveResource : BuiltInFunction
    {
        ...
        public override StatementResult Execute(IVariableSpace variables)
        {
            ...
            IHaveAccount chargen = (IHaveAccount)ParameterFunctions[0]?.Result;
            if (chargen is null)
            {
            ...
            string text = (string)ParameterFunctions[1].Result?.GetObject;
            ...
            int amount = (int)((decimal?)ParameterFunctions[2].Result?.GetObject ?? 0.0M);
            double current = chargen.Account.AccountResources.GetValueOrDefault(resource);
            ...
            using (new FMDB())
            {
                Models.Account dbaccount = FMDB.Context.Accounts.Find(chargen.Account.Id);
                if (dbaccount == null)
                {
                ...
                Models.AccountsChargenResources dbaccountresource = dbaccount.AccountsChargenResources.FirstOrDefault(x => x.ChargenResourceId == resource.Id);
                if (dbaccountresource == null)
                {
                    dbaccountresource = new Models.AccountsChargenResources
                    {
                        Account = dbaccount,
                        ChargenResourceId = resource.Id,
                        Amount = 0,
                        LastAwardDate = System.DateTime.MinValue
                    };
