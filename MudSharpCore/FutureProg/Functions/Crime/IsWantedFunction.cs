...
internal class IsWantedFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        List<ILegalAuthority> authorities = ParameterFunctions[1].Result?.GetObject is not ILegalAuthority authority
            ? Gameworld.LegalAuthorities.ToList()
            : new List<ILegalAuthority> { authority };
        foreach (ILegalAuthority item in authorities)
        {
            List<ICrime> crimes = item.KnownCrimesForIndividual(character).ToList();
