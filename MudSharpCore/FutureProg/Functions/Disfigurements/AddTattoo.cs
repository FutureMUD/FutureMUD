...
internal class AddTattoo : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        long tattooid = Convert.ToInt64(ParameterFunctions[1].Result?.GetObject ?? 0M);
        if (tattooid == 0)
        {
            Result = new BooleanVariable(false);
            return StatementResult.Normal;
        ...
        string bodypartName = ParameterFunctions[2].Result?.GetObject?.ToString();
        ...
        IBodypart bodypart = character.Body.GetTargetBodypart(bodypartName);
        if (bodypart == null)
        {
            Result = new BooleanVariable(false);
            return StatementResult.Normal;
        ...
        ICharacter tattooist = ParameterFunctions[3].Result?.GetObject as ICharacter;
        double tattooistSkill = ParameterFunctions[4].Result?.GetObject == null
            ? tattooist?.GetTrait(TattooTemplate.TattooistTrait)?.Value ?? 100.0
            : Convert.ToDouble(ParameterFunctions[4].Result.GetObject);

        double completion = Convert.ToDouble(ParameterFunctions[5].Result?.GetObject ?? -1);
        ...
        ITattoo newTattoo = tattoo.ProduceTattoo(tattooist ?? character, character, bodypart);
