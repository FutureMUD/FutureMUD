...
internal class AddHealingEffectFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        IPerceivable perceivable = (IPerceivable)ParameterFunctions[0].Result.GetObject;
        if (perceivable == null)
        {
        ...
        IFutureProg prog = ParameterFunctions[3].ReturnType.CompatibleWith(ProgVariableTypes.Number)
        ...
        TimeSpan timespan = ParameterFunctions[4].Result?.GetObject as TimeSpan? ?? TimeSpan.MinValue;

        HealingRateEffect effect = new(perceivable, Convert.ToDouble(ParameterFunctions[1].Result?.GetObject ?? 0.0M),
