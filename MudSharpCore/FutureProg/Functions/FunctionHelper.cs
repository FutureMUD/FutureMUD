...
public static class FunctionHelper
{
    ...
    public static string ColouriseFunction(string line, bool isDarkMode = false)
    {
        ...
        {
            return $"({ColouriseFunction(line[1..^1], isDarkMode)})";
        }

        using (BinarySplit binaryLogicCombiners = BinaryStringSplit(line, _binaryLogicCombinerSplitStrings))
        {
        ...
        using (BinarySplit binaryLogicComparers = BinaryStringSplit(line, _binaryLogicOperationSplitString))
        {
        ...
        using (BinarySplit binaryOperators = BinaryStringSplit(line, _binaryOperatorSplitString))
        {
        ...
        using (BinarySplit dotReferences = ReverseBinarySplit(line, '.'))
        {
            ...
            {
                Match match = _collectionExtensionFunctionRegex.Match(dotReferences.RHS);
                if (match.Success)
                {
                    if (isDarkMode)
                    {
        ...
        using (UnarySplit unaryOperators = UnaryStringSplit(line))
        {
            if (unaryOperators.IsFound)
            {
                ParameterSplit parameters = ParameterStringSplit(unaryOperators.FunctionContents, ',');
        ...
        using (NonFunctionSplit nonFunctions = NonFunctionStringSplit(line))
        {
        ...
        IDictionary<string, ProgVariableTypes> variableSpace, int lineNumber, IFuturemud gameworld)
    {
        ...
        CompileInfo.ICompileInfoFactory compileInfoFactory = CompileInfo.GetFactory();
        ...
        NonFunctionSplit nonFunctions = NonFunctionStringSplit(line);
        ...
        if (!nonFunctions.IsError && nonFunctions.IsFound)
        {
            switch (nonFunctions.Type)
            {
                ...
                case NonFunctionType.TimeSpanLiteral:
                    Match tsMatch = TimespanRegex.Match(nonFunctions.StringValue);
        ...
        BinarySplit binaryLogicCombiners = BinaryStringSplit(line, _binaryLogicCombinerSplitStrings);
        ...
        if (binaryLogicCombiners.IsFound)
        {
            ...
            ICompileInfo lhsCompileInfo = CompileFunction(binaryLogicCombiners.LHS, variableSpace, lineNumber, gameworld);
            ICompileInfo rhsCompileInfo = CompileFunction(binaryLogicCombiners.RHS, variableSpace, lineNumber, gameworld);
        ...
        BinarySplit binaryLogicComparers = BinaryStringSplit(line, _binaryLogicOperationSplitString);
        ...
        {
            // Compile LHS and RHS and check for errors
            ICompileInfo lhsCompileInfo = CompileFunction(binaryLogicComparers.LHS, variableSpace, lineNumber, gameworld);
            ICompileInfo rhsCompileInfo = CompileFunction(binaryLogicComparers.RHS, variableSpace, lineNumber, gameworld);
            if (lhsCompileInfo.IsError)
            {
            ...
            IFunction lhs = (IFunction)lhsCompileInfo.CompiledStatement;
            IFunction rhs = (IFunction)rhsCompileInfo.CompiledStatement;
        ...
        {
            Match match = _indexerRegex.Match(line);
            string variable = match.Groups["variable"].Value.ToLowerInvariant();
            ...
            ICompileInfo indexFunction = CompileFunction(match.Groups["key"].Value, variableSpace, lineNumber, gameworld);
            ...
            ProgVariableTypes variableType = variableSpace[variable];
            ...
            {
                ProgVariableTypes indexType = ((IFunction)indexFunction.CompiledStatement).ReturnType;
        ...
        BinarySplit binaryOperators = BinaryStringSplit(line, _binaryOperatorSplitString);
        ...
        {
            // Compile LHS and RHS and check for errors
            ICompileInfo lhsCompileInfo = CompileFunction(binaryOperators.LHS, variableSpace, lineNumber, gameworld);
            ICompileInfo rhsCompileInfo = CompileFunction(binaryOperators.RHS, variableSpace, lineNumber, gameworld);
            if (lhsCompileInfo.IsError)
            {
            ...
            switch ((lhs.ReturnType & ~ProgVariableTypes.Literal).LegacyCode)
            {
                ...
                case ProgVariableTypeCode.Text:
                    ...
                    ProgVariableTypes rhsCoreType = rhs.ReturnType & ~ProgVariableTypes.Literal;
        ...
        BinarySplit dotReferences = ReverseBinarySplit(line, '.');
            ...
            dotReferences.RHS.GetDouble() == null)
        {
            ...
            ICompileInfo lhsCompileInfo = CompileFunction(dotReferences.LHS, variableSpace, lineNumber, gameworld);
            if (lhsCompileInfo.IsError)
            {
            ...
            if (lhs.ReturnType.HasFlag(ProgVariableTypes.Collection))
            {
                ...
                if (!match.Success)
                {
                    ProgVariableTypes drType = ProgVariable.DotReferenceReturnTypeFor(lhs.ReturnType,
                        dotReferences.RHS);
            ...
            ProgVariableTypes returnType = ProgVariable.DotReferenceReturnTypeFor(lhs.ReturnType, dotReferences.RHS);
        ...
        UnarySplit unaryOperators = UnaryStringSplit(line);
        ...
        {
            // Extract parameters
            ParameterSplit parameterSplit = ParameterStringSplit(unaryOperators.FunctionContents, ',');
    ...
    public static ParameterSplit ParameterStringSplit(string input, char splitChar)
    {
        ...
        for (position = 0; position < input.Length; position++)
        {
            ...
            {
                parameters.Add(input[lastStartIndex..position]);
        ...
        {
            parameters.Add(input[lastStartIndex..]);
        }
    ...
    public static BinarySplit BinaryStringSplit(string input, IEnumerable<string> splitStrings)
    {
        while (true)
        {
            ...
            for (position = 0; position < input.Length; position++)
            {
                ...
                {
                    string lookahead = input[position..];
                    string splitterFound =
                    ...
                    if (splitterFound != null)
                    {
                        ...
                        string lhsLeading = new(input.TakeWhile(x => x == '(').ToArray());
                        string rhsTrailing =
                            new(input.Reverse().TakeWhile(x => x == ')').Take(lhsLeading.Length).ToArray());
                        return BinarySplit.CreateNew(lhsLeading, input[..position].Trim(),
                            lookahead[splitterFound.Length..].Trim(), rhsTrailing, splitterFound.Trim());
                    }
                }
            }
        ...
        switch (input[0])
        {
            case '"':
                ...
                return NonFunctionSplit.CreateNew(NonFunctionType.TextLiteral,
                    input[1..^1].ParseSpecialCharacters(), lhsLeading, rhsTrailing);
            case '@':
                input = input[1..];
                if (!input.Any())
                {
    ...
    private static UnarySplit UnaryStringSplit(string input)
    {
        ...
        string lhsLeading = new(input.TakeWhile(x => x == '(').ToArray());
        string rhsTrailing = new(input.Reverse().TakeWhile(x => x == ')').Take(lhsLeading.Length).ToArray());
        return
            UnarySplit.CreateNew(
                (udf ? input[1..firstBracketPosition] : input[..firstBracketPosition]).Trim
