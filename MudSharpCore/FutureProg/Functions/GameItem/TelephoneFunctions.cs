...
internal class PhoneNumberFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        string phoneNumber = item.GetItemType<ITelephone>()?.PhoneNumber ??
...
internal class SetPhoneNumberFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        ITelephoneNumberOwner owner = item.GetItemType<ITelephoneNumberOwner>() ?? item.GetItemType<ITelephone>()?.NumberOwner;
        if (owner == null)
        {
            Result = new BooleanVariable(false);
            return StatementResult.Normal;
        ...
        string normalised = new(text.Where(char.IsDigit).ToArray());
...
internal class SetPhoneSharedNumberFunction : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        bool shared = Convert.ToBoolean(ParameterFunctions[1].Result?.GetObject ?? false);
