...
internal class AddItemToOutfit : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        IGameItem item = (IGameItem)ParameterFunctions[1].Result;
        if (item == null)
        {
        ...
        {
            IGameItem container = (IGameItem)ParameterFunctions[2].Result;
            string profileText = ParameterFunctions[3].Result?.ToString();
            IWearProfile profile = string.IsNullOrEmpty(profileText)
        ...
        {
            string profileText = ParameterFunctions[2].Result?.ToString();
            IWearProfile profile = string.IsNullOrEmpty(profileText)
                ? item.GetItemType<IWearable>()?.Profiles.FirstOrDefault(x => x.Name.EqualTo(profileText))
                : default;
