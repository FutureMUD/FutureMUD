...
internal class RemoveOutfit : BuiltInFunction
{
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        List<IGameItem> items = character.Body.DirectWornItems.Where(x => outfit.Items.Any(y => y.Id == x.Id)).Reverse().ToList();
        if (!items.Any())
        {
        ...
        foreach (IGameItem outfitItem in items)
        {
            ...
            List<Tuple<WearableItemCoverStatus, IGameItem>> coverItemInfo = character.Body.CoverInformation(outfitItem).Where(x => x.Item2 != null).ToList();
            ...
            {
                long? containerId = targetContainer?.Id ??
                                  outfit.Items.FirstOrDefault(x => x.Id == item.Id)?.PreferredContainerId;
                IContainer container = targetContainer?.GetItemType<IContainer>() ?? character.ContextualItems.FirstOrDefault(
