...
internal class ConsoleEcho : Statement
{
    ...
    {
        Match match = ConsoleCompileRegex.Match(lines.First());
        ICompileInfo messageInfo = FunctionHelper.CompileFunction(match.Groups[1].Value, variableSpace, lineNumber, gameworld);
        if (messageInfo.IsError)
        {
            return CompileInfo.GetFactory().CreateError(messageInfo.ErrorMessage, lineNumber);
        }

        IFunction function = (IFunction)messageInfo.CompiledStatement;
...
For example:
    ...
    {
        StatementResult result = MessageFunction.Execute(variables);
