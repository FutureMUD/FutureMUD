...
internal class DelayProg : Statement
{
        ...
        IDictionary<string, ProgVariableTypes> variableSpace, int lineNumber, IFuturemud gameworld)
    {
        ...
        FunctionHelper.ParameterSplit splitArgs = FunctionHelper.ParameterStringSplit(match.Groups[2].Value, ' ');
        ...
        List<IFunction> parameters = compiledArgs.Skip(1).Select(x => x.CompiledStatement).OfType<IFunction>().ToList();
        List<ProgVariableTypes> otherTypes = parameters.Select(x => x.ReturnType).ToList();
        string progArg = match.Groups[1].Value;

        IFutureProg targetProg = gameworld.FutureProgs.FirstOrDefault(
...
Notice also that the parameter was wrapped in parentheses because it contained spaces.");
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        foreach (IFunction function in ParameterFunctions)
        {
        ...
        double delay = (double)(decimal)DelayFunction.Result.GetObject;
        if (delay <= 0)
        {
        ...
        object[] parameters = ParameterFunctions.Select(x => x.Result.GetObject).ToArray();
