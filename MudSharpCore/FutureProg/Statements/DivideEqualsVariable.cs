...
internal class DivideEqualsVariable : Statement
{
    ...
    protected DivideEqualsVariable(string nameToSet, ProgVariableTypes typeToSet, IFunction valueFunction)
    {
        NameToSet = nameToSet;
        TypeToSet = typeToSet;
        ...
        IDictionary<string, ProgVariableTypes> variableSpace, int lineNumber, IFuturemud gameworld)
    {
        ...
        return
            CompileInfo.GetFactory()
                       .CreateNew(
                           new DivideEqualsVariable(match.Groups[1].Value.ToLowerInvariant(), function.ReturnType, function),
    ...
    public static void RegisterCompiler()
    {
        ...
        FutureProg.RegisterStatementHelp("/=", @"The /= statement is used to assign a value to a number variable that has already been declared and also divide it.
