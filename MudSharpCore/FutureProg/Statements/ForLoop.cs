...
internal class ForLoop : Statement
{
    ...
    {
        Match match = ForLoopCompileRegex.Match(lines.First());
        string varName = match.Groups[1].Value.ToLowerInvariant();
        ICompileInfo functionInfo = FunctionHelper.CompileFunction(match.Groups[2].Value, variableSpace, lineNumber, gameworld);
        ...
        IFunction function = (IFunction)functionInfo.CompiledStatement;
    ...
    {
        Match match = ForLoopCompileRegex.Match(line);
        return
