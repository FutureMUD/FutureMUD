...
internal class IfBlock : Statement
{
    ...
    {
        Match match = IfBlockCompileRegex.Match(lines.First());
        ICompileInfo funcInfo = FunctionHelper.CompileFunction(match.Groups[1].Value, variableSpace, lineNumber, gameworld);
        if (funcInfo.IsError)
        {
            return CompileInfo.GetFactory().CreateError($"Error in If Statement\n{funcInfo.ErrorMessage}", funcInfo.ErrorLineNumber);
        }

        IFunction function = (IFunction)funcInfo.CompiledStatement;
        ...
        lines = lines.Skip(1);
        List<IStatement> trueStatements = new();
        List<IStatement> falseStatements = new();
        List<(IFunction ElseLogic, IEnumerable<IStatement> Statements)> elseIfBlocks = new();
        ...
        while (lines.Any())
        {
            ...
            if (IfBlockElseIfCompileRegex.IsMatch(line))
            {
                ...
                inElseIfBlock = true;
                Match elseifmatch = IfBlockElseIfCompileRegex.Match(lines.First());
                ICompileInfo elseiffuncInfo = FunctionHelper.CompileFunction(elseifmatch.Groups[1].Value, variableSpace, lineNumber, gameworld);
                ...
                IFunction elseiffunction = (IFunction)elseiffuncInfo.CompiledStatement;
            ...
            ICompileInfo statementInfo = FutureProg.CompileNextStatement(lines,
    ...
    {
        Match match = IfBlockCompileRegex.Match(line);
        return
    ...
    {
        Match match = IfBlockElseIfCompileRegex.Match(line);
        return
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        if ((bool)LogicFunction.Result.GetObject)
        {
            if (TrueBlock.Any())
            {
                ...
                foreach (IStatement statement in TrueBlock)
                {
        ...
        {
            foreach ((IFunction elseIfLogic, IEnumerable<IStatement> elseIfBlock) in ElseIfBlocks)
            {
                ...
                LocalVariableSpace localVariables = new(variables);
                foreach (IStatement statement in elseIfBlock)
                {
                    StatementResult result = statement.Execute(localVariables);
                    if (result == StatementResult.Error)
                    {
            ...
            {
                LocalVariableSpace localVariables = new(variables);
                foreach (IStatement statement in FalseBlock)
                {
                    StatementResult result = statement.Execute(localVariables);
                    if (result == StatementResult.Error)
                    {
