...
internal class OpenDoor : Statement
{
    private static readonly Regex CompileRegex = new(@"^\s*opendoor\s+(.+)\s*$",
    ...
    private OpenDoor(IFunction characterFunction, IFunction exitFunction, IFunction emoteFunction)
    {
        ...
        IDictionary<string, ProgVariableTypes> variableSpace, int lineNumber, IFuturemud gameworld)
    {
        ...
        {
            return CompileInfo.GetFactory().CreateError("Error with arguments of OpenDoor statement.", lineNumber);
        }
        ...
        if (splitArgs.ParameterStrings.Count() < 2 || splitArgs.ParameterStrings.Count() > 3)
        {
            return
                CompileInfo.GetFactory()
                           .CreateError("The OpenDoor statement requires exactly two or three parameters.", lineNumber);
        }
        ...
        if (compiledArgs.Any(x => x.IsError))
        {
            return
                CompileInfo.GetFactory()
                           .CreateError(
                               $"Compile error with OpenDoor statement arguments: {compiledArgs.First(x => x.IsError).ErrorMessage}", lineNumber);
        }
                ...
                ProgVariableTypes.Character))
        {
            return
                CompileInfo.GetFactory()
                           .CreateError("The first argument of the OpenDoor statement must be a character.",
                               lineNumber);
        }
        ...
        if (!((IFunction)compiledArgs[1].CompiledStatement).ReturnType.CompatibleWith(ProgVariableTypes.Exit))
        {
            return
                CompileInfo.GetFactory()
                           .CreateError("The second argument of the OpenDoor statement must be an exit.", lineNumber);
        }
            ...
            !((IFunction)compiledArgs[2].CompiledStatement).ReturnType.CompatibleWith(ProgVariableTypes.Text))
        {
            return
                CompileInfo.GetFactory()
                           .CreateError(
                               "The third argument of the OpenDoor statement must be text, if specified at all.",
                               lineNumber);
        }
        ...
        return
            CompileInfo.GetFactory()
                       .CreateNew(
                           new OpenDoor((IFunction)compiledArgs[0].CompiledStatement,
    ...
    private static string ColouriseStatement(string line)
    {
        ...
        return
            $"{"opendoor".Colour(Telnet.Cyan, Telnet.Black)} {FunctionHelper.ColouriseFunction(splitArgs.ParameterStrings.ElementAt(0))} {FunctionHelper.ColouriseFunction(splitArgs.ParameterStrings.ElementAt(1))}{(splitArgs.ParameterStrings.Count() > 2 ? $" {FunctionHelper.ColouriseFunction(splitArgs.ParameterStrings.ElementAt(2))}" : "")}";
    }
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        ICharacter character = (ICharacter)CharacterFunction.Result;
        if (character == null)
        {
        ...
        // TODO - unlocking as well?
        character.Body.Open(exit.Exit.Door, null, emote);
        return StatementResult.Normal;
    }
}