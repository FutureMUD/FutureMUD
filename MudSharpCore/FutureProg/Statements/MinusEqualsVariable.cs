...
internal class MinusEqualsVariable : Statement
{
    ...
    protected MinusEqualsVariable(string nameToSet, ProgVariableTypes typeToSet, IFunction valueFunction)
    {
        NameToSet = nameToSet;
        TypeToSet = typeToSet;
        ...
        IDictionary<string, ProgVariableTypes> variableSpace, int lineNumber, IFuturemud gameworld)
    {
        ...
        else if (lhsType.CompatibleWith(ProgVariableTypes.DateTime) || lhsType.CompatibleWith(ProgVariableTypes.MudDateTime))
        {
            ...
            {
                return CompileInfo.GetFactory().CreateError(
                    $"Tried to subtract a non-TimeSpan type to a DateTime type variable. Expected TimeSpan and got {function.ReturnType.Describe()}.",
                    lineNumber);
            }
        }
        ...
        return
            CompileInfo.GetFactory()
                       .CreateNew(
                           new MinusEqualsVariable(match.Groups[1].Value.ToLowerInvariant(), lhsType, function),
    ...
    public static void RegisterCompiler()
    {
        ...
        FutureProg.RegisterStatementHelp("-=", @"The -= statement is used to assign a value to a variable that has already been declared and remove something from it.

This can be used with numbers (numeric subtraction), text (search and erase), datetime/muddatetime with a timespan value (subtract), and collections (remove item from collection).
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        else
        {
            ...
            if (TypeToSet.CompatibleWith(ProgVariableTypes.Text))
            {
                ...
                string replaceText = (string)ValueFunction.Result?.GetObject ?? "";
                if (string.IsNullOrEmpty(replaceText))
                {
                    return StatementResult.Normal;
                }

                string newValue = text.Replace(replaceText, "");
