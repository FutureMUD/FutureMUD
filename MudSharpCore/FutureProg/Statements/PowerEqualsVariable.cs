...
internal class PowerEqualsVariable : Statement
{
    ...
    protected PowerEqualsVariable(string nameToSet, ProgVariableTypes typeToSet, IFunction valueFunction)
    {
        NameToSet = nameToSet;
        TypeToSet = typeToSet;
        ...
        IDictionary<string, ProgVariableTypes> variableSpace, int lineNumber, IFuturemud gameworld)
    {
        ...
        return
            CompileInfo.GetFactory()
                       .CreateNew(
                           new PowerEqualsVariable(match.Groups[1].Value.ToLowerInvariant(), function.ReturnType, function),
    ...
    public static void RegisterCompiler()
    {
        ...
        FutureProg.RegisterStatementHelp("^=", @"The ^= statement is used to assign a value to a number variable that has already been declared and also raise it to a power.
...
See also #3=#0, #3+=#0, #3-=#0, #3*=#0, #3/=#0 and #3%=#0 for other ways of assigning values combined with an operation.");
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        decimal newValue = (decimal)Math.Pow((double)number, (double)((decimal?)ValueFunction.Result?.GetObject ?? 0.0M));
