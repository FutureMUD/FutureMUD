...
internal class RemoveItemIndex : Statement
{
    ...
    {
        Match match = RemoveItemCompileRegex.Match(lines.First());
        ProgVariableTypes collectionType;
        if (match.Groups["collection"].Value[0] == '@')
        {
            ICompileInfo collectionFunctionInfo = FunctionHelper.CompileFunction(match.Groups["collection"].Value[1..],
        ...
        ICompileInfo removeItemFunctionInfo = FunctionHelper.CompileFunction(match.Groups["index"].Value, variableSpace, lineNumber,
            gameworld);
        ...
        IFunction removeItemFunction = (IFunction)removeItemFunctionInfo.CompiledStatement;
    ...
    {
        Match match = RemoveItemCompileRegex.Match(line);
        return
...
#Oremoveitemindex#0 #M@numbers#0 #22#0".SubstituteANSIColour());
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        int index = (int)(decimal)ItemFunction.Result.GetObject;
        IProgVariable collection = variables.GetVariable(CollectionVariable);
        List<IProgVariable> newList = ((IList<IProgVariable>)collection.GetObject).ToList();
