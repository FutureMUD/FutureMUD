...
internal class Return : Statement
{
    ...
    {
        Match match = ReturnCompileRegex.Match(lines.First());
        ...
        if (match.Groups[1].Success)
        {
            ...
            ICompileInfo lhs = FunctionHelper.CompileFunction(match.Groups[1].Value, variableSpace, lineNumber, gameworld);
            if (lhs.IsError)
            {
            ...
            IFunction lhsFunction = (IFunction)lhs.CompiledStatement;
            ProgVariableTypes returnVar = variableSpace["return"];
    ...
    {
        Match match = ReturnCompileRegex.Match(line);
        if (match.Groups[1].Success)
        {
