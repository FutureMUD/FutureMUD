...
internal class Switch : Statement
{
    ...
    {
        Match match = SwitchCompileRegex.Match(lines.First());
        string switchFunctionTest = match.Groups[1].Value;

        ICompileInfo switchFunctionCompileInfo = FunctionHelper.CompileFunction(switchFunctionTest, variableSpace, lineNumber,
            gameworld);
        ...
        IFunction switchFunctionCompiled = (IFunction)switchFunctionCompileInfo.CompiledStatement;

        IFunction currentCaseFunction = null;
        List<Tuple<IFunction, IEnumerable<IStatement>>> cases = new();
        List<IStatement> defaultCase = new();
        ...
        while (lines.Any())
        {
            ...
            if (SwitchCaseCompileRegex.IsMatch(line))
            {
                ...
                ICompileInfo caseFunctionCompileInfo = FunctionHelper.CompileFunction(match.Groups[1].Value, variableSpace,
    ...
    {
        Match match = SwitchCompileRegex.Match(line);
        return
    ...
    {
        Match match = SwitchCaseCompileRegex.Match(line);
        return
...
A DEFAULT statement block is ended by a #Obreak#0 or #Oend#0 statement.", "break, case, end, switch");
    ...
    public override StatementResult Execute(IVariableSpace variables)
    {
        ...
        foreach (Tuple<IFunction, IEnumerable<IStatement>> item in Cases)
        {
