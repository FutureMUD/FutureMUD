...
internal class WhileLoop : Statement
{
    ...
    {
        Match match = WhileLoopCompileRegex.Match(lines.First());
        string functionText = match.Groups[1].Value;

        ICompileInfo functionCompileInfo = FunctionHelper.CompileFunction(functionText, variableSpace, lineNumber, gameworld);
        ...
        IFunction compiledFunction = (IFunction)functionCompileInfo.CompiledStatement;
    ...
    {
        Match match = WhileLoopCompileRegex.Match(line);
        return
