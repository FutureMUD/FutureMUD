...
public class ActiveCraftGameItemComponent : GameItemComponent, IActiveCraftGameItemComponent
{
    ...
    public override void Delete()
    {
        if (!HasFinished)
        {
            foreach (KeyValuePair<ICraftProduct, ICraftProductData> item in ProducedProducts)
            {
                item.Value.Delete();
            }
        }

        foreach (KeyValuePair<ICraftInput, (IPerceivable Input, ICraftInputData Data)> item in ConsumedInputs)
        {
    ...
    {
        int revision = int.Parse(root.Element("CraftRevision")?.Value ?? "-1");
        if (revision >= 0)
        {
        ...
        XElement element = root.Element("CheckOutcome");
        if (element != null)
        {
    ...
    {
        foreach (KeyValuePair<ICraftProduct, ICraftProductData> product in ProducedProducts)
        {
        ...
        foreach (KeyValuePair<ICraftInput, (IPerceivable Input, ICraftInputData Data)> input in ConsumedInputs)
        {
