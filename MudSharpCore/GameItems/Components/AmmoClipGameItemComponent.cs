...
public class AmmoClipGameItemComponent : GameItemComponent, IAmmoClip
{
    ...
    public WhyCannotPutReason WhyCannotPut(IGameItem item)
    {
        ...
        if (_contents.Sum(x => x.Quantity) + item.Quantity > Capacity)
        {
            int remaining = Capacity - _contents.Sum(x => x.Quantity);
    ...
    {
        foreach (XElement item in root.Element("Contents").Elements("Item"))
        {
            IGameItem content = Gameworld.TryGetItem(long.Parse(item.Value), true);
            if (content != null)
            {
                content.Get(null);
