	...
	ISelectable, IConnectable, ICanConnectToTelecommunicationsGrid, IContainer, IProducePower
{
	...
	private void LoadFromXml(XElement root)
	{
		...
		_pendingTapeItemId = long.TryParse(root.Element("Tape")?.Value, out var tapeId) ? tapeId : 0L;
	...
	public override void FinaliseLoad()
	{
		...
		{
			var tape = Gameworld.TryGetItem(_pendingTapeItemId, true);
			if (tape != null)
			{
				tape.Get(null);
	...
	public override void Delete()
	{
		...
		foreach (var consumer in _powerUsers.ToList())
		{
	...
	private void AnsweringMachineHeartbeat()
	{
		...
		foreach (var segment in _scheduledGreetingSegments.Where(x => x.DueAtUtc <= now).OrderBy(x => x.DueAtUtc).ToList())
		{
	...
	private void PlayGreetingSegment(RecordedAudioSegment segment)
	{
		...
		var spoken = BuildPlaybackLanguage(segment);
		if (spoken == null)
		{
			return;
		}
	...
	{
		var language = Gameworld.Languages.Get(segment.LanguageId);
		var accent = Gameworld.Accents.Get(segment.AccentId);
	...
	private void EmitBeep()
	{
		...
		foreach (var phone in ConnectedPhones.ToList())
		{
	...
	private void AppendMessageSegment(SpokenLanguageInfo spokenLanguage)
	{
		...
		var now = DateTime.UtcNow;
		var delay = _lastMessageSegmentUtc.HasValue ? now - _lastMessageSegmentUtc.Value : TimeSpan.Zero;
		var segment = RecordedAudioSegment.FromSpokenLanguage(spokenLanguage, delay);
		var recording = new RecordedAudio(_workingMessageSegments.Concat([segment]));
		var stored = new StoredAudioRecording(_activeMessageName, recording, _activeMessageRecordedAtUtc.Value);
	...
	private void FinishMessageRecording()
	{
		...
		{
			var recording = new RecordedAudio(_workingMessageSegments);
	...
	private bool StopGreetingRecording(out string error)
	{
		...
		var stored = new StoredAudioRecording(GreetingRecordingName, new RecordedAudio(_workingGreetingSegments), DateTime.UtcNow);
		...
		string text, IPerceivable? target)
	{
		...
		var now = DateTime.UtcNow;
		var delay = _lastGreetingSegmentUtc.HasValue ? now - _lastGreetingSegmentUtc.Value : TimeSpan.Zero;
		var spoken = new SpokenLanguageInfo(language, accent, volume, text, Outcome.Pass, speaker, target, Parent);
		var segment = RecordedAudioSegment.FromSpokenLanguage(spoken, delay);
	...
	public bool Select(ICharacter character, string argument, IEmote playerEmote, bool silent = false)
	{
		...
		switch (verb)
		{
			...
			case "off":
				...
				return true;
			case "rings":
				if (ss.IsFinished || !int.TryParse(ss.SafeRemainingArgument, out var rings) || rings <= 0)
				{
	...
	private bool SelectGreeting(ICharacter character, StringStack ss)
	{
		switch (ss.PopSpeech().ToLowerInvariant())
		{
			case "record":
				...
				return true;
			case "stop":
				if (!StopGreetingRecording(out var error))
				{
	...
	private bool PlayMessages(ICharacter character)
	{
		...
		foreach (var message in MessageRecordings)
		{
	...
	{
		if (ss.IsFinished || !int.TryParse(ss.SafeRemainingArgument, out var index) || index <= 0)
		{
			character.Send("Which message number do you want to play?");
			return false;
		}

		var recording = MessageRecordings.ElementAtOrDefault(index - 1);
		if (recording == null)
		{
	...
	private bool EraseMessages(ICharacter character, StringStack ss)
	{
		...
		var target = ss.PopSpeech().ToLowerInvariant();
		if (target.EqualTo("all"))
		{
			foreach (var message in MessageRecordings.ToList())
			{
		...
		if (!int.TryParse(target, out var index) || index <= 0)
		{
	...
	public bool CanPut(IGameItem item) => _tapeItem == null && item.GetItemType<IAudioStorageTape>() != null;
	...
	public bool CanTake(ICharacter taker, IGameItem item, int quantity) => _tapeItem == item && item.CanGet(quantity).AsBool();
	...
	public int CanPutAmount(IGameItem item) => _tapeItem == null ? 1 : 0;
	...
	{
		var visited = new HashSet<long> { Parent.Id };
		foreach (var item in _connectedItems.Where(x => x.Item1.Gender != Gender.Male).Select(x => x.Item2.Parent))
		{
			foreach (var phone in GetDownstreamTelephones(item, visited))
			{
				yield return phone;
			}
		}
	...
	private IEnumerable<ITelephone> GetDownstreamTelephones(IGameItem item, HashSet<long> visited)
	{
		...
		foreach (var connectable in item.GetItemTypes<IConnectable>())
		{
			...
			{
				foreach (var otherPhone in GetDownstreamTelephones(other, visited))
				{
