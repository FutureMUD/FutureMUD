...
public class BeltGameItemComponent : GameItemComponent, IBelt
{
    ...
    {
        base.Delete();
        foreach (IBeltable item in ConnectedItems.ToList())
        {
    ...
    {
        foreach (IBeltable item in ConnectedItems)
        {
            item.Parent.Quit();
        }
    }
    ...
    public override double ComponentWeight => ConnectedItems.Sum(x => x.Parent.Weight);
    ...
    public override bool HandleDieOrMorph(IGameItem newItem, ICell location)
    {
        ...
        IBelt newItemBelt = newItem?.GetItemType<IBelt>();
        if (newItemBelt == null)
        {
            foreach (IBeltable item in ConnectedItems.ToList())
            {
    ...
    {
        foreach (XElement element in root.Elements("Connected"))
        {
    ...
    {
        foreach (IBeltable item in _connectedItems)
        {
