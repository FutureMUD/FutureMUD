...
public class BenchGameItemComponent : GameItemComponent, ITable, IFlip, IProvideCover
{
        ...
        : base(parent, proto, temporary)
    {
        ...
        for (int i = 0; i < _prototype.ChairCount; i++)
        {
            IGameItem newItem = _prototype.ChairProto.CreateNew();
            if (!temporary)
            {
                Gameworld.Add(newItem);
            }

            IChair newChair = newItem.GetItemType<IChair>();
    ...
    {
        base.Delete();
        foreach (IChair item in Chairs.ToList())
        {
            _chairs.Remove(item);
            item.Parent.Delete();
        }
        ...
        bool colour, PerceiveIgnoreFlags flags)
    {
        ...
        if (_chairs.Any())
        {
            ...
            foreach (IChair chair in Chairs)
            {
    ...
    public override bool HandleDieOrMorph(IGameItem newItem, ICell location)
    {
        ...
        BenchGameItemComponent newItemBench = newItem?.GetItemType<BenchGameItemComponent>();
    ...
    {
        foreach (IChair item in _chairs)
        {
    ...
    {
        bool oldNoSave = _noSave;
        _noSave = true;
        foreach (XElement element in root.Elements("Chair"))
        {
            IGameItem newItem = Gameworld.TryGetItem(long.Parse(element.Value), true);
            IChair chair = newItem.GetItemType<IChair>();
    ...
    private void DoFlip()
    {
        ...
        if (!_flipped && (Parent.GetItemType<IContainer>()?.Contents.Any() ?? false))
        {
            ...
            IContainer container = Parent.GetItemType<IContainer>();
