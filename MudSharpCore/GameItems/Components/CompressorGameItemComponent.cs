...
public class CompressorGameItemComponent : PoweredMachineBaseGameItemComponent, IConnectable
{
    protected CompressorGameItemComponentProto _prototype;
    public override IGameItemComponentProto Prototype => _prototype;

    protected override void UpdateComponentNewPrototype(IGameItemComponentProto newProto)
    {
        _prototype = (CompressorGameItemComponentProto)newProto;
    }

    #region Constructors

    public CompressorGameItemComponent(CompressorGameItemComponentProto proto, IGameItem parent, bool temporary = false)
        : base(proto, parent, temporary)
    {
        _prototype = proto;
    }

    public CompressorGameItemComponent(MudSharp.Models.GameItemComponent component,
        CompressorGameItemComponentProto proto, IGameItem parent) : base(component, proto, parent)
    {
        _prototype = proto;
        _noSave = true;
    ...
    public CompressorGameItemComponent(CompressorGameItemComponent rhs, IGameItem newParent, bool temporary = false) :
    ...
    {
        return new CompressorGameItemComponent(this, newParent, temporary);
    }
    ...
    {
        IGameItem downstream = ConnectedItems.FirstOrDefault(x => x.Item1.Gender == Gender.Male)?.Item2?.Parent;
        if (downstream == null)
        {
            return;
        }

        IGasContainer cannister = downstream.GetItemType<IGasContainer>();
        if (cannister == null)
        {
            return;
        }

        IGameItem upstream = ConnectedItems.FirstOrDefault(x => x.Item1.Gender == Gender.Female)?.Item2?.Parent;
        IGasSupply supplyCannister = upstream?.GetItemType<IGasSupply>();
        if (supplyCannister == null)
        {
            // If not connected to a gas supply, compress the atmosphere instead
            if (Parent.TrueLocations.First().Atmosphere is not IGas gas)
            {
                return;
            }

            if (cannister.Gas != null && cannister.Gas != gas)
            {
                return;
            }

            cannister.Gas = gas;
            cannister.GasVolumeAtOneAtmosphere = Math.Min(cannister.GasVolumeAtOneAtmosphere + _prototype.FlowRate,
                cannister.GasCapacityAtOneAtmosphere);
            return;
        }

        if (supplyCannister.Gas == null)
        {
            return;
        }

        if (cannister.Gas != null && cannister.Gas != supplyCannister.Gas)
        {
            return;
        }

        if (cannister.GasVolumeAtOneAtmosphere >= cannister.GasCapacityAtOneAtmosphere)
        {
            return;
        }

        double amount = Math.Min(_prototype.FlowRate,
            cannister.GasCapacityAtOneAtmosphere - cannister.GasVolumeAtOneAtmosphere);
        if (supplyCannister.CanConsumeGas(amount))
        {
            supplyCannister.ConsumeGas(amount);
            cannister.GasVolumeAtOneAtmosphere += amount;
        }
    }
    ...
    public override void FinaliseLoad()
    {
        foreach (Tuple<long, ConnectorType> item in _pendingLoadTimeConnections.ToList())
        {
            ...
            List<IConnectable> connectables = gitem.GetItemTypes<IConnectable>().ToList();
            if (!connectables.Any())
            {
                continue;
            }
