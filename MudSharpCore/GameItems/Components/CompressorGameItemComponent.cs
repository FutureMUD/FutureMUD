...
public class CompressorGameItemComponent : PoweredMachineBaseGameItemComponent, IConnectable
{
	...
	{
		var downstream = ConnectedItems.FirstOrDefault(x => x.Item1.Gender == Gender.Male)?.Item2?.Parent;
		if (downstream == null)
		{
			return;
		}

		var cannister = downstream.GetItemType<IGasContainer>();
		if (cannister == null)
		{
			return;
		}

		var upstream = ConnectedItems.FirstOrDefault(x => x.Item1.Gender == Gender.Female)?.Item2?.Parent;
		var supplyCannister = upstream?.GetItemType<IGasSupply>();
		...
		var amount = Math.Min(_prototype.FlowRate,
