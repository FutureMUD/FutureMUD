...
public class CurrencyGameItemComponent : GameItemComponent, ICurrencyPile
{
        ...
        rhs, newParent, temporary)
    {
        ...
        foreach (KeyValuePair<ICoin, int> coin in rhs._coins)
        {
        ...
        switch (type)
        {
            case DescriptionType.Short:
                switch (_coins.Count)
                {
                    ...
                    case 1:
                        string generalName = _coins.Select(x => x.Key.GeneralForm).Distinct().Single();
    ...
    public override double ComponentWeight => _coins.Sum(x => x.Key.Weight * x.Value);
    ...
    private void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement item in root.Element("Coins").Elements("Coin"))
        {
            ICoin coin = Currency.Coins.FirstOrDefault(x => x.Id == long.Parse(item.Attribute("Id").Value));
            if (coin == null)
            {
                continue;
            }
    ...
    public IEnumerable<Tuple<ICoin, int>> Coins => _coins.Select(x => Tuple.Create(x.Key, x.Value));
    ...
    public bool RemoveCoins(IEnumerable<Tuple<ICoin, int>> coins)
    {
        ...
        foreach (KeyValuePair<ICoin, int> item in _coins.Where(x => x.Value < 1).ToList())
        {
