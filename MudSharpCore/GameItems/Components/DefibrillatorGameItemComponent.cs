...
public class DefibrillatorGameItemComponent : GameItemComponent, IDefibrillator
{
    ...
    public void Shock(ICharacter shocker, IBody target)
    {
        ...
        shocker.OutputHandler.Handle(new EmoteOutput(new Emote(_prototype.DefibrillationEmote, shocker, shocker,
            target.Actor, Parent)));
        List<Tuple<HeartProto, double>> heartFunctions = target.Organs.OfType<HeartProto>()
                                   .Select(x => Tuple.Create(x, x.OrganFunctionFactor(target))).ToList();
        double heartFunction = heartFunctions.Select(x => x.Item2).DefaultIfEmpty(0).Sum();

        // Used on Healthy heart
        if (heartFunction >= 0.3)
        {
            double damageAmount = Gameworld.GetStaticDouble("DefibrillatorBaseDamage") +
        ...
        ICheck check = Gameworld.GetCheck(CheckType.Defibrillate);
                ...
                : "DefibrillatorStabiliseChance"))
        {
            foreach (Tuple<HeartProto, double> heart in heartFunctions)
            {
                ...
                {
                    bool canBreathe = target.CanBreathe;
