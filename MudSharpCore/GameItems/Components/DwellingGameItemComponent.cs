...
public class DwellingGameItemComponent : GameItemComponent
{
    ...
    {
        Dictionary<ICell, ICell> cellMap = new();
        CellOverlayPackage package = new(Gameworld, null, $"Autogenerated Dwelling Package {DateTime.UtcNow:R}");
        Gameworld.Add(package);
        Room baseRoom = new(Parent.Location.Room.Zone, package, _prototype.TemplateEntryCell, true);
        ICell baseCell = baseRoom.Cells.First();
        cellMap[_prototype.TemplateEntryCell] = baseCell;
        List<ICell> baseCells = _prototype.GetTemplateCells.ToList();
        foreach (ICell cell in baseCells.Except(baseCell))
        {
            Room newRoom = new(Parent.Location.Room.Zone, package, cell, true);
            cellMap[cell] = newRoom.Cells.First();
        }

        List<IExit> exitsAlreadyBuilt = new();
        foreach (ICell cell in baseCells)
        {
            foreach (ICellExit exit in cell.ExitsFor(null))
            {
                ...
                Exit newExit = new(Gameworld, cellMap[cell], cellMap[exit.Destination], (Exit)exit.Exit);
        ...
        foreach (KeyValuePair<ICell, ICell> cell in cellMap)
        {
            Gameworld.ExitManager.UpdateCellOverlayExits(cell.Value, cell.Value.CurrentOverlay);
        }

        Exit frontExit = new(Gameworld, Parent.Location, baseCell, 1.0,
        ...
        IEditableCellOverlay overlay = Parent.Location.GetOrCreateOverlay(Parent.Location.CurrentOverlay.Package);
        IEditableCellOverlay otherOverlay = baseCell.GetOrCreateOverlay(baseCell.CurrentOverlay.Package);
        ...
        {
            IGameItem newItem = _prototype.DoorProto.CreateNew(null);
    ...
    private void DestroyCells()
    {
        ...
        ICell fallbackCell = Gameworld.ExitManager.GetAllExits(EntranceCell)
                                    .First(x => !x.Destination.Temporary &&
                                                x.OutboundDirection == CardinalDirection.Unknown).Destination;
        foreach (ICell cell in EntranceCell.CellsInVicinity(10U, exit => true, cell => cell.Temporary))
        {
