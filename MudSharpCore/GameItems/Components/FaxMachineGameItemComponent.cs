...
public class FaxMachineGameItemComponent : TelephoneGameItemComponent, IFaxMachine, IContainer, IOpenable, ILockable
{
	...
	private void LoadFaxState(XElement root)
	{
		...
		var lockElement = root.Element("Locks");
		if (lockElement != null)
		{
			foreach (var item in lockElement.Elements("Lock")
		...
		var pendingRoot = root.Element("PendingFaxes");
		if (pendingRoot == null)
		{
			return;
		}

		foreach (var faxElement in pendingRoot.Elements("Fax"))
		{
			var senderNumber = faxElement.Attribute("sender")?.Value ?? "Unknown";
			var readables = new List<ICanBeRead>();
			foreach (var readableElement in faxElement.Elements())
			{
	...
	public void ReceiveFax(string senderNumber, IReadOnlyCollection<ICanBeRead> document)
	{
		...
		var job = new IncomingFaxJob(senderNumber, document.ToList());
	...
	public override double ComponentBuoyancy(double fluidDensity) =>
		_contents.Sum(x => x.Buoyancy(fluidDensity)) + _locks.Sum(x => x.Parent.Buoyancy(fluidDensity));
	...
	public override bool HandleEvent(EventType type, params dynamic[] arguments)
	{
		if (type == EventType.CommandInput && ((string)arguments[2]).Equals("refill"))
		{
			...
			var ink = ch.Body.HeldOrWieldedItems.FirstOrDefault(x => x.Prototype.Id == _prototype.InkCartridgePrototypeId);
			if (ink == null)
			{
	...
	private void TryPrintPendingFaxes()
	{
		...
		var printedPages = new List<IGameItem>();
		var ranOutOfPaper = false;
		var ranOutOfInk = false;

		foreach (var fax in _pendingFaxes.ToList())
		{
			IGameItem? currentSheet = null;
			var currentWritable = default(IWriteable);
			...
			while (fax.RemainingReadables.Any())
			{
				...
				var readable = fax.RemainingReadables[0];
				var added = readable switch
				{
