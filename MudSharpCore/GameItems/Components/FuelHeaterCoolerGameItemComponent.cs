...
public class FuelHeaterCoolerGameItemComponent : SwitchableThermalSourceGameItemComponent, IConnectable
{
    ...
    public override void FinaliseLoad()
    {
        ...
        IGameItem? item = Gameworld.Items.Get(_pendingConnectedItemId) ?? Gameworld.TryGetItem(_pendingConnectedItemId, true);
        if (item is null)
        {
            return;
        }

        IConnectable? connectable = item.GetItemTypes<IConnectable>().FirstOrDefault(x => x != this && CanConnect(null, x));
    ...
    {
        foreach (IConnectable? item in _connectedItems.Select(x => x.Item2).ToList())
        {
    ...
    internal void BurnFuel(double seconds)
    {
        ...
        {
            ILiquidContainer? liquid = GetConnectedLiquidContainer();
        ...
        IGasSupply? gasSupply = GetConnectedGasSupply();
