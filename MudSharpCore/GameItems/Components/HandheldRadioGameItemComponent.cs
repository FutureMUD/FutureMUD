    ...
    IOnOff
{
        ...
        ITransmit origin)
    {
        ...
        {
            List<EarProto> wornEars = new();
            foreach ((IGameItem Item, IWear Wearloc, IWearlocProfile Profile) wear in Parent.InInventoryOf?.WornItemsFullInfo.Where(x => x.Item1 == Parent) ?? [])
            {
                foreach (EarProto organ in wear.Item2.Organs.OfType<EarProto>())
                {
                    wornEars.Add(organ);
                }
            }
        ...
        SpokenLanguageInfo spoken = new(spokenLanguage, Volume, Parent);
        if (!inEars)
        {
