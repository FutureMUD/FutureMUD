...
public class ImplantBaseGameItemComponent : GameItemComponent, IImplant
{
    ...
    {
        XElement element = root.Element("OverridenBodypart");
        if (element != null)
        {
            IBodypart part = Gameworld.BodypartPrototypes.Get(long.Parse(element.Value));
            if (part != null)
            {
    ...
    public double FunctionFactor
    {
        get
        {
            ...
            double health = Parent.HealthStrategy.CurrentHealthPercentage(Parent);
