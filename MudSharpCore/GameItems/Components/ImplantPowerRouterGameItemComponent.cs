...
public class ImplantPowerRouterGameItemComponent : GameItemComponent, IImplantPowerPlant, IImplantReportStatus
{
    ...
    public void InstallImplant(IBody body)
    {
        ...
        if (_powered)
        {
            foreach (IConsumePower implant in body.Implants.Except(this).SelectNotNull(x => x.Parent.GetItemType<IConsumePower>())
                                        .ToList())
            {
    ...
    public void RemoveImplant()
    {
        if (_powered)
        {
            foreach (IConsumePower implant in InstalledBody.Implants.Except(this)
    ...
    public void OnPowerCutIn()
    {
        ...
        {
            _powered = true;
            foreach (IConsumePower item in _connectedConsumers.ToList())
            {
                item.OnPowerCutIn();
            }
        }
    ...
    private void CheckPowerPlants()
    {
        if (InstalledBody != null)
        {
            ...
            List<IImplantPowerPlant> installed = InstalledBody.Implants.OfType<IImplantPowerPlant>().ToList();
            foreach (IImplantPowerPlant implant in installed)
            {
            ...
            foreach (IImplantPowerPlant implant in _connectedPowerPlants.ToList())
            {
            ...
            if (!_powered)
            {
                foreach (IImplantPowerPlant item in _connectedPowerPlants)
                {
    ...
    {
        CheckPowerPlants();
        foreach (IImplantPowerPlant parent in _connectedPowerPlants)
        {
    ...
    public string ReportStatus()
    {
        ...
        foreach (IImplantPowerPlant pp in _connectedPowerPlants)
        {
            IImplantRespondToCommands iirtc = pp as IImplantRespondToCommands;
            sb.AppendLine(
        ...
        foreach (IImplant consumer in _connectedConsumers.OfType<IImplant>())
        {
            IImplantRespondToCommands iirtc = consumer as IImplantRespondToCommands;
            sb.AppendLine(
