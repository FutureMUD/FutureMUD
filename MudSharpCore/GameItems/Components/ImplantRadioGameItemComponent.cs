    ...
    IImplantRespondToCommands, ITransmit, IReceive
{
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement freq in root.Element("Listening").Elements())
        {
            _listeningFrequencies.Add(double.Parse(freq.Value));
        }

        foreach (XElement freq in root.Element("Broadcasting").Elements())
        {
    ...
    {
        XElement definition = SaveToXmlNoTextConversion();
    ...
    protected bool TransmittedThisTick
    {
        get
        {
            bool value = _transmittedThisTick;
    ...
    private void CommandListen(StringStack arguments)
    {
        ...
        List<string> removedChannels = new();

        string argument;
        do
        {
            argument = arguments.PopSpeech();
            int index = _prototype.ChannelNames.FindIndex(x => x.EqualTo(argument));
            if (index == -1)
            {
            ...
            string name = _prototype.ChannelNames[index];
            double freq = _prototype.Channels[index];
    ...
    {
        ICharacter actor = InstalledBody.Actor;
        if (!_powered)
        {
        ...
        SpokenLanguageInfo langInfo = new(actor.CurrentLanguage, actor.CurrentAccent, Form.Audio.AudioVolume.Decent,
    ...
    private void CommandEncryption(StringStack arguments)
    {
        ...
        if (!long.TryParse(arguments.PopSpeech(), out long value))
        {
