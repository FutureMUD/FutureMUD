    ...
    IImplantRespondToCommands, ITelephone, ITelephoneNumberOwner, ICanConnectToTelecommunicationsGrid
{
    ...
    {
        string numberText = PhoneNumber?.ColourValue() ?? "unassigned".ColourError();
        string gridText = TelecommunicationsGrid == null
            ? "no telecommunications grid".ColourError()
            : $"grid #{TelecommunicationsGrid.Id:N0}".ColourValue();
        string callState = IsConnected
    ...
    private void CommandDial(StringStack arguments)
    {
        ...
        string number = arguments.SafeRemainingArgument;
        if (!Dial(Actor, number, out string? error))
        {
    ...
    private void CommandAnswer(StringStack arguments)
    {
        ...
        if (!Answer(Actor, out string? error))
        {
            SendInternalMessage(error);
            return;
        }
    ...
    private void CommandPickup(StringStack arguments)
    {
        ...
        if (!PickUp(Actor, out string? error))
        {
            SendInternalMessage(error);
            return;
        }
    ...
    private void CommandHangup(StringStack arguments)
    {
        ...
        if (!HangUp(Actor, out string? error))
        {
            SendInternalMessage(error);
            return;
        }
    ...
    private void CommandTransmit(StringStack arguments)
    {
        ...
        SpokenLanguageInfo langInfo = new(
