...
public class LiquidConsumingPropGameItemComponent : GameItemComponent, ILiquidContainer, IConnectable
{
	...
	private void LoadFromXml(XElement root)
	{
		...
		{
			var liquid = Gameworld.Liquids.Get(long.Parse(root.Element("Liquid")!.Value));
			if (liquid != null)
			{
	...
	public void MergeLiquid(LiquidMixture otherMixture, ICharacter who, string action)
	{
		...
		{
			var remainingCapacity = LiquidCapacity - LiquidMixture.TotalVolume;
	...
	public bool CanOpen(IBody opener) => false;
	public WhyCannotOpenReason WhyCannotOpen(IBody opener) => WhyCannotOpenReason.NotOpenable;
	public void Open()
	{
	}
	public bool CanClose(IBody closer) => false;
	public WhyCannotCloseReason WhyCannotClose(IBody closer) => WhyCannotCloseReason.NotOpenable;
	public void Close()
	{
	}
