...
public class LockingDoorGameItemComponent : GameItemComponent, IDoor, ILock
{
    ...
    public override bool HandleDieOrMorph(IGameItem newItem, ICell location)
    {
        ...
        {
            foreach (ILock theLock in Locks.Where(theLock => !newItemLockable.InstallLock(theLock)))
            {
                if (location == null)
                {
                    theLock.Delete();
        ...
        IDoor newItemDoor = newItem?.GetItemType<IDoor>();
        if (newItemDoor != null)
        {
    ...
    {
        XAttribute attr = root.Attribute("State");
        if (attr != null)
        {
        ...
        XElement element = root.Element("HingeCell");
        if (element != null)
        {
        ...
        foreach (XElement sub in root.Elements("Lock"))
        {
            IGameItem newItem = Gameworld.TryGetItem(long.Parse(sub.Value), true);
    ...
    {
        XElement definition = new("Definition", new XAttribute("State", (int)State), from theLock in _locks
    ...
    {
        ICellExit targetExit = InstalledExit.CellExitFor(actor.Location);
        actor.OutputHandler.Handle(
            new MixedEmoteOutput(new Emote("@ knock|knocks upon $1", actor, actor, Parent)).Append(playerEmote));
        ICellExit oppositeExit = targetExit.Exit.CellExitFor(targetExit.Destination);
        ...
        foreach (IHandleEvents witness in targetExit.Origin.EventHandlers.Except(actor))
        {
            witness.HandleEvent(EventType.CharacterDoorKnockedSameSide, actor, actor.Location, targetExit,
                witness);
        }

        foreach (IGameItem witness in actor.Body.ExternalItems)
        {
            witness.HandleEvent(EventType.CharacterDoorKnockedSameSide, actor, actor.Location, targetExit,
                witness);
        }

        foreach (IHandleEvents witness in targetExit.Destination.EventHandlers)
        {
