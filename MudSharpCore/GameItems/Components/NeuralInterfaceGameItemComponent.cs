...
public class NeuralInterfaceGameItemComponent : ImplantBaseGameItemComponent, IImplantNeuralLink
{
    ...
    protected override string SaveToXml()
    {
        ...
        XElement basevalue = SaveToXmlNoTextConversion();
    ...
    {
        FinaliseLoadTimeConnectedImplants();
        IImplantRespondToCommands implant = _connectedImplants.OfType<IImplantRespondToCommands>()
        ...
        if (command.EqualTo("alias"))
        {
            ...
            string newalias = arguments.PopSpeech().ToLowerInvariant();
    ...
    {
        foreach (long id in _pendingImplantConnections)
        {
            IImplant implant = InstalledBody.Implants.FirstOrDefault(x => x.Id == id);
    ...
    public void DoReportStatus()
    {
        ...
        foreach (IImplantReportStatus implant in _connectedImplants.OfType<IImplantReportStatus>())
        {
            // TODO - should implants have their own "internal" sdesc separate to their regular item one?
            IImplantRespondToCommands iirtc = implant as IImplantRespondToCommands;
            sb.AppendLine();
            sb.AppendLine(
