...
namespace MudSharp.GameItems.Components
{
    public class PuddleGameItemComponent : GameItemComponent, ILiquidContainer
    {
        ...
        public static ITag PuddleResidueTag(IFuturemud gameworld)
        {
            return gameworld.Tags.Get(gameworld.GetStaticLong("PuddleResidueTagId"));
        }
        ...
        public static double EvaporationRatePerSecond(double liquidVolume, double liquidRelativeEnthalpy, double airTemperature, Climate.PrecipitationLevel precipitation, Climate.WindLevel wind)
        {
            ...
            double tempKelvin = airTemperature + 273.0;
            ...
            double maximumHumidityRatio = 0.62198 * saturationPressurePascals / (1013000 - saturationPressurePascals);
            // TODO - 1013000 is hardcoded 1013 barometric pressure. Get actual barometric pressure from atmosphere model

            double airVelocityMetersPerSecond = 0.0;
            switch (wind)
            {
            ...
            double evaporationCoefficient = 25 + 19 * airVelocityMetersPerSecond * 1000.0;
            double evaporationPerSecondPerUnitArea = evaporationCoefficient * (maximumHumidityRatio - (maximumHumidityRatio * humidity)) / 3600.0;
        ...
        private void DoFiveSecondUpdate()
        {
            ...
            double ambientTemperature = location.CurrentTemperature(Parent);
            IWeatherEvent weather = location.CurrentWeather(Parent);
            double totalEvaporation = ChangingNeedsModelBase.StaticRealSecondsToInGameSeconds * 5.0 * EvaporationRatePerSecond(LiquidVolume, LiquidMixture.RelativeEnthalpy, ambientTemperature, weather?.Precipitation ?? Climate.PrecipitationLevel.Parched, weather?.Wind ?? Climate.WindLevel.None);
            if (totalEvaporation <= 0.0)
            {
                return;
            }

            LiquidMixture removed = LiquidMixture.RemoveLiquidVolume(totalEvaporation);
            List<(LiquidInstance Instance, double)> ratios = removed.Instances
                                .Select(x => (Instance: x, x.Amount / removed.TotalVolume))
                                .ToList();
            ITag residueTag = PuddleResidueTag(Gameworld);
            foreach ((LiquidInstance instance, double amount) in ratios)
            {
                ...
                double residueWeight = amount * instance.Liquid.ResidueVolumePercentage;
                ...
                case DescriptionType.Full:
                    if (LiquidMixture?.IsEmpty == false)
                    {
                        if (voyeur is ICharacter ch && ch.IsAdministrator())
                        {
                            ...
                            double ambientTemperature = location.CurrentTemperature(Parent);
                            IWeatherEvent weather = location.CurrentWeather(Parent);
                            double totalEvaporation = EvaporationRatePerSecond(LiquidVolume, LiquidMixture.RelativeEnthalpy, ambientTemperature, weather?.Precipitation ?? Climate.PrecipitationLevel.Parched, weather?.Wind ?? Climate.WindLevel.None);
...
It is best described as {PuddleDescription(LiquidMixture.TotalVolume).A_An()} containing approximately {Gameworld.UnitManager.DescribeMostSignificant(LiquidMixture.TotalVolume, Framework.Units.UnitType.FluidVolume, voyeur).ColourValue()} of {LiquidMixture.ColouredLiquidDescription}.";
        ...
        public override double ComponentWeight => (LiquidMixture?.TotalWeight ?? 0.0);
        ...
        public override bool HandleDieOrMorph(IGameItem newItem, ICell location)
        {
            ...
            ILiquidContainer newItemLiquid = newItem?.GetItemType<ILiquidContainer>();
