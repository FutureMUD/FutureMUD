...
public class SmokeableGameItemComponent : GameItemComponent, ISmokeable
{
    ...
    {
        int previousAmount = RemainingFuel;
        ...
        ICharacter character = Parent.GetItemType<IHoldable>()?.HeldBy?.Actor;
        if (character != null && !string.IsNullOrWhiteSpace(_prototype.PlayerDescriptionEffectString))
        {
            IDescriptionAdditionEffect playerEffect =
                character
        ...
        {
            IDescriptionAdditionEffect roomEffect = location.EffectsOfType<IDescriptionAdditionEffect>()
                                     .FirstOrDefault(x =>
