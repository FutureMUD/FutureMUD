...
public class VendingMachineGameItemComponent : GameItemComponent, IContainer, IVendingMachine, IInsertable, IOnOff
{
    ...
    private void VendingMachineCommandAdd(ICharacter character, StringStack command)
    {
        ...
        string keyword = command.PopSpeech().ToLowerInvariant();
        ...
        string costText = command.PopSpeech();
        decimal result = _currency.GetBaseCurrency(costText, out bool success);
        if (!success)
        {
            character.Send(
        ...
        string description = command.PopSpeech();
    ...
    private void VendingMachineCommandSwap(ICharacter character, StringStack command)
    {
        ...
        string firstChoice = command.PopSpeech();
        if (command.IsFinished)
        {
        ...
        string secondChoice = command.PopSpeech();
    ...
    private void VendingMachineCommandRemove(ICharacter character, StringStack command)
    {
        ...
        string choice = command.PopSpeech();
    ...
    {
        XElement element = root.Element("Currency");
        if (element != null)
        {
        ...
        foreach (XElement item in root.Elements("Selection"))
        {
        ...
        using (new FMDB())
        {
            foreach (XElement item in root.Elements("Contained"))
            {
    ...
    public bool Refund(ICharacter character)
    {
        ...
        Dictionary<ICoin, int> coins = _currency.FindCoinsForAmount(_currentBalance, out bool exact);

        IGameItem newCurrencyPile = CurrencyGameItemComponentProto.CreateNewCurrencyPile(_currency,
    ...
    public bool Select(ICharacter character, string argument, IEmote playerEmote, bool silent)
    {
        ...
        IGameItem newItem = selection.Prototype.CreateNew(character, null, 1, selection.LoadArguments).First();
        newItem.Login();
