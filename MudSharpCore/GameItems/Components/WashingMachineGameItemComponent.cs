    ...
    IConsumePower, ILockable
{
    ...
    public override double ComponentWeight => Contents.Sum(x => x.Weight) + Locks.Sum(x => x.Parent.Weight) + LiquidMixture?.TotalWeight ?? 0.0;
    ...
    protected void LoadFromXml(XElement root)
    {
        ...
        if (attr != null)
        {
            ILiquid liquid = Gameworld.Liquids.Get(long.Parse(attr.Value));
            if (liquid != null)
            {
    ...
    public void Put(ICharacter putter, IGameItem item, bool allowMerge = true)
    {
        ...
        if (allowMerge)
        {
            IGameItem mergeTarget = _laundryContents.FirstOrDefault(x => x.CanMerge(item));
            if (mergeTarget != null)
            {
    ...
    public WhyCannotPutReason WhyCannotPut(IGameItem item)
    {
        ...
        {
            int capacity = (int)((_prototype.WeightCapacity - _laundryContents.Sum(x => x.Weight)) /
