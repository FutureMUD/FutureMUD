...
public class WaterSourceGameItemComponent : GameItemComponent, ILiquidContainer, ILockable, IOnOff, ISwitchable
{
    ...
    private void LiquidHeartbeat()
    {
        ...
        double amount = Math.Min(_prototype.LiquidCapacity - LiquidMixture.TotalVolume, _prototype.RefillRate);
    ...
    public override double ComponentWeight => Locks.Sum(x => x.Parent.Weight) + (LiquidMixture?.TotalWeight ?? 0.0);
