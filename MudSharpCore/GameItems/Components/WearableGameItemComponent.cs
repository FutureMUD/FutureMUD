...
public class WearableGameItemComponent : GameItemComponent, IWearable
{
    ...
    public override bool HandleDieOrMorph(IGameItem newItem, ICell location)
    {
        ...
        IHoldable held = Parent.GetItemType<IHoldable>();
        held?.HeldBy = null;

        WearableGameItemComponent newItemWearable = newItem?.GetItemType<WearableGameItemComponent>();
    ...
    public bool GloballyTransparent
    {
        get
        {
            IDestroyable destroyable = Parent.GetItemType<IDestroyable>();
            if (destroyable == null)
            {
                return false;
            }
            ...
            double ratio = 1.0 - Parent.Wounds.Sum(x => x.CurrentDamage) / destroyable.MaximumDamage;
    ...
    {
        IDestroyable destroyable = Parent.GetItemType<IDestroyable>();
        double ratio = 1.0 - Parent.Wounds.Sum(x => x.CurrentDamage) / destroyable?.MaximumDamage ?? 1.0;
        ...
        switch (type)
        {
            ...
            case DescriptionType.Evaluate:
                ...
                foreach (IWearProfile profile in Profiles.OrderByDescending(x => x == DefaultProfile).ThenBy(x => x.Name))
                {
                    sb.AppendLine();
                    sb.AppendLine(
                        $"{profile.Name.TitleCase().ColourName()} for the {profile.DesignedBody.Name.TitleCase().ColourValue()} body");
                    foreach (KeyValuePair<IWear, IWearlocProfile> loc in profile.AllProfiles)
                    {
