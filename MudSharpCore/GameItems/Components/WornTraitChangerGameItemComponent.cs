...
public class WornTraitChangerGameItemComponent : GameItemComponent, IChangeTraitsInInventory
{
    ...
    {
        bool worn = Parent.GetItemType<IWearable>()?.WornBy != null;
        StringBuilder sb = new(description);
        if (type == DescriptionType.Evaluate)
        {
            foreach ((ITraitDefinition trait, TraitBonusContext context, double modifier) in _prototype.TraitModifiers)
            {
                sb.AppendLine(
