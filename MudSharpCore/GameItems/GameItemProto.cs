...
public class GameItemProto : EditableItem, IGameItemProto
{
        ...
        : base(originator)
    {
        using (new FMDB())
        {
            ...
            {
                IGameItemComponentProto holdable = gameworld.ItemComponentProtos.First(x => x is HoldableGameItemComponentProto);
    ...
    public override string Show(ICharacter actor)
    {
        ...
        if (Components.Any())
        {
            foreach (IGameItemComponentProto component in _components)
            {
        ...
        if (ExtraDescriptions.Any())
        {
            ...
            int count = 1;
            foreach ((IFutureProg Prog, string ShortDescription, string FullDescription, string FullDescriptionAddendum) desc in ExtraDescriptions)
            {
                sb.AppendLine();
                sb.AppendLine(
        ...
        if (DefaultVariables.Any())
        {
            ...
            foreach (KeyValuePair<string, string> item in DefaultVariables)
            {
    ...
    public IEnumerable<IGameItem> CreateNew<T>(ICharacter loader, IGameItemSkin skin, int quantity, T variables) where T : IEnumerable<(ICharacteristicDefinition Definition, ICharacteristicValue Value)>
    {
        ...
        items.Add(newItem);
        newItem.Skin = skin;
        VariableGameItemComponentProto varProto = GetItemType<VariableGameItemComponentProto>();
        IVariable variable = newItem.GetItemType<IVariable>();
        if (varProto is not null)
        {
            foreach (ICharacteristicDefinition characteristic in variable.CharacteristicDefinitions)
            {
                ICharacteristicValue value = variables.FirstOrDefault(x => x.Definition == characteristic).Value;
    ...
    public IEnumerable<IGameItem> CreateNew(ICharacter loader, IGameItemSkin skin, int quantity, string loadString)
    {
        ...
        if (varProto is not null)
        {
            Dictionary<ICharacteristicDefinition, ICharacteristicValue> variables = varProto.GetValuesFromString(loadString);
            foreach (ICharacteristicDefinition characteristic in variable.CharacteristicDefinitions)
            {
                if (variables.TryGetValue(characteristic, out ICharacteristicValue value))
                {
                    variable.SetCharacteristic(characteristic, value);
                }
                else
    ...
    public override IEditableRevisableItem CreateNewRevision(ICharacter initiator)
    {
        using (new FMDB())
        {
            ...
            foreach (IGameItemComponentProto component in _components)
            {
                Models.GameItemComponentProto dbcomponent = FMDB.Context.GameItemComponentProtos.Find(component.Id, component.RevisionNumber);
            ...
            foreach (KeyValuePair<string, string> item in DefaultVariables)
            {
                GameItemProtosDefaultVariable dbvar = new()
                {
            ...
            int order = 0;
            foreach ((IFutureProg Prog, string ShortDescription, string FullDescription, string FullDescriptionAddendum) item in ExtraDescriptions)
            {
                dbnew.ExtraDescriptions.Add(new GameItemProtoExtraDescription
                {
                    ApplicabilityProgId = item.Prog.Id,
                    ShortDescription = item.ShortDescription,
                    FullDescription = item.FullDescription,
                    FullDescriptionAddendum = item.FullDescriptionAddendum,
                    Priority = order++
                });
            }
            ...
            foreach (IFutureProg prog in OnLoadProgs)
            {
                Models.FutureProg dbprog = FMDB.Context.FutureProgs.Find(prog.Id);
                if (dbprog != null)
                {
    ...
    private void LoadFromDatabase(MudSharp.Models.GameItemProto proto, IFuturemud gameworld)
    {
        ...
        foreach (GameItemProtosTags tag in proto.GameItemProtosTags)
        {
            _tags.Add(Gameworld.Tags.Get(tag.TagId));
        }

        foreach (GameItemProtosDefaultVariable item in proto.GameItemProtosDefaultVariables)
        {
            _defaultVariables[item.VariableName] = item.VariableValue;
        }

        foreach (GameItemProtoExtraDescription item in proto.ExtraDescriptions.OrderBy(x => x.Priority))
        {
        ...
        foreach (GameItemProtosOnLoadProgs prog in proto.GameItemProtosOnLoadProgs)
        {
            IFutureProg fprog = Gameworld.FutureProgs.Get(prog.FutureProgId);
            if (fprog == null ||
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            ...
            foreach (IFutureProg item in OnLoadProgs)
            {
                Models.FutureProg dbprog = FMDB.Context.FutureProgs.Find(item.Id);
                if (dbprog != null)
                {
    ...
    public IGameItem LoadDestroyedItem(IGameItem originalItem)
    {
        ...
        IGameItemProto proto = Gameworld.ItemProtos.Get(_onDestroyedGameItemProto);
        if (proto == null || proto.Status != RevisionStatus.Current)
        {
            return null;
        }

        IGameItem newItem = proto.CreateNew();
        newItem.RoomLayer = originalItem.RoomLayer;

        if (PreserveRegisterVariables)
        {
            foreach (Tuple<string, ProgVariableTypes> variable in Gameworld.VariableRegister.AllVariables(ProgVariableTypes.Item))
            {
    ...
    public IGameItem LoadMorphedItem(IGameItem originalItem)
    {
        ...
        IGameItemProto proto = Gameworld.ItemProtos.Get(_onMorphGameItemProto);
        if (proto == null || proto.Status != RevisionStatus.Current)
        {
            return null;
        }
    ...
    public bool CheckForComponentPrototypeUpdates()
    {
        ...
        bool updated = false;
        foreach (IGameItemComponentProto comp in _components.ToList())
        {
            if (comp.Status.In(RevisionStatus.Obsolete, RevisionStatus.Revised))
            {
                IGameItemComponentProto newProto =
                ...
                GameItemProtosGameItemComponentProtos protoToRemove =
                ...
                dbitem.GameItemProtosGameItemComponentProtos.Add(new GameItemProtosGameItemComponentProtos
                {
                    ...
                    GameItemComponentProtoId = newProto.Id,
    ...
    {
        string subCommand = command.PopForSwitch();
        switch (subCommand)
        {
            case "hidden":
    ...
    private bool BuildingCommandCost(ICharacter actor, StringStack command)
    {
        ...
        if (!actor.Currency.TryGetBaseCurrency(command.SafeRemainingArgument, out decimal amount) || amount < 0.0M)
        {
    ...
    private bool BuildingCommandSuggestDesc(ICharacter actor, StringStack command)
    {
        ...
        if (vp is not null)
        {
            ...
            foreach (ICharacteristicDefinition variable in vp.CharacteristicDefinitions)
            {
                ICharacteristicProfile profile = vp.ProfileFor(variable);
                List<ICharacteristicValue> random = new()
                {
    ...
    private bool BuildingCommandExtra(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.Last, out int value))
        {
	...
	<which#> clear addenudm - clears the addendum text for the full description");
        ...
        switch (command.PopSpeech().ToLowerInvariant())
        {
            ...
            case "shortdescription":
                ...
                (IFutureProg Prog, string ShortDescription, string FullDescription, string FullDescriptionAddendum) old = _extraDescriptions[value - 1];
            ...
            case "prog":
                ...
                IFutureProg prog = long.TryParse(command.PopSpeech(), out long progid)
    ...
    private bool BuildingCommandExtraSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int index1) || index1 < 1 || index1 > _extraDescriptions.Count)
        {
            actor.OutputHandler.Send(
        ...
        if (!int.TryParse(command.PopSpeech(), out int index2) || index2 < 1 || index2 > _extraDescriptions.Count)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandQuality(ICharacter actor, StringStack command)
    {
        ...
        if (!GameItemEnumExtensions.TryParseQuality(command.SafeRemainingArgument, out ItemQuality quality))
        {
            actor.Send(
    ...
    private bool BuildingCommandStrategy(ICharacter actor, StringStack command)
    {
        ...
        IHealthStrategy strategy = long.TryParse(command.Last, out long value)
    ...
    private bool BuildingCommandItemGroup(ICharacter actor, StringStack command)
    {
        ...
        IGameItemGroup itemgroup = long.TryParse(text, out long value)
    ...
    private bool BuildingCommandSize(ICharacter actor, StringStack command)
    {
        ...
        IEnumerable<SizeCategory> size = Enum.GetValues(typeof(SizeCategory)).OfType<SizeCategory>();
        SizeCategory target;
        SizeCategory[] itemSizeCategories = size as SizeCategory[] ?? size.ToArray();
    ...
    private bool BuildingCommandWeight(ICharacter actor, StringStack command)
    {
        ...
        double result = Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument, UnitType.Mass, out bool success);
        if (!success)
        {
    ...
    {
        string cmd = command.SafeRemainingArgument ?? "";
        if (cmd.Length == 0)
        {
    ...
    private bool BuildingCommandDesc(ICharacter actor, StringStack command)
    {
        ...
        VariableGameItemComponentProto variableComp = GetItemType<VariableGameItemComponentProto>();
        if (variableComp != null)
        {
            foreach (ICharacteristicDefinition variable in variableComp.CharacteristicDefinitions)
            {
    ...
    {
        string subCommand = command.SafeRemainingArgument;
        ...
        IGameItemComponentProto component = long.TryParse(subCommand, out long value)
    ...
    private bool BuildingCommandAttach(ICharacter actor, StringStack command)
    {
        ...
        component = !long.TryParse(subCommand, out long number)
    ...
    private bool BuildingCommandRegister(ICharacter actor, StringStack command)
    {
        ...
        ProgVariableTypes variableType = Gameworld.VariableRegister.GetType(ProgVariableTypes.Item, variableName);
