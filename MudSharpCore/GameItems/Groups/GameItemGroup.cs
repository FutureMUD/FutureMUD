...
public class GameItemGroup : SavableKeywordedItem, IGameItemGroup
{
    ...
    public GameItemGroup(ItemGroup group, IFuturemud gameworld)
    {
        ...
        foreach (ItemGroupForm form in group.ItemGroupForms)
        {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            ItemGroup dbitem = FMDB.Context.ItemGroups.Find(Id);
            dbitem.Name = Name;
    ...
    public string Show(IPerceiver voyeur)
    {
        ...
        {
            foreach (IGameItemGroupForm form in _forms)
            {
                sb.AppendLine();
        ...
        sb.AppendLine();
        List<IGameItemProto> items = Gameworld.ItemProtos
        ...
        if (items.Any())
        {
            foreach (IGameItemProto item in items)
            {
    ...
    private void BuildingCommandForm(ICharacter actor, StringStack command)
    {
        ...
        if (!long.TryParse(command.Last, out long value))
        {
            actor.Send("What is the ID of the form you wish to edit?");
            return;
        }

        IGameItemGroupForm form = _forms.FirstOrDefault(x => x.Id == value);
        if (form == null)
        {
    ...
    private void BuildingCommandFormAdd(ICharacter actor, StringStack command)
    {
        ...
        IGameItemGroupForm newForm = GameItemGroupFactory.CreateGameItemGroupForm(this, command.PopSpeech(), Gameworld);
        if (newForm == null)
        {
