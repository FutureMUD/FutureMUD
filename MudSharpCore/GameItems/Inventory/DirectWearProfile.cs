...
public class DirectWearProfile : WearProfile
{
    ...
    public override Dictionary<IWear, IWearlocProfile> AllProfiles
    {
        get
        {
            ...
            foreach (KeyValuePair<IBodypart, IWearlocProfile> loc in Locations)
            {
    ...
    private void LoadWearlocsFromXml(XElement root)
    {
        try
        {
            ...
            foreach (XElement loc in root.Elements("Profile"))
            {
                Locations.Add(
                    long.TryParse(loc.Attribute("Bodypart").Value, out long value)
    ...
    public override Dictionary<IWear, IWearlocProfile> Profile(IBody body)
    {
        ...
        foreach (KeyValuePair<IBodypart, IWearlocProfile> loc in Locations)
        {
            IWear targetloc = body.WearLocs.FirstOrDefault(x => x.CountsAs(loc.Key));
            if (targetloc == null)
            {
    ...
    private void BuildingCommandSet(ICharacter actor, StringStack command)
    {
        ...
        IBodypart part = long.TryParse(command.PopSpeech(), out long value)
