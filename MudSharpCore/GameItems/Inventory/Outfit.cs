...
public class Outfit : IOutfit
{
    public Outfit(ICharacter owner, XElement root)
    {
        ...
        foreach (XElement item in root.Element("Items").Elements())
        {
    ...
    public string Show(ICharacter voyeur)
    {
        ...
        {
            foreach (IOutfitItem item in Items.OrderBy(x => x.WearOrder))
            {
                sb.AppendLine(
    ...
    private bool BuildingCommandAddWorn(ICharacter builder, StringStack command)
    {
        ...
        foreach (IGameItem item in builder.Body.DirectWornItems)
        {
            ...
            else
            {
                ...
                IOutfitItem entry = _items.First(x => x.Id == item.Id);
                _items.Remove(entry);
                _items.Add(entry);
            }
        }
    ...
    private bool BuildingCommandAdd(ICharacter builder, StringStack command)
    {
        ...
        IGameItem item = builder.TargetItem(command.PopSpeech());
        if (item == null)
        {
        ...
        while (!command.IsFinished)
        {
            ...
            if (cmd[0] == '*')
            {
                container = builder.TargetItem(cmd[1..]);
                if (container == null)
                {
    ...
    private bool BuildingCommandRemove(ICharacter builder, StringStack command)
    {
        ...
        IOutfitItem item = Items.GetFromItemListByKeyword(command.PopSpeech(), builder);
        if (item == null)
        {
    ...
    private bool BuildingCommandContainer(ICharacter builder, StringStack command)
    {
        ...
        IGameItem container = builder.TargetItem(command.PopSpeech());
        if (container == null)
        {
    ...
    private bool BuildingCommandSwap(ICharacter builder, StringStack command)
    {
        ...
        IOutfitItem item1 = Items.GetFromItemListByKeyword(command.PopSpeech(), builder);
        if (item1 == null)
        {
            builder.OutputHandler.Send("There is no such item associated with this outfit.");
            return false;
        ...
        IOutfitItem item2 = Items.GetFromItemListByKeyword(command.PopSpeech(), builder);
        if (item2 == null)
        {
            builder.OutputHandler.Send("There is no such item associated with this outfit.");
            return false;
