...
public class InventoryPlanActionWield : InventoryPlanAction
{
    ...
    {
        List<(IGameItem Item, double Fitness, int InternalOrder)> potentialItems = new();
        int internalOrder = 0;
        ...
        foreach (IContainer container in executor.Inventory.SelectNotNull(x => x.GetItemType<IContainer>()))
        {
        ...
        IGameItem scoutedItem = potentialItems.OrderByDescending(x => x.Fitness).ThenBy(x => x.InternalOrder).FirstOrDefault()
                                        .Item;
