...
public class InventoryPlanTemplate : IInventoryPlanTemplate
{
    ...
    {
        Gameworld = gameworld;
        IEnumerable<XElement> elements = definition.Elements("Phase");
        if (elements.Any())
        {
            int phasenum = 1;
    ...
    public static IInventoryPlanAction? ParseActionFromBuilderInput(ICharacter actor, StringStack command)
    {
        ...
        switch (state)
        {
            ...
            case DesiredItemState.ConsumeLiquid:
                ...
                ILiquid? liquid = actor.Gameworld.Liquids.GetByIdOrName(command.PopSpeech());
                if (liquid is null)
                {
            ...
            case DesiredItemState.Apply:
                ...
                IBodypart bodypart = actor.Body.GetTargetBodypart(command.PopSpeech());
                if (bodypart == null)
                {
                ...
                if (!double.TryParse(command.SafeRemainingArgument, out double grams) || grams <= 0.0)
                {
...
#nullable restore
    ...
    public void FinalisePlan(ICharacter executor, bool restore, IInventoryPlan plan, IList<IGameItem> exemptItems)
    {
        if (restore)
        {
            ...
            foreach (IInventoryPlanItemEffect item in items.Where(x => x.DesiredState != DesiredItemState.Held &&
        ...
        IInventoryPlan plan)
    {
        ...
        int numAvailWieldingLocs = actor.Body.WieldLocs
        ...
        foreach (
            (IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) target in
            targets.Where(
                x =>
        ...
        foreach ((IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) target in targets.Where(x =>
        ...
        {
            IBodypart part = actor.Body.GetTargetBodypart(((InventoryPlanActionApply)target.Action).Bodypart);
            results.Add(new InventoryPlanActionResult
            {
                ActionState = target.Action.DesiredState,
                OriginalReference = target.Action.OriginalReference,
        ...
        IInventoryPlan plan)
    {
        ...
        if (numItemsRequiringWieldLocs > numAvailHoldingWieldLocs + numAvailWieldingLocs)
        {
            ...
            foreach (IGameItem item in actor.Body.HeldOrWieldedItems.Where(x => targets.All(y => y.Primary != x)).ToList())
            {
                ...
                IInventoryPlanItemEffect targetEffect =
        ...
        ICharacter actor, IInventoryPlanPhase phase)
    {
        ...
        foreach ((IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) target in targets)
        {
                ...
                target.Action.DesiredState == DesiredItemState.WieldedTwoHandedOnly)
            {
                ...
                IWield minHands = wieldingLocs.Concat(holdingWieldingLocs).FirstMin(x => x.Hands(target.Primary));
                ...
                else
                {
                    wieldingLocs.Remove(minHands);
                    holdingWieldingLocs.Remove(minHands);
                    for (int i = 0; i < minHands.Hands(target.Primary) - 1; i++)
                    {
                        if (wieldingLocs.Any())
                        {
    ...
    public InventoryPlanFeasibility PlanIsFeasible(ICharacter actor, IInventoryPlanPhase phase)
    {
        ...
        foreach ((IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) item in targets.Where(x =>
                     x.Action.DesiredState == DesiredItemState.Wielded ||
                     x.Action.DesiredState == DesiredItemState.WieldedOneHandedOnly ||
                     x.Action.DesiredState == DesiredItemState.WieldedTwoHandedOnly))
        {
            ...
            IWield minHands = wieldingLocs.Concat(holdingWieldingLocs).FirstMin(x => x.Hands(item.Primary));
            ...
            else
            {
                wieldingLocs.Remove(minHands);
                holdingWieldingLocs.Remove(minHands);
                for (int i = 0; i < minHands.Hands(item.Primary) - 1; i++)
                {
                    if (wieldingLocs.Any())
                    {
                        wieldingLocs.RemoveAt(0);
        ...
        foreach ((IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) item in targets.Where(x => x.Action.DesiredState == DesiredItemState.Held))
        {
            if (!holdingLocs.Any() && !holdingWieldingLocs.Any())
            {
        ...
        foreach ((IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) item in targets.Where(x => x.Action.DesiredState == DesiredItemState.Attached))
        {
        ...
        foreach ((IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) item in targets.Where(x => x.Action.DesiredState == DesiredItemState.InContainer))
        {
            if (item.Secondary == null)
            {
                return InventoryPlanFeasibility.NotFeasibleMissingItems;
        ...
        foreach ((IInventoryPlanAction Action, IGameItem Primary, IGameItem Secondary) item in targets.Where(x => x.Action.DesiredState == DesiredItemState.Sheathed))
        {
            if (item.Secondary == null)
            {
                return InventoryPlanFeasibility.NotFeasibleMissingItems;
        ...
        object originalReference)
    {
        ...
        InventoryPlanActionResult getResult = GetItem(actor, item, originalReference, 0, silent);
        ...
        bool silent, bool wieldOK = false)
    {
        ...
        if (container != null)
        {
            IOpenable openable = container.Parent.GetItemType<IOpenable>();
        ...
        if (item.ContainedIn?.IsItemType<ISheath>() ?? false)
        {
            IGameItem sheathe = item.ContainedIn;
    ...
    {
        DesiredItemState state = DesiredItemState.Wielded;
        switch (option)
        {
    ...
    {
        IBodypart part = actor.Body.GetTargetBodypart(action.Bodypart);
