...
public class ShapeWearProfile : WearProfile
{
    ...
    public override Dictionary<IWear, IWearlocProfile> AllProfiles
    {
        get
        {
            Dictionary<IWear, IWearlocProfile> result = new();
            foreach (KeyValuePair<IBodypartShape, Tuple<int, IWearlocProfile>> shape in ShapeCounts)
            {
                List<IWear> parts =
    ...
    {
        foreach (XElement shape in root.Elements("Shape"))
        {
            ShapeCounts.Add(
                long.TryParse(shape.Attribute("ShapeId").Value, out long value)
    ...
    {
        Models.WearProfile dbitem = FMDB.Context.WearProfiles.Find(Id);
    ...
    {
        Dictionary<IWear, IWearlocProfile> result = new();
        ILookup<IWear, int> locCounts = body.WornItemCounts;
        foreach (KeyValuePair<IBodypartShape, Tuple<int, IWearlocProfile>> shape in ShapeCounts)
        {
            IEnumerable<IGrouping<IWear, int>> locs = locCounts.Where(x => x.Key.Shape == shape.Key);
            if (locs.Count() < shape.Value.Item1 && shape.Value.Item2.Mandatory)
            {
                return null;
            }

            IEnumerable<IGrouping<IWear, int>> final = locs.OrderBy(x => x.First()).Take(shape.Value.Item1);
            foreach (IGrouping<IWear, int> loc in final)
            {
    ...
    private void BuildingCommandSet(ICharacter actor, StringStack command)
    {
        ...
        bool bValue;
        string whichCommand = command.PopSpeech().ToLowerInvariant();
        switch (whichCommand)
        {
            case "count":
                if (command.IsFinished)
                ...
                if (!int.TryParse(command.PopSpeech(), out int iValue) || iValue < 1)
                {
    ...
    private void BuildingCommandRemove(ICharacter actor, StringStack command)
    {
        ...
        IBodypartShape part = long.TryParse(command.PopSpeech(), out long value)
            ? ShapeCounts.Select(x => x.Key).FirstOrDefault(x => x.Id == value)
            : ShapeCounts.Select(x => x.Key)
                         .FirstOrDefault(x => x.Name.Equals(command.Last, StringComparison.InvariantCultureIgnoreCase));
