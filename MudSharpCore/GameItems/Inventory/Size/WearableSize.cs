...
public class WearableSize : FrameworkItem, IWearableSize, ISaveable
{
    ...
    public IWearableSize Clone()
    {
        using (new FMDB())
        {
            Models.WearableSize dbsize = new()
            {
                Height = Height,
                Weight = Weight,
            ...
            WearableSize size = new(dbsize, Gameworld);
            Gameworld.Add(size);
            return size;
        }
    }
    ...
    {
        IWearableSizeRules rules = body.SizeRules;
        return
    ...
    public void Save()
    {
        using (new FMDB())
        {
            Models.WearableSize dbsize = FMDB.Context.WearableSizes.Find(Id);
