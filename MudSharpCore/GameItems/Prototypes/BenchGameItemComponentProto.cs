...
public class BenchGameItemComponentProto : GameItemComponentProto, ITablePrototype, IFlipPrototype, IProvideCoverPrototype
{
    ...
    {
        XAttribute attribute = root.Attribute("MaximumChairSlots");
        if (attribute != null)
        {
        ...
        XElement element = root.Element("Cover");
        if (element != null)
        {
    ...
    private bool BuildingCommandCover(ICharacter actor, StringStack input)
    {
        ...
        IRangedCover cover = long.TryParse(input.PopSpeech(), out long value)
