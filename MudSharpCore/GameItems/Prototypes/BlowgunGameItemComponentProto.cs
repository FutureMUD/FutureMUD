...
public class BlowgunGameItemComponentProto : GameItemComponentProto, IRangedWeaponPrototype
{
	...
	protected BlowgunGameItemComponentProto(IFuturemud gameworld, IAccount originator)
		: base(gameworld, originator, "Blowgun")
	{
		RangedWeaponType =
			gameworld.RangedWeaponTypes.FirstOrDefault(x => x.RangedWeaponType == Combat.RangedWeaponType.Blowgun);
	}

	protected BlowgunGameItemComponentProto(MudSharp.Models.GameItemComponentProto proto, IFuturemud gameworld)
			...
			LoadTemplate = new InventoryPlanTemplate(Gameworld, new[]
			{
				new InventoryPlanPhaseTemplate(1, new[]
				{
					InventoryPlanAction.LoadAction(Gameworld, DesiredItemState.Held, 0, 0, item =>
					{
						...
						if (ammo?.AmmoType.RangedWeaponTypes.Contains(Combat.RangedWeaponType.Blowgun) != true)
						{
							return false;
						}
...
#nullable disable warnings
	...
	{
		manager.AddBuilderLoader("blowgun", true,
			(gameworld, account) => new BlowgunGameItemComponentProto(gameworld, account));
		manager.AddDatabaseLoader("Blowgun", (proto, gameworld) => new BlowgunGameItemComponentProto(proto, gameworld));

		manager.AddTypeHelpInfo(
			"Blowgun",
			$"Makes an item a {"[ranged weapon]".Colour(Telnet.BoldCyan)} with blowgun mechanics",
	...
	{
		return new BlowgunGameItemComponent(this, parent, temporary);
	}

	public override IGameItemComponent LoadComponent(MudSharp.Models.GameItemComponent component, IGameItem parent)
	{
		return new BlowgunGameItemComponent(component, this, parent);
	}

	public override IEditableRevisableItem CreateNewRevision(ICharacter initiator)
	{
		return CreateNewRevision(initiator,
			(proto, gameworld) => new BlowgunGameItemComponentProto(proto, gameworld));
	}
	...
	private bool BuildingCommand_Type(ICharacter actor, StringStack command)
	{
		...
		if (type.RangedWeaponType != Combat.RangedWeaponType.Blowgun)
		{
			actor.Send("You can only give blowguns a ranged weapon type that is also for a blowgun.");
			return false;
		}
