...
public class BoltActionGameItemComponentProto : FirearmBaseGameItemComponentProto, IFirearmPrototype,
	IFirearmAttachmentHostPrototype, IRangedWeaponPrototype, ISwitchablePrototype, IMeleeWeaponPrototype
{
...
#3ejectonfire#0 - toggles whether casings are ejected on fire or on ready"
    ...
    {
        List<string> types = Gameworld.ItemProtos.SelectNotNull(x => x.GetItemType<AmmoClipGameItemComponentProto>())
    ...
    protected override void RecalculateInventoryPlans()
    {
        LoadTemplate = new InventoryPlanTemplate(Gameworld, new[]
        {
            new InventoryPlanPhaseTemplate(1, new[]
            {
                ...
                {
                    IAmmoClip ammo = item.GetItemType<IAmmoClip>();
                    if (ammo == null)
                    {
                        return false;
                    }
