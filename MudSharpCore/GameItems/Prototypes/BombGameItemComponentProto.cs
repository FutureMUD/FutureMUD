...
public class BombGameItemComponentProto : GameItemComponentProto, IDetonatablePrototype
{
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement item in root.Element("Damages").Elements())
        {
    ...
    private bool BuildingCommandDamage(ICharacter actor, StringStack command)
    {
        ...
        if (!WoundExtensions.TryGetDamageType(command.PopSpeech(), out DamageType damageType))
        {
            actor.OutputHandler.Send(
        ...
        {
            int count = _damages.RemoveAll(x => x.DamageType == damageType);
            actor.OutputHandler.Send(
        ...
        Expression expression = new(command.PopSpeech());
        ...
        Expression stun = new(command.SafeRemainingArgument);
        if (stun.HasErrors())
        {
    ...
    private bool BuildingCommandExplosion(ICharacter actor, StringStack command)
    {
        ...
        Emote emote = new(command.RemainingArgument, new DummyPerceiver(), new DummyPerceivable());
        if (!emote.Valid)
        {
    ...
    private bool BuildingCommandSize(ICharacter actor, StringStack command)
    {
        ...
        if (!GameItemEnumExtensions.TryParseSize(command.SafeRemainingArgument, out SizeCategory value))
        {
            actor.OutputHandler.Send(
