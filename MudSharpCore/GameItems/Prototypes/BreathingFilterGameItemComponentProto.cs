...
public class BreathingFilterGameItemComponentProto : GameItemComponentProto, IProvideGasForBreathingPrototype, IContainerPrototype
{
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement gas in root.Element("Gases").Elements())
        {
    ...
    private bool BuildingCommandConvert(ICharacter actor, StringStack command)
    {
        ...
        IGas from = long.TryParse(command.PopSpeech(), out long value)
        ...
        IGas to = long.TryParse(command.PopSpeech(), out value)
            ? Gameworld.Gases.Get(value)
            : Gameworld.Gases.GetByName(command.Last);
        if (to == null)
        {
    ...
    private bool BuildingCommandCapacity(ICharacter actor, StringStack command)
    {
        ...
        double amount = Gameworld.UnitManager.GetBaseUnits(command.PopSpeech(), Framework.Units.UnitType.FluidVolume,
            out bool success);
        if (!success)
        {
