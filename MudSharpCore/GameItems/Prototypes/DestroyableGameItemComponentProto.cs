...
public class DestroyableGameItemComponentProto : GameItemComponentProto
{
    ...
    {
        foreach (DamageType value in Enum.GetValues(typeof(DamageType)).OfType<DamageType>())
        {
            switch (value)
            {
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        XElement element = root.Element("DamageMultipliers");
        if (element != null)
        {
    ...
    private bool BuildingCommandDamage(ICharacter actor, StringStack command)
    {
        ...
        if (!WoundExtensions.TryGetDamageType(command.PopSpeech(), out DamageType type))
        {
