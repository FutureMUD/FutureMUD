...
public class DiceGameItemComponentProto : GameItemComponentProto
{
    ...
    {
        foreach (XElement element in root.Element("Faces").Elements())
        {
            _faces.Add(element.Value);
        }

        foreach (XElement element in root.Element("Weights").Elements())
        {
    ...
    private bool BuildingCommandTemplate(ICharacter actor, StringStack command)
    {
        ...
        string template = command.PopSpeech();
        if (NumericalDiceRegex.IsMatch(template))
        {
            int sides = int.Parse(NumericalDiceRegex.Match(template).Groups["sides"].Value);
            if (sides < 4)
            {
    ...
    private bool BuildingCommandFace(ICharacter actor, StringStack command)
    {
        ...
        int faceIndex;
        string faceText = command.Last;
        if (int.TryParse(faceText, out int value))
        {
        ...
        {
            string face = Faces.FirstOrDefault(x => x.EqualTo(command.Last)) ?? Faces.FirstOrDefault(x =>
    ...
    {
        string face = _faces.ElementAt(faceIndex);
        if (command.IsFinished)
        {
        ...
        string faceValue = command.PopSpeech();
    ...
    private bool BuildingCommandSwap(ICharacter actor, StringStack command)
    {
        ...
        double prob1 = _faceProbabilities[index1];
        double prob2 = _faceProbabilities[index2];
