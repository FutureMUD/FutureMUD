...
public class DoorGameItemComponentProto : GameItemComponentProto
{
    ...
    {
        XAttribute attr = root.Attribute("SeeThrough");
        if (attr != null)
        {
        ...
        XElement element = root.Element("InstalledExitDescription");
        if (element != null)
        {
    ...
    private bool BuildingCommand_Uninstallable(ICharacter actor, StringStack command)
    {
        ...
        string hingeDifficultyText = command.PopSpeech();
        if (command.IsFinished)
        {
            actor.Send(
        ...
        string nonHingeDifficultyText = command.PopSpeech();
        if (command.IsFinished)
        {
        ...
        string traitText = command.PopSpeech();

        if (!CheckExtensions.GetDifficulty(hingeDifficultyText, out Difficulty hingeDifficulty))
        {
            actor.OutputHandler.Send($"The text {hingeDifficultyText.ColourCommand()} is not a valid difficulty.");
            return false;
        }

        if (!CheckExtensions.GetDifficulty(nonHingeDifficultyText, out Difficulty nonHingeDifficulty))
        {
            actor.OutputHandler.Send($"The text {nonHingeDifficultyText.ColourCommand()} is not a valid difficulty.");
            return false;
        }

        ITraitDefinition trait = Gameworld.Traits.GetByIdOrName(traitText);
        if (trait == null)
        {
    ...
    private bool BuildingCommand_Smashable(ICharacter actor, StringStack command)
    {
        ...
        string difficultyText = command.PopSpeech();

        if (!CheckExtensions.GetDifficulty(difficultyText, out Difficulty difficulty))
        {
