...
public class ExternalOrganGameItemComponentProto : GameItemComponentProto
{
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        foreach (long item in root.Element("Organs").Elements().Select(x => long.Parse(x.Value)))
        {
            IOrganProto organ = Body?.Organs.FirstOrDefault(x => x.Id == item);
            if (organ != null)
            {
                _organs.Add(organ);
            }
    ...
    private bool BuildingCommandWattage(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double wattage))
        {
        ...
        if (!double.TryParse(command.PopSpeech(), out double discount))
        {
    ...
    private bool BuildingCommandOrgan(ICharacter actor, StringStack command)
    {
        ...
        IOrganProto organ = Body.Organs.FirstOrDefault(x => x.FullDescription().EqualTo(text)) ??
