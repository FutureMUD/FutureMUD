...
public class ImplantPowerRouterGameItemComponentProto : GameItemComponentProto
{
    ...
    protected ImplantPowerRouterGameItemComponentProto(IFuturemud gameworld, IAccount originator) : base(gameworld,
        originator, "ImplantPowerRouter")
    {
    ...
    protected ImplantPowerRouterGameItemComponentProto(MudSharp.Models.GameItemComponentProto proto,
    ...
    {
        BaseEfficiencyMultiplier = double.Parse(root.Element("BaseEfficiencyMultiplier").Value);
        EfficiencyGainPerQuality = double.Parse(root.Element("EfficiencyGainPerQuality").Value);
        External = bool.Parse(root.Element("External").Value);
        ExternalDescription =
            root.Element("ExternalDescription")?.Value ?? "a small, polished chrome panel with hinges";
        TargetBody = Gameworld.BodyPrototypes.Get(long.Parse(root.Element("TargetBody").Value));
        long tbpid = long.Parse(root.Element("TargetBodypart").Value);
    ...
    {
        return new ImplantPowerRouterGameItemComponent(this, parent, temporary);
    }

    public override IGameItemComponent LoadComponent(MudSharp.Models.GameItemComponent component, IGameItem parent)
    {
        return new ImplantPowerRouterGameItemComponent(component, this, parent);
    }
    ...
    {
        manager.AddBuilderLoader("ImplantPowerRouter".ToLowerInvariant(), true,
    ...
    private bool BuildingCommandDifficulty(ICharacter actor, StringStack command)
    {
        ...
        if (!CheckExtensions.GetDifficulty(command.PopSpeech(), out Difficulty difficulty))
        {
    ...
    private bool BuildingCommandBodypart(ICharacter actor, StringStack command)
    {
        ...
        List<IBodypart> parts = TargetBody.AllBodypartsBonesAndOrgans.ToList();
        string partName = command.SafeRemainingArgument;
        IBodypart bodypart = long.TryParse(partName, out long value)
    ...
    private bool BuildingCommandBody(ICharacter actor, StringStack command)
    {
        ...
        IBodyPrototype body = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandExternalDescription(ICharacter actor, StringStack command)
    {
        ...
        Changed = true;
        actor.Send(
            $"This implant will no have the following description added to their look description when installed and external:\n\n\t{ExternalDescription.ColourObject()}");
        return true;
    }
    ...
    public override string ComponentDescriptionOLC(ICharacter actor)
    {
        return string.Format(
            actor,
            "{0} (#{1:N0}r{2:N0}, {3})\r\n\r\nThis item powers other implants when it is itself powered, and can be connected to multiple other parts. It has a base efficiency of [{4:N3} - quality * {5:N3}]. It is installed in the {6} body type in bodypart {7}. It is {9} for surgeons to implant and takes up {10} space. It {8}.",
