...
public class ImplantRadioGameItemComponentProto : ImplantBaseGameItemComponentProto, IImplantReportStatusPrototype, IImplantRespondToCommandsPrototype, ITransmitPrototype, IReceivePrototype
{
        ...
        originator, "ImplantRadio")
    {
        ...
        int channelNumber = 1;
        for (double i = 476.425; i < 477.400; i = i + 0.025)
        {
            Channels.Add(i);
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement item in root.Elements("Channel"))
        {
            Channels.Add(double.Parse(item.Value));
        }

        foreach (XElement item in root.Elements("ChannelName"))
        {
    ...
    {
        XElement def = SaveToXmlWithoutConvertingToString();
        def.Add(
