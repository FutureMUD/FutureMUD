...
public class LanternGameItemComponentProto : GameItemComponentProto, ILiquidContainerPrototype, ILockablePrototype, ILightablePrototype, IProduceLightPrototype
{
    ...
    private bool BuildingCommand_Capacity(ICharacter actor, StringStack command)
    {
        ...
        double value = Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument, UnitType.FluidVolume,
            out bool success);
        if (!success)
        {
    ...
    private bool BuildingCommand_LightEmote(ICharacter actor, StringStack command)
    {
        ...
        Emote emote = new(command.RemainingArgument, actor, actor, actor, actor);
        if (!emote.Valid)
        {
    ...
    private bool BuildingCommand_ExtinguishEmote(ICharacter actor, StringStack command)
    {
        ...
        // This line is essentially just leveraging Emote.ScoutTargets to make sure they didn't make a bad emote
        Emote emote = new(command.RemainingArgument, actor, actor, actor);
        if (!emote.Valid)
        {
            actor.Send(emote.ErrorMessage);
            return false;
    ...
    private bool BuildingCommand_TenPercent(ICharacter actor, StringStack command)
    {
        ...
        // This line is essentially just leveraging Emote.ScoutTargets to make sure they didn't make a bad emote
        Emote emote = new(command.RemainingArgument, actor, actor);
        if (!emote.Valid)
        {
            actor.Send(emote.ErrorMessage);
            return false;
