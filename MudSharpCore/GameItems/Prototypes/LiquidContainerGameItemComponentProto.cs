...
public class LiquidContainerGameItemComponentProto : GameItemComponentProto, ILiquidContainerPrototype, ILockablePrototype
{
    ...
    {
        XAttribute attr = root.Attribute("LiquidCapacity");
        if (attr != null)
        {
    ...
    {
        string volumeCmd = command.SafeRemainingArgument;
        double result = actor.Gameworld.UnitManager.GetBaseUnits(volumeCmd, UnitType.FluidVolume, out bool success);
        if (success)
        {
