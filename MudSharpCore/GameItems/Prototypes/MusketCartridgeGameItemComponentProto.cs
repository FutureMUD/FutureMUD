...
public class MusketCartridgeGameItemComponentProto : AmmunitionGameItemComponentProto, IMusketCartridgePrototype
{
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        PowderMass = double.TryParse(root.Element("PowderMass")?.Value, out var powderMass)
            ? powderMass
            : null;
        IncludesWad = bool.TryParse(root.Element("IncludesWad")?.Value, out var includesWad)
            ? includesWad
            : true;
    ...
    protected override string SaveToXml()
    {
        return new XElement("Definition",
            ...
            PowderMass is not null ? new XElement("PowderMass", PowderMass.Value) : null,
            new XElement("IncludesWad", IncludesWad)
            ).ToString();
    }
    #endregion
	...
	#3powder <mass>|legacy#0 - sets the measured powder charge, or uses the loading musket's charge
	#3wad#0 - toggles whether the cartridge includes its own wad";
    ...
    public override bool BuildingCommand(ICharacter actor, StringStack command)
    {
        switch (command.PopSpeech().ToLowerInvariant())
        {
            ...
            case "wad":
                IncludesWad = !IncludesWad;
                Changed = true;
                actor.OutputHandler.Send(
                    $"This cartridge {(IncludesWad ? "now includes" : "no longer includes")} its own wad.");
                return true;
            default:
    ...
    private bool BuildingCommandPowder(ICharacter actor, StringStack command)
    {
        ...
        {
            actor.OutputHandler.Send(
                $"How much powder does this cartridge contain? Use {"legacy".ColourCommand()} to use the loading musket's configured charge.");
            return false;
        }

        if (command.SafeRemainingArgument.EqualToAny("legacy", "none", "weapon"))
        {
            PowderMass = null;
            Changed = true;
            actor.OutputHandler.Send("This cartridge now uses the loading musket's configured powder charge.");
            return true;
        }

        if (!Gameworld.UnitManager.TryGetBaseUnits(command.SafeRemainingArgument, Framework.Units.UnitType.Mass,
                actor, out var value) || value <= 0.0)
        {
            actor.OutputHandler.Send("You must enter a positive mass of powder.");
            return false;
        }
        ...
        Changed = true;
        actor.OutputHandler.Send(
            $"This cartridge now contains {Gameworld.UnitManager.DescribeExact(value, Framework.Units.UnitType.Mass, actor).ColourValue()} of powder.");
        return true;
    }
    ...
    private bool BuildingCommandAmmunition(ICharacter actor, StringStack command)
    {
        ...
        IAmmunitionType ammo = long.TryParse(command.PopSpeech(), out long value)
    ...
    {
        return string.Format(actor, "{0} (#{1:N0}r{2:N0}, {3})\r\n\r\nThis is a pre-primed musket cartridge of type {4} with a bore of {5}in.\nPowder Charge: {6}\nIncludes Wad: {7}{8}",
            ...
            PowderMass is not null
                ? Gameworld.UnitManager.DescribeExact(PowderMass.Value, Framework.Units.UnitType.Mass, actor)
                    .ColourValue()
                : "Weapon-defined legacy charge".ColourValue(),
            IncludesWad.ToColouredString(),
    ...
    public double? PowderMass { get; set; }
    public bool IncludesWad { get; set; }
}
