...
public class MusketGameItemComponentProto : GameItemComponentProto, IJammableWeaponPrototype, IBeltPrototype, IMeleeWeaponPrototype, IEmplaceableRangedWeaponPrototype, IConditionDegradingComponentPrototype
{
        ...
        LoadTemplateClean = new InventoryPlanTemplate(Gameworld, new[]
            {
                new InventoryPlanPhaseTemplate(1, new[]
                {
					InventoryPlanAction.LoadAction(Gameworld, DesiredItemState.Held, 0, 0,
						item => HasTag(item, CleaningRodTag), null, 1, originalReference: "cleaning rod"),
                    InventoryPlanAction.LoadAction(Gameworld, DesiredItemState.Held, 0, 0,
    ...
    {
        get => _meleeWeaponType; set => _meleeWeaponType = value;
    }
#nullable enable
...
The functional tool tags are installed by the Useful Seeder. This component controls weapon profile, charge, bore, ignition family, attachment slots, condition, and the echoes for each drill step.
    ...
    private bool BuildingCommandStartUnjamEmote(ICharacter actor, StringStack command)
    {
        ...
        string emoteText = command.SafeRemainingArgument.Fullstop();
        Emote emote = new(emoteText, new DummyPerceiver(), new DummyPerceivable(), new DummyPerceivable(), new DummyPerceivable());
        if (!emote.Valid)
        {
    ...
    private bool BuildingCommandCatastrophyDamage(ICharacter actor, StringStack command)
    {
        ...
        Expression expr = new(command.SafeRemainingArgument);
        if (expr.HasErrors())
        {
    ...
    private bool BuildingCommandPowder(ICharacter actor, StringStack command)
    {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(command.SafeRemainingArgument, UnitType.Mass, actor, out double value) || value <= 0.0)
        {
    ...
    private bool BuildingCommandLoadEmotePowder(ICharacter actor, StringStack command)
    {
        ...
        string emoteText = command.SafeRemainingArgument.Fullstop();
        Emote emote = new(emoteText, new DummyPerceiver(), new DummyPerceivable(), new DummyPerceivable(), new DummyPerceivable(), new DummyPerceivable());
        if (!emote.Valid)
        {
            actor.OutputHandler.Send(emote.ErrorMessage);
            return false;
    ...
    private bool BuildingCommandLoadEmoteBall(ICharacter actor, StringStack command)
    {
        ...
        string emoteText = command.SafeRemainingArgument.Fullstop();
        Emote emote = new(emoteText, new DummyPerceiver(), new DummyPerceivable(), new DummyPerceivable(), new DummyPerceivable(), null);
        if (!emote.Valid)
        {
            actor.OutputHandler.Send(emote.ErrorMessage);
            return false;
