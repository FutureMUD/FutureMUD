...
public class RadioDetonatorTransmitterGameItemComponentProto : GameItemComponentProto, IConsumePowerPrototype, IOnOffPrototype, ISelectablePrototype, ISwitchablePrototype
{
    ...
    private bool BuildingCommandDetonationEmote(ICharacter actor, StringStack command)
    {
        ...
        string emoteText = command.RemainingArgument.ProperSentences().Fullstop();
    ...
    private bool BuildingCommandRange(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value) || value < 0 || value > 50)
        {
    ...
    private bool BuildingCommandBroadcast(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double value) || value < 0)
        {
