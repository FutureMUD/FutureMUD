...
public class RepairKitGameItemComponentProto : GameItemComponentProto
{
    ...
    private bool BuildingCommandDamageType(ICharacter actor, StringStack command)
    {
        ...
        if (!WoundExtensions.TryGetDamageType(command.SafeRemainingArgument, out DamageType value))
        {
    ...
    private bool BuildingCommandSeverity(ICharacter actor, StringStack command)
    {
        ...
        if (!WoundExtensions.TryParseWoundSeverity(command.SafeRemainingArgument, out WoundSeverity value))
        {
    ...
    private bool BuildingCommandCheckTrait(ICharacter actor, StringStack command)
    {
        ...
        ITraitDefinition trait = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandMaterial(ICharacter actor, StringStack command)
    {
        ...
        ISolid material = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandEchoAdd(ICharacter actor, StringStack command)
    {
        ...
        Emote emote = new(command.RemainingArgument, new DummyPerceiver(), new DummyPerceiver(),
    ...
    private bool BuildingCommandEchoDelete(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int value) || value <= 0 || value > _echoes.Count)
        {
    ...
    private bool BuildingCommandEchoSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value) || value <= 0 || value > _echoes.Count)
        {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value2) || value2 <= 0 || value2 > _echoes.Count)
        {
            actor.OutputHandler.Send($"You must enter a value between 1 and {_echoes.Count}.");
            return false;
        }
    ...
    public override string WhyCannotSubmit()
    {
        ...
        foreach (Emote emote in _echoes.Select(x => new Emote(x, new DummyPerceiver(), new DummyPerceiver(),
                     new DummyPerceivable(), new DummyPerceivable())))
        {
            if (!emote.Valid)
            {
