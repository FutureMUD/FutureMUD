...
public class RestraintGameItemComponentProto : GameItemComponentProto
{
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement item in root.Elements("LimbType"))
        {
    ...
    private bool BuildingCommandMaxSize(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<SizeCategory>(out SizeCategory size))
        {
    ...
    private bool BuildingCommandLimb(ICharacter actor, StringStack command)
    {
        ...
        LimbType[] limbs = Enum.GetValues(typeof(LimbType)).OfType<LimbType>().ToArray();
        ...
        LimbType limb = limbs.First(x => x.Describe().EqualTo(cmd) || x.DescribePlural().EqualTo(cmd));
