...
public class SelectableGameItemComponentProto : GameItemComponentProto, ISelectablePrototype
{
    ...
    private bool BuildingCommandAdd(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg canSelectProg = long.TryParse(command.PopSpeech(), out long value)
        ...
        IFutureProg onSelectProg = long.TryParse(command.PopSpeech(), out value)
            ? Gameworld.FutureProgs.Get(value)
            : Gameworld.FutureProgs.GetByName(command.Last);
        ...
        SelectableOption newOption = new()
        {
    ...
    private bool BuildingCommandRemove(ICharacter actor, StringStack command)
    {
        ...
        SelectableOption option = Options.FirstOrDefault(x => x.Keyword.EqualTo(which)) ?? Options.FirstOrDefault(x =>
    ...
    private bool BuildingCommandSwap(ICharacter actor, StringStack command)
    {
        ...
        string which1 = command.PopSpeech();
        if (command.IsFinished)
        {
            actor.Send("Which two options do you want to swap?");
            return false;
        ...
        string which2 = command.PopSpeech();

        SelectableOption option1 = Options.FirstOrDefault(x => x.Keyword.EqualTo(which1)) ?? Options.FirstOrDefault(x =>
            x.Keyword.StartsWith(which1, StringComparison.InvariantCultureIgnoreCase));
        SelectableOption option2 = Options.FirstOrDefault(x => x.Keyword.EqualTo(which2)) ?? Options.FirstOrDefault(x =>
