...
namespace MudSharp.GameItems.Prototypes
{
    public class ShopStallGameItemComponentProto : GameItemComponentProto
    {
        ...
        /// </summary>
        public double WeightLimit { get; protected set; } = 10000000.0;

        /// <summary>
        ///     The maximum SizeCategory of item that may be contained by this container
        /// </summary>
        public SizeCategory MaximumContentsSize { get; protected set; } = SizeCategory.Huge;
        ...
        public override string TypeDescription => "ShopStall";

        #region Constructors
        protected ShopStallGameItemComponentProto(IFuturemud gameworld, IAccount originator) : base(gameworld, originator, "ShopStall")
        {
        }

        protected ShopStallGameItemComponentProto(Models.GameItemComponentProto proto, IFuturemud gameworld) : base(proto, gameworld)
        {
        }
        ...
        {
            return new ShopStallGameItemComponent(this, parent, temporary);
        }

        public override IGameItemComponent LoadComponent(Models.GameItemComponent component, IGameItem parent)
        {
            return new ShopStallGameItemComponent(component, this, parent);
        }
        #endregion
        ...
        {
            manager.AddBuilderLoader("ShopStall".ToLowerInvariant(), true, (gameworld, account) => new ShopStallGameItemComponentProto(gameworld, account));
            manager.AddDatabaseLoader("ShopStall", (proto, gameworld) => new ShopStallGameItemComponentProto(proto, gameworld));
            manager.AddTypeHelpInfo(
                "ShopStall",
                $"Turns the item into a {"[container]".Colour(Telnet.BoldGreen)} that can also a {"[shop]".Colour(Telnet.Orange)}",
                BuildingHelpText
            );
        }
        ...
        {
            return CreateNewRevision(initiator, (proto, gameworld) => new ShopStallGameItemComponentProto(proto, gameworld));
        }
        #endregion
