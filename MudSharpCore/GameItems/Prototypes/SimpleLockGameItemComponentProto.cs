...
public class SimpleLockGameItemComponentProto : GameItemComponentProto, IHaveSimpleLockType, ILockPrototype
{
    ...
    public override string TypeDescription => "Simple Lock";

    public static void RegisterComponentInitialiser(GameItemComponentManager manager)
    {
        manager.AddBuilderLoader("lock", true,
            (gameworld, account) => new SimpleLockGameItemComponentProto(gameworld, account));
        manager.AddDatabaseLoader("Simple Lock",
            (proto, gameworld) => new SimpleLockGameItemComponentProto(proto, gameworld));

        manager.AddTypeHelpInfo(
            "Lock",
            $"Item is a {"[lock]".Colour(Telnet.Yellow)} that is opened with a physical {"[key]".Colour(Telnet.Yellow)}",
            BuildingHelpText
        );
    }
    ...
    public override string ComponentDescriptionOLC(ICharacter actor)
    {
        return string.Format(actor,
            "{0} (#{1:N0}r{2:N0}\n\nThis item is a simple lock, and is difficulty {3} to pick or {4} to force open. It takes keys of type {11}.\n\nIt uses the following emotes:\nLock: {5}\nUnlock: {6}\nLock (other side): {7}\nUnlock (other side): {8}\nLock (No Actor): {9}\nUnlock (No Actor): {10}",
            "Simple Lock Game Item Component".Colour(Telnet.Cyan),
            Id,
            RevisionNumber,
    ...
    {
        return new SimpleLockGameItemComponent(this, parent, temporary);
    }

    public override IGameItemComponent LoadComponent(MudSharp.Models.GameItemComponent component, IGameItem parent)
    {
        return new SimpleLockGameItemComponent(component, this, parent);
    }
    ...
    protected SimpleLockGameItemComponentProto(MudSharp.Models.GameItemComponentProto proto, IFuturemud gameworld)
        : base(proto, gameworld)
    {
    }

    protected SimpleLockGameItemComponentProto(IFuturemud gameworld, IAccount originator)
        : base(gameworld, originator, "Simple Lock")
    {
        ForceDifficulty = Difficulty.Normal;
        PickDifficulty = Difficulty.Normal;
        LockEmote = "@ lock|locks $1$?3| on $3||$$?2| with $2||$";
        UnlockEmote = "@ unlock|unlocks $1$?3| on $3||$$?2| with $2||$";
    ...
    private bool BuildingCommandUnlockNoActorEmote(ICharacter actor, StringStack command)
    {
        ...
        Emote emote = new(emoteText, new DummyPerceiver(), new DummyPerceivable(), null);
        if (!emote.Valid)
        {
    ...
    private bool BuildingCommandUnlockEmote(ICharacter actor, StringStack command)
    {
        ...
        Changed = true;
        actor.Send("You set the unlock emote for this lock to {0}", UnlockEmote.Fullstop().Colour(Telnet.Yellow));
        return true;
    }
    ...
    private bool BuildingCommandLockEmote(ICharacter actor, StringStack command)
    {
        ...
        LockEmote = emoteText;
        Changed = true;
        actor.Send("You set the lock emote for this lock to {0}", LockEmote.Fullstop().Colour(Telnet.Yellow));
        return true;
    }
