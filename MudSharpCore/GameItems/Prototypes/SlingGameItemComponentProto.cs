...
public class SlingGameItemComponentProto : GameItemComponentProto, IRangedWeaponWithUnreadyEventPrototype
{
	...
	protected SlingGameItemComponentProto(IFuturemud gameworld, IAccount originator)
		: base(gameworld, originator, "Sling")
	{
		RangedWeaponType =
			gameworld.RangedWeaponTypes.FirstOrDefault(x => x.RangedWeaponType == Combat.RangedWeaponType.Sling);
		StaminaPerTick = 3.0;
	}

	protected SlingGameItemComponentProto(MudSharp.Models.GameItemComponentProto proto, IFuturemud gameworld)
			...
			LoadTemplate = new InventoryPlanTemplate(Gameworld, new[]
			{
				new InventoryPlanPhaseTemplate(1, new[]
				{
					InventoryPlanAction.LoadAction(Gameworld, DesiredItemState.Held, 0, 0, item =>
					{
						...
						if (ammo?.AmmoType.RangedWeaponTypes.Contains(Combat.RangedWeaponType.Sling) != true)
						{
							return false;
						}
...
#nullable disable warnings
	...
	{
		XElement element = root.Element("RangedWeaponType");
		if (element != null)
		{
	...
	{
		manager.AddBuilderLoader("sling", true,
			(gameworld, account) => new SlingGameItemComponentProto(gameworld, account));
		manager.AddDatabaseLoader("Sling", (proto, gameworld) => new SlingGameItemComponentProto(proto, gameworld));

		manager.AddTypeHelpInfo(
			"Sling",
			$"Makes an item a {"[ranged weapon]".Colour(Telnet.BoldCyan)} with sling mechanics",
			$"You can use the following options:\n\tname <name> - sets the name of the component\n\tdesc <desc> - sets the description of the component\n\tranged <ranged type> - sets the ranged weapon type for this component. See {"show ranges".FluentTagMXP("send", "href='show ranges'")} for a list.\n\tstamina <amount> - sets the stamina drained per 5 seconds of whirling a readied sling."
		);
	}

	public override IGameItemComponent CreateNew(IGameItem parent, ICharacter? loader = null, bool temporary = false)
	{
		return new SlingGameItemComponent(this, parent, temporary);
	}

	public override IGameItemComponent LoadComponent(MudSharp.Models.GameItemComponent component, IGameItem parent)
	{
		return new SlingGameItemComponent(component, this, parent);
	}

	public override IEditableRevisableItem CreateNewRevision(ICharacter initiator)
	{
		return CreateNewRevision(initiator,
			(proto, gameworld) => new SlingGameItemComponentProto(proto, gameworld));
	}
	...
	#3stamina <amount>#0 - sets the stamina drained per 5 seconds of whirling a readied sling
	...
	private bool BuildingCommand_Stamina(ICharacter actor, StringStack command)
	{
		if (command.IsFinished)
		{
			actor.Send("How much stamina should this sling drain per 5 seconds while being whirled?");
			return false;
		}

		if (!double.TryParse(command.PopSpeech(), out double value) || value < 0.0)
		{
			actor.Send("You must enter a positive amount of stamina for this sling to drain.");
			return false;
		}

		StaminaPerTick = value;
		Changed = true;
		actor.Send($"This sling will now drain {StaminaPerTick:N2} stamina per 5 seconds while readied.");
		return true;
	}
	...
	private bool BuildingCommand_Type(ICharacter actor, StringStack command)
	{
		...
		if (type.RangedWeaponType != Combat.RangedWeaponType.Sling)
		{
			actor.Send("You can only give slings a ranged weapon type that is also for a sling.");
			return false;
		}

		RangedWeaponType = type;
		actor.Send($"This ranged weapon will now be of type {RangedWeaponType.Name.TitleCase().Colour(Telnet.Green)}.");
		Changed = true;
		return true;
	}
	...
	{
		return RangedWeaponType != null && base.CanSubmit();
	}
