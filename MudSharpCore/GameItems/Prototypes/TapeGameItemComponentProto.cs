...
public class TapeGameItemComponentProto : GameItemComponentProto
{
	...
	{
		Capacity = TimeSpan.FromMilliseconds(long.TryParse(root.Element("CapacityMs")?.Value, out var capacityMs)
			? capacityMs
	...
	private bool BuildingCommandCapacity(ICharacter actor, StringStack command)
	{
		...
		if (!double.TryParse(command.SafeRemainingArgument, out var minutes) || minutes <= 0.0)
		{
