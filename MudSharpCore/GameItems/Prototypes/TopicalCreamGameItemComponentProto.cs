...
public class TopicalCreamGameItemComponentProto : GameItemComponentProto
{
    ...
    protected override void LoadFromXml(XElement root)
    {
        ...
        XElement drugs = root.Element("Drugs");
        if (drugs == null)
        {
            return;
        }

        foreach (XElement element in drugs.Elements("Drug"))
        {
            IDrug drug = Gameworld.Drugs.Get(long.Parse(element.Attribute("id")?.Value ?? "0"));
            if (drug == null)
            {
                continue;
            }
    ...
    private bool BuildingCommandDrugAdd(ICharacter actor, StringStack command)
    {
        ...
        IDrug drug = Gameworld.Drugs.GetByIdOrName(command.PopSpeech());
        if (drug == null)
        {
        ...
        if (!double.TryParse(command.PopSpeech(), out double grams) || grams <= 0.0)
        {
        ...
        if (!double.TryParse(command.PopSpeech(), out double absorption) || absorption < 0.0 || absorption > 1.0)
        {
            actor.Send("You must enter a valid absorption fraction between 0 and 1.");
            return false;
        }

        CreamDrug existing = Drugs.FirstOrDefault(x => x.Drug == drug);
        if (existing == null)
        {
