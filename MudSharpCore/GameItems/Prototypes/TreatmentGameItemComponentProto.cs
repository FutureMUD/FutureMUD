...
public class TreatmentGameItemComponentProto : GameItemComponentProto, ITreatmentPrototype
{
    ...
    {
        XElement element = root.Element("MaximumUses");
        if (element != null)
        {
        ...
        foreach (XElement item in root.Elements("TreatmentType"))
        {
    ...
    private bool BuildingCommandTreatment(ICharacter actor, StringStack command)
    {
        ...
        if (!WoundExtensions.GetTreatmentType(command.SafeRemainingArgument, out TreatmentType value))
        {
