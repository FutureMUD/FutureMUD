...
public class WashingMachineGameItemComponentProto : GameItemComponentProto
{
    ...
    private bool BuildingCommandCycleTime(ICharacter actor, StringStack command)
    {
        ...
        if (!MudTimeSpan.TryParse(command.SafeRemainingArgument, out MudTimeSpan result))
        {
    ...
    private bool BuildingCommandCapacity(ICharacter actor, StringStack command)
    {
        ...
        double capacity = Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument, Framework.Units.UnitType.Mass,
            out bool success);
        if (!success)
        {
    ...
    private bool BuildingCommandLiquidCapacity(ICharacter actor, StringStack command)
    {
        ...
        double capacity = Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument,
            Framework.Units.UnitType.FluidVolume, out bool success);
        if (!success)
        {
