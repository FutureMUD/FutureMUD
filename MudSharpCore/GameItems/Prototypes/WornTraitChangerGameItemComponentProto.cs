...
public class WornTraitChangerGameItemComponentProto : GameItemComponentProto, IChangeTraitsInInventoryPrototype
{
    ...
    {
        foreach (XElement element in root.Elements("Modifier"))
        {
            ITraitDefinition trait = Gameworld.Traits.Get(long.Parse(element.Attribute("trait").Value));
            if (trait == null)
            {
                continue;
            }
    ...
    private bool BuildingCommandBonus(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double bonus))
        {
