...
public static class CorpseModelFactory
{
    ...
    {
        Type cmType = typeof(ICorpseModel);
        foreach (
            Type type in Futuremud.GetAllTypes().Where(x => x.GetInterfaces().Contains(cmType)))
        {
            MethodInfo method = type.GetMethod("RegisterTypeLoader", BindingFlags.Public | BindingFlags.Static);
