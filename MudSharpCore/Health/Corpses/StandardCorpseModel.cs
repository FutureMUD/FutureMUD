...
public class StandardCorpseModel : CorpseModel
{
    ...
    {
        XElement element = root.Element("Ranges");
        if (element == null)
        {
        ...
        foreach (XElement item in element.Elements())
        {
            try
            {
                string terrainAttr = item.Attribute("terrain").Value;
                ITerrain terrain = long.TryParse(terrainAttr, out long value)
        ...
        XElement subElement = element.Element("ShortDescriptions");
        if (subElement == null)
        {
        ...
        foreach (XElement item in subElement.Elements())
        {
            try
            {
        ...
        if (element != null)
        {
            foreach (XElement item in element.Elements("CorpseMaterial"))
            {
                try
                {
        ...
        IPerceiver voyeur, DecayState state, double eatenPercentage, IEnumerable<IWound> wounds)
    {
        description = DescriptionRegex.Replace(description, m =>
        {
            ...
            {
                case "wounds":
                    string genderWord = originalCharacter.ApparentGender(voyeur).Possessive();
                    ...
                    IHaveWounds oldParent = wounds.FirstOrDefault()?.Parent;
                    foreach (IWound wound in wounds)
                    {
                        wound.Parent = originalCharacter;
                    }

                    string woundDescs = wounds.Where(x => x.Bodypart != null).Select(x =>
