...
public abstract class Infection : LateInitialisingItem, IInfection
{
    ...
    public void Delete()
    {
        ...
        using (new FMDB())
        {
            if (_id != 0)
            {
                ...
                Models.Infection infection = FMDB.Context.Infections.Find(Id);
                if (infection != null)
                {
    ...
    public virtual void InfectionTick()
    {
        ...
        ICheck check = Gameworld.GetCheck(CheckType.InfectionHeartbeat);
        double externalBonus = 0.0;
        // Antibiotic effects
        externalBonus +=
        ...
        double liverFunction = Owner.OrganFunction<LiverProto>();
        ...
        Difficulty checkDifficulty = VirulenceDifficulty;
        ...
        CheckOutcome result = check.Check(Owner.Actor, checkDifficulty, default(ICharacter), null, externalBonus);
        ...
        {
            ICheck spreadCheck = Gameworld.GetCheck(CheckType.InfectionSpread);
            CheckOutcome spreadResult = spreadCheck.Check(Owner.Actor, checkDifficulty, default(ICharacter), null,
    ...
    {
        Models.Infection item = (MudSharp.Models.Infection)dbitem;
        _id = item.Id;
    }
    ...
    {
        IWound wound = body.Wounds.FirstOrDefault(x => x.Id == (infection.WoundId ?? 0));
        IBodypart bodypart = body.Bodyparts.FirstOrDefault(x => x.Id == (infection.BodypartId ?? 0)) ??
    ...
    protected virtual void SpreadFromBodypart(Outcome outcome)
    {
        switch (outcome)
        {
            ...
            case Outcome.MinorFail:
                ...
                IBodypart bodypart = Owner.Bodyparts
    ...
    protected virtual void SpreadFromOrgan(Outcome outcome)
    {
        switch (outcome)
        {
            ...
            case Outcome.MinorFail:
                ...
                IBodypart bodypart = Owner.Bodyparts.Where(x => x.OrganInfo.Any(y => y.Key.CountsAs(Bodypart)))
                                    .Where(x => Owner.PartInfections.All(y => y.Bodypart != x || y.InfectionType != InfectionType))
                                    .GetRandomElement();
                if (bodypart != null)
                {
    ...
    protected virtual void SpreadFromBody(Outcome outcome)
    {
        switch (outcome)
        {
            ...
            case Outcome.MinorFail:
                ...
                IBodypart bodypart = Owner.Bodyparts.Where(x => Owner.PartInfections.All(y => y.Bodypart != x || y.InfectionType != InfectionType))
                                    .GetRandomElement();
                if (bodypart != null)
                {
                    Owner.AddInfection(CreateSpreadInfection(Owner, null, bodypart));
