...
public class InfectiousInfection : SimpleInfection
{
    ...
    {
        ICharacter source = Owner.Actor;
        if (source?.Location == null)
        {
            return;
        }

        List<(ICharacter Character, Proximity Proximity)> targets = source.LocalThingsAndProximities()
        ...
        foreach ((ICharacter? target, Proximity proximity) in targets)
        {
    ...
    {
        double baseChance = proximity switch
        {
        ...
        double outcomeMultiplier = 1.0 + Math.Max(0, outcome.FailureDegrees()) *
                                Gameworld.GetStaticDouble("InfectiousInfectionSpreadChanceFailureDegreeMultiplier");
        double intensityMultiplier = Math.Max(
    ...
    {
        IWound? existingWound = target.Wounds.FirstOrDefault(x => x.Infection?.InfectionType == InfectionType);
        if (existingWound != null)
        {
            return;
        }

        IWound targetWound = target.Wounds.Where(x => x.EligableForInfection()).GetRandomElement();
        if (targetWound != null)
        {
