...
public abstract class BaseHealthStrategy : SaveableItem, IHealthStrategy
{
    ...
    {
        foreach (Type type in Futuremud.GetAllTypes().Where(x => x.IsSubclassOf(typeof(BaseHealthStrategy))))
        {
            MethodInfo method = type.GetMethod("RegisterHealthStrategyLoader", BindingFlags.Static | BindingFlags.Public);
    ...
    {
        return _typeHelps.TryGetValue(type, out string help)
    ...
    {
        if (!_databaseLoaders.TryGetValue(strategy.Type, out Func<HealthStrategy, IFuturemud, IHealthStrategy> loader))
        {
    ...
    {
        if (!_builderLoaders.TryGetValue(type, out Func<IFuturemud, string, IHealthStrategy> loader))
        {
            throw new ApplicationException(
    ...
    {
        List<string> lines = subtypeLines.ToList();
    ...
    private static string BuildBuilderHelp(IEnumerable<string> subtypeLines)
    {
        ...
        sb.AppendLine();
        foreach (string line in CommonBuilderHelpLines)
        {
            sb.AppendLine($"\t{line}");
        }
    ...
    {
        HealthStrategy dbitem = FMDB.Context.HealthStrategies.Find(Id);
        if (dbitem is null)
        {
            Changed = false;
            return;
    ...
    {
        XElement element = root.Element("SeverityRanges");
        if (element != null)
        {
            SeverityRanges = new RankedRange<WoundSeverity>();
            PercentageSeverityRanges = new RankedRange<WoundSeverity>();
            foreach (XElement item in element.Elements("Severity"))
            {
                double lower = double.Parse(item.Attribute("lower")!.Value, SystemCultureInfo.InvariantCulture);
                double upper = double.Parse(item.Attribute("upper")!.Value, SystemCultureInfo.InvariantCulture);
                WoundSeverity severity = (WoundSeverity)int.Parse(item.Attribute("value")!.Value, SystemCultureInfo.InvariantCulture);
                SeverityRanges.Add(severity, lower, upper);
                string lowerPercentage = item.Attribute("lowerperc")?.Value ?? item.Attribute("lowerpec")?.Value;
                string upperPercentage = item.Attribute("upperperc")?.Value;
    ...
    protected static int LoadInt(XElement root, string elementName, int defaultValue)
    {
        ...
        if (int.TryParse(value, NumberStyles.Integer, SystemCultureInfo.InvariantCulture, out int result))
        {
            return result;
        }
    ...
    protected static ITraitExpression CreateDefaultExpression(IFuturemud gameworld, string name, string formula)
    {
        using (new FMDB())
        {
            ...
            FMDB.Context.SaveChanges();
            TraitExpression expression = new(dbitem, gameworld);
    ...
    {
        TraitExpression clone = new((TraitExpression)expression);
        gameworld.Add(clone);
        return clone;
    }
    ...
    private void SetSeverityRanges(IEnumerable<(WoundSeverity Severity, double Lower, double Upper, double LowerPercentage, double UpperPercentage)> ranges)
    {
        SeverityRanges = new RankedRange<WoundSeverity>();
        PercentageSeverityRanges = new RankedRange<WoundSeverity>();
        foreach ((WoundSeverity Severity, double Lower, double Upper, double LowerPercentage, double UpperPercentage) range in ranges)
        {
    ...
    private void CopySeverityRangesFrom(BaseHealthStrategy rhs)
    {
        SeverityRanges = new RankedRange<WoundSeverity>();
        PercentageSeverityRanges = new RankedRange<WoundSeverity>();
        foreach (Range<WoundSeverity> range in rhs.SeverityRanges.Ranges)
        {
            SeverityRanges.Add(range.Value, range.LowerBound, range.UpperBound);
        }

        foreach (Range<WoundSeverity> range in rhs.PercentageSeverityRanges.Ranges)
        {
    ...
    protected bool BuildingCommandLodge(ICharacter actor, StringStack command)
    {
        ...
        Expression expression = new(command.SafeRemainingArgument);
    ...
    protected bool TryBuildingCommand<TStrategy>(ICharacter actor, StringStack command,
        ...
        where TStrategy : BaseHealthStrategy
    {
        string keyword = command.PopForSwitch();
        TraitExpressionBuilderField<TStrategy> field = fields.FirstOrDefault(x => x.Commands.Any(y => y.EqualTo(keyword)));
        if (field is null)
        {
            return false;
        }
        ...
        ITraitExpression expression = Gameworld.TraitExpressions.GetByIdOrName(command.SafeRemainingArgument);
    ...
    {
        string keyword = command.PopForSwitch();
        DoubleBuilderField<TStrategy> field = fields.FirstOrDefault(x => x.Commands.Any(y => y.EqualTo(keyword)));
        if (field is null)
        {
            return false;
        }
        ...
        {
            (bool Success, string Error, double Value) parse = field.Parser(actor, command.SafeRemainingArgument);
            if (!parse.Success)
            {
        ...
        {
            (bool Success, string Error, double Value) validation = field.Validator((TStrategy)this, value);
    ...
    {
        string keyword = command.PopForSwitch();
        IntBuilderField<TStrategy> field = fields.FirstOrDefault(x => x.Commands.Any(y => y.EqualTo(keyword)));
        if (field is null)
        {
            return false;
        }
        ...
        if (!TryParseInt(actor, command.SafeRemainingArgument, out int value))
        {
        ...
        if (field.Validator is not null)
        {
            (bool Success, string Error, int Value) validation = field.Validator((TStrategy)this, value);
            if (!validation.Success)
            {
                actor.OutputHandler.Send(validation.Error);
                return false;
    ...
    {
        string keyword = command.PopForSwitch();
        BoolBuilderField<TStrategy> field = fields.FirstOrDefault(x => x.Commands.Any(y => y.EqualTo(keyword)));
        if (field is null)
        {
            return false;
        }

        bool value = field.Getter((TStrategy)this);
        if (command.IsFinished)
        {
            value = !value;
        }
    ...
    {
        string keyword = command.PopForSwitch();
        TimeSpanBuilderField<TStrategy> field = fields.FirstOrDefault(x => x.Commands.Any(y => y.EqualTo(keyword)));
        if (field is null)
        {
            return false;
        }
        ...
        if (!TryParseDouble(actor, command.SafeRemainingArgument, out double seconds))
        {
        ...
        if (field.Validator is not null)
        {
            (bool Success, string Error, TimeSpan Value) validation = field.Validator((TStrategy)this, value);
            if (!validation.Success)
            {
                actor.OutputHandler.Send(validation.Error);
                return false;
    ...
    protected static void SaveBuilderFields<TStrategy>(XElement root, TStrategy strategy,
        ...
        where TStrategy : BaseHealthStrategy
    {
        foreach (TraitExpressionBuilderField<TStrategy> field in fields)
        {
    ...
    protected static void SaveBuilderFields<TStrategy>(XElement root, TStrategy strategy,
        ...
        where TStrategy : BaseHealthStrategy
    {
        foreach (DoubleBuilderField<TStrategy> field in fields)
        {
    ...
    protected static void SaveBuilderFields<TStrategy>(XElement root, TStrategy strategy,
        ...
        where TStrategy : BaseHealthStrategy
    {
        foreach (IntBuilderField<TStrategy> field in fields)
        {
    ...
    protected static void SaveBuilderFields<TStrategy>(XElement root, TStrategy strategy,
        ...
        where TStrategy : BaseHealthStrategy
    {
        foreach (BoolBuilderField<TStrategy> field in fields)
        {
            root.Add(new XElement(field.ElementName, field.Getter(strategy)));
        }
    }
    ...
    protected static void SaveBuilderFields<TStrategy>(XElement root, TStrategy strategy,
        ...
        where TStrategy : BaseHealthStrategy
    {
        foreach (TimeSpanBuilderField<TStrategy> field in fields)
        {
            root.Add(new XElement(field.ElementName,
        ...
        where TStrategy : BaseHealthStrategy
    {
        foreach (TraitExpressionBuilderField<TStrategy> field in fields)
        {
            ITraitExpression expression = field.Getter(strategy);
            sb.AppendLine(
    ...
    private static (bool Success, string Error, double Value) TryParsePercentage(ICharacter actor, string text)
    {
        ...
        if (!TryParseDouble(actor, sanitised, out double value))
        {
    ...
    {
        if (actor.Gameworld.UnitManager.TryGetBaseUnits(text, unitType, actor, out double value))
        {
    ...
    {
        double life = owner is ICharacter ch
