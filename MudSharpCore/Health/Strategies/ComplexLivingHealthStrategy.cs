...
public class ComplexLivingHealthStrategy : BaseHealthStrategy
{
...
#endif
        ICharacter chOwner = (ICharacter)owner;
        IGameItem lodgedItem = CheckDamageLodges(damage) ? damage.LodgableItem : null;

        if (bodypart is IBone bone)
        {
            List<IWound> wounds = new();
            (double ordinaryDamageAmount, double boneDamageAmount) = bone.ShouldBeBoneBreak(damage);
            if (boneDamageAmount > 0.0)
            {
                Damage boneDamage = new(damage) { DamageAmount = boneDamageAmount };
    ...
    private void DoTemperatureTick(ICharacter character)
    {
        ...
        (double floor, double ceiling) = character.Body.TolerableTemperatures(true);
        double temperature = character.Location.CurrentTemperature(character);
        Temperature subjective =
            TemperatureExtensions.SubjectiveTemperature(temperature, floor, ceiling);
        ThermalImbalance effect = DominantThermalImbalanceEffect(character);
    ...
    {
        ICharacter charOwner = (ICharacter)thing;
        ...
        if (charOwner.Body.CurrentBloodVolumeLitres > 0 && charOwner.LongtermExertion != ExertionLevel.Stasis)
        {
            List<BleedResult> bleedingWounds =
                thing.Wounds.Select(
                     ...
                     .ToList();
            double bleeding = bleedingWounds.Sum(x => x.BloodAmount);
            if (bleeding != 0)
            {
            ...
            List<BleedResult> visibleBleedingWounds = bleedingWounds.Where(x => x.Visible).ToList();
            if (visibleBleedingWounds.Any())
            {
                if (!charOwner.Body.EffectsOfType<ISuppressBleedMessage>().Any())
                {
                    ...
                    {
                        List<BleedResult> wounds = visibleBleedingWounds.Where(x => x.CoverItem == null).ToList();
                        string countDesc = WoundCountDesc(wounds.Count);
                        messages.Add(
                ...
                foreach (IPerceivable witness in charOwner.Location?.Perceivables ?? Enumerable.Empty<IPerceivable>())
                {
            ...
            {
                isBleeding = true;
                bool message = !thing.EffectsOfType<ISuppressBleedMessage>().Any();
                double airwayBlood = 0.0;
                double digestiveBlood = 0.0;
                charOwner.Body.CurrentBloodVolumeLitres -= internalBleeding;
                foreach (IInternalBleedingEffect effect in internalBleeders)
                {
                ...
                {
                    string amount = "a small amount of";
                    string action = "";
        ...
        double bloodRatio = charOwner.Body.CurrentBloodVolumeLitres / totalBlood;
        double oxygenHypoxia = 0.0;
        ...
        double bloodReplacementToBloodRatio =
        ...
        double breathRatio)
    {
        ...
        double stamRatio = 1.0 - charOwner.CurrentStamina / charOwner.MaximumStamina;
        sb.Append(" / Stam: ");
        switch (stamRatio)
        {
            case <= 0.0:
        ...
        if (breathRatio > 1.0)
        {
            double breathRatioReversed = 1.0 - breathRatio;
            sb.Append(" / Breath: ");
    ...
    public override void InjectedLiquid(IHaveWounds owner, LiquidMixture mixture)
    {
        ...
        foreach (LiquidInstance liquid in mixture.Instances)
        {
            ...
            IHarmfulBloodAdditiveEffect effect =
                cOwner.Body
    ...
    public override HealthTickResult EvaluateStatus(IHaveWounds thing)
    {
        ...
        double brainActivity = charOwner.Body.OrganFunction<BrainProto>();
        if (brainActivity <= 0)
        {
            return HealthTickResult.Dead;
        }
    ...
    private void RemoveContaminants(IBody body, List<IHarmfulBloodAdditiveEffect> contaminants)
    {
        ...
        IHarmfulBloodAdditiveEffect hydrate = contaminants.FirstOrDefault(x => x.Consequence == LiquidInjectionConsequence.Hydrating);
        if (hydrate != null)
        {
        ...
        IHarmfulBloodAdditiveEffect blood = contaminants.FirstOrDefault(x => x.Consequence == LiquidInjectionConsequence.BloodVolume);
        if (blood != null)
        {
    ...
    public override void PerformBloodGain(IHaveWounds owner)
    {
        ...
        ICharacter cOwner = (ICharacter)owner;
        IBody body = cOwner.Body;
        double totalBlood = body.TotalBloodVolumeLitres;
        if (body.CurrentBloodVolumeLitres / totalBlood < MinimumBloodRatioForRegeneration)
        {
            return;
        }

        List<IHarmfulBloodAdditiveEffect> contaminants =
        ...
        if (body.CurrentBloodVolumeLitres < totalBlood || contaminants.Any())
        {
                ...
                !body.NeedsModel.Status.HasFlag(NeedsResult.Thirsty))
            {
                ...
                {
                    double recoveryAmount = BloodRegenerationFraction *
                                         (cOwner.State.HasFlag(CharacterState.Sleeping)
                                             ? SleepingBloodRegenerationMultiplier
                                             : 1.0);

                    double newBlood = Math.Min(body.CurrentBloodVolumeLitres + recoveryAmount * totalBlood, totalBlood);
    ...
    {
        double spleenFunction = owner.OrganFunction<SpleenProto>();
        ...
        if (contaminant != null)
        {
            double multiplier = contaminant.Consequence == LiquidInjectionConsequence.BloodReplacement
                ? SpleenBloodReplacementCleanseMultiplier
                : SpleenHarmfulCleanseMultiplier;
