...
public class RobotHealthStrategy : BaseHealthStrategy
{
    ...
    public override HealthTickResult EvaluateStatus(IHaveWounds thing)
    {
        ...
        ILossOfConsciousnessEffect lossOfConsciousnessEffect = charOwner.Body.EffectsOfType<ILossOfConsciousnessEffect>().FirstOrDefault(x => x.Applies());
    ...
    {
        string fluidName = CirculatoryFluidName(liquidName);
        if (bloodlossRatio >= 0.95)
        {
            return string.Empty;
        }
