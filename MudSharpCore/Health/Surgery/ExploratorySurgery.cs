...
".SubstituteANSIColour();
        ...
        params object[] additionalArguments)
    {
        ...
        if (wounds.Any())
        {
            ...
            foreach (IGrouping<IOrganProto, IWound> organ in wounds.GroupBy(x => x.Bodypart as IOrganProto))
            {
                sb.AppendLine($"\tOn {organ.Key?.FullDescription().TitleCase() ?? "Unknown"}:");
                foreach (IWound wound in organ)
                {
        ...
        List<IInfection> infections = patient.Body.PartInfections
