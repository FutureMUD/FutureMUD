...
public class OrganStabilisationProcedure : OrganViaBodypartProcedure
{
    ...
    {
        int organDifficultyModifier = 0;
        IOrganProto organ = (IOrganProto)additionalArguments[1];
        switch (organ)
        {
            case BrainProto _:
        ...
        params object[] additionalArguments)
    {
        ...
        List<IWound> wounds = patient.Body.Wounds.Where(x => x.Bodypart == organ).ToList();
        ...
        List<IInternalBleedingEffect> bleeding =
        ...
        if (bleeding.Any())
        {
            ...
            double bleedReductionAmount = 0.0;
            ...
            foreach (IInternalBleedingEffect bleed in bleeding)
            {
                double newBleed = Math.Max(0, bleed.BloodlossPerTick - bleedReductionAmount);
                sb.AppendLine(
        ...
        List<IInfection> infections = patient.Body.PartInfections.Where(x => x.Bodypart.Id == organ.Id).ToList();
        if (infections.Any())
        {
            sb.AppendLine
            ...
            foreach (IInfection infection in infections)
            {
                sb.AppendLine(
        ...
        {
            double organFunction = patient.Body.OrganFunction(organ);
            double maxStabilisation = 0.5 + result.SuccessDegrees() * 0.1;
