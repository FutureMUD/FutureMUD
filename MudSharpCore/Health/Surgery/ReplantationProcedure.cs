...
public class ReplantationProcedure : BodypartSpecificSurgicalProcedure
{
        ...
        params object[] additionalArguments)
    {
        ...
        ILimb limb = patient.Body.GetLimbFor(bodypart);
        string replantationDescription = limb != null
...
".SubstituteANSIColour();
    ...
    protected override void SilentAbortProcedure(ICharacter surgeon, ICharacter patient, Outcome result,
        params object[] additionalArguments)
    {
        ISeveredBodypart item = (ISeveredBodypart)additionalArguments[0];
        AbortProg?.Execute(surgeon, patient, result, item.RootPart.Name, item.Parent);
    }
        ...
        params object[] additionalArguments)
    {
        ...
        Difficulty difficulty = rootPart.Significant ? Difficulty.Easy : Difficulty.ExtremelyEasy;
        ...
        foreach (IOrganProto organ in item.Organs)
        {
            patient.Body.RestoreOrgan(organ);
        }

        foreach (IBone bone in item.Bones)
        {
            patient.Body.RestoreBodypart(bone);
        }

        foreach (ITattoo tattoo in item.Tattoos)
        {
            patient.Body.AddTattoo(tattoo);
        }

        foreach (IWound wound in item.Wounds)
        {
            patient.Body.AddWound(wound);
            wound.SetNewOwner(patient);
        }

        foreach (IGameItem implant in item.Implants)
        {
        ...
        params object[] additionalArguments)
    {
        ...
        double replantationPartDamageThreshold = Gameworld.GetStaticDouble("ReplantationPartDamageThreshold");
        ...
        IBodypart rootPart = severItem.Parts.First(x => severItem.Parts.All(y => x.UpstreamConnection != y));
