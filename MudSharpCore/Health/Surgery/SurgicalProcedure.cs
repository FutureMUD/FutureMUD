...
public abstract class SurgicalProcedure : SaveableItem, ISurgicalProcedure
{
    ...
    public IEffect GetActionEffect(ICharacter surgeon, ICharacter patient, params object[] additionalArguments)
    {
        return new SurgicalProcedureEffect(surgeon, patient, this,
                    ...
                    select new Action<IPerceivable>(perceivable =>
                    {
                        ...
                        IInventoryPlan plan = phase.InventoryPlanTemplate
                            ?.CreatePlan(surgeon);
    ...
    {
        Models.SurgicalProcedure dbitem = FMDB.Context.SurgicalProcedures.Find(Id);
        dbitem.Name = Name;
        ...
        int phaseNumber = 1;
        foreach (SurgicalProcedurePhase phase in _phases)
        {
        ...
        IBodypart bodypartAffected, Difficulty baseDifficulty, bool worsenExisting = false)
    {
        ...
        Difficulty difficulty = baseDifficulty.StageDown(result.CheckDegrees());
        ...
        if (worsenExisting)
        {
            SurgeryFinalisationRequired effect = patient.CombinedEffectsOfType<SurgeryFinalisationRequired>()
    ...
    protected SurgicalProcedure(IFuturemud gameworld, string name, string gerund, IBodyPrototype body, string school, IKnowledge knowledge)
    {
        ...
        using (new FMDB())
        {
            Models.SurgicalProcedure dbitem = new()
            {
                Name = _name,
    ...
    private (Func<ICharacter, ICharacter, object[], bool> Success, Func<ICharacter, ICharacter, object[], string> Error, string Description) BleedingPhaseAction(Dictionary<Outcome, double> bleedingDictionary)
    {
        return ((surgeon, patient, parameters) =>
                {
                    CheckOutcome actualOutcome = Gameworld.GetCheck(Check).Check(surgeon, GetProcedureDifficulty(surgeon, patient, parameters), CheckTrait, patient);
    ...
    protected virtual void LoadFromDB(MudSharp.Models.SurgicalProcedure procedure)
    {
        ...
        foreach (Models.SurgicalProcedurePhase phase in procedure.SurgicalProcedurePhases)
        {
            SurgicalProcedurePhase nPhase = new()
            {
                PhaseEmote = phase.PhaseEmote,
...
{SpecialActionText}";
    ...
    {
        string cmd = command.PopForSwitch();
        switch (cmd)
        {
            case "add":
            case "new":
        ...
        if (!int.TryParse(cmd, out int value) || value < 1)
        {
        ...
        SurgicalProcedurePhase phase = _phases.ElementAtOrDefault(value - 1);
        if (phase is null)
        {
    ...
    private bool BuildingCommandPhaseInventoryRemove(ICharacter actor, StringStack command, SurgicalProcedurePhase phase)
    {
        ...
        IInventoryPlanAction action = phase.InventoryPlanTemplate.FirstPhase.Actions.ElementAtOrDefault(value - 1);
        if (action is null)
        {
    ...
    {
        IInventoryPlanAction action = InventoryPlanTemplate.ParseActionFromBuilderInput(actor, command);
        if (action is null)
        {
            return false;
        }
    ...
    {
        string helpText = $"That is not a valid special action. You must either specify a valid special action (see {"surgery set ?".MXPSend("surgery set ?")}), use {"remove <#>".ColourCommand()} to remove a specific one, or use {"none".ColourCommand()} to reset special actions.";
        if (command.IsFinished)
        {
        ...
        switch (command.PopSpeech())
        {
            ...
            case "bleed":
            case "blood":
                List<string> bloodTexts = new StringStack(command.RemainingArgument).PopSpeechAll().ToList();
                ...
                Dictionary<Outcome, double> bleedingDictionary = new();
                Outcome outcome = Outcome.MajorFail;
                foreach (string text in bloodTexts)
                {
                    if (!Gameworld.UnitManager.TryGetBaseUnits(text, UnitType.FluidVolume, actor, out double bleeding))
                    {
    ...
    private bool BuildingCommandPhaseEmote(ICharacter actor, StringStack command, SurgicalProcedurePhase phase)
    {
        ...
        if (command.IsFinished)
        {
            ...
            foreach (string item in phaseItems)
            {
                sb.AppendLine($"\t{item}");
            }
    ...
    private bool BuildingCommandPhaseSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value1) || value1 < 1)
        {
            actor.OutputHandler.Send(
        ...
        SurgicalProcedurePhase phase1 = _phases.ElementAtOrDefault(value1 - 1);
        if (phase1 is null)
        {
            actor.OutputHandler.Send(
        ...
        if (!int.TryParse(command.PopSpeech(), out int value2) || value2 < 1)
        {
            actor.OutputHandler.Send(
        ...
        SurgicalProcedurePhase phase2 = _phases.ElementAtOrDefault(value2 - 1);
        if (phase2 is null)
        {
            actor.OutputHandler.Send(
    ...
    {
        double bonusPerLevel = (double)StandardCheck.BonusesPerDifficultyLevel;
        if (command.IsFinished)
        {
    ...
    private bool BuildingCommandKnowledge(ICharacter actor, StringStack command)
    {
        ...
        IKnowledge knowledge = Gameworld.Knowledges.GetByIdOrName(command.SafeRemainingArgument);
...
Enter your new description in the editor below:");
    ...
    private bool BuildingCommandSchool(ICharacter actor, StringStack command)
    {
        if (command.IsFinished)
        {
            string[] existing = Gameworld.SurgicalProcedures.Select(x => x.MedicalSchool.TitleCase().ColourName()).Distinct().ToArray();
            if (existing.Any())
            {
    ...
    public virtual string Show(ICharacter actor)
    {
        ...
        int i = 1;
        foreach (SurgicalProcedurePhase phase in Phases)
        {
            sb.AppendLine();
            ...
            int j = 0;
            foreach (IInventoryPlanAction action in phase.InventoryPlanTemplate?.Phases.First().Actions ?? Enumerable.Empty<IInventoryPlanAction>())
            {
