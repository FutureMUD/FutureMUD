...
public class SurgicalSettingProcedure : BoneViaBodypartProcedure
{
        ...
        params object[] additionalArguments)
    {
        ...
        IBone bone = (IBone)additionalArguments[1];
        ...
        params object[] additionalArguments)
    {
        ...
        IBodypart bodypart = (IBodypart)additionalArguments[0];
        IBone bone = (IBone)additionalArguments[1];
        List<BoneFracture> wounds = patient.Wounds.Where(x => x.Bodypart == bone).OfType<BoneFracture>().ToList();
        BoneFracture wound = wounds.MinBy(x => x.CanBeTreated(TreatmentType.SurgicalSet));
        ...
        if (wound.CanBeTreated(TreatmentType.SurgicalSet) == Difficulty.Impossible)
        {
            ...
            foreach (BoneFracture wound1 in wounds)
            {
                sb.AppendLine(
        ...
        ICheck check = Gameworld.GetCheck(CheckType.SurgicalSetCheck);
        CheckOutcome outcome = check.Check(surgeon, wound.CanBeTreated(TreatmentType.SurgicalSet), patient);
    ...
    public override bool CanPerformProcedure(ICharacter surgeon, ICharacter patient,
        params object[] additionalArguments)
    {
        bool baseresult = base.CanPerformProcedure(surgeon, patient, additionalArguments);
        if (!baseresult)
        {
            return false;
        }
        ...
        IBodypart bodypart = (IBodypart)args[0];
        IBone bone = (IBone)args[1];
        BoneFracture wound = patient.Wounds.Where(x => x.Bodypart == bone).OfType<BoneFracture>().MinBy(x => x.CanBeTreated(TreatmentType.SurgicalSet));
        if (wound == null)
        {
            return false;
        }
