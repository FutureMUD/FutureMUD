...
public static class WoundExtensions
{
    public static void ProcessPassiveWounds(this IEnumerable<IWound> wounds)
    {
        ...
        List<ICharacter> chParent = parents.OfType<ICharacter>().ToList();
        bool supressing = chParent.Any(x => x.AffectedBy<SupressWoundMessages>());
        if (supressing)
        {
            foreach (IGameItem parent in itemParents)
            {
        ...
        if (supressing)
        {
            string message = chParent.SelectNotNull(x => x.CombinedEffectsOfType<SupressWoundMessages>().FirstOrDefault())
    ...
    public static double ApplyPainReduction(this IWound wound, ICharacter owner, double rawPain)
    {
        ...
        List<IPainReductionEffect> effects = owner.Body.EffectsOfType<IPainReductionEffect>()
                         .Where(x => x.Applies())
        ...
        double multiplier = effects.Aggregate(1.0, (sum, x) => sum * x.PainReductionMultiplier);
        double flatReduction = effects.Sum(x => x.FlatPainReductionAmount);
    ...
    {
        int result = (int)severity + degrees;
    ...
    {
        List<TreatmentType> treatmentTypes = Enum.GetValues(typeof(TreatmentType)).OfType<TreatmentType>().ToList();
        ...
        List<Tuple<TreatmentType, string>> diffNames = treatmentTypes.Select(x => Tuple.Create(x, Enum.GetName(typeof(TreatmentType), x))).ToList();
    ...
    {
        List<WoundSeverity> enums = Enum.GetValues(typeof(WoundSeverity)).OfType<WoundSeverity>().ToList();
        ...
        List<(WoundSeverity Value, string Name, string CamelCase)> enumsWithNames = enums.Select(x => (Value: x, Name: Enum.GetName(typeof(WoundSeverity), x),
