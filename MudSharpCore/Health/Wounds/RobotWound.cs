        ...
        double totalBloodLitres)
    {
        ...
        switch (_bleedStatus)
        {
            case BleedStatus.Bleeding:
                double bleedPercentage = Math.Max(0, (int)Severity + (int)activityExertionLevel - 4) *
                ...
                if (bleeding > 0)
                {
                    ...
                    if (!mixture.IsEmpty)
                    {
                        List<IGameItem> bindingItems = Parent.EffectsOfType<BeingBound>().Where(x => x.Bodypart == Bodypart)
                                                 .SelectMany(x => x.Binder.Body.HoldLocs.OfType<IWear>()
                                                                   .SelectNotNull(y =>
                                                                       x.Binder.Body.WornItemsFor(y).LastOrDefault()))
                                                 .ToList();
                        foreach (IGameItem bi in bindingItems)
                        {
