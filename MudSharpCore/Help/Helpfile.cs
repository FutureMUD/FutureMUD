...
public class Helpfile : SaveableItem, IEditableHelpfile
{
    ...
    public Helpfile(Models.Helpfile helpfile, IFuturemud gameworld)
    {
        ...
        foreach (HelpfilesExtraText et in helpfile.HelpfilesExtraTexts)
        {
    ...
    public string DisplayHelpFile(ICharacter actor)
    {
        ...
        foreach (Tuple<IFutureProg, string> addition in AdditionalTexts.Where(x => x.Item1?.ExecuteBool(actor) == true))
        {
            sb.AppendLine();
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Helpfile dbhelp = FMDB.Context.Helpfiles.Find(Id);
            dbhelp.Name = Name;
            ...
            int i = 0;
            foreach (Tuple<IFutureProg, string> et in _additionalTexts)
            {
                HelpfilesExtraText dbet = new()
                {
                    DisplayOrder = i++,
                    RuleId = et.Item1.Id,
                    Text = et.Item2
    ...
    public void Delete()
    {
        ...
        if (_id != 0)
        {
            using (new FMDB())
            {
                ...
                Models.Helpfile dbitem = FMDB.Context.Helpfiles.Find(Id);
                if (dbitem != null)
                {
