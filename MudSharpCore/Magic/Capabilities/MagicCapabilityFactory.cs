...
public static class MagicCapabilityFactory
{
    ...
    private static void InitialiseFactory()
    {
        ...
        Type iType = typeof(IMagicCapability);
        foreach (
            Type type in
        ...
        {
            MethodInfo method = type.GetMethod("RegisterLoader", BindingFlags.Static | BindingFlags.Public);
    ...
    public static IMagicCapability? LoaderFromBuilderInput(IFuturemud gameworld, ICharacter actor, StringStack input)
    {
        ...
        string name = input.PopSpeech().ToLowerInvariant().TitleCase();
