...
public class SkillLevelBasedMagicCapability : SaveableItem, IMagicCapability
{
    ...
    protected SkillLevelBasedMagicCapability(SkillLevelBasedMagicCapability rhs, string name)
    {
        ...
        foreach ((ITraitDefinition Trait, double MinValue, IMagicPower Power) power in rhs._skillPowerMap)
        {
    ...
    protected SkillLevelBasedMagicCapability(Models.MagicCapability capability, IFuturemud gameworld)
    {
        ...
        _resourceRegenerators.AddRange(root.Element("Regenerators")?.Elements().SelectNotNull(x =>
                           long.TryParse(x.Value, out long value)
    ...
    public static void RegisterLoader()
    {
        ...
        MagicCapabilityFactory.RegisterBuilderLoader("skilllevel", (game, builder, args, name) =>
        {
            ...
            IMagicSchool school = game.MagicSchools.GetByIdOrName(args.PopSpeech());
            if (school is null)
            {
            ...
            ITraitDefinition trait = game.Traits.GetByIdOrName(args.PopSpeech());
            if (trait is null)
            {
...
The following options have some more complex inputs:
    ...
    private bool BuildingCommandPower(ICharacter actor, StringStack command)
    {
        ...
        IMagicPower power = Gameworld.MagicPowers.GetByIdOrName(command.PopSpeech());
        if (power is null)
        {
    ...
    private bool BuildingCommandConcentrationDifficulty(ICharacter actor, StringStack command)
    {
        ...
        TraitExpression formula = new(command.SafeRemainingArgument, Gameworld);
    ...
    private bool BuildingCommandRegenerator(ICharacter actor, StringStack command)
    {
        ...
        IMagicResourceRegenerator regenerator = Gameworld.MagicResourceRegenerators.GetByIdOrName(command.SafeRemainingArgument);
    ...
    public string Show(ICharacter actor)
    {
        ...
        sb.AppendLine();
        foreach (IMagicResourceRegenerator regenerator in _resourceRegenerators)
        {
        ...
        sb.AppendLine();
        foreach ((ITraitDefinition Trait, double MinValue, IMagicPower Power) power in _skillPowerMap.OrderBy(x => x.Trait.Name).ThenBy(x => x.MinValue).ThenBy(x => x.Power.Name))
        {
    ...
    {
        Models.MagicCapability dbitem = FMDB.Context.MagicCapabilities.Find(Id);
        dbitem.Name = Name;
