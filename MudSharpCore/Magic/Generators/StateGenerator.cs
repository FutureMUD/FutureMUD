...
public class StateGenerator : BaseMagicResourceGenerator
{
    ...
    {
        foreach ((IFutureProg StateProg, IEnumerable<(IMagicResource Resource, double Amount)> Resources) state in rhs._states)
        {
    ...
    public StateGenerator(Models.MagicGenerator generator, IFuturemud gameworld) : base(generator, gameworld)
    {
        ...
        XElement states = root.Element("States");
        if (states == null)
        {
            throw new ApplicationException($"StateGenerator #{Id} ({Name}) is missing a States element.");
        }

        foreach (XElement sub in states.Elements())
        {
            List<(IMagicResource Resource, double Amount)> resources = new();
            foreach (XElement element in sub.Elements("Resource"))
            {
                IMagicResource whichResource = long.TryParse(element.Attribute("resource")?.Value ?? "0", out long value)
                ...
                string apm = element.Attribute("amountperminute")?.Value ?? "";

                if (!double.TryParse(apm, out double dvalue))
                {
                    throw new ApplicationException(
            ...
            XElement progElement = sub.Element("StateProg");
            if (progElement == null)
            {
                throw new ApplicationException($"StateGenerator #{Id} ({Name}) did not have a StateProg element.");
            }

            IFutureProg whichProg = long.TryParse(progElement.Value, out long pvalue)
    ...
    protected override HeartbeatManagerDelegate InternalGetOnMinuteDelegate(IHaveMagicResource thing)
    {
        return () =>
        {
            foreach ((IFutureProg StateProg, IEnumerable<(IMagicResource Resource, double Amount)> Resources) state in _states)
            {
                if (state.StateProg.Execute<bool?>(thing) == true)
                {
                    foreach ((IMagicResource resource, double amount) in state.Resources)
                    {
    ...
    {
        if (!int.TryParse(command.PopParentheses(), out int editIndex) || editIndex < 1 || editIndex > _states.Count)
        {
        ...
        if (!double.TryParse(command.SafeRemainingArgument, actor.Account.Culture, out double value))
        {
            actor.OutputHandler.Send($"The text {command.SafeRemainingArgument.ColourCommand()} is not a valid number.");
            return false;
        }
        (IFutureProg prog, IEnumerable<(IMagicResource Resource, double Amount)> resources) = _states[editIndex - 1];
        Changed = true;
        if (value == 0)
        {
    ...
    private bool BuildingCommandStateDelete(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopParentheses(), out int index) || index < 1 || index > _states.Count)
        {
            actor.OutputHandler.Send($"The text {command.Last.ColourCommand()} is not a valid index equal or less than {_states.Count.ToStringN0(actor)}.");
            return false;
        }

        (IFutureProg StateProg, IEnumerable<(IMagicResource Resource, double Amount)> Resources) state = _states[index - 1];
    ...
    private bool BuildingCommandStateSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopParentheses(), out int index1) || index1 < 1 || index1 > _states.Count)
        {
            actor.OutputHandler.Send($"The text {command.Last.ColourCommand()} is not a valid index equal or less than {_states.Count.ToStringN0(actor)}.");
            return false;
        }
        ...
        if (!int.TryParse(command.PopParentheses(), out int index2) || index2 < 1 || index2 > _states.Count)
        {
            actor.OutputHandler.Send($"The text {command.Last.ColourCommand()} is not a valid index equal or less than {_states.Count.ToStringN0(actor)}.");
            return false;
        }
    ...
    private bool BuildingCommandStateAdd(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.PopSpeech(), ProgVariableTypes.Boolean, [ProgVariableTypes.MagicResourceHaver]).LookupProg();
        if (prog is null)
        {
            return false;
        }
