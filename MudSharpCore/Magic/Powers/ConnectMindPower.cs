...
public class ConnectMindPower : SustainedMagicPower
{
    ...
    protected ConnectMindPower(MagicPower power, IFuturemud gameworld) : base(power, gameworld)
    {
        ...
        XElement element = root.Element("ConnectVerb");
        if (element == null)
        {
        ...
        TargetCanSeeIdentityProg = long.TryParse(element.Value, out long value)
        ...
        foreach (XElement item in element.Elements("OutcomeEcho"))
        {
            XAttribute outcomeAttribute = item.Attribute("outcome");
            ...
            XAttribute shouldEchoAttribute = item.Attribute("shouldecho");
        ...
        foreach (Outcome outcome in Enum.GetValues(typeof(Outcome)).OfType<Outcome>())
        {
    ...
    public void UseCommandConnect(ICharacter? actor, StringStack command)
    {
		...
		if (command.Peek().EqualTo("last"))
		{
            ...
            else
            {
                if (potentialTargets.Any())
                {
                    ...
                    foreach (MindConnectedToEffect badtarget in potentialTargets)
                    {
        ...
        else
        {
            ...
            string[] keywords = new[] { targetText };
            target = actor.TargetActor(targetText, PerceiveIgnoreFlags.IgnoreDark);
            if (target == null)
            {
                IDub dub = actor.Dubs.Where(x => x.TargetType.EqualTo("Character"))
        ...
        CheckOutcome result = results.Item1;
    ...
    public void UseCommandDisconnect(ICharacter actor, StringStack command)
    {
        ...
        {
            ICharacter targetCharacter = actor.EffectsOfType<ConnectMindEffect>().Where(x => x.PowerOrigin == this)
    ...
    {
        List<ConnectMindEffect> effects = actor.EffectsOfType<ConnectMindEffect>().Where(x => x.PowerOrigin == this).ToList();
        foreach (ConnectMindEffect effect in effects)
        {
    ...
    protected override void ShowSubtype(ICharacter actor, StringBuilder sb)
    {
        ...
        foreach (KeyValuePair<Outcome, bool> item in _outcomeEchoDictionary.OrderBy(x => x.Key))
        {
