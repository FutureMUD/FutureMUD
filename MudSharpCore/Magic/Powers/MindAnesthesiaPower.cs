...
public class MindAnesthesiaPower : SustainedMagicPower
{
    ...
    public override string DatabaseType => "mindanesthesia";
    ...
    protected MindAnesthesiaPower(Models.MagicPower power, IFuturemud gameworld) : base(power, gameworld)
    {
        ...
        XElement element = root.Element("BeginVerbs");
        if (element == null)
        {
        ...
        {
            string verb = sub.Value;
            double powerLevel = double.Parse(sub.Attribute("power")?.Value ??
                                          throw new ApplicationException(
                                              $"There was a BeginVerb with no power attribute in the definition XML for power {Id} ({Name})."));
            double costMultiplier = double.Parse(sub.Attribute("multiplier")?.Value ?? "1.0");
        ...
        if (element != null)
        {
            IFutureProg prog = long.TryParse(element.Value, out long lvalue)
        ...
        if (!Enum.TryParse<MagicPowerDistance>(element.Value, true, out MagicPowerDistance dist))
        {
            throw new ApplicationException(
        ...
        else
        {
            ...
            Emote resistEmote = new(TargetResistanceEmoteText, new DummyPerceiver(), new DummyPerceivable(),
    ...
    public override void UseCommand(ICharacter actor, string verb, StringStack command)
    {
        ...
        string powerVerb = PowerLevelVerbs.Keys.FirstOrDefault(x => x.EqualTo(verb)) ??
                        PowerLevelVerbs.Keys.First(x =>
                            x.StartsWith(verb, StringComparison.InvariantCultureIgnoreCase));
        (double powerLevel, double costMultiplier) = PowerLevelVerbs[powerVerb];

        ICharacter target = AcquireTarget(actor, command.PopSpeech(), PowerDistance);
        if (target == null)
        {
        ...
        ICheck check = Gameworld.GetCheck(CheckType.MagicAnesthesiaPower);
        Dictionary<Difficulty, CheckOutcome> results = check.CheckAgainstAllDifficulties(actor, SkillCheckDifficulty, SkillCheckTrait, target);

        if (TargetGetsResistanceCheck)
        {
            ICheck resistCheck = Gameworld.GetCheck(CheckType.ResistMagicAnesthesiaPower);
            Dictionary<Difficulty, CheckOutcome> resistResults = resistCheck.CheckAgainstAllDifficulties(target, ResistCheckDifficulty, null, actor);
            OpposedOutcome outcome = new(results, resistResults, SkillCheckDifficulty, ResistCheckDifficulty);
        ...
        {
            CheckOutcome result = results[SkillCheckDifficulty];
    ...
    private void UseCommandCancel(ICharacter actor, StringStack command)
    {
        ...
        List<ICharacter> targets = actor.EffectsOfType<MagicAnesthesia>(x => x.AnesthesiaPower == this)
                           .Select(x => x.CharacterTarget).ToList();
        ICharacter target = targets.GetFromItemListByKeyword(command.PopSpeech(), actor);
        if (target == null)
        {
    ...
    {
        foreach (KeyValuePair<string, (double Intensity, double Cost)> verb in PowerLevelVerbs)
        {
...
#6Note: Anesthesia intensity >1 = unconscious, >2.5 = breathing stops#0";
    ...
    private bool BuildingCommandRampAmount(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParsePercentage(actor.Account.Culture, out double value) || value <= 0.0 || value > 1.0)
        {
    ...
    private bool BuildingCommandRampRate(ICharacter actor, StringStack command)
    {
        ...
        if (!MudTimeSpan.TryParse(command.SafeRemainingArgument, actor, out MudTimeSpan ts))
        {
    ...
    private bool BuildingCommandEndVerb(ICharacter actor, StringStack command)
    {
        ...
        List<(IMagicResource Resource, double Cost)> costs = InvocationCosts[CancelVerb].ToList();
    ...
    private bool BuildingCommandBeginVerb(ICharacter actor, StringStack command)
    {
        ...
        string verb = command.PopSpeech().ToLowerInvariant();
        ...
        if (!command.SafeRemainingArgument.TryParsePercentage(actor.Account.Culture, out double cost))
        {
