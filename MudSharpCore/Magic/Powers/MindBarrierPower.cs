...
public class MindBarrierPower : SustainedMagicPower
{
    ...
    private MindBarrierPower(IFuturemud gameworld, IMagicSchool school, string name, ITraitDefinition trait) : base(gameworld, school, name)
    {
        ...
        FailIfOvercome = true;
        EmoteForBegin = PsionicPowerEmotes.Get("mindbarrier", "EmoteForBegin");
        EmoteForBeginSelf = PsionicPowerEmotes.Get("mindbarrier", "EmoteForBeginSelf");
        EmoteForEnd = PsionicPowerEmotes.Get("mindbarrier", "EmoteForEnd");
        EmoteForEndSelf = PsionicPowerEmotes.Get("mindbarrier", "EmoteForEndSelf");
        BlockEmoteSelf = PsionicPowerEmotes.Get("mindbarrier", "BlockEmoteSelf");
        BlockEmoteTarget = PsionicPowerEmotes.Get("mindbarrier", "BlockEmoteTarget");
        OvercomeEmoteSelf = PsionicPowerEmotes.Get("mindbarrier", "OvercomeEmoteSelf");
        OvercomeEmoteTarget = PsionicPowerEmotes.Get("mindbarrier", "OvercomeEmoteTarget");
        EndWhenNotSustainingError = PsionicPowerEmotes.Get("mindbarrier", "EndWhenNotSustainingError");
        BeginWhenAlreadySustainingError = PsionicPowerEmotes.Get("mindbarrier", "BeginWhenAlreadySustainingError");
    ...
    protected MindBarrierPower(MagicPower power, IFuturemud gameworld) : base(power, gameworld)
    {
        ...
        XElement element = root.Element("BeginVerb");
        if (element == null)
        {
        ...
        foreach (XElement sub in element.Elements("Bonus"))
        {
            XAttribute outcomeAttribute = sub.Attribute("outcome");
            ...
            double bonus = double.Parse(bonusAttribute.Value);

            TargetCheckBonusPerOutcome[outcome] = bonus;
            foreach (Outcome value in Enum.GetValues(typeof(Outcome)).OfType<Outcome>())
            {
    ...
    {
        List<MindBarrierEffect> effects = actor.EffectsOfType<MindBarrierEffect>().Where(x => x.PowerOrigin == this).ToList();
        if (!effects.Any())
        {
        ...
        foreach (MindBarrierEffect effect in effects)
        {
    ...
    private void UseCommandBegin(ICharacter actor, StringStack command)
    {
        ...
        ICheck check = Gameworld.GetCheck(CheckType.MindBarrierPowerCheck);
        CheckOutcome result = check.Check(actor, SkillCheckDifficulty, SkillCheckTrait);
    ...
    protected override void ShowSubtype(ICharacter actor, StringBuilder sb)
    {
        ...
        foreach (KeyValuePair<Outcome, double> item in TargetCheckBonusPerOutcome.OrderBy(x => x.Key))
        {
...
#6Note - for all emotes, $0 is the power user and $1 the intruder#0.";
    ...
    private bool BuildingCommandBonus(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<Outcome>(out Outcome value))
        {
    ...
    private bool BuildingCommandBeginEmote(ICharacter actor, StringStack command)
    {
        ...
        string text = command.SafeRemainingArgument.ProperSentences();
        Emote testEmote = new(text, new DummyPerceiver(), new DummyPerceivable());
        if (!testEmote.Valid)
        {
    ...
    private bool BuildingCommandBlockEmoteSelf(ICharacter actor, StringStack command)
    {
        ...
        string text = command.SafeRemainingArgument.ProperSentences();
        Emote testEmote = new(text, new DummyPerceiver(), new DummyPerceivable(), new DummyPerceivable());
        if (!testEmote.Valid)
        {
            actor.OutputHandler.Send(testEmote.ErrorMessage);
            return false;
