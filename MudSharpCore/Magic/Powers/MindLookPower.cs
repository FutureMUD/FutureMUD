...
public class MindLookPower : MagicPowerBase
{
    ...
    protected MindLookPower(MagicPower power, IFuturemud gameworld) : base(power, gameworld)
    {
        ...
        XElement element = root.Element("LookRoomVerb");
        if (element == null)
        {
        ...
        if (!int.TryParse(element.Value, out int ivalue))
        {
            if (!CheckExtensions.GetDifficulty(element.Value, out Difficulty diff))
            {
                throw new ApplicationException(
        ...
        if (!int.TryParse(element.Value, out ivalue))
        {
            if (!CheckExtensions.GetOutcome(element.Value, out Outcome outcome))
            {
                throw new ApplicationException(
        ...
        ITraitDefinition trait = long.TryParse(element.Value, out long value)
    ...
    public override void UseCommand(ICharacter actor, string verb, StringStack command)
    {
        ...
        ConnectMindEffect effect = actor.EffectsOfType<ConnectMindEffect>().FirstOrDefault(x => x.School == School);
        if (effect == null)
        {
            actor.OutputHandler.Send(
        ...
        ICheck check = Gameworld.GetCheck(CheckType.MindLookPower);
        ...
        CheckOutcome outcome = check.Check(actor, difficulty, SkillCheckTrait, effect.TargetCharacter);
        ...
        switch (everb)
        {
            ...
            default:
                ...
                IPerceivable target = effect.TargetCharacter.Target(command.PopSpeech());
                if (target == null)
                {
...
#6Note - for all emotes, $0 is the power user and $1 the target#0.";
    ...
    private bool BuildingCommandLookRoomVerb(ICharacter actor, StringStack command)
    {
        ...
        List<(IMagicResource Resource, double Cost)> costs = InvocationCosts[LookRoomVerb].ToList();
    ...
    private bool BuildingCommandLookThingVerb(ICharacter actor, StringStack command)
    {
        ...
        List<(IMagicResource Resource, double Cost)> costs = InvocationCosts[LookThingVerb].ToList();
        InvocationCosts[verb] = costs;
    ...
    private bool BuildingCommandLookInThingVerb(ICharacter actor, StringStack command)
    {
        ...
        List<(IMagicResource Resource, double Cost)> costs = InvocationCosts[LookInThingVerb].ToList();
        InvocationCosts[verb] = costs;
