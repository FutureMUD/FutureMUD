...
public class MindSayPower : MagicPowerBase
{
    ...
    public override string DatabaseType => "mindsay";
    ...
    protected MindSayPower(MagicPower power, IFuturemud gameworld) : base(power, gameworld)
    {
        ...
        XElement element = root.Element("UnknownIdentityDescription");
        if (element == null)
        {
    ...
    public override void UseCommand(ICharacter actor, string verb, StringStack command)
    {
        ...
        if (verb.EqualTo(TellVerb))
        {
            ...
            ICharacter targetCharacter = actor.EffectsOfType<ConnectMindEffect>().Where(x => x.School == School)
        ...
        ICheck check = Gameworld.GetCheck(CheckType.MindSayPower);
        CheckOutcome outcome = check.Check(actor, SkillCheckDifficulty, SkillCheckTrait, effect.TargetCharacter);
        ...
        string text = command.RemainingArgument;
        ...
        if (UseLanguage)
        {
            PsychicLanguageInfo langInfo = new(actor.CurrentLanguage, UseAccent ? actor.CurrentAccent : null,
                text, outcome, actor);
...
#6Note - for emotes, use $0 for the user and $1 for the target";
    ...
    private bool BuildingCommandSayVerb(ICharacter actor, StringStack command)
    {
        ...
        List<(IMagicResource Resource, double Cost)> costs = InvocationCosts[SayVerb].ToList();
    ...
    private bool BuildingCommandTellVerb(ICharacter actor, StringStack command)
    {
        ...
        List<(IMagicResource Resource, double Cost)> costs = InvocationCosts[TellVerb].ToList();
        InvocationCosts[verb] = costs;
