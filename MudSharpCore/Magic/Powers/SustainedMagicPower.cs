...
public abstract class SustainedMagicPower : MagicPowerBase
{
    protected SustainedMagicPower(Models.MagicPower power, IFuturemud gameworld) : base(power, gameworld)
    {
        ...
        XElement element = root.Element("ConcentrationPointsToSustain");
        if (element == null)
        {
        ...
        {
            List<(IMagicResource Resources, double Cost)> list = new();
            foreach (XElement sub in element.Elements())
            {
                ...
                IMagicResource resource = long.TryParse(which, out long value)
    ...
    {
        bool remove = false;
        foreach ((IMagicResource resource, double cost) in SustainCostsPerMinute)
        {
    ...
    {
        StringBuilder sb = new(base.ShowHelp(voyeur));
        sb.AppendLine();
