...
public class ChangeCharacteristicEffect : IMagicSpellEffectTemplate
{
    ...
    IMagicSpell spell)
    {
        ...
        ICharacteristicDefinition? definition = spell.Gameworld.Characteristics.GetByIdOrName(commands.PopSpeech());
        ...
        string valueText = commands.SafeRemainingArgument;
        if (valueText[0] == '*' && valueText.Length > 1)
        {
            valueText = valueText[1..];
            ICharacteristicProfile? profile = spell.Gameworld.CharacteristicProfiles.Where(x => x.IsProfileFor(definition)).GetByIdOrName(valueText);
            if (profile is null)
            {
        ...
        ICharacteristicValue? value = spell.Gameworld.CharacteristicValues.Where(x => definition.IsValue(x)).GetByIdOrName(valueText);
        if (value is null)
        {
        ...
        SpellPower power, IMagicSpellEffectParent parent, SpellAdditionalParameter[] additionalParameters)
    {
        ...
        ICharacter? ch = target as ICharacter;
    ...
    private bool BuildingCommandProfile(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicProfile? profile = Gameworld.CharacteristicProfiles.Where(x => x.IsProfileFor(WhichCharacteristic)).GetByIdOrName(command.SafeRemainingArgument);
        if (profile is null)
        {
    ...
    private bool BuildingCommandValue(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicValue? value = Gameworld.CharacteristicValues.Where(x => WhichCharacteristic.IsValue(x)).GetByIdOrName(command.SafeRemainingArgument);
        if (value is null)
        {
    ...
    private bool BuildingCommandDefinition(ICharacter actor, StringStack command)
    {
        ...
        {
            valueText = valueText[1..];
            ICharacteristicProfile? profile = Gameworld.CharacteristicProfiles.Where(x => x.IsProfileFor(definition)).GetByIdOrName(valueText);
            if (profile is null)
            {
        ...
        ICharacteristicValue? value = Gameworld.CharacteristicValues.Where(x => definition.IsValue(x)).GetByIdOrName(valueText);
        if (value is null)
        {
