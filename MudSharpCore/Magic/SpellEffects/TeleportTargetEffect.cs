...
public class TeleportTargetEffect : IMagicSpellEffectTemplate
{
    public static void RegisterFactory()
    {
        SpellEffectFactory.RegisterLoadTimeFactory("teleporttarget", (root, spell) => new TeleportTargetEffect(root, spell));
        SpellEffectFactory.RegisterBuilderFactory("teleporttarget", BuilderFactory,
            "Teleports a target character and/or their party",
            HelpText,
            true,
            true,
    ...
    private static (IMagicSpellEffectTemplate Trigger, string Error) BuilderFactory(StringStack commands,
        IMagicSpell spell)
    {
        return (new TeleportTargetEffect(new XElement("Effect",
            new XAttribute("type", "teleporttarget"),
    ...
    protected TeleportTargetEffect(XElement root, IMagicSpell spell)
    {
        Spell = spell;
    ...
    {
        return new TeleportTargetEffect(SaveToXml(), Spell);
    }
    ...
    private bool BuildingCommandLayer(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<RoomLayer>(out RoomLayer layer))
        {
    ...
    private bool BuildingCommandParty(ICharacter actor)
    {
        ...
        actor.OutputHandler.Send(
            TeleportParty ?
                "This spell will now also teleport the target's party, as well as anything or anyone they're dragging or grappling." :
                "This spell will now only teleport the target"
        );
        return true;
    }
