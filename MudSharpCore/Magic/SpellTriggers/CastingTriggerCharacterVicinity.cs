...
public class CastingTriggerCharacterVicinity : CastingTriggerBase
{
    ...
    {
        return (
            new CastingTriggerCharacterVicinity(
    ...
    protected CastingTriggerCharacterVicinity(XElement root, IMagicSpell spell) : base(root, spell)
    {
    ...
    public override XElement SaveToXml()
    {
        return new XElement("Trigger",
            new XAttribute("type", "charactervicinity"),
    ...
    {
        return new CastingTriggerCharacterVicinity(SaveToXml(), Spell);
    }
    ...
    public override void DoTriggerCast(ICharacter actor, StringStack additionalArguments)
    {
        ...
        IPerceivable target = actor.TargetLocal(additionalArguments.SafeRemainingArgument);
        if (target == null)
        {
        ...
        foreach ((IPerceivable other, Proximity proximity) in target.LocalThingsAndProximities() ?? new[] { (target, Proximity.Intimate) })
        {
    ...
    {
        return
            $"{"Cast@CharacterProximity".ColourName()} {Proximity.Describe().ColourValue()} {(CanTargetSelf ? "" : " [noself]".ColourName())} - {base.Show(actor)}{(TargetFilterProg != null ? $" Filter: {TargetFilterProg.MXPClickableFunctionName()}" : "")}";
    }
