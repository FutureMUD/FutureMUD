...
public class CastingTriggerItem : CastingTriggerBase
{
    ...
    {
        return new CastingTriggerItem(SaveToXml(), Spell);
    }
    ...
    public override void DoTriggerCast(ICharacter actor, StringStack additionalArguments)
    {
        ...
        IGameItem target = actor.TargetItem(additionalArguments.SafeRemainingArgument);
        if (target == null)
        {
    ...
    {
        return
            $"Cast Command - {Spell.School.SchoolVerb} cast {(Spell.Name.Contains(' ') ? Spell.Name.ToLowerInvariant().DoubleQuotes() : Spell.Name.ToLowerInvariant())} <power> <target item>";
    }
