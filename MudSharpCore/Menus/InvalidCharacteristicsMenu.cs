...
public class InvalidCharacteristicsMenu : CharacterLoginMenu
{
    ...
    public override bool ExecuteCommand(string command)
    {
        ...
        (ICharacteristicDefinition Definition, ICharacteristicValue Value) current = InvalidValues.First();
        List<ICharacteristicValue> choices = Gameworld.CharacteristicValues.Where(x =>
        ...
        if (int.TryParse(command, out int value))
        {
            ...
            ICharacteristicValue choice = choices.ElementAt(value - 1);
        ...
        ICharacteristicValue? textChoice = choices.FirstOrDefault(x => x.GetValue.EqualTo(command));
        if (textChoice != null)
        {
