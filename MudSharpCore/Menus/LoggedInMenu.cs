...
internal class LoggedInMenu : Menu
{
    ...
    private void CreateCharacter(string command)
    {
        using (new FMDB())
        {
            ...
            int inprogressCount = (from chargen in FMDB.Context.Chargens
                                   where chargen.AccountId == Account.Id
        ...
        ChargenMenu menu = new(Account, Gameworld);
        _nextContext = menu;
    }
    ...
    private void WithdrawApplication(string playerInput)
    {
        using (new FMDB())
        {
            List<Chargen> chargensQuery = (from chargen in FMDB.Context.Chargens
                                           where
            ...
            CommandManager characterSelectionManager = new()
            {
            ...
            int nextChoice = 1;
            foreach (Chargen character in chargensQuery)
            {
    ...
    private void SelectWithdrawApplication(Chargen chargen, string command)
    {
        using (new FMDB())
        {
            Chargen dbitem = FMDB.Context.Chargens.Find(chargen.Id);
            if (dbitem == null)
            {
    ...
    private void ConnectCharacter(string command)
    {
        using (new FMDB())
        {
            List<Models.Character> characterQuery = (from character in FMDB.Context.Characters
                                                     where character.AccountId == Account.Id
    ...
    private void SelectCharacter(MudSharp.Models.Character character, string command)
    {
        ...
        ICharacter loginCharacter = Gameworld.Characters.Get(character.Id);
        if (loginCharacter == null)
        {
            using (new FMDB())
            {
                ...
                {
                    ICharacterIntroTemplate template = Gameworld.CharacterIntroTemplates.OrderByDescending(x => x.ResolutionPriority)
            ...
            IScriptedEvent scripted = Gameworld.ScriptedEvents.FirstOrDefault(x => x.IsReady && !x.IsFinished && x.Character == loginCharacter && x.EarliestDate <= DateTime.UtcNow);
    ...
    private void ResendRegistrationEmailConfirm(string command)
    {
        ...
        if ("yes".StartsWith(command, StringComparison.InvariantCultureIgnoreCase))
        {
            using (new FMDB())
            {
                Models.Account dbAccount = FMDB.Context.Accounts.Find(Account.Id);
        ...
        if (Regex.Match(command, Constants.EmailRegex, RegexOptions.IgnoreCase).Success)
        {
            using (new FMDB())
            {
                ...
                else
                {
                    ...
                    Models.Account dbAccount = FMDB.Context.Accounts.FirstOrDefault(acc => acc.Id == Account.Id);
                    if (dbAccount != null)
                    {
    ...
    private void CheckOldPassword(string command)
    {
        using (new FMDB())
        {
            Models.Account currentAccount = (from account in FMDB.Context.Accounts
