...
public class MainMenu : Menu, IController
{
        ...
        : base(string.Empty, gameworld)
    {
        ...
        CommandManager frontPageManager = new("Invalid command. Please enter an option from the menu.\n");
    ...
    public bool CheckBanned()
    {
        try
        {
            using (new FMDB())
            {
                ...
                IEnumerable<int> count = FMDB.Connection.Query<int>(
    ...
    {
        if (!uint.TryParse(command, out uint value) || value > 2)
        {
            OutputHandler.Send(
    ...
    private void RecoverPasswordSendEmail(string command)
    {
        ...
        using (new FMDB())
        {
            ...
            Models.Account account = match.Success
            ...
            string code = SecurityUtilities.GetRandomString(20, Constants.ValidRandomCharacters.ToCharArray());
    ...
    private void SelectUnitPreference(string command)
    {
        ...
        string preference = int.TryParse(command, out int value)
    ...
    {
        if (!uint.TryParse(command, out uint value))
        {
            OutputHandler.Send(
    ...
    private void CreateAccountCulture(string command)
    {
        ...
        if (int.TryParse(command, out int value))
        {
            using (new FMDB())
            {
                Models.CultureInfo whichCulture = (from culture in FMDB.Context.CultureInfos
                                                   orderby culture.Order, culture.Id
    ...
    private void CreateAccountTimezoneGMTOffset(string command)
    {
        ...
        Match match = _gmtOffsetRegex.Match(command);
        if (!match.Success)
        {
        ...
        using (new FMDB())
        {
            if (!FMDB.Context.TimeZoneInfos.AsNoTracking().Any(x => x.Order == number))
            {
                ...
                Models.TimeZoneInfo closest = closestHigher ?? closestLower;
                OutputHandler.Send(
                    string.Format(_selectedCulture,
    ...
    private void CreateAccountTimezone(string command)
    {
        ...
        if (int.TryParse(command, out int value))
        {
            using (new FMDB())
            {
                Models.TimeZoneInfo whichTz = (from tz in FMDB.Context.TimeZoneInfos
                                               where !_timezoneGMTOffset.HasValue || _timezoneGMTOffset.Value == tz.Order
                                               orderby tz.Order, tz.Display
    ...
    {
        IAccount loginAttempt = Gameworld.LogIn(_accountName, command, AccountController);

        if (loginAttempt != null)
        {
            lock (Gameworld.Connections)
            {
                IPlayerConnection existingConnection =
