...
public class ScriptedEventMenu : CharacterLoginMenu
{
    ...
    public string ShowMenuText()
    {
        ...
        if (_currentMultipleChoiceQuestion is not null)
        {
            ...
            int i = 1;
            foreach (IScriptedEventMultipleChoiceQuestionAnswer? answer in _currentMultipleChoiceQuestion.Answers.Where(x => x.AnswerFilterProg?.Execute<bool?>(Character) != false))
            {
                sb.AppendLine();
        ...
        if (_currentFreeTextQuestion is not null)
        {
            ...
            IController original = Controller;
    ...
    public override bool ExecuteCommand(string command)
    {
        ...
        if (!int.TryParse(command, out int index) || index < 1 || index > _currentMultipleChoiceQuestion!.Answers.Count())
        {
        ...
        IScriptedEventMultipleChoiceQuestionAnswer answer = _currentMultipleChoiceQuestion.Answers.ElementAt(index - 1);
