...
public class Movement : IMovement
{
        ...
        )
    {
        ...
        IDragParticipant effect = character.EffectsOfType<IDragParticipant>().FirstOrDefault();
        ...
        foreach (ICharacter rider in character.Riders)
        {
        ...
        switch (effect)
        {
            case Dragging dragging:
                ...
                foreach (ICharacter helper in dragging.Helpers)
                {
    ...
    public static IMovement CreateMovement(ICharacter originalMover, ICellExit exit, IEmote emote = null, bool ignoreSafeMovement = false)
    {
        ...
        List<ICharacter> failedMovers = new();
        List<ICharacter> mounts = new();
        List<ICharacter> nonConsensualMovers = new();
        List<ICharacter> draggers = new();
        List<ICharacter> nondraggers = new();
        List<ICharacter> helpers = new();
        List<IPerceivable> targets = new();
        List<Dragging> effects = new();
        List<ICharacter> consideredMovers = new();
        ...
        )
    {
        ...
        foreach (ICharacter mover in NonDraggers)
        {
            ISneakEffect sneak = mover.EffectsOfType<ISneakEffect>().FirstOrDefault();
            if (sneak is null)
            {
                continue;
            }

            CheckOutcome stealthOutcome = mover.Gameworld.GetCheck(CheckType.SneakCheck)
                                  .Check(mover, mover.Location.Terrain(mover).HideDifficulty);
            SneakMove effect = new(mover)
            {
                Subtle = sneak.Subtle,
        ...
        foreach (ICharacter ch in CharacterMovers)
        {
            ...
            TimeSpan speed = TimeSpan.FromMilliseconds(ch.MoveSpeed(exit));
            if (speed > duration)
            {
                duration = speed;
            }
    ...
    public bool CancelForMoverOnly(IMove mover, bool echo = false)
    {
        ...
        if (ch.RidingMount is not null)
        {
            ...
            foreach (ICharacter rider in ch.RidingMount.Riders)
            {
                RemoveMover(rider);
                if (echo)
                {
        ...
        foreach (ICharacter rider in ch.Riders)
        {
            RemoveMover(rider);
            if (echo)
            {
    ...
    public void StopMovement()
    {
        ...
        else
        {
            ...
            {
                PerceivableGroup pg = new(CharacterMovers);
        ...
        foreach (ICharacter mover in CharacterMovers)
        {
            ...
            foreach (IGameItem witness in mover.Body.ExternalItems)
            {
                witness.HandleEvent(EventType.CharacterStopMovementWitness, mover, Exit.Origin, Exit, witness);
            }
        }
    ...
    public string DescribeEnterMove(IPerceiver voyeur)
    {
        ...
        string suffix = Phase == MovementPhase.OriginalRoom ? Exit.OutboundMovementSuffix : Exit.InboundMovementSuffix;
        List<ICharacter> seenMovers = CharacterMovers
        ...
        {
            foreach (IGrouping<(bool Riding, IMoveSpeed Speed, bool Sneaking), ICharacter> group in seenMovers.GroupBy(x => (Riding: x.RidingMount is not null, Speed: x.RidingMount?.CurrentSpeed ?? x.CurrentSpeed, Sneaking: (x.RidingMount ?? x).AffectedBy<ISneakMoveEffect>(y => !y.Subtle))))
            {
    ...
    public bool SeenBy(IPerceiver voyeur, PerceiveIgnoreFlags flags = PerceiveIgnoreFlags.None)
    {
        foreach (ICharacter mover in CharacterMovers)
        {
            ...
            ISneakMoveEffect sneakMoveEffect = SneakMoveEffects[mover];
            if (sneakMoveEffect is null)
            {
                continue;
            }

            ISawSneakerEffect sawSneakEffect = voyeur.EffectsOfType<ISawSneakerEffect>().FirstOrDefault(x => x.Sneaker == mover);

            if (sawSneakEffect == null && flags.HasFlag(PerceiveIgnoreFlags.IgnoreSpotting))
            {
                CheckOutcome voyeurCheck = mover.Gameworld.GetCheck(CheckType.SpotSneakCheck)
                                       .Check(voyeurCh,
                                           voyeur.Location.SpotDifficulty(voyeur).StageDown(sneakMoveEffect.Subtle ? 2 : 0),
                                           mover);
                OpposedOutcome opposed = new(sneakMoveEffect.StealthOutcome, voyeurCheck);
    ...
    public void InitialAction()
    {
        ...
        foreach (ICharacter ch in Exit.Origin.LayerCharacters(OriginLayer).Where(x => SeenBy(x)))
        {
        ...
        foreach (Dragging effect in DragEffects)
        {
            ...
            ICharacter leader = Draggers.First(x => effect.CharacterDraggers.Contains(x));
            List<ICharacter> helpers = Helpers.Where(x => effect.CharacterDraggers.Contains(x)).ToList();
            foreach (ICharacter ch in helpers)
            {
    ...
    public void IntermediateStep()
    {
        ...
        if (nonMovers.Count > 0)
        {
            ...
            foreach (ICharacter mover in nonMovers)
            {
        ...
        List<ICharacter> stuck = CharacterMovers.Where(x => !x.CanCross(Exit).Success).ToList();
        if (stuck.Any())
        {
            foreach (ICharacter ch in stuck)
            {
                (bool _, IEmoteOutput why) = ch.CanCross(Exit);
        ...
        List<ICharacter> movers = CharacterMovers.ToList();
        foreach (Dragging effect in DragEffects)
        {
        ...
        foreach (IPerceivable target in Targets)
        {
        ...
        TimeSpan finalDelay = TimeSpan.FromTicks(Duration.Ticks / 5);
    ...
    {
        ITrack track = DepartureTracks[mover];
        if (track is null)
        {
            return;
        }
    ...
    protected void CreateDepartureTracks(ICharacter actor, TrackCircumstances circumstance)
    {
        ...
        Track track = new(actor.Gameworld, actor, Exit, circumstance, true, visual, olfactory);
