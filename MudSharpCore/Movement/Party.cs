...
public class Party : PerceiverItem, IParty
{
    ...
    {
        Moved(movement);
        foreach (IMove member in ActiveMembers.ToList())
        {
    ...
    public void Disband()
    {
        ...
        foreach (IMove member in _members.ToList())
        {
    ...
    public override (bool, IEmoteOutput) CanCross(ICellExit exit)
    {
        ...
        {
            (bool canMove, IEmoteOutput whyNot) = member.CanCross(exit);
            if (!canMove)
            {
                return (false, whyNot);
            }
    ...
    public IEnumerable<ICharacter> CharacterMembers => _members.OfType<ICharacter>().Concat(_members.OfType<IParty>().SelectMany(x => x.CharacterMembers));
    ...
    public CanMoveResponse CanMove(CanMoveFlags flags)
    {
        ...
        foreach (ICharacter ch in ActiveCharacterMembers)
        {
            if (ch.CanMove(flags))
            {
                if (Leader == ch)
    ...
    public void LeaveParty(bool echo = true)
    {
        ...
        {
            foreach (IMove ch in _party.Members.ToList())
            {
                if (echo)
                {
    ...
    public IMoveSpeed SlowestSpeed(ICellExit exit)
    {
        ...
        ICharacter slowest = partyMembers.FirstOrDefault();
