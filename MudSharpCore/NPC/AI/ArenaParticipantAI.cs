...
public class ArenaParticipantAI : PathingAIBase
{
	...
	public override bool HandleEvent(EventType type, params dynamic[] arguments)
	{
		...
		{
			var character = arguments[0] as ICharacter;
	...
	{
		var participant = GetParticipant(arenaEvent, character);
	...
	{
		var prepEffect = character.CombinedEffectsOfType<ArenaNpcPreparationEffect>()
		...
		var arenaEvent = Gameworld.CombatArenas
	...
	private void EngageArenaOpponents(ICharacter character, IArenaEvent arenaEvent)
	{
		...
		var opponents = GetOpponents(arenaEvent, character)
	...
	private bool TryEngageVisibleOpponent(ICharacter character, IEnumerable<ICharacter> opponents)
	{
		...
		var rangedRange = character.Body.WieldedItems
		...
		var rangedTarget = character.Location.CellsInVicinity((uint)rangedRange, true, true)
	...
	private void ExecuteArenaEngagement(ICharacter character, ICharacter target)
	{
		...
		var shouldUseRanged = character.Body.WieldedItems
			.SelectNotNull(x => x.GetItemType<IRangedWeapon>())
	...
	{
		foreach (var weapon in character.Body.WieldedItems.SelectNotNull(x => x.GetItemType<IRangedWeapon>()))
		{
	...
	{
		var prepEffect = ch.CombinedEffectsOfType<ArenaNpcPreparationEffect>()
	...
	protected override (ICell Target, IEnumerable<ICellExit>) GetPath(ICharacter ch)
	{
		...
		var opponents = GetOpponents(arenaEvent, ch)
			.Select(x => x.Character)
			.Where(x => x is not null)
			.Cast<ICharacter>()
			.Where(x => !x.State.IsDead())
