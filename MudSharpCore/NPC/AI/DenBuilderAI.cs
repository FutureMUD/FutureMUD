...
public class DenBuilderAI : PathingAIBase
{
    ...
    protected override void LoadFromXML(XElement root)
    {
        ...
        long craftId = long.Parse(root.Element("DenCraftId")?.Value ?? "0");
    ...
    private void EvaluateDenLifecycle(ICharacter character)
    {
        ...
        IActiveCraftEffect? activeCraftEffect = character.EffectsOfType<IActiveCraftEffect>()
        ...
        {
            (bool canResume, string _) = DenCraft.CanResumeCraft(character, interruptedCraft);
            if (canResume)
            {
        ...
        (bool canDoCraft, string _) = DenCraft.CanDoCraft(character, null, true, true);
        if (canDoCraft)
        {
    ...
    private void TryDefendDen(ICharacter character)
    {
        ...
        ICharacter? target = character.Location.LayerCharacters(character.RoomLayer)
            .Except(character)
