...
public class DoorguardAI : ArtificialIntelligenceBase
{
    ...
    protected virtual void LoadFromXml(XElement root)
    {
        ...
        XElement element = root.Element("Social");
        if (element != null)
        {
    ...
    private bool BuildingCommandSocial(ICharacter actor, StringStack command)
    {
        ...
        ISocial social = actor.Gameworld.Socials.FirstOrDefault(x => x.Name.EqualTo(text));
        if (social is null)
        {
    ...
    protected virtual bool OnWitnessMove(ICharacter doorguard, ICharacter mover, ICellExit exit)
    {
        ...
        if (WillOpenDoorForProg?.ExecuteBool(doorguard, mover, exit) ?? false)
        {
            ...
            double baseDelay = Convert.ToDouble(BaseDelayProg.Execute(doorguard, mover, exit));
        ...
        bool socialTarget, ICellExit socialDirection)
    {
        ...
        double baseDelay = Convert.ToDouble(BaseDelayProg.Execute(doorguard, socialite, exit));
        doorguard.AddEffect(new DoorguardOpenDoor(doorguard,
    ...
    protected virtual bool OnDoorKnock(ICharacter doorguard, ICharacter knocker, ICellExit exit)
    {
        ...
        double baseDelay = Convert.ToDouble(BaseDelayProg.Execute(doorguard, knocker, exit));
        doorguard.AddEffect(
