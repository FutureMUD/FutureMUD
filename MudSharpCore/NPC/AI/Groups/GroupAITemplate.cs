...
public class GroupAITemplate : SaveableItem, IGroupAITemplate
{
    public GroupAITemplate(GroupAiTemplate template, IFuturemud gameworld)
    {
        ...
        _id = template.Id;
        _name = template.Name;
        XElement root = XElement.Parse(template.Definition);
        LoadFromXml(root);
    }
    ...
    private bool BuildingCommandThreat(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = long.TryParse(cmdText, out long value)
        ...
        Func<ICharacter, GroupAlertness, bool> func = CalculateConsidersThreatInvoker(prog);
        if (func == null)
        {
            actor.OutputHandler.Send(
    ...
    {
        string cmdText = command.PopSpeech().ToLowerInvariant();
        switch (cmdText)
        {
            case "add":
        ...
        if (!int.TryParse(cmdText, out int value))
        {
            actor.OutputHandler.Send("You must enter a number corresponding to the emote you want to edit.");
            return false;
        }

        IGroupEmote emote = _groupEmotes.ElementAtOrDefault(value - 1);
        if (emote == null)
        {
    ...
    private bool BuildingCommandAction(ICharacter actor, StringStack command, IGroupEmote emote)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<GroupAction>(out GroupAction action))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandEmoteMaxAlertness(ICharacter actor, StringStack command, IGroupEmote emote)
    {
        ...
        if (!Utilities.TryParseEnum<GroupAlertness>(command.PopSpeech(), out GroupAlertness alertness))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandEmoteAge(ICharacter actor, StringStack command, IGroupEmote emote)
    {
        ...
        if (!Utilities.TryParseEnum<AgeCategory>(cmdText, out AgeCategory value))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandEmoteTarget(ICharacter actor, StringStack command, IGroupEmote emote)
    {
        ...
        if (cmdText.EqualToAny("none", "clear", "remove"))
        {
            Emote test = new(emote.EmoteText, new DummyPerceiver(), new DummyPerceivable());
            if (!test.Valid)
            {
                actor.OutputHandler.Send(
        ...
        if (!Utilities.TryParseEnum<GroupRole>(cmdText, out GroupRole value))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandEmoteGender(ICharacter actor, StringStack command, IGroupEmote emote)
    {
        ...
        if (!Utilities.TryParseEnum<Gender>(cmdText, out Gender value))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandEmoteAdd(ICharacter actor, StringStack command)
    {
        ...
        Emote test = new(command.RemainingArgument, new DummyPerceiver(), new DummyPerceivable());
        if (!test.Valid)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandType(ICharacter actor, StringStack command)
    {
        ...
        string typeName = command.PopSpeech().ToLowerInvariant();
        (IGroupAIType type, string error) = GroupAITypeFactory.LoadFromBuilderArguments(typeName, command.RemainingArgument, Gameworld);
        if (type == null)
        {
        ...
        GroupAIType = type;
        foreach (IGroupAI group in Gameworld.GroupAIs.Where(x => x.Template == this))
        {
    ...
    {
        GroupAiTemplate dbitem = FMDB.Context.GroupAiTemplates.Find(Id);
        dbitem.Name = _name;
        dbitem.Definition = SaveToXml().ToString();
        Changed = false;
    }
