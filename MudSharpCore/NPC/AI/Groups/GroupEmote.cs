...
public class GroupEmote : IGroupEmote
{
    ...
    public GroupEmote(XElement root)
    {
        ...
        int value = int.Parse(root.Element("RequiredGender").Value);
        if (value != -1)
        {
    ...
    {
        List<ICharacter> primaryCandidates =
            herd.GroupRoles.Where(x => AppliesToIndividual(x.Key, x.Value)).Select(x => x.Key).ToList();
        if (RequiredTargetRole.HasValue)
        {
            (ICharacter Primary, ICharacter Secondary) combined = primaryCandidates
        ...
        ICharacter primary = primaryCandidates.GetRandomElement();
