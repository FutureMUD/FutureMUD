...
public abstract class GroupAIType : IGroupAIType, IHaveFuturemud
{
    ...
    protected string GroupActivityTimeDescription
    {
        get
        {
            bool dawn = ActiveTimesOfDay.Contains(TimeOfDay.Dawn);
            bool morning = ActiveTimesOfDay.Contains(TimeOfDay.Morning);
            bool afternoon = ActiveTimesOfDay.Contains(TimeOfDay.Afternoon);
            bool dusk = ActiveTimesOfDay.Contains(TimeOfDay.Dusk);
            bool night = ActiveTimesOfDay.Contains(TimeOfDay.Night);
    ...
    protected virtual void CheckHerdEmotes(IGroupAI herd)
    {
        ...
        IGroupEmote emote = herd.GroupEmotes.Where(x => x.Applies(herd)).GetRandomElement();
        if (emote == null)
        {
            return;
        }
    ...
    public virtual void EvaluateGroupRolesAndMemberships(IGroupAI group)
    {
        ...
        {
            List<ICharacter> outsiders = group.GroupRoles.Where(x => x.Value == GroupRole.Outsider).Select(x => x.Key).ToList();
    ...
    protected virtual void EnsureLeaderExists(IGroupAI group)
    {
        ...
        {
            // Pretenders first
            ICharacter newLeader = group.GroupRoles.Where(x => x.Value == GroupRole.Pretender).Select(x => x.Key)
                                 .GetRandomElement();
    ...
    protected virtual void HandleGeneral(IGroupAI group)
    {
        foreach (ICharacter ch in group.GroupMembers)
        {
            ...
            {
                IPositionState upright = ch.MostUprightMobilePosition();
                if (upright != null)
                {
        ...
        string argument)
    {
        ...
        List<TimeOfDay> activeTimes = new();
        switch (argument)
        {
            case "nocturnal":
