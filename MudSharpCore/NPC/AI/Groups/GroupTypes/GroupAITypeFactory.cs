...
public static class GroupAITypeFactory
{
    ...
    {
        string typename = root.Attribute("typename").Value;
        if (typename == "invalid")
        {
            return null;
        }
    ...
    {
        Type iType = typeof(IGroupAIType);
        foreach (
            Type type in
        ...
        {
            MethodInfo method = type.GetMethod("RegisterGroupAIType", BindingFlags.Static | BindingFlags.Public);
