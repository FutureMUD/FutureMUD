...
public class LairScavengerAI : PathingAIBase
{
    ...
    private void EvaluateScavenging(ICharacter character)
    {
        ...
        IGameItem? targetItem = FindVisibleScavengeItem(character, WillScavengeItemProg, home.AnchorItem);
    ...
    {
        IGameItem? anchorContainer = home.AnchorItem?.GetItemType<IContainer>() is not null &&
                              ReferenceEquals(home.AnchorItem.Location, character.Location)
            ? home.AnchorItem
            : null;

        foreach (IGameItem? item in carriedLoot.ToList())
        {
