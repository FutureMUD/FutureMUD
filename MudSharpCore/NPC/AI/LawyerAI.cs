...
public class LawyerAI : PathingAIBase
{
    ...
    private bool BuildingCommandBankAccountProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg? prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.BankAccount, [ProgVariableTypes.Character]).LookupProg();
        if (prog is null)
        {
            return false;
        }
    ...
    private bool BuildingCommandHomeBaseProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg? prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Location, [ProgVariableTypes.Character]).LookupProg();
        if (prog is null)
        {
            return false;
        }
