...
public abstract class PathingAIBase : ArtificialIntelligenceBase
{
    ...
    protected void Smash(ICharacter ch, ICellExit exit)
    {
        ...
        if (!ch.Race.CombatSettings.CanUseWeapons)
        {
            ...
            if (ch.Combat == null)
            {
                UnarmedSmashItemAttack nmove = new(attack.Attack)
        ...
        List<Tuple<IMeleeWeapon, IEnumerable<IWeaponAttack>>> weaponSmashes = weapons.Select(x => Tuple.Create(x,
        ...
        List<Tuple<Tuple<IMeleeWeapon, IEnumerable<IWeaponAttack>>, IInventoryPlan>> options = new();
