...
public class SemiAggressiveAI : PathingAIWithProgTargetsBase
{
    ...
    protected override void LoadFromXML(XElement root)
    {
        ...
        XElement element = root.Element("WillAttackProg");
        if (element != null)
        {
            WillAttackProg = long.TryParse(element.Value, out long value)
        ...
        if (element != null)
        {
            WillPostureProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
        ...
        if (element != null)
        {
            WillFleeProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
        ...
        if (element != null)
        {
            WillAttackPostureEscalationProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
        ...
        if (element != null)
        {
            PostureEmoteProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
        ...
        if (element != null)
        {
            AttackEmoteProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
        ...
        if (element != null)
        {
            FleeEmoteProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
        ...
        if (element != null)
        {
            FleeLocationsProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
    ...
    {
        string emoteText = AttackEmoteProg.Execute(attacker, target)?.ToString();
        if (!string.IsNullOrEmpty(emoteText))
        {
            Emote emote = new(emoteText, attacker, attacker, target);
            if (emote.Valid)
            {
    ...
    {
        AIPosturingEffect effect = character.EffectsOfType<AIPosturingEffect>().First();
        string emoteText = FleeEmoteProg.Execute(character, effect.PosturingTargets, effect.ThreatLevel)?.ToString();
        if (!string.IsNullOrEmpty(emoteText))
        {
            IPerceivable[] emoteArray = new IPerceivable[] { character }.Concat(effect.PosturingTargets).ToArray();
            int index = 1;
            Emote emote = new(
    ...
    protected virtual void BeginPosturing(ICharacter character, ICharacter target)
    {
        ...
        {
            double newThreat = currentThreat + ThreatPerEscalationTick;
            List<ICharacter> targetList = targets.ToList();
            ...
            string emoteText = PostureEmoteProg.Execute(character, targetList, newThreat)?.ToString();
            if (!string.IsNullOrEmpty(emoteText))
            {
                IPerceivable[] emoteArray = new IPerceivable[] { character }.Concat(targetList).ToArray();
                int index = 1;
                Emote emote = new(string.Format(emoteText, targetList.Select(x => "$" + index++).ListToString()),
        ...
        AIPosturingEffect effect = character.EffectsOfType<AIPosturingEffect>().FirstOrDefault();
        if (effect == null)
        {
            effect = new AIPosturingEffect(character, new[] { target }, OnPostureEffectExpire);
            AIRecentlyFledPosturing fledEffect = character.EffectsOfType<AIRecentlyFledPosturing>().FirstOrDefault();
    ...
    {
        HashSet<ICharacter> targets = new();
        if (character.Combat != null)
        {
            ...
            foreach (ICharacter tch in character.Combat.Combatants.Where(x => x.CombatTarget == character)
                                         .OfType<ICharacter>())
            {
                targets.Add(tch);
            }
        ...
        AIPosturingEffect posturing = character.EffectsOfType<AIPosturingEffect>().FirstOrDefault();
        if (posturing != null)
        {
            foreach (ICharacter target in posturing.PosturingTargets)
            {
                targets.Add(target);
            }
        }
    ...
    private bool TargetIncapacitated(ICharacter aggressor, ICharacter target)
    {
        ...
        foreach (IPerceiver other in aggressor.Combat?.Combatants.Where(x => x.CombatTarget == aggressor)
    ...
    {
        ICharacter[] targets = GetCharacterTargets(aggressor, null);
