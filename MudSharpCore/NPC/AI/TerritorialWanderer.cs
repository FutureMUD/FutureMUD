...
public class TerritorialWanderer : PathingAIBase
{
    ...
    {
        if (command.IsFinished || !TryParseWanderChance(command.SafeRemainingArgument, out double value))
        {
            actor.OutputHandler.Send(
    ...
    protected override void LoadFromXML(XElement root)
    {
        ...
        XElement element = root.Element("SuitableTerritoryProg");
        if (element != null)
        {
            SuitableTerritoryProg = long.TryParse(element.Value, out long value)
        ...
        if (element != null)
        {
            DesiredTerritorySizeProg = long.TryParse(element.Value, out long value)
                ? Gameworld.FutureProgs.Get(value)
                : Gameworld.FutureProgs.GetByName(element.Value);
