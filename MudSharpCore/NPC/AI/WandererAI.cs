...
public class WandererAI : ArtificialIntelligenceBase
{
    ...
    private void LoadFromXml(XElement root)
    {
        ...
        XElement element = root.Element("OpenDoors");
    ...
    {
        ICharacter ch = (type == EventType.EngagedInCombat ?
            arguments[1] :
        ...
        switch (type)
        {
            ...
            case EventType.LeaveCombat:
                ICharacter character = (ICharacter)arguments[0];
    ...
    private bool BuildingCommandSpeed(ICharacter actor, StringStack command)
    {
        ...
        IMoveSpeed move = TargetBodyPrototype.Speeds.GetByIdOrName(command.SafeRemainingArgument);
        if (move is null)
        {
    ...
    {
        List<INPC> npcs = Gameworld.NPCs.OfType<INPC>().Where(x => x.AIs.Contains(this)).ToList();
        List<INPCTemplate> templates = Gameworld.NpcTemplates
        ...
        IBodyPrototype proto = Gameworld.BodyPrototypes.GetByIdOrName(ss.SafeRemainingArgument);
        if (proto is null)
        {
    ...
    {
        string expr = command.SafeRemainingArgument;
