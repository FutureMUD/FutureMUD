...
Note that the #6Multi#0 strategy has additional building commands:
    ...
    private bool BuildingCommandZone(ICharacter actor, StringStack command)
    {
        ...
        IZone? zone = Gameworld.Zones.GetByIdOrName(command.SafeRemainingArgument);
        if (zone is null)
        {
    ...
    {
        if (command.IsFinished || !int.TryParse(command.SafeRemainingArgument, out int amount) || amount < 1)
        {
    ...
    {
        if (command.IsFinished || !int.TryParse(command.SafeRemainingArgument, out int amount) || amount < 0)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandTemplateMulti(ICharacter actor, StringStack command)
    {
        ...
        INPCTemplate? template = Gameworld.NpcTemplates.GetByIdOrName(command.SafeRemainingArgument);
        if (template is null)
        {
        ...
        if (!double.TryParse(command.SafeRemainingArgument, out double weighting) || weighting <= 0.0)
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        {
            case SpawnStrategy.Multi:
                List<(INPCTemplate Template, double Weighting)> templates = _targetTemplates.Select(x => (Template: Gameworld.NpcTemplates.Get(x.TemplateId), x.Weighting)).Where(x => x.Template is not null).ToList();
                double totalWeight = templates.Sum(x => x.Weighting);
                sb.AppendLine($"NPC Templates:");
                foreach ((INPCTemplate? template, double weighting) in templates)
                {
        ...
        foreach (IZone zone in MonitoredZones)
        {
    ...
    private void DoSpawnNPC(INPCTemplate npcTemplate, ICharacterTemplate chTemplate, ICell location)
    {
		...
		newCharacter.MoveTo(
			CharacterInstanceService.CreateDefaultSpawnLocation(location, RoomLayer.GroundLevel),
			noSave: true);
    ...
    private bool TrySpawn(INPCTemplate npcTemplate, ICharacterTemplate chTemplate)
    {
        switch (SpawnStrategy)
        {
            ...
            case SpawnStrategy.OpenTerritory:
                TerritorialWanderer? territorialAI = npcTemplate.ArtificialIntelligences
    ...
    private void CheckSpawnMulti()
    {
        ...
        List<(INPCTemplate Template, double Weighting)> npcs = _targetTemplates.Select(x => (Template: Gameworld.NpcTemplates.Get(x.TemplateId), x.Weighting)).ToList();
        if (!npcs.Any())
        {
            return;
        }

        int count = CountCurrentNPCs();
        ...
        int numberToSpawn = _targetCount - count;
        while (numberToSpawn > 0)
        {
            INPCTemplate npcTemplate = npcs.GetWeightedRandom();
            ICharacterTemplate chTemplate = npcTemplate.GetCharacterTemplate();
    ...
    public void CheckSpawn()
    {
        ...
        INPCTemplate? npcTemplate = TargetTemplate;
        ...
        int count = CountCurrentNPCs(npcTemplate);
        if (count >= _minimumCount)
        {
            return;
        }
