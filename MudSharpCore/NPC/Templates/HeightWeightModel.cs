...
public class HeightWeightModel : SaveableItem, IHeightWeightModel
{
    ...
    public HeightWeightModel(IFuturemud gameworld, string name)
    {
        ...
        using (new FMDB())
        {
            Models.HeightWeightModel dbitem = new()
            {
                Name = Name,
    ...
    {
        double height = MeanHeight + heightStdDev * StandardDeviationHeight;
        double weight = Math.Pow(height, 2) * (MeanBMI + bmiStdDev * StandardDeviationBMI) * BMIMultiplier;
        return (height, weight);
    }
    ...
    {
        Models.HeightWeightModel dbitem = FMDB.Context.HeightWeightModels.Find(Id);
        dbitem.Name = Name;
...
Note - models only use one of either BMI or weight. Setting one switches off the other.";
    ...
    private bool BuildingCommandStdDevHeight(ICharacter actor, StringStack ss)
    {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(ss.SafeRemainingArgument, UnitType.Length, actor, out double value))
        {
    ...
    private bool BuildingCommandMeanWeight(ICharacter actor, StringStack ss)
    {
        ...
        if (!Gameworld.UnitManager.TryGetBaseUnits(ss.SafeRemainingArgument, UnitType.Mass, actor, out double value))
        {
