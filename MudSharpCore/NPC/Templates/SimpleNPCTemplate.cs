...
public class SimpleNPCTemplate : NPCTemplateBase
{
    ...
    {
        return $"{SelectedSdesc} (#{Id.ToString("N0", voyeur)}r{RevisionNumber.ToString("N0", voyeur)})".ColourCharacter();
    }
    ...
    public override string Show(ICharacter actor)
    {
        using (new FMDB())
        {
            ...
            foreach ((ITraitDefinition, double) item in SkillValues)
            {
                sb.AppendLine(
            ...
            if (SelectedMerits.Any())
            {
                ...
                foreach (ICharacterMerit merit in SelectedMerits)
                {
            ...
            if (SelectedKnowledges.Any())
            {
                ...
                foreach (IKnowledge knowledge in SelectedKnowledges)
                {
            ...
            if (MissingBodyparts.Any())
            {
                ...
                foreach (IBodypart part in MissingBodyparts)
                {
    ...
    private void LoadFromXml(XElement root)
    {
        using (new FMDB())
        {
            foreach (XElement item in root.Element("SelectedEntityDescriptionPatterns").Elements("Pattern"))
            {
            ...
            catch (MUDDateException)
            {
                ...
                foreach (ICalendar calendar in Gameworld.Calendars)
                {
                    try
                    {
            ...
            foreach (XElement item in root.Element("SelectedAttributes").Elements("Attribute"))
            {
            ...
            foreach (XElement item in root.Element("SelectedSkills").Elements("Skill"))
            {
                ITraitDefinition trait = Gameworld.Traits.Get(long.Parse(item.Value));
                if (trait == null)
                {
                    continue;
                }
            ...
            foreach (XElement item in root.Element("SelectedCharacteristics").Elements("Characteristic"))
            {
                ICharacteristicDefinition definition = Gameworld.Characteristics.Get(long.Parse(item.Attribute("Definition").Value));
                ICharacteristicValue value = Gameworld.CharacteristicValues.Get(long.Parse(item.Attribute("Value").Value));
            ...
            if (element != null)
            {
                ...
                {
                    IChargenRole role = Gameworld.Roles.Get(long.Parse(item.Value));
                    if (role == null)
                    {
                        continue;
                    }
            ...
            if (element != null)
            {
                foreach (XElement item in element.Elements("Knowledge"))
                {
                    IKnowledge knowledge = Gameworld.Knowledges.Get(long.Parse(item.Value));
                    if (knowledge == null)
                    {
                        continue;
                    }
            ...
            if (element != null)
            {
                foreach (XElement item in element.Elements("Merit"))
                {
                    IMerit merit = Gameworld.Merits.Get(long.Parse(item.Value));
            ...
            if (element != null)
            {
                foreach (XElement item in element.Elements("Bodypart"))
                {
                    IBodypart bodypart = Gameworld.BodypartPrototypes.Get(long.Parse(item.Value));
                    if (bodypart == null)
                    {
                        continue;
                    }
    ...
    private void CheckCharacteristicsCreation()
    {
        ...
        ICharacterTemplate template = GetCharacterTemplate();
        IEthnicity ethnicity = SelectedEthnicity ??
    ...
    private bool BuildingCommandKnowledge(ICharacter actor, StringStack command)
    {
        ...
        IKnowledge knowledge = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandMerit(ICharacter actor, StringStack command)
    {
        ...
        IMerit merit = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    private bool BuildingCommandAttribute(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double dValue))
        {
    ...
    private bool BuildingCommandBirthday(ICharacter actor, StringStack command)
    {
        ...
        try
        {
            ...
            if (command.Peek().Equals("random", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                if (!int.TryParse(command.PopSpeech(), out int age))
                {
    ...
    private bool BuildingCommandRandomise(ICharacter actor, StringStack command)
    {
        ...
        AgeCategory category = RandomiseAgeChances.GetWeightedRandom(x => x.Weight).Age;
        int randomAge = 1;
        switch (category)
        {
        ...
        ICharacterTemplate template = GetCharacterTemplate();
        IHeightWeightModel defaultHWModel = SelectedRace.DefaultHeightWeightModel(SelectedGender);
        if (defaultHWModel is not null)
        {
            (double height, double weight) = defaultHWModel.GetRandomHeightWeight();
        ...
        else
        {
            ...
            HeightPickerScreenStoryboard cghp = (HeightPickerScreenStoryboard)Gameworld.ChargenStoryboard.StageScreenMap[ChargenStage.SelectHeight];
            ...
            WeightPickerScreenStoryboard cgwp =
        ...
        template = GetCharacterTemplate();
        IEnumerable<ICharacteristicDefinition> characteristicChoices = SelectedRace.Characteristics(SelectedGender);
        foreach (KeyValuePair<ICharacteristicDefinition, ICharacteristicProfile> choice in
        ...
        IEntityDescriptionPattern sdesc = Gameworld.EntityDescriptionPatterns
        ...
        IEntityDescriptionPattern fdesc = Gameworld.EntityDescriptionPatterns
    ...
    {
        ICulture culture = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandEthnicity(ICharacter actor, StringStack command)
    {
        ...
        IEthnicity ethnicity = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandPattern(ICharacter actor, StringStack command)
    {
        ...
        {
            pattern = long.TryParse(command.PopSpeech(), out long value) ? patterns.FirstOrDefault(x => x.Id == value) : null;
        }
    ...
    private bool BuildingCommandHeight(ICharacter actor, StringStack command)
    {
        ...
        double value = Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument, UnitType.Length, out bool success);
        if (!success)
        {
    ...
    private bool BuildingCommandName(ICharacter actor, StringStack command)
    {
        ...
        INameCulture nc = SelectedEthnicity?.NameCultureForGender(SelectedGender) ?? SelectedCulture.NameCultureForGender(SelectedGender);
        IPersonalName name = nc.GetPersonalName(command.SafeRemainingArgument);
        if (name == null)
        {
    ...
    {
        IRace race = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandWeight(ICharacter actor, StringStack command)
    {
        ...
        double value = Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument, UnitType.Mass, out bool success);
        if (!success)
        {
    ...
    public List<ITraitDefinition> SelectedSkills => SkillValues.Select(x => x.Item1).ToList();
