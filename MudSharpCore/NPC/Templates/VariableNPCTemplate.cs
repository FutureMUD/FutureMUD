...
public class VariableNPCTemplate : NPCTemplateBase
{
    ...
    public override string ReferenceDescription(IPerceiver voyeur)
    {
        return $"{Name} (#{Id.ToString("N0", voyeur)}r{RevisionNumber.ToString("N0", voyeur)})".ColourCharacter();
    }
    ...
    private void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement sub in element.Elements("NameProfile"))
        {
        ...
        foreach (XElement sub in element.Elements("Attribute"))
        {
        ...
        if (element != null)
        {
            foreach (XElement item in element.Elements("Role"))
            {
        ...
        if (element is not null)
        {
            ...
            foreach (XElement sub in element.Elements("Merit"))
            {
        ...
        if (element is not null)
        {
            foreach (XElement sub in element.Elements("Knowledge"))
            {
                IKnowledge knowledge = Gameworld.Knowledges.Get(long.Parse(sub.Attribute("which").Value));
        ...
        if (element is not null)
        {
            foreach (XElement item in element.Elements("Accent"))
            {
    ...
    public override string WhyCannotSubmit()
    {
        ...
        {
            List<Gender> genders = _genderChances.Where(x => _heightWeightModels.All(y => y.Key != x.Value))
    ...
    protected override ICharacterTemplate CharacterTemplate(ICell location)
    {
        ...
        List<IAccent> accents = new(languages.Count);
        ...
        IEthnicity ethnicity = _ethnicity ??
                        Gameworld.Ethnicities.Where(x => _race.SameRace(x.ParentRace)).GetRandomElement();
        List<IKnowledge> knowledges = _knowledgeChances.Where(x => x.Chance >= RandomUtilities.DoubleRandom(0.0, 1.0)).Select(x => x.Knowledge).ToList();
        List<ICharacterMerit> merits = new();
        List<ICharacterMerit> unselectedMerits = _validMerits.ToList();
        int i = 0;
        while (_numberMerits > i++)
        {
            ICharacterMerit merit = unselectedMerits.Where(x => x.MeritType == MeritType.Merit).GetRandomElement();
            if (merit is null)
            {
                break;
            }
        ...
        {
            ICharacterMerit merit = unselectedMerits.Where(x => x.MeritType == MeritType.Flaw).GetRandomElement();
            if (merit is null)
            {
                break;
            }
        ...
        {
            ICharacterMerit merit = unselectedMerits.GetRandomElement();
            if (merit is null)
            {
                break;
            }
        ...
        IEntityDescriptionPattern fdescTemplate = _fdescPattern ?? Gameworld.EntityDescriptionPatterns.Where(
                                                          x =>
        ...
        IEnumerable<ICharacteristicDefinition> characteristicChoices = template.SelectedRace.Characteristics(template.SelectedGender);
        foreach (KeyValuePair<ICharacteristicDefinition, ICharacteristicProfile> choice in ethnicity.CharacteristicChoices.Where(x => characteristicChoices.Contains(x.Key)))
        {
    ...
    public override string Show(ICharacter actor)
    {
        ...
        foreach ((Gender Value, int Weight) chance in _genderChances)
        {
            sb.AppendLine(
        ...
        foreach (KeyValuePair<Gender, IRandomNameProfile> profile in _nameProfiles)
        {
            sb.AppendLine(
        ...
        foreach (KeyValuePair<Gender, IHeightWeightModel> model in _heightWeightModels)
        {
            sb.AppendLine(
        ...
        sb.AppendLine();
        foreach (VariableSkillTemplate skill in _skillTemplates)
        {
            ILanguage language = Gameworld.Languages.FirstOrDefault(x => skill.Trait == x.LinkedTrait);
            if (language is null)
            {
                continue;
            }

            List<IAccent> accents = _validAccents.Where(x => x.Language == language).ToList();
            if (accents.Count == 0)
            {
        ...
        foreach ((IKnowledge Knowledge, double Chance) knowledge in _knowledgeChances)
        {
        ...
        sb.AppendLine();
        foreach (IArtificialIntelligence ai in ArtificialIntelligences)
        {
    ...
    private bool BuildingCommandAccent(ICharacter actor, StringStack command)
    {
        ...
        string cmd1 = command.PopSpeech();
        string cmd2 = command.SafeRemainingArgument;
        IAccent accent;
        ...
        {
            ILanguage language = Gameworld.Languages.GetByIdOrName(cmd1);
            if (language is null)
            {
    ...
    {
        if (!command.PopSpeech().TryParseEnum<Gender>(out Gender gender))
        {
    ...
    private bool BuildingCommandNameProfile(ICharacter actor, StringStack command)
    {
        ...
        string profileText = command.PopSpeech();
        IEnumerable<IRandomNameProfile> ncs = (_ethnicity?.NameCultures.FallbackIfEmpty(_culture.NameCultures) ?? _culture.NameCultures).SelectMany(x => x.RandomNameProfiles).Distinct();
        IRandomNameProfile profile = ncs.GetByIdOrName(profileText);
        if (profile == null)
        {
    ...
    private bool BuildingCommandRace(ICharacter actor, StringStack command)
    {
        ...
        IRace race = actor.Gameworld.Races.GetByIdOrName(command.SafeRemainingArgument);
        if (race == null)
        {
        ...
        foreach (Gender gender in _race.AllowedGenders)
        {
            switch (gender)
            {
        ...
        foreach (Gender gender in _genderChances.Select(x => x.Value))
        {
            IHeightWeightModel hwModel = race.DefaultHeightWeightModel(gender);
    ...
    private bool BuildingCommandCulture(ICharacter actor, StringStack command)
    {
        ...
        ICulture culture = Gameworld.Cultures.GetByIdOrName(command.SafeRemainingArgument);
        if (culture == null)
        {
    ...
    private bool BuildingCommandEthnicity(ICharacter actor, StringStack command)
    {
        ...
        IEthnicity ethnicity = Gameworld.Ethnicities.GetByIdOrName(command.SafeRemainingArgument);
        if (ethnicity == null)
        {
    ...
    private bool BuildingCommandHeightWeightModel(ICharacter actor, StringStack command)
    {
        ...
        IHeightWeightModel hwmodel = Gameworld.HeightWeightModels.GetByIdOrName(command.SafeRemainingArgument);
        if (hwmodel == null)
        {
    ...
    {
        if (!int.TryParse(command.PopSpeech(), out int minAge))
        {
            actor.OutputHandler.Send("You must enter a minimum age for this NPC.");
            return false;
        }

        if (!int.TryParse(command.PopSpeech(), out int maxAge))
        {
    ...
    private bool BuildingCommandAgeRange(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<AgeCategory>(out AgeCategory minAgeCat))
        {
            actor.OutputHandler.Send(
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<AgeCategory>(out AgeCategory maxAgeCat))
        {
            actor.OutputHandler.Send(
        ...
        int minAge = _race.MinimumAgeForCategory(minAgeCat);
        int maxAge = maxAgeCat == AgeCategory.Venerable
    ...
    {
        if (!int.TryParse(command.PopSpeech(), out int total))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandAttribute(ICharacter actor, StringStack command)
    {
        ...
        List<IAttributeDefinition> attributes =
    ...
    private bool BuildingCommandSdesc(ICharacter actor, StringStack command)
    {
        ...
        IEntityDescriptionPattern pattern = Gameworld.EntityDescriptionPatterns.Get(value);
        if (pattern == null)
        {
    ...
    private bool BuildingCommandSkill(ICharacter actor, StringStack command)
    {
        ...
        string skillText = command.PopSpeech();
    ...
    private bool BuildingCommandClass(ICharacter actor, StringStack command)
    {
        ...
        IChargenRole selectedClass = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandRole(ICharacter actor, StringStack command)
    {
        ...
        IChargenRole selectedRole = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandMerit(ICharacter actor, StringStack command)
    {
        ...
        ICharacterMerit merit = Gameworld.Merits.OfType<ICharacterMerit>().GetByIdOrName(command.SafeRemainingArgument);
        if (merit is null)
        {
    ...
    private bool BuildingCommandKnowledge(ICharacter actor, StringStack command)
    {
        ...
        IKnowledge knowledge = Gameworld.Knowledges.GetByIdOrName(command.PopSpeech());
        if (knowledge is null)
        {
