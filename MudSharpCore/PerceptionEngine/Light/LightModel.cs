...
public class LightModel : ILightModel
{
    ...
    {
        XElement element = definition.Element("SightDifficulties");
        if (element == null)
        {
            throw new NotSupportedException("There was no SightDifficulties element in the LightModel definition.");
        }

        foreach (XElement sub in element.Elements("SightDifficulty"))
        {
    ...
    {
        string stringValue = game.GetStaticConfiguration("LightModel");
