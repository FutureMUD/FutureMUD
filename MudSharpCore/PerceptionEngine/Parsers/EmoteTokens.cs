...
public partial class Emote
{
        ...
        IList<IPerceivable> perceivables = null)
    {
        ...
        int quoteCount = text.Length - text.Replace("\"", "").Length;
        if (quoteCount % 2 == 1)
        {
            text = text.Append("\"");
        }
        ...
        text = SpeechTokenRegex.Replace(text, m =>
        {
            ...
            LanguageToken token = new(Source, m.Groups["speech"].Value, language, accent, permitSpeech);
            _tokens.Add(token);
        ...
        if (!playerMode)
        {
            text = NullPerceivableTokenRegex.Replace(text, m =>
            {
                ...
                int index = Convert.ToInt32(m.Groups["perceivable"].Value);
                ...
                NullPerceivableToken token = new(Source, perceivables, perceivables[index],
                    m.Groups["null"].Value,
            ...
            text = OptionalItselfTokenRegex.Replace(text, m =>
            {
                ...
                int targetIndex = Convert.ToInt32(m.Groups["firsttoken"].Value);
                int otherIndex = Convert.ToInt32(m.Groups["secondtoken"].Value);
            ...
            text = FirstThirdInternalTokenRegex.Replace(text, m =>
            {
                ...
                FirstThirdEmoteToken token = new(perceivables[index], m.Groups[3].Value, m.Groups[4].Value);
                _tokens.Add(token);
                return "{" + targetCounter[0]++ + "}";
            });
            ...
            text = PluralityTokenRegex.Replace(text, m =>
            {
                ...
                PluralityEmoteToken token = new(perceivables[index], m.Groups["first"].Value,
            ...
            text = PronounPersonTokenRegex.Replace(text, m =>
            {
                ...
                PronounNumberToken token = new(perceivables[index], m.Groups["singular"].Value,
 ...
 $"Exception in Emote.ScoutTargets for emote:\n\n{RawText}\n\nTokens:\n\n{(from token in _tokens select (token?.Target?.HowSeen(Source, colour: false, flags: PerceiveIgnoreFlags.IgnoreSelf) ?? "null")).ListToString()}\n\n{e.Message}";
    ...
    private class PossessiveCharacterToken : EmoteToken
    {
        ...
        public override string DisplayThirdPerson(IPerceiver perceiver, PerceiveIgnoreFlags flags)
        {
            ...
            string pch = _target.HowSeen(perceiver, Proper, colour: Coloured, flags: flags).Strip_A_An(StripAAn);
    ...
    private class PossessiveObjectToken : EmoteToken
    {
        ...
        public override string DisplayThirdPerson(IPerceiver perceiver, PerceiveIgnoreFlags flags)
        {
            ...
            string pob = _target.HowSeen(perceiver, Proper, colour: Coloured, flags: flags).Strip_A_An(StripAAn);
    ...
    private class FirstThirdEmoteToken : EmoteToken
    {
        ...
        {
            _target = lookup[0] == '~'
                ? (IPerceivable)(source as ITarget)?.TargetBody(lookup[1..])
                : (source as ITarget)?.TargetItem(lookup[1..]);
            FirstPersonString = firstPersonString;
            ThirdPersonString = thirdPersonString;
        }
