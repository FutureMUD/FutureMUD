...
public partial class AIStoryteller
{
    ...
    private bool BuildingCommandModel(ICharacter actor, StringStack command)
    {
        ...
        AIStorytellerPromptScope scope = AIStorytellerPromptScope.Event;
        if (command.CountRemainingArguments() > 1 &&
            TryParsePromptScope(command.PeekSpeech(), out AIStorytellerPromptScope parsedScope))
        {
            scope = parsedScope;
            command.PopSpeech();
        }
        ...
        string model = command.SafeRemainingArgument;
        switch (scope)
        {
...
#pragma warning disable OPENAI001 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
        if (!TryParseReasoningEffort(command.SafeRemainingArgument, out ResponseReasoningEffortLevel effort))
        {
            actor.OutputHandler.Send(
...
#pragma warning restore OPENAI001 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
    ...
    private bool BuildingCommandSpeechContext(ICharacter actor, StringStack command)
    {
        ...
        switch (command.PopForSwitch())
        {
            ...
            case "events":
            case "lines":
                if (command.IsFinished || !int.TryParse(command.SafeRemainingArgument, out int count) || count < 0)
                {
            ...
            case "maxseparation":
                ...
                if (!MudTimeSpan.TryParse(command.SafeRemainingArgument, actor, out MudTimeSpan parsedWindow))
                {
                    actor.OutputHandler.Send(
                ...
                TimeSpan window = parsedWindow.AsTimeSpan();
    ...
    private bool BuildingCommandStatusProg(ICharacter actor, StringStack command)
    {
        ...
        string which = command.PopForSwitch();
        if (command.IsFinished)
        {
    ...
    {
        bool result = SurveillanceStrategy.BuildingCommand(actor, command);
        if (!result)
        {
            return false;
        }
    ...
    private bool BuildingCommandToolAdd(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = Gameworld.FutureProgs.GetByIdOrName(command.PopSpeech());
        if (prog is null)
        {
            actor.OutputHandler.Send("There is no such prog.");
            return false;
        ...
        bool includeWithEcho = !command.IsFinished && command.PopForSwitch().StartsWith("echo");
        AIStorytellerCustomToolCall tool = new(name, $"Custom tool {name}", prog);
        if (includeWithEcho)
        {
    ...
    private bool BuildingCommandToolRemove(ICharacter actor, StringStack command)
    {
        ...
        string name = command.SafeRemainingArgument;
        AIStorytellerCustomToolCall tool = CustomToolCalls.FirstOrDefault(x => x.Name.EqualTo(name));
        if (tool is not null)
        {
    ...
    private bool BuildingCommandToolDescription(ICharacter actor, StringStack command)
    {
        ...
        AIStorytellerCustomToolCall tool = FindCustomTool(command.PopSpeech());
        if (tool is null)
        {
            actor.OutputHandler.Send("There is no custom tool with that name.");
            return false;
    ...
    private bool BuildingCommandToolParameter(ICharacter actor, StringStack command)
    {
        ...
        string parameter = tool.Prog?.NamedParameters
            .Select(x => x.Item2)
    ...
    {
        string query = command.SafeRemainingArgument;
        List<IAIStorytellerReferenceDocument> docs = Gameworld.AIStorytellerReferenceDocuments
    ...
    {
        if (command.IsFinished || !long.TryParse(command.PopSpeech(), out long situationId))
        {
            actor.OutputHandler.Send("You must specify the numeric id of the situation.");
            return false;
        }

        IAIStorytellerSituation situation = _situations.FirstOrDefault(x => x.Id == situationId);
        ...
        long? oldCharacterScopeId = situation.ScopeCharacterId;
        long? oldRoomScopeId = situation.ScopeRoomId;
        switch (command.PopForSwitch())
        {
            case "none":
            case "universal":
            ...
            case "character":
            case "char":
                if (command.IsFinished || !long.TryParse(command.SafeRemainingArgument, out long characterId))
                {
            ...
            case "cell":
                {
                    ...
                    {
                        if (command.IsFinished || !long.TryParse(command.SafeRemainingArgument, out long roomId))
                        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        {
            foreach (AIStorytellerCustomToolCall item in CustomToolCalls)
            {
                bool valid = item.IsValid; // This call has side effects and so needs to happen before the foreach loop below to guarantee the keys all exist
                sb.AppendLine();
                ...
                foreach (Tuple<ProgVariableTypes, string> par in item.Prog?.NamedParameters ?? [])
                {
            ...
            foreach (AIStorytellerCustomToolCall item in CustomToolCallsEchoOnly)
            {
                bool valid = item.IsValid; // This call has side effects and so needs to happen before the foreach loop below to guarantee the keys all exist
                sb.AppendLine();
                sb.AppendLine($"\tFunction Name: {item.Name.ColourValue()}");
