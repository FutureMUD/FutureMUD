...
public partial class AIStoryteller
{
    ...
    private static bool TryReadId(JsonElement element, out long id)
    {
        ...
        switch (element.ValueKind)
        {
            ...
            case JsonValueKind.Object:
                ...
                {
                    if (element.TryGetProperty(name, out JsonElement property) && TryReadId(property, out id))
                    {
                        return true;
                    }
                }
    ...
    private static bool TryGetOptionalLong(JsonElement arguments, out long value, params string[] propertyNames)
    {
        ...
        {
            if (!arguments.TryGetProperty(name, out JsonElement property))
            {
                continue;
            }
    ...
    {
        if (TryParseAttentionClassifierOutput(response, out interested, out reason, out string parseError))
        {
            return true;
        }
        ...
        out string error)
    {
        ...
        switch (type.LegacyCode)
        {
            case ProgVariableTypeCode.Boolean:
                ...
                if (element.ValueKind == JsonValueKind.String &&
                    bool.TryParse(element.GetString(), out bool boolValue))
                {
            ...
            case ProgVariableTypeCode.Number:
                if (element.ValueKind == JsonValueKind.Number && element.TryGetDecimal(out decimal numberValue))
                {
            ...
            case ProgVariableTypeCode.Gender:
                if (TryResolveGenderArgument(element, out Gender gender, out error))
                {
            ...
            case ProgVariableTypeCode.TimeSpan:
                if (TryResolveTimeSpanArgument(element, out TimeSpan timeSpan, out error))
                {
            ...
            case ProgVariableTypeCode.DateTime:
                if (TryResolveDateTimeArgument(element, out DateTime dateTime, out error))
                {
        ...
        if (!TryReadId(element, out long id))
        {
            convertedValue = null;
    ...
    private static bool TryResolveGenderArgument(JsonElement element, out Gender gender, out string error)
    {
        ...
        {
            if (TryGetOptionalLong(element, out long numericGender, "GenderId", "Gender", "Value", "Id", "ID", "id") &&
    ...
    private static bool TryResolveDateTimeArgument(JsonElement element, out DateTime dateTime, out string error)
    {
        if (element.ValueKind == JsonValueKind.Object)
        {
            if (TryGetOptionalLong(element, out long unixSeconds, "UnixSeconds", "EpochSeconds", "Timestamp"))
            {
    ...
    private static bool TryResolveTimeSpanArgument(JsonElement element, out TimeSpan timeSpan, out string error)
    {
        if (element.ValueKind == JsonValueKind.Object)
        {
            if (TryGetOptionalLong(element, out long totalSeconds, "TotalSeconds", "Seconds"))
            {
        ...
        if (element.ValueKind == JsonValueKind.Number && element.TryGetDouble(out double seconds))
        {
    ...
    {
        item = null;
        List<T> searchList = searchItems?.ToList() ?? [];
        if (element.ValueKind == JsonValueKind.Object)
        {
            string[] allIdNames = idPropertyNames.Concat(["Id", "ID", "id"]).Distinct().ToArray();
            if (TryGetOptionalLong(element, out long idValue, allIdNames))
            {
        ...
        if (element.ValueKind == JsonValueKind.String)
        {
            ...
            List<T> matches = FindFrameworkItemMatchesByName(searchList, name);
            if (matches.Count == 1)
            {
                item = matches[0];
                error = string.Empty;
    ...
    {
        clan = null;
        if (TryGetOptionalLong(element, out long clanId, "ClanId", "ClanID", "Clan"))
        {
    ...
    private bool TryResolveClanPaygradeArgument(JsonElement element, out IPaygrade? paygrade, out string error)
    {
        ...
        List<IPaygrade> candidateList = candidates.ToList();
        ...
        List<IPaygrade> matches = candidateList
    ...
    private bool TryResolveEffectArgument(JsonElement element, out IEffect? effect, out string error)
    {
        ...
        if (element.ValueKind == JsonValueKind.Object)
        {
            ...
            if (TryGetOptionalLong(element, out long parsedOwnerId, "OwnerId", "ownerId", "OwnerCharacterId",
                    "CharacterId"))
            {
        ...
        List<IEffect> matches = EnumerateEffectOwners()
    ...
    private bool TryResolveOutfitArgument(JsonElement element, out IOutfit? outfit, out string error)
    {
        ...
        if (!TryGetOptionalLong(element, out long ownerId, "OwnerCharacterId", "CharacterId", "OwnerId"))
        {
    ...
    private bool TryResolveOutfitItemArgument(JsonElement element, out IOutfitItem? outfitItem, out string error)
    {
        ...
        if (!TryGetOptionalLong(element, out long itemId, "ItemId", "Id", "ID", "id"))
        {
        ...
        IEnumerable<IOutfit> outfitCandidates = string.IsNullOrWhiteSpace(outfitName)
            ? owner.Outfits
        ...
        List<IOutfitItem> matches = outfitCandidates
