...
public class AIStorytellerCustomToolCall
{
    public bool IsValid
    {
        get
        {
            ...
            List<string> parameterNames = Prog.NamedParameters.Select(x => x.Item2).ToList();
    ...
    public AIStorytellerCustomToolCall(XElement root, IFuturemud gameworld)
    {
        ...
        foreach (XElement description in root.Element("ParameterDescriptions")?.Elements("Description") ??
                                     Enumerable.Empty<XElement>())
        {
            string name = description.Attribute("name")?.Value;
