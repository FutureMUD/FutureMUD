...
public class AIStorytellerReferenceDocument : SaveableItem, IAIStorytellerReferenceDocument
{
        ...
        string documentType, string keywords, string documentContents)
    {
        ...
        using (new FMDB())
        {
            Models.AIStorytellerReferenceDocument dbitem = new()
            {
                Name = name,
    ...
    {
        Models.AIStorytellerReferenceDocument? dbitem = FMDB.Context.AIStorytellerReferenceDocuments.Find(Id);
        if (dbitem is null)
        {
            return;
        }
    ...
    public bool ReturnForSearch(string searchterm)
    {
        ...
        string[] terms = searchterm.Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    ...
    private static HashSet<long> ParseRestrictedStorytellerIds(string? text)
    {
        ...
        if (trimmed.StartsWith("<"))
        {
            try
            {
                XElement xml = XElement.Parse(trimmed);
                foreach (XElement element in xml.Descendants("Storyteller"))
                {
                    if (long.TryParse(element.Value, out long id))
                    {
                        result.Add(id);
                    }
                }
        ...
        foreach (string item in trimmed.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
        {
    ...
    private bool BuildingCommandStoryteller(ICharacter actor, StringStack command)
    {
        ...
        IAIStoryteller? storyteller = actor.Gameworld.AIStorytellers.GetByIdOrName(command.SafeRemainingArgument);
