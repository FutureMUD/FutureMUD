...
public class BranchCheck : FrameworkItem, ICheck
{
        ...
        params (string Parameter, object value)[] customParameters)
    {
        ...
        IncreasedBranchChance effect = checkee.EffectsOfType<IncreasedBranchChance>().FirstOrDefault();
        if (effect == null)
        {
    ...
    public CheckOutcome Check(IPerceivableHaveTraits checkee, Difficulty difficulty, ITraitDefinition trait,
        ...
        params (string Parameter, object value)[] customParameters)
    {
        double targetNumber = TargetNumber(checkee, difficulty, trait, target, customParameters: customParameters);
        Outcome outcome = RollAgainst(targetNumber, out List<double> rolls);
