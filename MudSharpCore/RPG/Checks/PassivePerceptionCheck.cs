...
public class PassivePerceptionCheck : TimeboundCheck
{
    ...
    {
        XElement root = XElement.Parse(check.CheckTemplate.Definition);
        XElement element = root.Element("CoreTraitExpression");
        if (element == null)
        {
        ...
        params (string Parameter, object value)[] customParameters)
    {
        ...
        ICharacter targetAsCharacter = target as ICharacter ?? (checkee as IBody)?.Actor;
        IGameItem targetAsItem = target as IGameItem;

        double core = CoreTraitExpression.EvaluateWith(checkee, values: customParameters);
        double opposed = (targetAsCharacter != null
