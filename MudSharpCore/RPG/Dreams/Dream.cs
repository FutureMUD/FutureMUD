...
public class Dream : SaveableItem, IDream
{
    ...
    public IDream Clone(string newName)
    {
        using (new FMDB())
        {
            ...
            foreach (DreamStage stage in DreamStages)
            {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Dream dbitem = FMDB.Context.Dreams.Find(Id);
            dbitem.Name = Name;
            ...
            foreach (DreamStage phase in DreamStages)
            {
                DreamPhase dbphase = new();
            ...
            foreach (long character in _characters)
            {
            ...
            foreach (long character in _haveDreamedBefore)
            {
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach (DreamStage stage in _dreamStages)
        {
            sb.AppendLine();
            sb.AppendLine(
    ...
    {
        string arg = command.PopForSwitch();
        switch (arg)
        {
            case "add":
            case "new":
	...
	#3text <##>#0 - drops you into an editor to edit a stage".SubstituteANSIColour());
        ...
        if (!int.TryParse(command.PopSpeech(), out int value) || value < 1 || value > _dreamStages.Count)
        {
    ...
    private bool BuildingCommandDelay(ICharacter actor, int value, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int seconds) || value < 1 || value > 120)
        {
            actor.OutputHandler.Send(
    ...
    {
        List<(int LinkNum, int Options)> existingOptions = _dreamStages
        ...
        if (existingOptions.Any())
        {
            ...
            foreach ((int LinkNum, int Options) option in existingOptions.OrderBy(x => x.LinkNum))
            {
                sb.AppendLine(
    ...
    {
        DreamStage stage = (DreamStage)args[0];
    ...
    private bool BuildingCommandSwap(ICharacter actor, int value1, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value2) || value2 < 1 || value2 > _dreamStages.Count)
        {
            actor.OutputHandler.Send(
    ...
    {
        DreamStage stage = _dreamStages[value - 1];
        actor.OutputHandler.Send(
