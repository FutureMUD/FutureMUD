...
public class Knowledge : SaveableItem, IKnowledge
{
    ...
    public Knowledge(MudSharp.Models.Knowledge knowledge, IFuturemud gameworld)
    {
        ...
        _canLearnProg = knowledge.CanLearnProgId.HasValue
            ...
            : null;
        foreach (KnowledgesCosts cost in knowledge.KnowledgesCosts)
        {
    ...
    {
        Models.Knowledge dbitem = FMDB.Context.Knowledges.Find(Id);
        dbitem.Name = Name;
        dbitem.Description = Description;
        dbitem.LongDescription = LongDescription;
        dbitem.Type = KnowledgeType;
        ...
        foreach (KeyValuePair<IChargenResource, int> cost in _resourceCosts)
        {
    ...
    {
        ProgVariableTypes returnVar = ProgVariableTypes.Error;
        switch (property.ToLowerInvariant())
        {
            case "id":
    ...
    private bool BuildingCommandLearnable(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<LearnableType>(out LearnableType value))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandResource(ICharacter actor, StringStack command)
    {
        ...
        IChargenResource resource = long.TryParse(command.PopSpeech(), out long value)
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int cost) || cost < 0)
        {
