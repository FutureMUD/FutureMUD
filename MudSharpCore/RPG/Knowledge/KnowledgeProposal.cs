...
public class KnowledgeLessonProposal : LessonProposal, IProposal
{
    ...
    public override void Accept(string message = "")
    {
        ...
        CheckOutcome teachoutcome = Teacher.Gameworld.GetCheck(CheckType.KnowledgeTeachCheck)
                                  .Check(Teacher, teachDifficulty, Student);
        // Skip the check on impossible difficulty to avoid skill boosting behaviour
        Outcome learnoutcome = learnDifficulty != Difficulty.Impossible
        ...
        ICharacterKnowledge teacherKnowledge = Teacher.CharacterKnowledges.FirstOrDefault(x => x.Knowledge == Knowledge);
        ...
        {
            IncreasedBranchChance effect = Student.EffectsOfType<IncreasedBranchChance>().FirstOrDefault();
            if (effect == null)
            {
