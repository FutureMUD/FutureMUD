...
public class Crime : LateInitialisingItem, ICrime
{
    ...
    public Crime(MudSharp.Models.Crime dbitem, ILaw law, IFuturemud gameworld)
    {
        ...
        {
            foreach (string item in dbitem.CriminalCharacteristics.Split('\n'))
            {
                string[] split = item.Split(' ');
                ICharacteristicDefinition? definition = Gameworld.Characteristics.Get(long.Parse(split[0]));
                ICharacteristicValue? value = Gameworld.CharacteristicValues.Get(long.Parse(split[1]));
    ...
    public string ShowCrimeInfo(ICharacter enforcer)
    {
        ...
        else
        {
            ...
            sb.AppendLine();
            foreach (KeyValuePair<ICharacteristicDefinition, ICharacteristicValue> item in _criminalCharacteristics)
            {
        ...
        if (enforcer.IsAdministrator())
        {
            ...
            sb.AppendLine();
            foreach (long id in _witnessIds)
            {
                ICharacter? witness = Gameworld.TryGetCharacter(id, true);
                if (witness is null)
                {
                    continue;
                }
    ...
    public void Forgive(ICharacter forgiver, string reason)
    {
        ...
        foreach (IPatrol? patrol in LegalAuthority.Patrols)
        {
