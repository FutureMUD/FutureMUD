...
public partial class LegalAuthority
{
    ...
    private bool BuildingCommandDiscordChannel(ICharacter actor, StringStack command)
    {
        ...
        if (!ulong.TryParse(command.SafeRemainingArgument, out ulong discordChannelId))
        {
    ...
    private bool BuildingCommandAutoConvictTime(ICharacter actor, StringStack command)
    {
        ...
        if (!TimeSpanParserUtil.TimeSpanParser.TryParse(command.SafeRemainingArgument, TimeSpanParserUtil.Units.Hours,
                TimeSpanParserUtil.Units.Days, out TimeSpan ts))
        {
    ...
    private bool BuildingCommandPatrol(ICharacter actor, StringStack command)
    {
        ...
        if (command.PeekSpeech().EqualToAny("add", "new", "create"))
        {
            ...
            PatrolRoute newRoute = new(this, name);
        ...
        if (command.PopSpeech().EqualToAny("delete", "del", "remove", "rem"))
        {
            ...
            IPatrolRoute route = long.TryParse(command.PopSpeech(), out long value)
        ...
        IPatrolRoute which = long.TryParse(command.Last, out long id)
    ...
    private bool BuildingCommandLaw(ICharacter actor, StringStack command)
    {
        ...
        if (cmd.EqualToAny("new", "create", "add"))
        {
            ...
            if (!command.SafeRemainingArgument.TryParseEnum<CrimeTypes>(out CrimeTypes crime))
            {
                actor.OutputHandler.Send(
            ...
            Law newLaw = new(newName, this, crime);
            AddLaw(newLaw);
            actor.OutputHandler.Send(
        ...
        if (cmd.EqualToAny("delete", "del", "remove", "remove"))
        {
            ...
            ILaw law = long.TryParse(command.PopSpeech(), out long lvalue)
        ...
        ILaw editingLaw = long.TryParse(cmd, out long value)
    ...
    private bool BuildingCommandEnforcement(ICharacter actor, StringStack command)
    {
        ...
        if (cmd.EqualToAny("new", "create", "add"))
        {
            ...
            string newName = command.SafeRemainingArgument.TitleCase();
            ...
            EnforcementAuthority newAuthority = new(newName, this);
        ...
        if (cmd.EqualToAny("delete", "del", "remove", "remove"))
        {
            ...
            IEnforcementAuthority enforcement = long.TryParse(command.PopSpeech(), out long lvalue)
        ...
        IEnforcementAuthority editingenforcement = long.TryParse(cmd, out long value)
    ...
    private bool BuildingCommandClass(ICharacter actor, StringStack command)
    {
        ...
        if (cmd.EqualToAny("new", "create", "add"))
        {
            ...
            LegalClass newClass = new(newName, this, prog);
        ...
        if (cmd.EqualToAny("delete", "del", "remove", "remove"))
        {
            ...
            ILegalClass legalClass = long.TryParse(command.PopSpeech(), out long lvalue)
            ...
            actor.AddEffect(new Accept(actor, new GenericProposal
            {
                AcceptAction = text =>
                {
                    ...
                    legalClass.Delete();
                    foreach (ILaw law in _laws.ToList())
                    {
                        law.RemoveAllReferencesTo(legalClass);
                    }

                    foreach (IEnforcementAuthority enforcement in _enforcementAuthorities)
                    {
        ...
        ILegalClass legal = long.TryParse(cmd, out long value)
    ...
    private bool BuildingCommandZone(ICharacter actor, StringStack command)
    {
        ...
        IZone zone = long.TryParse(command.PopSpeech(), out long value)
    ...
    public string Show(ICharacter actor)
    {
        ...
        {
            foreach (ILegalClass legal in _legalClasses)
            {
        ...
        {
            foreach (IEnforcementAuthority authority in _enforcementAuthorities)
            {
        ...
        {
            foreach (IPatrolRoute route in _patrolRoutes)
            {
                sb.AppendLine(
