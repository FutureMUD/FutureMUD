...
public class PatrolRoute : SaveableItem, IPatrolRoute, IEditableItem
{
    public PatrolRoute(ILegalAuthority authority, string name)
    {
        ...
        using (new FMDB())
        {
            ...
            foreach (TimeOfDay tod in _timeOfDays)
            {
    ...
    public override void Save()
    {
        ...
        foreach (KeyValuePair<IEnforcementAuthority, int> number in PatrollerNumbers)
        {
    ...
    private bool BuildingCommandNumbers(ICharacter actor, StringStack command)
    {
        ...
        IEnforcementAuthority enforcement = long.TryParse(command.PopSpeech(), out long value)
        ...
        if (command.IsFinished || !int.TryParse(command.SafeRemainingArgument, out int ivalue))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandNodeInsert(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int value) || value < 1 || value > _patrolNodes.Count)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandNodeDelete(ICharacter actor, StringStack command)
    {
        ...
        if (command.IsFinished || !long.TryParse(command.SafeRemainingArgument, out long value))
        {
    ...
    private bool BuildingCommandStrategy(ICharacter actor, StringStack command)
    {
        ...
        try
        {
            IPatrolStrategy strategy = PatrolStrategyFactory.GetStrategy(command.SafeRemainingArgument, actor.Gameworld);
    ...
    private bool BuildingCommandLinger(ICharacter actor, StringStack command)
    {
        ...
        string type = command.PopSpeech().ToLowerInvariant();
        ...
        if (!MudTimeSpan.TryParse(command.SafeRemainingArgument, out MudTimeSpan tspan))
        {
    ...
    private bool BuildingCommandTime(ICharacter actor, StringStack command)
    {
        ...
        List<TimeOfDay> values = new();
        while (!command.IsFinished)
        {
            string text = command.PopSpeech();
            if (!text.TryParseEnum<TimeOfDay>(out TimeOfDay value))
            {
                actor.OutputHandler.Send(
