...
public class SheriffPatrolStrategy : PatrolStrategyBase
{
    ...
    protected void CheckJusticeSystem(IPatrol patrol)
    {
        ...
        MudDateTime now = authority.EnforcementZones.FirstOrDefault()?.DateTime() ??
                  Gameworld.Calendars.First().CurrentDateTime;

        ICharacter trialCandidate = null;
        // Are there any criminals awaiting sentencing?
        foreach (ICharacter criminal in authority.KnownCrimes.Where(x => x.EligableForAutomaticConviction())
                                          .Select(x => x.Criminal).Distinct())
        {
            AwaitingSentencing awaitingEffect = criminal.EffectsOfType<AwaitingSentencing>(x => x.LegalAuthority == authority)
                                         .FirstOrDefault();
