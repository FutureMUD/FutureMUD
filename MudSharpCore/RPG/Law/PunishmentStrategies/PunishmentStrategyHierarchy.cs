...
public class PunishmentStrategyHierarchy : PunishmentStrategyBase
{
    ...
    public PunishmentStrategyHierarchy(IFuturemud gameworld, XElement root, ILegalAuthority authority) : base(gameworld,
        root)
    {
        foreach (XElement item in root.Elements("Member"))
        {
    ...
    public override bool BuildingCommand(ICharacter actor, ILegalAuthority authority, StringStack command)
    {
        switch (command.PopForSwitch())
        {
            ...
            default:
                if (int.TryParse(command.Last, out int index) && index > 0 && index < _strategyHierarchy.Count)
                {
    ...
    private bool BuildingCommandProg(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value) || value <= 0 || value > _strategyHierarchy.Count)
        {
    ...
    private bool BuildingCommandSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int index1) || index1 <= 0 || index1 > _strategyHierarchy.Count)
        {
            actor.OutputHandler.Send(
                $"You must enter a number between {1.ToString("N0", actor).ColourValue()} and {_strategyHierarchy.Count.ToString("N0", actor).ColourValue()}");
            return false;
        ...
        if (!int.TryParse(command.PopSpeech(), out int index2) || index2 <= 0 || index2 > _strategyHierarchy.Count)
        {
            actor.OutputHandler.Send(
                $"You must enter a number between {1.ToString("N0", actor).ColourValue()} and {_strategyHierarchy.Count.ToString("N0", actor).ColourValue()}");
            return false;
    ...
    private bool BuildingCommandDelete(ICharacter actor, StringStack command)
    {
        ...
        (IFutureProg Prog, IPunishmentStrategy Strategy) punishment = _strategyHierarchy[value - 1];
    ...
    private bool BuildingCommandCreate(ICharacter actor, ILegalAuthority authority, StringStack command)
    {
        ...
        IPunishmentStrategy strategy = PunishmentStrategyFactory.GetStrategyFromBuilderInput(Gameworld, authority, command.PopSpeech());
        if (strategy is null)
        {
    ...
    public override string Describe(IPerceiver voyeur)
    {
        ...
        bool includeNothing = _strategyHierarchy.Any(x => x.Prog.StaticType == FutureProgStaticType.FullyStatic && x.Prog.ExecuteBool(null));
        ...
        List<string> strategies = _strategyHierarchy.Select(x => x.Strategy.Describe(voyeur)).ToList();
        if (includeNothing)
        {
    ...
    {
        foreach ((IFutureProg Prog, IPunishmentStrategy Strategy) strategy in _strategyHierarchy)
        {
    ...
    protected override void SaveSpecificType(XElement root)
    {
        ...
        foreach ((IFutureProg Prog, IPunishmentStrategy Strategy) item in _strategyHierarchy)
        {
