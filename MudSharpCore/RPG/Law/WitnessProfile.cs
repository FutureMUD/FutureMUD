...
public class WitnessProfile : SaveableItem, IWitnessProfile
{
    public WitnessProfile(MudSharp.Models.WitnessProfile profile, IFuturemud gameworld)
    {
        ...
        foreach (Models.WitnessProfilesIgnoredCriminalClasses legal in profile.WitnessProfilesIgnoredCriminalClasses)
        {
        ...
        foreach (Models.WitnessProfilesIgnoredVictimClasses legal in profile.WitnessProfilesIgnoredVictimClasses)
        {
        ...
        foreach (Models.WitnessProfilesCooperatingAuthorities authority in profile.WitnessProfilesCooperatingAuthorities)
        {
    ...
    public WitnessProfile(IFuturemud gameworld, string name)
    {
        ...
        using (new FMDB())
        {
            Models.WitnessProfile profile = new();
    ...
    {
        double streetwise = StreetwiseSkillExpression.Evaluate(enquirer);
        ...
        {
            TimeOfDay leastTime = BaseReportingChance.FirstMin(x => x.Value).Key;
            TimeOfDay mostTime = BaseReportingChance.FirstMax(x => x.Value).Key;
            double average = BaseReportingChance.Average(x => x.Value);
    ...
    private bool BuildingCommandSkill(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double bias))
        {
        ...
        if (!double.TryParse(command.PopSpeech(), out double levels))
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandVictim(ICharacter actor, StringStack command)
    {
        ...
        ILegalClass victimClass = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandCriminal(ICharacter actor, StringStack command)
    {
        ...
        ILegalClass criminalClass = long.TryParse(command.PopSpeech(), out long value)
            ? Gameworld.LegalClasses.Get(value)
            : Gameworld.LegalClasses.GetByName(command.Last);
    ...
    private bool BuildingCommandCooperate(ICharacter actor, StringStack command)
    {
        ...
        ILegalAuthority authority = long.TryParse(command.PopSpeech(), out long value)
    ...
    private bool BuildingCommandChance(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<TimeOfDay>(out TimeOfDay time))
        {
            actor.OutputHandler.Send(
        ...
        if (!command.PopSpeech().TryParsePercentage(out double value) || value < 0.0)
        {
    ...
    private bool BuildingCommandReliability(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParsePercentage(out double value))
        {
    ...
    public string Show(ICharacter actor)
    {
        ...
        foreach (KeyValuePair<TimeOfDay, double> value in BaseReportingChance)
        {
    ...
    {
        Models.WitnessProfile dbitem = FMDB.Context.WitnessProfiles.Find(Id);
        dbitem.Name = Name;
        ...
        foreach (ILegalAuthority item in CooperatingAuthorities)
        {
        ...
        foreach (ILegalClass item in IgnoredCriminalLegalClasses)
        {
        ...
        foreach (ILegalClass item in IgnoredVictimLegalClasses)
        {
