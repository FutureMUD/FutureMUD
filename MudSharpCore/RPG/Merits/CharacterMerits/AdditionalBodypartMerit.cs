...
public class AdditionalBodypartMerit : CharacterMeritBase, IAdditionalBodypartsMerit
{
    public AdditionalBodypartMerit(Models.Merit merit, IFuturemud gameworld) : base(merit, gameworld)
    {
        ...
        foreach (XElement element in definition.Element("AddedBodyparts")!.Elements())
        {
            ...
            {
                if (!Utilities.TryParseEnum<Gender>(genderValue, out Gender parsedGender))
                {
            ...
            {
                race = long.TryParse(raceValue, out long value)
    ...
    private bool BuildingCommandPart(ICharacter actor, StringStack command, bool add)
    {
        ...
        IBodypart? part = Gameworld.BodyPrototypes.SelectMany(x => x.AllBodypartsBonesAndOrgans).GetByIdOrName(command.PopSpeech());
        if (part is null)
        {
