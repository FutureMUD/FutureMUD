...
public abstract class CharacterMeritBase : MeritBase, ICharacterMerit
{
    ...
    protected CharacterMeritBase(Merit merit, IFuturemud gameworld) : base(merit, gameworld)
    {
        ...
        foreach (MeritsChargenResources resource in merit.MeritsChargenResources)
        {
    ...
    {
        XElement element = root.Element("ChargenAvailableProg");
        if (element != null)
        {
    ...
    private bool BuildingCommandCost(ICharacter actor, StringStack command)
    {
        ...
        ChargenResourceCost cost = _costs.FirstOrDefault(x => x.Resource == resource);
...
Enter your blurb that will be visible in chargen in the editor below:");
    ...
    private bool BuildingCommandAppliesProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Boolean, AppliesProgValidTypes).LookupProg();
        if (prog is null)
        {
            return false;
        }
    ...
    private bool BuildingCommandChargenProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(actor, command.SafeRemainingArgument, ProgVariableTypes.Boolean, [ProgVariableTypes.Chargen]).LookupProg();
        if (prog is null)
        {
            return false;
        }
