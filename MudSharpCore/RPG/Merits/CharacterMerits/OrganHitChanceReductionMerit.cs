...
public class OrganHitChanceReductionMerit : CharacterMeritBase, IOrganHitReductionMerit
{
    ...
    {
        XElement root = XElement.Parse(merit.Definition);
        ...
        foreach (XElement item in root.Element("Organs")?.Elements("Organ") ?? Enumerable.Empty<XElement>())
        {
    ...
    private bool BuildingCommandOrgan(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<BodypartTypeEnum>(out BodypartTypeEnum value) || !value.IsOrgan())
        {
