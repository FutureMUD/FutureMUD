...
public class SkillLearningMerit : CharacterMeritBase, ITraitLearningMerit
{
    ...
    protected SkillLearningMerit(Merit merit, IFuturemud gameworld) : base(merit, gameworld)
    {
        ...
        XElement element = definition.Element("Groups");
        if (element != null)
        {
            foreach (XElement item in element.Elements("Group"))
            {
    ...
    {
        List<string> groups = Gameworld.Traits.OfType<ISkillDefinition>().Select(x => x.Group.TitleCase()).Distinct().ToList();
        ...
        string group = groups.FirstOrDefault(x => x.EqualTo(command.SafeRemainingArgument))?.ToLowerInvariant();
        if (group is null)
        {
