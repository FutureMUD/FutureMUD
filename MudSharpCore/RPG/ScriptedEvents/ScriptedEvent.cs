...
internal class ScriptedEvent : SaveableItem, IScriptedEvent
{
    ...
    private Models.ScriptedEvent CloneEventForAutoAssign(ICharacter? character)
    {
        using (new FMDB())
        {
            ...
            foreach (IScriptedEventMultipleChoiceQuestion question in MultipleChoiceQuestions)
            {
                Models.ScriptedEventMultipleChoiceQuestion dbquestion = new()
                {
                ...
                foreach (IScriptedEventMultipleChoiceQuestionAnswer answer in question.Answers)
                {
            ...
            foreach (IScriptedEventFreeTextQuestion question in FreeTextQuestions)
            {
    ...
    protected ScriptedEvent(ScriptedEvent rhs, ICharacter? character)
    {
        ...
        using (new FMDB())
        {
            ...
            foreach (IScriptedEventMultipleChoiceQuestion question in rhs.MultipleChoiceQuestions)
            {
                Models.ScriptedEventMultipleChoiceQuestion dbquestion = new()
                {
                    ScriptedEvent = dbitem,
            ...
            foreach (IScriptedEventFreeTextQuestion question in rhs.FreeTextQuestions)
            {
                dbitem.FreeTextQuestions.Add(new Models.ScriptedEventFreeTextQuestion { ScriptedEvent = dbitem, Question = question.Question });
            }
            ...
            _id = dbitem.Id;
            foreach (Models.ScriptedEventMultipleChoiceQuestion? dbquestion in dbitem.MultipleChoiceQuestions)
            {
                _multipleChoiceQuestions.Add(new ScriptedEventMultipleChoiceQuestion(dbquestion, Gameworld));
            }
            foreach (Models.ScriptedEventFreeTextQuestion? dbquestion in dbitem.FreeTextQuestions)
            {
    ...
    public ScriptedEvent(Models.ScriptedEvent dbitem, IFuturemud gameworld)
    {
        ...
        foreach (Models.ScriptedEventFreeTextQuestion? question in dbitem.FreeTextQuestions)
        {
            _freeTextQuestions.Add(new ScriptedEventFreeTextQuestion(question, Gameworld));
        }

        foreach (Models.ScriptedEventMultipleChoiceQuestion? question in dbitem.MultipleChoiceQuestions)
        {
    ...
    public IScriptedEvent Assign(ICharacter actor)
    {
        ...
        ScriptedEvent item = new(this, actor);
        Gameworld.Add(item);
        return item;
    }
    ...
    public void SetFinished()
    {
        ...
        using (new FMDB())
        {
            ...
            int i = 1;
            foreach (IScriptedEventMultipleChoiceQuestion multi in _multipleChoiceQuestions)
            {
                sb.AppendLine();
            ...
            foreach (IScriptedEventFreeTextQuestion text in _freeTextQuestions)
            {
                sb.AppendLine();
                sb.AppendLine($"Question #{i++.ToString("N0", Character)}".GetLineWithTitle(Character.InnerLineFormatLength, Character.Account.UseUnicode, Telnet.Cyan, Telnet.BoldWhite));
                sb.AppendLine();
    ...
    {
        ScriptedEvent item = new(this, null);
        Gameworld.Add(item);
        return item;
    }
    ...
    public void Delete()
    {
        ...
        if (_id != 0)
        {
            using (new FMDB())
            {
                ...
                Models.ScriptedEvent? dbitem = FMDB.Context.ScriptedEvents.Find(Id);
                if (dbitem != null)
                {
    ...
    private bool BuildingCommandMulti(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int index) || index < 1 || index > _multipleChoiceQuestions.Count)
        {
    ...
    private bool BuildingCommandText(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int index) || index < 1 || index > _freeTextQuestions.Count)
        {
    ...
    private bool BuildingCommandRemoveMultiQuestion(ICharacter actor, StringStack command)
    {
        ...
        IScriptedEventMultipleChoiceQuestion question = _multipleChoiceQuestions.ElementAt(index - 1);
    ...
    {
        ScriptedEventFreeTextQuestion newQuestion = new(this, text);
    ...
    {
        ScriptedEventMultipleChoiceQuestion newQuestion = new(this, text);
    ...
    private bool BuildingCommandRemoveFreeTextQuestion(ICharacter actor, StringStack command)
    {
        ...
        IScriptedEventFreeTextQuestion question = _freeTextQuestions.ElementAt(index - 1);
    ...
    private bool BuildingCommandAutoAssign(ICharacter actor)
    {
        ...
        HashSet<long> onlinePCIDs = actor.Gameworld.Characters.Select(x => x.Id).ToHashSet();
        using (new FMDB())
        {
        ...
        List<ICharacter> targets = actor.Gameworld.Characters.Where(x => CharacterFilterProg.Execute<bool?>(x) == true).ToList();
        if (!targets.Any())
        {
        ...
        actor.AddEffect(new Accept(actor, new GenericProposal
        {
            AcceptAction = text =>
            {
                using (new FMDB())
                {
                    List<Models.ScriptedEvent> events = new();
                    foreach (ICharacter ch in targets)
                    {
                        events.Add(CloneEventForAutoAssign(ch));
                    }
                    FMDB.Context.SaveChanges();
                    foreach (Models.ScriptedEvent newEvent in events)
                    {
    ...
    private bool BuildingCommandFilterProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg? prog = new ProgLookupFromBuilderInput(Gameworld, actor, command.SafeRemainingArgument, ProgVariableTypes.Boolean, new List<ProgVariableTypes> { ProgVariableTypes.Character }).LookupProg();
        if (prog is null)
        {
            return false;
        }
    ...
    private bool BuildingCommandEarliestDate(ICharacter actor, StringStack command)
    {
        ...
        if (!DateTime.TryParse(command.SafeRemainingArgument, out DateTime dt))
        {
    ...
    private bool BuildingCommandTemplate(ICharacter actor)
    {
        ...
        {
            ScriptedEvent newTemplate = new(this, null);
