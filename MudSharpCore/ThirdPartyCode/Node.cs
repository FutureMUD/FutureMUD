...
public class Node<T> : IEqualityComparer, IEnumerable<T>, IEnumerable<Node<T>>
{
    ...
    public IEnumerable<Node<T>> Leaves => Descendants.Where(n => !n.Children.Any());
    ...
    public static IEnumerable<Node<T>> CreateTree<TId>(IEnumerable<T> values, Func<T, TId> idSelector,
        ...
        where TId : struct
    {
        List<T> valuesCache = values.ToList();
        if (!valuesCache.Any())
        {
        ...
        IEnumerable<Node<T>> nodes = valuesCache.Select(v => new Node<T>(v));
    ...
    public static IEnumerable<Node<T>> CreateTree<TId>(IEnumerable<Node<T>> rootNodes, Func<T, TId> idSelector,
        ...
        where TId : struct
    {
        List<Node<T>> rootNodesCache = rootNodes.ToList();
        List<Node<T>> duplicates = rootNodesCache.Duplicates(n => n).ToList();
        if (duplicates.Any())
        {
            throw new ArgumentException(
        ...
        foreach (Node<T> rootNode in rootNodesCache)
        {
            TId? parentId = parentIdSelector(rootNode.Value);
            Node<T> parent = rootNodesCache.FirstOrDefault(n => IsSameId(idSelector(n.Value), parentId));
        ...
        IEnumerable<Node<T>> result = rootNodesCache.Where(n => n.IsRoot);
        return result;
    }
    ...
    {
        Node<T> valueThisType = anderePeriode as Node<T>;
    ...
    {
        Node<T> valueThisType1 = value1 as Node<T>;
        Node<T> valueThisType2 = value2 as Node<T>;
...
public class PolyNode<T> : IEqualityComparer, IEnumerable<T>, IEnumerable<PolyNode<T>>
{
    ...
    public IEnumerable<PolyNode<T>> Leaves => Descendants.Where(n => !n.Children.Any());
    ...
    {
        PolyNode<T> childNode = new(value);
        Add(childNode, index);
        return childNode;
    }
    ...
    public void Disconnect()
    {
        ...
        foreach (PolyNode<T> parent in _parents)
        {
            parent._children.Remove(this);
        }
        ...
        where TId : struct
    {
        ...
        IEnumerable<PolyNode<T>> nodes = valuesCache.Select(v => new PolyNode<T>(v));
        return CreateTree(nodes, idSelector, parentIdSelector);
    }
    ...
    public static IEnumerable<PolyNode<T>> CreateTree<TId>(IEnumerable<PolyNode<T>> rootNodes,
        ...
        where TId : struct
    {
        List<PolyNode<T>> rootNodesCache = rootNodes.ToList();
        List<PolyNode<T>> duplicates = rootNodesCache.Duplicates(n => n).ToList();
        if (duplicates.Any())
        {
            throw new ArgumentException(
                "One or more values contains {0} duplicate keys. The first duplicate is: [{1}]".FormatInvariant(
        ...
        foreach (PolyNode<T> rootNode in rootNodesCache)
        {
            TId? parentId = parentIdSelector(rootNode.Value);
            PolyNode<T> parent = rootNodesCache.FirstOrDefault(n => IsSameId(idSelector(n.Value), parentId));
        ...
        IEnumerable<PolyNode<T>> result = rootNodesCache.Where(n => n.IsRoot);
        return result;
    }
    ...
    public override bool Equals(object anderePeriode)
    {
        PolyNode<T> valueThisType = anderePeriode as PolyNode<T>;
        return this == valueThisType;
    }
    ...
    bool IEqualityComparer.Equals(object value1, object value2)
    {
        PolyNode<T> valueThisType1 = value1 as PolyNode<T>;
        PolyNode<T> valueThisType2 = value2 as PolyNode<T>;
