...
public class Clock : SaveableItem, IClock
{
    ...
    public void LoadFromXml(XElement root)
    {
        ...
        foreach (XElement subElement in element.Elements("HourIntervalName"))
        {
        ...
        foreach (XElement subElement in element.Elements("HourIntervalLongName"))
        {
            if (subElement.Value.Length == 0)
            {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Clock dbclock = FMDB.Context.Clocks.Find(Id);
    ...
    private Clock(Clock rhs, string name, string alias)
    {
        ...
        foreach (BoundRange<string> range in rhs.CrudeTimeIntervals.Ranges)
        {
        ...
        foreach (IMudTimeZone timezone in rhs.Timezones)
        {
            MudTimeZone tz = new(this, timezone.OffsetHours, timezone.OffsetMinutes, timezone.Description, timezone.Alias);
            AddTimezone(tz);
    ...
    public Clock(MudSharp.Models.Clock clock, IFuturemud game)
    {
        ...
        _name = Alias;
        foreach (Timezone timezone in clock.Timezones)
        {
    ...
    {
        string interval = CrudeTimeIntervals.Get(hour);
        double fraction = CrudeTimeIntervals.RangeFraction(hour);
        ...
        return Regex.Replace(timeString, @"\$(?<variable>[\w])", m =>
        {
            switch (m.Groups["variable"].Value)
            {
                ...
                case "l":
                    ...
                    int vh = GetVagueHour(theTime.Hours, theTime.Minutes, MinutesPerHour, HoursPerDay);
    ...
    public string Show(ICharacter actor)
    {
        ...
        if (CrudeTimeIntervals.Ranges.Any())
        {
            ...
            int i = 1;
            foreach (BoundRange<string> range in CrudeTimeIntervals.Ranges)
            {
