...
public class ButcheryProduct : SaveableItem, IButcheryProduct
{
        ...
        int quantity)
    {
        ...
        using (new FMDB())
        {
            ...
            foreach (IBodypart item in _requiredBodyparts)
            {
    ...
    public ButcheryProduct(ButcheryProduct rhs, string newName)
    {
        ...
        foreach (IBodypart item in rhs.RequiredBodyparts)
        {
        ...
        foreach (IButcheryProductItem item in rhs.ProductItems)
        {
    ...
    public ButcheryProduct(Models.ButcheryProducts product, IFuturemud gameworld)
    {
        ...
        {
            IBodypart bodypart = gameworld.BodypartPrototypes.Get(item.BodypartProtoId);
            if (bodypart is null)
            {
                continue;
            }
        ...
        foreach (ButcheryProductItems item in product.ButcheryProductItems)
        {
    ...
    private bool BuildingCommandDamaged(ICharacter actor, StringStack command, IButcheryProductItem product)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int quantity) || quantity < 1)
        {
        ...
        IGameItemProto proto = Gameworld.ItemProtos.Get(id);
        if (proto is null)
        {
    ...
    private bool BuildingCommandBody(ICharacter actor, StringStack command)
    {
        ...
        IBodyPrototype body = Gameworld.BodyPrototypes.GetByIdOrName(command.PopSpeech());
        if (body is null)
        {
        ...
        foreach (IBodypart part in RequiredBodyparts)
        {
        ...
        else
        {
            ...
            {
                IBodypart part = body.AllBodyparts.GetByNameOrAbbreviation(command.PopSpeech());
                if (part is null)
                {
    ...
    private bool BuildingCommandBodypart(ICharacter actor, StringStack command)
    {
        ...
        IBodypart part = TargetBody.AllBodyparts.GetByNameOrAbbreviation(command.SafeRemainingArgument);
        if (part is null)
        {
    ...
    {
        ButcheryProducts dbitem = FMDB.Context.ButcheryProducts.Find(Id);
        dbitem.Name = Name;
