...
public class RaceButcheryProfile : SaveableItem, IRaceButcheryProfile
{
    public RaceButcheryProfile(MudSharp.Models.RaceButcheryProfile profile, IFuturemud gameworld)
    {
        ...
        foreach (IButcheryProduct product in profile.RaceButcheryProfilesButcheryProducts.SelectNotNull(item =>
        ...
        foreach (RaceButcheryProfilesSkinningEmotes item in profile.RaceButcheryProfilesSkinningEmotes.OrderBy(x => x.Order))
        {
    ...
    public RaceButcheryProfile(IFuturemud gameworld, string name)
    {
        ...
        using (new FMDB())
        {
            Models.RaceButcheryProfile dbitem = new()
            {
                Name = name,
    ...
    public IRaceButcheryProfile Clone(string newName, bool includeProducts)
    {
        using (new FMDB())
        {
            Models.RaceButcheryProfile dbnew = new()
            {
                Name = newName,
            ...
            foreach (KeyValuePair<string, (ITraitDefinition Trait, Difficulty CheckDifficulty)> check in _breakdownChecks)
            {
            ...
            int i = 1;
            foreach ((string Emote, double Delay) emote in _skinEmotes)
            {
            ...
            foreach (KeyValuePair<string, List<(string Emote, double Delay)>> category in _breakdownEmotes)
            {
                i = 1;
                foreach ((string Emote, double Delay) emote in category.Value)
                {
            ...
            if (includeProducts)
            {
                foreach (IButcheryProduct product in _products)
                {
    ...
    {
        Models.RaceButcheryProfile dbitem = FMDB.Context.RaceButcheryProfiles.Find(Id);
        dbitem.Name = Name;
        ...
        foreach (KeyValuePair<string, (ITraitDefinition Trait, Difficulty CheckDifficulty)> item in _breakdownChecks)
        {
            dbitem.RaceButcheryProfilesBreakdownChecks.Add(new RaceButcheryProfilesBreakdownChecks
            {
                RaceButcheryProfile = dbitem,
                Subcageory = item.Key,
                TraitDefinitionId = item.Value.Trait.Id,
        ...
        foreach (KeyValuePair<string, List<(string Emote, double Delay)>> emote in _breakdownEmotes)
        {
            int order = 1;
            foreach ((string Emote, double Delay) value in emote.Value)
            {
...
#FSub-Component Butcher Phases#0
    ...
    private bool BuildingCommandCheck(ICharacter actor, StringStack command)
    {
        ...
        if (!command.PopSpeech().TryParseEnum<Difficulty>(out Difficulty difficulty))
        {
            actor.OutputHandler.Send("That is not a valid difficulty.");
            return false;
        }
    ...
    private bool BuildingCommandSkinEmoteDelay(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double dvalue) || dvalue <= 0.00)
        {
            actor.OutputHandler.Send("You must enter a valid number of seconds for the delay.");
            return false;
        }

        string old = _skinEmotes[value - 1].Emote;
    ...
    private bool BuildingCommandSkinEmoteRemove(ICharacter actor, StringStack command)
    {
        ...
        (string Emote, double Delay) emote = _skinEmotes[value - 1];
    ...
    private bool BuildingCommandSkinEmoteEdit(ICharacter actor, StringStack command)
    {
        ...
        double olddelay = _skinEmotes[value - 1].Delay;
    ...
    private bool BuildingCommandEmoteRemove(ICharacter actor, StringStack command, string subsystem)
    {
        ...
        (string Emote, double Delay) emote = _breakdownEmotes[subsystem].ElementAt(value - 1);
    ...
    private bool BuildingCommandEmoteEdit(ICharacter actor, StringStack command, string subsystem)
    {
        ...
        double olddelay = _breakdownEmotes[subsystem].ElementAt(value - 1).Delay;
    ...
    private bool BuildingCommandEmoteDelay(ICharacter actor, StringStack command, string subsystem)
    {
        ...
        string old = _breakdownEmotes[subsystem].ElementAt(value - 1).Emote;
    ...
    private bool BuildingCommandProduct(ICharacter actor, StringStack command)
    {
        ...
        IButcheryProduct product = long.TryParse(command.PopSpeech(), out long value)
    ...
    public string Show(ICharacter voyeur)
    {
        ...
        if (SkinEmotes.Any() || Products.Any(x => x.IsPelt))
        {
            ...
            sb.AppendLine("Skinning".GetLineWithTitle(voyeur.LineFormatLength, voyeur.Account.UseUnicode,
                ...
                Telnet.BoldRed));
            sb.AppendLine();
            foreach ((string Emote, double Delay) emote in SkinEmotes)
            {
                sb.AppendLine(
        ...
        sb.AppendLine();
        i = 1;
        foreach ((string Emote, double Delay) emote in BreakdownEmotes(string.Empty))
        {
            sb.AppendLine(
                $"\t#{i++.ToString("N0", voyeur)}) [{emote.Delay.ToString("N2", voyeur)}s] {emote.Emote.ColourCommand()}");
        }
        ...
        foreach (string category in _breakdownChecks.Keys)
        {
            ...
            sb.AppendLine();
            i = 1;
            foreach ((string Emote, double Delay) emote in BreakdownEmotes(category))
            {
                sb.AppendLine(
                    $"\t#{i++.ToString("N0", voyeur)}) [{emote.Delay.ToString("N2", voyeur)}s] {emote.Emote.ColourCommand()}");
            }
