...
public class Craft : Framework.Revision.EditableItem, ICraft
{
    ...
    public Craft(Craft rhs, IAccount newOriginator, string newName) : base(newOriginator)
    {
        ...
        using (new FMDB())
        {
            ...
            foreach (ICraftPhase phase in _craftPhases)
            {
                dbnew.CraftPhases.Add(phase.CreateDBPhase());
            }

            Dictionary<long, Models.CraftInput> inputMap = new();
            foreach (ICraftInput input in rhs._orderedInputs)
            {
                inputMap[input.Id] = input.CreateNewRevision(dbnew);
            }

            Dictionary<long, Models.CraftTool> toolMap = new();
            foreach (ICraftTool tool in rhs._orderedTools)
            {
                toolMap[tool.Id] = tool.CreateNewRevision(dbnew);
            }
            FMDB.Context.SaveChanges();

            foreach (ICraftProduct product in rhs._orderedProducts)
            {
            ...
            foreach (ICraftProduct product in rhs._orderedFailProducts)
            {
            ...
            foreach (Models.CraftInput input in dbnew.CraftInputs)
            {
                ICraftInput newInput = CraftInputFactory.Factory.LoadInput(input, this, Gameworld);
                _inputs[newInput.Id] = newInput;
            }

            _orderedInputs.AddRange(_inputs.Select(x => x.Value).OrderBy(x => x.OriginalAdditionTime));

            foreach (Models.CraftTool tool in dbnew.CraftTools)
            {
                ICraftTool newTool = CraftToolFactory.Factory.LoadTool(tool, this, Gameworld);
                _tools[newTool.Id] = newTool;
            }

            _orderedTools.AddRange(_tools.Select(x => x.Value).OrderBy(x => x.OriginalAdditionTime));

            foreach (Models.CraftProduct product in dbnew.CraftProducts)
            {
                ICraftProduct newProduct = CraftProductFactory.Factory.LoadProduct(product, this, Gameworld);
    ...
    public Craft(Models.Craft craft, IFuturemud gameworld) : base(craft.EditableItem)
    {
        ...
        foreach (Models.CraftPhase phase in craft.CraftPhases.OrderBy(x => x.PhaseNumber))
        {
            _craftPhases.Add(new CraftPhase(phase, this));
        }

        foreach (Models.CraftInput input in craft.CraftInputs)
        {
            ICraftInput newInput = CraftInputFactory.Factory.LoadInput(input, this, Gameworld);
            _inputs[newInput.Id] = newInput;
        }
        ...
        foreach (Models.CraftTool tool in craft.CraftTools)
        {
            ICraftTool newTool = CraftToolFactory.Factory.LoadTool(tool, this, Gameworld);
            _tools[newTool.Id] = newTool;
        }
        ...
        foreach (Models.CraftProduct product in craft.CraftProducts)
        {
            ICraftProduct newProduct = CraftProductFactory.Factory.LoadProduct(product, this, Gameworld);
            if (product.IsFailProduct)
            {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.Craft dbcraft = FMDB.Context.Crafts.Find(Id, RevisionNumber);
            if (dbcraft == null)
            {
                Changed = false;
                return;
            ...
            foreach (Models.CraftInput input in dbcraft.CraftInputs.ToList())
            {
            ...
            foreach (Models.CraftTool tool in dbcraft.CraftTools.ToList())
            {
            ...
            foreach (Models.CraftProduct product in dbcraft.CraftProducts.ToList())
            {
    ...
    private ItemQuality GetReferenceQuality(ICharacter character, IActiveCraftGameItemComponent component)
    {
        ...
        {
            ICheck qualityCheck = Gameworld.GetCheck(CheckType.CraftQualityCheck);
    ...
    private Outcome HandleSkillCheck(ICharacter character, IActiveCraftGameItemComponent component)
    {
        ...
        {
            ICheck outcomeCheck = Gameworld.GetCheck(CheckType.CraftOutcomeCheck);
            CheckOutcome result = outcomeCheck.Check(character, CheckDifficulty, CheckTrait,
            ...
            for (int i = 0; i < FreeSkillChecks; i++)
            {
    ...
    private void SetupPhaseInformation()
    {
        ...
        foreach (string echo in PhaseEchoes)
        {
            foreach (Match match in InputInEchoRegex.Matches(echo))
            {
                ICraftInput whichInput = _orderedInputs.ElementAtOrDefault(int.Parse(match.Groups["id"].Value) - 1);
                if (whichInput == null)
                {
                    _craftIsValid = false;
            ...
            {
                ICraftTool whichTool = _orderedTools.ElementAtOrDefault(int.Parse(match.Groups["id"].Value) - 1);
                if (whichTool == null)
                {
                    _craftIsValid = false;
                    _invalidEchoReferences.Add((match.Value, phase, false));
            ...
            {
                ICraftProduct whichProduct = _orderedProducts.ElementAtOrDefault(int.Parse(match.Groups["id"].Value) - 1);
        ...
        phase = 1;
        foreach (string echo in FailPhaseEchoes)
        {
            foreach (Match match in InputInEchoRegex.Matches(echo))
            {
                ICraftInput whichInput = _orderedInputs.ElementAtOrDefault(int.Parse(match.Groups["id"].Value) - 1);
            ...
            foreach (Match match in FailProductInEchoRegex.Matches(echo))
            {
                ICraftProduct whichProduct = _orderedFailProducts.ElementAtOrDefault(int.Parse(match.Groups["id"].Value) - 1);
                if (whichProduct == null)
                {
                    _craftIsValid = false;
                    _invalidEchoReferences.Add((match.Value, phase, true));
        ...
        foreach (long item in _inputs.Keys)
        {
        ...
        foreach (long item in _products.Keys)
        {
        ...
        foreach (long item in _failProducts.Keys)
        {
        ...
        foreach (long item in _tools.Keys)
        {
            if (_craftToolUsagePhases.All(x => !x.Value.Contains(item)))
            {
                foreach (int key in _craftToolUsagePhases.Keys)
                {
        ...
        List<string> phaseEchoes = _craftPhases.Select(x => x.Echo).ToList();
        if (phaseEchoes.Count > 0)
        {
            foreach (int phasenum in 1.GetIntRange(phaseEchoes.Count))
            {
    ...
    {
        string emoteText = echo;
        List<IPerceivable> perceivables = new()
        { character };
        int perceiverIndex = 1;
        emoteText = InputInEchoRegex.Replace(emoteText, m =>
        {
            ICraftInput input = _orderedInputs[int.Parse(m.Groups["id"].Value) - 1];
            ICraftInputData inputData = component.ConsumedInputs[input].Data;
        ...
        {
            ICraftTool tool = _orderedTools[int.Parse(m.Groups["id"].Value) - 1];
            IGameItem actualTool = results.First(x => x.OriginalReference == tool).PrimaryTarget;
        ...
        if (component.HasFailed)
        {
            ...
            {
                ICraftProduct product = _orderedFailProducts[int.Parse(m.Groups["id"].Value) - 1];
                ...
                IPerceivable productPerceivable = component.ProducedProducts[product].Perceivable;
        ...
        else
        {
            ...
            {
                ICraftProduct product = _orderedProducts[int.Parse(m.Groups["id"].Value) - 1];
                if (!component.ProducedProducts.ContainsKey(product))
                {
                    throw new ApplicationException(
            ...
            IActiveCraftGameItemComponent component, int fromPhase = 1, int toPhase = int.MaxValue)
    {
        ...
        Dictionary<int, IInventoryPlan> plans = new();
        // First check to see if the tool plan can work
        foreach (KeyValuePair<int, IInventoryPlanTemplate> plan in _phaseInventoryPlans)
        {
            ...
            IInventoryPlanTemplate template = component?.HasFailed == true ? _failPhaseInventoryPlans[plan.Key] : plan.Value;
            IInventoryPlan phasePlan = template.CreatePlan(character);
        ...
        List<int> phasesChecked = fromPhase.GetIntRange(toPhase);
        List<(ICraftInput Input, IEnumerable<IPerceivable> Options)> scoutedTargets = new();
        List<ICraftInput> missingInputs = new();

        // Next gather a list of the potential options for each input not already consumed in those phases
        foreach (ICraftInput input in Inputs)
        {
        ...
        foreach (KeyValuePair<ICraftInput, (IPerceivable Original, ICraftInputData Data)> consumed in component?.ConsumedInputs ??
                                 Enumerable
        ...
        OptionSolver<ICraftInput, IPerceivable> clashSolver = new(
        ...
        (bool Success, List<(ICraftInput Problem, IPerceivable Option)> Solution, List<ICraftInput> UnsolvableChoices) clashResults = clashSolver.SolveOptions();
        ...
        bool allowStartOnly, bool ignoreToolAndMaterialFailure)
    {
        ...
        (bool success, IEnumerable<(ICraftInput Input, IPerceivable Target)> _, Dictionary<int, IInventoryPlan> plans, IEnumerable<ICraftInput> missingInput) = ScoutToolsAndInputs(character, component);
        if (success)
        {
        ...
        foreach (KeyValuePair<int, IInventoryPlan> plan in plans)
        {
    ...
    {
        (bool success, string error) = CanDoCraft(character, null, false, true);

        (bool _, IEnumerable<(ICraftInput Input, IPerceivable Target)> inputs, Dictionary<int, IInventoryPlan> plans, IEnumerable<ICraftInput> missing) = ScoutToolsAndInputs(character, null);
        ...
        sb.AppendLine("Tools:");
        sb.AppendLine();
        IEnumerable<InventoryPlanActionResult> results = plans.SelectMany(x => x.Value.ScoutAllTargets());
        foreach (ICraftTool item in _orderedTools)
        {
            InventoryPlanActionResult result = results.FirstOrDefault(x => x.OriginalReference == item && x.PrimaryTarget != null);
            if (result == null)
            {
        ...
        sb.AppendLine();
        foreach (ICraftInput input in _orderedInputs)
        {
            ...
            {
                IPerceivable tInput = inputs.FirstOrDefault(x => x.Input == input).Target;
                if (tInput == null)
                {
                    sb.AppendLine(
    ...
    {
        (bool success, IEnumerable<(ICraftInput Input, IPerceivable Target)> _, Dictionary<int, IInventoryPlan> plans, IEnumerable<ICraftInput> missingInput) = ScoutToolsAndInputs(character, active, active.Phase);
        if (!success)
        {
            if (Interruptable)
            {
            ...
            foreach (IInventoryPlan plan in plans.Values)
            {
    ...
    {
        (bool success, IEnumerable<(ICraftInput Input, IPerceivable Target)> inputs, Dictionary<int, IInventoryPlan> plans, IEnumerable<ICraftInput> missing) = ScoutToolsAndInputs(character, component, phase, phase);
        if (!success)
        {
            if (missing != null)
            {
        ...
        ICraftPhase craftPhase = _craftPhases[phase - 1];
        ...
        IInventoryPlan plan = plans.Single().Value;
        List<InventoryPlanActionResult> results = plan.ExecuteWholePlan().ToList();
        double phaseLengthSeconds = craftPhase.PhaseLengthInSeconds;
        List<(IGameItem Item, ICraftTool Tool)> tools = results
        ...
        if (!component.HasFailed)
        {
            foreach (KeyValuePair<long, ICraftInput> input in _inputs.Where(x =>
            ...
            {
                IPerceivable target = inputs.First(x => x.Input == input.Value).Target;
                component.ConsumedInputs[input.Value] = (target, input.Value.ReserveInput(target));
            }
        }

        HandleSkillCheck(character, component);
        ItemQuality referenceQuality = GetReferenceQuality(character, component);

        string echo;
        if (component.HasFailed)
        {
            echo = FailPhaseEchoes.ElementAt(phase - 1);
            foreach (KeyValuePair<long, ICraftProduct> product in _failProducts.Where(x =>
        ...
        else
        {
            ...
            foreach (KeyValuePair<long, ICraftProduct> product in _products.Where(x =>
    ...
    public string DisplayCraft(ICharacter character)
    {
        ...
        if (Tools.Any())
        {
            ...
            foreach (ICraftTool tool in Tools)
            {
                sb.AppendLine(
        ...
        if (Products.Any())
        {
            ...
            foreach (ICraftProduct product in Products)
            {
        ...
        if (FailProducts.Any())
        {
            ...
            foreach (ICraftProduct product in FailProducts)
            {
                sb.AppendLine($"\t{product.HowSeen(character).ColourIncludingReset(Telnet.Yellow)}");
            }
        }
    ...
    private bool BuildingCommandFailProduct(ICharacter actor, StringStack command)
    {
        ...
        if (!string.IsNullOrEmpty(command.Last) && int.TryParse(command.Last, out int value))
        {
    ...
    private bool BuildingCommandFailProductRemove(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.SafeRemainingArgument, out int value))
        {
            actor.OutputHandler.Send(
        ...
        ICraftProduct product = _orderedFailProducts[value - 1];
    ...
    private bool BuildingCommandFailProductAdd(ICharacter actor, StringStack command)
    {
        ...
        ICraftProduct newProduct = CraftProductFactory.Factory.LoadProduct(command.PopSpeech(), this, Gameworld, true);
        if (newProduct == null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandProductRemove(ICharacter actor, StringStack command)
    {
        ...
        ICraftProduct product = _orderedProducts[value - 1];
    ...
    private bool BuildingCommandProductAdd(ICharacter actor, StringStack command)
    {
        ...
        Gameworld.SaveManager.Flush();
        ICraftProduct newProduct = CraftProductFactory.Factory.LoadProduct(command.PopSpeech(), this, Gameworld, false);
        if (newProduct == null)
        {
            actor.OutputHandler.Send(
                $"There is no such product type to add.\nValid types are: {CraftProductFactory.Factory.ValidBuilderTypes.ListToString()}");
    ...
    private bool BuildingCommandToolRemove(ICharacter actor, StringStack command)
    {
        ...
        ICraftTool tool = _orderedTools[value - 1];
        _tools.Remove(tool.Id);
        _orderedTools.RemoveAt(value - 1);
        foreach (KeyValuePair<int, HashSet<long>> item in _craftToolUsagePhases)
        {
            item.Value.Remove(tool.Id);
        }

        foreach (KeyValuePair<int, HashSet<long>> item in _craftToolUsageFailPhases)
        {
            item.Value.Remove(tool.Id);
        }
    ...
    private bool BuildingCommandToolAdd(ICharacter actor, StringStack command)
    {
        ...
        Gameworld.SaveManager.Flush();
        ICraftTool newTool = CraftToolFactory.Factory.LoadTool(command.PopSpeech(), this, Gameworld);
        if (newTool == null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandInputRemove(ICharacter actor, StringStack command)
    {
        ...
        ICraftInput input = _orderedInputs[value - 1];
    ...
    private bool BuildingCommandInputAdd(ICharacter actor, StringStack command)
    {
        ...
        Gameworld.SaveManager.Flush();
        ICraftInput newInput = CraftInputFactory.Factory.LoadInput(command.PopSpeech(), this, Gameworld);
        if (newInput == null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandPhaseAdd(ICharacter actor, StringStack command)
    {
        ...
        string phaseText = command.PopSpeech();
        string failText = command.IsFinished ? phaseText : command.PopSpeech();
    ...
    private bool BuildingCommandPhaseRemove(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value) || value <= 0 || value > LastPhase)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandPhaseEdit(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int index) || index <= 0 || index > LastPhase)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandPhaseSwap(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int value2) || value2 <= 0 || value2 > LastPhase)
        {
            actor.OutputHandler.Send(
                $"You must specify a valid phase. There are currently {_craftPhases.Count} phases.");
            return false;
    ...
    private bool BuildingCommandPhaseExertion(ICharacter actor, StringStack command)
    {
        ...
        if (!command.SafeRemainingArgument.TryParseEnum<ExertionLevel>(out ExertionLevel value))
        {
    ...
    private bool BuildingCommandFailThreshold(ICharacter actor, StringStack command)
    {
        ...
        if (!CheckExtensions.GetOutcome(command.SafeRemainingArgument, out Outcome outcome))
        {
    ...
    private bool BuildingCommandCheck(ICharacter actor, StringStack command)
    {
        ...
        if (!int.TryParse(command.PopSpeech(), out int checks) || checks < 0)
        {
    ...
    public override string Show(ICharacter actor)
    {
        ...
        List<List<string>> phaseStrings = new();
        foreach (ICraftPhase phase in _craftPhases)
        {
            phaseStrings.Add([
    ...
    public override IEditableRevisableItem CreateNewRevision(ICharacter initiator)
    {
        using (new FMDB())
        {
            ...
            Dictionary<long, Models.CraftTool> toolMap = new();
            foreach (ICraftTool tool in _orderedTools)
            {
                toolMap[tool.Id] = tool.CreateNewRevision(dbnew);
            }
            FMDB.Context.SaveChanges();

            foreach (ICraftProduct product in _orderedProducts)
            {
                product.CreateNewRevision(dbnew, false, inputMap.ToDictionary(x => x.Key, x => x.Value.Id), toolMap.ToDictionary(x => x.Key, x => x.Value.Id));
            }

            foreach (ICraftProduct product in _orderedFailProducts)
            {
                product.CreateNewRevision(dbnew, true, inputMap.ToDictionary(x => x.Key, x => x.Value.Id), toolMap.ToDictionary(x => x.Key, x => x.Value.Id));
            }
