...
public class CommodityInput : BaseInput, ICraftInputConsumesGameItem
{
	...
	public override string Name
	{
		get
		{
			...
			return
				$"{Gameworld.UnitManager.DescribeExact(Weight, Framework.Units.UnitType.Mass, DummyAccount.Instance).Colour(Telnet.Green)} of {Material.Name.Colour(Material.ResidueColour)}";
		}
	}
	...
	public override string HowSeen(IPerceiver voyeur)
	{
		...
		return
			$"{Gameworld.UnitManager.DescribeExact(Weight, Framework.Units.UnitType.Mass, voyeur).Colour(Telnet.Green)} of {Material.Name.Colour(Material.ResidueColour)}";
	}
	...
	public override IEnumerable<IPerceivable> ScoutInput(ICharacter character)
	{
		return character.DeepContextualItems
		                ...
		                .Where(x => x.Material == Material && x.Weight >= Weight)
		                .Select(x => x.Parent)
		                .ToList();
	}
