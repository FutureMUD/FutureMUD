...
public class CommodityTagInput : BaseInput, ICraftInputConsumesGameItem
{
    internal class CommodityInputData : ICraftInputData
    {
        ...
        public void ConsumeInput()
        {
            ...
            IConnectable originalConnectable = ConsumedInput.GetItemType<IConnectable>();
            foreach (IGameItem attached in ConsumedInput.AttachedAndConnectedItems)
            {
    ...
    protected CommodityTagInput(Models.CraftInput input, ICraft craft, IFuturemud gameworld) : base(input, craft,
        gameworld)
    {
    ...
    {
        CraftInputFactory.RegisterCraftInputType("CommodityTag",
            (input, craft, game) => new CommodityTagInput(input, craft, game));
        CraftInputFactory.RegisterCraftInputTypeForBuilders("commoditytag",
            (craft, game) => new CommodityTagInput(craft, game));
    }
    ...
    {
        return MaterialTag != null && Weight > 0.0;
    }
    ...
    public override string WhyNotValid()
    {
        if (MaterialTag == null)
        {
            return "You must first set a material tag for this input to consume.";
        }
    ...
    public override string Name
    {
        get
        {
            if (MaterialTag == null)
            {
                return "An unspecified amount of an unspecified material tag".Colour(Telnet.Red);
            }
...
{matchedtags.Select(x => $"\t[{x.Id.ToString("N0", actor)}] {x.FullName.ColourName()}").ListToLines()}");
    ...
    private bool BuildingCommandQuantity(ICharacter actor, StringStack command)
    {
        if (MaterialTag == null)
        {
            actor.OutputHandler.Send("You must first set a material tag before you set a weight.");
            return false;
        }
        ...
        InputChanged = true;
        actor.OutputHandler.Send(
            $"This input will now consume {Gameworld.UnitManager.DescribeExact(Weight, Framework.Units.UnitType.Mass, actor).Colour(Telnet.Green)} of a material tagged as {MaterialTag.FullName.Colour(Telnet.Cyan)}.");
        return true;
    }
    ...
    private bool BuildingCommandMaterial(ICharacter actor, StringStack command)
    {
        if (command.IsFinished)
        {
            actor.OutputHandler.Send("Which material tag did you want this input to consume materials from?");
            return false;
        }
