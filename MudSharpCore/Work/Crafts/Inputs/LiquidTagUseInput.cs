...
public class LiquidTagUseInput : BaseInput, ICraftInputConsumeLiquid
{
    ...
    {
        List<(ILiquidContainer Container, bool CountsAs)> items = character.ContextualItems
        ...
        double sum = 0.0;
        List<IGameItem> finalItems = new();
        foreach ((ILiquidContainer Container, bool CountsAs) item in items)
        {
    ...
    private bool BuildingCommandAmount(ICharacter actor, StringStack command)
    {
        ...
        double amount =
            Gameworld.UnitManager.GetBaseUnits(command.SafeRemainingArgument, UnitType.FluidVolume, out bool success);
        if (!success)
        {
    ...
    internal class LiquidUseInputData : ICraftInputConsumeLiquidData
    {
        ...
        public LiquidUseInputData(XElement root, IFuturemud gameworld)
        {
            ...
            foreach (XElement item in root.Elements("OriginalItem"))
            {
                IGameItem gitem = gameworld.TryGetItem(long.Parse(item.Value));
                if (gitem != null)
                {
        ...
        public void ConsumeInput()
        {
            ...
            List<(ItemQuality Quality, double Weight)> qualities = new();
            List<LiquidInstance> takenInstances = new();
            foreach (IGameItem container in OriginalItems)
            {
                ILiquidContainer lcontainer = container.GetItemType<ILiquidContainer>();
                List<LiquidInstance> instances = lcontainer.LiquidMixture.Instances
                                          .Where(x => x.Liquid.IsA(Target))
                                          .ToList();
                foreach (LiquidInstance instance in instances)
                {
        ...
        {
            foreach (IGameItem item in OriginalItems)
            {
