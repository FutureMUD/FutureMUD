...
public class LiquidUseInput : BaseInput, ICraftInputConsumeLiquid
{
    ...
    {
        List<(ILiquidContainer Container, (bool Truth, ItemQuality Quality) CountsAs)> items = character.ContextualItems
        ...
        foreach ((ILiquidContainer Container, (bool Truth, ItemQuality Quality) CountsAs) item in items)
        {
    ...
    private bool BuildingCommandLiquid(ICharacter actor, StringStack command)
    {
        ...
        ILiquid liquid = long.TryParse(command.SafeRemainingArgument, out long value)
