...
public class TagVariableInput : TagInput, IVariableInput
{
    ...
    private bool BuildingCommandVariable(ICharacter actor, StringStack command)
    {
        ...
        ICharacteristicDefinition definition = long.TryParse(command.SafeRemainingArgument, out long value)
    ...
    public override IEnumerable<IPerceivable> ScoutInput(ICharacter character)
    {
        ...
        List<IPerceivable> returnItems = new();
        foreach (IGameItem item in character.DeepContextualItems.Except(character.Body.WornItems)
