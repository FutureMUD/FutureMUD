...
public abstract class BaseProduct : LateInitialisingItem, ICraftProduct
{
    ...
    {
        CraftProduct dbitem = FMDB.Context.CraftProducts.Find(Id);
    ...
    protected ISolid DetermineOverrideMaterial(IActiveCraftGameItemComponent component)
    {
        ...
        ICraftInput input = Craft.Inputs.ElementAt(MaterialDefiningInputIndex.Value);
        (IPerceivable consumed, ICraftInputData _) = component.ConsumedInputs.ValueOrDefault(input, default);
        if (consumed == null)
        {
            return null;
        }
