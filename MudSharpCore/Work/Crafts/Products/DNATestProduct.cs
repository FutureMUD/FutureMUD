...
public class DNATestProduct : BaseProduct
{
    ...
    internal class DNATestProductData : ICraftProductData
    {
        ...
        public void ReleaseProducts(ICell location, RoomLayer layer)
        {
            List<BloodLiquidInstance> bloods1 = Mixture1.Instances
                                  ...
                                  .ToList();
            List<BloodLiquidInstance> bloods2 = Mixture2.Instances
                                  .OfType<BloodLiquidInstance>()
                                  .ToList();
            bool match = bloods1.Any(x => bloods2.Any(y => x.SourceId == y.SourceId));
    ...
    public override string Name
    {
        get
        {
            ICraftInput input1 = Craft.Inputs.FirstOrDefault(x => x.Id == WhichInputId1);
            ICraftInput input2 = Craft.Inputs.FirstOrDefault(x => x.Id == WhichInputId2);
            string input1text = input1 == null || !(input1 is ICraftInputConsumeLiquid)
                ? "an invalid input".Colour(Telnet.Red)
                : $"{input1.Name} ($i{Craft.Inputs.ToList().IndexOf(input1) + 1})";
            string input2text = input2 == null || !(input2 is ICraftInputConsumeLiquid)
                ? "an invalid input".Colour(Telnet.Red)
    ...
    {
        KeyValuePair<ICraftInput, (IPerceivable Input, ICraftInputData Data)> input1 = component.ConsumedInputs.FirstOrDefault(x => x.Key.Id == WhichInputId1);
        if (input1.Key == null)
        {
            throw new ApplicationException("Couldn't find a valid input for craft product to load.");
        }

        KeyValuePair<ICraftInput, (IPerceivable Input, ICraftInputData Data)> input2 = component.ConsumedInputs.FirstOrDefault(x => x.Key.Id == WhichInputId2);
        if (input2.Key == null)
        {
            throw new ApplicationException("Couldn't find a valid input for craft product to load.");
        }
