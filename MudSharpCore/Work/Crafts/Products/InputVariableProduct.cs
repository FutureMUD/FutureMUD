...
internal class InputVariableProduct : BaseProduct
{
    ...
    protected InputVariableProduct(CraftProduct product, ICraft craft, IFuturemud gameworld) : base(product, craft, gameworld)
    {
        ...
        {
            ICharacteristicDefinition definition = Gameworld.Characteristics.Get(long.Parse(element.Attribute("definition").Value));
            ...
            List<(long, ICharacteristicValue)> list = new();
            foreach (XElement sub in element.Elements("Specific"))
            {
                ICharacteristicValue value = Gameworld.CharacteristicValues.Get(long.Parse(sub.Attribute("value").Value));
                if (value is null)
                {
                    continue;
                }
    ...
    public override ICraftProductData ProduceProduct(IActiveCraftGameItemComponent component, ItemQuality referenceQuality)
    {
        ...
        foreach ((ICharacteristicDefinition definition, int input) in VariableIndexes)
        {
            ICraftInput ivi = Craft.Inputs.ElementAt(input - 1);
            if (!component.ConsumedInputs.ContainsKey(ivi))
            {
                continue;
            }

            ICraftInputDataWithItems data = component.ConsumedInputs[ivi].Data as ICraftInputDataWithItems;
            if (data is null)
            {
                continue;
            }

            List<(long Proto, ICharacteristicValue Value)> targets = VariableSpecifics[definition];
            foreach (IGameItem item in data.ConsumedItems)
            {
                (long Proto, ICharacteristicValue Value) result = targets.FirstOrDefault(x => x.Proto == item.Prototype.Id);
                if (result.Proto == 0)
                {
                    continue;
                }
    ...
    public override bool IsValid()
    {
        ...
        foreach ((ICharacteristicDefinition definition, int index) in VariableIndexes)
        {
            ICraftInput ivi = Craft.Inputs.ElementAtOrDefault(index - 1);
            if (ivi is null)
            {
                return false;
            }
    ...
    private bool BuildingCommmandVariableDefault(ICharacter actor)
    {
        ...
        foreach ((ICharacteristicDefinition variable, int index) in VariableIndexes)
        {
            ICraftInput input = Craft.Inputs.ElementAtOrDefault(index - 1);
            sb.AppendLine();
            ...
            sb.AppendLine();
            foreach ((long proto, ICharacteristicValue value) in VariableSpecifics[variable])
            {
                IGameItemProto iProto = Gameworld.ItemProtos.Get(proto);
                if (iProto is null)
                {
    ...
    private bool BuildingCommandVariableItem(ICharacter actor, StringStack command, ICharacteristicDefinition definition)
    {
        ...
        IGameItemProto item = Gameworld.ItemProtos.GetByIdOrName(command.PopSpeech());
        if (item is null)
        {
        ...
        ICharacteristicValue value = Gameworld.CharacteristicValues.Where(x => definition.IsValue(x)).GetByIdOrName(command.SafeRemainingArgument);
        if (value is null)
        {
