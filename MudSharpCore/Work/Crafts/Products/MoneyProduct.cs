...
public class MoneyProduct : BaseProduct
{
    ...
    {
        IGameItem newitem = GameItems.Prototypes.CurrencyGameItemComponentProto.CreateNewCurrencyPile(Currency,
            Currency.FindCoinsForAmount(Amount, out bool _).Select(x => Tuple.Create<ICoin, int>(x.Key, x.Value)));
    ...
    {
        ICurrencyPile currencyPile = item.GetItemType<ICurrencyPile>();
    ...
    private bool BuildingCommandAmount(ICharacter actor, StringStack command)
    {
        ...
        decimal amount = Currency.GetBaseCurrency(command.SafeRemainingArgument, out bool success);
        if (!success)
        {
