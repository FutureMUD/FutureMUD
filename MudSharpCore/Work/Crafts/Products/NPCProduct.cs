...
internal class NPCProduct : BaseProduct
{
    ...
    {
        List<SimpleCharacterTemplate> templates = new();
    ...
    private bool BuildingCommandProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = new ProgLookupFromBuilderInput(Gameworld, actor, command.SafeRemainingArgument, ProgVariableTypes.Void, [ProgVariableTypes.Character]).LookupProg();
        if (prog is null)
        {
            return false;
        }
