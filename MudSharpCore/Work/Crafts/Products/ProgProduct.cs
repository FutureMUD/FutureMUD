...
public class ProgProduct : BaseProduct
{
    ...
    public override ICraftProductData ProduceProduct(IActiveCraftGameItemComponent component, ItemQuality referenceQuality)
    {
        ...
        List<ICraftInput> inputList = Craft.Inputs.ToList();
        foreach (ICraftInput? input in Craft.Inputs)
        {
            int inputNumber = inputList.IndexOf(input) + 1;
            if (!component.ConsumedInputs.ContainsKey(input))
            {
                continue;
            }

            ICraftInputData data = component.ConsumedInputs[input].Data;
    ...
    private bool BuildingCommandProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg? prog = new ProgLookupFromBuilderInputMultipleReturnTypes(actor, command.SafeRemainingArgument, [ProgVariableTypes.Item, ProgVariableTypes.Collection | ProgVariableTypes.Item], [
