...
public class ScrapInputProduct : BaseProduct
{
        ...
        ItemQuality referenceQuality)
    {
        ...
        {
            List<IGameItem> items = ((PerceivableGroup)input.Value.Data.Perceivable).Members.OfType<IGameItem>().ToList();
            material = items.First().Material;
            totalWeight = items.Sum(x => x.Weight);
        }
        else
        {
            IGameItem item = (IGameItem)input.Value.Data.Perceivable;
    ...
    {
        if (command.IsFinished || !command.SafeRemainingArgument.TryParsePercentage(out double perc))
        {
            actor.OutputHandler.Send(
