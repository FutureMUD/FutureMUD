...
public class ForagableProfile : EditableItem, IForagableProfile
{
    ...
    public override IEditableRevisableItem CreateNewRevision(ICharacter initiator)
    {
        using (new FMDB())
        {
            Models.ForagableProfile dbnew = new()
            {
                Id = Id,
            ...
            foreach (IForagable item in Foragables)
            {
            ...
            foreach (KeyValuePair<string, double> item in MaximumYieldPoints)
            {
            ...
            foreach (KeyValuePair<string, double> item in HourlyYieldPoints)
            {
    ...
    public override void Save()
    {
        using (new FMDB())
        {
            Models.ForagableProfile dbitem = FMDB.Context.ForagableProfiles.Find(Id, RevisionNumber);
            if (_statusChanged)
            {
    ...
    private void LoadFromDb(MudSharp.Models.ForagableProfile profile)
    {
        _id = profile.Id;
        _name = profile.Name;
        foreach (ForagableProfilesForagables item in profile.ForagableProfilesForagables)
        {
    ...
    public ForagableProfile(IAccount originator) : base(originator)
    {
        ...
        using (new FMDB())
        {
            Models.ForagableProfile dbitem = new()
            {
    ...
    private bool BuildingCommandYield(ICharacter actor, StringStack command)
    {
        ...
        if (!double.TryParse(command.PopSpeech(), out double hourlyGain))
        {
