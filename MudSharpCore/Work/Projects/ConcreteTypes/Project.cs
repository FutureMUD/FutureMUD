...
public abstract class Project : EditableItem, IProject
{
    protected Project(MudSharp.Models.Project project, IFuturemud gameworld) : base(project.EditableItem)
    {
        ...
        foreach (Models.ProjectPhase phase in project.ProjectPhases)
        {
        ...
        AppearInProjectListProg =
            long.TryParse(
                ...
                throw new ApplicationException(
                    $"Project definition {Id} did not have an AppearInProjectListProg element."), out long value)
...
Editing Actions:
	...
	#3<phase> material ...#0 - see the material sub-command for full help".SubstituteANSIColour());
        ...
        IProjectPhase phase = TargetPhase(command.Last);
        if (phase == null)
        {
	...
	#3<action> ...#0 - see sub-command for further help".SubstituteANSIColour());
        ...
        IProjectAction action = phase.CompletionActions.FirstOrDefault(x => x.Name.EqualTo(command.Last)) ??
    ...
    private bool BuildingCommandPhaseActionDuplicate(ICharacter actor, IProjectPhase phase, StringStack command)
    {
        ...
        IProjectAction action = phase.CompletionActions.FirstOrDefault(x => x.Name.EqualTo(text)) ??
                     phase.CompletionActions.FirstOrDefault(x =>
        ...
        IProjectAction newAction = action.Duplicate(phase);
    ...
    private bool BuildingCommandPhaseActionCreate(ICharacter actor, IProjectPhase phase, StringStack command)
    {
        ...
        IProjectAction action = ProjectFactory.CreateAction(phase, Gameworld, command.PopSpeech());
        if (action == null)
        {
            actor.OutputHandler.Send(
	...
	#3<material> ...#0 - see sub-command for further help".SubstituteANSIColour());
        ...
        IProjectMaterialRequirement material = phase.MaterialRequirements.FirstOrDefault(x => x.Name.EqualTo(command.Last)) ??
    ...
    private bool BuildingCommandPhaseMaterialDuplicate(ICharacter actor, IProjectPhase phase, StringStack command)
    {
        ...
        string text = command.PopSpeech();
        IProjectMaterialRequirement material = phase.MaterialRequirements.FirstOrDefault(x => x.Name.EqualTo(text)) ??
                       phase.MaterialRequirements.FirstOrDefault(x =>
                           x.Name.StartsWith(text, StringComparison.InvariantCultureIgnoreCase));
        if (material == null)
        {
        ...
        IProjectMaterialRequirement newMaterial = material.Duplicate(phase);
    ...
    private bool BuildingCommandPhaseMaterialCreate(ICharacter actor, IProjectPhase phase, StringStack command)
    {
        ...
        IProjectMaterialRequirement material = ProjectFactory.CreateMaterial(phase, Gameworld, command.PopSpeech());
        if (material == null)
        {
            actor.OutputHandler.Send(
	...
	#3<labour> impact ...#0 - see the impact sub-command for more specific help".SubstituteANSIColour());
        ...
        IProjectLabourRequirement labour = phase.LabourRequirements.FirstOrDefault(x => x.Name.EqualTo(command.Last)) ??
    ...
    private bool BuildingCommandPhaseLabourDuplicate(ICharacter actor, IProjectPhase phase, StringStack command)
    {
        ...
        string text = command.PopSpeech();
        IProjectLabourRequirement labour = phase.LabourRequirements.FirstOrDefault(x => x.Name.EqualTo(text)) ??
                     phase.LabourRequirements.FirstOrDefault(x =>
                         x.Name.StartsWith(text, StringComparison.InvariantCultureIgnoreCase));
        if (labour == null)
        {
        ...
        IProjectLabourRequirement newLabour = labour.Duplicate(phase);
    ...
    private bool BuildingCommandPhaseLabourCreate(ICharacter actor, IProjectPhase phase, StringStack command)
    {
        ...
        string labourType = command.PopSpeech();
        string newName = "Work";
        if (!command.IsFinished)
        {
        ...
        IProjectLabourRequirement labour = ProjectFactory.CreateLabour(phase, Gameworld, labourType, newName);
        if (labour == null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandPhaseDuplicate(ICharacter actor, StringStack command)
    {
        ...
        IProjectPhase phase = TargetPhase(command.PopSpeech());
        if (phase == null)
        {
            actor.OutputHandler.Send("There is no such phase.");
            return false;
        ...
        ProjectPhase newPhase = new(this, phase);
    ...
    private bool BuildingCommandPhaseSwap(ICharacter actor, StringStack command)
    {
        ...
        IProjectPhase otherPhase = TargetPhase(command.PopSpeech());
        if (otherPhase == null)
        {
            actor.OutputHandler.Send("There is no such phase.");
            return false;
    ...
    {
        ProjectPhase newPhase = new(Gameworld, this);
        _phases.Add(newPhase);
        actor.OutputHandler.Send(
    ...
    public override IEditableRevisableItem CreateNewRevision(ICharacter initiator)
    {
        using (new FMDB())
        {
            Models.Project dbold = FMDB.Context.Projects.Find(Id, RevisionNumber);
            Models.Project dbnew = new();
            ...
            Project newProject = (Project)ProjectFactory.LoadProject(dbnew, Gameworld);
            foreach (IProjectPhase phase in Phases)
            {
    ...
    {
        Models.Project dbitem = FMDB.Context.Projects.Find(Id, RevisionNumber);
    ...
    public override string WhyCannotSubmit()
    {
        ...
        (IProjectPhase Phase, (bool Truth, string Error) Result) phase = _phases.Select(x => (Phase: x, Result: x.CanSubmit())).FirstOrDefault(x => !x.Result.Truth);
        if (phase.Phase != null)
        {
