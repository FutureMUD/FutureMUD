...
public class ProjectPhase : SaveableItem, IProjectPhase
{
    public ProjectPhase(MudSharp.Models.ProjectPhase phase, IFuturemud gameworld)
    {
        ...
        foreach (Models.ProjectMaterialRequirement material in phase.ProjectMaterialRequirements)
        {
            _materialRequirements.Add(ProjectFactory.LoadMaterial(material, gameworld));
        }

        foreach (Models.ProjectAction action in phase.ProjectActions)
        {
    ...
    public ProjectPhase(IFuturemud gameworld, IProject project)
    {
        ...
        using (new FMDB())
        {
            Models.ProjectPhase dbitem = new();
    ...
    public ProjectPhase(IProject project, IProjectPhase rhs)
    {
        ...
        foreach (IProjectLabourRequirement labour in rhs.LabourRequirements)
        {
            _labourRequirements.Add(labour.Duplicate(this));
        }

        foreach (IProjectMaterialRequirement material in rhs.MaterialRequirements)
        {
            _materialRequirements.Add(material.Duplicate(this));
        }

        foreach (IProjectAction action in rhs.CompletionActions)
        {
    ...
    {
        Models.ProjectPhase dbitem = FMDB.Context.ProjectPhases.Find(Id);
        dbitem.Description = Description;
        dbitem.PhaseNumber = PhaseNumber;
        Changed = false;
    }
    ...
    public (bool Truth, string Error) CanSubmit()
    {
        ...
        (IProjectLabourRequirement Labour, (bool Truth, string Error) Result) failedLabour = _labourRequirements.Select(x => (Labour: x, Result: x.CanSubmit()))
        ...
        (IProjectMaterialRequirement Material, (bool Truth, string Error) Result) failedMaterial = _materialRequirements.Select(x => (Material: x, Result: x.CanSubmit()))
                                                  .FirstOrDefault(x => !x.Result.Truth);
        ...
        (IProjectAction Action, (bool Truth, string Error) Result) failedAction = _completionActions.Select(x => (Action: x, Result: x.CanSubmit()))
                                             .FirstOrDefault(x => !x.Result.Truth);
