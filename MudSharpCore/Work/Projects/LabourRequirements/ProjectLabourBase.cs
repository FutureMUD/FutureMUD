...
public abstract class ProjectLabourBase : SaveableItem, IProjectLabourRequirement
{
    ...
    protected ProjectLabourBase(Models.ProjectLabourRequirement labour, IFuturemud gameworld)
    {
        ...
        foreach (ProjectLabourImpact impact in labour.ProjectLabourImpacts)
        {
    ...
    protected ProjectLabourBase(IFuturemud gameworld, IProjectPhase phase, string type, string name)
    {
        ...
        using (new FMDB())
        {
            ProjectLabourRequirement dbitem = new();
    ...
    protected ProjectLabourBase(ProjectLabourBase rhs, IProjectPhase newPhase, string type)
    {
        ...
        foreach (ILabourImpact impact in rhs.LabourImpacts)
        {
    ...
    {
        ProjectLabourRequirement dbitem = FMDB.Context.ProjectLabourRequirements.Find(Id);
        dbitem.Name = Name;
        dbitem.Description = Description;
        dbitem.TotalProgressRequired = TotalProgressRequired;
        dbitem.MaximumSimultaneousWorkers = MaximumSimultaneousWorkers;
    ...
    private bool BuildingCommandQualifiedProg(ICharacter actor, StringStack command)
    {
        ...
        IFutureProg prog = long.TryParse(text, out long value)
    ...
    private bool BuildingCommandTrait(ICharacter actor, StringStack command)
    {
        ...
        ITraitDefinition trait = long.TryParse(text, out long value) ? Gameworld.Traits.Get(value) : Gameworld.Traits.GetByName(text);
        if (trait == null)
        {
	...
	#3<impact> ...#0 - see individual impacts for detailed help".SubstituteANSIColour());
        ...
        ILabourImpact impact = LabourImpacts.FirstOrDefault(x => x.Name.EqualTo(command.Last)) ??
    ...
    private bool BuildingCommandImpactAdd(ICharacter actor, StringStack command, IProjectPhase phase)
    {
        ...
        string impactType = command.PopSpeech();
        string newName = command.IsFinished ? "Impact" : command.PopSpeech();
        ILabourImpact impact = ProjectFactory.CreateImpact(this, Gameworld, impactType, newName);
        if (impact == null)
        {
            actor.OutputHandler.Send(
    ...
    private bool BuildingCommandImpactDelete(ICharacter actor, StringStack command)
    {
        ...
        string text = command.PopSpeech();
        ILabourImpact impact = LabourImpacts.FirstOrDefault(x => x.Name.EqualTo(text)) ??
                     LabourImpacts.FirstOrDefault(x =>
    ...
    private bool BuildingCommandImpactCopy(ICharacter actor, StringStack command)
    {
        ...
        ILabourImpact newImpact = impact.Duplicate(this);
    ...
    {
        ICheck check = Gameworld.GetCheck(CheckType.ProjectLabourCheck);
        if (!previewOnly)
        {
    ...
    public virtual (bool Truth, string Error) CanSubmit()
    {
        ...
        (ILabourImpact Impact, (bool Truth, string Error) Result) failedImpact = _labourImpacts.Select(x => (Impact: x, Result: x.CanSubmit()))
