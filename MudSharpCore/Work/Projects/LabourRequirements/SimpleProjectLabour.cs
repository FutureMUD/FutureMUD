...
public class SimpleProjectLabour : ProjectLabourBase
{
    ...
    {
        double totalRemaining = TotalProgressRequired - project.LabourProgress[this];
        List<(ICharacter Character, IProjectLabourRequirement Labour)> active = project.ActiveLabour.Where(x => x.Labour == this).ToList();
        if (!active.Any())
        {
