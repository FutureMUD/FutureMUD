...
public sealed class DatabaseUpgradeCoordinator : IDatabaseUpgradeCoordinator
{
    ...
    public DatabaseUpgradePreparation PrepareForStartup(DatabaseUpgradeRequest request)
    {
        ...
        DatabaseBackupSettings settings = DatabaseBackupSettings.Load(request.WorkingDirectory);
        string stateFilePath = DatabaseUpgradeState.GetStateFilePath(request.WorkingDirectory);
        string databaseName = new MySqlConnectionStringBuilder(request.ConnectionString).Database;
        bool restoredPreviousFailedUpgrade = false;

        if (File.Exists(stateFilePath))
        {
            DatabaseUpgradeState? state = LoadState(stateFilePath);
        ...
        IReadOnlyList<string> pendingMigrations = _migrationService.GetPendingMigrations(request.ConnectionString);
        DatabaseUpgradePreparation preparation = new()
        {
            Request = request,
            Settings = settings,
        ...
        DatabaseUpgradeState upgradeState = new()
        {
            DatabaseName = databaseName,
        ...
        Action<DatabaseMigrationProgress>? progressAction = null)
    {
        ...
        DatabaseUpgradeState state = LoadState(preparation.StateFilePath) ?? new DatabaseUpgradeState
        {
    ...
    public void CreateBlankDatabaseSnapshot(DatabaseUpgradeRequest request, string snapshotPath, string databaseNamePlaceholder)
    {
        ...
        string tempDirectory = Path.Combine(Path.GetTempPath(), "FutureMUD-SnapshotRefresh", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDirectory);
        try
        {
            string exportedPath = _backupService.CreateBackup(request.ConnectionString, tempDirectory);
            string databaseName = new MySqlConnectionStringBuilder(request.ConnectionString).Database;
    ...
    {
        DatabaseBackupSettings settings = DatabaseBackupSettings.Load(request.WorkingDirectory);
        string backupDirectory = settings.ResolveBackupDirectory(request.WorkingDirectory);
        string path = _backupService.CreateBackup(request.ConnectionString, backupDirectory);
    ...
    private static void PruneBackups(string backupDirectory, int retainedBackupCount)
    {
        ...
        List<FileInfo> files = new DirectoryInfo(backupDirectory)
            .GetFiles("*.sql")
