...
internal class FutureMUDItemTransformer
{
    ...
    {
        foreach (RPIMaterial material in Enum.GetValues<RPIMaterial>())
        {
        ...
        foreach (RPISkill skill in Enum.GetValues<RPISkill>())
        {
            switch (skill)
            {
        ...
        foreach (GameItemComponentProto component in context.GameItemComponentProtos.Include(x => x.EditableItem))
        {
    ...
    public MudSharp.Models.GameItemProto ConvertItem(RPIItem item)
    {
        ...
        // Handle Wear Flags
        IEnumerable<RPIWearBits> wearFlags = item.WearBits.GetAllFlags();
        foreach (RPIWearBits flag in wearFlags)
        {
            switch (flag)
            {
