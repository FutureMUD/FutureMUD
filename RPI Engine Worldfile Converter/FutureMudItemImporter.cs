...
using GameItemQuality = MudSharp.GameItems.ItemQuality;
...
public static class FutureMudItemValidation
{
		...
		IEnumerable<ConvertedItemDefinition> definitions)
	{
		...
		{
			if (definition.Status == ConversionStatus.SkippedImport)
			{
				continue;
			}
			...
			if (definition.PreparedFoodDefinition is not null)
			{
				generatedComponentNames.Add(definition.PreparedFoodDefinition.ComponentName);
			}
...
public sealed class FutureMudItemImporter
{
	...
	public FutureMudImportResult Apply(IEnumerable<ConvertedItemDefinition> definitions, bool execute)
	{
		var ordered = definitions
			...
			.ThenBy(x => x.Vnum)
			.ToList();
		var importable = ordered
			.Where(x => x.Status != ConversionStatus.SkippedImport)
			.ToList();
		var skippedByDesignCount = ordered.Count - importable.Count;
		...
		{
			return new FutureMudImportResult(0, 0, skippedByDesignCount, issues);
		}

		var existingMarkers = LoadExistingMarkers();
		var skippedExistingCount = importable.Count(x => existingMarkers.Contains(CreateProvenanceMarker(x)));
		if (!execute)
		{
			return new FutureMudImportResult(0, skippedExistingCount, skippedByDesignCount, issues);
		}

		var insertedCount = 0;

		foreach (var definition in importable)
		{
		...
		_context.SaveChanges();
		return new FutureMudImportResult(insertedCount, skippedExistingCount, skippedByDesignCount, issues);
	}
	...
	private void EnsureGeneratedDependencies(ConvertedItemDefinition definition)
	{
		...
		if (definition.PreparedFoodDefinition is not null)
		{
			EnsurePreparedFoodComponent(definition.PreparedFoodDefinition);
		}
	}
	...
	private void EnsurePreparedFoodComponent(FutureMudPreparedFoodDefinition preparedFoodDefinition)
	{
		if (_catalog.HasComponent(preparedFoodDefinition.ComponentName))
		{
			return;
		}
		...
		var existingComponent = _context.GameItemComponentProtos
			...
			.AsEnumerable()
			.FirstOrDefault(x =>
				x.Name.Equals(preparedFoodDefinition.ComponentName, StringComparison.OrdinalIgnoreCase) &&
				x.Type.Equals("PreparedFood", StringComparison.OrdinalIgnoreCase) &&
				x.EditableItem is not null &&
				x.EditableItem.RevisionStatus == 4);
		if (existingComponent is not null)
		{
			_catalog.RegisterComponent(new FutureMudComponentReference(
		...
		var component = new GameItemComponentProto
		{
			...
			RevisionNumber = 0,
			Type = "PreparedFood",
			Name = preparedFoodDefinition.ComponentName,
			Description = preparedFoodDefinition.Description,
			Definition = BuildPreparedFoodComponentDefinition(preparedFoodDefinition),
			EditableItem = new EditableItem
			{
				RevisionNumber = 0,
				RevisionStatus = 4,
				BuilderAccountId = _builderAccountId,
				BuilderDate = _now,
				BuilderComment = $"RPIIMPORT|generated-prepared-food-component|{preparedFoodDefinition.ComponentName}",
				ReviewerAccountId = _builderAccountId,
				ReviewerDate = _now,
				ReviewerComment = "Generated by the RPI Engine Worldfile Converter."
			}
	...
	private static string BuildPreparedFoodComponentDefinition(FutureMudPreparedFoodDefinition definition)
	{
		return new XElement("Definition",
			new XAttribute("ServingScope", "WholeItem"),
			new XAttribute("Satiation", FormatDouble(definition.Satiation)),
			new XAttribute("Water", FormatDouble(definition.Water)),
			new XAttribute("Thirst", FormatDouble(definition.Thirst)),
			new XAttribute("Alcohol", FormatDouble(definition.Alcohol)),
			new XAttribute("Bites", FormatDouble(definition.Bites)),
			new XAttribute("QualityScale", FormatDouble(0.08)),
			new XAttribute("StaleMultiplier", FormatDouble(0.25)),
			new XAttribute("SpoiledMultiplier", FormatDouble(0.0)),
			new XAttribute("LiquidAbsorption", FormatDouble(0.0)),
			new XAttribute("StaleAfterSeconds", FormatDouble(0.0)),
			new XAttribute("SpoilAfterSeconds", FormatDouble(0.0)),
			new XAttribute("Decorator", 0),
			new XElement("Taste", new XCData(definition.TasteTemplate)),
			new XElement("Short", new XCData(definition.ShortDescriptionTemplate)),
			new XElement("Full", new XCData(definition.FullDescriptionTemplate)),
			new XElement("OnEatProg", 0),
			new XElement("OnStaleProg", 0),
			new XElement("Ingredients",
				new XElement("Ingredient",
					new XAttribute("role", definition.IngredientRole),
					new XAttribute("source", 0),
					new XAttribute("material", 0),
					new XAttribute("liquid", 0),
					new XAttribute("weight", 0),
					new XAttribute("volume", 0),
					new XAttribute("quality", (int)GameItemQuality.Standard),
					new XElement("Description", new XCData(definition.IngredientDescription)),
					new XElement("Taste", new XCData(definition.IngredientTaste))
				)
			),
	...
	{
		return value.ToString("R", SystemCultureInfo.InvariantCulture);
	}
	...
	private static string BuildBuilderComment(ConvertedItemDefinition definition)
	{
		...
		[
			...
			$"PreparedFood={definition.PreparedFoodDefinition?.ComponentName ?? "none"}",
