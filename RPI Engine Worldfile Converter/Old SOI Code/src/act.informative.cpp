...
char *verbal_hunger[] =
  { "starving", "famished", "hungry", "feeling slightly hungry", "feeling peckish",
  "full", "quite full", "satiated", "absolutely stuffed"
  };
  ...
  if (mode == 7 && obj->description)
      ...
      else
	{
	  ...
	  if (GET_ITEM_TYPE (obj) == ITEM_WEAPON && mode == 15)
	    {
	      /* With weapon skill changes, this now just looks silly.

                sprintf (buffer + strlen (buffer),
		       "\n  t #6You'd guess it to be a %s weapon.#0\n",
...
do_look (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  bool change = false, again = true, abrt = false, found = false;
...
/** Arena **/
  if (strcasecmp (arg1, "battlefield") == STR_MATCH && (ch->in_room == 60005))
    {
	   ...
	   || IS_SET (ch->room->room_flags, INDOORS)) || !IS_MORTAL (ch))
	{
	  ...
	  for (dir = 0; dir <= LAST_DIR; dir++)
		...
		sprintf (buf + strlen (buf), "#4%s#0 ", dirs[dir]);
                     ...
                     || ch->room->sector_type == SECT_TRAIL) 
                     && (troom->sector_type == SECT_ROAD 
                     || troom->sector_type == SECT_TRAIL))
...
read_virtual_message (CHAR_DATA * ch, char *name, char *argument)
{
  ...
  *name = toupper (*name);
  ...
  sprintf (b_buf, "#6Date:#0     %s\n"
	   "#6Author:#0   %s\n"
	   "#6Subject:#0  %s\n"
	   "#6Context:#0  http://www.lost-tales.org/staffportal.php?context=%ld&db=all##%ld\n\n%s",
...
hunger_thirst_process (CHAR_DATA * ch)
{
  ...
  *update_buf = '\0';
  ...
  // Bright weed makes you become thirstier and hungrier at a much quicker rate.

  if((af = get_soma_affect(ch, SOMA_PLANT_ALERTNESS)))
  {
    if (ch->hunger > 2)
      ch->hunger -= 2;
    if (ch->thirst > 2)
...
do_score (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if (ch->desc && ch->desc->acct && ch->desc->acct->get_last_nominate () > 0)
    {
      std::string date = ch->desc->acct->get_nominate_rpp_date_string ();
...
#define skill_lev2(val) val >= 90 ? " #6(Legend)#0 " : val >= 80 ? " #6(Heroic)#0 " : val >= 70 ? " #6(Master)#0 " : val >= 60 ? " #6(Adroit)#0 " : val >= 50 ? "#6(Talented)#0" : val >= 35 ? "#6(Familiar)#0" : val >= 20 ? " #6(Novice)#0 " : "#6(Beginner)#0"
  ...
  if (IS_MORTAL (ch))
    {
      ...
      sprintf (buf, "\n        #2Prowess:#0 %s  ", skill_lev (who->offense));
      ...
      for (i = 1; i <= LAST_SKILL; i++)
	{
	    ...
	    continue;
          ...
          af = get_affect (ch, MAGIC_FLAG_FOCUS + i);

          if (!af)
          sprintf (buf2, "#2%15.15s:#0 %s  ", skills[i], skill_lev (who->skills[i]));
          else
          sprintf (buf2, "#2%15.15s:#0%s ", skills[i], skill_lev2 (who->skills[i]));

          while (strlen (buf2) < 26)
	      strcat (buf2, " ");
...
time_string (CHAR_DATA * ch)
{
  ...
  weekday = (int)time_info.month * 30;

  day = time_info.day + 1;

  weekday += day;

  switch((int)time_info.month + 1)
  {
    case 3:
      month = 0;
      break;
  ...
  if (time_info.holiday > 0)
    sprintf (buf + strlen (buf), "on %s,", holiday_names[time_info.holiday]);
  else if (month == 1 && day == 30)
    sprintf (buf + strlen (buf), "on Feanoraur, The Birthdate of Feanor,");
  else if (month == 0 && day == 5)
    sprintf (buf + strlen (buf), "on Dagor Eriuaur, the Day of the First Battle,");
  else if (month == 4 && day == 72)
    sprintf (buf + strlen (buf), "on Awarthaur, the Day of Penance,");
  else if (month == 1 && day == 2)
    sprintf (buf + strlen (buf), "on Finrodaur, the Day of Arrival,");
  else
    sprintf (buf + strlen (buf), "on the %s, the %s day of the %s", 
    day_name[weekday%6], day_buf,  season_name[month]);

  sprintf (buf + strlen (buf),
	   " in the year %d of Noldor Reckoning.\n", time_info.year);
...
{
  int sunrise[] = { 6,  6,  6,  6,  5,  5,  4,  4,  5,  6,  6,  6 };
  ...
  if (!IS_MORTAL (ch) && GET_TRUST (ch) > 4 && *buf)
      ...
      else if ((ind = index_lookup (fog_states, buf)) != -1)
	{
      ...
      else if(!str_cmp(buf, "moonlight"))
      {
        if(global_moon_light)
        {
          global_moon_light = 0;
	  moon_light[10] = global_moon_light;
	  moon_light[11] = global_moon_light;
	  moon_light[12] = global_moon_light;
          send_to_char("You've disabled the moonlight.\n", ch);
        }
        else
        {
          global_moon_light = 1;
	  ...
	  moon_light[11] = global_moon_light;
	  moon_light[12] = global_moon_light;
          send_to_char("You've enabled the moonlight.\n", ch);
        }
        return;
      }
  ...
  if (IS_OUTSIDE (ch))
	...
	sprintf (w_phrase, "night");
      ...
      /*
      if (time_info.month == 2 || time_info.month == 3)
	sprintf (buf2, "spring");
      if (time_info.month >= 4 && time_info.month <= 7)
	sprintf (buf2, "summer");
      if (time_info.month == 8 || time_info.month == 9)
	sprintf (buf2, "autumn");
      if (time_info.month <= 1 && time_info.month >= 10)
	sprintf (buf2, "winter");*/

      sprintf (buf2, "%s", season_string[time_info.season]);
  ...
  if (!*arg1)
      ...
      else
	{
      ...
      sprintf (b_buf, 
	       ...
	       //" #6http://www.lost-tales.org/help.php#0.\n",
	       MUD_NAME, 
	       help_available, 
...
do_who (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  *buf = '\0';
  ...
  if (!mortals)
    sprintf (s_buf,
	     "\nThere currently aren't any players in Beleriand.\n");
  else if (mortals == 1)
    sprintf (s_buf, "\nThere is currently #21#0 player in Beleriand.\n");
  else
    sprintf (s_buf, "\nThere are currently #2%d#0 players in Beleriand.\n",
	     mortals);

  if (guests)
    sprintf (s_buf + strlen (s_buf),
	     "There %s currently #2%d#0 guest%s suffering the noxious clime of Angband.\n",
...
/* cmd = 1 for scan without echo */
  ...
  directional_scan(ch, dir);
}

void area_scan(CHAR_DATA * ch)
{
  int dir = 0;
  ROOM_DIRECTION_DATA *exit = NULL;
  ROOM_DATA *next_room = NULL;
  ...
  for (dir = 0; dir <= LAST_DIR;dir++)
  {
	...
	{
	  bSaw = true;
        sprintf (buf, "%s, #6%s#0, you see nothing.\n", dirs[dir], 
	  ...
	  *buf = toupper (*buf);
      ...
      for (tobj = next_room->contents; tobj; tobj = tobj->next_content)
      {
        ...
        // Don't show objects weight weight 0.01 lbs - for -very- small things.
        if (tobj->obj_flags.weight <= 1)
	  continue;
...
void
directional_scan (CHAR_DATA * ch, int dir)
{
  int blank = 0;
  int seen = 0;
  ...
  // Check for people in the first room.
  ...
  for (tch = next_room->people; tch; tch = tch->next_in_room)
    {
      ...
      *buf2 = '\0';
      ...
      // Chance to spot hiding people in the first room.

      penalty -= (skill_level (tch, SKILL_HIDE, 0));

      if(get_affect(tch, MAGIC_HIDDEN) && (penalty > number(1, 100)
         || CAN_SEE(ch, tch)))
        sprintf(buf2, " #1(hidden)#0");
      else if (!could_see (ch, tch))
	continue;
  ...
  for (tobj = next_room->contents; tobj; tobj = tobj->next_content)
	...
	continue;
      ...
      // Don't show objects weight weight 0.01 lbs.
  ...
  for (tch = next_room->people; tch; tch = tch->next_in_room)
	...
	continue;
      ...
      // Can't see critters weighing less than 10 lbs from this distance.
      if (get_weight (tch) / 100 <= 10)
	continue;
  ...
  for (tobj = next_room->contents; tobj; tobj = tobj->next_content)
	...
	continue;
      ...
      // Don't show objects that weight 10lbs or less from 2 rooms away.
      if (tobj->obj_flags.weight <= 1000)
	continue;
  ...
  for (tch = next_room->people; tch; tch = tch->next_in_room)
	...
	continue;
      ...
      // Can't see critters weighing less than 25 lbs from this distance.
      if (get_weight (tch) / 100 <= 25)
	continue;
  ...
  for (tobj = next_room->contents; tobj; tobj = tobj->next_content)
	...
	continue;
      ...
      // Don't show objects that weight 25lbs of less from 3 rooms away.
      if (tobj->obj_flags.weight <= 2500)
	continue;
...
{
  if (coin->nVirtual == 995)
    {
      if (coin->count > 1)
	return "faceted, ridged gold coins";
      else
	return "faceted, ridged gold coin";
    }
  else if (coin->nVirtual == 994)
    {
      if (coin->count > 1)
	return "eight-sided gold coins";
      else
	return "eight-sided gold coin";
    }
  else if (coin->nVirtual == 993)
    {
      if (coin->count > 1)
	return "hexagonal silver coins";
      else
	return "hexagonal silver coin";
    }
  else if (coin->nVirtual == 992)
    {
      if (coin->count > 1)
	return "small, squared silver coins";
      else
	return "small, squared silver coin";
    }
  else if (coin->nVirtual == 991)
    {
      if (coin->count > 1)
	return "weighty copper coins";
      else
	return "weighty copper coin";
    }
  else if (coin->nVirtual == 990)
    {
      if (coin->count > 1)
	return "tiny copper coins";
      else
	return "tiny copper coin";
    }
  else
    return "coin";
}
	     ...
	     char *date, char *subject, char *info, char *message, long flags)
{
  ...
  *date_buf = '\0';
  ...
  day = time_info.day + 1;
  xday = day;
  month = elf_season(&xday, time_info.month);

  if (time_info.holiday > 0)
    sprintf (date_buf, "%s, %d NR", holiday_short_names[time_info.holiday], time_info.year);
  else
    sprintf (date_buf, "%s, %d NR", short_time_string(day, time_info.month), time_info.year);
...
/*** ECON FLAGS ***/
    ...
    if(obj->obj_flags.type_flag == ITEM_FOOD && GET_MATERIAL_TYPE(obj) == 1 << 9)
    {
      ...
      if(ch->skills[SKILL_HERBALISM] >= obj->o.od.value[4] 
	...
	switch(obj->o.od.value[3])
	{
      ...
      else if(ch->skills[SKILL_HERBALISM] >= obj->o.od.value[4] / 2 
...
/**** Guess at the Weight based on scan skill **/
	...
	if (IS_SET (obj->obj_flags.wear_flags, ITEM_TAKE)){
		...
		*/
		...
		// Dumb. "I can see real good at a distance, so I can estimate weight!"
