...
const int terrain_penalty[21][2] = {
  {20, 20}, // inside
  {20, 20}, // city
  {20, 20}, // road
  {0, 10}, // trail
...
bool
small_animal(CHAR_DATA *ch)
{
 if(ch->race == 34 || // rabbit
    ch->race == 25 || // bird
    ch->race == 39 || // small snake
    ch->race == 38 || // cat
    ch->race == 43 || // rodent
    ch->race == 44 || // insect
    ch->race == 27 || // small spider
    ch->race == 49 || // beaver
    ch->race == 48 || // otter
    ch->race == 21 || // fox
    ch->race == 46 || // marmot
...
ride_mount (CHAR_DATA *mount, CHAR_DATA *rider, int current, int target, int dir)
{
  ...
  rskill = MIN(rider->skills[SKILL_RIDE] + 55 - mod, 99);
  mskill = MIN(mount->skills[SKILL_RIDE] + 55 - mod, 99);
  ...
  { 
    ...
    else if(mod < 45)
    {
      ...
      GET_POS (mount) = SIT;
      add_second_affect (SA_STAND, ((25-GET_AGI(mount))+number(1,3)), mount, NULL, NULL, 0);
      wound_to_char (mount, figure_location (mount, 0), dice (1, 8), 3, 0, 0, 0);
      wound_to_char (mount, figure_location (mount, 0), dice (1, 8), 3, 0, 0, 0);
      wound_to_char (mount, figure_location (mount, 1), dice (1, 8), 3, 0, 0, 0);
      wound_to_char (mount, figure_location (mount, 1), dice (1, 8), 3, 0, 0, 0);
    }
    else
    {
      ...
      wound_to_char (mount, figure_location (mount, 0), dice (1, 12), 3, 0, 0, 0);
      wound_to_char (mount, figure_location (mount, 0), dice (1, 12), 3, 0, 0, 0);
      wound_to_char (mount, figure_location (mount, 3), dice (1, 12), 3, 0, 0, 0);
      wound_to_char (mount, figure_location (mount, 3), dice (1, 12), 3, 0, 0, 0);
    }
    ...
    else if(mod < 30)
    {
      ...
      GET_POS (rider) = SIT;
      add_second_affect (SA_STAND, ((25-GET_AGI(rider)) + number(1,3)), rider, NULL, NULL, 0);
      wound_to_char (rider, figure_location (rider, 0), dice (1, 8), 3, 0, 0, 0);
      wound_to_char (rider, figure_location (rider, 0), dice (1, 8), 3, 0, 0, 0);
      wound_to_char (rider, figure_location (rider, 1), dice (1, 8), 3, 0, 0, 0);
      wound_to_char (rider, figure_location (rider, 2), dice (1, 8), 3, 0, 0, 0);
      rider->mount = NULL;
      mount->mount = NULL;
    }
    else
    {
      ...
      GET_POS (rider) = SIT;
      add_second_affect (SA_STAND, ((25-GET_AGI(rider)) + number(1,3)), rider, NULL, NULL, 0);
      wound_to_char (rider, figure_location (rider, 0), dice (1, 12), 3, 0, 0, 0);
      wound_to_char (rider, figure_location (rider, 0), dice (1, 12), 3, 0, 0, 0);
      wound_to_char (rider, figure_location (rider, 3), dice (1, 12), 3, 0, 0, 0);
      wound_to_char (rider, figure_location (rider, 4), dice (1, 12), 3, 0, 0, 0);
      rider->mount = NULL;
      mount->mount = NULL;
    }
...
outside_inside (CHAR_DATA *ch)
{
  ...
  if (IS_OUTSIDE (ch))
	...
	{
	case 0:
	  sprintf (buf, "it is a dangerously searing %s %s, and ", buf2, w_phrase);
	  break;
	case 1:
	  sprintf (buf, "it is a painfully blazing %s %s, and ", buf2, w_phrase);
	  break;
	case 2:
	  sprintf (buf, "it is a blistering %s %s, and ", buf2, w_phrase);
	  break;
	case 3:
	  sprintf (buf, "it is a sweltering %s %s, and ", buf2, w_phrase);
	  break;
	case 4:
	  sprintf (buf, "it is a hot %s %s, and ", buf2, w_phrase);
	  break;
	case 5:
	  sprintf (buf, "it is a temperate %s %s, and ", buf2, w_phrase);
	  break;
	case 6:
	  sprintf (buf, "it is a cool %s %s, and ", buf2, w_phrase);
	  break;
	case 7:
	  sprintf (buf, "it is a chill %s %s, and ", buf2, w_phrase);
	  break;
	case 8:
	  sprintf (buf, "it is a cold %s %s, and ", buf2, w_phrase);
	  break;
	case 9:
	  sprintf (buf, "it is a very cold %s %s, and ", buf2, w_phrase);
	  break;
	case 10:
	  sprintf (buf, "it is a frigid %s %s, and ", buf2, w_phrase);
	  break;
	case 11:
	  sprintf (buf, "it is a freezing %s %s, and ", buf2, w_phrase);
	  break;
	case 12:
	  sprintf (buf, "it is a numbingly frigid %s %s, and ", buf2, w_phrase);
	  break;
	case 13:
	  sprintf (buf, "it is a painfully freezing %s %s, and ", buf2, w_phrase);
	  break;
	case 14:
	  sprintf (buf, "it is a dangerously freezing %s %s, and ", buf2, w_phrase);
	  break;
	default:
	  sprintf (buf, "it is a cool %s %s, and ", buf2, w_phrase);
	  break;
	  ...
	  && weather_info[ch->room->zone].fog < THICK_FOG)
	{
	  ...
	  switch (weather_info[ch->room->zone].wind_speed)
	    {
	    case CALM:
	      sprintf (buf, "%s hang motionless in the sky.", buf2);
	      break;
	    case BREEZE:
	      sprintf (buf, "%s waft overhead upon a %s breeze.", buf2,
		       wind);
	      break;
	    case WINDY:
	      sprintf (buf, "%s waft overhead upon the %s winds.", buf2,
		       wind);
	      break;
	    case GALE:
	      sprintf (buf, "%s rush overhead upon a %s gale.", buf2, wind);
	      break;
	    case STORMY:
	      sprintf (buf,
...
*/
	  ...
	  if((waf = get_affect (tch, AFFECT_WATCH_DIR)))
	      ...
	      if(rev_dir[dir] == waf->a.shadow.edge)
		{
		    ...
		    sprintf (buf, "To the %s, you see $n arrive stealthily from %s.\n",
			   direction[rev_dir[waf->a.shadow.edge]], rev_d[qe->dir]);
		    act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
                  }
                  else if(!IS_SET (qe->flags, MF_SNEAK))
                  {
			   ...
			   direction[rev_dir[waf->a.shadow.edge]], qe->group_str, rev_d[qe->dir]);
                      act(buf, true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
                      target_sighted (tch, ch);
                    }
  ...
  if (qe->event_time <= 0)
  {
    ...
    {
      sprintf (buf, "$n arives from %s, #5a %s#0 following behind.", rev_d[qe->dir], group_str);
	  ...
	  ;
        ...
        else if(get_second_affect(ch, SA_LEAD, NULL) || get_second_affect(ch->mount, SA_LEAD, NULL))
        {
          sprintf (buf, "$n %ss $N in from %s, #5a %s#0 following behind.", 
                   mount_speeds[ch->mount->pc ? ch->mount->pc->mount_speed :
		   ch->speed], rev_d[qe->dir], group_str);
	  ...
	  if (IS_SET (qe->flags, MF_SNEAK)
		   ...
		   tmp_ch = tmp_ch->next_in_room)
		{
		  if (tmp_ch != ch && CAN_SEE (ch, tmp_ch)
		      && !are_grouped (tmp_ch, ch) && !small_animal(tmp_ch))
		    {
		      observed = 1;
		      break;
		    }
	  ...
	  if (!IS_SET (qe->flags, MF_SNEAK))
	    {
		  ...
		  ;
                ...
                {
                  sprintf (buf, "$n is entering from %s%s, #5a %s#0 following behind.", rev_d[qe->dir], (drag_af ? ", dragging $N" : travel_str), group_str);
	      act (buf, true, ch, 0, drag_af ?
		   (CHAR_DATA *) drag_af->a.spell.t : 0, TO_NOTVICT | _ACT_FORMAT);
                }
                else
                {
                  sprintf (buf, "$n is entering from %s%s.", rev_d[qe->dir], (drag_af ? ", dragging $N" : travel_str));
	          act (buf, true, ch, 0, drag_af ?
		   (CHAR_DATA *) drag_af->a.spell.t : 0, TO_NOTVICT | _ACT_FORMAT);
                }
              }
		      ...
		      int *speed, int *speed_name, float *walk_time)
{
  ...
  *speed += enc_tab[i].delay;
  ...
  if (i <= 1)
    ...
    cost_modifier = cost_modifier * 2.00;
...
/*
		  ...
		  if (get_affect (tch, MAGIC_HIDDEN))
			...
			("$N emerges from hiding and moves to block your egress in that direction.",
			 true, ch, 0, tch, TO_CHAR | _ACT_FORMAT);
		      act
			("$n attempts to move past you, but you emerge from hiding and intercept $m.", true, ch, 0, tch, TO_VICT | _ACT_FORMAT);
		    }
		  else
		    {
  ...
  if (IS_MORTAL (ch)
       && (!IS_SET (flags, MF_SWIM) && SWIM_ONLY (vtor (room_exit->to_room)) 
...
do_move (CHAR_DATA * ch, char *argument, int dir)
{
  ...
  af = get_soma_affect(ch, SOMA_SPIDER_PARALYZE);
...
do_stand (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if(get_soma_affect(ch, SOMA_SPIDER_PARALYZE) && get_second_affect(ch, SA_STAND, NULL))
  {
		  ...
		  opt_room->nVirtual)
		{
		   ...
		   */ continue;
              ...
              if (!IS_SET (ch->act, ACT_FLYING) &&
                  ...
                  || room->sector_type == SECT_OCEAN || room->sector_type == SECT_REEF
...
skill_tracking (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  delay = 11 - (int) (ch->skills[SKILL_TRACKING] / 10);
...
do_mount (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if (mount->skills[SKILL_RIDE] < 90 && !skill_use (mount, SKILL_RIDE, -75))
    {
...
dump_rider (CHAR_DATA * rider, int forced)
{
  ...
  if (!skill_use (rider, SKILL_RIDE, 0))
    {
      if(forced == 2)
      {
...
watched_action(CHAR_DATA *ch, char *arg, int dist, int mode)
{
  ...
  for (dir = 0; dir <= LAST_DIR; dir++)
  {
    ...
    for (tch = next_room->people; tch; tch = tch->next_in_room)
    {
      if((waf = get_affect (tch, AFFECT_WATCH_DIR)))
      {
        if(rev_dir[dir] == waf->a.shadow.edge && CAN_SEE(tch, ch))
        {    
          target_sighted (tch, ch);
          if(!mode)
