...
do_upgrade (CHAR_DATA *ch, char *argument, int cmd)
{
  ...
  if(!*arg)
  {
    send_to_char("You need to enter an attribute to raise (#6str, int, agi, dex, con, wil or aur#0) or a talent to purchase.\n", ch);
    return;
  }

  if(ch->desc->acct->get_rpp() < 1)
  {
    send_to_char("You need at least one reward point before you can upgrade your character.\n", ch);
    return;
  }
  ...
  if(!str_cmp(arg, "str"))
  {
    ...
    {
      send_to_char("You have already chosen to upgrade your strength.\n", ch);
      return;
    }

    act("Through the trials and perils you have faced, you now feel stronger and mightier, ready to face you next adventure in Beleriand.", false, ch, 0, 0, TO_CHAR | _ACT_FORMAT); 

    ch->desc->acct->deduct_rpp (1);
    ch->str += 1;
    ch->talents |= TAL_STR1;

    sprintf(buf, "%s spent a reward point, and upgraded Strength from %d to %d.\n", 
      ch->tname, ch->pc->start_str, ch->str);

    add_message (1, "PC_Upgrades", -5, "Server", date, ch->tname, "", buf, 0);
    add_message (1, ch->tname, -2, "Server", date, "Strength Upgrade", "", buf, 0);
  }
  else if(!str_cmp(arg, "agi"))
  {
    if(IS_SET(ch->talents, TAL_AGI1))
    {
      send_to_char("You have already chosen to upgrade your agility.\n", ch);
      return;
    }

    act("Through the trials and perils you have faced, you now feel faster and more agile, ready to face you next adventure in Beleriand.", false, ch, 0, 0, TO_CHAR | _ACT_FORMAT); 

    ch->desc->acct->deduct_rpp (1);
    ch->agi += 1;
    ch->talents |= TAL_AGI1;

    sprintf(buf, "%s spent a reward point, and upgraded Agility from %d to %d.\n", 
      ch->tname, ch->pc->start_agi, ch->agi);

    add_message (1, "PC_Upgrades", -5, "Server", date, ch->tname, "", buf, 0);
    add_message (1, ch->tname, -2, "Server", date, "Agility Upgrade", "", buf, 0);
  }
  else if(!str_cmp(arg, "con"))
  {
    if(IS_SET(ch->talents, TAL_CON1))
    {
      send_to_char("You have already chosen to upgrade your constitution.\n", ch);
      return;
    }

    act("Through the trials and perils you have faced, you now feel tougher and sturdier, ready to face you next adventure in Beleriand.", false, ch, 0, 0, TO_CHAR | _ACT_FORMAT); 

    ch->desc->acct->deduct_rpp (1);
    ch->con += 1;
    ch->talents |= TAL_CON1;

    sprintf(buf, "%s spent a reward point, and upgraded Constitution from %d to %d.\n", 
      ch->tname, ch->pc->start_con, ch->con);

    add_message (1, "PC_Upgrades", -5, "Server", date, ch->tname, "", buf, 0);
    add_message (1, ch->tname, -2, "Server", date, "Constitution Upgrade", "", buf, 0);
  }
  else if(!str_cmp(arg, "wil"))
  {
    if(IS_SET(ch->talents, TAL_WIL1))
    {
      send_to_char("You have already chosen to upgrade your willpower.\n", ch);
      return;
    }

    act("Through the trials and perils you have faced, you now feel mentally stronger and more stubborn, ready to face you next adventure in Beleriand.", false, ch, 0, 0, TO_CHAR | _ACT_FORMAT); 

    ch->desc->acct->deduct_rpp (1);
    ch->wil += 1;
    ch->talents |= TAL_WIL1;

    sprintf(buf, "%s spent a reward point, and upgraded Willpower from %d to %d.\n", 
      ch->tname, ch->pc->start_wil, ch->wil);

    add_message (1, "PC_Upgrades", -5, "Server", date, ch->tname, "", buf, 0);
    add_message (1, ch->tname, -2, "Server", date, "Willpower Upgrade", "", buf, 0);
  }
  else if(!str_cmp(arg, "int"))
  {
    if(IS_SET(ch->talents, TAL_INT1))
    {
      send_to_char("You have already chosen to upgrade your intelligence.\n", ch);
      return;
    }

    act("Through the trials and perils you have faced, you now feel smarter and more perceptive, ready to face you next adventure in Beleriand.", false, ch, 0, 0, TO_CHAR | _ACT_FORMAT); 

    ch->desc->acct->deduct_rpp (1);
    ch->intel += 1;
    ch->talents |= TAL_INT1;

    sprintf(buf, "%s spent a reward point, and upgraded Intelligence from %d to %d.\n", 
      ch->tname, ch->pc->start_intel, ch->intel);

    add_message (1, "PC_Upgrades", -5, "Server", date, ch->tname, "", buf, 0);
    add_message (1, ch->tname, -2, "Server", date, "Intelligence Upgrade", "", buf, 0);
  }
  else if(!str_cmp(arg, "aur"))
  {
    if(IS_SET(ch->talents, TAL_AUR1))
    {
      send_to_char("You have already chosen to upgrade your aura.\n", ch);
      return;
    }

    act("Through the trials and perils you have faced, you now feel more at one with the world, ready to face you next adventure in Beleriand.", false, ch, 0, 0, TO_CHAR | _ACT_FORMAT); 

    ch->desc->acct->deduct_rpp (1);
    ch->aur += 1;
    ch->talents |= TAL_AUR1;

    sprintf(buf, "%s spent a reward point, and upgraded Aura from %d to %d.\n", 
      ch->tname, ch->pc->start_aur, ch->aur);

    add_message (1, "PC_Upgrades", -5, "Server", date, ch->tname, "", buf, 0);
    add_message (1, ch->tname, -2, "Server", date, "Aura Upgrade", "", buf, 0);
  }
  else if(!str_cmp(arg, "dex"))
  {
    if(IS_SET(ch->talents, TAL_DEX1))
    {
      send_to_char("You have already chosen to upgrade your dexterity.\n", ch);
      return;
    }

    act("Through the trials and perils you have faced, you now feel more dextrous and skilled with your hands, ready to face you next adventure in Beleriand.", false, ch, 0, 0, TO_CHAR | _ACT_FORMAT); 

    ch->desc->acct->deduct_rpp (1);
    ch->dex += 1;
    ch->talents |= TAL_DEX1;

    sprintf(buf, "%s spent a reward point, and upgraded Dexterity from %d to %d.\n", 
      ch->tname, ch->pc->start_dex, ch->dex);

    add_message (1, "PC_Upgrades", -5, "Server", date, ch->tname, "", buf, 0);
    add_message (1, ch->tname, -2, "Server", date, "Dexterity Upgrade", "", buf, 0);
  }
...
do_commence (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  act
    ("$n decides to take the plunge, venturing in to Beleriand for the very first time!",
...
if (IS_SET (ch->plr_flags, START_MORDOR))
	  ...
	  || ch->skills[SKILL_SPEAK_BLACK_SPEECH] < 30)
	{
      ...
      ch->was_in_room = 0;
      add_clan (ch, "hules", CLAN_MEMBER);
      if (!ch->skills[SKILL_SPEAK_SINDARIN]
	  || ch->skills[SKILL_SPEAK_SINDARIN] < 30)
	{
	  ch->skills[SKILL_SPEAK_SINDARIN] = 30;
	  ch->pc->skills[SKILL_SPEAK_SINDARIN] = 30;
	}
  ...
  act ("$n has entered Beleriand for the very first time!", true, ch, 0, 0,
       TO_ROOM | _ACT_FORMAT);
  sprintf (buf,
	   "#3[%s has entered Beleriand for the first time in %s.]#0",
	   char_short (ch), (morgul) ? "the Tur Edendor settlement" : "Hule's Outpost");
...
do_ic (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  sprintf (buf, "Weary of the OOC chatter, #5%s#0 returns to Beleriand.",
	   char_short (ch));
  act (buf, false, ch, 0, 0, TO_ROOM | _ACT_FORMAT);

  send_to_char ("Weary of the OOC chatter, you return to Beleriand.\n\n",
		ch);
...
do_hide (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  {
    send_to_char("You hide with a group this size following you.\n", ch);
    return;
  }
  ...
  for (tch = ch->room->people; tch; tch = tch->next_in_room)
  {
    if ((af = get_affect (tch, MAGIC_WATCH)))
      if ((CHAR_DATA *) af->a.spell.t == ch)
        act("$N begins to conceal $mself beneath your watchful gaze.\n", 
             false, tch, 0, ch, TO_CHAR | _ACT_FORMAT);
  }

  delay = 12 - (int) (skill_level(ch, SKILL_HIDE, 0) / 10);
  delay = MAX(3,delay);
...
delayed_hide (CHAR_DATA * ch)
{
  ...
  for (tch = ch->room->people; tch; tch = tch->next_in_room)
  {
    if ((af = get_affect (tch, MAGIC_WATCH)))         
    {
      if ((CHAR_DATA *) af->a.spell.t == ch)
      {
        act("$N vanishes from your sight.\n", false, tch, 0, ch, TO_CHAR | _ACT_FORMAT); 
        remove_affect_type (tch, MAGIC_WATCH);
      }
    }
  }
...
do_nominate (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if((time (0) - acct->get_last_nominate ()) <= 60 * 60 * 24 * 15)
  {
    send_to_char("You've already made a nomination within the last fifteen days.\n", ch);
    return;
  }
...
//Regular Skills
  ...
  switch (i)
    ...
    case SKILL_LONGBOW:
    case SKILL_THROWN:
      if ((victim->skills[SKILL_SMALL_BLADE]
	   || victim->skills[SKILL_SWORD]
	   || victim->skills[SKILL_AXE]
	   || victim->skills[SKILL_POLEARM]
	   || victim->skills[SKILL_CLUB]
	   || victim->skills[SKILL_FLAIL]
...
//Dependant skills
    ...
    case SKILL_APOTHECARY:
      if (!prereq_skill (ch, victim, SKILL_APOTHECARY, SKILL_HERBALISM, 0))
				{
    ...
    case SKILL_LARGE_WEAPON:
      if (victim->offense < 26)
	{
	  send_to_char ("Their offense is too low for them to "
			"learn to wield a large weapon.\n\r", ch);
	  return;
	}
      break;
	...
	{
	  send_to_char ("Their offense is too low for them to "
			"learn to wield a use a single weapon.\n\r", ch);
	  return;
	}
      break;

    case SKILL_SHIELD_STYLE:
      if (victim->offense < 26)
	{
	  send_to_char ("Their offense is too low for them to "
			"learn to wield a use a shield and sword.\n\r", ch);
	  return;
	}
      break;
...
//Skills that are not supported at this time
      ...
      if (!prereq_skill (ch, victim, SKILL_POISONING,
			 SKILL_APOTHECARY, SKILL_HERBALISM))
				{
					send_to_char ("They don't know enought about several things.\n\r", ch);
					return;
				}
...
//Scripts
    case SKILL_SCRIPT_SARATI:
    case SKILL_SCRIPT_TENGWAR:
    case SKILL_SCRIPT_CIRTH:
			break;

//Langauges     
    case  SKILL_SPEAK_TALISKA:
    case  SKILL_SPEAK_HALADIN:
    case  SKILL_SPEAK_THRUNON:
    case  SKILL_SPEAK_BEAST_TONGUE:
    case  SKILL_SPEAK_VALARIN:
    case  SKILL_SPEAK_NANDORIN:
    case  SKILL_SPEAK_DRUAG:
    case  SKILL_SPEAK_SINDARIN:
    case  SKILL_SPEAK_QUENYA:
    case  SKILL_SPEAK_AVARIN:
    case  SKILL_SPEAK_KHUZDUL:
    case  SKILL_SPEAK_ORKISH:
    case  SKILL_SPEAK_TROLLISH:
      break;
    }
...
do_scout (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  skill_learn(ch, SKILL_LUMBERJACK);

  if (!real_skill (ch, SKILL_LUMBERJACK))
    {
...
delayed_scout (CHAR_DATA * ch)
{
  ...
  if (skill_level (ch, SKILL_LUMBERJACK, -35))
    {
      if((zone = ch->in_room / 1000) == 10)
      {
        if(ch->delay_info1 != 0)
        {
          obj = load_colored_object (72 + ch->delay_info3, add_hash (dorthonion_trees[ch->delay_info1 - 1]), 0, 0);
        }      
        else
        {
          obj = load_colored_object (72 + ch->delay_info3, add_hash (dorthonion_trees[number(0,12)]), 0, 0);
        }
      }
      else
      {
        if(ch->delay_info1 != 0)
        {
          obj = load_colored_object (72 + ch->delay_info3, add_hash (hardwood_colors[ch->delay_info1 - 1]), 0, 0);
        }
        else
        {	    
...
do_logging (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if (!((obj = get_equip (ch, WEAR_PRIM)) || (obj = get_equip (ch, WEAR_BOTH)))
      || (obj->o.weapon.use_skill != SKILL_AXE)
...
delayed_log3 (CHAR_DATA *ch)
{
  ...
  skill_use(ch, SKILL_LUMBERJACK, 0);

  i = number(1, SKILL_CEILING);
  j = ch->skills[SKILL_LUMBERJACK];
  ...
  if(tree->nVirtual == 70)
    felled = load_colored_object (71, tree->var_color, 0, 0);
  else if(tree->nVirtual == 72)
    felled = load_colored_object (73, tree->var_color, 0, 0);
  else
  {
