...
/** group check **/
 ...
 /* Check to see if attribute check succeeded.  This really is
  ...
  // That is possible the dumbest thing I've ever read.
  // Denying utility for "theme" concerns is preposterous.
  // Desginers can use crafts for any number of devices,
  // many which don't rely on skills but instead had to have
  // skills in them for some test ,e.g. all the siegecraft stuff.
  ...
  for (i = 0; i < MAX_ITEMS_PER_SUBCRAFT; i++)
	...
	item->color = str_dup (obj_list[i]->var_color);
      ...
      if (obj_list[i] && obj_list[i]->var_color2
	  && str_cmp (obj_list[i]->var_color2, "none"))
       {
	item->color2 = str_dup (obj_list[i]->var_color2);
      }
  ...
  for (i = 0; i < MAX_ITEMS_PER_SUBCRAFT; i++)
	...
	{
	  if (color && IS_SET (vtoo (item->items[index])->obj_flags.extra_flags, ITEM_VARIABLE))
            obj_list[i] = load_colored_object (item->items[index], color, color2, color3);
	  else
	  {
...
/* Fails - We need to account for the quantity of an item */
    ...
    *color = '\0';
    ...
    if (color && IS_SET (vtoo(item->items[0])->obj_flags.extra_flags,           ITEM_VARIABLE))
        ...
        fobj_list[i] = load_colored_object (item->items[0], color, 0, 0);
      else
