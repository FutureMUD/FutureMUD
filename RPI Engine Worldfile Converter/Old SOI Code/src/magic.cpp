...
process_spell_save (CHAR_DATA * ch, void *target, int id, int target_type)
{
  ...
  if (tch != NULL && save_type != SAVE_NONE)
	...
	case SAVE_EVADE:
	  ...
	  if (tch->skills[SKILL_DODGE] > tch->skills[SKILL_AVERT]
	    ...
	    {
	      if ((save = skill_use (tch, SKILL_AVERT, penalty)) > 0)
		{
		  if (save_for == SAVE_FOR_NO_EFFECT)
		    {
  ...
  if (!*d->character->pc->msg)
	  ...
	  && d->character->pc->is_guide)
	{
	  sprintf (buf, "Greetings,\n"
		   ...
		   "Beleriand at your earliest convenience. We'll see you there!\n"
		   "\n"
...
{
  ch->skills[skill] = (180 / num) - 5 + number(0,(18 - num));
}
...
setup_new_character (CHAR_DATA * tch)
{
  ...
  open_skill (tch, SKILL_AVERT);
  starting_skill_boost (tch, SKILL_AVERT, 6);

  open_skill (tch, SKILL_BRAWLING);
  starting_skill_boost (tch, SKILL_BRAWLING, 6);
...
*/
  ...
  // What races should get infravision?
...
do_apply (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if (obj->obj_flags.type_flag != ITEM_SALVE && obj->obj_flags.type_flag != ITEM_POISON_PASTE)
    {
      act ("$p isn't a salve or a poison paste.", false, ch, obj, 0, TO_CHAR);
      return;
    }
  ...
  if(obj->obj_flags.type_flag == ITEM_POISON_PASTE)
  {
    ...
    if(obj->o.od.value[1] == SOMA_PLANT_RELAX)
    {
        wound->infection = 0;
...
do_poison (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if (target->obj_flags.type_flag != ITEM_WEAPON &&
      ...
      target->obj_flags.type_flag != ITEM_FOOD && target->obj_flags.type_flag != ITEM_MISSILE)
    {
      act ("You can only poison weapons, arrows, liquid containers, and food.",
  ...
  if(target->count > 1)
  {
    send_to_char("For the moment, you can only poison one object at a time.\n", ch);
    return;
  }
  ...
  if ((poison->obj_flags.type_flag != ITEM_POISON_PASTE 
      && poison->obj_flags.type_flag != ITEM_POISON_LIQUID) || poison->o.od.value[1] == 0)
    {
  ...
  if(poison->obj_flags.type_flag == ITEM_POISON_PASTE 
     && IS_SET(poison->obj_flags.extra_flags, ITEM_BENIGN))
  {
    act ("The paste can only be applied directly to wounds.", false, ch, 0, 0, TO_CHAR);
  }
...
do_reach (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  whisper_it (ch, argument, msg_to_send);
