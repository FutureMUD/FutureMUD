...
get_profession_desc (int prof_id)
{
  ...
  if ((result = mysql_store_result (database)) == NULL)
    {
      sprintf (buf, "Warning: get_profession_desc(): %s",
	       mysql_error (database));
      system_log (buf, true);
    }
  else
...
load_char_mysql (const char *name)
{
  ...
  int lastbled = 0, bindskill = 0, lastbound = 0, fracture = 0;
  ...
  char l_short_description[MAX_STRING_LENGTH];
  MYSQL_RES *result;
  MYSQL_ROW row;
  CHAR_DATA *ch = NULL;
  ...
  if (str_cmp (row[84], "~") && str_cmp (row[84], " "))
      ...
      while (1)
	{
	  ...
	  sscanf (buf, "%s %d %d %s %s %s %s", location, &i, &l_colored, l_var_color, l_var_color2, l_var_color3, l_short_description);
	  ...
	  lodged->short_description = str_dup (l_short_description);
  ...
  if (ch->speaks == SKILL_HEALING)
    ch->speaks = SKILL_SPEAK_SINDARIN;
...
save_char_mysql (CHAR_DATA * ch)
{
  ...
  *affects = '\0';
  ...
  if (ch->wounds)
	...
	sprintf (buf + strlen (buf),
		 ...
		 wound->bindskill, wound->lastbound, wound->fracture);
  ...
  *buf = '\0';
  for (lodged_obj = ch->lodged; lodged_obj; lodged_obj = lodged_obj->next)
    sprintf (buf + strlen (buf), "%s %d %d %s %s %s %s\n", 
              lodged_obj->location, lodged_obj->vnum, lodged_obj->colored, lodged_obj->var_color,
              lodged_obj->var_color2, lodged_obj->var_color3, lodged_obj->short_description);
  ...
  if (result && mysql_num_rows (result) >= 1)
	...
	("UPDATE %s.pfiles SET keywords = '%s', account = '%s', sdesc = '%s', ldesc = '%s', description = '%s', msg = '%s', create_comment = '%s', create_state = %d, "
	 ...
	 "wounds = '%s', lodged = '%s', writes = %d, profession = %d, was_in_room = %d, travelstr = '%s', bmi = %d, guardian_mode = %d, hire_storeroom = %d, hire_storeobj = %d, plan = '%s', goal = '%s', role_id = %d, power_level = %d, talents = %d WHERE name = '%s'",
	...
	("INSERT INTO %s.pfiles (name, keywords, account, sdesc, ldesc, description, msg, create_comment, create_state, "
	 ...
	 "wounds, lodged, writes, profession, was_in_room, travelstr, bmi, hire_storeroom, hire_storeobj, plan, goal, role_id, power_level, talents) VALUES "
	 "('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, "
