...
nanny_check_password (DESCRIPTOR_DATA * d, char *argument)
{
  ...
  if (!strstr (d->strClientHostname, "lost-tales.org"))
    {
      ...
      if ((result = mysql_store_result (database)) != NULL)
	{
	  ...
	  if (!IS_SET (d->acct->flags, ACCOUNT_IPSHARER) && nSharedIP > 0
	      ...
	      && d->acct->last_ip.find ("lost-tales.org") == std::string::npos)
	    {
...
get_weapon_skills (CHAR_DATA * ch, int *melee, int *ranged)
{
  ...
  for (i = SKILL_SMALL_BLADE; i <= SKILL_FLAIL; i++)
    {
      if (real_skill (ch, i))
	j++;
    }
  ...
  for (i = SKILL_THROWN; i <= SKILL_LONGBOW; i++)
    {
      if (real_skill (ch, i))
	j++;
    }
...
nanny_choose_pc (DESCRIPTOR_DATA * d, char *argument)
{
  ...
  if (!str_cmp (d->character->room->name, PREGAME_ROOM_NAME))
    {
      send_to_char ("\n", d->character);
      act
	("Welcome to Beleriand: The First Age! If you are a new player and would like to speak one-on-one with an experienced player guide before beginning play, please type #6ASSIST REQUEST#0, and then #6HELP ASSIST#0. Otherwise, look at the sign by the door (#6LOOK SIGN#0) for further instructions on how to begin play.",
  ...
  */
  ...
  if ((d->character->race >= 0 && d->character->race <= 11)
      && (d->character->max_hit !=
	  40 + d->character->con * CONSTITUTION_MULTIPLIER))
    {
  ...
  if (d->character->race == 12 || d->character->race == 13)
    {
     d->character->max_hit = 200 + d->character->con * CONSTITUTION_MULTIPLIER;
...
*/
  ...
  if (is_newbie (ch))
    SEND_TO_Q
      ("\nCommands:  Age, Attributes, Comment, Description, Frame, Height, Location\n"
...
pickable_skill (CHAR_DATA * ch, const char *buf)
{
  ...
  if (is_restricted_skill (ch, ind, 0) == 0)
    return ind;
  ...
  if (!picks_left && !ch->skills[skill])
	  ...
	  if (skill == SKILL_LITERACY)
	    {
	      ...
	      ch->skills[SKILL_SCRIPT_CIRTH] = 0;
	    }
	  if (is_restricted_skill (ch, SKILL_DUAL, 0))
	    {
      ...
      else
	{
	  ...
	  if ((skill == native_tongue)
	      || ((skill == SKILL_SPEAK_SINDARIN)
		  && (IS_SET (ch->plr_flags, START_GONDOR) || start_loc == 1))
	      || ((skill == SKILL_SPEAK_ORKISH)
  ...
  *buf = '\0';
  ...
  for (i = 1; i <= LAST_SKILL && *skills[i] != '\n'; i++)
	...
	continue;
      ...
      if ((i == native_tongue)
	  || ((i == SKILL_SPEAK_SINDARIN)
	      && (IS_SET (d->character->plr_flags, START_GONDOR)
		  || start_loc == 1)) || ((i == SKILL_SPEAK_ORKISH)
					  &&
					  (IS_SET
...
*/
	...
	("You will receive an email at the address registered for this\n"
	 ...
	 "in advance for your patience, and for your interest in Lost Tales\n"
	 "of Beleriand! We'll see you soon, in Beleriand.\n", d);
      d->character->time.birth = time (0);
      d->character->time.played = 0;
      d->character->time.logon = time (0);
      display_main_menu (d);
