...
const struct constant_data constant_info[] = {
  ...
  {"design", "Random Object Colors     ", (void **) design_type_colors},
  {"hardwood", "Random Object Colors     ", (void **) hardwood_colors},
  {"animal", "Random Object Colors     ", (void **) animal_colors},
  {"orcolor", "Random Object Colors     ", (void **) orc_colors},
  {"blade", "Random Object Colors     ", (void **) blade_type_colors},
  {"metdam", "Random Object Colors     ", (void **) metal_damage_colors},
...
fwrite_mobile (CHAR_DATA * tmob, FILE * fp)
{
  ...
  fprintf (fp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n", tmob->mob->skinned_vnum, tmob->circle, tmob->cell_1, tmob->mob->carcass_vnum, tmob->cell_2,	/* Formerly tmob->defense_bonus */
	   ...
	   tmob->height,
	   tmob->frame, tmob->mob->noaccess_flags, tmob->cell_3, tmob->room_pos, tmob->mob->fallback);
  ...
  // For mobiles that are venomous!
    ...
    if (tmob->venom)
    {
        fprintf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d\n", 
                tmob->venom->poison_type, tmob->venom->duration, tmob->venom->latency,
                tmob->venom->minute, tmob->venom->max_power, tmob->venom->lvl_power,
                tmob->venom->atm_power, tmob->venom->attack, tmob->venom->decay, 
                tmob->venom->sustain, tmob->venom->release, tmob->venom->uses);
    }
  ...
  */
  ...
  for (poison = tobj->poison; poison; poison = poison->next)
        fprintf(fp, "P\n%d %d %d %d %d %d %d %d %d %d %d %d\n", 
                tobj->poison->poison_type, tobj->poison->duration, tobj->poison->latency,
                tobj->poison->minute, tobj->poison->max_power, tobj->poison->lvl_power,
                tobj->poison->atm_power, tobj->poison->attack, tobj->poison->decay, 
                tobj->poison->sustain, tobj->poison->release, tobj->poison->uses);
...
do_olist (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if (!*buf)
	...
	("   <item-type>               Include items of item-type.\n", ch);
      send_to_char
	("   *econ               Include items with this econ flag.\n", ch);
      send_to_char ("\nExample:   olist +sword -rusty weapon 10 *beor ^fine\n", ch);
      send_to_char
	("will only get non-rusty swords of type weapon from zone 10 that are econ-flagged beor.\n", ch);
      return;
    }
...
do_minit (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  newmob->speaks = SKILL_SPEAK_SINDARIN;
  newmob->skills[SKILL_SPEAK_SINDARIN] = 100;
  newmob->max_hit = 80;
  newmob->hit = 80;
  newmob->max_move = 80;
  newmob->move = 80;
...
redefine_objects (OBJ_DATA * proto)
{
  ...
  for (obj = object_list; obj; obj = obj->next)
	...
	obj->obj_flags.extra_flags = proto->obj_flags.extra_flags;
      ...
      if (IS_SET (obj->obj_flags.extra_flags, ITEM_VARIABLE))
	insert_string_variables (obj, proto, 0, 0, 0);
      else
      {
	obj->var_color = 0;
        obj->var_color2 = 0;
	...
	obj->o.od.value[1] = proto->o.od.value[1];
      ...
      obj->creater = proto->creater;
      obj->editer = proto->editer;

      //obj->room_pos = proto->room_pos; // don't copy the position, or peril will follow.
...
redefine_mobiles (CHAR_DATA * proto)
{
  ...
  for (mob = character_list; mob; mob = mob->next)
	...
	continue;
      ...
      mob->mob->fallback = proto->mob->fallback;
...
do_oset (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  while (*subcmd)
	...
	{
	  send_to_char
	    ("This object is variable, and you must change it on the Build Port.\n",
	     ch);
	  return;
	}
      ...
      else if (!strn_cmp (subcmd, "affects", strlen (subcmd)))
	{
      ...
      else if (!str_cmp (subcmd, "xpoison"))
      {
        argument = one_argument (argument, buf);
        if (!*buf)
        {
	  ...
	  return;
        ...
        ind = atoi(buf);
        if (ind < MAGIC_FIRST_SOMA || ind > MAGIC_LAST_SOMA)
        {
	...
	if (!str_cmp (buf, "remove"))
	{
	  if (edit_obj->poison)
	  {
	    for (poison = edit_obj->poison; poison; poison = poison->next)
	    { 
	      if (poison->poison_type == ind)
	        remove_object_poison(edit_obj, poison);
	    }
          }
	  ...
	  return;
        ...
        {
	  sscanf (argument, "%d %d %d %d %d %d %d %d %d %d", &duration, &latency, &minute,
                  &max_power, &lvl_power, &atm_power, &attack, &decay, &sustain, &release);
	  ...
	  poison->poison_type = ind;
	  poison->duration = duration;
	  poison->latency = latency;
	  poison->minute = minute;
	  poison->max_power = max_power;
	  poison->lvl_power = lvl_power;
	  poison->atm_power = atm_power;
	  poison->attack = attack;
	  poison->decay = decay;
	  poison->sustain = sustain;
	  poison->release = release;
          poison->uses = -1;
	  poison->next = NULL;

          poison_to_obj(edit_obj, poison);
	...
	{
	  if (GET_ITEM_TYPE (edit_obj) == ITEM_WEAPON
	      && (edit_obj->o.od.value[3] == SKILL_LONGBOW || edit_obj->o.od.value[3] == SKILL_SHORTBOW))
	  {
	    send_to_char ("This oval cannot be edited.\n", ch);
	    return;
	  }
      ...
      else if (!str_cmp (subcmd, "econ"))
	{
	  argument = one_argument (argument, buf);
          ...
          if(GET_ITEM_TYPE(edit_obj) == ITEM_RESOURCE)
          {
	    if ((ind = index_lookup (resources, buf)) == -1)
	    {
	      send_to_char ("I could not find that resource flag.\n", ch);
	      return;
	    }
          }
          else
...
do_mset (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  while (*subcmd)
      ...
      else if (!str_cmp (subcmd, "venom"))
	{
	  ...
	  if (!str_cmp (buf, "remove"))
	  ...
	  else
	    {
	      ...
	      if ((duration < 0 || latency < 0 || minute < 0 || max_power < lvl_power || lvl_power < 0 || atm_power < 0 || attack < 1 || decay < attack || sustain < decay || release < sustain) && str_cmp (buf, "remove"))
		{
		  send_to_char ("You'll need to enter the duration, latency, minute, max power, level power, current power, attack, decay, sustain, release and uses values to get a poison to work.\n", ch);
		  return;
		}

              if(!IS_SET (edit_mob->act, ACT_VENOM))
              {
                edit_mob->act |= ACT_VENOM;
              }
	      ...
	      if (!edit_mob->venom)
		{
		  ...
		  edit_mob->venom->duration = duration;
		  edit_mob->venom->latency = latency;
		  edit_mob->venom->minute = minute;
		  edit_mob->venom->max_power = max_power;
		  edit_mob->venom->lvl_power = lvl_power;
		  edit_mob->venom->atm_power = atm_power;
		  edit_mob->venom->attack = attack;
		  edit_mob->venom->decay = decay;
		  edit_mob->venom->sustain = sustain;
		  edit_mob->venom->release = release;
                  edit_mob->venom->uses = -1;
		...
		for (poison = edit_mob->venom; poison; poison = poison->next)
		  {
			...
			poison->next->duration = duration;
			poison->next->latency = latency;
			poison->next->minute = minute;
			poison->next->max_power = max_power;
			poison->next->lvl_power = lvl_power;
			poison->next->atm_power = atm_power;
			poison->next->attack = attack;
			poison->next->decay = decay;
			poison->next->sustain = sustain;
			poison->next->release = release;
                        poison->next->uses = -1;
			poison->next = NULL;
			break;
		      }
      ...
      else if (!str_cmp (subcmd, "currency"))
	{
	  ...
	  *buf = tolower (*buf);
	  if (!str_cmp (buf, "orkish") || !str_cmp (buf, "blacks"))
	    {
	      edit_mob->mob->currency_type = CURRENCY_ORKISH;
	      send_to_char
		("That mobile will now deal in Orkish currency (Blacks).\n",
		 ch);
	    }
	  else if (!str_cmp (buf, "noldo") || !str_cmp (buf, "crowns"))
	    {
	      edit_mob->mob->currency_type = CURRENCY_NOLDO;
	      send_to_char
		("That mobile will now deal in Noldo currency (Crowns).\n",
		 ch);
	    }

	  else if (!str_cmp (buf, "sinda") || !str_cmp (buf, "silvers"))
	    {
	      edit_mob->mob->currency_type = CURRENCY_SINDA;
	      send_to_char
		("That mobile will now deal in Sinda currency (Silvers).\n",
		 ch);
	    }

	  else if (!str_cmp (buf, "dwarvish") || !str_cmp (buf, "pieces"))
	    {
	      edit_mob->mob->currency_type = CURRENCY_SINDA;
	      send_to_char
		("That mobile will now deal in Dwarvish currency (Pieces).\n",
		 ch);
	    }
	    ...
	    {
	      send_to_char
		("Specify Orkish, Sinda, Noldo or Dwarvish currency.\n", ch);
	      return;
	    }
	}
 ...
 /** morphing mobs **/
	...
	if(IS_SET(edit_mob->act, ACT_BULKTRADER))
        {
          if((ind = index_lookup (resources, buf)) == -1)
          {
	    send_to_char ("No such resource flag.\n", ch);
	    break;
          }
        }
        else 
      ...
      else if (!str_cmp (subcmd, "noskills"))
	{
	  for (ind = 1; ind <= LAST_SKILL; ind++)
		...
		{
		  if (ind == SKILL_SPEAK_SINDARIN)
		    {
		      send_to_char ("NOTE:  Leaving Sindarin set.\n", ch);
		      continue;
		    }
		...
		send_to_char ("No such room exists\n", ch);
	        ...
	        edit_mob->mob->fallback = atoi (buf);
                sprintf(buf2, "Fallback room  set as #6[%d] %s#0.\n", 
                  vtor(atoi(buf))->nVirtual, vtor(atoi(buf))->name);
                send_to_char (buf2, ch);
                break;
              }
	    }
      ...
      else
	{
	  if ((ind = index_lookup (action_bits, buf)) != -1 &&
	      (1 << ind == ACT_AGGRESSIVE ||
	       1 << ind == ACT_SENTINEL ||
	       1 << ind == ACT_WIMPY || 1 << ind == ACT_MEMORY))
	    {
	      TOGGLE (mob->act, 1 << ind);
	    }
	  ...
	  else
	    {
	      send_to_char ("Expected a standing order of:\n\n"
			    "Aggressive, Fallback, Memory, Sentinel, or Wimpy.\n", ch);
	      return;
	    }
	}
...
do_rxerox (CHAR_DATA * ch, char *argument, int cmd)
{
  ...
  if(source_room->nVirtual == target_room->nVirtual)
  {
    ...
    sprintf(buf2, "Room %d will now emulate Room %d, '%s'.\n", target_room->nVirtual, source_room->nVirtual, source_room->name);
    send_to_char(buf2, ch);
}
