...
r_load_clone (CHAR_DATA * ch, char *argument)
{
		...
		||(obj = get_obj_in_dark (holder, arg2, holder->left_hand)))
	{
		if (IS_SET (obj->obj_flags.extra_flags, ITEM_VARIABLE))
			clone = load_colored_object (obj->nVirtual, obj->var_color, obj->var_color2, obj->var_color3);
		else
...
void
r_skilluse (CHAR_DATA * ch, char *argument)
{
  char buf[AVG_STRING_LENGTH];
  argument = one_argument (argument, buf);
  int ind;
...
void
r_owner (CHAR_DATA * ch, char *argument)
{
...
r_load_obj (CHAR_DATA *ch, char *argument)
{
	...
	if (!color.empty())
	{
		if (!exit && !(obj = load_colored_object (ovnum, (char *) color.c_str(), NULL, NULL)))
		{
	...
	if ( !exit)
	{
		if ( IS_SET (obj->obj_flags.extra_flags, ITEM_VARIABLE))
		{
			...
			{
				if (!color.empty())
					obj = load_colored_object(ovnum, (char *) color.c_str(), NULL, NULL);
				else
				{
...
r_info (CHAR_DATA *ch, std::string argument, room_prog_var *& variable_list)
{
	...
	else if (buf.find("room") != std::string::npos)
	{
		...
		else if (buf.find("obj") != std::string::npos)
		{
		  ...
		  if (atoi(buf.c_str()) > 0 && buf.find(".") == std::string::npos)
		  {
