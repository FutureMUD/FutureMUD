...
fwrite_a_obj (OBJ_DATA * obj, FILE * fp)
{
  ...
  if (obj->var_color2 && str_cmp (obj->var_color2, "none"))
    modifiers++;

  if (obj->var_color3 && str_cmp (obj->var_color3, "none"))
    modifiers++;
  ...
  fprintf (fp, "Id       %d %d %d %d\n", obj->nVirtual, affect_count, poison_count, modifiers);
  ...
  if (obj->var_color && str_cmp (obj->var_color, "none"))
    fprintf (fp, "VarColor1  %s~\n", obj->var_color);

  if (obj->var_color2 && str_cmp (obj->var_color2, "none"))
    fprintf (fp, "VarColor2  %s~\n", obj->var_color2);

  if (obj->var_color3 && str_cmp (obj->var_color3, "none"))
    fprintf (fp, "VarColor3  %s~\n", obj->var_color3);
  ...
  {		
    if (poison->poison_type)
    {
        fprintf(fp, "Poison %d %d %d %d %d %d %d %d %d %d %d %d\n", 
...
fread_obj (FILE * fp)
{
  ...
  while ((poison = obj->poison))
    {
      obj->poison = poison->next;
      mem_free (poison);
    }
  ...
  for (; modifiers; modifiers--)
	...
	obj->var_color2 = unspace (fread_string (fp));
      else if (!strcmp (p, "VarColor3"))
	obj->var_color3 = unspace (fread_string (fp));
  ...
  for (i = 0; obj && i < poison_count; i++)
  {
    if (strcmp (fread_word (fp), "Poison"))
    {
      return (NULL);
    }
    ...
    &poison->poison_type, &poison->duration, &poison->latency, &poison->minute,
    &poison->max_power, &poison->lvl_power, &poison->atm_power, &poison->attack,
    &poison->decay, &poison->sustain, &poison->release, &poison->uses);
