...
is_restricted_skill (CHAR_DATA * ch, int skill, int prof)
{
  ...
  // As nearly every case of dual-wielding will be with a small_blade, go off that.

  if (skill == SKILL_DUAL)
  {
    if(ch->skills[SKILL_SMALL_BLADE])
      return 0;
    else
      return 1;
  }
  ...
  if (skill == SKILL_POISONING && ch->race != 11)
  	{
  	if (ch->skills[SKILL_HERBALISM])
  		return 0;
  	else
  		return 1;
  	}

// orcs can learn trollish, and poisoning.

  if (ch->race == 11)
    {
      if (skill == SKILL_SPEAK_TROLLISH)
	return 0;
      if (skill == SKILL_SPEAK_BEAST_TONGUE)
  	{
  	if (ch->skills[SKILL_SPEAK_TROLLISH])
  		return 0;
  	else
  		return 1;
  	}
...
/* werewolves and wolfspawn will need to have a lot of restrictions upon what they can learn

  if (ch->race >= 19 && ch->race <= 20)
    {
	
    }
*/

/* likewise will trolls
...
*/
  ...
  if (restricted_skills[skill] == -4)
	...
	return 0;
      if (ch->race == 2 || ch->race == 4)
	{			// Beorian and Marachian
	  if (skill == SKILL_SPEAK_HALADIN)
	    return 0;
	...
	{			// Noldo
	  ...
	  if (skill == SKILL_SPEAK_TALISKA)
	    return 0;
	  if (skill == SKILL_SPEAK_AVARIN)
	    return 0;
	  if (skill == SKILL_SPEAK_NANDORIN)
	    return 0;
	}
      else if (ch->race == 8)
	{			// Sinda
	...
	{			// Avari
      ...
      else if (ch->race >= 19 && ch->race <= 20)
	{			// Werewolves and Wolfspawn can speak, and get Sindarin for free.
	  if (skill == SKILL_SPEAK_QUENYA)
	    return 0;
	  if (skill == SKILL_SPEAK_HALADIN)
	    return 0;
  ...
  // to know anything interesting.
	...
	return 0;
      ...
      if (!real_skill(ch, SKILL_SCRIPT_TENGWAR) && (ch->race < 7 || ch->race > 10))
        return 1;
	    ...
	    char *message)
{
  ...
  fprintf (fp,
	   "\n\n\n\n\n--\nLost Tales of Beleriand: http://www.lost-tales.org\n");
...
pc_to_game (CHAR_DATA * ch)
{
  ...
  if (!ch->writes)
    {
      if (real_skill (ch, SKILL_SCRIPT_TENGWAR))
	ch->writes = SKILL_SCRIPT_TENGWAR;
      else if (real_skill (ch, SKILL_SCRIPT_SARATI))
	ch->writes = SKILL_SCRIPT_SARATI;
      else if (real_skill (ch, SKILL_SCRIPT_CIRTH))
	ch->writes = SKILL_SCRIPT_CIRTH;
      else if (real_skill (ch, SKILL_SCRIPT_VALARIN_SCRIPT))
	ch->writes = SKILL_SCRIPT_VALARIN_SCRIPT;
    }
}
...
obj_short_desc (OBJ_DATA * obj)
{
  ...
  *buf = '\0';
  ...
  if (GET_ITEM_TYPE (obj) == ITEM_MONEY)
	...
	{
	  if (obj->count == 1)
	    strcat (coins, " faceted, ridged gold coin");
	  else
	    strcat (coins, " faceted, ridged gold coins");
	}

      else if (obj->nVirtual == 994)
	{
	  if (obj->count == 1)
	    strcat (coins, " eight-sided gold coin");
	  else
	    strcat (coins, " eight-sided gold coins");
	}
      else if (obj->nVirtual == 993)
	{
	  if (obj->count == 1)
	    strcat (coins, " hexagonal silver coin");
	  else
	    strcat (coins, " hexagonal silver coins");
	}
      else if (obj->nVirtual == 992)
	{
	  if (obj->count == 1)
	    strcat (coins, " small, squared silver coin");
	  else
	    strcat (coins, " small, squared silver coins");
	}

      else if (obj->nVirtual == 991)
	{
	  if (obj->count == 1)
	    strcat (coins, " weighty copper coin");
	  else
	    strcat (coins, " weighty copper coins");
	}

      else if (obj->nVirtual == 990)
	{
	  if (obj->count == 1)
	    strcat (coins, " tiny copper coin");
	  else
	    strcat (coins, " tiny copper coins");
	}
      return coins;
    }
  ...
  // Bows need to display what level of stringing they're at.
    ...
    (obj->o.od.value[3] == SKILL_LONGBOW || obj->o.od.value[3] == SKILL_SHORTBOW))
    {
...
{
  // Beor, Marach, Haladin, Druedain, Easterling.

  if (ch->race >= 2 || ch->race <= 6)
    return 1;
