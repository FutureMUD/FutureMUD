	       ...
	       int bleeding, int poison, int infection)
{
  ...
  if (ch->wounds)
    ...
    {
      // Added in a simple hack so you can't be killed from blows to the hands or feet.
      // The wounds still count for damage totals for everything else, you will just
      // never receive a killing blow (or a knock-out blow) because you cut someone's
      // finger off.
  ...
  if (str_cmp (location, "bloodloss"))
    ...
    else if(type == 3 || type == 9)
    {
      ...
      if (!str_cmp (severity, "moderate") && (number(1,25) >= GET_CON(ch)))
      {
	    ...
	    mem_free (p);
          ...
          // arms or hand 
	        ...
	        add_second_affect (SA_GET_OBJ, (10-(GET_DEX(ch)/5)+number(2,4)), ch, tobj, "", 0);
	        add_second_affect (SA_WEAR_OBJ, (13-(GET_DEX(ch)/5)+number(2,5)), ch, tobj, "",0);
      ...
      else if ((!str_cmp (severity, "severe") || !str_cmp (severity, "grievous") || !str_cmp (severity, "terrible") || !str_cmp (severity, "mortal")) && (number(1,25) + (impact/2)) >= GET_CON(ch))
      {
        if(loc != -1)
        {
...
**/
  ...
  roll = number (1, (SKILL_CEILING-15));
  if (roll <= skill_level (ch, SKILL_HEALING, 0) - 15)
  {
    ...
    treat_effect =  2;
  else if (roll <= skill_level (ch, SKILL_HEALING, 0) + 15)
    treat_effect =  1;
  else
    treat_effect = 0;
  ...
  for (wound = tch->wounds; wound; wound = next_wound)
  {
    ...
    if (!str_cmp (wound->location, location)
	 ...
	 && ((str_cmp (wound->severity, "minor") && str_cmp (wound->severity, "small")) 
         || wound->infection || wound->fracture))
    {
/***
...
figure_location (CHAR_DATA * tar, int location)
{
  ...
  switch (tar->body_proto)
      ...
      if (location == 0 || location == 3)
	{
      ...
      else if (location == 1 || location == 2 || location == 6 || location == 5)
	sprintf (loc, "tail");
      else if (location == 4)
	{
	  sprintf (loc, "neck");
      ...
      if (location == 0)
	{
      ...
      else if (location == 1 || location == 6 || location == 2 || location == 5)
	{
      ...
      else if (location == 2)
	{
	  ...
	  else if (locroll > 30 && locroll <= 45)
	    sprintf (loc, "rupperarm");
	  else if (locroll > 45 && locroll <= 60)
	    sprintf (loc, "lupperarm");
	  else if (locroll > 60 && locroll <= 65)
	    sprintf (loc, "relbow");
	  else if (locroll > 65 && locroll <= 70)
	    sprintf (loc, "lelbow");
	  else if (locroll > 70 && locroll <= 85)
...
char__do_bind (CHAR_DATA * thisPtr, char *argument, int cmd)
{
  ...
  for (tch = thisPtr->room->people; tch; tch = tch->next_in_room)
  {
    if (tch == thisPtr)
      continue;
  
    if(IS_SET(tch->flags, FLAG_BINDING) && tch->delay_type == DEL_BIND_WOUNDS)
    {
      if(tch->delay_ch && tch->delay_ch == pTargetActor)
      {
        act("$N's wounds are already being tended.", 
...
delayed_bind (CHAR_DATA * thisPtr)
{
  ...
  for (pWound = pTargetActor->wounds; pWound; pWound = pWound->next)
  {
    if (pWound->bleeding)
    {
	  ...
	  // bonus for having both helpful skills
    	  ...
    	  thisPtr->skills[SKILL_EMPATHIC_HEAL]) ? 
          (((100 - pWound->bindskill) * MIN (thisPtr->skills[SKILL_HEALING],
          thisPtr->skills[SKILL_EMPATHIC_HEAL])) /100) : 1;
  ...
  if (pTargetActor != thisPtr)
  {
    ...
    {
      act("You finish binding your wounds, and have managed to stem the bleeding.",
	 false, thisPtr, 0, 0, TO_CHAR | _ACT_FORMAT);
      act("$n finishes binding $s wounds, and has managed to stem the bleeding.",
	 false, thisPtr, 0, 0, TO_ROOM | _ACT_FORMAT);
    }
