...
namespace RPI_Engine_Worldfile_Converter
{
    ...
    internal record RPIItem
    {
        ...
        public RPIItem(string definition)
        {
            ...
            Vnum = int.Parse(lines[0][1..]);
            Name = new StringStack(lines[1]).Pop();
            Material = RPIMaterial.Other;
            foreach (string? value in new StringStack(lines[1]).PopAll())
            {
                ...
                if (keyword.All(x => char.IsUpper(x)) && Enum.TryParse<RPIMaterial>(keyword, true, out RPIMaterial result))
                {
                    Material = result;
                    break;
                }
            ...
            string oval4line = lines[3];
            StringStack oval4SS = new(oval4line);
            ...
            lines = lines.Skip(1).ToArray();
            List<(RPISkill Skill, int Value)> affected = new();
            while (lines.Any())
            {
                if (lines[0] == "C")
                {
                ...
                {
                    line = new StringStack(lines[1]);
                    RPISkill skill = (RPISkill)(int.Parse(line.Pop()) - 10000);
                    int impact = int.Parse(line.Pop());
