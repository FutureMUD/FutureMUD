...
internal static class GNHProgram
{
	...
	{
		Console.Error.WriteLine("Usage: GatheringNativePersistenceHarness --probe|--schema|--run|--reader <scenario arguments>");
		return 2;
	}
	...
	private sealed class TestDatabase : IDisposable
	{
		...
		{
			if (!name.StartsWith("futuremud_gather_gc_", StringComparison.Ordinal))
			{
		...
		public static TestDatabase CreateFresh()
		{
			MySqlConnectionStringBuilder builder = ServerConnectionBuilder();
		...
		public void Dispose()
		{
			...
			if (!Name.StartsWith("futuremud_gather_gc_", StringComparison.Ordinal) || string.IsNullOrWhiteSpace(_ownershipToken))
			{
		...
		{
			return $"futuremud_gather_gc_{DateTime.UtcNow:yyyyMMddHHmmss}_{Convert.ToHexString(RandomNumberGenerator.GetBytes(5)).ToLowerInvariant()}";
		}
