...
The correction probes add C-P01 for saved pending pasture allocations at factors 0.5 and 0, followed by native establishment, owner checkpoint, independent database read and reconstruction. The pending state is seeded as versioned field XML rather than inserted through the field constructor. C-P02 checks a retained orchard harvest near capacity with separate prepaid and positive-recovery fractions across an owner checkpoint and later reconstructed tick. The combined database-backed coordinator C-P03 probe is not part of this harness; the real coordinator's dynamic validation is exercised in `EnvironmentalMagicCoordinatorTests`.
