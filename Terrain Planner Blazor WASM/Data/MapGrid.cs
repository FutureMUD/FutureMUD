...
#nullable enable
    public class MapGrid : INotifyPropertyChanged
    {
        ...
        {
            string[] terrains = importText.Split(',');
        ...
        {
            List<MapCell> oldCells = MapCells.ToList();
            MapCells.Clear();
...

            {
                for (int x = 0; x < _width; x++)
                {
                    MapCell? old = reuseOldValues ? oldCells.FirstOrDefault(item => item.X == x && item.Y == y) : default;
