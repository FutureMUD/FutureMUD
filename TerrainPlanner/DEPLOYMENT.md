...
Open an elevated PowerShell prompt in the extracted package and run:

```powershell
.\deploy\windows\Install-TerrainPlanner.ps1 -Hostname planner.example.com
```

On the first run it creates `%ProgramFiles%\FutureMUD\TerrainPlanner\shared\appsettings.Production.json` and stops. Replace `REPLACE_WITH_SECRET`, then rerun the command. The installer creates the `FutureMUDTerrainPlanner` Windows Service under its dedicated virtual service account, restricts the durable configuration/key directory ACL, creates a stable `current` junction, and provides health-check rollback, release retention, and validated Caddy configuration with backup/restore.
...
The updater downloads `update-manifest.json`, its Ed25519 signature, and the named archive from futuremud.com. The bundled verifier checks the signature, product/version/runtime identity, size, and SHA-256 before the normal health-checked installer activates it.
